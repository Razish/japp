...
int WP_GetVelocityForForceJump( gentity_t *self, vector3 *jumpVel, usercmd_t *ucmd )
{
	...
	if (self->client->ps.velocity.z < -30)
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			{//FIXME: make enemy jedi able to hunker down and resist this?
				...
				if (push_list[x]->client)
				{
					...
					if ((int)push_list[x]->client->ps.velocity.z == 0)
			...
			{
				if ( japp_allowFlagPull.integer && push_list[x]->s.eType == ET_ITEM && (!strcmp( push_list[x]->classname, "team_CTF_redflag" ) || !strcmp( push_list[x]->classname, "team_CTF_blueflag" )) )
				{//Flags
					vector3 end = { 0, 0, 0 };
