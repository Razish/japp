...
void AI_SortGroupByPathCostToEnemy(AIGroupInfo_t *group) {
    ...
    if (group->enemy != NULL) { // FIXME: just use enemy->waypoint?
    ...
    for (i = 0; i < group->numGroup; i++) {
        if (group->enemyWP == WAYPOINT_NONE) { // FIXME: just use member->waypoint?
            group->member[i].waypoint = WAYPOINT_NONE;
            group->member[i].pathCostToEnemy = Q3_INFINITE;
        } else { // FIXME: just use member->waypoint?
    ...
    if (sort) {
        ...
        for (i = 0; i < group->numGroup; i++) {
            for (j = 0; j < group->numGroup; j++) {
                if (bestMembers[j].number != ENTITYNUM_NONE) { // slot occupied
                    if (group->member[i].pathCostToEnemy <
                        bestMembers[j].pathCostToEnemy) { // this guy has a shorter path than the one currenly in this spot, bump him and put myself in here
                ...
                } else { // slot unoccupied, reached end of list, throw self in here
                    memcpy(&bestMembers[j], &group->member[i], sizeof(bestMembers[j]));
                    break;
                }
            }
...
qboolean AI_FindSelfInPreviousGroup(gentity_t *self) { // go through other groups made this frame and see if any of those contain me already
    int i, j;
...
void AI_InsertGroupMember(AIGroupInfo_t *group, gentity_t *member) {
    ...
    for (i = 0; i < group->numGroup; i++) {
        if (group->member[i].number == member->s.number) { // already in here
            break;
        }
    }
    if (i < group->numGroup) { // found him in group already
    } else {                   // add him in
        group->member[group->numGroup++].number = member->s.number;
        group->numState[member->NPC->squadState]++;
    }
    if (!group->commander || (member->NPC->rank > group->commander->NPC->rank)) { // keep track of highest rank
...
qboolean
AI_TryJoinPreviousGroup(gentity_t *self) { // go through other groups made this frame and see if any of those have the same enemy as me... if so, add me in!
    int i;
    for (i = 0; i < MAX_FRAME_GROUPS; i++) {
...
qboolean AI_GetNextEmptyGroup(gentity_t *self) {
    ...
    if (AI_FindSelfInPreviousGroup(self)) { // already in one, no need to make a new one
        return qfalse;
    }

    if (AI_TryJoinPreviousGroup(self)) { // try to just put us in one that already exists
        return qfalse;
    }

    // okay, make a whole new one, then
    for (i = 0; i < MAX_FRAME_GROUPS; i++) {
        if (!level.groups[i].numGroup) { // make a new one
            self->NPC->group = &level.groups[i];
            return qtrue;
        }
    }
...
qboolean AI_ValidateNoEnemyGroupMember(AIGroupInfo_t *group, gentity_t *member) {
    ...
    } else { // hmm, just pick the first member
    ...
    if (!trap->InPVS(&member->r.currentOrigin, &center)) { // not within PVS of the group enemy
        return qfalse;
    }
    return qtrue;
}
...
qboolean AI_ValidateGroupMember(AIGroupInfo_t *group, gentity_t *member) {
    ...
    if (member->NPC->group != NULL && member->NPC->group != group) { // FIXME: if that group's enemy is mine, why not absorb that group into mine?
        return qfalse;
    }
    ...
    if ( member->client->ps.weapon == WP_SABER ||//!= self->s.weapon )
		...
		member->client->ps.weapon == WP_TIE_FIGHTER*/ ) {//not really a squad-type guy
        return qfalse;
    }

    if (member->client->NPC_class == CLASS_ATST || member->client->NPC_class == CLASS_PROBE || member->client->NPC_class == CLASS_SEEKER ||
        member->client->NPC_class == CLASS_REMOTE || member->client->NPC_class == CLASS_SENTRY || member->client->NPC_class == CLASS_INTERROGATOR ||
        member->client->NPC_class == CLASS_MINEMONSTER || member->client->NPC_class == CLASS_HOWLER || member->client->NPC_class == CLASS_MARK1 ||
        member->client->NPC_class == CLASS_MARK2) { // these kinds of enemies don't actually use this group AI
        return qfalse;
    }

    // should have same enemy
    if (member->enemy != group->enemy) {
        if (member->enemy != NULL) { // he's fighting someone else, leave him out
            return qfalse;
        }
        if (!trap->InPVS(&member->r.currentOrigin, &group->enemy->r.currentOrigin)) { // not within PVS of the group enemy
            return qfalse;
        }
    } else if (group->enemy == NULL) { // if the group is a patrol group, only take those within the room and radius
...
void AI_GetGroup(gentity_t *self) {
    ...
    if (!AI_GetNextEmptyGroup(self)) { // either no more groups left or we're already in a group built earlier
        return;
    }
    ...
    for (i = 0; i < level.num_entities; i++) {
        ...
        if (!AI_ValidateGroupMember(
                self->NPC->group, member)) { // FIXME: keep track of those who aren't angry yet and see if we should wake them after we assemble the core group
            continue;
        }
        ...
        if (self->NPC->group->numGroup >= (MAX_GROUP_MEMBERS - 1)) { // full
            break;
        }
    }
    ...
    */
    ...
    if (self->NPC->group->numGroup <= 0) { // none in group
        self->NPC->group = NULL;
        return;
    }
...
void AI_SetNewGroupCommander(AIGroupInfo_t *group) {
    ...
    for (i = 0; i < group->numGroup; i++) {
        ...
        if (!group->commander ||
            (member && member->NPC && group->commander->NPC && member->NPC->rank > group->commander->NPC->rank)) { // keep track of highest rank
            group->commander = member;
        }
    }
}
...
void AI_GroupMemberKilled(gentity_t *self) {
    ...
    if (!self || !self->NPC || self->NPC->rank < RANK_ENSIGN) { // I'm not an officer, let's not really care for now
        return;
    }
    ...
    for (i = 0; i < group->numGroup; i++) {
        ...
        if (member->NPC->rank > RANK_ENSIGN) { // officers do not panic
            noflee = qtrue;
        } else {
    ...
    if (group->commander != self) { // I'm not the commander... hmm, should maybe a couple flee... maybe those near me?
        return;
    }
    ...
    if (!noflee) {
        ...
        for (i = 0; i < group->numGroup; i++) {
            ...
            if (member->NPC->rank < RANK_ENSIGN) { // grunt
                if (group->enemy &&
                    DistanceSquared(&member->r.currentOrigin, &group->enemy->r.currentOrigin) < 65536 /*256*256*/) { // those close to enemy run away!
                    ST_StartFlee(member, group->enemy, &member->r.currentOrigin, AEL_DANGER_GREAT, 3000, 5000);
                } else if (DistanceSquared(&member->r.currentOrigin, &self->r.currentOrigin) < 65536 /*256*256*/) { // those close to me run away!
                    ST_StartFlee(member, group->enemy, &member->r.currentOrigin, AEL_DANGER_GREAT, 3000, 5000);
                } else {                                                   // else, maybe just a random chance
                    if (Q_irand(0, self->NPC->rank) > member->NPC->rank) { // lower rank they are, higher rank I am, more likely they are to flee
                        ST_StartFlee(member, group->enemy, &member->r.currentOrigin, AEL_DANGER_GREAT, 3000, 5000);
                    } else {
...
qboolean AI_RefreshGroup(AIGroupInfo_t *group) {
    ...
    for (i = 0; i < MAX_FRAME_GROUPS; i++) {
        if (&level.groups[i] == group) {
            ...
            if (level.groups[i].enemy == group->enemy) {                                    // 2 groups with same enemy
                if (level.groups[i].numGroup + group->numGroup < (MAX_GROUP_MEMBERS - 1)) { // combining the members would fit in one group
                    ...
                    for (j = 0; j < group->numGroup; j++) {
                        ...
                        if (level.groups[i].enemy == NULL) { // special case for groups without enemies, must be in range
    ...
    */
    ...
    for (i = 0; i < group->numGroup; i++) {
        member = &g_entities[group->member[i].number];
        if (member->NPC->rank < RANK_ENSIGN) { // grunts
            group->morale++;
        } else {
    ...
    if (group->enemy) { // modify morale based on enemy health and weapon
    ...
    if (group->moraleDebounce < level.time) { // slowly degrade whatever moraleAdjusters we may have
