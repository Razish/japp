...
void NPC_SetMoveGoal(gentity_t *ent, vector3 *point, int radius, qboolean isNavGoal, int combatPoint, gentity_t *targetEnt) {
    ...
    if (ent->NPC->tempGoal == NULL) { // must still have a goal
        return;
    }
...
qboolean NAV_HitNavGoal(vector3 *point, vector3 *mins, vector3 *maxs, vector3 *dest, int radius, qboolean flying) {
    ...
    if (radius & NAVGOAL_USE_RADIUS) {
        ...
        if (!flying) { // Allow for a little z difference
            vector3 diff;
...
qboolean NAV_ClearPathToPoint(gentity_t *self, vector3 *pmins, vector3 *pmaxs, vector3 *point, int clipmask, int okToHitEntNum) {
    ...
    if (self->flags & FL_NAVGOAL) {
        ...
        if (trace.startsolid && (trace.contents & CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
            clipmask &= ~CONTENTS_BOTCLIP;
            trap->Trace(&trace, point, &mins, &maxs, &self->r.currentOrigin, self->parent->s.number, (clipmask | CONTENTS_MONSTERCLIP) & ~CONTENTS_BODY, qfalse,
                        0, 0);
        }
        ...
        if (NAV_HitNavGoal(&self->r.currentOrigin, &self->parent->r.mins, &self->parent->r.maxs, &trace.endpos, NPCInfo->goalRadius,
        ...
        if (((trace.startsolid == qfalse) && (trace.allsolid == qfalse)) && (trace.fraction == 1.0f)) { // FIXME: check for drops
            return qtrue;
        }
...
qboolean NAV_CheckAhead(gentity_t *self, vector3 *end, trace_t *trace, int clipmask) {
    ...
    radius = (self->r.maxs.x > self->r.maxs.y) ? self->r.maxs.x : self->r.maxs.y;
...
qboolean NAV_Bypass(gentity_t *self, gentity_t *blocker, vector3 *blocked_dir, float blocked_dist, vector3 *movedir) {
    ...
    if (blocker->client && !VectorCompare(&blocker->client->ps.velocity, &vec3_origin)) {
        ...
        if (dot < 0.35f && dot > -0.35f) { // he's moving to the side of me
            vector3 block_pos;
            trace_t tr;
...
qboolean NAV_MoveBlocker(gentity_t *self, vector3 *shove_dir) {
    ...
    self->client->ps.velocity.z += SHOVE_LIFT;
...
qboolean NAV_TrueCollision(gentity_t *self, gentity_t *blocker, vector3 *movedir, vector3 *blocked_dir) {
    ...
    float speed, dot;
    vector3 testPos;
    vector3 ptmins, ptmaxs, tmins, tmaxs;
...
qboolean NAV_StackedCanyon(gentity_t *self, gentity_t *blocker, vector3 *pathDir) {
    ...
    if (tr.startsolid && (tr.contents & CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
...
qboolean NAV_AvoidCollision(gentity_t *self, gentity_t *goal, navInfo_t *info) {
    ...
    if (self && self->NPC && (self->NPC->aiFlags & NPCAI_NO_COLL_AVOID)) { // pretend there's no-one in the way
        return qtrue;
    }
...
int NAV_TestBestNode(gentity_t *self, int startID, int endID, qboolean failEdge) { // check only against architectrure
    ...
    // Do a simple check
    if ((trace.allsolid == qfalse) && (trace.startsolid == qfalse) && (trace.fraction == 1.0f)) { // it's clear
        return endID;
    }
    ...
    if (self->s.weapon != WP_SABER && fabsf(self->r.currentOrigin.z - end.z) > 48) {
        ...
        // This is a work around
        float radius = (self->r.maxs.x > self->r.maxs.y) ? self->r.maxs.x : self->r.maxs.y;
        float dist = Distance(&self->r.currentOrigin, &end);
        float tFrac = 1.0f - (radius / dist);

        if (trace.fraction >= tFrac) { // it's clear
            return endID;
        }
    }
    ...
    if (trace.entityNum < ENTITYNUM_WORLD) {
        ...
        if VALIDSTRING (blocker->classname) { // special case: doors are architecture, but are dynamic, like entitites
            if (G_EntIsUnlockedDoor(blocker->s.number))
            // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
            { // it's unlocked, go for it
                ...
                if (self->s.weapon != WP_SABER && fabsf(self->r.currentOrigin.z - end.z) > 48) { // too far above
                } else {
                    return endID;
                }
            } else if (G_EntIsDoor(blocker->s.number)) { // a locked door!
            ...
            } else if (G_EntIsBreakable(blocker->s.number)) { // do same for breakable brushes/models/glass?
                // path is blocked by a breakable, mark it as such if instructed to do so
                if (failEdge) {
                    trap->Nav_AddFailedEdge(self->s.number, startID, endID);
                }
            } else if (G_EntIsRemovableUsable(blocker->s.number)) { // and removable usables
                // path is blocked by a removable usable, mark it as such if instructed to do so
                if (failEdge) {
                    trap->Nav_AddFailedEdge(self->s.number, startID, endID);
                }
            } else if (blocker->targetname && blocker->s.solid == SOLID_BMODEL &&
                       ((blocker->r.contents & CONTENTS_MONSTERCLIP) ||
                        (blocker->r.contents & CONTENTS_BOTCLIP))) { // some other kind of do not enter entity brush that will probably be removed
...
SOLID_OK - only use if placing inside solid is unavoidable in map, but may be clear in-game (ie: at the bottom of a tall, solid lift that starts at the top
position)
...
void SP_waypoint(gentity_t *ent) {
    if (navCalculatePaths) {
        ...
        if (!(ent->spawnflags & 1) && G_CheckInSolid(ent, qtrue)) { // if not SOLID_OK, and in solid
            ent->r.maxs.z = CROUCH_MAXS_2;
...
void Svcmd_Nav_f(void) {
    ...
    if (Q_stricmp(cmd, "show") == 0) {
        ...
        if (Q_stricmp(cmd, "all") == 0) {
            ...
            NAVDEBUG_showCollision = NAVDEBUG_showNavGoals = NAVDEBUG_showCombatPoints = NAVDEBUG_showEnemyPath = NAVDEBUG_showEdges = NAVDEBUG_showRadius =
                NAVDEBUG_showNodes;
...
qboolean navCalculatePaths = qtrue;

qboolean NAVDEBUG_showNodes = qtrue;
qboolean NAVDEBUG_showRadius = qtrue;
qboolean NAVDEBUG_showEdges = qtrue;
qboolean NAVDEBUG_showTestPath = qtrue;
qboolean NAVDEBUG_showEnemyPath = qtrue;
qboolean NAVDEBUG_showCombatPoints = qtrue;
qboolean NAVDEBUG_showNavGoals = qtrue;
...
qboolean NAV_WaypointsTooFar(gentity_t *wp1, gentity_t *wp2) {
    if (Distance(&wp1->r.currentOrigin, &wp2->r.currentOrigin) > 1024) {
        ...
        } else { // they both have valid targetnames
...
void NAV_ClearStoredWaypoints(void) { numStoredWaypoints = 0; }
...
void NAV_Shutdown(void) { trap->Nav_Free(); }
