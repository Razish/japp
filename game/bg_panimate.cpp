...
qboolean BG_InSpecialJump(int anim) {
    ...
    case BOTH_A7_SOULCAL:
        return qtrue;
    default:
        return (BG_InReboundJump(anim) || BG_InReboundHold(anim) || BG_InReboundRelease(anim) || BG_InBackFlip(anim));
    }
}
...
qboolean BG_SaberInAttackPure(int move) { return (move >= LS_A_TL2BR && move <= LS_A_T2B); }
...
qboolean PM_SaberInDeflect(int move) { return (move >= LS_D1_BR && move <= LS_D1_B_); }

qboolean PM_SaberInParry(int move) { return (move >= LS_PARRY_UP && move <= LS_PARRY_LL); }

qboolean PM_SaberInKnockaway(int move) { return (move >= LS_K1_T_ && move <= LS_K1_BL); }

qboolean PM_SaberInReflect(int move) { return (move >= LS_REFLECT_UP && move <= LS_REFLECT_LL); }

qboolean PM_SaberInStart(int move) { return (move >= LS_S_TL2BR && move <= LS_S_T2B); }

qboolean PM_SaberInReturn(int move) { return (move >= LS_R_TL2BR && move <= LS_R_T2B); }

qboolean BG_SaberInReturn(int move) { return PM_SaberInReturn(move); }

qboolean PM_InSaberAnim(int anim) { return (anim >= BOTH_A1_T__B_ && anim <= BOTH_H1_S1_BR); }
...
qboolean PM_SpinningAnim(int anim) { return BG_SpinningSaberAnim(anim); }
...
qboolean PM_CanRollFromSoulCal(playerState_t *ps) { return (ps->legsAnim == BOTH_A7_SOULCAL && ps->legsTimer < 700 && ps->legsTimer > 250); }
...
void PM_DebugLegsAnim(int anim) {
    ...
    if (oldAnim < MAX_TOTALANIMATIONS && oldAnim >= BOTH_DEATH1 && newAnim < MAX_TOTALANIMATIONS && newAnim >= BOTH_DEATH1) {
...
void BG_ClearAnimsets(void) {
    ...
    {
    trap->TrueFree((void **)&bgAllAnims[i].anims);
    }
    i++;
    }
    */
...
stringID_table_t footstepTypeTable[NUM_FOOTSTEP_TYPES + 1] = {ENUM2STRING(FOOTSTEP_R),
...
int CheckAnimFrameForEventType(animevent_t *animEvents, int keyFrame, animEventType_e eventType) {
    ...
    for (i = 0; i < MAX_ANIM_EVENTS; i++) {
        if (animEvents[i].keyFrame == keyFrame) {       // there is an animevent on this frame already
...
void ParseAnimationEvtBlock(const char *aeb_filename, animevent_t *animEvents, animation_t *animations, int *i, const char **text_p) {
    ...
    while (1) {
        ...
        if (animNum == -1) { // Unrecognized ANIM ENUM name, or we're skipping this line, keep going till you get a good one
        ...
        case AEV_SOUND: //# animID AEV_SOUND framenum soundpath randomlow randomhi chancetoplay
            ...
            if (lowestVal && highestVal) {
                ...
                for (n = lowestVal, num = AED_SOUNDINDEX_START; n <= highestVal && num <= AED_SOUNDINDEX_END; n++, num++) {
                    if (stringData[0] == '*') { // FIXME? Would be nice to make custom sounds work with animEvents.
...
#ifdef _DEBUG
                if (!animEvents[curAnimEvent].eventData[AED_SOUNDINDEX_START] && stringData[0] != '*') { // couldn't register it - file not found
            ...
            if (!Q_stricmpn("sound/weapons/saber/saberhup", stringData, 28)) { // a saber swing
                animEvents[curAnimEvent].eventType = AEV_SABER_SWING;
                animEvents[curAnimEvent].eventData[AED_SABER_SWING_SABERNUM] =
                    0; // since we don't know which one they meant if we're hacking this, always use first saber
            ...
            } else if (!Q_stricmpn("sound/weapons/saber/saberspin", stringData, 29)) { // a saber spin
        ...
        case AEV_EFFECT: //# animID AEV_EFFECT framenum effectpath boltName
            ...
            if (Q_stricmp("none", token) != 0 && Q_stricmp("NULL", token) != 0) { // actually are specifying a bolt to use
                if (!animEvents[curAnimEvent].stringData) {                       // eh, whatever. no dynamic stuff, so this will do.
...
int BG_ParseAnimationEvtFile(const char *as_filename, int animFileIndex, int eventFileIndex) {
    ...
    // Raz: moved to heap, so prepare it each call
    text[0] = '\0';
    ...
    if (bg_animParseIncluding <= 0) {                // if we should be parsing an included file, skip this part
    ...
    if (bg_animParseIncluding <= 0) { // if we should be parsing an included file, skip this part
        ...
        while (i < bgNumAnimEvents && forcedIndex != 0) {
            if (!Q_stricmp(as_filename, bgAllEvents[i].filename)) { // looks like we have it already.
                return i;
            }
            i++;
        }
    ...
    if (bg_animParseIncluding <= 0) { // should already be done if we're including
        ...
        for (i = 0; i < MAX_ANIM_EVENTS; i++) {
            ...
            // we allow storage of one string, temporarily (in case we have to look up an index later, then make sure to set stringData to NULL so we only do
            // the look-up once)
...
int BG_ParseAnimationFile(const char *filename, animation_t *animset, qboolean isHumanoid) {
    ...
    if (!isHumanoid) {
        ...
        if (!animset) {
            if (strstr(filename, "players/_humanoid/")) { // then use the static humanoid set.
                ...
                nextIndex = 0;
            } else if (strstr(filename, "players/rockettrooper/")) { // rockettrooper always index 1
                nextIndex = 1;
    ...
    */
    ...
    if (isHumanoid) {
        ...
        if (nextIndex > 1) { // don't bother increasing the number if this ended up as a humanoid/rockettrooper load.
            bgNumAllAnims++;
        } else {
            BGPAFtextLoaded = qtrue;
            usedIndex = nextIndex;
...
void PM_ContinueLegsAnim(int anim) {
    ...
    // dont override perm animations with this function. This is nessisary for cutscene animations to run properly.
...
void PM_ForceLegsAnim(int anim) {
    if (BG_InSpecialJump(pm->ps->legsAnim) && pm->ps->legsTimer > 0 && !BG_InSpecialJump(anim)) {
        return;
    }

    if (BG_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->legsTimer > 0 && !BG_InRoll(pm->ps, anim)) {
        return;
    }
...
void BG_SetLegsAnimTimer(playerState_t *ps, int time) {
    ...
    if (ps->legsTimer < 0 &&
        time != -1) { // Cap timer to 0 if was counting down, but let it be -1 if that was intentional.  NOTENOTE Yeah this seems dumb, but it mirrors SP.
        ps->legsTimer = 0;
    }
}
...
void BG_SaberStartTransAnim(int clientNum, int saberAnimLevel, int weapon, int anim, float *animSpeed, int broken) {
    ...
    if (((anim) >= BOTH_T1_BR__R && (anim) <= BOTH_T1_BL_TL) || ((anim) >= BOTH_T2_BR__R && (anim) <= BOTH_T2_BL_TL) ||
...
void BG_SetAnimFinal(playerState_t *ps, animation_t *animations, int setAnimParts, int anim, uint32_t setAnimFlags,
                     int blendTime) // default blendTime=350
{
    ...
    if (setAnimParts & SETANIM_TORSO) {
        ...
        if (setAnimFlags & SETANIM_FLAG_HOLD) {
            if (setAnimFlags & SETANIM_FLAG_HOLDLESS) { // Make sure to only wait in full 1/20 sec server frame intervals.
                int dur;
                int speedDif;
...
void BG_SetAnim(playerState_t *ps, animation_t *animations, int setAnimParts, int anim, uint32_t setAnimFlags, int blendTime) {
    ...
    if (animations[anim].firstFrame == 0 && animations[anim].numFrames == 0) {
        ...
        if (animations[anim].firstFrame == 0 && animations[anim].numFrames == 0) { // still? Just return then I guess.
            return;
        }
    }
...
float BG_GetTorsoAnimPoint(playerState_t *ps, int AnimIndex) {
    ...
    if (animSpeedFactor > 0) {
        attackAnimLength = (bgAllAnims[AnimIndex].anims[ps->torsoAnim].numFrames - 1) * fabsf((float)(bgAllAnims[AnimIndex].anims[ps->torsoAnim].frameLerp)) *
                           (1 / animSpeedFactor);
        attackAnimLength--;
    }
...
float BG_GetLegsAnimPoint(playerState_t *ps, int AnimIndex) {
    ...
    if (animSpeedFactor > 0) {
        attackAnimLength = (bgAllAnims[AnimIndex].anims[ps->legsAnim].numFrames - 1) * fabsf((float)(bgAllAnims[AnimIndex].anims[ps->legsAnim].frameLerp)) *
                           (1 / animSpeedFactor);
        attackAnimLength--;
    }
