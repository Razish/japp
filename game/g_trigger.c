...
void multi_trigger( gentity_t *ent, gentity_t *activator ) {
	...
	if ( level.gametype == GT_SIEGE &&
		!gSiegeRoundBegun ) { //nothing can be used til the round starts.
		return;
	}
	...
	if ( level.gametype == GT_SIEGE &&
		ent->idealclass && ent->idealclass[0] ) { //only certain classes can activate it
		if ( !activator ||
			!activator->client ||
			activator->client->siegeClass < 0 ) { //no class
			return;
		}
		...
		if ( activator && activator->client &&
			...
			if ( objItem && objItem->inuse ) {
				if ( objItem->goaltarget && objItem->goaltarget[0] &&
					...
					if ( objItem->genericValue7 != activator->client->sess.sessionTeam ) { //The carrier of the item is not on the team which disallows objective scoring for it
	...
	if ( ent->genericValue2 ) { //has "teambalance" property
		...
		if ( !activator->client ||
			(activator->client->sess.sessionTeam != SIEGETEAM_TEAM1 && activator->client->sess.sessionTeam != SIEGETEAM_TEAM2) ) { //activator must be a valid client to begin with
			return;
		}
	...
	if ( ent->nextthink > level.time ) {
		...
		{
			if ( ent->painDebounceTime && ent->painDebounceTime != level.time ) {//this should still allow subsequent ents to fire this trigger in the current frame
...
void Touch_Multi( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( self->spawnflags & 4 ) {//USE_BUTTON
		...
		if ( self->genericValue7 ) { //we have to be holding the use key in this trigger for x milliseconds before firing
			if ( level.gametype == GT_SIEGE &&
				self->idealclass && self->idealclass[0] ) { //only certain classes can activate it
				if ( !other ||
					!other->client ||
					other->client->siegeClass < 0 ) { //no class
					return;
				}
	...
	if ( self->radius ) {
		...
		if ( G_PointInBounds( &eyeSpot, &self->r.absmin, &self->r.absmax ) ) {
			if ( !(other->client->pers.cmd.buttons & BUTTON_ATTACK) &&
				!(other->client->pers.cmd.buttons & BUTTON_ALT_ATTACK) ) {//not attacking, so hiding bonus
				/*
...
void trigger_push_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	*/
	...
	if ( !other->client ) {
		if ( other->s.pos.trType != TR_STATIONARY && other->s.pos.trType != TR_LINEAR_STOP && other->s.pos.trType != TR_NONLINEAR_STOP && VectorLengthSquared( &other->s.pos.trDelta ) ) {//already moving
...
void hurt_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( level.gametype == GT_SIEGE && self->team && self->team[0] ) {
		...
		if ( other->inuse && other->s.number < MAX_CLIENTS && other->client &&
			other->client->sess.sessionTeam != team ) { //real client don't hurt
			return;
		}
		else if ( other->inuse && other->client && other->s.eType == ET_NPC &&
			other->s.NPC_class == CLASS_VEHICLE && other->s.teamowner != team ) { //vehicle owned by team don't hurt
			return;
		}
	}
...
void space_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( !other->client->inSpaceIndex ||
		other->client->inSpaceIndex == ENTITYNUM_NONE ) { //freshly entering space
...
void shipboundary_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( !other || !other->inuse || !other->client ||
		other->s.number < MAX_CLIENTS ||
		!other->m_pVehicle ) { //only let vehicles touch
		return;
	}
	...
	if ( !other->client->ps.m_iVehicleNum || other->m_pVehicle->m_iRemovedSurfaces ) { //if a vehicle touches a boundary without a pilot in it or with parts missing, just blow the thing up
