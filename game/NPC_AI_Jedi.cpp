...
#define CAUTIOUS_THRESHOLD (REALIZE_THRESHOLD * 0.3f)
...
void WP_ResistForcePush(gentity_t *self, gentity_t *pusher, qboolean noPenalty) {
    ...
    if (((self->s.number >= 0 && self->s.number < MAX_CLIENTS) || self->client->NPC_class == CLASS_DESANN || !Q_stricmp("Yoda", self->NPC_type) ||
         self->client->NPC_class == CLASS_LUKE) &&
        (VectorLengthSquared(&self->client->ps.velocity) > 10000 || self->client->ps.fd.forcePowerLevel[FP_PUSH] >= FORCE_LEVEL_3 ||
         self->client->ps.fd.forcePowerLevel[FP_PULL] >= FORCE_LEVEL_3)) {
        runningResist = qtrue;
    }
    if (!runningResist && self->client->ps.groundEntityNum != ENTITYNUM_NONE && !BG_SpinningSaberAnim(self->client->ps.legsAnim) &&
        !BG_FlippingAnim(self->client->ps.legsAnim) && !PM_RollingAnim(self->client->ps.legsAnim) && !PM_InKnockDown(&self->client->ps) &&
        !BG_CrouchAnim(self->client->ps.legsAnim)) { // if on a surface and not in a spin or flip, play full body resist
        parts = SETANIM_BOTH;
...
qboolean Boba_StopKnockdown(gentity_t *self, gentity_t *pusher, vector3 *pushDir, qboolean forceKnockdown) // forceKnockdown = qfalse
{
    vector3 pDir, fwd, right, ang;
    float fDot, rDot;
    int strafeTime;
    ...
    if (Q_irand(0, 2)) { // flip or roll with it
        usercmd_t tempCmd;
        if (fDot >= 0.4f) {
    ...
    } else if (!Q_irand(0, 1) && forceKnockdown) { // resist
...
void Boba_FlyStart(gentity_t *self) { // switch to seeker AI for a while
...
void Boba_FireFlameThrower(gentity_t *self) {
    ...
    vector3 start, end, dir, traceMins = {-4, -4, -4}, traceMaxs = {4, 4, 4};

    trap->G2API_GetBoltMatrix(self->ghoul2, 0, self->client->renderInfo.handLBolt, &boltMatrix, &self->r.currentAngles, &self->r.currentOrigin, level.time,
...
void Boba_StartFlameThrower(gentity_t *self) {
    ...
    */
    ...
    trap->G2API_GetBoltMatrix(NPC->ghoul2, 0, NPC->client->renderInfo.handRBolt, &boltMatrix, &NPC->r.currentAngles, &NPC->r.currentOrigin, level.time, NULL,
                              &NPC->modelScale);
...
void Boba_FireDecide(void) {
    ...
    if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE && (int)NPC->client->ps.fd.forceJumpZStart && !BG_FlippingAnim(NPC->client->ps.legsAnim) &&
        !Q_irand(0, 10)) { // take off
        Boba_FlyStart(NPC);
    }
    ...
    if (dot > 0.5f || (enemyDist * (1.0f - dot)) < 10000) { // enemy is in front of me or they're very close and not behind me
        enemyInFOV = qtrue;
    }

    if ((enemyDist < (128 * 128) && enemyInFOV) || !TIMER_Done(NPC, "flameTime")) { // flamethrower
    ...
    {                                               // enemy within 128
        if ((NPC->client->ps.weapon == WP_FLECHETTE || NPC->client->ps.weapon == WP_REPEATER) &&
            (NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // shooting an explosive, but enemy too close, switch to primary fire
            NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
    ...
    if (TIMER_Done(NPC, "nextAttackDelay") && TIMER_Done(NPC, "flameTime")) {
        if (NPC_ClearLOS4(NPC->enemy)) {
            ...
            } else {              // can we shoot our target?
                if ((NPC->client->ps.weapon == WP_ROCKET_LAUNCHER || (NPC->client->ps.weapon == WP_FLECHETTE && (NPCInfo->scriptFlags & SCF_ALT_FIRE))) &&
                    enemyDist < MIN_ROCKET_DIST_SQUARED) // 128*128
                {
                    enemyCS = qfalse; // not true, but should stop us from firing
                    hitAlly = qtrue;  // us!
                    // FIXME: if too close, run away!
                } else if (enemyInFOV) { // if enemy is FOV, go ahead and check for shooting
                    ...
                    if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) ||
                        (hitEnt && hitEnt->takedamage &&
                         ((hitEnt->r.svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 ||
                          NPC->s.weapon == WP_EMPLACED_GUN))) { // can hit enemy or enemy ally or will hit glass or other minor breakable (or in emplaced gun),
                                                                // so shoot anyway
                        enemyCS = qtrue;
                        // NPC_AimAdjust( 2 );//adjust aim better longer we have clear shot at enemy
                        VectorCopy(&NPC->enemy->r.currentOrigin, &NPCInfo->enemyLastSeenLocation);
                    } else { // Hmm, have to get around this bastard
                        // NPC_AimAdjust( 1 );//adjust aim better longer we can see enemy
                        if (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->playerTeam) { // would hit an ally, don't fire!!!
                            hitAlly = qtrue;
                        } else { // Check and see where our shot *would* hit... if it's not close to the enemy (within 256?), then don't fire
                        }
                    }
                } else {
                    enemyCS = qfalse; // not true, but should stop us from firing
                ...
                && NPCInfo->enemyLastSeenTime > 0) // we've seen the enemy
            {
                if (level.time - NPCInfo->enemyLastSeenTime < 10000) // we have seem the enemy in the last 10 seconds
                {
                    if (!Q_irand(0, 10)) {
                        ...
                        if (VectorCompare(&impactPos, &vec3_origin)) { // never checked ShotEntity this frame, so must do a trace...
                            trace_t tr;
                            // vector3	mins = {-2,-2,-2}, maxs = {2,2,2};
                            vector3 forward, end;
                            AngleVectors(&NPC->client->ps.viewangles, &forward, NULL, NULL);
                            VectorMA(&muzzle, 8192, &forward, &end);
                            trap->Trace(&tr, &muzzle, &vec3_origin, &vec3_origin, &end, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
                        ...
                        if (dist < distThreshold) { // impact would be too close to me
                            tooClose = qtrue;
                        } else if (level.time - NPCInfo->enemyLastSeenTime > 5000 ||
                                   (NPCInfo->group && level.time - NPCInfo->group->lastSeenEnemyTime > 5000)) { // we've haven't seen them in the last 5 seconds
                            ...
                            if (dist > distThreshold) { // impact would be too far from enemy
                                tooFar = qtrue;
                            }
                        }

                        if (!tooClose && !tooFar) { // okay too shoot at last pos
        ...
        if (NPC->client->ps.weaponTime > 0) {
            if (NPC->s.weapon == WP_ROCKET_LAUNCHER) {
                if (!enemyLOS || !enemyCS) { // cancel it
                    NPC->client->ps.weaponTime = 0;
                } else { // delay our next attempt
                    TIMER_Set(NPC, "nextAttackDelay", Q_irand(500, 1000));
                }
            }
        } else if (shoot) { // try to shoot if it's time
            if (TIMER_Done(NPC, "nextAttackDelay")) {
                ...
                // NASTY
                if (NPC->s.weapon == WP_ROCKET_LAUNCHER && (ucmd.buttons & BUTTON_ATTACK) && !Q_irand(0, 3)) { // every now and then, shoot a homing rocket
...
void Jedi_Cloak(gentity_t *self) {
    if (self) {
        ...
        if (self->client) {
            if (!self->client->ps.powerups[PW_CLOAKED]) { // cloak
...
void Jedi_Decloak(gentity_t *self) {
    if (self) {
        self->flags &= ~FL_NOTARGET;
        if (self->client) {
            if (self->client->ps.powerups[PW_CLOAKED]) { // Uncloak
...
void Jedi_CheckCloak(void) {
    if (NPC && NPC->client && NPC->client->NPC_class == CLASS_SHADOWTROOPER) {
        if (!NPC->client->ps.saberHolstered || NPC->health <= 0 || NPC->client->ps.saberInFlight ||
            //	(NPC->client->ps.eFlags&EF_FORCE_GRIPPED) ||
            //	(NPC->client->ps.eFlags&EF_FORCE_DRAINED) ||
            NPC->painDebounceTime > level.time) { // can't be cloaked if saber is on, or dead or saber in flight or taking pain or being gripped
            Jedi_Decloak(NPC);
        } else if (NPC->health > 0 &&
                   !NPC->client->ps.saberInFlight
                   //	&& !(NPC->client->ps.eFlags&EF_FORCE_GRIPPED)
                   //	&& !(NPC->client->ps.eFlags&EF_FORCE_DRAINED)
                   && NPC->painDebounceTime < level.time) { // still alive, have saber in hand, not taking pain and not being gripped
            Jedi_Cloak(NPC);
        }
    }
}
...
static void Jedi_Aggression(gentity_t *self, int change) {
    ...
    if (self->client->playerTeam == NPCTEAM_PLAYER) { // good guys are less aggressive
        upper_threshold = 7;
        lower_threshold = 1;
    } else { // bad guys are more aggressive
...
static void Jedi_AggressionErosion(int amt) {
    if (TIMER_Done(NPC, "roamTime")) { // the longer we're not alerted and have no enemy, the more our aggression goes down
        TIMER_Set(NPC, "roamTime", Q_irand(2000, 5000));
        Jedi_Aggression(NPC, amt);
    }

    if (NPCInfo->stats.aggression < 4 || (NPCInfo->stats.aggression < 6 && NPC->client->NPC_class == CLASS_DESANN)) { // turn off the saber
...
void Jedi_RageStop(gentity_t *self) {
    if (self->NPC) { // calm down and back off
...
static qboolean Jedi_BattleTaunt(void) {
    if (TIMER_Done(NPC, "chatter") && !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
        jediSpeechDebounceTime[NPC->client->playerTeam] < level.time) {
        int event = -1;
        if (NPC->client->playerTeam == NPCTEAM_PLAYER && NPC->enemy && NPC->enemy->client &&
            NPC->enemy->client->NPC_class == CLASS_JEDI) {                                 // a jedi fighting a jedi - training
            if (NPC->client->NPC_class == CLASS_JEDI && NPCInfo->rank == RANK_COMMANDER) { // only trainer taunts
                event = EV_TAUNT1;
            }
        } else { // reborn or a jedi fighting an enemy
        ...
        if (event != -1) {
            ...
            if (NPC->enemy && NPC->enemy->NPC && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client &&
                NPC->enemy->client->NPC_class == CLASS_JEDI) { // Have the enemy jedi say something in response when I'm done?
            }
            return qtrue;
        }
    }
...
static qboolean Jedi_ClearPathToSpot(vector3 *dest, int impactEntNum) {
    ...
    VectorSet(&mins, NPC->r.mins.x, NPC->r.mins.y, NPC->r.mins.z + STEPSIZE);

    trap->Trace(&trace, &NPC->r.currentOrigin, &mins, &NPC->r.maxs, dest, NPC->s.number, NPC->clipmask, qfalse, 0, 0);

    // Do a simple check
    if (trace.allsolid || trace.startsolid) { // inside solid
        return qfalse;
    }

    if (trace.fraction < 1.0f) {                                                 // hit something
        if (impactEntNum != ENTITYNUM_NONE && trace.entityNum == impactEntNum) { // hit what we're going after
            return qtrue;
        } else {
            return qfalse;
        }
    ...
    if (dest->z > NPC->r.currentOrigin.z) { // going up, check for steps
        drop = STEPSIZE;
    } else { // going down or level, check for moderate drops
        drop = 64;
    }
    for (i = NPC->r.maxs.x * 2; i < dist;
         i += NPC->r.maxs.x * 2) { // FIXME: does this check the last spot, too?  We're assuming that should be okay since the enemy is there?
        VectorMA(&NPC->r.currentOrigin, i, &dir, &start);
        VectorCopy(&start, &end);
        end.z -= drop;
        trap->Trace(&trace, &start, &mins, &NPC->r.maxs, &end, NPC->s.number, NPC->clipmask, qfalse, 0, 0); // NPC->r.mins?
        if (trace.fraction < 1.0f || trace.allsolid || trace.startsolid) {                                  // good to go
            continue;
        }
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
    vector3 forward, right, testPos, angles, mins;
    trace_t trace;
    float fwdDist, rtDist;
    float bottom_max = -STEPSIZE * 4 - 1;

    if (!forwardmove && !rightmove) { // not even moving
        // Com_Printf( "%d skipping walk-cliff check (not moving)\n", level.time );
        return qtrue;
    }

    if (ucmd.upmove > 0 || (int)NPC->client->ps.fd.forceJumpCharge) { // Going to jump
        // Com_Printf( "%d skipping walk-cliff check (going to jump)\n", level.time );
        return qtrue;
    }

    if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // in the air
        // Com_Printf( "%d skipping walk-cliff check (in air)\n", level.time );
        return qtrue;
    }
    /*
    if ( fabsf( AngleDelta( NPC->r.currentAngles.yaw, NPCInfo->desiredYaw ) ) < 5.0f )//!ucmd.angles.yaw )
    ...
    */
    ...
    trap->Trace(&trace, &NPC->r.currentOrigin, &mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask | CONTENTS_BOTCLIP, qfalse, 0, 0);
    if (trace.allsolid || trace.startsolid) { // hmm, trace started inside this brush... how do we decide if we should continue?
    ...
    if (trace.fraction < 0.6f) { // Going to bump into something very close, don't move, just turn
        if ((NPC->enemy && trace.entityNum == NPC->enemy->s.number) ||
            (NPCInfo->goalEntity && trace.entityNum == NPCInfo->goalEntity->s.number)) { // okay to bump into enemy or goal
            // Com_Printf( "%d bump into enemy/goal okay\n", level.time );
            return qtrue;
        } else if (reset) { // actually want to screw with the ucmd
    ...
    if (NPCInfo->goalEntity) {
        if (NPCInfo->goalEntity->r.currentOrigin.z < NPC->r.currentOrigin.z) { // goal is below me, okay to step off at least that far plus stepheight
            bottom_max += NPCInfo->goalEntity->r.currentOrigin.z - NPC->r.currentOrigin.z;
        }
    }
    VectorCopy(&trace.endpos, &testPos);
    testPos.z += bottom_max;

    trap->Trace(&trace, &trace.endpos, &mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask, qfalse, 0, 0);
    ...
    if (trace.allsolid || trace.startsolid) { // Not going off a cliff
        // Com_Printf( "%d walk off cliff okay (droptrace in solid)\n", level.time );
        return qtrue;
    }

    if (trace.fraction < 1.0f) { // Not going off a cliff
    ...
    if (reset) { // actually want to screw with the ucmd
        // Com_Printf( "%d avoiding walk off cliff\n", level.time );
        ucmd.forwardmove *= -1.0f; //= 0;
        ucmd.rightmove *= -1.0f;   //= 0;
...
static void Jedi_Move(gentity_t *goal, qboolean retreat) {
    ...
    if (retreat) { // FIXME: should we trace and make sure we can go this way?  Or somehow let NPC_MoveToGoal know we want to retreat and have it handle it?
        ucmd.forwardmove *= -1;
        ucmd.rightmove *= -1;
        VectorScale(&NPC->client->ps.moveDir, -1, &NPC->client->ps.moveDir);
    }
...
static qboolean Jedi_Hunt(void) {
    ...
    if (NPCInfo->stats.aggression > 1) { // approach enemy
        NPCInfo->combatMove = qtrue;
...
static void Jedi_Retreat(void) {
    if (!TIMER_Done(NPC, "noRetreat")) { // don't actually move
        return;
    }
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel) {
    ...
    if (self->client->NPC_class == CLASS_TAVION) { // special attacks
        self->client->ps.fd.saberAnimLevel = FORCE_LEVEL_5;
        return;
    } else if (self->client->NPC_class == CLASS_DESANN) { // special attacks
        self->client->ps.fd.saberAnimLevel = FORCE_LEVEL_4;
        return;
    }
    if (self->client->playerTeam == NPCTEAM_ENEMY) {
        if (self->NPC->rank == RANK_CIVILIAN || self->NPC->rank == RANK_LT_JG) { // grunt and fencer always uses quick attacks
            self->client->ps.fd.saberAnimLevel = FORCE_LEVEL_1;
            return;
        }
        if (self->NPC->rank == RANK_CREWMAN || self->NPC->rank == RANK_ENSIGN) { // acrobat & force-users always use medium attacks
    ...
    if (newLevel > self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) { // cap it
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
    if (!NPC->client->ps.weaponTime && !(ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // not attacking
...
static void Jedi_CombatDistance(int enemy_dist) { // FIXME: for many of these checks, what we really want is horizontal distance to enemy
    if (NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) && NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // when gripping, don't move
        return;
    } else if (!TIMER_Done(NPC, "gripping")) { // stopped gripping, clear timers just in case
        TIMER_Set(NPC, "gripping", -level.time);
        TIMER_Set(NPC, "attackDelay", Q_irand(0, 1000));
    }

    if (NPC->client->ps.fd.forcePowersActive & (1 << FP_DRAIN) && NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1) { // when draining, don't move
        return;
    } else if (!TIMER_Done(NPC, "draining")) { // stopped draining, clear timers just in case
    ...
    if (NPC->client->NPC_class == CLASS_BOBAFETT) {
    ...
    } else if (NPC->client->ps.saberInFlight && !PM_SaberInBrokenParry(NPC->client->ps.saberMove) &&
               NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // maintain distance
        ...
        if (NPC->client->ps.weapon == WP_SABER                                                                                // using saber
            ...
            && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
        {                                                                                                                     // hold it out there
            ucmd.buttons |= BUTTON_ALT_ATTACK;
            // FIXME: time limit?
        }
    } else if (!TIMER_Done(NPC, "taunting")) {
        if (enemy_dist <= 64) { // he's getting too close
            ucmd.buttons |= BUTTON_ATTACK;
            if (!NPC->client->ps.saberInFlight) {
                WP_ActivateSaber(NPC);
            }
        ...
        // else if ( NPC->client->ps.torsoAnim == BOTH_GESTURE1 && NPC->client->ps.torsoTimer < 2000 )
                 ...
                 (NPC->client->ps.forceHandExtendTime - level.time) < 200) { // we're almost done with our special taunt
            // FIXME: this doesn't always work, for some reason
            if (!NPC->client->ps.saberInFlight) {
                WP_ActivateSaber(NPC);
            }
        }
    } else if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage
        if (enemy_dist > 0) {                                    // get closer so we can hit!
            Jedi_Advance();
        }
        if (enemy_dist > 128) { // lost 'em
            NPC->client->ps.saberEventFlags &= ~SEF_LOCK_WON;
        }
        if (NPC->enemy->painDebounceTime + 2000 < level.time) { // the window of opportunity is gone
            NPC->client->ps.saberEventFlags &= ~SEF_LOCK_WON;
        }
        // don't strafe?
        ...
        TIMER_Set(NPC, "strafeRight", -1);
               ...
               NPC->client->ps.saberLockTime < level.time) { // enemy is in a saberLock and we are not
        if (enemy_dist < 64) {                               // FIXME: maybe just pick another enemy?
            Jedi_Retreat();
        }
    }
    // rwwFIXMEFIXME: Give them the ability to do this again (turret needs to be fixed up to allow it)
                                  ...
                                  (!Q_stricmp("Yoda", NPC->NPC_type) && !Q_irand(0, 10)))) { // can't use saber and they're in striking range
        if (!Q_irand(0, 5) && InFront(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, 0.2f)) {
            if (((NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
                 NPC->client->pers.maxHealth - NPC->health > NPC->client->pers.maxHealth * 0.25f) // lost over 1/4 of our health or not firing
        ...
        Jedi_Retreat();
    } else if (enemy_dist <= 64 && NPC->client->pers.maxHealth - NPC->health > NPC->client->pers.maxHealth * 0.25f // lost over 1/4 of our health
               && NPC->client->ps.fd.forcePowersKnown & (1 << FP_DRAIN)                                            // know how to drain
               && WP_ForcePowerAvailable(NPC, FP_DRAIN, 20)                                                        // have enough power
               && !Q_irand(0, 10) && InFront(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, 0.2f)) {
        TIMER_Set(NPC, "draining", 3000);
        TIMER_Set(NPC, "attackDelay", 3000);
        Jedi_Advance();
        return;
    } else if (enemy_dist <= -16) { // we're too damn close!
        Jedi_Retreat();
    } else if (enemy_dist <= 0) { // we're within striking range
        // if we are attacking, see if we should stop
        if (NPCInfo->stats.aggression < 4) { // back off and defend
            Jedi_Retreat();
        }
    } else if (enemy_dist > 256) { // we're way out of range
        qboolean usedForce = qfalse;
        if (NPCInfo->stats.aggression < Q_irand(0, 20) && NPC->health < NPC->client->pers.maxHealth * 0.75f && !Q_irand(0, 2)) {
            if ((NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) != 0 && (NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) == 0 && Q_irand(0, 1)) {
                ForceHeal(NPC);
                usedForce = qtrue;
                // FIXME: check level of heal and know not to move or attack when healing
            } else if ((NPC->client->ps.fd.forcePowersKnown & (1 << FP_PROTECT)) != 0 && (NPC->client->ps.fd.forcePowersActive & (1 << FP_PROTECT)) == 0 &&
                       Q_irand(0, 1)) {
                ForceProtect(NPC);
                usedForce = qtrue;
            } else if ((NPC->client->ps.fd.forcePowersKnown & (1 << FP_ABSORB)) != 0 && (NPC->client->ps.fd.forcePowersActive & (1 << FP_ABSORB)) == 0 &&
                       Q_irand(0, 1)) {
                ForceAbsorb(NPC);
                usedForce = qtrue;
            } else if ((NPC->client->ps.fd.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) == 0 &&
                       Q_irand(0, 1)) {
                Jedi_Rage();
                usedForce = qtrue;
            }
        ...
        if (enemy_dist > 384) { // FIXME: check for enemy facing away and/or moving away
        ...
        if (NPCInfo->stats.aggression > 0) { // approach enemy
            if (!usedForce) {
                Jedi_Advance();
            }
        }
    ...
    */
    ...
    {                         // we're out of striking range and we are allowed to attack
        ...
        if (NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.fd.forceGripBeingGripped > level.time)) { // They're being gripped, rush them!
            if (NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) {                                       // they're on the ground, so advance
                if (TIMER_Done(NPC, "parryTime") || NPCInfo->rank > RANK_LT) {                                    // not parrying
                    if (enemy_dist > 200 || !(NPCInfo->scriptFlags & SCF_DONT_FIRE)) {                            // far away or allowed to use saber
                        Jedi_Advance();
                    }
                }
            }
            if (NPCInfo->rank >= RANK_LT_JG && !Q_irand(0, 5) && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
                !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
            {                                                     // throw saber
                ucmd.buttons |= BUTTON_ALT_ATTACK;
            }
        } else if (NPC->enemy && NPC->enemy->client && // valid enemy
                   NPC->enemy->client->ps.saberInFlight &&
                   /*NPC->enemy->client->ps.saber[0].Active()*/ NPC->enemy->client->ps.saberEntityNum && // enemy throwing saber
        ...
        {                                                                // They're throwing their saber, grip them!
            ...
            if (NPC->enemy && NPC->enemy->client &&
                (NPC->enemy->client->ps.fd.forcePowersActive &
                 (1 << FP_GRIP))) { // They're choking someone, probably an ally, run at them and do some sort of attack
                if (NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) {            // they're on the ground, so advance
                    if (TIMER_Done(NPC, "parryTime") || NPCInfo->rank > RANK_LT) {         // not parrying
                        if (enemy_dist > 200 || !(NPCInfo->scriptFlags & SCF_DONT_FIRE)) { // far away or allowed to use saber
                            Jedi_Advance();
            ...
            if (chanceScale &&
                ...
                enemy_dist < 500 &&
                (Q_irand(0, chanceScale * 10) < 5 || (NPC->enemy->client && NPC->enemy->client->ps.weapon != WP_SABER &&
                                                      !Q_irand(0, chanceScale)))) { // else, randomly try some kind of attack every now and then
                if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !Q_irand(0, 1)) {
                    ...
                    // rwwFIXMEFIXME: After new drain stuff from SP is in re-enable this.
                        ...
                        // taunt
                        if (TIMER_Done(NPC, "chatter") && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time &&
                        ...
                        if (WP_ForcePowerAvailable(NPC, FP_SABERTHROW, 0) && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
                            !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
                        {                                                     // throw saber
                            ucmd.buttons |= BUTTON_ALT_ATTACK;
                        }
                    ...
                    if (NPCInfo->rank >= RANK_LT_JG && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
                        !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
                    {                                                     // throw saber
                        ucmd.buttons |= BUTTON_ALT_ATTACK;
                    }
            ...
            else if (NPCInfo->stats.aggression > 5) {                                                      // approach enemy
                if (TIMER_Done(NPC, "parryTime") || NPCInfo->rank > RANK_LT) {                             // not parrying
                    if (!NPC->enemy->client || NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // they're on the ground, so advance
                        if (enemy_dist > 200 || !(NPCInfo->scriptFlags & SCF_DONT_FIRE)) {                 // far away or allowed to use saber
                            Jedi_Advance();
                        }
                    }
                }
            } else { // maintain this distance?
                     // walk?
            }
        }
    } else {                                 // we're not close enough to attack, but not far enough away to be safe
        if (NPCInfo->stats.aggression < 4) { // back off and defend
            Jedi_Retreat();
        } else if (NPCInfo->stats.aggression > 5) {                                                        // try to get closer
            if (enemy_dist > 0 && !(NPCInfo->scriptFlags & SCF_DONT_FIRE)) {                               // we're allowed to use our lightsaber, get closer
                if (TIMER_Done(NPC, "parryTime") || NPCInfo->rank > RANK_LT) {                             // not parrying
                    if (!NPC->enemy->client || NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // they're on the ground, so advance
                        Jedi_Advance();
                    }
        ...
        } else { // agression is 4 or 5... somewhere in the middle
                 // what do we do here?  Nothing?
                 // Move forward and back?
        }
    }
    // if really really mad, rage!
    if (NPCInfo->stats.aggression > Q_irand(5, 15) && NPC->health < NPC->client->pers.maxHealth * 0.75f && !Q_irand(0, 2)) {
        if ((NPC->client->ps.fd.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) == 0) {
            Jedi_Rage();
        }
    }
}
...
static qboolean Jedi_Strafe(int strafeTimeMin, int strafeTimeMax, int nextStrafeTimeMin, int nextStrafeTimeMax, qboolean walking) {
    if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON && NPC->enemy &&
        NPC->enemy->painDebounceTime > level.time) { // don't strafe if pressing the advantage of winning a saberLock
        return qfalse;
    }
    if (TIMER_Done(NPC, "strafeLeft") && TIMER_Done(NPC, "strafeRight")) {
        ...
        if (strafed) {
            ...
            if (walking) { // should be a slow strafe
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot, float zdiff) {
    ...
    if (self->client &&
        (self->client->ps.fd.forceRageRecoveryTime > level.time || (self->client->ps.fd.forcePowersActive & (1 << FP_RAGE)))) { // no fancy dodges when raging
        return EVASION_NONE;
    }
    // Check for:
    // ARIALS/CARTWHEELS
    ...
    if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT || self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT) { // already running on a wall
        vector3 right, fwdAngles;
        int anim = -1;
        float animLength;
        ...
        } else if (self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT && rightdot > 0) { // I'm running on a wall to my right and the attack is on the right
            if (animLength - self->client->ps.legsTimer > 400 && self->client->ps.legsTimer > 400) { // not at the beginning or end of the anim
                anim = BOTH_WALL_RUN_RIGHT_FLIP;
            }
        }
        if (anim != -1) { // flip off the wall!
            int parts;
            ...
            if (anim == BOTH_WALL_RUN_LEFT_FLIP) {
                self->client->ps.velocity.x *= 0.5f;
                self->client->ps.velocity.y *= 0.5f;
            ...
            return EVASION_OTHER;
               ...
               && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT) && Q_irand(0, 1) &&
               !BG_InRoll(&self->client->ps, self->client->ps.legsAnim) && !PM_InKnockDown(&self->client->ps) &&
        ...
        if (self->client->ps.weapon == WP_SABER) {
            if (self->client->saber[0].model[0] && (self->client->saber[0].saberFlags & SFL_NO_CARTWHEELS)) {
                allowCartWheels = qfalse;
            } else if (self->client->saber[1].model[0] && (self->client->saber[1].saberFlags & SFL_NO_CARTWHEELS)) {
                allowCartWheels = qfalse;
            }
            if (self->client->saber[0].model[0] && (self->client->saber[0].saberFlags & SFL_NO_WALL_FLIPS)) {
                allowWallFlips = qfalse;
            } else if (self->client->saber[1].model[0] && (self->client->saber[1].saberFlags & SFL_NO_WALL_FLIPS)) {
                allowWallFlips = qfalse;
            }
        }

        VectorSet(&mins, self->r.mins.x, self->r.mins.y, 0);
        VectorSet(&maxs, self->r.maxs.x, self->r.maxs.y, 24);
        VectorSet(&fwdAngles, 0, self->client->ps.viewangles.yaw, 0);
        ...
        if (BG_SaberInAttack(self->client->ps.saberMove) || PM_SaberInStart(self->client->ps.saberMove)) {
            parts = SETANIM_LEGS;
        }
        if (rightdot >= 0) {
            if (Q_irand(0, 1)) {
        ...
        trap->Trace(&trace, &self->r.currentOrigin, &mins, &maxs, &traceto, self->s.number, CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, qfalse, 0,
                    0);
        if (trace.fraction >= 1.0f && allowCartWheels) { // it's clear, let's do it
        ...
        } else if (!(trace.contents & CONTENTS_BOTCLIP)) { // hit a wall, not a do-not-enter brush
            ...
            if ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) ||
                DotProduct(&trace.plane.normal, &idealNormal) > 0.7f) { // it's a ent of some sort or it's a wall roughly facing us
                float bestCheckDist = 0;
                // hmm, see if we're moving forward
                if (DotProduct(&self->client->ps.velocity, &fwd) < 200) { // not running forward very fast
                    // check to see if it's okay to move the other way
                    if ((trace.fraction * checkDist) <= 32) { // wall on that side is close enough to wall-flip off of or wall-run on
                        ...
                        if (trace.fraction >= 1.0f) { // it's clear, let's do it
                            if (allowWallFlips) {     // okay to do wall-flips with this saber
                                // FIXME: check for drops?
                                // turn the cartwheel into a wallflip in the other dir
                                if (rightdot > 0) {
                                    anim = BOTH_WALL_FLIP_LEFT;
                                    self->client->ps.velocity.x = self->client->ps.velocity.y = 0;
                                    VectorMA(&self->client->ps.velocity, 150, &right, &self->client->ps.velocity);
                                } else {
                                ...
                                self->client->ps.velocity.z = forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
                                // animate me
                                parts = SETANIM_LEGS;
                                if (!self->client->ps.weaponTime)
                                    parts = SETANIM_BOTH;
                        ...
                        } else {                                                    // boxed in on both sides
                            if (DotProduct(&self->client->ps.velocity, &fwd) < 0) { // moving backwards
                                return EVASION_NONE;
                            }
                    ...
                    } else { // too far from that wall to flip or run off it, check other side
                        checkDist *= -1.0f;
                        VectorMA(&self->r.currentOrigin, checkDist, &right, &traceto);
                        // trace in the dir that we want to go
                        trap->Trace(&trace, &self->r.currentOrigin, &mins, &maxs, &traceto, self->s.number,
                                    CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, qfalse, 0, 0);
                        if ((trace.fraction * checkDist) <= 32) { // wall on this side is close enough
                            bestCheckDist = checkDist;
                        } else { // neither side has a wall within 32
                            return EVASION_NONE;
                        }
                    }
                }
                // Try wall run?
                if ((int)bestCheckDist) { // one of the walls was close enough to wall-run on
                    qboolean allowWallRuns = qtrue;
                    if (self->client->ps.weapon == WP_SABER) {
                        if (self->client->saber[0].model[0] && (self->client->saber[0].saberFlags & SFL_NO_WALL_RUNS)) {
                            allowWallRuns = qfalse;
                        } else if (self->client->saber[1].model[0] && (self->client->saber[1].saberFlags & SFL_NO_WALL_RUNS)) {
                            allowWallRuns = qfalse;
                        }
                    }
                    if (allowWallRuns) { // okay to do wallruns with this saber
                        // FIXME: check for long enough wall and a drop at the end?
                        if (bestCheckDist > 0) { // it was to the right
                            anim = BOTH_WALL_RUN_RIGHT;
                        } else { // it was to the left
...
int Jedi_ReCalcParryTime(gentity_t *self, evasionType_t evasionType) {
    ...
    if (self->s.number >= 0 && self->s.number < MAX_CLIENTS) { // player
        ...
        if (!g_saberRealisticCombat.integer && (g_spSkill.integer == 2 || (g_spSkill.integer == 1 && self->client->NPC_class == CLASS_TAVION))) {
            ...
            if (evasionType == EVASION_DODGE) {
                ...
                } else if (self->NPC->rank == RANK_CREWMAN) {                                                                     // acrobats aren't so bad
                    if (evasionType == EVASION_PARRY || evasionType == EVASION_DUCK_PARRY || evasionType == EVASION_JUMP_PARRY) { // slower with parries
                        baseTime = baseTime * Q_irand(1, 2);
                    } else { // faster with acrobatics
                             // baseTime = baseTime;
                    }
                } else { // force users are kinda slow
                    baseTime = baseTime * Q_irand(1, 2);
                }
...
qboolean Jedi_QuickReactions(gentity_t *self) {
    if ((self->client->NPC_class == CLASS_JEDI && NPCInfo->rank == RANK_COMMANDER) || self->client->NPC_class == CLASS_TAVION ||
...
qboolean Jedi_SaberBusy(gentity_t *self) {
    if (self->client->ps.torsoTimer > 300 &&
        ((BG_SaberInAttack(self->client->ps.saberMove) && self->client->ps.fd.saberAnimLevel == FORCE_LEVEL_3) ||
         ...
         || BG_FlippingAnim(self->client->ps.torsoAnim) || PM_RollingAnim(self->client->ps.torsoAnim))) { // my saber is not in a parrying position
        return qtrue;
    }
    return qfalse;
}
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vector3 *pHitloc, vector3 *phitDir, gentity_t *incoming, float dist) {
    ...
    if (!incoming) {
        ...
        if (self->client->ps.saberInFlight) { // DOH!  do non-saber evasion!
            saberBusy = qtrue;
        } else if (Jedi_QuickReactions(self)) { // jedi trainer and tavion are must faster at parrying and can do it whenever they like
        ...
        if (incoming->s.weapon == WP_SABER) { // flying lightsaber, face it!
    ...
    zdiff = hitloc.z - self->client->renderInfo.eyePoint.z; // + Q_irand(-6,6);

    // see if we can dodge if need-be
    if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) || self->client->ps.saberInFlight || BG_SabersOff(&self->client->ps) ||
        self->client->NPC_class == CLASS_BOBAFETT) { // either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
        if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT_JG)) {            // acrobat or fencer or above
            if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&                                     // on the ground
                ...
                && (self->client->ps.saberInFlight || self->client->NPC_class == CLASS_BOBAFETT ||
    ...
    if (zdiff >= -5) // was 0
    {
        if (incoming || !saberBusy) {
            if (rightdot > 12 || (rightdot > 3 && zdiff < 5) || (!incoming && fabsf(hitdir.z) < 0.25f)) // was normalized, 0.3f
            {                                                                                           // coming from right
                if (doDodge) {
                    if (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand(0, 2)) { // roll!
            ...
            } else if (rightdot < -12 || (rightdot < -3 && zdiff < 5) || (!incoming && fabsf(hitdir.z) < 0.25f)) // was normalized, -0.3f
            {                                                                                                    // coming from left
                if (doDodge) {
                    if (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand(0, 2)) { // roll!
                        TIMER_Start(self, "duck", Q_irand(500, 1500));
    ...
    else if (zdiff > -22) // was-15 )
    {
        ...
        {      // hmm, pretty low, but not low enough to use the low block, so we need to duck
            ...
            } else { // in air!  Ducking does no good
            }
        }
        if (incoming || !saberBusy) {
            if (rightdot > 8 || (rightdot > 3 && zdiff < -11)) // was normalized, 0.2f
            {
                if (doDodge) {
                    if (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand(0, 2)) { // roll!
                        TIMER_Start(self, "strafeLeft", Q_irand(500, 1500));
            ...
            } else if (rightdot < -8 || (rightdot < -3 && zdiff < -11)) // was normalized, -0.2f
            {
                if (doDodge) {
                    if (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand(0, 2)) { // roll!
                        TIMER_Start(self, "strafeLeft", Q_irand(500, 1500));
    ...
    {                                                                         // jump!
        if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) {             // already in air, duck to pull up legs
            ...
            if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank > RANK_LT_JG) &&
                (!Q_irand(0, 10) || (!Q_irand(0, 2) && (cmd->forwardmove || cmd->rightmove)))) { // superjump
                // FIXME: check the jump, if can't, then block
                if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.fd.forceRageRecoveryTime < level.time &&
                    !(self->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) && !PM_InKnockDown(&self->client->ps)) {
                ...
                if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.fd.forceRageRecoveryTime < level.time &&
                    ...
                    if (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand(0, 1)) { // roll!
                        if (rightdot > 0) {
                            TIMER_Start(self, "strafeLeft", Q_irand(500, 1500));
                            TIMER_Set(self, "strafeRight", 0);
                            TIMER_Set(self, "walking", 0);
                ...
                if (self->client->NPC_class == CLASS_TAVION) {
                    if (!incoming && self->client->ps.groundEntityNum < ENTITYNUM_NONE && !Q_irand(0, 2)) {
                        if (!BG_SaberInAttack(self->client->ps.saberMove) && !PM_SaberInStart(self->client->ps.saberMove) &&
                            !BG_InRoll(&self->client->ps, self->client->ps.legsAnim) && !PM_InKnockDown(&self->client->ps) &&
                            !BG_SaberInSpecialAttack(self->client->ps.torsoAnim)) { // do the butterfly!
                            int butterflyAnim;
                            if (Q_irand(0, 1)) {
        ...
        if (incoming || !saberBusy) {
            ...
            if (incoming && incoming->s.weapon == WP_SABER) { // thrown saber!
                if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank > RANK_LT_JG) &&
                    (!Q_irand(0, 10) || (!Q_irand(0, 2) && (cmd->forwardmove || cmd->rightmove)))) { // superjump
                    // FIXME: check the jump, if can't, then block
                    if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.fd.forceRageRecoveryTime < level.time &&
...
static qboolean Jedi_SaberBlock(int saberNum, int bladeNum) // saberNum = 0, bladeNum = 0
{
    ...
    vector3 pointDir, baseDir, tipDir, saberHitPoint, saberMins, saberMaxs;
    ...
    */
    ...
    if (!TIMER_Done(NPC, "parryReCalcTime")) { // can't do our own re-think of which parry to use yet
        return qfalse;
    }

    if (NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] > level.time) { // can't move the saber to another position yet
        return qfalse;
    }
    ...
    */
    ...
    if (NPC->enemy->health <= 0 || !NPC->enemy->client) { // don't keep blocking him once he's dead (or if not a client)
        return qfalse;
    }
    /*
    ...
    VectorNormalize( dir );
    VectorMA( axisPoint, NPC->r.maxs[0]*1.22f, dir, hitloc );
    }
    else
    {
    ...
    */
    ...
    VectorMA(&NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePointOld, NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
             &NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDirOld, &saberTipOld);
    VectorMA(&NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePoint, NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
             &NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDir, &saberTip);
    ...
    VectorCopy(&NPC->r.currentOrigin, &top);
    top.z = NPC->r.absmax.z;
    VectorCopy(&NPC->r.currentOrigin, &bottom);
    bottom.z = NPC->r.absmin.z;
    ...
    // get the actual point of impact
    trap->Trace(&tr, &saberPoint, &saberMins, &saberMaxs, &hitloc, NPC->enemy->s.number, CONTENTS_BODY, qfalse, 0, 0); //, G2_RETURNONHIT, 10 );
    if (tr.allsolid || tr.startsolid || tr.fraction >= 1.0f) {                                                         // estimate
        vector3 dir2Me;
        VectorSubtract(&axisPoint, &saberPoint, &dir2Me);
        dist = VectorNormalize(&dir2Me);
        if (DotProduct(&dir, &dir2Me) < 0.2f) { // saber is not swinging in my direction
            /*
            if ( dist < 300 //close
            && !Jedi_QuickReactions( NPC )//quick reaction people can interrupt themselves
            && (PM_SaberInStart( NPC->enemy->client->ps.saberMove ) || BG_SaberInAttack( NPC->enemy->client->ps.saberMove )) )//enemy is swinging at me
    ...
    if ((evasionType = Jedi_SaberBlockGo(NPC, &ucmd, &hitloc, &dir, NULL, dist)) != EVASION_DODGE) { // we did block (not dodge)
        int parryReCalcTime;

        if (!NPC->client->ps.saberInFlight) { // make sure saber is on
            WP_ActivateSaber(NPC);
        }
        ...
        if (TIMER_Done(NPC, "parryTime")) {
            if (NPC->client->NPC_class == CLASS_TAVION) {
                TIMER_Set(NPC, "parryTime", Q_irand(parryReCalcTime / 2, parryReCalcTime * 1.5f));
            } else if (NPCInfo->rank >= RANK_LT_JG) { // fencers and higher hold a parry less
                TIMER_Set(NPC, "parryTime", parryReCalcTime);
            } else { // others hold it longer
                TIMER_Set(NPC, "parryTime", Q_irand(1, 2) * parryReCalcTime);
            }
        }
    } else {
        int dodgeTime = NPC->client->ps.torsoTimer;
        if (NPCInfo->rank > RANK_LT_COMM && NPC->client->NPC_class != CLASS_DESANN) { // higher-level guys can dodge faster
            dodgeTime -= 200;
        }
...
static void Jedi_EvasionSaber(vector3 *enemy_movedir, float enemy_dist, vector3 *enemy_dir) {
    ...
    if (!NPC->enemy->client) {
        return;
               ...
               NPC->enemy->client->ps.saberLockTime > level.time) { // don't try to block/evade an enemy who is in a saberLock
        return;
    } else if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON && NPC->enemy->painDebounceTime > level.time) { // pressing the advantage of winning a saber lock
        return;
    }

    if (NPC->enemy->client->ps.saberInFlight && !TIMER_Done(NPC, "taunting")) { // if he's throwing his saber, stop taunting
        TIMER_Set(NPC, "taunting", -level.time);
        if (!NPC->client->ps.saberInFlight) {
            WP_ActivateSaber(NPC);
        }
    ...
    if (TIMER_Done(NPC, "parryTime")) {
        if (NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE && NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
    ...
    if (NPC->enemy->client->ps.weaponTime && NPC->enemy->client->ps.weaponstate == WEAPON_FIRING) { // enemy is attacking
        enemy_attacking = qtrue;
        evasionChance = 90;
    }

    if ((NPC->enemy->client->ps.fd.forcePowersActive & (1 << FP_LIGHTNING))) { // enemy is shooting lightning
        enemy_attacking = qtrue;
        shooting_lightning = qtrue;
        evasionChance = 50;
    }

    if (NPC->enemy->client->ps.saberInFlight && NPC->enemy->client->ps.saberEntityNum != ENTITYNUM_NONE &&
        NPC->enemy->client->ps.saberEntityState != SES_RETURNING) { // enemy is shooting lightning
        enemy_attacking = qtrue;
        throwing_saber = qtrue;
    }

    // FIXME: this needs to take skill and rank(reborn type) into account much more
    if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
        float facingAmt;
        if (VectorCompare(enemy_movedir, &vec3_origin) || shooting_lightning ||
            throwing_saber) { // he's not moving (or he's using a ranged attack), see if he's facing me
            vector3 enemy_fwd;
        ...
        if (flrand(0.25f, 1) < facingAmt) { // coming at/facing me!
            int whichDefense = 0;
            if (NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight ||
                NPC->client->NPC_class == CLASS_BOBAFETT) { // I'm attacking or recovering from a parry, can only try to strafe/jump right now
                if (Q_irand(0, 10) < NPCInfo->stats.aggression) {
                    return;
                }
                whichDefense = 100;
            } else {
                if (shooting_lightning) { // check for lightning attack
                    // only valid defense is strafe and/or jump
                    whichDefense = 100;
                } else if (throwing_saber) { // he's thrown his saber!  See if it's coming at me
                    float saberDist;
                    vector3 saberDir2Me;
                    vector3 saberMoveDir;
                    ...
                    if (DotProduct(&saberMoveDir, &saberDir2Me) > 0.5f) { // it's heading towards me
                        if (saberDist < 100) {                            // it's close
                            whichDefense = Q_irand(3, 6);
                        } else if (saberDist < 200) { // got some time, yet, try pushing
                            whichDefense = Q_irand(0, 8);
                        }
                    }
                }
                if (whichDefense) {                                   // already chose one
                } else if (enemy_dist > 80 || !enemy_attacking) {     // he's pretty far, or not swinging, just strafe
                    if (VectorCompare(enemy_movedir, &vec3_origin)) { // if he's not moving, not swinging and far enough away, no evasion necc.
                        return;
                    }
                    if (Q_irand(0, 10) < NPCInfo->stats.aggression) {
                        return;
                    ...
                    whichDefense = 100;
                } else { // he's getting close and swinging at me
                    vector3 fwd;
                    // see if I'm facing him
                    AngleVectors(&NPC->client->ps.viewangles, &fwd, NULL, NULL);
                    if (DotProduct(enemy_dir, &fwd) < 0.5f) { // I'm not really facing him, best option is to strafe
                        whichDefense = Q_irand(5, 16);
                    } else if (enemy_dist < 56) { // he's very close, maybe we should be more inclined to block or throw
            ...
            if (whichDefense >= 4 && whichDefense <= 12) { // would try to block
                if (NPC->client->ps.saberInFlight) {       // can't, saber in not in hand, so fall back to strafe/jump
                    whichDefense = 100;
                }
            }
            ...
            case 3:
                ...
                if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) &&
                    TIMER_Done(NPC, "parryTime")) { // FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
                    ForceThrow(NPC, qfalse);
                }
                break;
            case 4:
            ...
            default:
                ...
                if (!Q_irand(0, 5) || !Jedi_Strafe(300, 1000, 0, 1000, qfalse)) { // certain chance they will pick an alternative evasion
                    ...
                    if (shooting_lightning || throwing_saber || enemy_dist < 80) {
                        ...
                        if (shooting_lightning || (!Q_irand(0, 2) && NPCInfo->stats.aggression < 4 && TIMER_Done(NPC, "parryTime"))) {
                            if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !shooting_lightning &&
                                Q_irand(0, 2)) { // FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
                                ForceThrow(NPC, qfalse);
                            } else if ((NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) && !(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) &&
                                       NPC->client->ps.fd.forceRageRecoveryTime < level.time && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) &&
                                       !PM_InKnockDown(&NPC->client->ps)) { // FIXME: make this a function call?
                    ...
                    if (!(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPC->client->ps.fd.forceRageRecoveryTime < level.time &&
                        !(NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) && (NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) &&
                        !PM_InKnockDown(&NPC->client->ps) && !Q_irand(0, 5)) { // FIXME: make this a function call?
                        // FIXME: check for clearance, safety of landing spot?
                        if (NPC->client->NPC_class == CLASS_BOBAFETT) {
...
gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback, float minDot) {
    ...
    for (e = 0; e < numListedEntities; e++) {
        ...
        if (!(check->inuse)) { // freed
            continue;
        }
        if (!check->client) { // not a client - FIXME: what about turrets?
            continue;
        }
        if (check->client->playerTeam != self->client->enemyTeam) { // not an enemy - FIXME: what about turrets?
            continue;
        }
        if (check->health <= 0) { // dead
            continue;
        }

        if (!trap->InPVS(&check->r.currentOrigin, &self->r.currentOrigin)) { // can't potentially see them
            continue;
        }
        ...
        if (DotProduct(&dir, &forward) < minDot) { // not in front
            continue;
        }
        ...
        if (tr.fraction < 1.0f && tr.entityNum != check->s.number) { // must have clear shot
            continue;
        }

        if (dist < bestDist) { // closer than our last best one
            bestDist = dist;
            enemy = check;
        }
    }
...
static void Jedi_SetEnemyInfo(vector3 *enemy_dest, vector3 *enemy_dir, float *enemy_dist, vector3 *enemy_movedir, float *enemy_movespeed, int prediction) {
    if (!NPC || !NPC->enemy) { // no valid enemy
        return;
    }
    if (!NPC->enemy->client) {
        ...
        *enemy_movespeed = 0;
        ...
        enemy_dest->z += NPC->enemy->r.mins.z + 24; // get it's origin to a height I can work with
        VectorSubtract(enemy_dest, &NPC->r.currentOrigin, enemy_dir);
        // FIXME: enemy_dist calc needs to include all blade lengths, and include distance from hand to start of blade....
        *enemy_dist = VectorNormalize(enemy_dir); // - (NPC->client->ps.saberLengthMax + NPC->r.maxs[0]*1.5f + 16);
    } else {                                      // see where enemy is headed
        ...
        *enemy_movespeed = VectorNormalize(enemy_movedir);
        ...
        VectorMA(&NPC->enemy->r.currentOrigin, *enemy_movespeed * 0.001f * prediction, enemy_movedir, enemy_dest);
        ...
        // FIXME: enemy_dist calc needs to include all blade lengths, and include distance from hand to start of blade....
        *enemy_dist = VectorNormalize(enemy_dir) - (NPC->client->saber[0].blade[0].lengthMax + NPC->r.maxs.x * 1.5f + 16); // just use the blade 0 len I guess
...
static void Jedi_FaceEnemy(qboolean doPitch) {
    ...
    if (NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) && NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // don't update?
    ...
    if (NPC->client->NPC_class == CLASS_BOBAFETT && TIMER_Done(NPC, "flameTime") && NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_DISRUPTOR &&
        (NPC->s.weapon != WP_ROCKET_LAUNCHER || !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) && NPC->s.weapon != WP_THERMAL && NPC->s.weapon != WP_TRIP_MINE &&
        NPC->s.weapon != WP_DET_PACK && NPC->s.weapon != WP_STUN_BATON
        /*&& NPC->s.weapon != WP_MELEE*/) {                     // boba leads his enemy
        if (NPC->health < NPC->client->pers.maxHealth * 0.5f) { // lead
    ...
    if (!NPC->client->ps.saberInFlight && (NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPC->client->ps.legsAnim == BOTH_CROUCHATTACKBACK1 ||
                                           NPC->client->ps.legsAnim == BOTH_ATTACK_BACK)) { // point *away*
        GetAnglesForDirection(&enemy_eyes, &eyes, &angles);
    } else { // point towards him
    ...
    */
    ...
    if (doPitch) {
        ...
        if (NPC->client->ps.saberInFlight) { // tilt down a little
...
static void Jedi_DebounceDirectionChanges(void) {
    ...
    if (ucmd.forwardmove > 0) {
        ...
        } else if (TIMER_Done(NPC, "moveforward")) { // FIXME: should be if it's zero?
        ...
        } else if (TIMER_Done(NPC, "moveback")) { // FIXME: should be if it's zero?
            TIMER_Set(NPC, "moveback", Q_irand(250, 1000));
        }
    } else if (!TIMER_Done(NPC, "moveforward")) { // NOTE: edge checking should stop me if this is bad... but what if it sends us colliding into the enemy?
        ucmd.forwardmove = 127;
        VectorClear(&NPC->client->ps.moveDir);
    } else if (!TIMER_Done(NPC, "moveback")) { // NOTE: edge checking should stop me if this is bad...
        ucmd.forwardmove = -127;
        VectorClear(&NPC->client->ps.moveDir);
    }
    ...
    if (ucmd.rightmove > 0) {
        ...
        } else if (TIMER_Done(NPC, "moveright")) { // FIXME: should be if it's zero?
        ...
        } else if (TIMER_Done(NPC, "moveleft")) { // FIXME: should be if it's zero?
            TIMER_Set(NPC, "moveleft", Q_irand(250, 1500));
        }
    } else if (!TIMER_Done(NPC, "moveright")) { // NOTE: edge checking should stop me if this is bad...
        ucmd.rightmove = 127;
        VectorClear(&NPC->client->ps.moveDir);
    } else if (!TIMER_Done(NPC, "moveleft")) { // NOTE: edge checking should stop me if this is bad...
        ucmd.rightmove = -127;
        VectorClear(&NPC->client->ps.moveDir);
    }
}
...
static void Jedi_TimersApply(void) {
    if (!ucmd.rightmove) { // only if not already strafing
        // FIXME: if enemy behind me and turning to face enemy, don't strafe in that direction, too
        if (!TIMER_Done(NPC, "strafeLeft")) {
            if (NPCInfo->desiredYaw > NPC->client->ps.viewangles.yaw + 60) { // we want to turn left, don't apply the strafing
            } else {                                                         // go ahead and strafe left
                ucmd.rightmove = -127;
                VectorClear(&NPC->client->ps.moveDir);
            }
        } else if (!TIMER_Done(NPC, "strafeRight")) {
            if (NPCInfo->desiredYaw < NPC->client->ps.viewangles.yaw - 60) { // we want to turn right, don't apply the strafing
            } else {                                                         // go ahead and strafe left
                ucmd.rightmove = 127;
                VectorClear(&NPC->client->ps.moveDir);
            }
    ...
    if (!TIMER_Done(NPC, "gripping")) { // FIXME: what do we do if we ran out of power?  NPC's can't?
        // FIXME: don't keep turning to face enemy or we'll end up spinning around
        ucmd.buttons |= BUTTON_FORCEGRIP;
    }

    if (!TIMER_Done(NPC, "draining")) { // FIXME: what do we do if we ran out of power?  NPC's can't?
        // FIXME: don't keep turning to face enemy or we'll end up spinning around
...
static void Jedi_CombatTimersUpdate(int enemy_dist) {
    if (TIMER_Done(NPC, "roamTime")) {
        ...
        if (NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) { // raging
        ...
        if (NPC->enemy && NPC->enemy->client) {
            ...
            case WP_SABER:
                ...
                if (BG_SabersOff(&NPC->enemy->client->ps)) { // fool!  Standing around unarmed, charge!
            ...
            case WP_ROCKET_LAUNCHER:
                ...
                if (NPC->enemy->attackDebounceTime < level.time) { // does this apply to players?
    ...
    if (TIMER_Done(NPC, "noStrafe") && TIMER_Done(NPC, "strafeLeft") && TIMER_Done(NPC, "strafeRight")) {
        ...
        } else { // postpone any strafing for a while
            TIMER_Set(NPC, "noStrafe", Q_irand(1000, 3000));
        }
    }

    if (NPC->client->ps.saberEventFlags) { // some kind of saber combat event is still pending
        uint32_t newFlags = NPC->client->ps.saberEventFlags;
        if (NPC->client->ps.saberEventFlags & SEF_PARRIED) { // parried
            TIMER_Set(NPC, "parryTime", -1);
            /*
            ...
            */
            if (NPC->enemy && PM_SaberInKnockaway(NPC->enemy->client->ps.saberMove)) {   // advance!
        ...
        {                                                                                    // we hit our enemy last time we swung, drop our aggression
            if (!Q_irand(0, 1))                                                              // FIXME: dependant on rank/diff?
            {
                ...
                if (!Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time &&
        ...
        if ((NPC->client->ps.saberEventFlags & SEF_BLOCKED)) { // was blocked whilst attacking
            if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) || NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN) {
                ...
                if (NPC->client->ps.saberInFlight) { // lost our saber, too!!!
        ...
        if (NPC->client->ps.saberEventFlags & SEF_DEFLECTED) { // deflected a shot
            newFlags &= ~SEF_DEFLECTED;
            if (!Q_irand(0, 3)) {
                Jedi_AdjustSaberAnimLevel(NPC, (NPC->client->ps.fd.saberAnimLevel - 1));
            }
        }
        if (NPC->client->ps.saberEventFlags & SEF_HITWALL) { // hit a wall
            newFlags &= ~SEF_HITWALL;
        }
        if (NPC->client->ps.saberEventFlags & SEF_HITOBJECT) { // hit some other damagable object
            if (!Q_irand(0, 3)) {
                Jedi_AdjustSaberAnimLevel(NPC, (NPC->client->ps.fd.saberAnimLevel - 1));
            }
...
static void Jedi_CombatIdle(int enemy_dist) {
    ...
    if (NPC->client->ps.saberInFlight) { // don't do this idle stuff if throwing saber
        return;
    }
    if (NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE) ||
        NPC->client->ps.fd.forceRageRecoveryTime > level.time) { // never taunt while raging or recovering from rage
        return;
    }
    // FIXME: make these distance numbers defines?
    if (enemy_dist >= 64) { // FIXME: only do this if standing still?
        ...
        if (Q_irand(2, chance) < NPCInfo->stats.aggression) {
            if (TIMER_Done(NPC, "chatter") && NPC->client->ps.forceHandExtend == HANDEXTEND_NONE) { // FIXME: add more taunt behaviors
                // FIXME: sometimes he turns it off, then turns it right back on again???
                if (enemy_dist > 200 && NPC->client->NPC_class != CLASS_BOBAFETT && !NPC->client->ps.saberHolstered &&
                    !Q_irand(0, 5)) { // taunt even more, turn off the saber
                ...
                } else if (Jedi_BattleTaunt()) { // FIXME: pick some anims
                }
            }
        }
    }
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
    if (NPC->enemy->client && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client->ps.saberLockTime > level.time &&
        NPC->client->ps.saberLockTime < level.time) { // enemy is in a saberLock and we are not
        return qfalse;
    }

    if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage with an attack!
        int chance = 0;
        if (NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_LUKE || !Q_stricmp("Yoda", NPC->NPC_type)) { // desann and luke
            chance = 20;
        } else if (NPC->client->NPC_class == CLASS_TAVION) { // tavion
            chance = 10;
        } else if (NPC->client->NPC_class == CLASS_REBORN && NPCInfo->rank == RANK_LT_JG) { // fencer
            chance = 5;
        } else {
            chance = NPCInfo->rank;
        }
        if (Q_irand(0, 30) < chance) {                        // based on skill with some randomness
    ...
    if (NPC->client->NPC_class == CLASS_TAVION || (NPC->client->NPC_class == CLASS_REBORN && NPCInfo->rank == RANK_LT_JG) ||
        (NPC->client->NPC_class == CLASS_JEDI &&
         NPCInfo->rank == RANK_COMMANDER)) { // tavion, fencers, jedi trainer are all good at following up a parry with an attack
        if ((PM_SaberInParry(NPC->client->ps.saberMove) || PM_SaberInKnockaway(NPC->client->ps.saberMove)) &&
            NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // try to attack straight from a parry
            NPC->client->ps.weaponTime = NPCInfo->shotTime = NPC->attackDebounceTime = 0;
            // NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + 500;
            NPC->client->ps.saberBlocked = BLOCKED_NONE;
    ...
    if (!(ucmd.buttons & BUTTON_ATTACK) && !(ucmd.buttons & BUTTON_ALT_ATTACK)) { // not already attacking
        // Try to attack
        WeaponThink(qtrue);
    }
    ...
    if (ucmd.buttons & BUTTON_ATTACK) { // attacking
        /*
        ...
        */
        ...
        if (!ucmd.rightmove) {    // not already strafing
            if (!Q_irand(0, 3)) { // 25% chance of doing this
                vector3 right, dir2enemy;

                AngleVectors(&NPC->r.currentAngles, NULL, &right, NULL);
                VectorSubtract(&NPC->enemy->r.currentOrigin, &NPC->r.currentAngles, &dir2enemy);
                if (DotProduct(&right, &dir2enemy) > 0) { // he's to my right, strafe left
                    ucmd.rightmove = -127;
                    VectorClear(&NPC->client->ps.moveDir);
                } else { // he's to my left, strafe right
                    ucmd.rightmove = 127;
                    VectorClear(&NPC->client->ps.moveDir);
                }
            }
...
static qboolean Jedi_Jump(vector3 *dest, int goalEntNum) { // FIXME: if land on enemy, knock him down & jump off again
    float targetDist, shotSpeed = 300, travelTime, impactDist, bestImpactDist = Q3_INFINITE;
    vector3 targetDir, shotVel, failCase, lastPos, testPos, bottom;
    ...
    while (hitCount < maxHits) {
        ...
        shotVel.z += travelTime * 0.5f * NPC->client->ps.gravity;

        if (!hitCount) { // save the first one as the worst case scenario
        ...
        // This may be kind of wasteful, especially on long throws... use larger steps?  Divide the travelTime into a certain hard number of slices?  Trace just
        // to apex and down?
        for (elapsedTime = timeStep; elapsedTime < floorf(travelTime) + timeStep; elapsedTime += timeStep) {
            ...
            if (testPos.z < lastPos.z) { // going down, ignore botclip
                trap->Trace(&trace, &lastPos, &NPC->r.mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask, qfalse, 0, 0);
            } else { // going up, check for botclip
                trap->Trace(&trace, &lastPos, &NPC->r.mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask | CONTENTS_BOTCLIP, qfalse, 0, 0);
            }
            ...
            if (trace.fraction < 1.0f) {             // hit something
                if (trace.entityNum == goalEntNum) { // hit the enemy, that's perfect!
                    // Hmm, don't want to land on him, though...
                    break;
                } else {
                    if (trace.contents & CONTENTS_BOTCLIP) { // hit a do-not-enter brush
                        blocked = qtrue;
                        break;
                    }
                    if (trace.plane.normal.z > 0.7f && DistanceSquared(&trace.endpos, dest) < 4096) // hit within 64 of desired location, should be okay
                    {                                                                               // close enough!
                        break;
                    } else { // FIXME: maybe find the extents of this brush and go above or below it on next try somehow?
            ...
            if (elapsedTime == (int)floorf(travelTime)) { // reached end, all clear
                if (trace.fraction >= 1.0f) {             // hmm, make sure we'll land on the ground...
                    ...
                    if (trace.fraction >= 1.0f) { // would fall too far
                        blocked = qtrue;
                    }
                }
                break;
        ...
        if (blocked) { // hit something, adjust speed (which will change arc)
            hitCount++;
            shotSpeed = 300 + ((hitCount - 2) * 100); // from 100 to 900 (skipping 300)
            if (hitCount >= 2) {                      // skip 300 since that was the first value we tested
                shotSpeed += 100;
            }
        } else { // made it!
            break;
    ...
    if (hitCount >= maxHits) { // NOTE: worst case scenario, use the one that impacted closest to the target (or just use the first try...?)
...
static qboolean Jedi_TryJump(gentity_t *goal) { // FIXME: never does a simple short, regular jump...
    ...
    if (TIMER_Done(NPC, "jumpChaseDebounce")) {
        if ((!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
            if (!PM_InKnockDown(&NPC->client->ps) && !BG_InRoll(&NPC->client->ps, NPC->client->ps.legsAnim)) { // enemy is on terra firma
                vector3 goal_diff;
                float goal_z_diff;
                float goal_xy_dist;
                VectorSubtract(&goal->r.currentOrigin, &NPC->r.currentOrigin, &goal_diff);
                goal_z_diff = goal_diff.z;
                goal_diff.z = 0;
                goal_xy_dist = VectorNormalize(&goal_diff);
                if (goal_xy_dist < 550 && goal_z_diff > -400 /*was -256*/) // for now, jedi don't take falling damage && (NPC->health > 20 || goal_z_diff > 0 )
                                                                           // && (NPC->health >= 100 || goal_z_diff > -128 ))//closer than @512
                {
                    qboolean debounce = qfalse;
                    if (NPC->health < 150 &&
                        ((NPC->health < 30 && goal_z_diff < 0) || goal_z_diff < -128)) { // don't jump, just walk off... doesn't help with ledges, though
                        debounce = qtrue;
                    } else if (goal_z_diff < 32 && goal_xy_dist < 200) { // what is their ideal jump height?
                        ucmd.upmove = 127;
                        debounce = qtrue;
                    } else {
                        /*
                        ...
                        */
                        if (goal_z_diff > 0 || goal_xy_dist > 128) { // Fake a force-jump
                            // Screw it, just do my own calc & throw
                            vector3 dest;
                            VectorCopy(&goal->r.currentOrigin, &dest);
                            if (goal == NPC->enemy) {
                                int sideTry = 0;
                                while (sideTry < 10) { // FIXME: make it so it doesn't try the same spot again?
                                    trace_t trace;
                                    vector3 bottom;

                                    if (Q_irand(0, 1))
                                        dest.x += NPC->enemy->r.maxs.x * 1.25f;
                                    else
                                        dest.x += NPC->enemy->r.mins.x * 1.25f;
                                    if (Q_irand(0, 1))
                                        dest.y += NPC->enemy->r.maxs.y * 1.25f;
                                    else
                                        dest.y += NPC->enemy->r.mins.y * 1.25f;
                                    VectorCopy(&dest, &bottom);
                                    bottom.z -= 128;
                                    trap->Trace(&trace, &dest, &NPC->r.mins, &NPC->r.maxs, &bottom, goal->s.number, NPC->clipmask, qfalse, 0, 0);
                                    if (trace.fraction < 1.0f) { // hit floor, okay to land here
                                        break;
                                    }
                                    sideTry++;
                                }
                                if (sideTry >= 10) { // screw it, just jump right at him?
                                    VectorCopy(&goal->r.currentOrigin, &dest);
                                }
                            }
                            if (Jedi_Jump(&dest, goal->s.number)) {
                                ...
                                */
                                { // FIXME: make this a function call
                                    ...
                                    if (NPC->client->NPC_class == CLASS_BOBAFETT ||
                                        (NPCInfo->rank != RANK_CREWMAN && NPCInfo->rank <= RANK_LT_JG)) { // can't do acrobatics
                                ...
                                NPC->client->ps.fd.forceJumpZStart = NPC->r.currentOrigin.z;
...
static qboolean Jedi_Jumping(gentity_t *goal) {
    if (!TIMER_Done(NPC, "forceJumpChasing") && goal) { // force-jumping at the enemy
        ...
        {                                                      // landed
            ...
            /*
            vector3	viewangles_xy={0,0,0}, goal_dir, goal_xy_dir, forward, right;
            float	goal_dist;
...
static void Jedi_CheckEnemyMovement(float enemy_dist) {
    ...
    if (NPC->client->NPC_class != CLASS_TAVION && NPC->client->NPC_class != CLASS_DESANN && NPC->client->NPC_class != CLASS_LUKE &&
        Q_stricmp("Yoda", NPC->NPC_type)) {
        if (NPC->enemy->enemy && NPC->enemy->enemy == NPC) { // enemy is mad at *me*
            if (NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
                NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSTABDOWN) { // enemy is flipping over me
                if (Q_irand(0, NPCInfo->rank) < RANK_LT) {                  // be nice and stand still for him...
            ...
            } else if (NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_BACK1 || NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_RIGHT ||
                       NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_LEFT || NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_LEFT_FLIP ||
                       NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT_FLIP) { // he's flipping off a wall
                if (NPC->enemy->client->ps.groundEntityNum == ENTITYNUM_NONE) {       // still in air
                    if (enemy_dist < 256) {                                           // close
                        if (Q_irand(0, NPCInfo->rank) < RANK_LT) {                    // be nice and stand still for him...
                            vector3 enemyFwd, dest, dir;
            ...
            } else if (NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1) {                                               // he's stabbing backwards
                if (enemy_dist < 256 && enemy_dist > 64) {                                                                   // close
                    if (!InFront(&NPC->r.currentOrigin, &NPC->enemy->r.currentOrigin, &NPC->enemy->r.currentAngles, 0.0f)) { // behind him
                        if (!Q_irand(0, NPCInfo->rank)) { // be nice and stand still for him...
                            vector3 enemyFwd, dest, dir;
...
static void Jedi_CheckJumps(void) {
    ...
    trace_t trace;
    trajectory_t tr;
    vector3 lastPos, testPos, bottom;
    int elapsedTime;
    ...
    // This may be kind of wasteful, especially on long throws... use larger steps?  Divide the travelTime into a certain hard number of slices?  Trace just to
    // apex and down?
    for (elapsedTime = 500; elapsedTime <= 4000; elapsedTime += 500) {
        ...
        if (testPos.z < lastPos.z) { // going down, don't check for BOTCLIP
            trap->Trace(&trace, &lastPos, &NPC->r.mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask, qfalse, 0, 0); // FIXME: include CONTENTS_BOTCLIP?
        } else {                                                                                                             // going up, check for BOTCLIP
            trap->Trace(&trace, &lastPos, &NPC->r.mins, &NPC->r.maxs, &testPos, NPC->s.number, NPC->clipmask | CONTENTS_BOTCLIP, qfalse, 0, 0);
        }
        if (trace.allsolid || trace.startsolid) { // WTF?
    ...
    VectorCopy(&trace.endpos, &bottom);
    if (bottom.z > NPC->r.currentOrigin.z) { // only care about dist down from current height or lower
        bottom.z = NPC->r.currentOrigin.z;
    } else if (NPC->r.currentOrigin.z - bottom.z > 400) { // whoa, long drop, don't do it!
        // probably no floor at end of jump, so don't jump
        goto jump_unsafe;
    }
    bottom.z -= 128;
    trap->Trace(&trace, &trace.endpos, &NPC->r.mins, &NPC->r.maxs, &bottom, NPC->s.number, NPC->clipmask, qfalse, 0, 0);
    if (trace.allsolid || trace.startsolid || trace.fraction < 1.0f) { // hit ground!
        if (trace.entityNum < ENTITYNUM_WORLD) {                       // landed on an ent
            gentity_t *groundEnt = &g_entities[trace.entityNum];
            if (groundEnt->r.svFlags & SVF_GLASS_BRUSH) { // don't land on breakable glass!
                goto jump_unsafe;
            }
        }
...
static void Jedi_Combat(void) {
    vector3 enemy_dir, enemy_movedir, enemy_dest;
    ...
    if (Jedi_Jumping(NPC->enemy)) { // I'm in the middle of a jump, so just see if I should attack
        Jedi_AttackDecide(enemy_dist);
        return;
    }

    if (!(NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) || NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2) { // not gripping
        // If we can't get straight at him
        if (!Jedi_ClearPathToSpot(&enemy_dest, NPC->enemy->s.number)) { // hunt him down
            // Com_Printf( "No Clear Path\n" );
            if ((NPC_ClearLOS4(NPC->enemy) || NPCInfo->enemyLastSeenTime > level.time - 500) &&
                NPC_FaceEnemy(qtrue)) //( NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG ) &&
            {
                ...
                trap->Trace( &trace, NPC->r.currentOrigin, NPC->r.mins, NPC->r.maxs, end, NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0 );
                ...
                if ( !trace.allsolid && !trace.startsolid )
                {
                if ( trace.fraction >= 1.0f || trace.plane.normal[2] > 0 )
                {
                ucmd.upmove = 127;
                ucmd.forwardmove = 127;
                return;
                ...
                */
                ...
                if (Jedi_TryJump(NPC->enemy)) { // FIXME: what about jumping to his enemyLastSeenLocation?
                    return;
                }
            }

            // Check for evasion
            if (TIMER_Done(NPC, "parryTime")) {                                                                                   // finished parrying
                if (NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE && NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
                    NPC->client->ps.saberBlocked = BLOCKED_NONE;
                }
            }
            ...
            {                                                       // can macro-navigate to him
                if (enemy_dist < 384 && !Q_irand(0, 10) && NPCInfo->blockedSpeechDebounceTime < level.time &&
                    jediSpeechDebounceTime[NPC->client->playerTeam] < level.time && !NPC_ClearLOS4(NPC->enemy)) {
            ...
            /*
                            ...
                            */
            else {                                      // FIXME: try to find a waypoint that can see enemy, jump from there
                if (NPCInfo->aiFlags & NPCAI_BLOCKED) { // try to jump to the blockedDest
                    ...
                    if (Jedi_TryJump(tempGoal)) { // going to jump to the dest
    ...
    } else { // do we need to do any evasion for other kinds of enemies?
    }

    // apply strafing/walking timers, etc.
    Jedi_TimersApply();

    if (!NPC->client->ps.saberInFlight && (!(NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) ||
                                           NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2)) { // not throwing saber or using force grip
        // see if we can attack
        if (!Jedi_AttackDecide(enemy_dist)) { // we're not attacking, decide what else to do
    ...
    if (!NPC_MoveDirClear(ucmd.forwardmove, ucmd.rightmove, qtrue)) { // uh-oh, we are going to fall or hit something
        navInfo_t info;
        // Get the move info
        NAV_GetLastMove(&info);
        if (!(info.flags & NIF_MACRO_NAV)) { // micro-navigation told us to step off a ledge, try using macronav for now
...
qboolean Jedi_CheckDanger(void) {
    ...
    if (level.alertEvents[alertEvent].level >= AEL_DANGER) { // run away!
        if (!level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client ||
            (level.alertEvents[alertEvent].owner != NPC && level.alertEvents[alertEvent].owner->client->playerTeam != NPC->client->playerTeam)) { // no owner
            return qfalse;
        }
...
qboolean Jedi_CheckAmbushPlayer(void) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (NPC->client->ps.powerups[PW_CLOAKED] ||
            !NPC_SomeoneLookingAtMe(NPC)) // rwwFIXMEFIXME: Need to pay attention to who is under crosshair for each player or something.
        {                                 // if I'm not cloaked and the player's crosshair is on me, I will wake up, otherwise do this stuff down here...
            if (!trap->InPVS(&player->r.currentOrigin, &NPC->r.currentOrigin)) { // must be in same room
                continue;
            } else {
                if (!NPC->client->ps.powerups[PW_CLOAKED]) {
            ...
            zDiff = NPC->r.currentOrigin.z - player->r.currentOrigin.z;
            if (zDiff <= 0 || zDiff > 512) { // never ambush if they're above me or way way below me
                continue;
            }

            // If the target is this close, then wake up regardless
            if ((target_dist = DistanceHorizontalSquared(&player->r.currentOrigin, &NPC->r.currentOrigin)) > 4096) { // closer than 64 - always ambush
                if (target_dist > 147456) {                                                                          //> 384, not close enough to ambush
                    continue;
                }
                // Check FOV first
                if (NPC->client->ps.powerups[PW_CLOAKED]) {
...
static void Jedi_Patrol(void) {
    ...
    if (Jedi_WaitingAmbush(NPC)) { // hiding on the ceiling
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_CEILING_CLING, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        if (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) {        // look for enemies
            if (Jedi_CheckAmbushPlayer() || Jedi_CheckDanger()) { // found him!
                Jedi_Ambush(NPC);
                NPC_UpdateAngles(qtrue, qtrue);
                return;
            }
    ...
    {                                                       // look for enemies
        ...
        for (i = 0; i < ENTITYNUM_WORLD; i++) {
            ...
            if (enemy && enemy->client && NPC_ValidEnemy(enemy) && enemy->client->playerTeam == NPC->client->enemyTeam) {
                if (trap->InPVS(&NPC->r.currentOrigin, &enemy->r.currentOrigin)) { // we could potentially see him
                    ...
                    if (enemy->s.eType == ET_PLAYER || enemy_dist < best_enemy_dist) {
                        ...
                        if (enemy_dist < (220 * 220) || (NPCInfo->investigateCount >= 3 && !NPC->client->ps.saberHolstered)) {
                            ...
                            NPCInfo->stats.aggression = 3;
                            break;
                        } else if (enemy->client->ps.saberInFlight &&
                                   !enemy->client->ps.saberHolstered) { // threw his saber, see if it's heading toward me and close enough to consider a threat
                            float saberDist;
                            vector3 saberDir2Me;
                            vector3 saberMoveDir;
        ...
        if (!NPC->enemy) { // still not mad
            if (!best_enemy) {
                // Com_Printf( "(%d) drop agg - no enemy (patrol)\n", level.time );
                Jedi_AggressionErosion(-1);
                // FIXME: what about alerts?  But not if ignore alerts
            } else {                             // have one to consider
                if (NPC_ClearLOS4(best_enemy)) { // we have a clear (of architecture) LOS to him
                    if (best_enemy->s.number) {  // just attack
                        G_SetEnemy(NPC, best_enemy);
                        NPCInfo->stats.aggression = 3;
                    } else if (NPC->client->NPC_class != CLASS_BOBAFETT) { // the player, toy with him
                        // get progressively more interested over time
                        if (TIMER_Done(NPC, "watchTime")) {          // we want to pick him up in stages
                            if (TIMER_Get(NPC, "watchTime") == -1) { // this is the first time, we'll ignore him for a couple seconds
                                TIMER_Set(NPC, "watchTime", Q_irand(3000, 5000));
                                goto finish;
                            } else { // okay, we've ignored him, now start to notice him
                        ...
                        if (best_enemy_dist < (440 * 440) || NPCInfo->investigateCount >= 2) { // stage three: keep facing him
                            NPC_FaceEntity(best_enemy, qtrue);
                            if (best_enemy_dist < (330 * 330)) { // stage four: turn on the saber
                                if (!NPC->client->ps.saberInFlight) {
                                    WP_ActivateSaber(NPC);
                                }
                            }
                        } else if (best_enemy_dist < (550 * 550) || NPCInfo->investigateCount == 1) { // stage two: stop and face him every now and then
                            if (TIMER_Done(NPC, "watchTime")) {
                                NPC_FaceEntity(best_enemy, qtrue);
                            }
                        } else { // stage one: look at him.
                            NPC_SetLookTarget(NPC, best_enemy->s.number, 0);
                        }
                    }
                } else if (TIMER_Done(NPC, "watchTime")) { // haven't seen him in a bit, clear the lookTarget
...
finish:
    ...
    if (NPC->enemy) { // just picked one up
...
qboolean Jedi_CanPullBackSaber(gentity_t *self) {
    ...
    if (self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_TAVION || self->client->NPC_class == CLASS_LUKE ||
        self->client->NPC_class == CLASS_DESANN || !Q_stricmp("Yoda", self->NPC_type)) {
        return qtrue;
    }
...
void NPC_BSJedi_FollowLeader(void) {
    ...
    if (NPC->client->ps.saberInFlight) {                                                           // saber is not in hand
        if (NPC->client->ps.saberEntityNum < ENTITYNUM_NONE && NPC->client->ps.saberEntityNum > 0) // player is 0
        {                                                                                          //
            if (g_entities[NPC->client->ps.saberEntityNum].s.pos.trType == TR_STATIONARY) {        // fell to the ground, try to pick it up...
                if (Jedi_CanPullBackSaber(NPC)) {
                    ...
                    ucmd.buttons |= BUTTON_ATTACK;
                    if (NPC->enemy && NPC->enemy->health > 0) { // get our saber back NOW!
    ...
    if (NPCInfo->goalEntity) {
        ...
        if (Jedi_Jumping(NPCInfo->goalEntity)) { // in mid-jump
            return;
        }

        if (!NAV_CheckAhead(NPC, &NPCInfo->goalEntity->r.currentOrigin, &trace,
                            (NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP)) {                 // can't get straight to him
            if (NPC_ClearLOS4(NPCInfo->goalEntity) && NPC_FaceEntity(NPCInfo->goalEntity, qtrue)) { // no line of sight
                if (Jedi_TryJump(NPCInfo->goalEntity)) {                                            // started a jump
                    return;
                }
            }
        }
        if (NPCInfo->aiFlags & NPCAI_BLOCKED) { // try to jump to the blockedDest
            if (fabsf(NPCInfo->blockedDest.z - NPC->r.currentOrigin.z) > 64) {
                gentity_t *tempGoal = G_Spawn(); // ugh, this is NOT good...?
                G_SetOrigin(tempGoal, &NPCInfo->blockedDest);
                trap->LinkEntity((sharedEntity_t *)tempGoal);
...
static void Jedi_Attack(void) {
    ...
    if (NPC->client->ps.saberLockTime > level.time) {
        ...
        if (NPC->client->ps.fd.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2 && NPC->client->ps.saberLockTime < level.time + 5000 && !Q_irand(0, 10)) {
            ForceThrow(NPC, qfalse);
        }
            ...
            {      // fell to the ground, try to pick it up
                ...
                if (1) // no matter
                {
                    ...
                    if (NPC->enemy && NPC->enemy->health > 0) { // get our saber back NOW!
                        ...
                        NPC_UpdateAngles(qtrue, qtrue);
                        if (NPC->enemy->s.weapon == WP_SABER) { // be sure to continue evasion
                            vector3 enemy_dir, enemy_movedir, enemy_dest;
                            float enemy_dist, enemy_movespeed;
                            Jedi_SetEnemyInfo(&enemy_dest, &enemy_dir, &enemy_dist, &enemy_movedir, &enemy_movespeed, 300);
                            Jedi_EvasionSaber(&enemy_movedir, enemy_dist, &enemy_dir);
        ...
        {                                                                                                     // my enemy is dead and I killed him
            ...
            if (NPC->client->NPC_class == CLASS_BOBAFETT) {
                ...
                if (!TIMER_Done(NPC, "gloatTime")) {
                    if (DistanceHorizontalSquared(&NPC->client->renderInfo.eyePoint, &NPC->enemy->r.currentOrigin) > 4096 &&
                ...
                if (!NPC->client->ps.saberHolstered &&
                    NPC->client->ps.saberInFlight) { // saber is still on (or we're trying to pull it back), count down erosion and keep facing the enemy
                    ...
                    if (BG_SabersOff(&NPC->client->ps) && !NPC->client->ps.saberInFlight) { // turned off saber (in hand), gloat
                        G_AddVoiceEvent(NPC, Q_irand(EV_VICTORY1, EV_VICTORY3), 3000);
                        jediSpeechDebounceTime[NPC->client->playerTeam] = level.time + 3000;
                        NPCInfo->desiredPitch = 0;
                        NPCInfo->goalEntity = NULL;
                ...
                if (!NPC->client->ps.saberHolstered || NPC->client->ps.saberInFlight || !TIMER_Done(NPC, "gloatTime")) { // keep walking
                    if (DistanceHorizontalSquared(&NPC->client->renderInfo.eyePoint, &NPC->enemy->r.currentOrigin) > 4096 &&
                        (NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) // 64 squared
                    {
                        NPCInfo->goalEntity = NPC->enemy;
                        ...
                        if (NPC->health < NPC->client->pers.maxHealth && (NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) != 0 &&
    ...
    if (NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp("PAS", NPC->enemy->classname)) {
        if (NPC->enemy->count <= 0) { // it's out of ammo
    ...
    if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) || ((NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) &&
                                                        NPC->client->ps.fd.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_2)) { // this is really stupid, but okay...
        ucmd.forwardmove = 0;
        ucmd.rightmove = 0;
        if (ucmd.upmove > 0) {
            ucmd.upmove = 0;
    ...
    // NOTE: for now, we clear ucmd.forwardmove & ucmd.rightmove while in air to avoid jumps going awry...
    if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // don't push while in air, throws off jumps!
    ...
    if (NPC->client->NPC_class != CLASS_BOBAFETT) {
        if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) ||
            NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN) { // just make sure they don't pull their saber to them if they're being blocked
            ucmd.buttons &= ~BUTTON_ATTACK;
        }
    }
    ...
    if (ucmd.buttons & BUTTON_ATTACK && NPC->client->playerTeam == NPCTEAM_ENEMY) {
        if (Q_irand(0, NPC->client->ps.fd.saberAnimLevel) > 0 && Q_irand(0, NPC->client->pers.maxHealth + 10) > NPC->health &&
            !Q_irand(0, 3)) { // the more we're hurt and the stronger the attack we're using, the more likely we are to make a anger noise when we swing
    ...
    if (NPC->client->NPC_class != CLASS_BOBAFETT) {
        if (NPC->client->NPC_class == CLASS_TAVION ||
            (g_spSkill.integer && (NPC->client->NPC_class == CLASS_DESANN ||
                                   NPCInfo->rank >= Q_irand(RANK_CREWMAN, RANK_CAPTAIN)))) { // Tavion will kick in force speed if the player does...
            if (NPC->enemy && NPC->enemy->s.number >= 0 && NPC->enemy->s.number < MAX_CLIENTS && NPC->enemy->client &&
                (NPC->enemy->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED))) {
                int chance = 0;
                switch (g_spSkill.integer) {
                case 0:
                    chance = 9;
...
void NPC_BSJedi_Default(void) {
    ...
    {                                  // have an enemy
        if (Jedi_WaitingAmbush(NPC)) { // we were still waiting to drop down - must have had enemy set on me outside my AI
            Jedi_Ambush(NPC);
        }
        if (NPC->client->NPC_class == CLASS_BOBAFETT) {
            if (NPC->enemy->enemy != NPC && NPC->health == NPC->client->pers.maxHealth &&
                DistanceSquared(&NPC->r.currentOrigin, &NPC->enemy->r.currentOrigin) > (800 * 800)) {
                NPCInfo->scriptFlags |= SCF_ALT_FIRE;
                Boba_ChangeWeapon(WP_DISRUPTOR);
        ...
        Jedi_Attack();
        // if we have multiple-jedi combat, probably need to keep checking (at certain debounce intervals) for a better (closer, more active) enemy and switch
        // if needbe...
        if (((!ucmd.buttons && !NPC->client->ps.fd.forcePowersActive) || (NPC->enemy && NPC->enemy->health <= 0)) &&
            NPCInfo->enemyCheckDebounceTime < level.time) { // not doing anything (or walking toward a vanquished enemy - fixme: always taunt the player?), not
                                                            // using force powers and it's time to look again
            // FIXME: build a list of all local enemies (since we have to find best anyway) for other AI factors- like when to use group attacks, determine when
            // to change tactics, when surrounded, when blocked by another in the enemy group, etc.  Should we build this group list or let the enemies maintain
            // their own list and we just access it?
