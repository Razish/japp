...
qboolean Client_Supports(const gentity_t *ent, uint32_t supportFlag) { return !!(ent->client->pers.CSF & supportFlag); }

qboolean Client_Disabled(const gentity_t *self, uint32_t pluginDisable) { return !!(self->client->pers.CPD & pluginDisable); }
...
const char *clientPluginDisableNames[CPD_NUM] = {
    "CPD_NEWDRAINEFX",    "CPD_DUELSEEOTHERS",       "CPD_ENDDUELROTATION", "CPD_BLACKSABERSDISABLE", "CPD_AUTOREPLYDISABLE",
    "CPD_NEWFORCEEFFECT", "CPD_NEWDEATHMSG_DISABLE", "CPD_NEWSIGHTEFFECT",  "CPD_NOALTDIMEFFECT",     "CPD_HOLSTEREDSABER",
    "CPD_LEDGEGRAB",      "CPD_NEWDFAPRIM",          "CPD_NEWDFAALT",
