...
gitem_t	*BG_FindItemForWeapon( weapon_t weapon ) {
	...
	Com_Error( ERR_DROP, "BG_FindItemForWeapon: Couldn't find item for weapon %i", weapon);
	return NULL;
}
...
qboolean BG_CanItemBeGrabbed( int gametype, const entityState_t *ent, const playerState_t *ps ) {
	...
	case IT_WEAPON:
		...
		else if ( BG_AlwaysPickupWeapons() || (item->giTag == WP_THERMAL || item->giTag == WP_TRIP_MINE || item->giTag == WP_DET_PACK) )
...
void BG_TouchJumpPad( playerState_t *ps, entityState_t *jumppad ) {
	...
	//Raz: no more force draining after bouncing the jumppad
