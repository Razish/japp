...
void G_AngerAlert(gentity_t *self) {
    ...
    if (!TIMER_Done(self, "interrogating")) { // I'm interrogating, don't wake everyone else up yet... FIXME: this may never wake everyone else up, though!
        return;
    }
...
void G_AttackDelay(gentity_t *self, gentity_t *enemy) {
    if (enemy && self->client && self->NPC) { // delay their attack based on how far away they're facing from enemy
        vector3 fwd, dir;
        int attDelay;
        ...
        if (self->client->playerTeam == NPCTEAM_PLAYER) { // clamp it
            if (attDelay > 2000) {
                attDelay = 2000;
            }
        }
...
void G_ForceSaberOn(gentity_t *ent) {
    if (ent->client->ps.saberInFlight) { // alright, can't turn it on now in any case, so forget it.
        return;
    }

    if (!ent->client->ps.saberHolstered) { // it's already on!
        return;
    }

    if (ent->client->ps.weapon != WP_SABER) { // This probably should never happen. But if it does we'll just return without complaining.
        return;
    }
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy) {
    ...
    if (self->client && self->NPC && enemy->client && enemy->client->playerTeam == self->client->playerTeam) { // Probably a damn script!
    ...
    if (self->enemy == NULL) {
        ...
        if (Q_stricmp("desperado", self->NPC_type) != 0 && Q_stricmp("paladin", self->NPC_type) != 0) { // special holodeck enemies exception
            if (self->client->ps.fd.forceGripBeingGripped < level.time) {                               // gripped people can't call for help
                G_AngerAlert(self);
            }
        }
...
#else
    ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
    ent->NPC->burstSpacing = weaponData[newWeapon].fireTime * 2.0f;
#endif
}
...
*/
void NPC_ApplyWeaponFireDelay(void) {
    if (NPC->attackDebounceTime > level.time) { // Just fired, if attacking again, must be a burst fire, so don't add delay
...
*/ // rwwFIXMEFIXME: support for this
    ...
    case WP_STUN_BATON:
        ...
        // RAZTODO: NPC attack delay for saber
...
qboolean HaveWeapon(int weapon) { return (client->ps.stats[STAT_WEAPONS] & (1 << weapon)); }

qboolean EntIsGlass(gentity_t *check) {
    if (check->classname && !Q_stricmp("func_breakable", check->classname) && check->count == 1 && check->health <= 100) {
        return qtrue;
    }
...
int NPC_AttackDebounceForWeapon(void) { return (NPC->client->ps.weapon == WP_SABER) ? 0 : NPCInfo->burstSpacing; }
...
qboolean ValidEnemy(gentity_t *ent) {
    ...
    if (!(ent->flags & FL_NOTARGET)) {
        if (ent->health > 0) {
            if (!ent->client) {
                return qtrue;
            } else if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) { // don't go after spectators
                return qfalse;
            } else if (ent->client->tempSpectate >= level.time) { // don't go after spectators
                return qfalse;
            } else {
                ...
                if (entTeam == NPCTEAM_FREE || NPC->client->enemyTeam == NPCTEAM_FREE || entTeam == NPC->client->enemyTeam) {
...
gentity_t *NPC_PickEnemy(gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst, qboolean findClosest) {
    ...
    if (NPCInfo->behaviorState == BS_STAND_AND_SHOOT || NPCInfo->behaviorState == BS_HUNT_AND_KILL) { // Formations guys don't require inFov to pick up a target
        ...
        if (newenemy->client && !(newenemy->flags & FL_NOTARGET) && !(newenemy->s.eFlags & EF_NODRAW)) {
            if (newenemy->health > 0) {
                ...
                {                                     // FIXME:  check for range and FOV or vis?
                    if (newenemy != NPC->lastEnemy) { // Make sure we're not just going back and forth here
                        if (trap->InPVS(&newenemy->r.currentOrigin, &NPC->r.currentOrigin)) {
                            ...
                            if (!failed) {
                                ...
                                if (newenemy->client->hiddenDist > 0) {
                                    if (relDist > newenemy->client->hiddenDist * newenemy->client->hiddenDist) {
                                        // out of hidden range
                                        if (VectorLengthSquared(&newenemy->client->hiddenDir)) { // They're only hidden from a certain direction, check
                                            float dot;
                                            VectorNormalize(&diff);
                                            dot = DotProduct(&newenemy->client->hiddenDir, &diff);
                                            if (dot > 0.5f) { // I'm not looking in the right dir toward them to see them
                                                failed = qtrue;
                                            } else {
                                                Debug_Printf(&d_npcai, DEBUG_LEVEL_INFO, "%s saw %s trying to hide - hiddenDir %s targetDir %s dot %f\n",
                                                             NPC->targetname, newenemy->targetname, vtos(&newenemy->client->hiddenDir), vtos(&diff),
                                                             (double)dot);
                                            }
                                        } else {
                                            failed = qtrue;
                                        ...
                                        Debug_Printf(&d_npcai, DEBUG_LEVEL_INFO, "%s saw %s trying to hide - hiddenDist %f\n", NPC->targetname,
                                                     newenemy->targetname, (double)newenemy->client->hiddenDist);
                                    }
                                }
    ...
    */
    ...
    for (entNum = 0; entNum < level.num_entities; entNum++) {
        ...
        if (newenemy != NPC && (newenemy->client /*|| newenemy->svFlags & SVF_NONNPC_ENEMY*/) && !(newenemy->flags & FL_NOTARGET) &&
            !(newenemy->s.eFlags & EF_NODRAW)) {
            if (newenemy->health > 0) {
                if ((newenemy->client && NPC_ValidEnemy(newenemy)) ||
                    (!newenemy->client && newenemy->alliedTeam == enemyTeam)) {                     // FIXME:  check for range and FOV or vis?
                    if (NPC->client->playerTeam == NPCTEAM_PLAYER && enemyTeam == NPCTEAM_PLAYER) { // player allies turning on ourselves?  How?
                        if (newenemy->s.number >= MAX_CLIENTS) {                                    // only turn on the player, not other player allies
                            continue;
                        }
                    }
                        ...
                        if (newenemy->client && newenemy->client->hiddenDist > 0) {
                            if (relDist > newenemy->client->hiddenDist * newenemy->client->hiddenDist) {
                                ...
                                Debug_Printf(&d_npcai, DEBUG_LEVEL_INFO, "%s saw %s trying to hide - hiddenDist %f\n", NPC->targetname, newenemy->targetname,
                                             (double)newenemy->client->hiddenDist);
                            }
                        }
        ...
        if (ally->client) {
            if (ally->health > 0) {
                ...
                {                                                               // if on same team or if player is disguised as your team
                    ...
                    if (!trap->InPVS(&ally->r.currentOrigin, &NPC->r.currentOrigin)) {
                        continue;
                    }

                    if (movingOnly && ally->client && NPC->client) { // They have to be moving relative to each other
                    ...
                    if (relDist < bestDist) {
                        if (facingEachOther) {
                            ...
                            if (dot < 0.5f) { // Not facing in dir to me
                                continue;
                            }
                            ...
                            if (dot > -0.5f) { // I'm not facing opposite of dir to me
                                continue;
                            }
                            // I am facing him
                        }
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk, qboolean setEnemy) {
    ...
    */
    ...
    if (NPC->enemy) {
        if (NPC_EnemyTooFar(NPC->enemy, 0, qfalse)) {
        ...
        } else if (!trap->InPVS(&NPC->r.currentOrigin, &NPC->enemy->r.currentOrigin)) { // FIXME: should this be a line-of site check?
            ...
            if (NPC->enemy->client && NPC->enemy->client->hiddenDist) { // He ducked into shadow while we weren't looking
    ...
    if (NPCInfo->defendEnt) {                                  // Trying to protect someone
        if (NPCInfo->defendEnt->health > 0) {                  // Still alive, We presume we're close to them, navigation should handle this?
            if (NPCInfo->defendEnt->enemy) {                   // They were shot or acquired an enemy
                if (NPC->enemy != NPCInfo->defendEnt->enemy) { // They have a different enemy, take it!
    ...
    if (!NPC->enemy || (NPC->enemy && NPC->enemy->health <= 0) || forcefindNew) { // FIXME: NPCs that are moving after an enemy should ignore the can't hit
                                                                                  // enemy counter- that should only be for NPCs that are standing still
        ...
        if (NPC->client->enemyTeam != NPCTEAM_NEUTRAL) {
            ...
            // newenemy = NPC_PickEnemy( closestTo, NPC->client->enemyTeam, (NPC->cantHitEnemyCounter > 10), qfalse, qtrue );//3rd parm was (NPC->enemyTeam ==
            // TEAM_STARFLEET) For now, made it so you ALWAYS have to check VIS
...
qboolean NPC_ClearShot(gentity_t *ent) {
    ...
    if (NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/) // any other guns to check for?
    {
        ...
        trap->Trace(&tr, &muzzle, &mins, &maxs, &ent->r.currentOrigin, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
    } else {
        trap->Trace(&tr, &muzzle, NULL, NULL, &ent->r.currentOrigin, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
    }
...
int NPC_ShotEntity(gentity_t *ent, vector3 *impactPos) {
    ...
    if (NPC->s.weapon == WP_THERMAL) { // thermal aims from slightly above head
        ...
        vector3 angles, forward, end;

        CalcEntitySpot(NPC, SPOT_HEAD, &muzzle);
        VectorSet(&angles, 0, NPC->client->ps.viewangles.y, 0);
    ...
    if (NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/) // any other guns to check for?
    {
        ...
        trap->Trace(&tr, &muzzle, &mins, &maxs, &targ, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
    } else {
        trap->Trace(&tr, &muzzle, NULL, NULL, &targ, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
    }
...
qboolean NPC_CheckCanAttack(float attack_scale, qboolean stationary) {
    ...
    if (NPC_EnemyTooFar(NPC->enemy, distanceToEnemy * distanceToEnemy, qtrue)) { // Too far away?  Do not attack
        return qfalse;
    }

    if (client->ps.weaponTime > 0) { // already waiting for a shot to fire
    ...
    if (enemyVisibility >= VIS_FOV) { // He's in our FOV
        ...
        if (attack_ok) {
            ...
            trap->Trace(&tr, &muzzle, NULL, NULL, &hitspot, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
...
static int NPC_CollectCombatPoints(const vector3 *origin, const float radius, combatPt_t *points, const int flags) {
...
int NPC_FindCombatPoint(const vector3 *position, const vector3 *avoidPosition, vector3 *enemyPosition, const uint32_t flags, const float avoidDist,
    ...
    for (j = 0; j < numPoints; j++) {
        ...
        if ((flags & CP_AVOID) && (DistanceSquared(&level.combatPoints[i].origin, position) <
                                   modifiedAvoidDist)) // was using MIN_AVOID_DISTANCE_SQUARED, not passed in modifiedAvoidDist
            continue;
        ...
        if (flags & CP_RETREAT) {
            if (flags & CP_HORZ_DIST_COLL) {
                if (pdist < DistanceHorizontalSquared(position, enemyPosition)) { // it's closer, don't use it
                    continue;
                }
            } else {
                if (pdist < DistanceSquared(position, enemyPosition)) { // it's closer, don't use it
                    continue;
                }
            }
        }
        ...
        trap->Trace(&tr, &level.combatPoints[i].origin, &NPC->r.mins, &NPC->r.maxs, &level.combatPoints[i].origin, NPC->s.number, NPC->clipmask, qfalse, 0, 0);
        ...
        if (flags & CP_HAS_ROUTE) {
            ...
            */
            ...
            if (waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE ||
                trap->Nav_GetBestNodeAltRoute2(waypoint, level.combatPoints[i].waypoint, NODE_NONE) ==
                ...
                if (!NAV_ClearPathToPoint(NPC, &NPC->r.mins, &NPC->r.maxs, &level.combatPoints[i].origin, NPC->clipmask,
...
gentity_t *NPC_SearchForWeapons(void) {
    ...
    for (i = 0; i < level.num_entities; i++) {
        ...
        if (CheckItemCanBePickedUpByNPC(found, NPC)) {
            if (trap->InPVS(&found->r.currentOrigin, &NPC->r.currentOrigin)) {
                dist = DistanceSquared(&found->r.currentOrigin, &NPC->r.currentOrigin);
                if (dist < bestDist) {
                    if (!trap->Nav_GetBestPathBetweenEnts((sharedEntity_t *)NPC, (sharedEntity_t *)found, NF_CLEAR_PATH) ||
                        trap->Nav_GetBestNodeAltRoute2(NPC->waypoint, found->waypoint, NODE_NONE) ==
                            WAYPOINT_NONE) { // can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
                        if (NAV_ClearPathToPoint(NPC, &NPC->r.mins, &NPC->r.maxs, &found->r.currentOrigin, NPC->clipmask,
...
void NPC_CheckGetNewWeapon(void) {
    if (NPC->s.weapon == WP_NONE && NPC->enemy) { // if running away because dropped weapon...
        if (NPCInfo->goalEntity && NPCInfo->goalEntity == NPCInfo->tempGoal && NPCInfo->goalEntity->enemy &&
            !NPCInfo->goalEntity->enemy->inuse) { // maybe was running at a weapon that was picked up
            NPCInfo->goalEntity = NULL;
        }
        if (TIMER_Done(NPC, "panic") && NPCInfo->goalEntity == NULL) { // need a weapon, any lying around?
...
void NPC_AimAdjust(int change) {
    ...
    if (TIMER_Done(NPC, "aimDebounce")) {
        ...
        if (NPCInfo->currentAim > NPCInfo->stats.aim) { // can never be better than max aim
            NPCInfo->currentAim = NPCInfo->stats.aim;
        } else if (NPCInfo->currentAim < -30) { // can never be worse than this
