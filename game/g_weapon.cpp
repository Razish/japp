...
float WP_SpeedOfMissileForWeapon(int wp, qboolean alt_fire) { return 500; }
...
void DEMP2_AltRadiusDamage(gentity_t *ent) {
    ...
    if (ent->r.ownerNum >= 0 && ent->r.ownerNum < /*MAX_CLIENTS ... let npc's/shooters use it*/ MAX_GENTITIES) {
            ...
            if (gent->takedamage && gent->client) {
                if (gent->client->ps.electrifyTime < level.time) {
                    ...
                    if (gent->s.eType == ET_NPC && gent->s.NPC_class == CLASS_VEHICLE && gent->m_pVehicle &&
                        (gent->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER || gent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER)) {
                        // do some extra stuff to speeders/walkers
                        gent->client->ps.electrifyTime = level.time + Q_irand(3000, 4000);
                    } else if (gent->s.NPC_class != CLASS_VEHICLE || (gent->m_pVehicle && gent->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER)) {
    ...
    // store the last fraction so that next time around we can test against those things that fall between that last point and where the current shockwave edge
    // is
    ...
    if (ent->client && ent->client->ps.rocketLockIndex != ENTITYNUM_NONE) {
        ...
        if (dif >= 10 && rTime != -1) {
            ...
            if (missile->enemy && missile->enemy->client && missile->enemy->health > 0 &&
                !OnSameTeam(ent, missile->enemy)) { // if enemy became invalid, died, or is on the same team, then don't seek it
                missile->angle = 0.5f;
...
qboolean WP_LobFire(gentity_t *self, vector3 *start, vector3 *target, vector3 *mins, vector3 *maxs, int clipmask, vector3 *velocity, qboolean tracePath,
    ...
    while (hitCount < maxHits) {
        ...
        if (tracePath) { // do a rough trace of the path
            ...
            // This may be kind of wasteful, especially on long throws... use larger steps?  Divide the travelTime into a certain hard number of slices?  Trace
            // just to apex and down?
...
void laserTrapExplode(gentity_t *self) {
    ...
    G_PlayEffect((self->s.weapon == WP_FLECHETTE) ? EFFECT_EXPLOSION_FLECHETTE : EFFECT_EXPLOSION_TRIPMINE, &self->r.currentOrigin, &v);
...
void proxMineThink(gentity_t *ent) {
    ...
    if (ent->genericValue15 < level.time || !owner || !owner->inuse || !owner->client || owner->client->pers.connected != CON_CONNECTED) {
        // time to die!
        ent->think = laserTrapExplode;
        return;
    }
...
void charge_stick(gentity_t *self, gentity_t *other, trace_t *trace) {
    ...
    if (other && (other->flags & FL_BBRUSH) && other->s.pos.trType == TR_STATIONARY && other->s.apos.trType == TR_STATIONARY) {
...
static void WP_FireConcussionAlt(gentity_t *ent) { // a rail-gun-like beam
    ...
    for (i = 0; i < traces; i++) {
        ...
        if (d_projectileGhoul2Collision.integer) {
            trap->Trace(&tr, &start, &shot_mins, &shot_maxs, &end, skip, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_HITCORPSES,
        ...
        if (d_projectileGhoul2Collision.integer && traceEnt->inuse && traceEnt->client) {
            // g2 collision checks -rww
            if (traceEnt->inuse && traceEnt->client && traceEnt->ghoul2) { // since a we used G2TRFLAG_GETSURFINDEX, tr.surfaceFlags will actually contain the
                                                                           // index of the surface on the ghoul2 model we collided with.
                traceEnt->client->g2LastSurfaceHit = tr.surfaceFlags;
                traceEnt->client->g2LastSurfaceTime = level.time;
    ...
    if (tr_ent && tr_ent->takedamage) {
        ...
        if (tr_ent->client) {
            ...
            if (tr_ent->client->NPC_class == CLASS_VEHICLE) {
                // not on vehicles
                if (!tr_ent->m_pVehicle || tr_ent->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL || tr_ent->m_pVehicle->m_pVehicleInfo->type == VH_FLIER) {
                    // can zap animals
...
qboolean LogAccuracyHit(gentity_t *target, gentity_t *attacker) {
    if (!target->takedamage || target == attacker || !target->client || !attacker || !attacker->client || target->client->ps.stats[STAT_HEALTH] <= 0) {
        return qfalse;
    }
...
void WP_CalcVehMuzzle(gentity_t *ent, int muzzleNum) {
    ...
    if (pVeh->m_pVehicleInfo &&
        (pVeh->m_pVehicleInfo->type == VH_ANIMAL || pVeh->m_pVehicleInfo->type == VH_WALKER || pVeh->m_pVehicleInfo->type == VH_SPEEDER)) {
        vehAngles.pitch = vehAngles.roll = 0;
    }

    trap->G2API_GetBoltMatrix_NoRecNoRot(ent->ghoul2, 0, pVeh->m_iMuzzleTag[muzzleNum], &boltMatrix, &vehAngles, &ent->client->ps.origin, level.time, NULL,
                                         &ent->modelScale);
...
void G_EstimateCamPos(vector3 *viewAngles, vector3 *cameraFocusLoc, float viewheight, float thirdPersonRange, float thirdPersonHorzOffset, float vertOffset,
                      float pitchOffset, int ignoreEntNum, vector3 *camPos) {
...
void WP_GetVehicleCamPos(gentity_t *ent, gentity_t *pilot, vector3 *camPos) {
    ...
    G_EstimateCamPos(&ent->client->ps.viewangles, &pilot->client->ps.origin, pilot->client->ps.viewheight, thirdPersonRange, thirdPersonHorzOffset, vertOffset,
                     pitchOffset, pilot->s.number, camPos);
}
...
qboolean WP_VehCheckTraceFromCamPos(gentity_t *ent, const vector3 *shotStart, vector3 *shotDir) {
    ...
    if (!ent || !ent->m_pVehicle || !ent->m_pVehicle->m_pVehicleInfo || !ent->m_pVehicle->m_pPilot || !((gentity_t *)ent->m_pVehicle->m_pPilot)->client ||
    ...
    if ((ent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER && g_cullDistance > MAX_XHAIR_DIST_ACCURACY) ||
        ...
        if (ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) {
            ...
            trace_t extraTrace;
            vector3 newEnd;
            int camTraceEntNum = BG_VehTraceFromCamPos(&extraTrace, (bgEntity_t *)ent, &ent->r.currentOrigin, shotStart, &end, &newEnd, shotDir,
...
void FireVehicleWeapon(gentity_t *ent, qboolean alt_fire) {
    ...
    if (pVeh->m_pVehicleInfo->type == VH_WALKER && ent->client->ps.electrifyTime > level.time) { // don't fire while being electrocuted
        return;
    }
...
static void WP_FireEmplaced(gentity_t *ent, qboolean altFire) {
    ...
    if (!gun->inuse ||
        gun->health <= 0) { // gun was removed or killed, although we should never hit this check because we should have been forced off it already
        return;
    }
...
void emplaced_gun_realuse(gentity_t *self, gentity_t *other, gentity_t *activator) { emplaced_gun_use(self, other, NULL); }
