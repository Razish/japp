...
/*
qboolean G_ClearLineOfSight(const vector3 *point1, const vector3 *point2, int ignore, int clipmask)
...
qboolean InFOV3(vector3 *spot, vector3 *from, vector3 *fromAngles, int hFOV, int vFOV) {
    vector3 deltaVector, angles, deltaAngles;
...
qboolean InFOV(gentity_t *ent, gentity_t *from, int hFOV, int vFOV) {
    ...
    vector3 spot;
    vector3 deltaVector;
    vector3 angles, fromAngles;
    vector3 deltaAngles;

    if (from->client) {
        if (!VectorCompare(&from->client->renderInfo.eyeAngles, &vec3_origin)) { // Actual facing of tag_head!
...
qboolean InVisrange(gentity_t *ent) { // FIXME: make a calculate visibility for ents that takes into account
    ...
    {
    float	vel, avel;
    if(ent->client->ps.velocity.x || ent->client->ps.velocity.y || ent->client->ps.velocity[2])
    {
    ...
    if(ent->avelocity.x || ent->avelocity.y || ent->avelocity[2])
...
static int G_CheckSoundEvents(gentity_t *self, float maxHearDist, int ignoreAlert, qboolean mustHaveOwner, alertEventLevel_t minAlertLevel) {
    ...
    for (i = 0; i < level.numAlertEvents; i++) {
        ...
        if (level.alertEvents[i].addLight) {                                   // a quiet sound, must have LOS to hear it
...
float G_GetLightLevel(vector3 *pos, vector3 *fromDir) {
    /*
    vector3	ambient={0}, directed, lightDir;
...
int G_CheckAlertEvents(gentity_t *self, qboolean checkSight, qboolean checkSound, float maxSeeDist, float maxHearDist, int ignoreAlert, qboolean mustHaveOwner,
                       int minAlertLevel) {
    ...
    if (bestSightEvent >= 0 && bestSightAlert > bestSoundAlert) { // valid best sight event, more important than the sound event
        // get the light level of the alert event for this checker
        vector3 eyePoint, sightDir;
        // get eye point
        ...
        level.alertEvents[bestSightEvent].light =
            level.alertEvents[bestSightEvent].addLight + G_GetLightLevel(&level.alertEvents[bestSightEvent].position, &sightDir);
...
qboolean G_CheckForDanger(gentity_t *self, int alertEvent) { // FIXME: more bStates need to call this?
    if (alertEvent == -1) {
        return qfalse;
    }
    ...
    if (level.alertEvents[alertEvent].level >= AEL_DANGER) { // run away!
        if (!level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client ||
            (level.alertEvents[alertEvent].owner != self && level.alertEvents[alertEvent].owner->client->playerTeam != self->client->playerTeam)) {
            if (self->NPC) {
                if (self->NPC->scriptFlags & SCF_DONT_FLEE) { // can't flee
                    return qfalse;
                } else {
                    NPC_StartFlee(level.alertEvents[alertEvent].owner, &level.alertEvents[alertEvent].position, level.alertEvents[alertEvent].level, 3000,
                                  6000);
                    return qtrue;
                }
            } else {
...
qboolean NPC_CheckForDanger(int alertEvent) { // FIXME: more bStates need to call this?
...
void AddSoundEvent(gentity_t *owner, vector3 *position, float radius, alertEventLevel_t alertLevel, qboolean needLOS) {
    ...
    if (needLOS) {                                            // a very low-level sound, when check this sound event, check for LOS
...
void ClearPlayerAlertEvents(void) {
    ...
    for (i = 0; i < curNumAlerts; i++) {
        ...
        if (level.alertEvents[i].timestamp && level.alertEvents[i].timestamp + ALERT_CLEAR_TIME < level.time) { // this event has timed out
            // drop the count
            level.numAlertEvents--;
            // shift the rest down
            if (level.numAlertEvents > 0) { // still have more in the array
                if ((i + 1) < MAX_ALERT_EVENTS) {
                    memmove(&level.alertEvents[i], &level.alertEvents[i + 1], sizeof(alertEvent_t) * (MAX_ALERT_EVENTS - (i + 1)));
                }
            } else { // just clear this one... or should we clear the whole array?
    ...
    if (eventClearTime <
        level.time) { // this is just a 200ms debouncer so things that generate constant alerts (like corpses and missiles) add an alert every 200 ms
    ...
    for (i = 0; i < level.numInterestPoints; i++) {
        ...
        if (trap->InPVS(&level.interestPoints[i].origin, &eyes)) {
            ...
            if ((fabsf(diffVec.x) + fabsf(diffVec.y)) / 2 < 48 &&
                fabsf(diffVec.z) > (fabsf(diffVec.x) + fabsf(diffVec.y)) / 2) { // Too close to look so far up or down
                continue;
            }
...
void SP_target_interest(gentity_t *self) { // FIXME: rename point_interest
