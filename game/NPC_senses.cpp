...
void ClearPlayerAlertEvents( void ) {
	...
	if ( eventClearTime < level.time ) {//this is just a 200ms debouncer so things that generate constant alerts (like corpses and missiles) add an alert every 200 ms
...
qboolean G_ClearLOS( gentity_t *self, const vector3 *start, const vector3 *end ) {
	...
	while ( tr.fraction < 1.0f && traceCount < 3 ) {//can see through 3 panes of glass
		if ( tr.entityNum < ENTITYNUM_WORLD ) {
			if ( &g_entities[tr.entityNum] != NULL && (g_entities[tr.entityNum].r.svFlags&SVF_GLASS_BRUSH) ) {//can see through glass, trace again, ignoring me
