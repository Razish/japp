...
int adjustRespawnTime( float preRespawnTime, int itemType, int itemTag ) {
	...
	if ( itemType == IT_WEAPON ) {
		if ( itemTag == WP_THERMAL ||
			...
			itemTag == WP_DET_PACK ) { //special case for these, use ammo respawn rate
...
static qboolean pas_find_enemies( gentity_t *self ) {
	...
	for ( i = 0; i < count; i++ ) {
		...
		if ( target->s.eType == ET_NPC &&
			target->s.NPC_class == CLASS_VEHICLE ) { //don't get mad at vehicles, silly.
			continue;
		}
...
void ItemUse_Jetpack( gentity_t *ent ) {
	...
	if ( !ent->client->jetPackOn &&
		ent->client->ps.jetpackFuel < 5 ) { //too low on fuel to start it up
		return;
	}
...
void ItemUse_UseCloak( gentity_t *ent ) {
	...
	if ( !ent->client->ps.powerups[PW_CLOAKED] &&
		ent->client->ps.cloakFuel < 5 ) { //too low on fuel to start it up
		return;
	}
...
void ItemUse_UseDisp( gentity_t *ent, int type ) {
	...
	if ( !ent->client ||
		ent->client->tossableItemDebounce > level.time ) { //can't use it again yet
		return;
	}

	if ( ent->client->ps.weaponTime > 0 ||
		ent->client->ps.forceHandExtend != HANDEXTEND_NONE ) { //busy doing something else
		return;
	}
	...
	if ( self->r.ownerNum != ENTITYNUM_NONE ) {
		...
		if ( owner->inuse && owner->client ) {
			...
			if ( owner->client->ps.stats[STAT_HOLDABLE_ITEM] > 0 &&
				...
				bg_itemlist[owner->client->ps.stats[STAT_HOLDABLE_ITEM]].giTag == HI_EWEB ) { //he has it selected so deselect it and select the first thing available
...
void EWebUpdateBoneAngles( gentity_t *owner, gentity_t *eweb ) {
	...
	if ( incr > turnCap )	incr = turnCap;
	else if ( incr < -turnCap )	incr = -turnCap;
...
qboolean CheckItemCanBePickedUpByNPC( gentity_t *item, gentity_t *pickerupper ) {
	if ( (item->flags&FL_DROPPED_ITEM)
		...
		/*&& item->item->giTag != INV_SECURITY_KEY*/ ) {//non-player, in combat, picking up a dropped item that does NOT belong to the player and it *not* a security key
...
void G_BounceItem( gentity_t *ent, trace_t *trace ) {
	...
	if ( ent->s.eType == ET_HOLOCRON ||
		(ent->s.shouldtarget && ent->s.eType == ET_GENERAL && ent->physicsObject) ) { //holocrons and sentry guns
		if ( ent->touch ) {
			ent->touch( ent, &g_entities[trace->entityNum], trace );
		}
	}
