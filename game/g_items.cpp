...
int adjustRespawnTime(float preRespawnTime, int itemType, int itemTag) {
    ...
    if (itemType == IT_WEAPON) {
        if (itemTag == WP_THERMAL || itemTag == WP_TRIP_MINE || itemTag == WP_DET_PACK) { // special case for these, use ammo respawn rate
    ...
    if (level.numPlayingClients > 4) {      // Start scaling the respawn times.
        if (level.numPlayingClients > 32) { // 1/4 time minimum.
            respawnTime *= 0.25f;
        } else if (level.numPlayingClients > 12) { // From 12-32, scale from 0.5f to 0.25f;
            respawnTime *= 20.0f / (float)(level.numPlayingClients + 8);
        } else { // From 4-12, scale from 1.0f to 0.5f;
            respawnTime *= 8.0f / (float)(level.numPlayingClients + 4);
        }
    }

    if (respawnTime < 1.0f) { // No matter what, don't go lower than 1 second, or the pickups become very noisy!
        respawnTime = 1.0f;
    }
...
void ShieldTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
    if (level.gametype >= GT_TEAM) { // let teammates through
    ...
    } else { // let the person who dropped the shield through
...
void CreateShield(gentity_t *ent) {
    trace_t tr;
    vector3 mins, maxs, end, posTraceEnd, negTraceEnd, start;
    ...
    {
        posTraceEnd.y += MAX_SHIELD_HALFWIDTH;
        negTraceEnd.y -= MAX_SHIELD_HALFWIDTH;
        xaxis = qfalse;
    } else // shield runs along x-axis
    {
        posTraceEnd.x += MAX_SHIELD_HALFWIDTH;
        negTraceEnd.x -= MAX_SHIELD_HALFWIDTH;
        xaxis = qtrue;
    }
    ...
    if (xaxis)
        ent->r.currentOrigin.x = ent->r.currentOrigin.x - negWidth + halfWidth;
    else
        ent->r.currentOrigin.y = ent->r.currentOrigin.y - negWidth + halfWidth;
    ent->r.currentOrigin.z += (height >> 1);
...
qboolean PlaceShield(gentity_t *playerent) {
    ...
    trace_t tr;
    vector3 fwd, pos, dest, mins = {-4, -4, 0}, maxs = {4, 4, 4};
    ...
    if (tr.fraction > 0.9f) { // room in front
        ...
        if (!tr.startsolid && !tr.allsolid) {
            ...
            if (fabsf(fwd.x) > fabsf(fwd.y)) { // shield is north/south, facing east.
...
void ItemUse_Binoculars(gentity_t *ent) {
    ...
    if (ent->client->ps.weaponstate != WEAPON_READY) { // So we can't fool it and reactivate while switching to the saber or something.
        return;
    }
...
void ItemUse_Shield(gentity_t *ent) { PlaceShield(ent); }

#define PAS_DAMAGE 2

void SentryTouch(gentity_t *ent, gentity_t *other, trace_t *trace) { return; }
...
static qboolean pas_find_enemies(gentity_t *self) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (target->s.eType == ET_NPC && target->s.NPC_class == CLASS_VEHICLE) { // don't get mad at vehicles, silly.
            continue;
        }
...
void sentryExpire(gentity_t *self) { turret_die(self, self, self, 1000, MOD_UNKNOWN); }

void pas_think(gentity_t *ent) {
    qboolean moved;
    float diffYaw, diffPitch;
    vector3 enemyDir, org;
    vector3 frontAngles, backAngles;
    vector3 desiredAngles;
    int iEntityList[MAX_GENTITIES];
    int numListedEntities;
    int i = 0;
    qboolean clTrapped = qfalse;
    vector3 testMins, testMaxs;

    testMins.x = ent->r.currentOrigin.x + ent->r.mins.x + 4;
    testMins.y = ent->r.currentOrigin.y + ent->r.mins.y + 4;
    testMins.z = ent->r.currentOrigin.z + ent->r.mins.z + 4;

    testMaxs.x = ent->r.currentOrigin.x + ent->r.maxs.x - 4;
    testMaxs.y = ent->r.currentOrigin.y + ent->r.maxs.y - 4;
    testMaxs.z = ent->r.currentOrigin.z + ent->r.maxs.z - 4;

    numListedEntities = trap->EntitiesInBox(&testMins, &testMaxs, iEntityList, MAX_GENTITIES);

    while (i < numListedEntities) {
        if (iEntityList[i] < MAX_CLIENTS) { // client stuck inside me. go nonsolid.
...
void ItemUse_Sentry(gentity_t *ent) {
    ...
    fwdorg.x = ent->client->ps.origin.x + fwd.x * 64;
    fwdorg.y = ent->client->ps.origin.y + fwd.y * 64;
    fwdorg.z = ent->client->ps.origin.z + fwd.z * 64;
    ...
    sentry->r.absmin.x = sentry->s.pos.trBase.x + sentry->r.mins.x;
    sentry->r.absmin.y = sentry->s.pos.trBase.y + sentry->r.mins.y;
    sentry->r.absmin.z = sentry->s.pos.trBase.z + sentry->r.mins.z;
    sentry->r.absmax.x = sentry->s.pos.trBase.x + sentry->r.maxs.x;
    sentry->r.absmax.y = sentry->s.pos.trBase.y + sentry->r.maxs.y;
    sentry->r.absmax.z = sentry->s.pos.trBase.z + sentry->r.maxs.z;
...
void ItemUse_MedPack_Big(gentity_t *ent) { MedPackGive(ent, MAX_MEDPACK_BIG_HEAL_AMOUNT); }

void ItemUse_MedPack(gentity_t *ent) { MedPackGive(ent, MAX_MEDPACK_HEAL_AMOUNT); }

#define JETPACK_TOGGLE_TIME 1000
void Jetpack_Off(gentity_t *ent) { // create effects?
    assert(ent && ent->client);

    if (!ent->client->jetPackOn) { // aready off
        return;
    }
...
void Jetpack_On(gentity_t *ent) { // create effects?
    assert(ent && ent->client);

    if (ent->client->jetPackOn) { // aready on
        return;
    }

    if (ent->client->ps.fd.forceGripBeingGripped >= level.time) { // can't turn on during grip interval
        return;
    }

    if (ent->client->ps.fallingToDeath) { // too late!
        return;
    }
...
void ItemUse_Jetpack(gentity_t *ent) {
    ...
    if (ent->health <= 0 || ent->client->ps.stats[STAT_HEALTH] <= 0 || (ent->client->ps.eFlags & EF_DEAD) ||
        ent->client->ps.pm_type == PM_DEAD) { // can't use it when dead under any circumstances.
        return;
    }

    if (!ent->client->jetPackOn && ent->client->ps.jetpackFuel < 5) { // too low on fuel to start it up
        return;
    }
...
void ItemUse_UseCloak(gentity_t *ent) {
    ...
    if (!ent->client->ps.powerups[PW_CLOAKED] && ent->client->ps.cloakFuel < 5) { // too low on fuel to start it up
        return;
    }
...
void ItemUse_UseDisp(gentity_t *ent, int type) {
    ...
    if (!ent->client || ent->client->tossableItemDebounce > level.time) { // can't use it again yet
        return;
    }

    if (ent->client->ps.weaponTime > 0 || ent->client->ps.forceHandExtend != HANDEXTEND_NONE) { // busy doing something else
        return;
    }
...
void EWebDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
    ...
    if (self->r.ownerNum != ENTITYNUM_NONE) {
        ...
        if (owner->inuse && owner->client) {
            ...
            if (owner->client->ps.stats[STAT_HOLDABLE_ITEM] > 0 && bg_itemlist[owner->client->ps.stats[STAT_HOLDABLE_ITEM]].giType == IT_HOLDABLE &&
                bg_itemlist[owner->client->ps.stats[STAT_HOLDABLE_ITEM]].giTag ==
                    HI_EWEB) { // he has it selected so deselect it and select the first thing available
...
void EWeb_SetBoneAngles(gentity_t *ent, const char *bone, vector3 *angles) {
    ...
    while (thebone) {
        if (!*thebone && !firstFree) { // if the value is 0 then this index is clear, we can use it if we don't find the bone we want already existing.
            firstFree = thebone;
            freeBoneVec = boneVector;
        } else if (*thebone) {
            if (*thebone == boneIndex) { // this is it
                break;
            }
        }
    ...
    if (!thebone) {       // didn't find it, create it
        if (!firstFree) { // no free bones.. can't do a thing then.
...
void EWeb_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame) {
    ...
    if (eweb->s.torsoAnim == startFrame && eweb->s.legsAnim == endFrame) { // already playing this anim, let's flag it to restart
    ...
    assert(eweb->ghoul2);
    trap->G2API_SetBoneAnim(eweb->ghoul2, 0, "model_root", startFrame, endFrame, (BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND), 1.0f, level.time, -1, 100);
}
...
void EWebFire(gentity_t *owner, gentity_t *eweb) {
    ...
    if (eweb->genericValue10 == -1) { // oh no
        assert(!"Bad e-web bolt");
        return;
    }

    // get the muzzle point
    trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue10, &boltMatrix, &eweb->s.apos.trBase, &eweb->r.currentOrigin, level.time, NULL,
                              &eweb->modelScale);
...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb) {
    ...
    trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue9, &boltMatrix, &eweb->s.apos.trBase, &eweb->r.currentOrigin, level.time, NULL,
                              &eweb->modelScale);
    BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &p);
    BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_X, &d);

    VectorMA(&p, 32.0f, &d, &p);
    p.z = eweb->r.currentOrigin.z;

    p.z += 4.0f;

    trap->Trace(&tr, &owner->client->ps.origin, &owner->r.mins, &owner->r.maxs, &p, owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);

    if (!tr.startsolid && !tr.allsolid && tr.fraction == 1.0f) { // all clear, we can move there
        vector3 pDown;
        ...
        if (!tr.startsolid && !tr.allsolid) {
            ...
            if (VectorLength(&d) > 1.0f) { // we moved, do some animating
                vector3 dAng;
                ...
                if (dAng.yaw > 0.0f) {
                    if (owner->client->ps.legsAnim == BOTH_STRAFE_RIGHT1) { // reset to change direction
                        aFlags |= SETANIM_FLAG_OVERRIDE;
                    }
                    G_SetAnim(owner, &owner->client->pers.cmd, SETANIM_LEGS, BOTH_STRAFE_LEFT1, aFlags, 0);
                } else {
                    if (owner->client->ps.legsAnim == BOTH_STRAFE_LEFT1) { // reset to change direction
                        aFlags |= SETANIM_FLAG_OVERRIDE;
                    }
                    G_SetAnim(owner, &owner->client->pers.cmd, SETANIM_LEGS, BOTH_STRAFE_RIGHT1, aFlags, 0);
                }
            } else if (owner->client->ps.legsAnim == BOTH_STRAFE_RIGHT1 || owner->client->ps.legsAnim == BOTH_STRAFE_LEFT1) { // don't keep animating in place
    ...
    } else { // can't move here.. stop using the thing I guess
...
void EWebUpdateBoneAngles(gentity_t *owner, gentity_t *eweb) {
    ...
    if (!owner->client->ewebIndex) { // was removed during position function
        return;
    }

    VectorClear(&yAng);
    yAng.z = AngleSubtract(owner->client->ps.viewangles.pitch, eweb->s.angles.pitch) * 0.8f;
...
void EWebThink(gentity_t *self) {
    ...
    if (self->r.ownerNum == ENTITYNUM_NONE) {
        ...
        if (!owner->inuse || !owner->client || owner->client->pers.connected != CON_CONNECTED || owner->client->ewebIndex != self->s.number ||
            owner->health < 1) {
            killMe = qtrue;
        } else if (owner->client->ps.emplacedIndex != self->s.number) { // just go back to the inventory then
            EWebDisattach(owner, self);
            return;
        }
        ...
        if (!killMe) {
            ...
            if (BG_EmplacedView(&owner->client->ps.viewangles, &self->s.angles, &yaw, self->s.origin2.pitch)) {
            ...
            if (self->genericValue8 < level.time) { // make sure the anim timer is done
                ...
                if (owner->client->pers.cmd.buttons & BUTTON_ATTACK) {
                    if (self->genericValue5 < level.time) { // we can fire another shot off
                        EWebFire(owner, self);
                ...
                } else if (self->genericValue5 < level.time && self->genericValue3) { // reset the anim back to non-firing
    ...
    if (killMe) { // something happened to the owner, let's explode
...
gentity_t *EWeb_Create(gentity_t *spawner) {
    ...
    gentity_t *ent;
    trace_t tr;
    vector3 fAng, fwd, pos, downPos, s;
    vector3 mins, maxs;

    VectorSet(&mins, -32, -32, -24);
    VectorSet(&maxs, 32, 32, 24);

    VectorSet(&fAng, 0, spawner->client->ps.viewangles.y, 0);
    ...
    if (tr.allsolid || tr.startsolid || tr.fraction != 1.0f) { // can't spawn here, we are in solid
    ...
    if (tr.startsolid || tr.allsolid || tr.fraction == 1.0f || tr.entityNum < ENTITYNUM_WORLD) { // didn't hit ground.
        G_FreeEntity(ent);
        G_Sound(spawner, CHAN_AUTO, failSound);
        return NULL;
    }
    ...
    if (spawner->client->ewebHealth <= 0) { // refresh the owner's e-web health if its last e-web did not exist or was killed
    ...
    if (!ent->ghoul2) { // should not happen, but just to be safe.
        G_FreeEntity(ent);
        return NULL;
    }
    ...
    ent->s.origin2.pitch = 360.0f; // 360 degrees in either direction
...
void ItemUse_UseEWeb(gentity_t *ent) {
    if (ent->client->ewebTime > level.time) { // can't use again yet
        return;
    }
    ...
    if (ent->client->ps.emplacedIndex && !ent->client->ewebIndex) { // using an emplaced gun already that isn't our own e-web
        return;
    }

    if (ent->client->ewebIndex) { // put it away
        EWebDisattach(ent, &g_entities[ent->client->ewebIndex]);
    } else { // create it
        gentity_t *eweb = EWeb_Create(ent);

        if (eweb) { // if it's null the thing couldn't spawn (probably no room)
...
int Pickup_Ammo(gentity_t *ent, gentity_t *other) {
    ...
    if (ent->item->giTag == -1) {       // an ammo_all, give them a bit of everything
...
qboolean CheckPushItem(gentity_t *ent) {
    ...
    if (ent->item->giType == IT_AMMO || ent->item->giType == IT_HEALTH || ent->item->giType == IT_ARMOR || ent->item->giType == IT_HOLDABLE) {
        return qtrue; // these don't have placeholders
    }

    if (ent->item->giType == IT_WEAPON || (japp_itemPush.integer == 2 && ent->item->giType == IT_TEAM) || ent->item->giType == IT_POWERUP) {
...
qboolean CheckItemCanBePickedUpByNPC(gentity_t *item, gentity_t *pickerupper) {
    if ((item->flags & FL_DROPPED_ITEM) && (item->activator && item->activator->s.number >= MAX_CLIENTS) && pickerupper->s.number &&
        pickerupper->s.weapon == WP_NONE && pickerupper->enemy && pickerupper->painDebounceTime < level.time && pickerupper->NPC &&
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
    ...
    if (ent->genericValue10 > level.time && other &&
        other->s.number == ent->genericValue11) { // this is the ent that we don't want to be able to touch us for x seconds
        return;
    }
    ...
    if (ent->item && ent->item->giType == IT_WEAPON && ent->s.powerups && (int)ent->s.powerups < level.time) {
        ent->s.generic1 = 0;
        ent->s.powerups = 0;
    }
    ...
    if (ent->item->giType == IT_POWERUP && (ent->item->giTag == PW_FORCE_ENLIGHTENED_LIGHT || ent->item->giTag == PW_FORCE_ENLIGHTENED_DARK)) {
    ...
    if (other->client->NPC_class == CLASS_ATST || other->client->NPC_class == CLASS_GONK || other->client->NPC_class == CLASS_MARK1 ||
        other->client->NPC_class == CLASS_MARK2 || other->client->NPC_class == CLASS_MOUSE || other->client->NPC_class == CLASS_PROBE ||
        other->client->NPC_class == CLASS_PROTOCOL || other->client->NPC_class == CLASS_R2D2 || other->client->NPC_class == CLASS_R5D2 ||
        other->client->NPC_class == CLASS_SEEKER || other->client->NPC_class == CLASS_REMOTE || other->client->NPC_class == CLASS_RANCOR ||
        ...
        other->client->NPC_class == CLASS_SENTRY) {   // FIXME: some flag would be better
        // droids can't pick up items/weapons!
        return;
    }

    if (CheckItemCanBePickedUpByNPC(ent, other)) {
        if (other->NPC && other->NPC->goalEntity && other->NPC->goalEntity->enemy == ent) { // they were running to pick me up, they did, so clear goal
            other->NPC->goalEntity = NULL;
            other->NPC->squadState = SQUAD_STAND_AND_SHOOT;
        }
    } else if (!(ent->spawnflags & ITMSF_ALLOWNPC)) { // NPCs cannot pick it up
        if (other->s.eType == ET_NPC) {               // Not the player?
            qboolean dontGo = qfalse;
            if (ent->item->giType == IT_AMMO && ent->item->giTag == -1 && other->s.NPC_class == CLASS_VEHICLE && other->m_pVehicle &&
                other->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) { // yeah, uh, atst gets healed by these things
                if (other->maxHealth && other->health < other->maxHealth) {
                    other->health += 80;
    ...
    if (ent->genericValue9) { // dropped item, should be removed when picked up
        ent->think = G_FreeEntity;
        ent->nextthink = level.time;
        return;
    }
...
gentity_t *LaunchItem_Drop(const gitem_t *item, vector3 *origin, vector3 *dir) {
    ...
    vectoangles(dir, &dropped->s.angles);
    dropped->s.angles.pitch = 0;
...
gentity_t *Drop_Item(gentity_t *ent, const gitem_t *item, float angle) {
    ...
    // make sure items don't get stuck in the wall
    VectorMA(&ent->s.pos.trBase, 64.0f, &velocity, &newOrigin);
...
void FinishSpawningItem(gentity_t *ent) {
    ...
    if (level.gametype == GT_SIEGE) { // in siege remove all powerups
        if (ent->item->giType == IT_POWERUP) {
            G_FreeEntity(ent);
            return;
        }
    ...
    if (level.gametype == GT_HOLOCRON) {
        if (ent->item->giType == IT_POWERUP) {
            if (ent->item->giTag == PW_FORCE_ENLIGHTENED_LIGHT || ent->item->giTag == PW_FORCE_ENLIGHTENED_DARK) {
                G_FreeEntity(ent);
                return;
            }
        }
    ...
    if (g_forcePowerDisable.integer && !japp_alwaysSpawnPowerups.integer) { // if force powers disabled, don't add force powerups
        if (ent->item->giType == IT_POWERUP) {
            if (ent->item->giTag == PW_FORCE_ENLIGHTENED_LIGHT || ent->item->giTag == PW_FORCE_ENLIGHTENED_DARK || ent->item->giTag == PW_FORCE_BOON) {
                G_FreeEntity(ent);
                return;
            }
        }
    ...
    if (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) {
        if (ent->item->giType == IT_ARMOR || ent->item->giType == IT_HEALTH ||
...
void ClearRegisteredItems(void) {
    ...
    if (level.gametype == GT_SIEGE) { // kind of cheesy, maybe check if siege class with disp's is gonna be on this map too
...
void G_BounceItem(gentity_t *ent, trace_t *trace) {
    ...
    if ((ent->s.weapon == WP_DET_PACK && ent->s.eType == ET_GENERAL && ent->physicsObject)) { // detpacks only
        ...
        JPLua::Entity_CallFunction(ent, JPLua::JPLUA_ENTITY_TOUCH, (intptr_t)&g_entities[trace->entityNum], (intptr_t)&trace);
        return;
    }
    ...
    if (ent->s.eType == ET_HOLOCRON || (ent->s.shouldtarget && ent->s.eType == ET_GENERAL && ent->physicsObject)) { // holocrons and sentry guns
        if (ent->touch) {
            ent->touch(ent, &g_entities[trace->entityNum], trace);
        }
        JPLua::Entity_CallFunction(ent, JPLua::JPLUA_ENTITY_TOUCH, (intptr_t)&g_entities[trace->entityNum], (intptr_t)&trace);
