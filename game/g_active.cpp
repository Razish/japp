...
qboolean G_ActionButtonPressed(int buttons) {
    return !!(buttons & (BUTTON_ATTACK | BUTTON_USE_HOLDABLE | BUTTON_GESTURE | BUTTON_USE | BUTTON_FORCEGRIP | BUTTON_ALT_ATTACK | BUTTON_FORCEPOWER |
                         BUTTON_FORCE_LIGHTNING | BUTTON_FORCE_DRAIN));
}
...
void G_CheckClientIdle(gentity_t *ent, usercmd_t *ucmd) {
    ...
    if (!ent || !ent->client || ent->health <= 0 || ent->client->ps.stats[STAT_HEALTH] <= 0 || ent->client->sess.sessionTeam == TEAM_SPECTATOR ||
    ...
    if (!VectorCompare(&vec3_origin, &ent->client->ps.velocity) || actionPressed || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove ||
        !G_StandingAnim(ent->client->ps.legsAnim) || (ent->health + ent->client->ps.stats[STAT_ARMOR]) != ent->client->idleHealth ||
        VectorLength(&viewChange) > 10 || ent->client->ps.legsTimer > 0 || ent->client->ps.torsoTimer > 0 || ent->client->ps.weaponTime > 0 ||
        ent->client->ps.weaponstate == WEAPON_CHARGING || ent->client->ps.weaponstate == WEAPON_CHARGING_ALT || ent->client->ps.zoomMode ||
        (ent->client->ps.weaponstate != WEAPON_READY && ent->client->ps.weapon != WP_SABER) || ent->client->ps.forceHandExtend != HANDEXTEND_NONE ||
        ent->client->ps.saberBlocked != BLOCKED_NONE || ent->client->ps.saberBlocking >= level.time || ent->client->ps.weapon == WP_MELEE ||
        (ent->client->ps.weapon != ent->client->pers.cmd.weapon && ent->s.eType != ET_NPC)) { // FIXME: also check for turning?
        qboolean brokeOut = qfalse;

        if (!VectorCompare(&vec3_origin, &ent->client->ps.velocity) || actionPressed || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove ||
            (ent->health + ent->client->ps.stats[STAT_ARMOR]) != ent->client->idleHealth || ent->client->ps.zoomMode ||
            (ent->client->ps.weaponstate != WEAPON_READY && ent->client->ps.weapon != WP_SABER) ||
            (ent->client->ps.weaponTime > 0 && ent->client->ps.weapon == WP_SABER) || ent->client->ps.weaponstate == WEAPON_CHARGING ||
            ent->client->ps.weaponstate == WEAPON_CHARGING_ALT || ent->client->ps.forceHandExtend != HANDEXTEND_NONE ||
            ent->client->ps.saberBlocked != BLOCKED_NONE || ent->client->ps.saberBlocking >= level.time || ent->client->ps.weapon == WP_MELEE ||
        ...
        if (brokeOut && (ent->client->ps.weaponstate == WEAPON_CHARGING || ent->client->ps.weaponstate == WEAPON_CHARGING_ALT)) {
...
void G_CheckMovingLoopingSounds(gentity_t *ent, usercmd_t *ucmd) {
    if (ent->client) {
            ...
            || (FlyingCreature(ent) && !VectorCompare(&vec3_origin, &ent->client->ps.velocity) && ent->health > 0)) // flier using velocity to move
        {
            ...
            if (ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_R5D2 || ent->client->NPC_class == CLASS_MARK2 ||
                ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_PROBE) {
                ent->s.loopSound = 0;
            }
        }
    }
        ...
        if (monster && monster->client) {
            ...
            if (monster->s.NPC_class == CLASS_RANCOR) { // only possibility right now, may add Wampa and Sand Creature later
                BG_AttachToRancor(monster->ghoul2,      // ghoul2 info
                                  monster->r.currentAngles.yaw, &monster->r.currentOrigin, level.time, NULL, &monster->modelScale,
                                  (monster->client->ps.eFlags2 & EF2_GENERIC_NPC_FLAG), &ent->client->ps.origin, &ent->client->ps.viewangles, NULL);
            }
...
void G_SetTauntAnim(gentity_t *ent, int taunt) {
    if (ent->client->pers.cmd.upmove || ent->client->pers.cmd.forwardmove || ent->client->pers.cmd.rightmove) { // hack, don't do while moving
        return;
    }

    BG_ClearRocketLock(&ent->client->ps);

    if (ent->client->ps.torsoTimer < 1 && ent->client->ps.forceHandExtend == HANDEXTEND_NONE && ent->client->ps.legsTimer < 1 &&
        ent->client->ps.weaponTime < 1 && ent->client->ps.saberLockTime < level.time) {
        int anim = -1;
        switch (taunt) {
        case TAUNT_TAUNT:
...
void ClientThink_real(gentity_t *ent) {
    ...
    if (ent->s.number < MAX_CLIENTS && ent->client->ps.m_iVehicleNum) { // driving a vehicle
        if (g_entities[ent->client->ps.m_iVehicleNum].client) {
            ...
            if (veh->m_pVehicle && veh->m_pVehicle->m_pPilot == (bgEntity_t *)ent) { // only take input from the pilot...
    ...
    if (!(client->ps.pm_flags & PMF_FOLLOW)) {
        if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
            ...
            if (!(bgSiegeClasses[client->siegeClass].saberStance &
                  (1 << client->ps.fd.saberAnimLevel))) { // the current stance is not in the bitmask, so find the first one that is.
                int i = SS_FAST;

                while (i < SS_NUM_SABER_STYLES) {
                    if (bgSiegeClasses[client->siegeClass].saberStance & (1 << i)) {
                        if (i == SS_DUAL && client->ps.saberHolstered == 1) { // one saber should be off, adjust saberAnimLevel accordinly
            ...
            if (client->ps.saberHolstered == 1) {                            // one saber should be off, adjust saberAnimLevel accordinly
                ...
                if (!WP_SaberStyleValidForSaber(&client->saber[0], &client->saber[1], client->ps.saberHolstered,
                                                client->ps.fd.saberAnimLevel)) { // only use dual style if the style we're trying to use isn't valid
    ...
    if (level.intermissiontime) {
        if (ent->s.number < MAX_CLIENTS || client->NPC_class == CLASS_VEHICLE) { // players and vehicles do nothing in intermissions
...
#define MIN_NPC_SPEED 16.0f
    ...
    if (client->bodyGrabIndex != ENTITYNUM_NONE) {
        ...
        if (!grabbed->inuse || grabbed->s.eType != ET_BODY || (grabbed->s.eFlags & EF_DISINTEGRATION) || (grabbed->s.eFlags & EF_NODRAW)) {
            ...
            trap->G2API_GetBoltMatrix(ent->ghoul2, 0, 0, &rhMat, &tAng, &ent->client->ps.origin, level.time, NULL,
                                      &ent->modelScale); // 0 is always going to be right hand bolt
    ...
    if (!ent->NPC || !(ent->NPC->aiFlags & NPCAI_CUSTOM_GRAVITY)) {                                                         // use global gravity
        if (ent->NPC && ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle && ent->m_pVehicle->m_pVehicleInfo->gravity) { // use custom veh gravity
    ...
    if (ent->client->ps.duelInProgress) {
        ...
        if (ent->client->ps.duelTime < level.time) {
            ...
            if (japp_duelActivateSaber.integer && duelAgainst && duelAgainst->client && duelAgainst->inuse && duelAgainst->client->ps.weapon == WP_SABER &&
                duelAgainst->client->ps.saberHolstered && duelAgainst->client->ps.duelTime) {
        ...
        if (!duelAgainst || !duelAgainst->client || !duelAgainst->inuse || duelAgainst->client->ps.duelIndex != ent->s.number) {
            ...
            if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0) {
                ...
                Com_sprintf(pre2, sizeof(pre2), "You have defeated %s", loser);
                ...
                if (japp_duelStats.bits & DUELSTATS_HEALTH) {
                    ...
                    Q_strcat(buf, sizeof(buf),
                             va(" " S_COLOR_WHITE "with " S_COLOR_RED "%i" S_COLOR_WHITE "/" S_COLOR_GREEN "%i " S_COLOR_WHITE "remaining", health, armor));
                }
                ...
                if (japp_duelStats.bits & DUELSTATS_TIME) {
                    ...
                    Q_strcat(buf, sizeof(buf), va(" " S_COLOR_WHITE "in " S_COLOR_CYAN "%i" S_COLOR_WHITE ":" S_COLOR_CYAN "%02i", mins, secs));
                }

                // with y hits
                if (japp_duelStats.bits & DUELSTATS_HITS) {
                    const int hits = ent->duelHitCount;
                    Q_strcat(buf, sizeof(buf), va(" " S_COLOR_WHITE "with " S_COLOR_YELLOW "%i" S_COLOR_WHITE " hits", hits));
                }
                ...
                trap->SendServerCommand(-1, va("print \"%s: %s " S_COLOR_WHITE "vs %s!\n\"", G_GetStringEdString("MP_SVGAME", "PLDUELTIE"),
    ...
    if (ent->client && ent->client->ps.fallingToDeath && (level.time - FALL_FADE_TIME) > ent->client->ps.fallingToDeath) { // die!
        if (ent->health > 0) {
            gentity_t *otherKiller = ent;
            if (ent->client->ps.otherKillerTime > level.time && ent->client->ps.otherKiller != ENTITYNUM_NONE) {
    ...
    if (ent->client->ps.otherKillerTime > level.time && ent->client->ps.groundEntityNum != ENTITYNUM_NONE &&
        ent->client->ps.otherKillerDebounceTime < level.time) {
        ent->client->ps.otherKillerTime = 0;
        ent->client->ps.otherKiller = ENTITYNUM_NONE;
    } else if (ent->client->ps.otherKillerTime > level.time && ent->client->ps.groundEntityNum == ENTITYNUM_NONE) {
    ...
    if (ent->client->ps.useDelay > level.time && ent->client->ps.m_iVehicleNum) { // when in a vehicle, debounce the use...
    ...
    if (ent->client->ps.saberLockTime > level.time) {
        ...
        if (ent->client->ps.saberLockHitCheckTime < level.time) { // have moved to next frame since last lock push
            ...
            if ((ent->client->buttons & BUTTON_ATTACK) && !(ent->client->oldbuttons & BUTTON_ATTACK)) {
                if (ent->client->ps.saberLockHitIncrementTime < level.time) { // have moved to next frame since last saberlock attack button press
                    ...
                    if (ent->client->ps.fd.forceRageRecoveryTime > level.time && Q_irand(0, 1)) { // finished raging: weak
                        lockHits -= 1;
                    }
                    lockHits += ent->client->saber[0].lockBonus;
                    if (ent->client->saber[1].model[0] && !ent->client->ps.saberHolstered) {
        ...
        // check for taunt
        if ((pm.cmd.generic_cmd == GENCMD_ENGAGE_DUEL) &&
            (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL)) { // already in a duel, make it a taunt command
    ...
    if (pm.checkDuelLoss) {
        if (pm.checkDuelLoss > 0 &&
            (pm.checkDuelLoss <= MAX_CLIENTS || (pm.checkDuelLoss < (MAX_GENTITIES - 1) && g_entities[pm.checkDuelLoss - 1].s.eType == ET_NPC))) {
            ...
            if (clientLost && clientLost->inuse && clientLost->client && Q_irand(0, 40) > clientLost->health) {
                ...
                gGAvoidDismember = 0;
            } else if (clientLost && clientLost->inuse && clientLost->client && clientLost->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN &&
                       clientLost->client->ps.saberEntityNum) { // if we didn't knock down it was a circle lock. So as punishment, make them lose their saber
                                                                // and go into a proper anim
    ...
    if (pm.cmd.generic_cmd && (pm.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time)) {
        ...
        case GENCMD_USE_SEEKER:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SEEKER)) && G_ItemUsable(&ent->client->ps, HI_SEEKER)) {
                ItemUse_Seeker(ent);
        ...
        case GENCMD_USE_FIELD:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SHIELD)) && G_ItemUsable(&ent->client->ps, HI_SHIELD)) {
                ItemUse_Shield(ent);
        ...
        case GENCMD_USE_BACTA:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_MEDPAC)) && G_ItemUsable(&ent->client->ps, HI_MEDPAC)) {
                ItemUse_MedPack(ent);
        ...
        case GENCMD_USE_ELECTROBINOCULARS:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_BINOCULARS)) && G_ItemUsable(&ent->client->ps, HI_BINOCULARS)) {
                ItemUse_Binoculars(ent);
        ...
        case GENCMD_USE_SENTRY:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SENTRY_GUN)) && G_ItemUsable(&ent->client->ps, HI_SENTRY_GUN)) {
                ItemUse_Sentry(ent);
        ...
        case GENCMD_USE_JETPACK:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_JETPACK)) && G_ItemUsable(&ent->client->ps, HI_JETPACK)) {
                ItemUse_Jetpack(ent);
        ...
        case GENCMD_USE_HEALTHDISP:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_HEALTHDISP)) && G_ItemUsable(&ent->client->ps, HI_HEALTHDISP)) {
                // ItemUse_UseDisp(ent, HI_HEALTHDISP);
                G_AddEvent(ent, EV_USE_ITEM0 + HI_HEALTHDISP, 0);
            }
            break;
        case GENCMD_USE_AMMODISP:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_AMMODISP)) && G_ItemUsable(&ent->client->ps, HI_AMMODISP)) {
                // ItemUse_UseDisp(ent, HI_AMMODISP);
                G_AddEvent(ent, EV_USE_ITEM0 + HI_AMMODISP, 0);
            }
            break;
        case GENCMD_USE_EWEB:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_EWEB)) && G_ItemUsable(&ent->client->ps, HI_EWEB)) {
                ItemUse_UseEWeb(ent);
                G_AddEvent(ent, EV_USE_ITEM0 + HI_EWEB, 0);
            }
            break;
        case GENCMD_USE_CLOAK:
            if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_CLOAK)) && G_ItemUsable(&ent->client->ps, HI_CLOAK)) {
    ...
    if (ent->s.eType != ET_NPC && ent->client && ent->client->pers.connected == CON_CONNECTED && !!(japp_allowHook.bits & (1 << level.gametype))) {
        const bool saberBusy = PM_SaberInStart(ent->client->ps.saberMove) || BG_SaberInAttack(ent->client->ps.saberMove) ||
                               PM_SaberInReturn(ent->client->ps.saberMove) || PM_SaberInTransition(ent->client->ps.saberMove);
        const bool oldGrapple = GetCPD((bgEntity_t *)ent, CPD_OLDGRAPPLE) || !Client_Supports(ent, CSF_GRAPPLE_SWING);
        ...
        if (ent->client->hook && pullGrapple && (saberBusy || ent->client->ps.duelInProgress || ent->client->ps.forceHandExtend != HANDEXTEND_NONE)) {
            Weapon_HookFree(ent->client->hook);
        } else if (!ent->client->hook && pullGrapple && ent->client->ps.pm_type != PM_DEAD &&
                   ent->client->lastHookTime <= level.time - japp_hookDebouncer.integer && !saberBusy) {
            Weapon_GrapplingHook_Fire(ent);
        }

        if (ent->client->hook && ent->client->hook->inuse) {
            if ((releaseGrapple && ent->client->hookHasBeenFired && !ent->client->fireHeld) || (oldGrapple && !pullGrapple && ent->client->hook) ||
    ...
    if (ent->client->emote.freeze) {
        ...
        const qboolean animDone = ent->client->ps.forceHandExtendTime <= level.time && ent->client->ps.forceHandExtendTime != INT32_MAX;
    ...
    if (japp_flipKick.integer && client->ps.forceKickFlip) {
        ...
        if (faceKicked && faceKicked->client && (!OnSameTeam(ent, faceKicked) || g_friendlyFire.integer) &&
            ...
            if (faceKicked->health > 0 && faceKicked->takedamage && !(faceKicked->client->ps.eFlags & EF_INVULNERABLE)) {
                ...
                if (faceKicked->client->ps.weapon != WP_SABER || faceKicked->client->ps.fd.saberAnimLevel != FORCE_LEVEL_3 ||
                    (!BG_SaberInAttack(faceKicked->client->ps.saberMove) && !PM_SaberInStart(faceKicked->client->ps.saberMove) &&
                     !PM_SaberInReturn(faceKicked->client->ps.saberMove) && !PM_SaberInTransition(faceKicked->client->ps.saberMove))) {
                    if (faceKicked->health > 0 && faceKicked->client->ps.stats[STAT_HEALTH] > 0 &&
    ...
    if (client->ps.stats[STAT_HEALTH] <= 0 && !(client->ps.eFlags2 & EF2_HELD_BY_MONSTER) // can't respawn while being eaten
        && ent->s.eType != ET_NPC) {
        ...
        if (level.time > client->respawnTime && !gDoSlowMoDuel) {
            ...
            if (level.gametype == GT_POWERDUEL) {
                forceRes = 1;
            } else if (level.gametype == GT_SIEGE && g_siegeRespawn.integer) { // wave respawning on
                forceRes = 1;
            }
...
static void G_SendScoreboardUpdate(gentity_t *ent) {
    ...
    for (i = 0; i < numSorted; i++) {
        ...
        if (!Client_Supports(ent, CSF_SCOREBOARD_KD)) {
            Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.sortedClients[i], cl->ps.persistant[PERS_SCORE], ping,
                        (level.time - cl->pers.enterTime) / 60000, scoreFlags, g_entities[level.sortedClients[i]].s.powerups, accuracy,
                        cl->ps.persistant[PERS_IMPRESSIVE_COUNT], cl->ps.persistant[PERS_EXCELLENT_COUNT], cl->ps.persistant[PERS_GAUNTLET_FRAG_COUNT],
                        cl->ps.persistant[PERS_DEFEND_COUNT], cl->ps.persistant[PERS_ASSIST_COUNT], perfect, cl->ps.persistant[PERS_CAPTURES]);
        }
        // client mod hints K/D
        else {
            Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.sortedClients[i], cl->ps.persistant[PERS_SCORE], ping,
                        (level.time - cl->pers.enterTime) / 60000, scoreFlags, g_entities[level.sortedClients[i]].s.powerups, accuracy,
                        cl->ps.persistant[PERS_IMPRESSIVE_COUNT], cl->ps.persistant[PERS_EXCELLENT_COUNT], cl->ps.persistant[PERS_GAUNTLET_FRAG_COUNT],
                        cl->ps.persistant[PERS_DEFEND_COUNT], cl->ps.persistant[PERS_ASSIST_COUNT], perfect, cl->ps.persistant[PERS_CAPTURES],
    ...
    trap->SendServerCommand(ent - g_entities,
                            va("scores %i %i %i%s", level.numConnectedClients, level.teamScores[TEAM_RED], level.teamScores[TEAM_BLUE], string));
}
...
void ClientEndFrame(gentity_t *ent) {
    ...
    //
    if (level.intermissiontime) {
        if (ent->s.number < MAX_CLIENTS || ent->client->NPC_class == CLASS_VEHICLE) { // players and vehicles do nothing in intermissions
            return;
        }
    }
