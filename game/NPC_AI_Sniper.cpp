...
void NPC_Sniper_PlayConfusionSound(gentity_t *self) { // FIXME: make this a custom sound in sound set
    if (self->health > 0) {
...
static qboolean Sniper_Move(void) {
    ...
    if (moved == qfalse) {                                                                                            // couldn't get to enemy
        if ((NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) && NPCInfo->goalEntity && NPCInfo->goalEntity == NPC->enemy) { // we were running after enemy
            ...
            if (cp == -1 && !(NPCInfo->scriptFlags & SCF_USE_CP_NEAREST)) { // okay, try one by the enemy
                cp = NPC_FindCombatPoint(&NPC->r.currentOrigin, &NPC->r.currentOrigin, &NPC->enemy->r.currentOrigin,
            ...
            if (cp != -1) { // found a combat point that has a clear shot to enemy
    ...
    if (NPCInfo->confusionTime < level.time) {
        ...
        if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
            ...
            if (NPC_CheckForDanger(alertEvent)) {
                NPC_UpdateAngles(qtrue, qtrue);
                return;
            } else { // check for other alert events
                ...
                if (alertEvent >= 0 && level.alertEvents[alertEvent].ID != NPCInfo->lastAlertID) {
                    ...
                    if (level.alertEvents[alertEvent].level == AEL_DISCOVERED) {
                        if (level.alertEvents[alertEvent].owner && level.alertEvents[alertEvent].owner->client &&
                            ...
                            level.alertEvents[alertEvent].owner->client->playerTeam == NPC->client->enemyTeam) { // an enemy
                            G_SetEnemy(NPC, level.alertEvents[alertEvent].owner);
                            // NPCInfo->enemyLastSeenTime = level.time;
                            TIMER_Set(NPC, "attackDelay", Q_irand((6 - NPCInfo->stats.aim) * 100, (6 - NPCInfo->stats.aim) * 500));
                        }
                    } else { // FIXME: get more suspicious over time?
                        ...
                        if (level.alertEvents[alertEvent].level == AEL_SUSPICIOUS) { // suspicious looks longer
            ...
            if (NPCInfo->investigateDebounceTime > level.time) { // FIXME: walk over to it, maybe?  Not if not chase enemies flag
...
static void Sniper_ResolveBlockedShot(void) {
    if (TIMER_Done(NPC, "duck")) {         // we're not ducking
        if (TIMER_Done(NPC, "roamTime")) { // not roaming
...
static void Sniper_CheckFireState(void) {
    ...
    if (!Q_irand(0, 1) && NPCInfo->enemyLastSeenTime &&
        level.time - NPCInfo->enemyLastSeenTime < ((5 - NPCInfo->stats.aim) * 1000)) // FIXME: incorporate skill too?
    {
        ...
        return;
    } else if (level.time - NPCInfo->enemyLastSeenTime > 10000) { // next time we see him, we'll miss few times first
        NPC->count = 0;
    }
}
...
qboolean Sniper_EvaluateShot(int hit) {
    ...
    if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) ||
        ...
        (hitEnt && (hitEnt->r.svFlags & SVF_GLASS_BRUSH))) { // can hit enemy or will hit glass, so shoot anyway
        return qtrue;
    }
    return qfalse;
}
...
void Sniper_FaceEnemy(void) {
    ...
    if (NPC->enemy) {
        vector3 muzzle, target, angles, forward, right, up;
        // Get the positions
        ...
        {
            if (NPC->count < (5 - NPCInfo->stats.aim)) {                                           // miss a few times first
                if (shoot2 && TIMER_Done(NPC, "attackDelay") && level.time >= NPCInfo->shotTime) { // ready to fire again
            ...
            target.z += flrand(0, NPC->enemy->r.maxs.z);
...
void NPC_BSSniper_Attack(void) {
    ...
    {                                                  // too close, so switch to primary fire
        if (NPC->client->ps.weapon == WP_DISRUPTOR) {  // sniping... should be assumed
            if (NPCInfo->scriptFlags & SCF_ALT_FIRE) { // use primary fire
                trace_t trace;
                trap->Trace(&trace, &NPC->enemy->r.currentOrigin, &NPC->enemy->r.mins, &NPC->enemy->r.maxs, &NPC->r.currentOrigin, NPC->enemy->s.number,
                            NPC->enemy->clipmask, qfalse, 0, 0);
                if (!trace.allsolid && !trace.startsolid &&
                    (flcmp(trace.fraction, 1.0f, 0.001f) || trace.entityNum == NPC->s.number)) { // he can get right to me
    ...
    */
    ...
    if (enemyCS2) {
        shoot2 = qtrue;
    } else if (level.time - NPCInfo->enemyLastSeenTime > 3000) { // Hmm, have to get around this bastard... FIXME: this NPCInfo->enemyLastSeenTime builds up
                                                                 // when ducked seems to make them want to run when they uncrouch
