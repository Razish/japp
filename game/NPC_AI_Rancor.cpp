...
qboolean Rancor_CheckRoar(gentity_t *self) {
    if (!(int)self->wait) { // haven't ever gotten mad yet
...
void Rancor_DropVictim(gentity_t *self) {
    ...
    if (self->activator) {
        ...
        if (self->activator->health <= 0) {
            ...
            {                           // never free player
                if (self->count == 1) { // in my hand, just drop them
                    if (self->activator->client) {
            ...
            if (self->activator->NPC) { // start thinking again
    ...
    for (i = 0; i < numEnts; i++) {
        ...
        if (DistanceSquared(&radiusEnt->r.currentOrigin, &boltOrg) <= radiusSquared) {
            if (tryGrab && NPC->count != 1 // don't have one in hand or in mouth already - FIXME: allow one in hand and any number in mouth!
                && radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_GALAKMECH &&
                radiusEnt->client->NPC_class != CLASS_ATST && radiusEnt->client->NPC_class != CLASS_GONK && radiusEnt->client->NPC_class != CLASS_R2D2 &&
                radiusEnt->client->NPC_class != CLASS_R5D2 && radiusEnt->client->NPC_class != CLASS_MARK1 && radiusEnt->client->NPC_class != CLASS_MARK2 &&
                radiusEnt->client->NPC_class != CLASS_MOUSE && radiusEnt->client->NPC_class != CLASS_PROBE && radiusEnt->client->NPC_class != CLASS_SEEKER &&
                radiusEnt->client->NPC_class != CLASS_REMOTE && radiusEnt->client->NPC_class != CLASS_SENTRY &&
                radiusEnt->client->NPC_class != CLASS_INTERROGATOR && radiusEnt->client->NPC_class != CLASS_VEHICLE) { // grab
                ...
                */
                ...
                if (radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_ATST) {
...
void Rancor_Smash(void) {
    ...
    for (i = 0; i < numEnts; i++) {
        ...
        if (distSq <= radiusSquared) {
            G_Sound(radiusEnt, CHAN_AUTO, G_SoundIndex("sound/chars/rancor/swipehit.wav"));
            if (distSq < halfRadSquared) { // close enough to do damage, too
            ...
            if (radiusEnt->health > 0 && radiusEnt->client && radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_ATST) {
                if (distSq < halfRadSquared ||
                    radiusEnt->client->ps.groundEntityNum != ENTITYNUM_NONE) { // within range of my fist or withing ground-shaking range and not in the air
...
void Rancor_Bite(void) {
    ...
    for (i = 0; i < numEnts; i++) {
        ...
        if (DistanceSquared(&radiusEnt->r.currentOrigin, &boltOrg) <= radiusSquared) {
            ...
            if (radiusEnt->health <= 0 && radiusEnt->client) { // killed them, chance of dismembering
                if (!Q_irand(0, 1)) {                          // bite something off
    ...
    if (TIMER_Done2(NPC, "attack_dmg", qtrue)) {
        ...
        case BOTH_ATTACK1:
            if (NPC->count == 1 && NPC->activator) {
                G_Damage(NPC->activator, NPC, NPC, &vec3_origin, &NPC->activator->r.currentOrigin, Q_irand(25, 40), DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK,
                         MOD_MELEE);
        ...
        case BOTH_ATTACK3:
            if (NPC->count == 1 && NPC->activator) {
                ...
                // KILL
                G_Damage(NPC->activator, NPC, NPC, &vec3_origin, &NPC->activator->r.currentOrigin, NPC->enemy->health + 10,
                         DAMAGE_NO_PROTECTION | DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_MELEE); //, HL_NONE );//
                if (NPC->activator->client) {
                    NPC->activator->client->ps.forceHandExtend = HANDEXTEND_NONE;
                    NPC->activator->client->ps.forceHandExtendTime = 0;
                    NPC_SetAnim(NPC->activator, SETANIM_BOTH, BOTH_SWIM_IDLE1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void Rancor_Combat(void) {
...

    {
        ...
        if (advance) { // have to get closer
            ...
            if (NPC->enemy->health > 0 && fabsf(distance - 250) <= 80 && InFOV3(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &yawOnlyAngles, 30, 30)) {
...
void NPC_Rancor_Pain(gentity_t *self, gentity_t *attacker, int damage) {
    ...
    if (attacker && attacker->inuse && attacker != self->enemy && !(attacker->flags & FL_NOTARGET)) {
        if (!self->count) {
            if ((!attacker->s.number && !Q_irand(0, 3)) || !self->enemy || self->enemy->health == 0 ||
                (self->enemy->client && self->enemy->client->NPC_class == CLASS_RANCOR) ||
                (self->NPC && self->NPC->consecutiveBlockedMoves >= 10 &&
                 DistanceSquared(&attacker->r.currentOrigin, &self->r.currentOrigin) <
                     DistanceSquared(&self->enemy->r.currentOrigin, &self->r.currentOrigin))) { // if my enemy is dead (or attacked by player) and I'm not still
                                                                                                // holding/eating someone, turn on the attacker
    ...
    if ((hitByRancor || (self->count == 1 && self->activator && !Q_irand(0, 4)) ||
         Q_irand(0, 200) < damage) // hit by rancor, hit while holding live victim, or took a lot of damage
        && self->client->ps.legsAnim != BOTH_STAND1TO2 && TIMER_Done(self, "takingPain")) {
        if (!Rancor_CheckRoar(self)) {
            if (self->client->ps.legsAnim != BOTH_MELEE1 && self->client->ps.legsAnim != BOTH_MELEE2 &&
...
void Rancor_CheckDropVictim(void) {
    ...
    VectorSet(&mins, NPC->activator->r.mins.x - 1, NPC->activator->r.mins.y - 1, 0);
    VectorSet(&maxs, NPC->activator->r.maxs.x + 1, NPC->activator->r.maxs.y + 1, 1);
...
void Rancor_Crush(void) {
    ...
    if (!NPC || !NPC->client || NPC->client->ps.groundEntityNum >= ENTITYNUM_WORLD) { // nothing to crush
        return;
    }

    crush = &g_entities[NPC->client->ps.groundEntityNum];
    if (crush->inuse && crush->client && !crush->localAnimIndex) { // a humanoid, smash them good.
...
void NPC_BSRancor_Default(void) {
    ...
    if (TIMER_Done2(NPC, "clearGrabbed", qtrue)) {
        Rancor_DropVictim(NPC);
    } else if (NPC->client->ps.legsAnim == BOTH_PAIN2 && NPC->count == 1 && NPC->activator) {
        if (!Q_irand(0, 3)) {
    ...
    if (NPC->enemy) {
        ...
        && (!NPC->enemy->enemy || !NPC->enemy->enemy->client || NPC->enemy->enemy->client->NPC_class!=CLASS_RANCOR) )//enemy's enemy is not a client or is not a
        rancor (which is as scary as me anyway)
            ...
            if (ValidEnemy(NPC->enemy) == qfalse) {
                ...
                if (!NPC->enemy->inuse ||
                    level.time - NPC->enemy->s.time >
                        Q_irand(10000, 15000)) { // it's been a while since the enemy died, or enemy is completely gone, get bored with him
                    NPC->enemy = NULL;
                    Rancor_Patrol();
                    NPC_UpdateAngles(qtrue, qtrue);
                    return;
