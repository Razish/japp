...
Q_CDECL void BotAI_Print( int type, const char *fmt, ... ) {
...
void BotChangeViewAngles( bot_state_t *bs, float thinktime ) {
	...
	if ( bs->currentEnemy && bs->frame_Enemy_Vis ) {
		...
		else if ( bs->settings.skill <= 2 )	factor = (bs->skills.turnspeed_combat*0.6f)*bs->settings.skill;
		else if ( bs->settings.skill <= 3 )	factor = (bs->skills.turnspeed_combat*0.8f)*bs->settings.skill;
		else								factor = bs->skills.turnspeed_combat*bs->settings.skill;
	}
	else
...
int PassWayCheck( bot_state_t *bs, int windex ) {
	...
	else if ( !bs->wpDirection && (gWPArray[windex]->flags & WPFLAG_ONEWAY_BACK) ) { //we're not travelling in a direction on the trail that will allow us to pass this point
		return 0;
	}

	if ( bs->wpCurrent && gWPArray[windex]->forceJumpTo &&
		gWPArray[windex]->origin.z > (bs->wpCurrent->origin.z + 64) &&
		bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] < gWPArray[windex]->forceJumpTo ) { //waypoint requires force jump level greater than our current one to pass
		return 0;
	}
...
void WPConstantRoutine( bot_state_t *bs ) {
	...
	if ( bs->wpCurrent->flags & WPFLAG_JUMP ) { //jump while travelling to this point
		...
		if ( heightDif > 40 && (bs->cur_ps.fd.forcePowersKnown & (1 << FP_LEVITATION)) && (bs->cur_ps.fd.forceJumpCharge < (forceJumpStrength[bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION]] - 100) || bs->cur_ps.groundEntityNum == ENTITYNUM_NONE) ) { //alright, let's jump
			bs->forceJumpChargeTime = level.time + 1000;
...
#else
	if ( (bs->wpCurrent->flags & WPFLAG_JUMP) && !bs->wpCurrent->forceJumpTo ) { //jump if we're flagged to but not if this indicates a force jump point. Force jumping is
		//handled elsewhere.
		bs->jumpTime = level.time + 100;
	}
#endif
...
int BotTrace_Strafe( bot_state_t *bs, vector3 *traceto ) {
	...
	if ( AngleDifference( bs->viewangles.yaw, dirAng.yaw ) > 60 ||
		AngleDifference( bs->viewangles.yaw, dirAng.yaw ) < -60 ) { //If we aren't facing the direction we're going here, then we've got enough excuse to be too stupid to strafe around anyway
		return 0;
	}
...
int PassLovedOneCheck( bot_state_t *bs, gentity_t *ent ) {
	...
	while ( i < bs->lovednum ) {
		if ( strcmp( level.clients[loved->client].pers.netname, bs->loved[i].name ) == 0 ) {
			...
			else if ( IsTeamplay() && !OnSameTeam( &g_entities[bs->client], &g_entities[loved->client] ) && bs->loved[i].level < 2 ) { //is teamplay, but not on same team and level < 2
				return 1;
			}
			else
				return 0;
...
int ScanForEnemies( bot_state_t *bs ) {
	...
	if ( bs->currentEnemy && bs->currentEnemy->client &&
		bs->currentEnemy->client->ps.isJediMaster ) { //The Jedi Master must die.
		return -1;
	}
...
int WaitingForNow( bot_state_t *bs, vector3 *goalpos ) { //checks if the bot is doing something along the lines of waiting for an elevator to raise up
	...
	if ( (int)goalpos->x != (int)bs->wpCurrent->origin.x ||
		(int)goalpos->y != (int)bs->wpCurrent->origin.y ||
		(int)goalpos->z != (int)bs->wpCurrent->origin.z ) {
		return 0;
	}
...
jmPass:
	...
	if ( bs->currentEnemy && bs->currentEnemy->client &&
		...
		bs->frame_Enemy_Len < 512 && bs->cur_ps.weapon != WP_SABER ) { //if close to an enemy with a saber and not using a saber, then try to back off
		return 1;
	}
...
gentity_t *GetNearestBadThing( bot_state_t *bs ) {
	...
	while ( i < level.num_entities ) {
		...
		if ( (ent &&
			...
			g_entities[ent->genericValue3].client && !OnSameTeam( &g_entities[bs->client], &g_entities[ent->genericValue3] )) ) { //try to escape from anything with a non-0 s.weapon and non-0 damage. This hopefully only means dangerous projectiles.
			...
			if ( ent->s.weapon == WP_ROCKET_LAUNCHER &&
				...
				g_entities[ent->r.ownerNum].client && OnSameTeam( &g_entities[bs->client], &g_entities[ent->r.ownerNum] ))) ) { //don't be afraid of your own rockets or your teammates' rockets
				factor = 0;
			}
		...
		if ( ent && !ent->client && ent->inuse && ent->damage && ent->s.weapon && ent->r.ownerNum < MAX_CLIENTS && ent->r.ownerNum >= 0 ) { //if we're in danger of a projectile belonging to someone and don't have an enemy, set the enemy to them
...
int SiegeTakesPriority( bot_state_t *bs ) {
	...
	if ( bs->siegeState == SIEGESTATE_ATTACKER ) {
		if ( !Siege_TargetClosestObjective( bs, flagForAttackableObjective ) ) { //looks like we have no goals other than to keep the other team from completing objectives
...
void GetIdealDestination( bot_state_t *bs ) {
	...
	if ( tempInt == BWEAPONRANGE_MELEE )		distChange = 1;
	else if ( tempInt == BWEAPONRANGE_SABER )		distChange = 1;
	else if ( tempInt == BWEAPONRANGE_MID )		distChange = 128;
	else if ( tempInt == BWEAPONRANGE_LONG )		distChange = 300;

	if ( bs->revengeEnemy && bs->revengeEnemy->health > 0 &&
		bs->revengeEnemy->client && (bs->revengeEnemy->client->pers.connected == CON_CONNECTED || bs->revengeEnemy->client->pers.connected == CON_CONNECTING) ) { //if we hate someone, always try to get to them
		if ( bs->wpDestSwitchTime < level.time ) {
...
void CommanderBotCTFAI( bot_state_t *bs ) {
	...
	if ( enemyHasOurFlag && !weHaveEnemyFlag ) { //start off with an attacker instead of a retriever if we don't have the enemy flag yet so that they can't capture it first.
	...
	while ( i < squadmates ) {
		if ( squad[i] && squad[i]->client && botstates[squad[i]->s.number] ) {
			...
			else if ( (numOnMyTeam < 2 || !numAttackers) && enemyHasOurFlag ) { //I'm the only one on my team who will attack and the enemy has my flag, I have to go after him
				botstates[squad[i]->s.number]->ctfState = CTFSTATE_RETRIEVAL;
			}
		}
...
void SaberCombatHandling( bot_state_t *bs ) {
	...
	if ( me_down == en_down &&
		en_down == mid_down ) {
		if ( usethisvec.z > (bs->origin.z + 32) &&
...
void BotAimOffsetGoalAngles( bot_state_t *bs ) {
	...
	if ( bs->currentEnemy && BotMindTricked( bs->client, bs->currentEnemy->s.number ) ) { //having to judge where they are by hearing them, so we should be quite inaccurate here
		accVal *= 7;
	...
	if ( bs->revengeEnemy && bs->revengeHateLevel &&
		bs->currentEnemy == bs->revengeEnemy ) { //bot becomes more skilled as anger level raises
	...
	if ( bs->currentEnemy && bs->frame_Enemy_Vis ) { //assume our goal is aiming at the enemy, seeing as he's visible and all
		if ( !bs->currentEnemy->s.pos.trDelta.x &&
			!bs->currentEnemy->s.pos.trDelta.y &&
			!bs->currentEnemy->s.pos.trDelta.z ) {
...
int BotSelectChoiceWeapon( bot_state_t *bs, int weapon, int doselection ) { //if !doselection then bot will only check if he has the specified weapon and return 1 (yes) or 0 (no)
	int i;
	int hasit = 0;
...
void CTFFlagMovement( bot_state_t *bs ) {
	...
	if ( bs->wantFlag && (bs->wantFlag->flags & FL_DROPPED_ITEM) ) {
		if ( bs->staticFlagSpot.x == bs->wantFlag->s.pos.trBase.x &&
			bs->staticFlagSpot.y == bs->wantFlag->s.pos.trBase.y &&
...
stillmadeit:
	...
	if ( enLen < BOT_PLANT_BLOW_DISTANCE && OrgVisible( &bs->currentEnemy->client->ps.origin, &myDet->s.pos.trBase, bs->currentEnemy->s.number ) ) { //we could just call the "blow all my detpacks" function here, but I guess that's cheating.
...
int BotUseInventoryItem( bot_state_t *bs ) {
	...
	if ( bs->cur_ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SHIELD) ) {
		if ( bs->currentEnemy && bs->frame_Enemy_Vis && bs->runningToEscapeThreat ) { //this will (hopefully) result in the bot placing the shield down while facing
...
void StandardBotAI( bot_state_t *bs, float thinktime ) {
	...
	if ( g_entities[bs->client].health < 1 ) {
		...
		if ( !bs->deathActivitiesDone && bs->lastHurt && bs->lastHurt->client && bs->lastHurt->s.number != bs->client ) {
			...
			if ( PassLovedOneCheck( bs, bs->lastHurt ) ) {
				...
				PassLovedOneCheck( botstates[bs->lastHurt->s.number], &g_entities[bs->client] ) ) { //killed by a bot that I love, but that does not love me
				bs->chatObject = bs->lastHurt;
				bs->chatAltObject = NULL;
...
#endif
	{
		...
		if ( (bs->cur_ps.fd.forcePowersKnown & (1 << FP_PUSH)) && (bs->doForcePush > level.time || bs->cur_ps.fd.forceGripBeingGripped > level.time) && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_PUSH]][FP_PUSH] /*&& InFieldOfVision(bs->viewangles, 50, a_fo)*/ ) {
			...
			if ( (bs->cur_ps.fd.forcePowersKnown & (1 << FP_GRIP)) && (bs->cur_ps.fd.forcePowersActive & (1 << FP_GRIP)) && InFieldOfVision( &bs->viewangles, 50, &a_fo ) ) { //already gripping someone, so hold it
			...
			if ( (bs->cur_ps.fd.forcePowersKnown & (1 << FP_ABSORB)) && bs->cur_ps.fd.forceGripCripple &&
				level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_ABSORB]][FP_ABSORB] ) { //absorb to get out
				...
				level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_ABSORB]][FP_ABSORB] ) { //absorb lightning
				level.clients[bs->client].ps.fd.forcePowerSelected = FP_ABSORB;
				useTheForce = 1;
				forceHostile = 0;
			}
	...
	if ( !useTheForce ) { //try powers that we don't care if we have an enemy for
		...
		else if ( (bs->cur_ps.fd.forcePowersKnown & (1 << FP_HEAL)) && g_entities[bs->client].health < 50 && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_HEAL]][FP_HEAL] && !bs->currentEnemy && bs->isCamping > level.time ) { //only meditate and heal if we're camping
			level.clients[bs->client].ps.fd.forcePowerSelected = FP_HEAL;
			useTheForce = 1;
			forceHostile = 0;
		}
	...
	if ( useTheForce && forceHostile ) {
		if ( bs->currentEnemy && bs->currentEnemy->client &&
			!ForcePowerUsableOn( &g_entities[bs->client], bs->currentEnemy,
				(forcePowers_t)level.clients[bs->client].ps.fd.forcePowerSelected ) )
		{
			useTheForce = 0;
			forceHostile = 0;
		}
	...
	if ( bs->cur_ps.weapon == WP_SABER &&
		...
		!bs->cur_ps.saberEntityNum ) { //saber knocked away, keep trying to get it back
		bs->doAttack = 1;
		bs->doAltAttack = 0;
	}
