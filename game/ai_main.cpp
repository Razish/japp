...
const char *ctfStateNames[] = {"CTFSTATE_NONE",         "CTFSTATE_ATTACKER",    "CTFSTATE_DEFENDER",    "CTFSTATE_RETRIEVAL",
                               "CTFSTATE_GUARDCARRIER", "CTFSTATE_GETFLAGHOME", "CTFSTATE_MAXCTFSTATES"};

const char *ctfStateDescriptions[] = {"I'm not occupied",          "I'm attacking the enemy's base", "I'm defending our base",
                                      "I'm getting our flag back", "I'm escorting our flag carrier", "I've got the enemy's flag"};

const char *siegeStateDescriptions[] = {"I'm not occupied", "I'm attemtping to complete the current objective",
                                        "I'm preventing the enemy from completing their objective"};

const char *teamplayStateDescriptions[] = {"I'm not occupied", "I'm following my squad commander", "I'm assisting my commanding",
...
void BotEntityInfo(int entnum, aas_entityinfo_t *info) { trap->AAS_EntityInfo(entnum, info); }
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (bs->viewanglespeed.raw[i] > 180)
            bs->viewanglespeed.raw[i] = maxchange;
        if (bs->viewanglespeed.raw[i] < -180)
            bs->viewanglespeed.raw[i] = -maxchange;
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, ivector3 *delta_angles, int time, int useTime) {
    ...
    if (bi->actionflags & ACTION_FORCEPOWER)
        ucmd->buttons |= BUTTON_FORCEPOWER;

    if (useTime < level.time && Q_irand(1, 10) < 5) { // for now just hit use randomly in case there's something useable around
    ...
    ucmd->upmove = abs((int)(forward.z)) * bi->dir.z * bi->speed;
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
    ...
    if (PlayersInGame()) { // don't talk to yourself
...
int WPOrgVisible(gentity_t *bot, vector3 *org1, vector3 *org2, int ignore) {
    trace_t tr;
    gentity_t *ownent;
...
// often.
int CheckForFunc(vector3 *org, int ignore) {
    gentity_t *fent;
    vector3 under;
    trace_t tr;
...
int PassWayCheck(bot_state_t *bs, int windex) {
    ...
    if (bs->wpDirection &&
        (gWPArray[windex]->flags & WPFLAG_ONEWAY_FWD)) { // we're not travelling in a direction on the trail that will allow us to pass this point
        return 0;
    } else if (!bs->wpDirection &&
               (gWPArray[windex]->flags & WPFLAG_ONEWAY_BACK)) { // we're not travelling in a direction on the trail that will allow us to pass this point
        return 0;
    }

    if (bs->wpCurrent && gWPArray[windex]->forceJumpTo && gWPArray[windex]->origin.z > (bs->wpCurrent->origin.z + 64) &&
        bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] <
            gWPArray[windex]->forceJumpTo) { // waypoint requires force jump level greater than our current one to pass
        return 0;
    }
...
void CheckForShorterRoutes(bot_state_t *bs, int newwpindex) {
    ...
    while (i < gWPArray[newwpindex]->neighbornum) { // now go through the neighbors and check the distance to the desired point from each neighbor
        checklen = TotalTrailDistance(gWPArray[newwpindex]->neighbors[i].num, bs->wpDestination->index, bs);

        if (checklen < bestlen - 64 || bestlen == -1) { // this path covers less distance, let's take it instead
    ...
    if (bestindex != newwpindex && bestindex != -1) { // we found a path we want to switch to, let's do it
        bs->wpCurrent = gWPArray[bestindex];
        bs->wpSwitchTime = level.time + 3000;

        if (fj) { // do we have to force jump to get to this neighbor?
...
void WPConstantRoutine(bot_state_t *bs) {
    ...
    if (bs->wpCurrent->flags & WPFLAG_DUCK) { // duck while travelling to this point
        bs->duckTime = level.time + 100;
    }

#ifndef FORCEJUMP_INSTANTMETHOD
    if (bs->wpCurrent->flags & WPFLAG_JUMP) { // jump while travelling to this point
        float heightDif = (bs->wpCurrent->origin.z - bs->origin.z + 16);

        if (bs->origin.z + 16 >= bs->wpCurrent->origin.z) { // don't need to jump, we're already higher than this point
            heightDif = 0;
        }

        if (heightDif > 40 && (bs->cur_ps.fd.forcePowersKnown & (1 << FP_LEVITATION)) &&
            (bs->cur_ps.fd.forceJumpCharge < (forceJumpStrength[bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION]] - 100) ||
             bs->cur_ps.groundEntityNum == ENTITYNUM_NONE)) { // alright, let's jump
            bs->forceJumpChargeTime = level.time + 1000;
        ...
        } else if (heightDif > 64 && !(bs->cur_ps.fd.forcePowersKnown & (1 << FP_LEVITATION))) { // this point needs force jump to reach and we don't have it
...
static void WPTouchRoutine(bot_state_t *bs) {
    ...
    if (bs->wpCurrent->flags & WPFLAG_NOMOVEFUNC) { // don't try to use any nearby map objects for a little while
        bs->noUseTime = level.time + 4000;
    }

#ifdef FORCEJUMP_INSTANTMETHOD
    if ((bs->wpCurrent->flags & WPFLAG_JUMP) &&
        bs->wpCurrent->forceJumpTo) { // jump if we're flagged to but not if this indicates a force jump point. Force jumping is
        // handled elsewhere.
        bs->jumpTime = level.time + 100;
    }
#else
    ...
    if (bs->isCamper && bot_camp.integer && (BotIsAChickenWuss(bs) || BotCTFGuardDuty(bs) || bs->isCamper == 2) &&
        ((bs->wpCurrent->flags & WPFLAG_SNIPEORCAMP) || (bs->wpCurrent->flags & WPFLAG_SNIPEORCAMPSTAND)) && bs->cur_ps.weapon != WP_SABER &&
        bs->cur_ps.weapon != WP_MELEE && bs->cur_ps.weapon != WP_STUN_BATON) { // if we're a camper and a chicken then camp
        ...
        if (lastNum >= 0 && lastNum < gWPArray.size() && gWPArray[lastNum] && gWPArray[lastNum]->index && bs->isCamping < level.time) {
            ...
            if (bs->wpCurrent->flags & WPFLAG_SNIPEORCAMPSTAND) {
               ...
               bs->isCamping > level.time) { // don't snipe/camp with a melee weapon, that would be silly
        bs->isCamping = 0;
        bs->wpCampingTo = NULL;
        bs->wpCamping = NULL;
    }
    ...
    if (bs->wpDestination) {
        if (bs->wpCurrent->index == bs->wpDestination->index) {
            ...
            if (bs->runningLikeASissy) { // this obviously means we're scared and running, so we'll want to keep our navigational priorities less delayed
...
void MoveTowardIdealAngles(bot_state_t *bs) { VectorCopy(&bs->goalAngles, &bs->ideal_viewangles); }
...
int BotTrace_Strafe(bot_state_t *bs, vector3 *traceto) {
    vector3 playerMins = {-15, -15, /*DEFAULT_MINS_2*/ -8};
    vector3 playerMaxs = {15, 15, DEFAULT_MAXS_2};
    vector3 from, to;
    vector3 dirAng, dirDif;
    vector3 forward, right;
    trace_t tr;

    if (bs->cur_ps.groundEntityNum == ENTITYNUM_NONE) { // don't do this in the air, it can be.. dangerous.
        return 0;
    }
    ...
    if (AngleDifference(bs->viewangles.yaw, dirAng.yaw) > 60 ||
        AngleDifference(bs->viewangles.yaw, dirAng.yaw) <
            -60) { // If we aren't facing the direction we're going here, then we've got enough excuse to be too stupid to strafe around anyway
        return 0;
    }
    ...
    to.x = from.x + forward.x * 32;
    to.y = from.y + forward.y * 32;
    to.z = from.z + forward.z * 32;
...
int BotTrace_Jump(bot_state_t *bs, vector3 *traceto) {
    vector3 mins, maxs, a, fwd, traceto_mod, tracefrom_mod;
    trace_t tr;
    int orTr;
    ...
    traceto_mod.x = bs->origin.x + fwd.x * 4;
    traceto_mod.y = bs->origin.y + fwd.y * 4;
    traceto_mod.z = bs->origin.z + fwd.z * 4;
    ...
    if (tr.fraction == 1) {
        ...
        if (bs->currentEnemy && bs->currentEnemy->s.number == orTr &&
            (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER || BotGetWeaponRange(bs) == BWEAPONRANGE_MELEE))
            return 0;
...
int BotTrace_Duck(bot_state_t *bs, vector3 *traceto) {
    vector3 mins, maxs, a, fwd, traceto_mod, tracefrom_mod;
    trace_t tr;
...
int PassStandardEnemyChecks(bot_state_t *bs, gentity_t *en) {
    if (!bs || !en) { // shouldn't happen
        return 0;
    }

    if (!en->client) { // not a client, don't care about him
        return 0;
    }

    if (en->health < 1) { // he's already dead
        return 0;
    }

    if (!en->takedamage) { // a client that can't take damage?
        return 0;
    }

    if (bs->doingFallback &&
        (gLevelFlags & LEVELFLAG_IGNOREINFALLBACK)) { // we screwed up in our nav routines somewhere and we've reverted to a fallback state to
        // try to get back on the trail. If the level specifies to ignore enemies in this state,
        // then ignore them.
        return 0;
    }

    if (en->client->ps.pm_type == PM_INTERMISSION || en->client->ps.pm_type == PM_SPECTATOR ||
        en->client->sess.sessionTeam == TEAM_SPECTATOR) { // don't attack spectators
        return 0;
    }
    ...
    if (!en->s.solid) { // shouldn't happen
        return 0;
    }

    if (bs->client == en->s.number) { // don't attack yourself
        return 0;
    }

    if (OnSameTeam(&g_entities[bs->client], en)) { // don't attack teammates
        return 0;
    }

    if (BotMindTricked(bs->client, en->s.number)) {
        if (bs->currentEnemy && bs->currentEnemy->s.number == en->s.number) { // if mindtricked by this enemy, then be less "aware" of them, even though
    ...
    if (en->client->ps.duelInProgress && en->client->ps.duelIndex != bs->client) { // don't attack duelists unless you're dueling them
        return 0;
    }

    if (bs->cur_ps.duelInProgress && en->s.number != bs->cur_ps.duelIndex) { // ditto, the other way around
        return 0;
    }

    if (level.gametype == GT_JEDIMASTER && !en->client->ps.isJediMaster && !bs->cur_ps.isJediMaster) { // rules for attacking non-JM in JM mode
        vector3 vs;
        float vLen = 0;
...
void BotDamageNotification(gclient_t *bot, gentity_t *attacker) {
    ...
    if (attacker->s.number >= MAX_CLIENTS) { // if attacker is an npc also don't care I suppose.
        return;
    }

    bs_a = botstates[attacker->s.number];

    if (bs_a) { // if the client attacking us is a bot as well
        bs_a->lastAttacked = &g_entities[bot->ps.clientNum];
        i = 0;

        while (i < MAX_CLIENTS) {
            if (botstates[i] && i != bs_a->client && botstates[i]->lastAttacked == &g_entities[bot->ps.clientNum]) {
    ...
    if (bs->currentEnemy) { // we don't care about the guy attacking us if we have an enemy already
        return;
    }

    if (!PassStandardEnemyChecks(bs, attacker)) { // the person that hurt us is not a valid enemy
        return;
    }

    if (PassLovedOneCheck(bs, attacker)) { // the person that hurt us is the one we love!
...
int BotCanHear(bot_state_t *bs, gentity_t *en, float endist) {
    ...
    if (en && en->client && en->client->ps.otherSoundTime > level.time) { // they made a noise in recent time
        minlen = en->client->ps.otherSoundLen;
        goto checkStep;
    }

    if (en && en->client && en->client->ps.footstepTime > level.time) { // they made a footstep
        minlen = 256;
        goto checkStep;
    }

    if (gBotEventTracker[en->s.number].eventTime < level.time) { // no recent events to check
        return 0;
    }

    switch (gBotEventTracker[en->s.number].events[gBotEventTracker[en->s.number].eventSequence & (MAX_PS_EVENTS - 1)]) { // did the last event contain a sound?
...
checkStep:
    if (BotMindTricked(bs->client, en->s.number)) { // if mindtricked by this person, cut down on the minlen so they can't "hear" as well
        minlen /= 4;
    }

    if (endist <= minlen) { // we heard it
        return 1;
    }
...
void UpdateEventTracker(void) {
    ...
    while (i < MAX_CLIENTS) {
        if (gBotEventTracker[i].eventSequence != level.clients[i].ps.eventSequence) { // updated event
...
int PassLovedOneCheck(bot_state_t *bs, gentity_t *ent) {
    ...
    if (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) { // There is no love in 1-on-1
        return 1;
    }

    i = 0;

    if (!botstates[ent->s.number]) { // not a bot
        return 1;
    }
    ...
    while (i < bs->lovednum) {
        if (strcmp(level.clients[loved->client].pers.netname, bs->loved[i].name) == 0) {
            if (!IsTeamplay() && bs->loved[i].level < 2) { // if FFA and level of love is not greater than 1, just don't care
                return 1;
            } else if (IsTeamplay() && !OnSameTeam(&g_entities[bs->client], &g_entities[loved->client]) &&
                       bs->loved[i].level < 2) { // is teamplay, but not on same team and level < 2
                return 1;
            } else
                return 0;
        }
...
int ScanForEnemies(bot_state_t *bs) {
    ...
    while (i <= MAX_CLIENTS) {
        if (i != bs->client && g_entities[i].client && !OnSameTeam(&g_entities[bs->client], &g_entities[i]) && PassStandardEnemyChecks(bs, &g_entities[i]) &&
            trap->InPVS(&g_entities[i].client->ps.origin, &bs->eye) && PassLovedOneCheck(bs, &g_entities[i])) {
            VectorSubtract(&g_entities[i].client->ps.origin, &bs->eye, &a);
            distcheck = VectorLength(&a);
            vectoangles(&a, &a);

            if (g_entities[i].client->ps.isJediMaster) { // make us think the Jedi Master is close so we'll attack him above all
                distcheck = 1;
            }
            ...
            if (distcheck < closest &&
                ...
                if (BotMindTricked(bs->client, i)) {
                    if (distcheck < 256 || (level.time - g_entities[i].client->dangerTime) < 100) {
                        if (!hasEnemyDist ||
                            distcheck < (hasEnemyDist - 128)) { // if we have an enemy, only switch to closer if he is 128+ closer to avoid flipping out
...
int WaitingForNow(bot_state_t *bs, vector3 *goalpos) { // checks if the bot is doing something along the lines of waiting for an elevator to raise up
    vector3 xybot, xywp, a;

    if (!bs->wpCurrent) {
        return 0;
    }

    if ((int)goalpos->x != (int)bs->wpCurrent->origin.x || (int)goalpos->y != (int)bs->wpCurrent->origin.y || (int)goalpos->z != (int)bs->wpCurrent->origin.z) {
        return 0;
    }
...
int BotIsAChickenWuss(bot_state_t *bs) {
    ...
    if (gLevelFlags & LEVELFLAG_IMUSTNTRUNAWAY) { // The level says we mustn't run away!
        return 0;
    }

    if (level.gametype == GT_SINGLE_PLAYER) { //"coop" (not really)
        return 0;
    }

    if (level.gametype == GT_JEDIMASTER && !bs->cur_ps.isJediMaster) { // Then you may know no fear.
        // Well, unless he's strong.
        if (bs->currentEnemy && bs->currentEnemy->client && bs->currentEnemy->client->ps.isJediMaster && bs->currentEnemy->health > 40 &&
            bs->cur_ps.weapon < WP_ROCKET_LAUNCHER) { // explosive weapons are most effective against the Jedi Master
            goto jmPass;
        }
        return 0;
    }
    ...
    if (level.gametype == GT_CTF && bs->currentEnemy && bs->currentEnemy->client) {
        if (bs->currentEnemy->client->ps.powerups[PW_REDFLAG] ||
            bs->currentEnemy->client->ps.powerups[PW_BLUEFLAG]) { // don't be afraid of flag carriers, they must die!
            return 0;
        }
    }
...
jmPass:
    ...
    if (bs->cur_ps.fd.forcePowersActive & (1 << FP_RAGE)) { // don't run while raging
        return 0;
    }

    if (level.gametype == GT_JEDIMASTER && !bs->cur_ps.isJediMaster) { // be frightened of the jedi master? I guess in this case.
        return 1;
    }

    bs->chickenWussCalculationTime = level.time + MAX_CHICKENWUSS_TIME;

    if (g_entities[bs->client].health < BOT_RUN_HEALTH) { // we're low on health, let's get away
        return 1;
    }
    ...
    if (bWRange == BWEAPONRANGE_MELEE || bWRange == BWEAPONRANGE_SABER) {
        if (bWRange != BWEAPONRANGE_SABER || !bs->saberSpecialist) { // run away if we're using melee, or if we're using a saber and not a "saber specialist"
            return 1;
        }
    }

    if (bs->cur_ps.weapon == WP_BRYAR_PISTOL) { // the bryar is a weak weapon, so just try to find a new one if it's what you're having to use
        return 1;
    }

    if (bs->currentEnemy && bs->currentEnemy->client && bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 512 &&
        bs->cur_ps.weapon != WP_SABER) { // if close to an enemy with a saber and not using a saber, then try to back off
        return 1;
    }

    if ((level.time - bs->cur_ps.electrifyTime) < 16000) { // lightning is dangerous.
        return 1;
    }
...
gentity_t *GetNearestBadThing(bot_state_t *bs) {
    ...
    while (i < level.num_entities) {
        ...
        if ((ent && !ent->client && ent->inuse && ent->damage &&
             /*(ent->s.weapon == WP_THERMAL || ent->s.weapon == WP_FLECHETTE)*/
             ent->s.weapon && ent->splashDamage) ||
            (ent && ent->genericValue5 == 1000 && ent->inuse && ent->health > 0 && ent->genericValue3 != bs->client && g_entities[ent->genericValue3].client &&
             !OnSameTeam(&g_entities[bs->client], &g_entities[ent->genericValue3]))) { // try to escape from anything with a non-0 s.weapon and non-0 damage.
                                                                                       // This hopefully only means dangerous projectiles.
            ...
            if (ent->s.weapon != WP_THERMAL && ent->s.weapon != WP_FLECHETTE && ent->s.weapon != WP_DET_PACK && ent->s.weapon != WP_TRIP_MINE) {
                factor = 0.5f;

                if (ent->s.weapon && glen <= 256 && bs->settings.skill > 2) { // it's a projectile so push it away
            ...
            if (ent->s.weapon == WP_ROCKET_LAUNCHER &&
                (ent->r.ownerNum == bs->client ||
                 (ent->r.ownerNum > 0 && ent->r.ownerNum < MAX_CLIENTS && g_entities[ent->r.ownerNum].client &&
                  OnSameTeam(&g_entities[bs->client], &g_entities[ent->r.ownerNum])))) { // don't be afraid of your own rockets or your teammates' rockets
                factor = 0;
            }
        ...
        if (ent && !ent->client && ent->inuse && ent->damage && ent->s.weapon && ent->r.ownerNum < MAX_CLIENTS &&
            ent->r.ownerNum >= 0) { // if we're in danger of a projectile belonging to someone and don't have an enemy, set the enemy to them
...
void GetNewFlagPoint(wpobject_t *wp, gentity_t *flagEnt, int team) { // get the nearest possible waypoint to the flag since it's not in its original position
    ...
    if (bestdist <= WP_KEEP_FLAG_DIST) {
        ...
        if (tr.fraction == 1) { // this point is good
            return;
        }
    }
...
int CTFTakesPriority(bot_state_t *bs) {
    ...
    } else if (bs->cur_ps.weapon == WP_BRYAR_PISTOL && (level.time - bs->lastDeadTime) < BOT_MAX_WEAPON_CHASE_CTF && bs->wpDestination &&
               bs->wpDestination->weight) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client) {
            ...
            if (botstates[ent->s.number]) {
                if (botstates[ent->s.number]->ctfState == CTFSTATE_ATTACKER || botstates[ent->s.number]->ctfState == CTFSTATE_RETRIEVAL)
                    numAttackers++;
                else
                    numDefenders++;
            } else { // assume real players to be attackers in our logic
                numAttackers++;
            }
        }
        i++;
    ...
    if (bs->ctfState == CTFSTATE_RETRIEVAL) {
        if (BotGetFlagBack(bs))
            goto success;
        else // can't find anyone on another team being a carrier, so ignore this priority
            bs->ctfState = 0;
    }
    ...
    if (bs->ctfState == CTFSTATE_GUARDCARRIER) {
        if (BotGuardFlagCarrier(bs))
            goto success;
        else // can't find anyone on our team being a carrier, so ignore this priority
            bs->ctfState = 0;
    }
...
success:
    if (dosw) { // allow ctf code to run, but if after a particular item then keep going after it
...
int EntityVisibleBox(vector3 *org1, vector3 *mins, vector3 *maxs, vector3 *org2, int ignore, int ignore2) {
    trace_t tr;
...
hasPoint:
    ...
    dif.x = (goalent->r.absmax.x + goalent->r.absmin.x) / 2;
    dif.y = (goalent->r.absmax.y + goalent->r.absmin.y) / 2;
    dif.z = (goalent->r.absmax.z + goalent->r.absmin.z) / 2;
    // brush models can have tricky origins, so this is our hacky method of getting the center point

    if (goalent->takedamage && testdistance < BOT_MIN_SIEGE_GOAL_SHOOT && EntityVisibleBox(&bs->origin, &mins, &maxs, &dif, bs->client, goalent->s.number)) {
        bs->shootGoal = goalent;
        bs->touchGoal = NULL;
    } else if (goalent->use && testdistance < BOT_MIN_SIEGE_GOAL_TRAVEL) {
        bs->shootGoal = NULL;
        bs->touchGoal = goalent;
    } else { // don't know how to handle this goal object!
        bs->shootGoal = NULL;
        bs->touchGoal = NULL;
    }
...
void Siege_DefendFromAttackers(bot_state_t *bs) { // this may be a little cheap, but the best way to find our defending point is probably
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && ent->client->sess.sessionTeam != g_entities[bs->client].client->sess.sessionTeam && ent->health > 0 &&
...
int Siege_CountDefenders(bot_state_t *bs) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && bot) {
            if (bot->siegeState == SIEGESTATE_DEFENDER && ent->client->sess.sessionTeam == g_entities[bs->client].client->sess.sessionTeam) {
                num++;
            }
        }
...
int SiegeTakesPriority(bot_state_t *bs) {
    ...
    } else if (bs->cur_ps.weapon == WP_BRYAR_PISTOL && (level.time - bs->lastDeadTime) < BOT_MAX_WEAPON_CHASE_TIME && bs->wpDestination &&
               bs->wpDestination->weight) {
        dest_sw = bs->wpDestination;
        dosw = 1;
    }
    ...
    if (attacker) {
        ...
        if (defenders > teammates / 3 && teammates > 1) { // devote around 1/4 of our team to completing our own side goals even if we're a defender.
    ...
    if (bs->siegeState == SIEGESTATE_ATTACKER) {
        if (!Siege_TargetClosestObjective(
                bs, flagForAttackableObjective)) { // looks like we have no goals other than to keep the other team from completing objectives
    ...
    if (dosw) { // allow siege objective code to run, but if after a particular item then keep going after it
        bs->wpDestination = dest_sw;
    }
...
int JMTakesPriority(bot_state_t *bs) {
    ...
    while (i < MAX_CLIENTS) {
        if (g_entities[i].client && g_entities[i].inuse && g_entities[i].client->ps.isJediMaster) {
            bs->jmState = i;
            break;
        }
...
int BotHasAssociated(bot_state_t *bs, wpobject_t *wp) {
    ...
    if (wp->associated_entity == ENTITYNUM_NONE) { // make it think this is an item we have so we don't go after nothing
        return 1;
    }
...
int GetBestIdleGoal(bot_state_t *bs) {
    ...
    if (bs->randomNav) { // stop looking for items and/or camping on them
        return -1;
    }

    for (int i = 0, size = gWPArray.size(); i < size; i++) {
        if (gWPArray[i] && (gWPArray[i]->flags & WPFLAG_GOALPOINT) && gWPArray[i]->weight > highestweight && !BotHasAssociated(bs, gWPArray[i])) {
...
void GetIdealDestination(bot_state_t *bs) {
    ...
    if (!badthing && bs->dontGoBack > level.time) {
        ...
        bs->wpDestination = NULL;
        return;
    } else if (!badthing && bs->wpStoreDest) { // after we finish running away, switch back to our original destination
    ...
    if (bs->revengeEnemy && bs->revengeEnemy->health > 0 && bs->revengeEnemy->client &&
        (bs->revengeEnemy->client->pers.connected == CON_CONNECTED ||
         bs->revengeEnemy->client->pers.connected == CON_CONNECTING)) { // if we hate someone, always try to get to them
        if (bs->wpDestSwitchTime < level.time) {
    ...
    } else if (bs->squadLeader && bs->squadLeader->health > 0 && bs->squadLeader->client &&
               (bs->squadLeader->client->pers.connected == CON_CONNECTED || bs->squadLeader->client->pers.connected == CON_CONNECTING)) {
        if (bs->wpDestSwitchTime < level.time) {
        ...
        if (bs->frame_Enemy_Len < distChange || (bChicken && bChicken != 2)) {
            ...
            if (bs->frame_Enemy_Len > 400) { // good distance away, start running toward a good place for an item or powerup or whatever
                ...
                if (idleWP != -1 && gWPArray[idleWP])
                    bs->wpDestination = gWPArray[idleWP];
            } else if (cWPIndex > 0 && cWPIndex + 1 < gWPArray.size() && gWPArray[cWPIndex - 1] && gWPArray[cWPIndex + 1]) {
            ...
            if (tempInt != -1 && TotalTrailDistance(bs->wpCurrent->index, tempInt, bs) != -1) {
                ...
                if (level.gametype == GT_SINGLE_PLAYER) { // be more aggressive
...
void CommanderBotCTFAI(bot_state_t *bs) {
    ...
    if (enemyHasOurFlag &&
        !weHaveEnemyFlag) { // start off with an attacker instead of a retriever if we don't have the enemy flag yet so that they can't capture it first.
    ...
    while (i < squadmates) {
        if (squad[i] && squad[i]->client && botstates[squad[i]->s.number]) {
            if (botstates[squad[i]->s.number]->ctfState != CTFSTATE_GETFLAGHOME) { // never tell a bot to stop trying to bring the flag to the base
            ...
            } else if ((numOnMyTeam < 2 || !numAttackers) &&
                       enemyHasOurFlag) { // I'm the only one on my team who will attack and the enemy has my flag, I have to go after him
                botstates[squad[i]->s.number]->ctfState = CTFSTATE_RETRIEVAL;
            }
        }
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && OnSameTeam(&g_entities[bs->client], ent) && botstates[ent->s.number]) {
            ...
            if (bst && !bst->isSquadLeader && !bst->state_Forced) {
                squad[squadmates] = ent;
                squadmates++;
            } else if (bst && !bst->isSquadLeader && bst->state_Forced) { // count them as commanded
                commanded++;
            }
        }
...
void BotDoTeamplayAI(bot_state_t *bs) {
    ...
    if (bs->teamplayState == TEAMPLAYSTATE_REGROUP) { // force to find a new leader
        bs->squadLeader = NULL;
        bs->isSquadLeader = 0;
    }
}
...
void CommanderBotTeamplayAI(bot_state_t *bs) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && OnSameTeam(&g_entities[bs->client], ent) && botstates[ent->s.number]) {
            ...
            if (foundsquadleader && bst && bst->isSquadLeader) { // never more than one squad leader
                bst->isSquadLeader = 0;
            }
    ...
    while (i < squadmates && squad[i]) {
        ...
        if (bst && !bst->state_Forced) {                      // only order if this guy is not being ordered directly by the real player team leader
            if (teammate_indanger >= 0 && !teammate_helped) { // send someone out to help whoever needs help most at the moment
                bst->teamplayState = TEAMPLAYSTATE_ASSISTING;
                bst->squadLeader = &g_entities[teammate_indanger];
                teammate_helped = 1;
            } else if ((teammate_indanger == -1 || teammate_helped) &&
                       bst->teamplayState == TEAMPLAYSTATE_ASSISTING) { // no teammates need help badly, but this guy is trying to help them anyway, so stop
                bst->teamplayState = TEAMPLAYSTATE_FOLLOWING;
                bst->squadLeader = &g_entities[bs->client];
            }

            if (bs->squadRegroupInterval < level.time && Q_irand(1, 10) < 5) { // every so often tell the squad to regroup for the sake of variation
...
void MeleeCombatHandling(bot_state_t *bs) {
    ...
    me_down = (int)tr.endpos.z;

    VectorSubtract(&usethisvec, &bs->origin, &a);
    vectoangles(&a, &a);
    AngleVectors(&a, &fwd, NULL, NULL);

    midorg.x = bs->origin.x + fwd.x * bs->frame_Enemy_Len / 2;
    midorg.y = bs->origin.y + fwd.y * bs->frame_Enemy_Len / 2;
    midorg.z = bs->origin.z + fwd.z * bs->frame_Enemy_Len / 2;
...
void SaberCombatHandling(bot_state_t *bs) {
    ...
    if (me_down == en_down && en_down == mid_down) {
        if (usethisvec.z > (bs->origin.z + 32) && bs->currentEnemy->client && bs->currentEnemy->client->ps.groundEntityNum == ENTITYNUM_NONE) {
            bs->jumpTime = level.time + 100;
        }

        if (bs->frame_Enemy_Len > 128) { // be ready to attack
            bs->saberDefending = 0;
        ...
        if (bs->currentEnemy && bs->currentEnemy->client) {
            if (!BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove) && bs->frame_Enemy_Len > 90 && bs->saberBFTime > level.time &&
                bs->saberBTime > level.time && bs->beStill < level.time && bs->saberSTime < level.time) {
                bs->beStill = level.time + Q_irand(500, 1000);
                bs->saberSTime = level.time + Q_irand(1200, 1800);
            } else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80 &&
                       ((Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) || bs->saberBTime > level.time ||
                        BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK ||
                ...
                if (BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK ||
                    bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL)
                    idealDist = 256;
                else
                    idealDist = 64;
                ...
                while (checkIncr < idealDist) {
                    bs->goalPosition.x = bs->origin.x + vs.x * checkIncr;
                    bs->goalPosition.y = bs->origin.y + vs.y * checkIncr;
                    bs->goalPosition.z = bs->origin.z + vs.z * checkIncr;
                    ...
                    if (tr.fraction == 1.0f) { // don't back off of a ledge
                        VectorCopy(&usethisvec, &bs->goalPosition);
                        break;
                    }
                    checkIncr += 64;
        ...
        if (bs->meleeStrafeDir)
        {
        bs->goalPosition.x += fwd.x*16;
        bs->goalPosition.y += fwd.y*16;
        bs->goalPosition.z += fwd.z*16;
        }
        else
        {
        bs->goalPosition.x -= fwd.x*16;
...
void BotAimLeading(bot_state_t *bs, vector3 *headlevel, float leadAmount) {
    int x;
    ...
    if (bs->currentEnemy->client->ps.velocity.x < 0)
        vtotal += -bs->currentEnemy->client->ps.velocity.x;
    else
        vtotal += bs->currentEnemy->client->ps.velocity.x;

    if (bs->currentEnemy->client->ps.velocity.y < 0)
        vtotal += -bs->currentEnemy->client->ps.velocity.y;
    else
        vtotal += bs->currentEnemy->client->ps.velocity.y;

    if (bs->currentEnemy->client->ps.velocity.z < 0)
        vtotal += -bs->currentEnemy->client->ps.velocity.z;
    else
        vtotal += bs->currentEnemy->client->ps.velocity.z;
    ...
    predictedSpot.z = headlevel->z + (movementVector.z * x);
...
void BotAimOffsetGoalAngles(bot_state_t *bs) {
    ...
    if (bs->currentEnemy &&
        BotMindTricked(bs->client, bs->currentEnemy->s.number)) { // having to judge where they are by hearing them, so we should be quite inaccurate here
        accVal *= 7;
    ...
    if (bs->revengeEnemy && bs->revengeHateLevel && bs->currentEnemy == bs->revengeEnemy) { // bot becomes more skilled as anger level raises
        accVal = accVal / bs->revengeHateLevel;
    }

    if (bs->currentEnemy && bs->frame_Enemy_Vis) { // assume our goal is aiming at the enemy, seeing as he's visible and all
        if (!bs->currentEnemy->s.pos.trDelta.x && !bs->currentEnemy->s.pos.trDelta.y && !bs->currentEnemy->s.pos.trDelta.z) {
        ...
        if (g_entities[bs->client].s.pos.trDelta.x || g_entities[bs->client].s.pos.trDelta.y || g_entities[bs->client].s.pos.trDelta.z) {
...
int ShouldSecondaryFire(bot_state_t *bs) {
    ...
    if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT && bs->cur_ps.weapon == WP_ROCKET_LAUNCHER) {
        ...
        if (heldTime > 5000) { // just give up and release it if we can't manage a lock in 5 seconds
            return 2;
        }
...
int CombatBotAI(bot_state_t *bs, float thinktime) {
    ...
    if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER) {
        ...
        if (bs->cur_ps.weapon == WP_THERMAL || bs->cur_ps.weapon == WP_ROCKET_LAUNCHER) { // be careful with the hurty weapons
            fovcheck = 40;

            if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT &&
                bs->cur_ps.weapon == WP_ROCKET_LAUNCHER) { // if we're charging the weapon up then we can hold fire down within a normal fov
                fovcheck = 60;
            }
        } else {
            fovcheck = 60;
        ...
        if (InFieldOfVision(&bs->viewangles, fovcheck, &a)) {
            if (bs->cur_ps.weapon == WP_THERMAL) {
                if (((level.time - bs->cur_ps.weaponChargeTime) < (bs->frame_Enemy_Len * 2) && (level.time - bs->cur_ps.weaponChargeTime) < 4000 &&
                     bs->frame_Enemy_Len > 64) ||
                    (bs->cur_ps.weaponstate != WEAPON_CHARGING && bs->cur_ps.weaponstate != WEAPON_CHARGING_ALT)) {
                    if (bs->cur_ps.weaponstate != WEAPON_CHARGING && bs->cur_ps.weaponstate != WEAPON_CHARGING_ALT) {
                ...
                if (secFire == 1) {
                    ...
                    if (bs->cur_ps.weapon != WP_THERMAL) {
                        if (bs->cur_ps.weaponstate != WEAPON_CHARGING || bs->altChargeTime > (level.time - bs->cur_ps.weaponChargeTime)) {
                            bs->doAttack = 1;
                        }
                    } else {
                        bs->doAttack = 1;
                ...
                if (secFire == 2) { // released a charge
                    return 1;
                }
            }
        }
...
int BotFallbackNavigation(bot_state_t *bs) {
    vector3 b_angle, fwd, trto, mins, maxs;
    trace_t tr;
    ...
    trto.x = bs->origin.x + fwd.x * 16;
    trto.y = bs->origin.y + fwd.y * 16;
    trto.z = bs->origin.z + fwd.z * 16;
...
int BotTryAnotherWeapon(bot_state_t *bs) { // out of ammo, resort to the first weapon we come across that has ammo
    int i;

    i = 1;

    while (i < WP_NUM_WEAPONS) {
        if (bs->cur_ps.ammo[weaponData[i].ammoIndex] >= weaponData[i].shotCost && (bs->cur_ps.stats[STAT_WEAPONS] & (1 << i))) {
            bs->virtualWeapon = i;
    ...
    if (bs->cur_ps.weapon != 1 && bs->virtualWeapon != 1) { // should always have this.. shouldn't we?
        bs->virtualWeapon = 1;
...
qboolean BotWeaponSelectable(bot_state_t *bs, int weapon) {
    ...
    if (bs->cur_ps.ammo[weaponData[weapon].ammoIndex] >= weaponData[weapon].shotCost && (bs->cur_ps.stats[STAT_WEAPONS] & (1 << weapon))) {
        return qtrue;
    }
...
int BotSelectIdealWeapon(bot_state_t *bs) {
    ...
    while (i < WP_NUM_WEAPONS) {
        if (bs->cur_ps.ammo[weaponData[i].ammoIndex] >= weaponData[i].shotCost && bs->botWeaponWeights[i] > bestweight &&
    ...
    if (bs->currentEnemy && bs->frame_Enemy_Len < 300 && (bestweapon == WP_BRYAR_PISTOL || bestweapon == WP_BLASTER || bestweapon == WP_BOWCASTER) &&
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SABER))) {
        bestweapon = WP_SABER;
        bestweight = 1;
    }

    if (bs->currentEnemy && bs->frame_Enemy_Len > 300 && bs->currentEnemy->client && bs->currentEnemy->client->ps.weapon != WP_SABER &&
        (bestweapon == WP_SABER)) { // if the enemy is far away, and we have our saber selected, see if we have any good distance weapons instead
...
int BotSelectChoiceWeapon(bot_state_t *bs, int weapon,
                          int doselection) { // if !doselection then bot will only check if he has the specified weapon and return 1 (yes) or 0 (no)
    int i;
    int hasit = 0;
    ...
    while (i < WP_NUM_WEAPONS) {
        if (bs->cur_ps.ammo[weaponData[i].ammoIndex] > weaponData[i].shotCost && i == weapon && (bs->cur_ps.stats[STAT_WEAPONS] & (1 << i))) {
            hasit = 1;
            break;
        }
...
void BotLovedOneDied(bot_state_t *bs, bot_state_t *loved, int lovelevel) {
    if (!loved->lastHurt || !loved->lastHurt->client || loved->lastHurt->s.number == loved->client) {
        return;
    }
    ...
    } else if (OnSameTeam(&g_entities[bs->client], loved->lastHurt)) { // don't hate teammates no matter what
        return;
    }
    ...
    if (!PassLovedOneCheck(bs, loved->lastHurt)) { // a loved one killed a loved one.. you cannot hate them
    ...
    } else if (bs->revengeHateLevel < bs->loved_death_thresh - 1) { // only switch hatred if we don't hate the existing revenge-enemy too much
...
void BotDeathNotify(bot_state_t *bs) { // in case someone has an emotional attachment to us, we'll notify them
    int i = 0;
    int ltest = 0;
...
void StrafeTracing(bot_state_t *bs) {
    vector3 mins, maxs;
    vector3 right, rorg, drorg;
    trace_t tr;

    VectorSet(&mins, -15, -15, -22 /*-24*/);
    VectorSet(&maxs, 15, 15, 32);

    AngleVectors(&bs->viewangles, NULL, &right, NULL);

    if (bs->meleeStrafeDir) {
        rorg.x = bs->origin.x - right.x * 32;
        rorg.y = bs->origin.y - right.y * 32;
        rorg.z = bs->origin.z - right.z * 32;
    } else {
        rorg.x = bs->origin.x + right.x * 32;
        rorg.y = bs->origin.y + right.y * 32;
        rorg.z = bs->origin.z + right.z * 32;
    ...
    if (tr.fraction == 1) { // this may be a dangerous ledge, so don't strafe over it just in case
        bs->meleeStrafeDisable = level.time + Q_irand(500, 1500);
    }
}
...
gentity_t *CheckForFriendInLOF(bot_state_t *bs) {
    ...
    trto.x = trfrom.x + fwd.x * 2048;
    trto.y = trfrom.y + fwd.y * 2048;
    trto.z = trfrom.z + fwd.z * 2048;
...
void BotScanForLeader(bot_state_t *bs) { // bots will only automatically obtain a leader if it's another bot using this method.
    int i = 0;
    gentity_t *ent;
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && botstates[i] && botstates[i]->isSquadLeader && bs->client != i) {
            ...
            if (GetLoveLevel(bs, botstates[i]) > 1 && !IsTeamplay()) { // ignore love status regarding squad leaders if we're in teamplay
                bs->squadLeader = ent;
                break;
            }
        }
...
void BotReplyGreetings(bot_state_t *bs) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (numhello > 3) { // don't let more than 4 bots say hello at once
            return;
        }
...
void CTFFlagMovement(bot_state_t *bs) {
    ...
    VectorSet(&mins, -15, -15, -7);
    VectorSet(&maxs, 15, 15, 7);

    if (bs->wantFlag && (bs->wantFlag->flags & FL_DROPPED_ITEM)) {
        if (bs->staticFlagSpot.x == bs->wantFlag->s.pos.trBase.x && bs->staticFlagSpot.y == bs->wantFlag->s.pos.trBase.y &&
            bs->staticFlagSpot.z == bs->wantFlag->s.pos.trBase.z) {
    ...
    if (flagRed && flagBlue) {
        if (bs->wpDestination == flagRed || bs->wpDestination == flagBlue) {
            if (bs->wpDestination == flagRed && droppedRedFlag && (droppedRedFlag->flags & FL_DROPPED_ITEM) && droppedRedFlag->classname &&
                strcmp(droppedRedFlag->classname, "freed") != 0) {
                desiredDrop = droppedRedFlag;
                diddrop = 1;
            }
            if (bs->wpDestination == flagBlue && droppedBlueFlag && (droppedBlueFlag->flags & FL_DROPPED_ITEM) && droppedBlueFlag->classname &&
                strcmp(droppedBlueFlag->classname, "freed") != 0) {
...
void BotCheckDetPacks(bot_state_t *bs) {
    ...
    if (!bs->currentEnemy || !bs->currentEnemy->client || !bs->frame_Enemy_Vis) { // require the enemy to be visilbe just to be fair..

        // unless..
        if (bs->currentEnemy && bs->currentEnemy->client &&
            (level.time - bs->plantContinue) < 5000) { // it's a fresh plant (within 5 seconds) so we should be able to guess
            goto stillmadeit;
        }
        return;
    }
...
stillmadeit:
    ...
    if (enLen < BOT_PLANT_BLOW_DISTANCE &&
        OrgVisible(&bs->currentEnemy->client->ps.origin, &myDet->s.pos.trBase,
                   bs->currentEnemy->s.number)) { // we could just call the "blow all my detpacks" function here, but I guess that's cheating.
...
int BotUseInventoryItem(bot_state_t *bs) {
    ...
    if (bs->cur_ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SHIELD)) {
        if (bs->currentEnemy && bs->frame_Enemy_Vis &&
            bs->runningToEscapeThreat) { // this will (hopefully) result in the bot placing the shield down while facing
...
int BotSurfaceNear(bot_state_t *bs) {
    ...
    fwd.x = bs->origin.x + (fwd.x * 64);
    fwd.y = bs->origin.y + (fwd.y * 64);
    fwd.z = bs->origin.z + (fwd.z * 64);
...
void StandardBotAI(bot_state_t *bs, float thinktime) {
    ...
    vector3 a, ang, headlevel, eorg, noz_x, noz_y, dif, a_fo, preFrameGAngles;
    float reaction, bLeadAmount, mLen;
    int visResult = 0, selResult = 0, mineSelect = 0, detSelect = 0;
    ...
    if (g_entities[bs->client].inuse && g_entities[bs->client].client && g_entities[bs->client].client->sess.sessionTeam == TEAM_SPECTATOR) {
        bs->wpCurrent = NULL;
        bs->currentEnemy = NULL;
        bs->wpDestination = NULL;
        bs->wpDirection = 0;
...
#ifdef _DEBUG
    if (bot_getinthecarrr.integer) { // stupid vehicle debug, I tire of having to connect another client to test passengers.
        gentity_t *botEnt = &g_entities[bs->client];

        if (botEnt->inuse && botEnt->client && botEnt->client->ps.m_iVehicleNum) { // in a vehicle, so...
            bs->noUseTime = level.time + 5000;

            if (bot_getinthecarrr.integer != 2) {
                trap->EA_MoveForward(bs->client);

                if (bot_getinthecarrr.integer == 3) { // use alt fire
                    trap->EA_Alt_Attack(bs->client);
                }
            }
        } else { // find one, get in
            int i = 0;
            ...
            while (i < MAX_GENTITIES) {
                ...
                if (vehicle->inuse && vehicle->client && vehicle->s.eType == ET_NPC && vehicle->s.NPC_class == CLASS_VEHICLE && vehicle->m_pVehicle &&
                    (vehicle->client->ps.m_iVehicleNum || bot_getinthecarrr.integer == 2)) { // ok, this is a vehicle, and it has a pilot/passengers
                    break;
                }
                i++;
            }
            if (i != MAX_GENTITIES && vehicle) { // broke before end so we must've found something
                vector3 v;
    ...
    if (bot_forgimmick.integer) {
        ...
        if (bot_forgimmick.integer == 2) { // for debugging saber stuff, this is handy
            trap->EA_Attack(bs->client);
        }

        if (bot_forgimmick.integer == 3) { // for testing cpu usage moving around rmg terrain without AI
            vector3 mdir;
        ...
        if (bot_forgimmick.integer == 4) { // constantly move toward client 0
    ...
    if (!bs->lastDeadTime) { // just spawned in?
    ...
    if (g_entities[bs->client].health < 1) {
        ...
        if (!bs->deathActivitiesDone && bs->lastHurt && bs->lastHurt->client && bs->lastHurt->s.number != bs->client) {
            ...
            if (PassLovedOneCheck(bs, bs->lastHurt)) {
                ...
                bs->chatAltObject = NULL;
                BotDoChat(bs, "Died", 0);
            } else if (!PassLovedOneCheck(bs, bs->lastHurt) && botstates[bs->lastHurt->s.number] &&
                       PassLovedOneCheck(botstates[bs->lastHurt->s.number],
                                         &g_entities[bs->client])) { // killed by a bot that I love, but that does not love me
                bs->chatObject = bs->lastHurt;
                bs->chatAltObject = NULL;
    ...
    if (bs->revengeEnemy && bs->revengeEnemy->client && bs->revengeEnemy->client->pers.connected != CON_CONNECTED &&
        bs->revengeEnemy->client->pers.connected != CON_CONNECTING) {
...
#endif
    {
        ...
        if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PUSH)) && (bs->doForcePush > level.time || bs->cur_ps.fd.forceGripBeingGripped > level.time) &&
            level.clients[bs->client].ps.fd.forcePower >
                forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_PUSH]][FP_PUSH] /*&& InFieldOfVision(bs->viewangles, 50, a_fo)*/) {
            level.clients[bs->client].ps.fd.forcePowerSelected = FP_PUSH;
            useTheForce = 1;
            forceHostile = 1;
        } else if (bs->cur_ps.fd.forceSide == FORCESIDE_DARK) { // try dark side powers
            // in order of priority top to bottom
            if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_GRIP)) && (bs->cur_ps.fd.forcePowersActive & (1 << FP_GRIP)) &&
                InFieldOfVision(&bs->viewangles, 50, &a_fo)) { // already gripping someone, so hold it
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_GRIP;
                useTheForce = 1;
                forceHostile = 1;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_LIGHTNING)) && bs->frame_Enemy_Len < FORCE_LIGHTNING_RADIUS &&
                       level.clients[bs->client].ps.fd.forcePower > 50 && InFieldOfVision(&bs->viewangles, 50, &a_fo)) {
                ...
                forceHostile = 1;
                       ...
                       level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_GRIP]][FP_GRIP] &&
                       InFieldOfVision(&bs->viewangles, 50, &a_fo)) {
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_GRIP;
                useTheForce = 1;
                forceHostile = 1;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_RAGE)) && g_entities[bs->client].health < 25 &&
                       level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_RAGE]][FP_RAGE]) {
                ...
                forceHostile = 0;
                       ...
                       level.clients[bs->client].ps.fd.forcePower > 50 && InFieldOfVision(&bs->viewangles, 50, &a_fo) &&
                       bs->currentEnemy->client->ps.fd.forcePower > 10 && bs->currentEnemy->client->ps.fd.forceSide == FORCESIDE_LIGHT) {
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_DRAIN;
                useTheForce = 1;
                forceHostile = 1;
            }
        } else if (bs->cur_ps.fd.forceSide == FORCESIDE_LIGHT) { // try light side powers
            if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_ABSORB)) && bs->cur_ps.fd.forceGripCripple &&
                level.clients[bs->client].ps.fd.forcePower >
                    forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_ABSORB]][FP_ABSORB]) { // absorb to get out
                ...
                forceHostile = 0;
                       ...
                       level.clients[bs->client].ps.fd.forcePower >
                           forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_ABSORB]][FP_ABSORB]) { // absorb lightning
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_ABSORB;
                useTheForce = 1;
                forceHostile = 0;
                       ...
                       InFieldOfVision(&bs->viewangles, 50, &a_fo) && !(bs->currentEnemy->client->ps.fd.forcePowersActive & (1 << FP_SEE))) {
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_TELEPATHY;
                useTheForce = 1;
                forceHostile = 1;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_ABSORB)) && g_entities[bs->client].health < 75 &&
                       bs->currentEnemy->client->ps.fd.forceSide == FORCESIDE_DARK &&
                ...
                useTheForce = 1;
                forceHostile = 0;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PROTECT)) && g_entities[bs->client].health < 35 &&
                       level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_PROTECT]][FP_PROTECT]) {
        ...
        if (!useTheForce) { // try neutral powers
            if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PUSH)) && bs->cur_ps.fd.forceGripBeingGripped > level.time &&
                ...
                useTheForce = 1;
                forceHostile = 1;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_SPEED)) && g_entities[bs->client].health < 25 &&
                ...
                useTheForce = 1;
                forceHostile = 0;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_SEE)) && BotMindTricked(bs->client, bs->currentEnemy->s.number) &&
                       level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_SEE]][FP_SEE]) {
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_SEE;
                useTheForce = 1;
                forceHostile = 0;
            } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PULL)) && bs->frame_Enemy_Len < 256 && level.clients[bs->client].ps.fd.forcePower > 75 &&
                       InFieldOfVision(&bs->viewangles, 50, &a_fo)) {
    ...
    if (!useTheForce) { // try powers that we don't care if we have an enemy for
        if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_HEAL)) && g_entities[bs->client].health < 50 &&
            level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_HEAL]][FP_HEAL] &&
            bs->cur_ps.fd.forcePowerLevel[FP_HEAL] > FORCE_LEVEL_1) {
            level.clients[bs->client].ps.fd.forcePowerSelected = FP_HEAL;
            useTheForce = 1;
            forceHostile = 0;
        } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_HEAL)) && g_entities[bs->client].health < 50 &&
                   level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_HEAL]][FP_HEAL] &&
                   !bs->currentEnemy && bs->isCamping > level.time) { // only meditate and heal if we're camping
            level.clients[bs->client].ps.fd.forcePowerSelected = FP_HEAL;
            useTheForce = 1;
            forceHostile = 0;
        }
    ...
    if (bs->cur_ps.ammo[weaponData[bs->cur_ps.weapon].ammoIndex] < weaponData[bs->cur_ps.weapon].shotCost) {
        ...
        if (bs->currentEnemy && bs->lastVisibleEnemyIndex == bs->currentEnemy->s.number && bs->frame_Enemy_Vis &&
            bs->forceWeaponSelect /*&& bs->plantContinue < level.time*/)
    ...
    if (bs->currentEnemy) {
        if (bs->enemySeenTime < level.time || !PassStandardEnemyChecks(bs, bs->currentEnemy)) {
            if (bs->revengeEnemy == bs->currentEnemy && bs->currentEnemy->health < 1 && bs->lastAttacked && bs->lastAttacked == bs->currentEnemy) {
                ...
                bs->revengeEnemy = NULL;
                bs->revengeHateLevel = 0;
            } else if (bs->currentEnemy->health < 1 && PassLovedOneCheck(bs, bs->currentEnemy) && bs->lastAttacked && bs->lastAttacked == bs->currentEnemy) {
                // CHAT: Killed
    ...
    if (bot_honorableduelacceptance.integer) {
        if (bs->currentEnemy && bs->currentEnemy->client && bs->cur_ps.weapon == WP_SABER && g_privateDuel.integer && bs->frame_Enemy_Vis &&
            bs->frame_Enemy_Len < 400 && bs->currentEnemy->client->ps.weapon == WP_SABER && bs->currentEnemy->client->ps.saberHolstered) {
            vector3 e_ang_vec;

            VectorSubtract(&bs->currentEnemy->client->ps.origin, &bs->eye, &e_ang_vec);

            if (InFieldOfVision(&bs->viewangles, 100, &e_ang_vec)) { // Our enemy has his saber holstered and has challenged us to a duel, so challenge him back
                if (!bs->cur_ps.saberHolstered) {
                    Cmd_ToggleSaber_f(&g_entities[bs->client]);
                } else {
                    if (bs->currentEnemy->client->ps.duelIndex == bs->client && bs->currentEnemy->client->ps.duelTime > level.time &&
    ...
    if (!bs->squadLeader && bs->squadCannotLead < level.time) { // if still no leader after scanning, then become a squad leader
        bs->isSquadLeader = 1;
    }

    if (bs->isSquadLeader && bs->squadLeader) { // we don't follow anyone if we are a leader
        bs->squadLeader = NULL;
    }
    ...
    if (bs->currentEnemy) {
        if (bs->currentEnemy->client) {
            VectorCopy(&bs->currentEnemy->client->ps.origin, &eorg);
            eorg.z += bs->currentEnemy->client->ps.viewheight;
        } else {
            VectorCopy(&bs->currentEnemy->s.origin, &eorg);
        }
    ...
    if (bs->wpCurrent) {
        ...
        if (!bs->wpCurrent) { // WPConstantRoutine has the ability to nullify the waypoint if it fails certain checks, so..
            return;
        }
        ...
        } else if (goalWPIndex >= 0 && goalWPIndex < gWPArray.size() && gWPArray[goalWPIndex] && !(gLevelFlags & LEVELFLAG_NOPOINTPREDICTION)) {
        ...
        if (bs->frame_Waypoint_Len < wpTouchDist) {
            ...
            if (desiredIndex >= 0 && desiredIndex < gWPArray.size() && gWPArray[desiredIndex] && PassWayCheck(bs, desiredIndex)) {
    ...
    if (bs->timeToReact < level.time && bs->currentEnemy && bs->enemySeenTime > level.time + (ENEMY_FORGET_MS - (ENEMY_FORGET_MS * 0.2f))) {
        ...
        } else if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT) { // keep charging in case we see him again before we lose track of him
            bs->doAltAttack = 1;
        } else if (bs->cur_ps.weaponstate == WEAPON_CHARGING) { // keep charging in case we see him again before we lose track of him
            bs->doAttack = 1;
        }

        if (bs->destinationGrabTime > level.time + 100) {
            bs->destinationGrabTime =
                level.time + 100; // assures that we will continue staying within a general area of where we want to be in a combat situation
        }

        if (bs->currentEnemy->client) {
            VectorCopy(&bs->currentEnemy->client->ps.origin, &headlevel);
            headlevel.z += bs->currentEnemy->client->ps.viewheight;
        } else {
            VectorCopy(&bs->currentEnemy->client->ps.origin, &headlevel);
        }
        ...
        if (!bs->frame_Enemy_Vis) {
            ...
            if (OrgVisible(&bs->eye, &bs->lastEnemySpotted, -1)) {
                ...
                if (bs->cur_ps.weapon == WP_FLECHETTE && bs->cur_ps.weaponstate == WEAPON_READY && bs->currentEnemy && bs->currentEnemy->client) {
    ...
    if (bs->currentEnemy) {
        if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER) {
            ...
            if (bs->saberPowerTime < level.time) { // Don't just use strong attacks constantly, switch around a bit
                if (Q_irand(1, 10) <= 5) {
                    bs->saberPower = qtrue;
                } else {
            ...
            if (g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_STAFF && g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_DUAL) {
                if (bs->currentEnemy->health > 75 && g_entities[bs->client].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] > 2) {
                    if (g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_STRONG &&
                        bs->saberPower) { // if we are up against someone with a lot of health and we have a strong attack available, then h4q them
                        Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
                    }
                } else if (bs->currentEnemy->health > 40 && g_entities[bs->client].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] > 1) {
                    if (g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_MEDIUM) { // they're down on health a little, use level 2 if we can
                        Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
                    }
                } else {
                    if (g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_FAST) { // they've gone below 40 health, go at them with quick attacks
                        Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
                    }
                }
            }
            ...
            } else if (bs->saberThrowTime < level.time && !bs->cur_ps.saberInFlight && (bs->cur_ps.fd.forcePowersKnown & (1 << FP_SABERTHROW)) &&
                       InFieldOfVision(&bs->viewangles, 30, &a_fo) && bs->frame_Enemy_Len < BOT_SABER_THROW_RANGE && bs->cur_ps.fd.saberAnimLevel != SS_STAFF) {
                bs->doAltAttack = 1;
                bs->doAttack = 0;
    ...
    if (/*bs->wpDestination &&*/ bs->shootGoal &&
        ...
        if (!bs->currentEnemy || bs->frame_Enemy_Len > 256) { // if someone is close then don't stop shooting them for this
            VectorSubtract(&dif, &bs->eye, &a);
            vectoangles(&a, &a);
            VectorCopy(&a, &bs->goalAngles);

            if (InFieldOfVision(&bs->viewangles, 30, &a) && EntityVisibleBox(&bs->origin, NULL, NULL, &dif, bs->client, bs->shootGoal->s.number)) {
                bs->doAttack = 1;
            }
        }
    }

    if (bs->cur_ps.hasDetPackPlanted) { // check if our enemy gets near it and detonate if he does
        BotCheckDetPacks(bs);
        ...
        if (bs->plantDecided > level.time || (bs->frame_Enemy_Len < BOT_PLANT_DISTANCE * 2 && VectorLength(&a) < BOT_PLANT_DISTANCE)) {
            ...
            if (bs->plantDecided > level.time && bs->forceWeaponSelect && bs->cur_ps.weapon == bs->forceWeaponSelect) {
                bs->doAttack = 1;
                bs->plantDecided = 0;
                ...
                if (BotSurfaceNear(bs)) {
                    if (!mineSelect) { // if no mines use detpacks, otherwise use mines
                    ...
                    if (detSelect && detSelect != 2) { // We have it and it is now our weapon
    ...
    if (bs->beStill < level.time && !WaitingForNow(bs, &bs->goalPosition) && !fjHalt) {
        ...
        if (bs->jumpTime > level.time && bs->jDelay < level.time && level.clients[bs->client].pers.cmd.upmove > 0) {
    ...
    if (bs->jumpTime > level.time && bs->jDelay < level.time) {
        if (bs->jumpHoldTime > level.time) {
            trap->EA_Jump(bs->client);
            if (bs->wpCurrent) {
                if ((bs->wpCurrent->origin.z - bs->origin.z) < 64) {
                    trap->EA_MoveForward(bs->client);
                }
            } else {
                trap->EA_MoveForward(bs->client);
    ...
    if (bs->dangerousObject && bs->dangerousObject->inuse && bs->dangerousObject->health > 0 && bs->dangerousObject->takedamage &&
        (!bs->frame_Enemy_Vis || !bs->currentEnemy) && (BotGetWeaponRange(bs) == BWEAPONRANGE_MID || BotGetWeaponRange(bs) == BWEAPONRANGE_LONG) &&
        bs->cur_ps.weapon != WP_DET_PACK && bs->cur_ps.weapon != WP_TRIP_MINE && !bs->shootGoal) {
        float danLen;
    ...
    if (PrimFiring(bs) || AltFiring(bs)) {
        ...
        if (friendInLOF) {
            ...
            if (!useTheForce && friendInLOF->client) { // we have a friend here and are not currently using force powers, see if we can help them out
                if (friendInLOF->health <= 50 && level.clients[bs->client].ps.fd.forcePower >
                                                     forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_TEAM_HEAL]][FP_TEAM_HEAL]) {
    ...
    } else if (level.gametype >= GT_TEAM) { // still check for anyone to help..
        friendInLOF = CheckForFriendInLOF(bs);

        if (!useTheForce && friendInLOF) {
            if (friendInLOF->health <= 50 &&
                level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_TEAM_HEAL]][FP_TEAM_HEAL]) {
                level.clients[bs->client].ps.fd.forcePowerSelected = FP_TEAM_HEAL;
                useTheForce = 1;
                forceHostile = 0;
            } else if (friendInLOF->client->ps.fd.forcePower <= 50 &&
    ...
    if (bs->doAttack && bs->cur_ps.weapon == WP_DET_PACK &&
        bs->cur_ps.hasDetPackPlanted) { // maybe a bit hackish, but bots only want to plant one of these at any given time to avoid complications
        bs->doAttack = 0;
    }

    if (bs->doAttack && bs->cur_ps.weapon == WP_SABER && bs->saberDefending && bs->currentEnemy && bs->currentEnemy->client &&
    ...
    if (bs->cur_ps.weapon == WP_SABER && bs->cur_ps.saberInFlight && !bs->cur_ps.saberEntityNum) { // saber knocked away, keep trying to get it back
        bs->doAttack = 1;
        bs->doAltAttack = 0;
    }
