...
static int G_SaberAttackPower(gentity_t *ent, qboolean attacking) {
    ...
    if (attacking) { // the attacker gets a boost to help penetrate defense.
        ...
        if (ent->client->lastSaberStorageTime >= (level.time - 50) && ent->client->olderIsValid) {
            vector3 vSub;
            int swingDist;
            int toleranceAmt;
    ...
    if (level.gametype == GT_POWERDUEL && ent->client->sess.duelTeam == DUELTEAM_LONE) { // get more power then
        return baseLevel * 2;
...
void WP_DeactivateSaber(gentity_t *self, qboolean clearLength) {
    ...
    if (!self->client->ps.saberHolstered) {
        ...
        */
        ...
        if (self->client->saber[1].soundOff && self->client->saber[1].model[0]) {
...
void WP_ActivateSaber(gentity_t *self) {
    ...
    if (self->NPC && self->client->ps.forceHandExtend == HANDEXTEND_JEDITAUNT &&
...
void SaberUpdateSelf(gentity_t *ent) {
    ...
    if (g_entities[ent->r.ownerNum].client->ps.saberInFlight &&
        g_entities[ent->r.ownerNum].health > 0) { // let The Master take care of us now (we'll get treated like a missile until we return)
        ent->nextthink = level.time;
    ...
    if (g_entities[ent->r.ownerNum].client->ps.weapon != WP_SABER || (g_entities[ent->r.ownerNum].client->ps.pm_flags & PMF_FOLLOW) ||
        // RWW ADDED 7-19-03 BEGIN
        g_entities[ent->r.ownerNum].client->sess.sessionTeam == TEAM_SPECTATOR || g_entities[ent->r.ownerNum].client->tempSpectate >= level.time ||
        // RWW ADDED 7-19-03 END
        g_entities[ent->r.ownerNum].health < 1 || BG_SabersOff(&g_entities[ent->r.ownerNum].client->ps) ||
        (!g_entities[ent->r.ownerNum].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] &&
         g_entities[ent->r.ownerNum].s.eType != ET_NPC)) { // owner is not using saber, spectating, dead, saber holstered, or has no attack level
        ent->r.contents = 0;
        ent->clipmask = 0;
        ...
        if (ent->r.contents != CONTENTS_LIGHTSABER) {
            if ((level.time - g_entities[ent->r.ownerNum].client->lastSaberStorageTime) <=
                200) { // Only go back to solid once we're sure our owner has updated recently
...
static void SetSaberBoxSize(gentity_t *saberent) {
    ...
    } else if (saberent->r.ownerNum >= 0 && saberent->r.ownerNum < ENTITYNUM_WORLD && g_entities[saberent->r.ownerNum].s.eType == ET_NPC) {
        owner = &g_entities[saberent->r.ownerNum];
    }
        ...
        for (saberNum = 0; saberNum < MAX_SABERS; saberNum++) {
            if (saberNum > 0 && !dualSabers) { // not using a second saber, set it to not blocking
                ...
                if (owner->client->saber[saberNum].bladeStyle2Start > 0) {
                    for (bladeNum = owner->client->saber[saberNum].bladeStyle2Start; bladeNum < owner->client->saber[saberNum].numBlades; bladeNum++) {
        ...
        for (saberNum = 0; saberNum < (dualSabers ? MAX_SABERS : 1); saberNum++) {
            ...
            for (bladeNum = 0; bladeNum < owner->client->saber[saberNum].numBlades; bladeNum++) {
                ...
                */
                ...
                VectorMA(&owner->client->saber[saberNum].blade[bladeNum].muzzlePoint, owner->client->saber[saberNum].blade[bladeNum].lengthMax,
                         &owner->client->saber[saberNum].blade[bladeNum].muzzleDir, &saberTip);

                if (saberOrg.raw[i] < saberent->r.mins.raw[i])
                    saberent->r.mins.raw[i] = saberOrg.raw[i];
                if (saberTip.raw[i] < saberent->r.mins.raw[i])
                    saberent->r.mins.raw[i] = saberTip.raw[i];
                if (saberOrg.raw[i] > saberent->r.maxs.raw[i])
                    saberent->r.maxs.raw[i] = saberOrg.raw[i];
                if (saberTip.raw[i] > saberent->r.maxs.raw[i])
                    saberent->r.maxs.raw[i] = saberTip.raw[i];
...
void WP_SaberInitBladeData(gentity_t *ent) {
    ...
    while (i < level.num_entities) { // make sure there are no other saber entities floating around that think they belong to this client.
        ...
        if (checkEnt->inuse && checkEnt->neverFree && checkEnt->r.ownerNum == ent->s.number && checkEnt->classname && checkEnt->classname[0] &&
...
static qboolean G_CheckLookTarget(gentity_t *ent, vector3 *lookAngles, float *lookingSpeed) {
    ...
    if (ent->s.eType == ET_NPC && ent->s.m_iVehicleNum && ent->s.NPC_class != CLASS_VEHICLE) { // an NPC bolted to a vehicle should just look around randomly
    ...
    if (ent->client->renderInfo.lookTarget >= 0 && ent->client->renderInfo.lookTarget < ENTITYNUM_WORLD) {
        ...
        } else if (ent->client->renderInfo.lookMode == LM_INTEREST && ent->client->renderInfo.lookTarget > -1 &&
                   ent->client->renderInfo.lookTarget < MAX_INTEREST_POINTS) {
...
static void G_G2NPCAngles(gentity_t *ent, vector3 legs[3], vector3 *angles) {
    ...
    if (ent->client) {
        if ((ent->client->NPC_class == CLASS_PROBE) || (ent->client->NPC_class == CLASS_R2D2) || (ent->client->NPC_class == CLASS_R5D2) ||
            (ent->client->NPC_class == CLASS_ATST)) {
            vector3 trailingLegsAngles;

            if (ent->s.eType == ET_NPC && ent->s.m_iVehicleNum && ent->s.NPC_class != CLASS_VEHICLE) { // an NPC bolted to a vehicle should use the full angles
...
static void G_G2PlayerAngles(gentity_t *ent, vector3 legs[3], vector3 *legsAngles) {
    qboolean tPitching = qfalse, tYawing = qfalse, lYawing = qfalse;
    float tYawAngle = ent->client->ps.viewangles.yaw, tPitchAngle = 0, lYawAngle = ent->client->ps.viewangles.yaw;
    ...
    if (ent->s.eType == ET_NPC && ent->client) { // sort of hacky, but it saves a pretty big load off the server
        ...
        while (i < MAX_CLIENTS) {
            ...
            if (clEnt && clEnt->inuse && clEnt->client && trap->InPVS(&clEnt->client->ps.origin, &ent->client->ps.origin)) { // this client can see him
                break;
            }
    ...
    if (ent->localAnimIndex <= 1) { // don't do these things on non-humanoids
        ...
        BG_G2PlayerAngles(ent->ghoul2, ent->client->renderInfo.motionBolt, &ent->s, level.time, &lerpOrg, &lerpAng, legs, legsAngles, &tYawing, &tPitching,
                          &lYawing, &tYawAngle, &tPitchAngle, &lYawAngle, FRAMETIME, &turAngles, &ent->modelScale, ciLegs, ciTorso, &ent->client->corrTime,
                          &lookAngles, &ent->client->lastHeadAngles, ent->client->lookTime, emplaced, NULL);
        ...
        if (ent->client->ps.heldByClient && ent->client->ps.heldByClient <= MAX_CLIENTS) { // then put our arm in this client's hand
            ...
            if (lHandBolt)
                BG_IK_MoveArm(ent->ghoul2, lHandBolt, level.time, &ent->s, ent->client->ps.torsoAnim /*BOTH_DEAD1*/, &vec3_origin, &ent->client->ikStatus,
        ...
        if (ent->s.eType == ET_NPC && ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle &&
...
enum sabersLockMode_e { LOCK_FIRST = 0, LOCK_TOP = LOCK_FIRST, LOCK_DIAG_TR, LOCK_DIAG_TL, LOCK_DIAG_BR, LOCK_DIAG_BL, LOCK_R, LOCK_L, LOCK_RANDOM };
...
int G_SaberLockAnim(int attackerSaberStyle, int defenderSaberStyle, int topOrSide, int lockOrBreakOrSuperBreak, int winOrLose) {
    ...
    if (lockOrBreakOrSuperBreak == SABERLOCK_LOCK) { // special case: if we're using the same style and locking
        if (attackerSaberStyle == defenderSaberStyle || (attackerSaberStyle >= SS_FAST && attackerSaberStyle <= SS_TAVION && defenderSaberStyle >= SS_FAST &&
                                                         defenderSaberStyle <= SS_TAVION)) { // using same style
...
static qboolean WP_SabersCheckLock2(gentity_t *attacker, gentity_t *defender, sabersLockMode_e lockMode) {
    ...
    if (attacker->client->ps.fd.saberAnimLevel >= SS_FAST && attacker->client->ps.fd.saberAnimLevel <= SS_TAVION &&
        ...
        if (lockMode == LOCK_TOP) {      // top lock
            attAnim =
                G_SaberLockAnim(attacker->client->ps.fd.saberAnimLevel, defender->client->ps.fd.saberAnimLevel, SABERLOCK_TOP, SABERLOCK_LOCK, SABERLOCK_WIN);
            defAnim =
                G_SaberLockAnim(defender->client->ps.fd.saberAnimLevel, attacker->client->ps.fd.saberAnimLevel, SABERLOCK_TOP, SABERLOCK_LOCK, SABERLOCK_LOSE);
            attStart = defStart = 0.5f;
        } else { // side lock
            switch (lockMode) {
            case LOCK_DIAG_TR:
                attAnim = G_SaberLockAnim(attacker->client->ps.fd.saberAnimLevel, defender->client->ps.fd.saberAnimLevel, SABERLOCK_SIDE, SABERLOCK_LOCK,
                                          SABERLOCK_WIN);
                defAnim = G_SaberLockAnim(defender->client->ps.fd.saberAnimLevel, attacker->client->ps.fd.saberAnimLevel, SABERLOCK_SIDE, SABERLOCK_LOCK,
                                          SABERLOCK_LOSE);
                attStart = defStart = 0.5f;
                break;
            case LOCK_DIAG_TL:
    ...
    attacker->client->ps.saberLockFrame =
        bgAllAnims[attacker->localAnimIndex].anims[attAnim].firstFrame + (bgAllAnims[attacker->localAnimIndex].anims[attAnim].numFrames * attStart);

    G_SetAnim(defender, NULL, SETANIM_BOTH, defAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
    defender->client->ps.saberLockFrame =
        bgAllAnims[defender->localAnimIndex].anims[defAnim].firstFrame + (bgAllAnims[defender->localAnimIndex].anims[defAnim].numFrames * defStart);
...
qboolean WP_SabersCheckLock(gentity_t *ent1, gentity_t *ent2) {
    ...
    if (ent1->s.eType == ET_NPC || ent2->s.eType == ET_NPC) { // if either ents is NPC, then never let an NPC lock with someone on the same playerTeam
        if (ent1->client->playerTeam == ent2->client->playerTeam) {
            return qfalse;
        }
    }

    if (!ent1->client->ps.saberEntityNum || !ent2->client->ps.saberEntityNum || ent1->client->ps.saberInFlight ||
    ...
    if (ent1->s.eType != ET_NPC && ent2->s.eType != ET_NPC) { // can always get into locks with NPCs
        if (!ent1->client->ps.duelInProgress || !ent2->client->ps.duelInProgress || ent1->client->ps.duelIndex != ent2->s.number ||
    ...
    if (ent1->client->ps.groundEntityNum == ENTITYNUM_NONE || ent2->client->ps.groundEntityNum == ENTITYNUM_NONE) {
        return qfalse;
    }
    ...
    if (ent1->client->ps.forceHandExtend != HANDEXTEND_NONE || ent2->client->ps.forceHandExtend != HANDEXTEND_NONE) {
        return qfalse;
    }

    if ((ent1->client->ps.pm_flags & PMF_DUCKED) || (ent2->client->ps.pm_flags & PMF_DUCKED)) {
        return qfalse;
    }

    if ((ent1->client->saber[0].saberFlags & SFL_NOT_LOCKABLE) || (ent2->client->saber[0].saberFlags & SFL_NOT_LOCKABLE)) {
        return qfalse;
    }
    if (ent1->client->saber[1].model[0] && !ent1->client->ps.saberHolstered && (ent1->client->saber[1].saberFlags & SFL_NOT_LOCKABLE)) {
        return qfalse;
    }
    if (ent2->client->saber[1].model[0] && !ent2->client->ps.saberHolstered && (ent2->client->saber[1].saberFlags & SFL_NOT_LOCKABLE)) {
        return qfalse;
    }
    ...
    // T to B lock
    if (ent1->client->ps.torsoAnim == BOTH_A1_T__B_ || ent1->client->ps.torsoAnim == BOTH_A2_T__B_ || ent1->client->ps.torsoAnim == BOTH_A3_T__B_ ||
        ent1->client->ps.torsoAnim == BOTH_A4_T__B_ || ent1->client->ps.torsoAnim == BOTH_A5_T__B_ || ent1->client->ps.torsoAnim == BOTH_A6_T__B_ ||
        ent1->client->ps.torsoAnim == BOTH_A7_T__B_) { // ent1 is attacking top-down
        return WP_SabersCheckLock2(ent1, ent2, LOCK_TOP);
    }

    if (ent2->client->ps.torsoAnim == BOTH_A1_T__B_ || ent2->client->ps.torsoAnim == BOTH_A2_T__B_ || ent2->client->ps.torsoAnim == BOTH_A3_T__B_ ||
        ent2->client->ps.torsoAnim == BOTH_A4_T__B_ || ent2->client->ps.torsoAnim == BOTH_A5_T__B_ || ent2->client->ps.torsoAnim == BOTH_A6_T__B_ ||
        ent2->client->ps.torsoAnim == BOTH_A7_T__B_) { // ent2 is attacking top-down
        return WP_SabersCheckLock2(ent2, ent1, LOCK_TOP);
    }

    if (ent1->s.number >= 0 && ent1->s.number < MAX_CLIENTS && ent1->client->ps.saberBlocking == BLK_WIDE && ent1->client->ps.weaponTime <= 0) {
        ent1BlockingPlayer = qtrue;
    }
    if (ent2->s.number >= 0 && ent2->s.number < MAX_CLIENTS && ent2->client->ps.saberBlocking == BLK_WIDE && ent2->client->ps.weaponTime <= 0) {
        ent2BlockingPlayer = qtrue;
    }

    // TR to BL lock
    if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL || ent1->client->ps.torsoAnim == BOTH_A2_TR_BL || ent1->client->ps.torsoAnim == BOTH_A3_TR_BL ||
        ent1->client->ps.torsoAnim == BOTH_A4_TR_BL || ent1->client->ps.torsoAnim == BOTH_A5_TR_BL || ent1->client->ps.torsoAnim == BOTH_A6_TR_BL ||
        ...
        if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL || ent2->client->ps.torsoAnim == BOTH_A2_TR_BL || ent2->client->ps.torsoAnim == BOTH_A3_TR_BL ||
            ent2->client->ps.torsoAnim == BOTH_A4_TR_BL || ent2->client->ps.torsoAnim == BOTH_A5_TR_BL || ent2->client->ps.torsoAnim == BOTH_A6_TR_BL ||
            ent2->client->ps.torsoAnim == BOTH_A7_TR_BL || ent2->client->ps.torsoAnim == BOTH_P1_S1_TL) { // ent2 is attacking in the opposite diagonal
            return WP_SabersCheckLock2(ent1, ent2, LOCK_DIAG_TR);
        }
        if (ent2->client->ps.torsoAnim == BOTH_A1_BR_TL || ent2->client->ps.torsoAnim == BOTH_A2_BR_TL || ent2->client->ps.torsoAnim == BOTH_A3_BR_TL ||
            ent2->client->ps.torsoAnim == BOTH_A4_BR_TL || ent2->client->ps.torsoAnim == BOTH_A5_BR_TL || ent2->client->ps.torsoAnim == BOTH_A6_BR_TL ||
            ent2->client->ps.torsoAnim == BOTH_A7_BR_TL || ent2->client->ps.torsoAnim == BOTH_P1_S1_BL) { // ent2 is attacking in the opposite diagonal
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL || ent2->client->ps.torsoAnim == BOTH_A2_TR_BL || ent2->client->ps.torsoAnim == BOTH_A3_TR_BL ||
        ...
        if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL || ent1->client->ps.torsoAnim == BOTH_A2_TR_BL || ent1->client->ps.torsoAnim == BOTH_A3_TR_BL ||
            ent1->client->ps.torsoAnim == BOTH_A4_TR_BL || ent1->client->ps.torsoAnim == BOTH_A5_TR_BL || ent1->client->ps.torsoAnim == BOTH_A6_TR_BL ||
            ent1->client->ps.torsoAnim == BOTH_A7_TR_BL || ent1->client->ps.torsoAnim == BOTH_P1_S1_TL) { // ent1 is attacking in the opposite diagonal
            return WP_SabersCheckLock2(ent2, ent1, LOCK_DIAG_TR);
        }
        if (ent1->client->ps.torsoAnim == BOTH_A1_BR_TL || ent1->client->ps.torsoAnim == BOTH_A2_BR_TL || ent1->client->ps.torsoAnim == BOTH_A3_BR_TL ||
            ent1->client->ps.torsoAnim == BOTH_A4_BR_TL || ent1->client->ps.torsoAnim == BOTH_A5_BR_TL || ent1->client->ps.torsoAnim == BOTH_A6_BR_TL ||
            ent1->client->ps.torsoAnim == BOTH_A7_BR_TL || ent1->client->ps.torsoAnim == BOTH_P1_S1_BL) { // ent1 is attacking in the opposite diagonal
    ...
    // TL to BR lock
    if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR || ent1->client->ps.torsoAnim == BOTH_A2_TL_BR || ent1->client->ps.torsoAnim == BOTH_A3_TL_BR ||
        ent1->client->ps.torsoAnim == BOTH_A4_TL_BR || ent1->client->ps.torsoAnim == BOTH_A5_TL_BR || ent1->client->ps.torsoAnim == BOTH_A6_TL_BR ||
        ent1->client->ps.torsoAnim == BOTH_A7_TL_BR) { // ent1 is attacking diagonally
        if (ent2BlockingPlayer) {                      // player will block this anyway
            return WP_SabersCheckLock2(ent1, ent2, LOCK_DIAG_TL);
        }
        if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR || ent2->client->ps.torsoAnim == BOTH_A2_TL_BR || ent2->client->ps.torsoAnim == BOTH_A3_TL_BR ||
            ent2->client->ps.torsoAnim == BOTH_A4_TL_BR || ent2->client->ps.torsoAnim == BOTH_A5_TL_BR || ent2->client->ps.torsoAnim == BOTH_A6_TL_BR ||
            ent2->client->ps.torsoAnim == BOTH_A7_TL_BR || ent2->client->ps.torsoAnim == BOTH_P1_S1_TR) { // ent2 is attacking in the opposite diagonal
            return WP_SabersCheckLock2(ent1, ent2, LOCK_DIAG_TL);
        }
        if (ent2->client->ps.torsoAnim == BOTH_A1_BL_TR || ent2->client->ps.torsoAnim == BOTH_A2_BL_TR || ent2->client->ps.torsoAnim == BOTH_A3_BL_TR ||
            ent2->client->ps.torsoAnim == BOTH_A4_BL_TR || ent2->client->ps.torsoAnim == BOTH_A5_BL_TR || ent2->client->ps.torsoAnim == BOTH_A6_BL_TR ||
            ent2->client->ps.torsoAnim == BOTH_A7_BL_TR || ent2->client->ps.torsoAnim == BOTH_P1_S1_BR) { // ent2 is attacking in the opposite diagonal
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR || ent2->client->ps.torsoAnim == BOTH_A2_TL_BR || ent2->client->ps.torsoAnim == BOTH_A3_TL_BR ||
        ...
        if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR || ent1->client->ps.torsoAnim == BOTH_A2_TL_BR || ent1->client->ps.torsoAnim == BOTH_A3_TL_BR ||
            ent1->client->ps.torsoAnim == BOTH_A4_TL_BR || ent1->client->ps.torsoAnim == BOTH_A5_TL_BR || ent1->client->ps.torsoAnim == BOTH_A6_TL_BR ||
            ent1->client->ps.torsoAnim == BOTH_A7_TL_BR || ent1->client->ps.torsoAnim == BOTH_P1_S1_TR) { // ent1 is attacking in the opposite diagonal
            return WP_SabersCheckLock2(ent2, ent1, LOCK_DIAG_TL);
        }
        if (ent1->client->ps.torsoAnim == BOTH_A1_BL_TR || ent1->client->ps.torsoAnim == BOTH_A2_BL_TR || ent1->client->ps.torsoAnim == BOTH_A3_BL_TR ||
            ent1->client->ps.torsoAnim == BOTH_A4_BL_TR || ent1->client->ps.torsoAnim == BOTH_A5_BL_TR || ent1->client->ps.torsoAnim == BOTH_A6_BL_TR ||
            ent1->client->ps.torsoAnim == BOTH_A7_BL_TR || ent1->client->ps.torsoAnim == BOTH_P1_S1_BR) { // ent1 is attacking in the opposite diagonal
            return WP_SabersCheckLock2(ent2, ent1, LOCK_DIAG_BR);
        }
        return qfalse;
    }
    // L to R lock
    if (ent1->client->ps.torsoAnim == BOTH_A1__L__R || ent1->client->ps.torsoAnim == BOTH_A2__L__R || ent1->client->ps.torsoAnim == BOTH_A3__L__R ||
        ent1->client->ps.torsoAnim == BOTH_A4__L__R || ent1->client->ps.torsoAnim == BOTH_A5__L__R || ent1->client->ps.torsoAnim == BOTH_A6__L__R ||
        ...
        if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR || ent2->client->ps.torsoAnim == BOTH_A2_TL_BR || ent2->client->ps.torsoAnim == BOTH_A3_TL_BR ||
            ent2->client->ps.torsoAnim == BOTH_A4_TL_BR || ent2->client->ps.torsoAnim == BOTH_A5_TL_BR || ent2->client->ps.torsoAnim == BOTH_A6_TL_BR ||
            ent2->client->ps.torsoAnim == BOTH_A7_TL_BR || ent2->client->ps.torsoAnim == BOTH_P1_S1_TR ||
            ent2->client->ps.torsoAnim == BOTH_P1_S1_BL) { // ent2 is attacking or blocking on the r
            return WP_SabersCheckLock2(ent1, ent2, LOCK_L);
        }
        return qfalse;
    }
    if (ent2->client->ps.torsoAnim == BOTH_A1__L__R || ent2->client->ps.torsoAnim == BOTH_A2__L__R || ent2->client->ps.torsoAnim == BOTH_A3__L__R ||
        ent2->client->ps.torsoAnim == BOTH_A4__L__R || ent2->client->ps.torsoAnim == BOTH_A5__L__R || ent2->client->ps.torsoAnim == BOTH_A6__L__R ||
        ...
        if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR || ent1->client->ps.torsoAnim == BOTH_A2_TL_BR || ent1->client->ps.torsoAnim == BOTH_A3_TL_BR ||
            ent1->client->ps.torsoAnim == BOTH_A4_TL_BR || ent1->client->ps.torsoAnim == BOTH_A5_TL_BR || ent1->client->ps.torsoAnim == BOTH_A6_TL_BR ||
            ent1->client->ps.torsoAnim == BOTH_A7_TL_BR || ent1->client->ps.torsoAnim == BOTH_P1_S1_TR ||
    ...
    // R to L lock
    if (ent1->client->ps.torsoAnim == BOTH_A1__R__L || ent1->client->ps.torsoAnim == BOTH_A2__R__L || ent1->client->ps.torsoAnim == BOTH_A3__R__L ||
        ent1->client->ps.torsoAnim == BOTH_A4__R__L || ent1->client->ps.torsoAnim == BOTH_A5__R__L || ent1->client->ps.torsoAnim == BOTH_A6__R__L ||
        ...
        if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL || ent2->client->ps.torsoAnim == BOTH_A2_TR_BL || ent2->client->ps.torsoAnim == BOTH_A3_TR_BL ||
            ent2->client->ps.torsoAnim == BOTH_A4_TR_BL || ent2->client->ps.torsoAnim == BOTH_A5_TR_BL || ent2->client->ps.torsoAnim == BOTH_A6_TR_BL ||
            ent2->client->ps.torsoAnim == BOTH_A7_TR_BL || ent2->client->ps.torsoAnim == BOTH_P1_S1_TL ||
            ent2->client->ps.torsoAnim == BOTH_P1_S1_BR) { // ent2 is attacking or blocking on the l
            return WP_SabersCheckLock2(ent1, ent2, LOCK_R);
        }
        return qfalse;
    }
    if (ent2->client->ps.torsoAnim == BOTH_A1__R__L || ent2->client->ps.torsoAnim == BOTH_A2__R__L || ent2->client->ps.torsoAnim == BOTH_A3__R__L ||
        ent2->client->ps.torsoAnim == BOTH_A4__R__L || ent2->client->ps.torsoAnim == BOTH_A5__R__L || ent2->client->ps.torsoAnim == BOTH_A6__R__L ||
        ...
        if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL || ent1->client->ps.torsoAnim == BOTH_A2_TR_BL || ent1->client->ps.torsoAnim == BOTH_A3_TR_BL ||
            ent1->client->ps.torsoAnim == BOTH_A4_TR_BL || ent1->client->ps.torsoAnim == BOTH_A5_TR_BL || ent1->client->ps.torsoAnim == BOTH_A6_TR_BL ||
            ent1->client->ps.torsoAnim == BOTH_A7_TR_BL || ent1->client->ps.torsoAnim == BOTH_P1_S1_TL ||
...
static qboolean WP_GetSaberDeflectionAngle(gentity_t *attacker, gentity_t *defender, float saberHitFraction) {
    ...
    if ((level.time - attacker->client->lastSaberStorageTime) >
        500) { // last update was too long ago, something is happening to this client to prevent his saber from updating
        return qfalse;
    }
...
static int G_GetAttackDamage(gentity_t *self, int minDmg, int maxDmg, float multPoint) {
    ...
    float peakPoint = 0;
    float attackAnimLength = bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].numFrames *
                             fabsf((float)(bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].frameLerp));
...
static qboolean G_ClientIdleInWorld(gentity_t *ent) {
    ...
    if (!ent->client->pers.cmd.upmove && !ent->client->pers.cmd.forwardmove && !ent->client->pers.cmd.rightmove &&
        !(ent->client->pers.cmd.buttons & BUTTON_GESTURE) && !(ent->client->pers.cmd.buttons & BUTTON_FORCEGRIP) &&
        !(ent->client->pers.cmd.buttons & BUTTON_ALT_ATTACK) && !(ent->client->pers.cmd.buttons & BUTTON_FORCEPOWER) &&
        !(ent->client->pers.cmd.buttons & BUTTON_FORCE_LIGHTNING) && !(ent->client->pers.cmd.buttons & BUTTON_FORCE_DRAIN) &&
...
static qboolean G_G2TraceCollide(trace_t *tr, vector3 *lastValidStart, vector3 *lastValidEnd, vector3 *traceMins,
                                 vector3 *traceMaxs) { // Hit the ent with the normal trace, try the collision trace.
    G2Trace_t G2Trace;
    gentity_t *g2Hit;
    vector3 angles;
    int tN = 0;
    ...
    if (traceMins->x || traceMins->y || traceMins->z || traceMaxs->x || traceMaxs->y || traceMaxs->z) {
    ...
    if (g2Hit && g2Hit->inuse && g2Hit->ghoul2) {
        ...
        if (com_optvehtrace.integer && g2Hit->s.eType == ET_NPC && g2Hit->s.NPC_class == CLASS_VEHICLE && g2Hit->m_pVehicle) {
            trap->G2API_CollisionDetectCache(G2Trace, g2Hit->ghoul2, &angles, &g2HitOrigin, level.time, g2Hit->s.number, lastValidStart, lastValidEnd,
                                             &g2Hit->modelScale, 0, g_g2TraceLod.integer, fRadius);
        } else {
            trap->G2API_CollisionDetect(G2Trace, g2Hit->ghoul2, &angles, &g2HitOrigin, level.time, g2Hit->s.number, lastValidStart, lastValidEnd,
                                        &g2Hit->modelScale, 0, g_g2TraceLod.integer, fRadius);
        }
...
static void G_BuildSaberFaces(vector3 *base, vector3 *tip, float radius, vector3 *fwd, vector3 *right, int *fNum, saberFace_t **fList) {
...
static void G_SabCol_CalcPlaneEq(vector3 *x, vector3 *y, vector3 *z, float *planeEq) {
    ...
    planeEq[3] = -(x->x * (y->y * z->z - z->y * y->z) + y->x * (z->y * x->z - x->y * z->z) + z->x * (x->y * y->z - y->y * x->z));
}
...
static qboolean G_SaberFaceCollisionCheck(int fNum, saberFace_t *fList, vector3 *atkStart, vector3 *atkEnd, vector3 *atkMins, vector3 *atkMaxs,
                                          vector3 *impactPoint) {
...
static qboolean G_SaberCollide(gentity_t *atk, gentity_t *def, vector3 *atkStart, vector3 *atkEnd, vector3 *atkMins, vector3 *atkMaxs, vector3 *impactPoint) {
    static int i, j;
    ...
    if (atk->client->pers.adminData.isSlept || atk->client->pers.adminData.isGhost || def->client->pers.adminData.isSlept ||
...
FIXME: test the intersection to see if the sabers really did intersect (weren't going in the same direction and/or passed through same point at different
times)?
*/
...
qboolean WP_SabersIntersect(gentity_t *ent1, int ent1SaberNum, int ent1BladeNum, gentity_t *ent2, qboolean checkDir) {
    ...
    if (BG_SabersOff(&ent1->client->ps) || BG_SabersOff(&ent2->client->ps)) {
        return qfalse;
    }
            ...
            for (ent2BladeNum = 0; ent2BladeNum < ent2->client->saber[ent2SaberNum].numBlades; ent2BladeNum++) {
                if (ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax > 0) { // valid saber and this blade is on
                    // if ( ent1->client->saberInFlight )
                    {
                        ...
                        VectorSubtract(&ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzlePoint,
                                       &ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzlePointOld, &dir);
                        VectorNormalize(&dir);
                        VectorMA(&saberBaseNext1, SABER_EXTRAPOLATE_DIST, &dir, &saberBaseNext1);

                        VectorMA(&saberBase1, ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax + SABER_EXTRAPOLATE_DIST,
                                 &ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDirOld, &saberTip1);
                        VectorMA(&saberBaseNext1, ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax + SABER_EXTRAPOLATE_DIST,
                                 &ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir, &saberTipNext1);
                    ...
                    VectorMA( saberBase1, ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax,
                    ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir, saberTip1 ); VectorMA( saberBaseNext1,
                    ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax, ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDirNext,
                    saberTipNext1 );
                    }
                    */
                    ...
                    // if ( ent2->client->saberInFlight )
                    {
                        ...
                        VectorSubtract(&ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzlePoint,
                                       &ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzlePointOld, &dir);
                        VectorNormalize(&dir);
                        VectorMA(&saberBaseNext2, SABER_EXTRAPOLATE_DIST, &dir, &saberBaseNext2);

                        VectorMA(&saberBase2, ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax + SABER_EXTRAPOLATE_DIST,
                                 &ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDirOld, &saberTip2);
                        VectorMA(&saberBaseNext2, ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax + SABER_EXTRAPOLATE_DIST,
                                 &ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDir, &saberTipNext2);
                    ...
                    VectorMA( saberBase2, ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax,
                    ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDir, saberTip2 ); VectorMA( saberBaseNext2,
                    ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax, ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDirNext,
                    saberTipNext2 );
                    }
                    */
                    if (checkDir) { // check the direction of the two swings to make sure the sabers are swinging towards each other
                        ...
                        dot = DotProduct(&ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir,
                                         &ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDir);
...
static int G_PowerLevelForSaberAnim(gentity_t *ent, int saberNum, qboolean mySaberHit) {
    if (!ent || !ent->client || saberNum >= MAX_SABERS) {
        ...
        case BOTH_JUMPATTACK6:
            ...
            */
            if ((animTimer >= 1450 && animTimeElapsed >= 400) || (animTimer >= 400 && animTimeElapsed >= 1100)) { // pretty much sideways
                return FORCE_LEVEL_3;
            }
            return FORCE_LEVEL_0;
...
void WP_SaberDamageAdd(int trVictimEntityNum, vector3 *trDmgDir, vector3 *trDmgSpot, int trDmg, qboolean doDismemberment, uint32_t knockBackFlags) {
...
void WP_SaberApplyDamage(gentity_t *self) {
    ...
    for (i = 0; i < numVictims; i++) {
        ...
        if (self->client->ps.duelInProgress && self->client->ps.duelIndex == victim->s.number && level.time > self->lastHit + 300) {
            self->duelHitCount++;
...
void WP_SaberDoHit(gentity_t *self, int saberNum, int bladeNum) {
    ...
    for (i = 0; i < numVictims; i++) {
        ...
        if (victim->client) {
            class_e npc_class = victim->client->NPC_class;

            if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE || npc_class == CLASS_MOUSE || npc_class == CLASS_REMOTE || npc_class == CLASS_GONK ||
                npc_class == CLASS_R2D2 || npc_class == CLASS_R5D2 || npc_class == CLASS_PROTOCOL || npc_class == CLASS_MARK1 || npc_class == CLASS_MARK2 ||
        ...
        if (te) {
            ...
            if (!isDroid && (victim->client || victim->s.eType == ET_NPC || victim->s.eType == ET_BODY)) {
                if (totalDmg[i] < 5)
                    te->s.eventParm = 3;
                ...
                if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[saberNum], bladeNum) &&
                    ...
                    if (totalDmg[i] >
                        SABER_NONATTACK_DAMAGE) { // I suppose I could tie this into the saberblock event, but I'm tired of adding flags to that thing.
        ...
        for (i = 0; i < numEnts; i++) {
            ...
            if (dist <= radius) { // in range
                ...
                if (knockBack > 0) { // do knockback
                    if (radiusEnt->client && radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_ATST &&
                        !(radiusEnt->flags & FL_NO_KNOCKBACK)) // don't throw them back
                    {
                        ...
                        if (radiusEnt->health > 0) { // still alive
                            if (knockbackStr > 50) { // close enough and knockback high enough to possibly knock down
                                if (dist < (radius * 0.5f) || radiusEnt->client->ps.groundEntityNum !=
                                                                  ENTITYNUM_NONE) { // within range of my fist or within ground-shaking range and not in the air
...
static qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vector3 *saberStart, vector3 *saberEnd, qboolean doInterpolate, int trMask,
    ...
    if (self->client->ps.saberAttackWound < level.time &&
        (SaberAttacking(self) || BG_SuperBreakWinAnim(self->client->ps.torsoAnim) ||
         ...
         (self->client->ps.m_iVehicleNum &&
          self->client->ps.saberMove > LS_READY))) { // this animation is that of the last attack movement, and so it should do full damage
    ...
    if (!dmg) {
        if (tr.entityNum < MAX_CLIENTS || (g_entities[tr.entityNum].inuse && (g_entities[tr.entityNum].r.contents & CONTENTS_LIGHTSABER))) {
            return qtrue;
        }
        return qfalse;
    }
    ...
    if (dmg > SABER_NONATTACK_DAMAGE) {
        ...
        if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum], rBladeNum) && self->client->saber[rSaberNum].damageScale != 1.0f) {
            dmg = ceilf((float)dmg * self->client->saber[rSaberNum].damageScale);
        } else if (WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum], rBladeNum) && self->client->saber[rSaberNum].damageScale2 != 1.0f) {
            dmg = ceilf((float)dmg * self->client->saber[rSaberNum].damageScale2);
        }

        if ((self->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)) || (self->client->ps.brokenLimbs & (1 << BROKENLIMB_LARM))) {
         ...
         || self->client->ps.saberMove == LS_A_JUMP_T__B_)) // or in the strong jump-fwd-attack "death from above" move
    {
        ...
        if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum], rBladeNum)) {
            WP_SaberRadiusDamage(self, &tr.endpos, self->client->saber[rSaberNum].splashRadius, self->client->saber[rSaberNum].splashDamage,
                                 self->client->saber[rSaberNum].splashKnockback);
        } else {
            WP_SaberRadiusDamage(self, &tr.endpos, self->client->saber[rSaberNum].splashRadius2, self->client->saber[rSaberNum].splashDamage2,
                                 self->client->saber[rSaberNum].splashKnockback2);
        }
    ...
    if ((tr.fraction != 1.0f || tr.startsolid) && g_entities[tr.entityNum].takedamage &&
        (g_entities[tr.entityNum].health > 0 || !(g_entities[tr.entityNum].s.eFlags & EF_DISINTEGRATION)) && tr.entityNum != self->s.number &&
        g_entities[tr.entityNum].inuse) {
        // hit something that had health and takes damage
        if (idleDamage && g_entities[tr.entityNum].client && OnSameTeam(self, &g_entities[tr.entityNum]) && !g_friendlySaber.integer) {
            return qfalse;
        }

        if (g_entities[tr.entityNum].client && g_entities[tr.entityNum].client->ps.duelInProgress &&
        ...
        if (g_entities[tr.entityNum].client &&
            (g_entities[tr.entityNum].client->pers.adminData.isSlept || g_entities[tr.entityNum].client->pers.adminData.isGhost ||
             self->client->pers.adminData.isSlept || self->client->pers.adminData.isGhost)) {
            return qfalse;
        }
        ...
        if (BG_StabDownAnim(self->client->ps.torsoAnim) && g_entities[tr.entityNum].client &&
        ...
        if (!d_saberSPStyleDamage.integer && g_entities[tr.entityNum].client && !unblockable &&
            ...
            if (!finalDamage) {
                ...
                if (!d_saberSPStyleDamage.integer && !unblockable && g_entities[tr.entityNum].client &&
                    g_entities[tr.entityNum].client->ps.weapon == WP_SABER && level.gametype != GT_SIEGE && dmg > SABER_NONATTACK_DAMAGE) {
                    dmg *= 0.5f;
                }
            }

            if (self->s.eType == ET_NPC && g_entities[tr.entityNum].client && self->client->playerTeam == g_entities[tr.entityNum].client->playerTeam) {
            ...
            if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum], rBladeNum) && self->client->saber[rSaberNum].knockbackScale > 0.0f) {
                if (rSaberNum < 1) {
    ...
    } else if ((tr.fraction != 1 || tr.startsolid) && (g_entities[tr.entityNum].r.contents & CONTENTS_LIGHTSABER) &&
               g_entities[tr.entityNum].r.contents != -1 && g_entities[tr.entityNum].inuse) { // saber clash
        ...
        if (otherOwner && otherOwner->client && otherOwner->client->ps.saberInFlight) { // don't do extra collision checking vs sabers in air
            ...
            if (d_saberSPStyleDamage.integer || japp_saberTweaks.integer & SABERTWEAK_REDUCEBLOCKS) { // use SP-style blade-collision test
                ...
                if (!G_SaberCollide(self, otherOwner, &lastValidStart, &lastValidEnd, &saberTrMins, &saberTrMaxs,
                                    &tr.endpos)) { // detailed collision did not produce results...
                    return qfalse;
                }
            }
        }

        if (OnSameTeam(self, otherOwner) && !g_friendlySaber.integer) {
            return qfalse;
        }
        ...
        if (otherOwner->client->ps.duelInProgress && otherOwner->client->ps.duelIndex != self->s.number) {
            return qfalse;
        }

        if (self->client->ps.duelInProgress && self->client->ps.duelIndex != otherOwner->s.number) {
            return qfalse;
        }
        ...
        if (self->client->ps.saberEntityNum && self->client->ps.saberInFlight && rSaberNum == 0 &&
    ...
    blockStuff:
        ...
        if (sabersClashed && dmg > SABER_NONATTACK_DAMAGE && selfSaberLevel < FORCE_LEVEL_3 && !PM_SaberInBounce(otherOwner->client->ps.saberMove) &&
            !PM_SaberInParry(self->client->ps.saberMove) && !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
            !BG_SaberInSpecial(self->client->ps.saberMove) && !PM_SaberInBounce(self->client->ps.saberMove) && !PM_SaberInDeflect(self->client->ps.saberMove) &&
        ...
        if (((selfSaberLevel < FORCE_LEVEL_3 && ((tryDeflectAgain && Q_irand(1, 10) <= 3) || (!tryDeflectAgain && Q_irand(1, 10) <= 7))) ||
             (Q_irand(1, 10) <= 1 && otherSaberLevel >= FORCE_LEVEL_3)) &&
            !PM_SaberInBounce(self->client->ps.saberMove)

            && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) && !BG_SaberInSpecial(otherOwner->client->ps.saberMove) &&
            !PM_SaberInBounce(otherOwner->client->ps.saberMove) && !PM_SaberInDeflect(otherOwner->client->ps.saberMove) &&
            ...
            !unblockable && !otherUnblockable && dmg > SABER_NONATTACK_DAMAGE &&
            !didOffense) // don't allow the person we're attacking to do this if we're making an unblockable attack
        { // knockaways can make fast-attacker go into a broken parry anim if the ent is using fast or med. In MP, we also randomly decide this for level 3
          // attacks.
            ...
            didDefense = qtrue;
                   ...
                   (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] < selfSaberLevel ||
                    (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == selfSaberLevel &&
                     (Q_irand(1, 10) >= otherSaberLevel * 1.5f || unblockable))) &&
                   PM_SaberInParry(otherOwner->client->ps.saberMove) && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
                   !PM_SaberInParry(self->client->ps.saberMove) && !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
                   !PM_SaberInBounce(self->client->ps.saberMove) && dmg > SABER_NONATTACK_DAMAGE && !didOffense &&
                   !otherUnblockable) { // they are in a parry, and we are slamming down on them with a move of equal or greater force than their defense, so
                                        // send them into a broken parry.. unless they are already in one.
            ...
            didDefense = qtrue;
                   ...
                   !PM_SaberInDeflect(self->client->ps.saberMove) && !PM_SaberInReflect(self->client->ps.saberMove) && dmg > SABER_NONATTACK_DAMAGE &&
                   !didOffense && !unblockable) { // they are in a parry, and we are slamming down on them with a move of equal or greater force than their
                                                  // defense, so send them into a broken parry.. unless they are already in one.
            if (g_saberDebugPrint.integer) {
            ...
            didOffense = qtrue;
        } else if (SaberAttacking(otherOwner) && dmg > SABER_NONATTACK_DAMAGE && !BG_SaberInSpecial(otherOwner->client->ps.saberMove) && !didOffense &&
                   !otherUnblockable) { // they were attacking and our saber hit their saber, make them bounce. But if they're in a special attack, leave them.
            if (!PM_SaberInBounce(self->client->ps.saberMove) && !PM_SaberInBounce(otherOwner->client->ps.saberMove) &&
                !PM_SaberInDeflect(self->client->ps.saberMove) && !PM_SaberInDeflect(otherOwner->client->ps.saberMove) &&

                !PM_SaberInReflect(self->client->ps.saberMove) && !PM_SaberInReflect(otherOwner->client->ps.saberMove)) {
                int attackAdv, defendStr = G_PowerLevelForSaberAnim(otherOwner, 0, qtrue), attackBonus = 0;
                if (otherOwner->client->ps.torsoAnim == BOTH_A1_SPECIAL || otherOwner->client->ps.torsoAnim == BOTH_A2_SPECIAL ||
                ...
                attackAdv = (attackStr + attackBonus + self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) -
        ...
        if (d_saberGhoul2Collision.integer && !didDefense && dmg <= SABER_NONATTACK_DAMAGE &&
            !otherUnblockable) // with perpoly, it looks pretty weird to have clash flares coming off the guy's face and whatnot
        {
            if (!PM_SaberInParry(otherOwner->client->ps.saberMove) && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
                !BG_SaberInSpecial(otherOwner->client->ps.saberMove) && !PM_SaberInBounce(otherOwner->client->ps.saberMove) &&
                !PM_SaberInDeflect(otherOwner->client->ps.saberMove) && !PM_SaberInReflect(otherOwner->client->ps.saberMove)) {
                WP_SaberBlockNonRandom(otherOwner, &tr.endpos, qfalse);
        ...
        {                                                                            // block
            if (!PM_SaberInParry(otherOwner->client->ps.saberMove) && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
                ...
                if (!SaberAttacking(otherOwner)) {
                    ...
                    if (attackStr + self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] >
                        otherIdleStr + otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE]) {
                        crushTheParry = qtrue;
                    } else {
                        tryDeflectAgain = qtrue;
                    }
                    ...
                    if (selfSaberLevel == otherSaberLevel) { // equal level, try to bounce off each other's sabers
                        if (!didOffense && !PM_SaberInParry(self->client->ps.saberMove) && !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
                            !BG_SaberInSpecial(self->client->ps.saberMove) && !PM_SaberInBounce(self->client->ps.saberMove) &&
                            !PM_SaberInDeflect(self->client->ps.saberMove) && !PM_SaberInReflect(self->client->ps.saberMove) && !unblockable) {
                            self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
                            didOffense = qtrue;
                        }
                        if (!didDefense && !PM_SaberInParry(otherOwner->client->ps.saberMove) && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
                            !BG_SaberInSpecial(otherOwner->client->ps.saberMove) && !PM_SaberInBounce(otherOwner->client->ps.saberMove) &&
                            !PM_SaberInDeflect(otherOwner->client->ps.saberMove) && !PM_SaberInReflect(otherOwner->client->ps.saberMove) && !unblockable) {
                            otherOwner->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
                        }
                        if (g_saberDebugPrint.integer) {
                ...
                if (crushTheParry && PM_SaberInParry(G_GetParryForBlock(
                                         otherOwner->client->ps.saberBlocked))) { // This means that the attack actually hit our saber, and we went to block it.
                ...
                } else if (PM_SaberInParry(G_GetParryForBlock(otherOwner->client->ps.saberBlocked)) && !didOffense &&
                           tryDeflectAgain) { // We want to try deflecting again because the other is in the parry and we haven't made any new moves
...
int VectorCompare2(const vector3 *v1, const vector3 *v2) {
    if (v1->x > v2->x + 0.0001f || v1->x < v2->x - 0.0001f || v1->y > v2->y + 0.0001f || v1->y < v2->y - 0.0001f || v1->z > v2->z + 0.0001f ||
        v1->z < v2->z - 0.0001f)
        return 0;
...
void G_SPSaberDamageTraceLerped(gentity_t *self, int saberNum, int bladeNum, vector3 *baseNew, vector3 *endNew, int clipmask) {
    ...
    if (VectorCompare2(&baseOld, baseNew) && VectorCompare2(&endOld, endNew)) { // no diff
        ...
        if (BG_SaberInAttack(self->client->ps.saberMove) || BG_SaberInSpecialAttack(self->client->ps.torsoAnim) ||
            BG_SpinningSaberAnim(self->client->ps.torsoAnim) || BG_InSpecialJump(self->client->ps.torsoAnim))
        ...
        while (1) {
            ...
            for (step = stepsize;
                 step <= self->client->saber[saberNum].blade[bladeNum].lengthMax /*&& step < self->client->saber[saberNum].blade[bladeNum].lengthOld*/;
                 step += stepsize) {
...
void WP_SaberStartMissileBlockCheck(gentity_t *self, usercmd_t *ucmd) {
    ...
    if (self->client->ps.weapon != WP_SABER && self->client->NPC_class != CLASS_BOBAFETT) {
        ...
        doFullRoutine = qfalse;
    } else if (self->client->ps.fd.forcePowersActive &
               (1 << FP_GRIP)) { // can't block while gripping (FIXME: or should it break the grip?  Pain should break the grip, I think...)
        doFullRoutine = qfalse;
    }
    ...
    if (self->s.eType == ET_PLAYER) { // don't do this if already attacking!
        if (ucmd->buttons & BUTTON_ATTACK || BG_SaberInAttack(self->client->ps.saberMove) || BG_SaberInSpecialAttack(self->client->ps.torsoAnim) ||
            BG_SaberInTransitionAny(self->client->ps.saberMove)) {
            doFullRoutine = qfalse;
        }
    }

    if (self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] >
    ...
    for (e = 0; e < numListedEntities; e++) {
        ...
        if (ent->s.eType != ET_MISSILE && !(ent->s.eFlags & EF_MISSILE_STICK)) { // not a normal projectile
            ...
            if (!pOwner->client->ps.saberEntityNum || !pOwner->client->ps.saberInFlight ||
                pOwner->client->ps.saberEntityNum !=
                    ent->s.number) { // the saber is knocked away and/or not flying actively, or this ent is not the cl's saber ent at all
                continue;
            }
        ...
        if (ent->s.weapon == WP_THERMAL) { // thermal detonator!
            if (self->NPC && dist < ent->splashRadius) {
                if (dist < ent->splashRadius && ent->nextthink < level.time + 600 && ent->count && self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
                    (ent->s.pos.trType == TR_STATIONARY || ent->s.pos.trType == TR_INTERPOLATE ||
                     (dot1 = DotProduct(&dir, &forward)) < SABER_REFLECT_MISSILE_CONE ||
                     !WP_ForcePowerUsable(self, FP_PUSH))) { // TD is close enough to hurt me, I'm on the ground and the thing is at rest or behind me and about
                                                             // to blow up, or I don't have force-push so force-jump!
            ...
            if (self->s.eType == ET_PLAYER) { // players don't auto-handle these at all
                ...
                if (ent->s.pos.trType == TR_STATIONARY && (ent->s.eFlags & EF_MISSILE_STICK) &&
                    self->client->NPC_class != CLASS_BOBAFETT) { // a placed explosive like a tripmine or detpack
                ...
                } else if (dist < ent->splashRadius && self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
                           (DotProduct(&dir, &forward) < SABER_REFLECT_MISSILE_CONE || !WP_ForcePowerUsable(self, FP_PUSH))) { // NPCs try to evade it
                    self->client->ps.fd.forceJumpCharge = 480;
        ...
        if (dist < closestDist) {
            ...
            // FIXME: if NPC, predict the intersection between my current velocity/path and the missile's, see if it intersects my bounding box
            // (+/-saberLength?), don't try to deflect unless it does?
            closestDist = dist;
            incoming = ent;
        }
    }
    ...
    if (lookT) { // we got a looktarget at some point so we'll assign it then.
        if (!(self->client->ps.eFlags2 &
              EF2_HELD_BY_MONSTER)) { // lookTarget is set by and to the monster that's holding you, no other operations can change that
    ...
    if (incoming) {
        if (self->NPC /*&& !G_ControlledByPlayer( self )*/) {
            ...
            if (self->client->NPC_class == CLASS_BOBAFETT && (self->client->ps.eFlags2 & EF2_FLYING) // moveType == MT_FLYSWIM
            ...
            } else if (Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL, incoming, 0.0f) !=
            ...
            if (tr.fraction == 1 || tr.entityNum == ent->s.number) {
                ...
                if (ent->s.eType == ET_MOVER) {
                    if (saberOwner && saberOwner->client &&
                        (saberOwner->client->saber[0].saberFlags2 &
                         SFL2_NO_CLASH_FLARE)) { // don't do clash flare - NOTE: assumes same is true for both sabers if using dual sabers!
...
static void saberCheckRadiusDamage(gentity_t *saberent,
                                   int returning) { // we're going to cheat and damage players within the saber's radius, just for the sake of doing things more
                                                    // "efficiently" (and because the saber entity has no server g2 instance)
    int i = 0;
    int dist = 0;
    gentity_t *ent;
...
static void saberMoveBack(gentity_t *ent, qboolean goingBack) {
    ...
    if (!goingBack &&
        ent->s.pos.trType != TR_GRAVITY) { // acts as a fallback in case touch code fails, keeps saber from going through things between predictions
        ...
        if ((tr.fraction != 1 || tr.startsolid || tr.allsolid) && tr.entityNum != ent->r.ownerNum &&
            ...
            tr.startsolid = 0;
            if (tr.entityNum ==
                ENTITYNUM_NONE) { // eh, this is a filthy lie. (obviously it had to hit something or it wouldn't be in here, so we'll say it hit the world)
...
void MakeDeadSaber(gentity_t *ent) { // spawn a "dead" saber entity here so it looks like the saber fell out of the air.
    ...
    if (ent->r.ownerNum >= 0 && ent->r.ownerNum < ENTITYNUM_WORLD) {
        ...
        if (owner->inuse && owner->client && owner->client->saber[0].model[0]) {
...
void DownedSaberThink(gentity_t *saberent) {
    ...
    if (!saberOwn || !saberOwn->inuse || !saberOwn->client || saberOwn->client->sess.sessionTeam == TEAM_SPECTATOR ||
    ...
    if (saberOwn->client->ps.saberEntityNum) {
        if (saberOwn->client->ps.saberEntityNum ==
            saberent->s.number) { // owner shouldn't have this set if we're thinking in here. Must've fallen off a cliff and instantly respawned or something.
            notDisowned = qtrue;
    ...
    if (notDisowned || saberOwn->health < 1 ||
        !saberOwn->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) { // He's dead, just go back to our normal saber status
...
void saberKnockDown(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other) {
    ...
    //(normally at the player's right hand) could result in the saber being stuck
    // in the the map and then freaking out.
    trap->Trace(&tr, &saberent->r.currentOrigin, &saberent->r.mins, &saberent->r.maxs, &saberent->r.currentOrigin, saberent->s.number, saberent->clipmask,
                qfalse, 0, 0);
    if (tr.startsolid || tr.fraction != 1) { // bad position, try popping our origin up a bit
    ...
    if (saberOwner->client->saber[1].soundOff && saberOwner->client->saber[1].model[0]) {
...
#define SABERINVALID                                                                                                                                           \
    (!saberent || !saberOwner || !other || !saberent->inuse || !saberOwner->inuse || !other->inuse || !saberOwner->client || !other->client ||                 \
     !saberOwner->client->ps.saberEntityNum || saberOwner->client->ps.saberLockTime > (level.time - 100))
...
qboolean saberKnockOutOfHand(gentity_t *saberent, gentity_t *saberOwner, const vector3 *velocity) {
    if (!saberent || !saberOwner || !saberent->inuse || !saberOwner->inuse || !saberOwner->client) {
        return qfalse;
    }
...
qboolean saberCheckKnockdown_BrokenParry(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other) {
    ...
    if (!other->client->olderIsValid ||
        (level.time - other->client->lastSaberStorageTime) >=
            200) { // if we don't know which way to throw the saber based on momentum between saber positions, just don't throw it
        return qfalse;
    }
    ...
    if (doKnock) {
        ...
        if (!totalDistance) { // fine, try our own
            if (!saberOwner->client->olderIsValid ||
                (level.time - saberOwner->client->lastSaberStorageTime) >=
                    200) { // if we don't know which way to throw the saber based on momentum between saber positions, just don't throw it
                return qfalse;
            }
...
qboolean saberCheckKnockdown_Smashed(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other, int damage) {
    ...
    if (other && other->inuse && other->client && BG_InExtraDefenseSaberMove(other->client->ps.saberMove)) { // make sure the blow was strong enough
...
void saberBackToOwner(gentity_t *saberent) {
    ...
    if (!saberOwner->inuse || !saberOwner->client || saberOwner->client->sess.sessionTeam == TEAM_SPECTATOR) {
        MakeDeadSaber(saberent);
    ...
    if (saberOwner->health < 1 || !saberOwner->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) { // He's dead, just go back to our normal saber status
        ...
        if (saberOwner->client && saberOwner->client->saber[0].soundOff) {
    ...
    assert(saberOwner->client->ps.saberEntityNum == saberent->s.number || saberOwner->client->saberStoredIndex == saberent->s.number);
    ...
    if (saberent->speed < level.time) {
        ...
        if (saberOwner->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
            FORCE_LEVEL_3) { // allow players with high saber throw rank to control the return speed of the saber
            baseSpeed = 900;
    ...
    */
    ...
    if (saberOwner->client->ps.saberEntityNum == saberent->s.number) {
        if (!(saberOwner->client->saber[0].saberFlags & SFL_RETURN_DAMAGE) || saberOwner->client->ps.saberHolstered) {
            saberent->s.saberInFlight = qfalse;
        }
...
void thrownSaberTouch(gentity_t *saberent, gentity_t *other, trace_t *trace) {
    ...
    if (other && other->r.ownerNum < MAX_CLIENTS && (other->r.contents & CONTENTS_LIGHTSABER) && g_entities[other->r.ownerNum].client &&
...
void saberFirstThrown(gentity_t *saberent) {
    ...
    if (saberOwn->health < 1 || !saberOwn->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) { // He's dead, just go back to our normal saber status
        ...
        if (saberOwn->client && saberOwn->client->saber[0].soundOff) {
    ...
    if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_2 &&
        ...
        if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
            FORCE_LEVEL_3) { // if highest saber throw rank, we can direct the saber toward players directly by looking at them
        ...
        if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
            FORCE_LEVEL_3) { // we'll treat them to a quicker update rate if their throw rank is high enough
...
void UpdateClientRenderinfo(gentity_t *self, vector3 *renderOrigin, vector3 *renderAngles) {
    ...
    if (ri->mPCalcTime < level.time) {
      ...
      // Should you wish to make any of this valid then feel free to do so.
        ...
        if (self->ghoul2 && self->ghoul2 != ri->lastG2) { // the g2 instance changed, so update all the bolts.
...
static qboolean G_KickDownable(gentity_t *ent) {
    ...
    if (BG_InKnockDown(ent->client->ps.legsAnim) || BG_InKnockDown(ent->client->ps.torsoAnim)) {
        return qfalse;
    }

    if (ent->client->ps.weaponTime <= 0 && ent->client->ps.weapon == WP_SABER && ent->client->ps.groundEntityNum != ENTITYNUM_NONE) {
        return qfalse;
    }
...
static void G_TossTheMofo(gentity_t *ent, vector3 *tossDir, float tossStr) {
    ...
    if (ent->health > 0 && ent->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN && BG_KnockDownable(&ent->client->ps) &&
...
static gentity_t *G_KickTrace(gentity_t *ent, vector3 *kickDir, float kickDist, vector3 *kickEnd, int kickDamage, float kickPush) {
    ...
    if (trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid) {
        ...
        if (hitEnt->inuse) { // we hit an entity
            ...
            if (hitEnt->client && !(hitEnt->client->ps.pm_flags & PMF_TIME_KNOCKBACK) // not already flying through air?  Intended to stop multiple hits, but...
...
static void G_GrabSomeMofos(gentity_t *self) {
    ...
    if (trace.fraction != 1.0f && trace.entityNum < ENTITYNUM_WORLD) {
        ...
        if (grabbed->inuse && (grabbed->s.eType == ET_PLAYER || grabbed->s.eType == ET_NPC) && grabbed->client && grabbed->health > 0 &&
            (!grabbed->client->ps.duelInProgress || grabbed->client->ps.duelIndex == self->s.number) &&
            (!self->client->ps.duelInProgress || self->client->ps.duelIndex == grabbed->s.number) && G_CanBeEnemy(self, grabbed) &&
            ...
            if (grabbed->client->ps.torsoAnim == correspondingAnim) { // providing the anim set succeeded..
                if (grabbed->client->ps.weapon == WP_SABER) {         // turn it off
                    if (!grabbed->client->ps.saberHolstered) {
                        ...
                        if (grabbed->client->saber[1].soundOff && grabbed->client->saber[1].model[0]) {
...
void WP_SaberPositionUpdate(
    gentity_t *self,
    usercmd_t *ucmd) { // rww - keep the saber position as updated as possible on the server so that we can try to do realistic-looking contact stuff
    ...
    if (self && self->inuse && self->client && self->client->saberCycleQueue && (self->client->ps.weaponTime <= 0 || self->health < 1)) {
    ...
    if (BG_KickingAnim(self->client->ps.legsAnim)) {
        ...
        if (!grappler->inuse || !grappler->client || grappler->client->grappleIndex != self->s.number || !BG_InGrappleMove(grappler->client->ps.torsoAnim) ||
            !BG_InGrappleMove(grappler->client->ps.legsAnim) || !BG_InGrappleMove(self->client->ps.torsoAnim) || !BG_InGrappleMove(self->client->ps.legsAnim) ||
            !self->client->grappleState || !grappler->client->grappleState || grappler->health < 1 || self->health < 1 ||
            ...
            if (self->client->ps.torsoAnim == BOTH_KYLE_PA_3 && (self->client->ps.torsoTimer > 100 || self->client->ps.legsTimer > 100)) {
                ...
                if ((BG_InGrappleMove(self->client->ps.torsoAnim) && self->client->ps.torsoTimer > 100) ||
                    (BG_InGrappleMove(self->client->ps.legsAnim) &&
                     self->client->ps.legsTimer > 100)) { // if they're pretty far from finishing the anim then shove them into another anim
                    G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_MISS, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
                    if (self->client->ps.torsoAnim == BOTH_KYLE_MISS) { // providing the anim set succeeded..
                        self->client->ps.weaponTime = self->client->ps.torsoTimer;
                    }
                ...
                if (self->client->grappleState >= 20) { // grapplee
                    ...
                    if (grappler->client->ps.weapon == WP_SABER) { // make sure their saber is shut off
                        if (!grappler->client->ps.saberHolstered) {
                            ...
                            if (grappler->client->saber[1].soundOff && grappler->client->saber[1].model[0]) {
    ...
    if (self->client->ps.weapon != WP_SABER || self->client->ps.weaponstate == WEAPON_RAISING || self->client->ps.weaponstate == WEAPON_DROPPING ||
        self->health < 1) {
        if (!self->client->ps.saberInFlight) {
            returnAfterUpdate = 1;
        }
    ...
    if (self->client->ps.saberThrowDelay < level.time) {
        if ((self->client->saber[0].saberFlags & SFL_NOT_THROWABLE)) {                              // cant throw it normally!
            if ((self->client->saber[0].saberFlags & SFL_SINGLE_BLADE_THROWABLE)) {                 // but can throw it if only have 1 blade on
                if (self->client->saber[0].numBlades > 1 && self->client->ps.saberHolstered == 1) { // have multiple blades and only one blade on
...
nextStep:
    ...
    if (returnAfterUpdate && saberNum) { // We don't even need to do GetBoltMatrix if we're only in here to keep the g2 server instance in sync
        ...
        if (mySaber && ((mySaber->r.contents & CONTENTS_LIGHTSABER) || mySaber->r.contents == 0) &&
            !self->client->ps.saberInFlight) { // Since we haven't got a bolt position, place it on top of the player origin.
            VectorCopy(&self->client->ps.origin, &mySaber->r.currentOrigin);
        }
    ...
    if (self->client->ps.saberEntityNum) {
        // I guess it's good to keep the position updated even when contents are 0
        if (mySaber && ((mySaber->r.contents & CONTENTS_LIGHTSABER) || mySaber->r.contents == 0) &&
            !self->client->ps.saberInFlight) { // place it roughly in the middle of the saber..
    ...
    if (!BG_SabersOff(&self->client->ps)) {
        ...
        if (self->client->ps.saberLockTime > level.time && self->client->ps.saberEntityNum) {
            ...
            while (rSaberNum < MAX_SABERS) {
                rBladeNum = 0;
                while (rBladeNum <
                       self->client->saber[rSaberNum]
                           .numBlades) { // Don't bother updating the bolt for each blade for this, it's just a very rough fallback method for during saberlocks
        ...
        if (self->client->ps.saberInFlight) { // if saber is thrown then only do the standard stuff for the left hand saber
            if (!self->client->ps
                     .saberEntityNum) { // however, if saber is not in flight but rather knocked away, our left saber is off, and thus we may do nothing.
                rSaberNum = 1;          // was 2?
        ...
        while (rSaberNum < MAX_SABERS) {
            ...
            */
            ...
            if (!(japp_saberTweaks.integer & SABERTWEAK_TWOBLADEDEFLECTFIX) && rSaberNum > 0 && self->client->saber[1].model[0] &&
            ...
            while (rBladeNum < self->client->saber[rSaberNum].numBlades) {
                ...
                if (!(japp_saberTweaks.integer & SABERTWEAK_TWOBLADEDEFLECTFIX) && rBladeNum > 0 // more than one blade
                    && !self->client->saber[1].model[0]                                          // not using dual blades
                    && self->client->saber[rSaberNum].numBlades > 1                              // using a multi-bladed saber
                    && self->client->ps.saberHolstered == 1) {                                   // don't to extra blades if they're off
                    break;
                }
                // get the new data
                ...
                if (rSaberNum == 0 && self->client->ps.saberInFlight) {
                    if (!self->client->ps.saberEntityNum) { // dropped it... shouldn't get here, but...
                        ...
                        if (!saberEnt || !saberEnt->inuse || !saberEnt->ghoul2) { // wtf?
                            rSaberNum++;
                            rBladeNum = 0;
                            continue;
                        }
                        ...
                        VectorMA(&boltOrigin, self->client->saber[rSaberNum].blade[rBladeNum].lengthMax,
                                 &self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir, &end);
                    }

                } else {
                    trap->G2API_GetBoltMatrix(self->ghoul2, rSaberNum + 1, rBladeNum, &boltMatrix, &properAngles, &properOrigin, level.time, NULL,
                                              &self->modelScale);
                    BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint);
                    BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_Y, &self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir);
                    VectorCopy(&self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint, &boltOrigin);
                    VectorMA(&boltOrigin, self->client->saber[rSaberNum].blade[rBladeNum].lengthMax, &self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir,
                             &end);
                }
                ...
                if (self->client->hasCurrentPosition && d_saberInterpolate.integer) {
                    if (self->client->ps.weaponTime <=
                        0) { // rww - 07/17/02 - don't bother doing the extra stuff unless actually attacking. This is in attempt to save CPU.
                        ...
                        } else if (g_saberTraceSaberFirst.integer >= 2 && level.gametype != GT_DUEL && level.gametype != GT_POWERDUEL &&
                        ...
                        if (skipSaberTrace) { // skip the saber-contents-only trace and get right to the full trace
                            ...
                            while (sN < MAX_CLIENTS) {
                                if (g_entities[sN].inuse && g_entities[sN].client && g_entities[sN].r.linked && g_entities[sN].health > 0 &&
                                    (g_entities[sN].r.contents &
                                     CONTENTS_BODY)) { // Take this mask off before the saber trace, because we want to hit the saber first
                        ...
                        while (!gotHit) {
                            if (!CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qfalse, trMask, qfalse)) {
                                if (!CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qtrue, trMask, qfalse)) {
                                    ...
                                    if ((deltaX != 0 || deltaY != 0 || deltaZ != 0) && deltaX < 180 && deltaY < 180 && deltaZ < 180 &&
                                        (BG_SaberInAttack(self->client->ps.saberMove) ||
                                         PM_SaberInTransition(self->client->ps.saberMove))) { // don't go beyond here if we aren't attacking/transitioning or
                    ...
                    {
                        if (!CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qfalse, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT),
                                              qfalse)) {
                            CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qtrue, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT),
                                             qfalse);
                        }
                    }
...
int WP_SaberCanBlock(gentity_t *self, vector3 *point, uint32_t dflags, int mod, qboolean projectile, int attackStr) {
    ...
    if (self->client->pers.adminData.isSlept || self->client->pers.adminData.isGhost || BG_SaberInAttack(self->client->ps.saberMove)) {
        return 0;
    }
    ...
    if (PM_InSaberAnim(self->client->ps.torsoAnim) && !self->client->ps.saberBlocked && self->client->ps.saberMove != LS_READY &&
    ...
    if (PM_SaberInBrokenParry(self->client->ps.saberMove) || !self->client->ps.saberEntityNum // saber is knocked away
        || BG_SabersOff(&self->client->ps) || self->client->ps.weapon != WP_SABER || self->client->ps.weaponstate == WEAPON_RAISING ||
