...
PAIN_FUNC
CrystalCratePain, emplaced_gun_pain, func_usable_pain, funcBBrushPain, GasBurst, misc_model_breakable_pain, NPC_ATST_Pain, NPC_Droid_Pain, NPC_GM_Pain,
    NPC_Howler_Pain, NPC_Jedi_Pain, NPC_Mark1_Pain, NPC_Mark2_Pain, NPC_MineMonster_Pain, NPC_Pain, NPC_Probe_Pain, NPC_Rancor_Pain, NPC_Remote_Pain,
    NPC_Seeker_Pain, NPC_Sentry_Pain, NPC_ST_Pain, NPC_Wampa_Pain, PlayerPain, station_pain, TurretPain;
...
// NPC_misc.cpp
//
void Debug_Printf(vmCvar_t *cv, int level, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
void Debug_NPCPrintf(gentity_t *printNPC, vmCvar_t *cv, int debugLevel, const char *fmt, ...) __attribute__((format(printf, 4, 5)));
...
int NPC_FindCombatPoint(const vector3 *position, const vector3 *avoidPosition, vector3 *enemyPosition, const uint32_t flags, const float avoidDist,
                        const int ignorePoint); // ignorePoint = -1
...
int NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound, int ignoreAlert, qboolean mustHaveOwner,
                         int minAlertLevel); // ignoreAlert = -1, mustHaveOwner = qfalse, minAlertLevel = AEL_MINOR
...
void NPC_SetMoveGoal(gentity_t *ent, vector3 *point, int radius, qboolean isNavGoal, int combatPoint,
                     gentity_t *targetEnt); // isNavGoal = qfalse, combatPoint = -1, targetEnt = NULL
