...
void NPC_Wampa_Precache(void) { G_SoundIndex("sound/chars/rancor/swipehit.wav"); }
...
void Wampa_Move(qboolean visible) {
    if (NPCInfo->localState != LSTATE_WAITING) {
        ...
        if (NPC->enemy) { // pick correct movement speed and anim
            // run by default
            ucmd.buttons &= ~BUTTON_WALKING;
            if (!TIMER_Done(NPC, "runfar") || !TIMER_Done(NPC, "runclose")) { // keep running with this anim & speed for a bit
            } else if (!TIMER_Done(NPC, "walk")) {                            // keep walking for a bit
                ucmd.buttons |= BUTTON_WALKING;
            } else if (visible && enemyDist > 384 && NPCInfo->stats.runSpeed == 180) { // fast run, all fours
                NPCInfo->stats.runSpeed = 300;
                TIMER_Set(NPC, "runfar", Q_irand(2000, 4000));
            } else if (enemyDist > 256 && NPCInfo->stats.runSpeed == 300) { // slow run, upright
                NPCInfo->stats.runSpeed = 180;
                TIMER_Set(NPC, "runclose", Q_irand(3000, 5000));
            } else if (enemyDist < 128) { // walk
                NPCInfo->stats.runSpeed = 180;
                ucmd.buttons |= BUTTON_WALKING;
        ...
        if (NPCInfo->stats.runSpeed == 300) { // need to use the alternate run - hunched over on all fours
...
void Wampa_Slash(int boltIndex, qboolean backhand) {
    ...
    for (i = 0; i < numEnts; i++) {
        ...
        if (radiusEnt == NPC) { // Skip the wampa ent
            continue;
        }
        ...
        if (DistanceSquared(&radiusEnt->r.currentOrigin, &boltOrg) <= radiusSquared) {
            // smack
            G_Damage(radiusEnt, NPC, NPC, &vec3_origin, &radiusEnt->r.currentOrigin, damage,
            ...
            if (backhand) {
                ...
                if (radiusEnt->client->NPC_class != CLASS_WAMPA && radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_ATST) {
                    G_Throw(radiusEnt, &pushDir, 65);
                    if (BG_KnockDownable(&radiusEnt->client->ps) && radiusEnt->health > 0 && Q_irand(0, 1)) { // do pain on enemy
            ...
            } else if (radiusEnt->health <= 0 && radiusEnt->client) { // killed them, chance of dismembering
            ...
            } else if (!Q_irand(0, 3) && radiusEnt->health > 0) { // one out of every 4 normal hits does a knockdown, too
                vector3 pushDir;
                vector3 angs;
                VectorCopy(&NPC->client->ps.viewangles, &angs);
                angs.yaw += flrand(25, 50);
...
void Wampa_Attack(float distance, qboolean doCharge) {
    if (!TIMER_Exists(NPC, "attacking")) {
        ...
        } else if (doCharge || (distance > 270 && distance < 430 && !Q_irand(0, 1))) { // leap
            vector3 fwd, yawAng;
    ...
    if (NPC->client->ps.legsAnim == BOTH_ATTACK1 && distance > (NPC->r.maxs.x + MIN_DISTANCE)) { // okay to keep moving
        ucmd.buttons |= BUTTON_WALKING;
        Wampa_Move(1);
    }
}
...
void Wampa_Combat(void) {
    ...
    if (!NPC_ClearLOS(&NPC->r.currentOrigin, &NPC->enemy->r.currentOrigin)) {
        ...
        qboolean advance = (qboolean)(distance > (NPC->r.maxs.x + MIN_DISTANCE) ? qtrue : qfalse);
        ...
        if ((advance || NPCInfo->localState == LSTATE_WAITING) && TIMER_Done(NPC, "attacking")) // waiting monsters can't attack
        {
            ...
            if (!Q_irand(0, 20)) { // FIXME: only do this if we just damaged them or vice-versa?
                if (Wampa_CheckRoar(NPC)) {
                    return;
                }
            }
            if (!Q_irand(0, 1)) { // FIXME: base on skill
...
void NPC_Wampa_Pain(gentity_t *self, gentity_t *attacker, int damage) {
    ...
    if (attacker && attacker->inuse && attacker != self->enemy && !(attacker->flags & FL_NOTARGET)) {
        if ((!attacker->s.number && !Q_irand(0, 3)) || !self->enemy || self->enemy->health == 0 ||
            ...
            (!Q_irand(0, 4) &&
             DistanceSquared(&attacker->r.currentOrigin, &self->r.currentOrigin) <
                 DistanceSquared(
                     ...
                     &self->r.currentOrigin))) { // if my enemy is dead (or attacked by player) and I'm not still holding/eating someone, turn on the attacker
            ...
            if (hitByWampa) { // stay mad at this Wampa for 2-5 secs before looking for attacker enemies
    ...
    if ((hitByWampa || Q_irand(0, 100) < damage) // hit by wampa, hit while holding live victim, or took a lot of damage
        ...
        if (!Wampa_CheckRoar(self)) {
            if (self->client->ps.legsAnim != BOTH_ATTACK1 && self->client->ps.legsAnim != BOTH_ATTACK2 &&
                self->client->ps.legsAnim != BOTH_ATTACK3) { // cant interrupt one of the big attack anims
...
void NPC_BSWampa_Default(void) {
    ...
    if (NPC->enemy) {
        if (!TIMER_Done(NPC, "attacking")) { // in middle of attack
            // face enemy
            NPC_FaceEnemy(qtrue);
            ...
            if (NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_WAMPA) { // got mad at another Wampa, look for a valid enemy
                ...
                if (ValidEnemy(NPC->enemy) == qfalse) {
                    ...
                    if (!NPC->enemy->inuse ||
                        ...
                        // just lost my enemy
                        if ((NPC->spawnflags & 2)) { // search around me if I don't have an enemy
                            NPC_BSSearchStart(NPC->waypoint, BS_SEARCH);
                            NPCInfo->tempBehavior = BS_DEFAULT;
                        } else if ((NPC->spawnflags & 1)) { // wander if I don't have an enemy
        ...
        if ((NPC->spawnflags & 2)) {                // search around me if I don't have an enemy
            if (NPCInfo->homeWp == WAYPOINT_NONE) { // no homewap, initialize the search behavior
            ...
            if (NPCInfo->homeWp == WAYPOINT_NONE) { // no homewap, initialize the wander behavior
