	...
	if ( NPCInfo->localState != LSTATE_WAITING ) {
		...
		if ( NPC->enemy ) {//pick correct movement speed and anim
			...
			if ( !TIMER_Done( NPC, "runfar" )
				|| !TIMER_Done( NPC, "runclose" ) ) {//keep running with this anim & speed for a bit
			}
			...
			if ( backhand ) {
				...
				if ( radiusEnt->client->NPC_class != CLASS_WAMPA
					...
					if ( BG_KnockDownable( &radiusEnt->client->ps ) &&
						radiusEnt->health > 0 && Q_irand( 0, 1 ) ) {//do pain on enemy
...
void NPC_Wampa_Pain( gentity_t *self, gentity_t *attacker, int damage ) {
	...
	if ( attacker
		...
		if ( (!attacker->s.number&&!Q_irand( 0, 3 ))
			...
			|| (!Q_irand( 0, 4 ) && DistanceSquared( &attacker->r.currentOrigin, &self->r.currentOrigin ) < DistanceSquared( &self->enemy->r.currentOrigin, &self->r.currentOrigin )) ) {//if my enemy is dead (or attacked by player) and I'm not still holding/eating someone, turn on the attacker
