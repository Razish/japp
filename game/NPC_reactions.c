	...
	if ( random() < pain_chance ) {
		...
		if ( self->client->ps.fd.forceGripBeingGripped < level.time ) {//not being force-gripped or force-drained
			if ( /*G_CheckForStrongAttackMomentum( self ) //rwwFIXMEFIXME: Is this needed?
				...
				|| (BG_FlippingAnim( self->client->ps.legsAnim ) && !PM_InCartwheel( self->client->ps.legsAnim )) ) {//strong attacks, rolls, knockdowns, flips and spins cannot be interrupted by pain
			}
			else {//play an anim
				int parts;
	...
	{//hit by a teammate
		if ( other != self->enemy && self != other->enemy ) {//we weren't already enemies
			if ( self->enemy || other->enemy
				...
				/*|| (!other->s.number&&Q_irand( 0, 3 ))*/ ) {//if one of us actually has an enemy already, it's okay, just an accident OR wasn't hit by player or someone controlled by player OR player hit ally and didn't get 25% chance of getting mad (FIXME:accumulate anger+base on diff?)
			...
			{//dammit, stop that!
				...
				else {//okay, we're going to turn on our ally, we need to set and lock our enemy and put ourselves in a bstate that lets us attack him (and clear any flags that would stop us)
		...
		if ( /*!(self->svFlags&SVF_LOCKEDENEMY) && !(self->svFlags&SVF_IGNORE_ENEMIES) &&*/ !(other->flags & FL_NOTARGET) ) {
			if ( self->client->enemyTeam ) {//See if we bumped into an enemy
				if ( other->client->playerTeam == self->client->enemyTeam ) {//bumped into an enemy
					if ( NPCInfo->behaviorState != BS_HUNT_AND_KILL && !NPCInfo->tempBehavior ) {//MCG - Begin: checking specific BS mode here, this is bad, a HACK
						//FIXME: not medics?
...
void NPC_UseResponse( gentity_t *self, gentity_t *user, qboolean useWhenDone ) {
	...
	if ( user->client && self->client->playerTeam != user->client->playerTeam && self->client->playerTeam != NPCTEAM_NEUTRAL ) {//only those on the same team react
		if ( useWhenDone ) {
			G_ActivateBehavior( self, BSET_USE );
		}
		return;
