...
static void CVU_Duel(void) { SetCInfo((g_privateDuel.bits & PRIVDUEL_WEAP), CINFO_PRIVDUELWEAP); }
...
static void CVU_DFA(void) { SetCInfo(!japp_allowDFA.integer, CINFO_NODFA); }

static void CVU_Kata(void) { SetCInfo(!japp_allowKata.integer, CINFO_NOKATA); }

static void CVU_Ledge(void) { SetCInfo(japp_allowLedgeGrab.integer, CINFO_LEDGEGRAB); }

static void CVU_SPCartwheel(void) { SetCInfo(!japp_allowSPCartwheel.integer, CINFO_NOSPCARTWHEEL); }

static void CVU_SpinKicks(void) { SetCInfo(japp_allowSpinkicks.integer, CINFO_SPINKICKS); }

static void CVU_Stab(void) { SetCInfo(!japp_allowStab.integer, CINFO_NOSTAB); }

static void CVU_ToggleAtk(void) { SetCInfo(japp_allowToggleSpecialAttacks.integer, CINFO_TOGGLESPECIALATK); }
...
static void CVU_Flipkick(void) { SetCInfo(japp_flipKick.integer, CINFO_FLIPKICK); }

static void CVU_YellowDFA(void) { SetCInfo(japp_improveYellowDFA.integer, CINFO_YELLOWDFA); }
...
static void CVU_HeadSlide(void) { SetCInfo(japp_slideOnHead.integer, CINFO_HEADSLIDE); }

static void CVU_VQ3Physics(void) { SetCInfo(japp_vq3physics.integer, CINFO_VQ3PHYS); }

static void CVU_WeaponPU(void) { SetCInfo(japp_weaponPickupAlways.integer, CINFO_ALWAYSPICKUPWEAP); }

static void CVU_WeaponRoll(void) { SetCInfo(japp_weaponRoll.integer, CINFO_WEAPONROLL); }
...
static int G_AddBox(const vector3 *mins, const vector3 *maxs) {
    ...
    int xylen = std::min(std::min(510.0f, maxs->x - mins->x), maxs->y - mins->y);
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
    ...
    // logging
    if (g_logConsole.integer)
        G_OpenLog(LOG_DIRECTORY "console.log", &level.log.console, g_logConsole.integer == 2);
    else
        trap->Print("Not logging console to disk.\n");
    if (g_logSecurity.integer)
        G_OpenLog(LOG_DIRECTORY "security.log", &level.log.security, g_logSecurity.integer == 2);
    else
        trap->Print("Not logging security events to disk.\n");
    if (g_logAdmin.integer)
        G_OpenLog(LOG_DIRECTORY "admin.log", &level.log.admin, g_logAdmin.integer == 2);
    else
...
void AdjustTournamentScores(void) {
    ...
    if (level.clients[level.sortedClients[0]].ps.persistant[PERS_SCORE] == level.clients[level.sortedClients[1]].ps.persistant[PERS_SCORE] &&
        level.clients[level.sortedClients[0]].pers.connected == CON_CONNECTED && level.clients[level.sortedClients[1]].pers.connected == CON_CONNECTED) {
...
qboolean G_CanResetDuelists(void) {
    ...
    while (i < 3) { // precheck to make sure they are all respawnable
        ...
        if (!ent->inuse || !ent->client || ent->health <= 0 || ent->client->sess.sessionTeam == TEAM_SPECTATOR || ent->client->sess.duelTeam <= DUELTEAM_FREE) {
            return qfalse;
        }
        i++;
    }
...
void CalculateRanks(void) {
    ...
    if (level.gametype >= GT_TEAM) {
        ...
        if (level.gametype != GT_DUEL &&
            level.gametype != GT_POWERDUEL) { // when not in duel, use this configstring to pass the index of the player currently in first place
...
void FindIntermissionPoint(void) {
    ...
    if (level.gametype == GT_SIEGE && level.intermissiontime && level.intermissiontime <= level.time && gSiegeRoundEnded) {
...
void BeginIntermission(void) {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        // respawn if dead
        if (client->health <= 0) {
            if (level.gametype != GT_POWERDUEL || !client->client ||
...
void ExitLevel(void) {
    ...
    if (level.gametype == GT_SIEGE && g_siegeTeamSwitch.integer) { // switch out now
        SiegeDoTeamAssign();
    }
...
void LogExit(const char *string) {
    ...
    */
    ...
    if (japp_showNextMap.integer) {
        ...
        trap->SendServerCommand(-1, va("print \"" S_COLOR_CYAN "next map is " S_COLOR_YELLOW "%s\n\"", buffer));
    }
}
...
void CheckIntermissionExit(void) {
    ...
    if ((level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) && !gDidDuelStuff && (level.time > level.intermissiontime + 2000)) {
        gDidDuelStuff = qtrue;
        ...
        if (g_austrian.integer && level.gametype != GT_POWERDUEL) {
            ...
            G_LogPrintf(level.log.console, "winner: %s, score: %d, wins/losses: %d/%d\n", level.clients[level.sortedClients[0]].pers.netname,
                        level.clients[level.sortedClients[0]].ps.persistant[PERS_SCORE], level.clients[level.sortedClients[0]].sess.wins,
            ...
            G_LogPrintf(level.log.console, "loser: %s, score: %d, wins/losses: %d/%d\n", level.clients[level.sortedClients[1]].pers.netname,
                        level.clients[level.sortedClients[1]].ps.persistant[PERS_SCORE], level.clients[level.sortedClients[1]].sess.wins,
            ...
            if (g_austrian.integer) {
                if (level.gametype == GT_POWERDUEL) {
                    G_LogPrintf(level.log.console, "Power Duel Initiated: %s %d/%d vs %s %d/%d and %s %d/%d, kill limit: %d\n",
                                level.clients[level.sortedClients[0]].pers.netname, level.clients[level.sortedClients[0]].sess.wins,
                                level.clients[level.sortedClients[0]].sess.losses, level.clients[level.sortedClients[1]].pers.netname,
                                level.clients[level.sortedClients[1]].sess.wins, level.clients[level.sortedClients[1]].sess.losses,
                                level.clients[level.sortedClients[2]].pers.netname, level.clients[level.sortedClients[2]].sess.wins,
                                level.clients[level.sortedClients[2]].sess.losses, fraglimit.integer);
                } else {
                    G_LogPrintf(level.log.console, "Duel Initiated: %s %d/%d vs %s %d/%d, kill limit: %d\n", level.clients[level.sortedClients[0]].pers.netname,
                                level.clients[level.sortedClients[0]].sess.wins, level.clients[level.sortedClients[0]].sess.losses,
                                level.clients[level.sortedClients[1]].pers.netname, level.clients[level.sortedClients[1]].sess.wins,
                                level.clients[level.sortedClients[1]].sess.losses, fraglimit.integer);
                }
            }
    ...
    if (gEscaping) {
        ...
        while (i < MAX_CLIENTS) {
            if (g_entities[i].inuse && g_entities[i].client && g_entities[i].health > 0) {
                if (g_entities[i].client->sess.sessionTeam != TEAM_SPECTATOR && !(g_entities[i].client->ps.pm_flags & PMF_FOLLOW)) {
                    numLiveClients++;
                }
            }
            ...
            if (cl->ps.persistant[PERS_SCORE] >= fraglimit.integer) {
                ...
                if (printLimit) {
                    trap->SendServerCommand(
                        -1, va("print \"%s" S_COLOR_WHITE " %s.\n\"", cl->pers.netname, G_GetStringEdString("MP_SVGAME", "HIT_THE_KILL_LIMIT")));
                }
                return;
            }
        }
...
void G_RemoveDuelist(int team) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent->inuse && ent->client && ent->client->sess.sessionTeam != TEAM_SPECTATOR && ent->client->sess.duelTeam == team) {
    ...
    if (level.gametype == GT_DUEL) {
        ...
        if (level.numPlayingClients >= 2) {
            ...
            if (g_showDuelHealths.integer) {
                ...
                trap->SetConfigstring(CS_CLIENT_DUELHEALTHS, va("%i|%i|!", ps1->stats[STAT_HEALTH], ps2->stats[STAT_HEALTH]));
            }
        }
        ...
        if (level.numPlayingClients < 3 && !g_dontFrickinCheck) {
            ...
            if (level.numPlayingClients >= 3 && G_CanResetDuelists()) {
...
int BG_GetTime(void) { return level.time; }
...
void G_RunFrame(int levelTime) {
    ...
    if (level.gametype == GT_SIEGE && g_siegeRespawn.integer && g_siegeRespawnCheck < level.time) { // check for a respawn wave
        int i = 0;
        gentity_t *clEnt;
        while (i < MAX_CLIENTS) {
            clEnt = &g_entities[i];

            if (clEnt->inuse && clEnt->client && clEnt->client->tempSpectate >= level.time && clEnt->client->sess.sessionTeam != TEAM_SPECTATOR) {
                respawn(clEnt);
    ...
    if (g_allowNPC.integer) {
        ...
        for (i = 0; i < level.num_entities; i++) {
            ...
            if (ent->waypoint != WAYPOINT_NONE && ent->noWaypointTime < level.time) {
        ...
        if (i < MAX_CLIENTS) {
            ...
            if (ent->client->isHacking && level.pause.state == PAUSE_NONE) { // hacking checks
                ...
                if (!(ent->client->pers.cmd.buttons & BUTTON_USE)) { // have to keep holding use
                    ...
                    ent->client->isHacking = 0;
                    ent->client->ps.hackingTime = 0;
                } else if (!G_PointInBounds(&ent->client->ps.origin, &hacked->r.absmin,
                                            &hacked->r.absmax)) { // they stepped outside the thing they're hacking, so reset hacking time
                    ent->client->isHacking = 0;
                    ent->client->ps.hackingTime = 0;
...
#define CLOAK_REFUEL_RATE 150                                                              // seems fair
            ...
            if (level.pause.state == PAUSE_NONE && !level.intermissiontime && !(ent->client->ps.pm_flags & PMF_FOLLOW) &&
...
#ifdef _G_FRAME_PERFANAL
    Com_Printf("---------------\nItemRun: %i\nROFF: %i\nClientEndframe: %i\nGameChecks: %i\nQueues: %i\n---------------\n", iTimer_ItemRun, iTimer_ROFF,
               iTimer_ClientEndframe, iTimer_GameChecks, iTimer_Queues);
#endif
...
static void _G_ROFF_NotetrackCallback(int entID, const char *notetrack) { G_ROFF_NotetrackCallback(&g_entities[entID], notetrack); }
...
static qboolean G_NPC_ClearLOS2(int entID, const vector3 *end) { return NPC_ClearLOS2(&g_entities[entID], end); }
static qboolean G_NAV_CheckNodeFailedForEnt(int entID, int nodeNum) { return NAV_CheckNodeFailedForEnt(&g_entities[entID], nodeNum); }
