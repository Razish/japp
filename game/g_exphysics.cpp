                ...
                if (!hasFirstCollision) { // don't have one yet so just use this one
                    ...
                    if (tr.allsolid && !bestCollision.allsolid) { // If the whole trace is solid then this one is deeper
                        ...
                        VectorCopy(&boneOrg, &collisionRootPos);
                               ...
                               tr.fraction < bestCollision.fraction) { // and finally, if neither is startsolid/allsolid, but the new one has a smaller
                                                                       // fraction, then it's closer to an impact point so we will use it
                        bestCollision = tr;
                        VectorCopy(&boneOrg, &collisionRootPos);
                    }
                }
