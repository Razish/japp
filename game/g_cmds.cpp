...
void Cmd_Score_f(gentity_t *ent) { ent->client->scoresWaiting = qtrue; }
...
static void Cmd_Ignore_f(gentity_t *ent) {
    ...
    if (atoi(arg) == -1) {
        if (ent->client->pers.ignore == (0xFFFFFFFFu & ~(1 << (ent - g_entities)))) {
            ...
            trap->SendServerCommand(ent - g_entities, "print \"" S_COLOR_YELLOW "Unignoring " S_COLOR_WHITE "everyone\n\"");
        } else {
...
static gentity_t *G_AttackerCheck(gentity_t *ent) {
    ...
    if (attacker && attacker->client && ent->client->ps.otherKillerTime > level.time && attacker->client->sess.sessionTeam != TEAM_SPECTATOR &&
        attacker->client->tempSpectate < level.time &&
        (attacker->client->sess.sessionTeam != ent->client->sess.sessionTeam ||
         (g_gametype.integer < GT_TEAM && attacker->client->sess.sessionTeam == TEAM_FREE)) &&
        attacker->health > 0) {
        otherKiller = qtrue;
    }
...
void Cmd_Kill_f(gentity_t *ent) {
    ...
    if ((level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) && level.numPlayingClients > 1 && !level.warmupTime && !g_allowDuelSuicide.integer) {
...
qboolean G_SetSaber(gentity_t *ent, int saberNum, const char *saberName, qboolean siegeOverride) {
    ...
    if (!siegeOverride && level.gametype == GT_SIEGE && ent->client->siegeClass != -1 &&
        (bgSiegeClasses[ent->client->siegeClass].saberStance || bgSiegeClasses[ent->client->siegeClass].saber1[0] ||
         bgSiegeClasses[ent->client->siegeClass].saber2[0])) { // don't let it be changed if the siege class has forced any saber-related things
        return qfalse;
    }
...
static void G_SayTo(gentity_t *ent, gentity_t *other, int mode, char color, const char *name, const char *message, char *locMsg) {
    ...
    if (level.gametype == GT_SIEGE && ent->client && (ent->client->tempSpectate >= level.time || ent->client->sess.sessionTeam == TEAM_SPECTATOR) &&
    ...
    if (locMsg) {
        trap->SendServerCommand(other - g_entities,
                                va("%s \"%s\" \"%s\" \"%c\" \"%s\"", (mode == SAY_TEAM) ? "ltchat" : "lchat", name, locMsg, color, message));
    } else {
        trap->SendServerCommand(other - g_entities, va("%s \"%s%c%c%s\"", (mode == SAY_TEAM) ? "tchat" : "chat", name, Q_COLOR_ESCAPE, color, message));
    }
}
...
static void Cmd_SayTeamMod_f(gentity_t *ent) {
    ...
    trap->SendServerCommand(ent - g_entities, va("print \"" S_COLOR_CYAN "redirecting team messages to: " S_COLOR_YELLOW "%s\n\"",
...
static const char *gc_orders[] = {"hold your position", "hold this position", "come here", "cover me", "guard location", "search and destroy", "report"};
...
void SV_ToggleAllowVote_f(void) {
    if (trap->Argc() == 1) {
        ...
        trap->Print("%s %s^7\n", validVoteStrings[index].string, ((g_allowVote.bits & (1 << index)) ? "^2Enabled" : "^1Disabled"));
    }
}
    ...
    if (i == validVoteStringsSize) {
        ...
        for (i = 0; i < validVoteStringsSize; i++) {
            ...
            if (validVoteStrings[i].shortHelp) {
                Q_strcat(buf, sizeof(buf), va("^%c%s %s ", toggle ? COLOR_GREEN : COLOR_YELLOW, validVoteStrings[i].string, validVoteStrings[i].shortHelp));
            } else {
                Q_strcat(buf, sizeof(buf), va("^%c%s ", toggle ? COLOR_GREEN : COLOR_YELLOW, validVoteStrings[i].string));
            }
        }
...
void Cmd_EngageDuel_f(gentity_t *ent, bool fullforce) {
    ...
    // New: Don't let a player duel if he just did and hasn't waited 10 seconds yet (note: If someone challenges him, his duel timer will reset so he can
    // accept)
    ...
    if (tr->fraction < 1.0f && tr->entityNum < MAX_CLIENTS) {
        ...
        if (challenged->client->ps.duelIndex == ent->s.number && challenged->client->ps.duelTime >= level.time) {
            ...
            if (japp_duelBow.integer) {
                ...
                ent->client->ps.duelTime = challenged->client->ps.duelTime = level.time + bowTime;
            }

            else {
                ent->client->ps.forceHandExtend = HANDEXTEND_DUELCHALLENGE;
                ent->client->ps.forceHandExtendTime = level.time + 1000;
                ent->client->ps.duelTime = challenged->client->ps.duelTime = level.time + 500;
            }
            ...
            challenged->client->ps.fd.privateDuelTime =
                0; // reset the timer in case this player just got out of a duel. He should still be able to accept the challenge.
...
void Cmd_EngageFullForceDuel_f(gentity_t *ent) { Cmd_EngageDuel_f(ent, true); }
...
static void Cmd_Sabercolor_f(gentity_t *ent) {
    ...
    if (trap->Argc() < 5) {
        trap->SendServerCommand(ent - g_entities, "print \"" S_COLOR_YELLOW "Usage: \\sabercolor " S_COLOR_WHITE "<" S_COLOR_YELLOW "1-2" S_COLOR_WHITE "> "
...
static void Cmd_Playertint_f(gentity_t *ent) {
    ...
    if (trap->Argc() < 4) {
        trap->SendServerCommand(ent - g_entities, "print \"" S_COLOR_YELLOW "Usage: \\playertint " S_COLOR_WHITE "<" S_COLOR_YELLOW "1-2" S_COLOR_WHITE "> "
                                                  "<" S_COLOR_RED "0-255" S_COLOR_WHITE "> "
                                                  "<" S_COLOR_GREEN "0-255" S_COLOR_WHITE "> "
                                                  "<" S_COLOR_CYAN "0-255" S_COLOR_WHITE ">\n\"");
...
static void Cmd_Drop_f(gentity_t *ent) {
    ...
    if (!Q_stricmp(arg, "flag")) {
        ...
        if (!(ent->client->ps.stats[STAT_WEAPONS] & (1 << wp)) || !ent->client->ps.ammo[weaponData[wp].ammoIndex] || wp == WP_SABER || wp == WP_MELEE ||
            wp == WP_EMPLACED_GUN || wp == WP_TURRET || wp <= WP_NONE || wp > WP_NUM_WEAPONS) {
        ...
        } else if (ent->client->ps.powerups[PW_FORCE_ENLIGHTENED_LIGHT] >= level.time &&
...
static void PB_Callback(const char *buffer, int clientNum) { trap->SendServerCommand(clientNum, va("print \"%s\"", buffer)); }
...
static void Cmd_AMInfo_f(gentity_t *ent) {
    ...
    Q_PrintBuffer(&pb, S_COLOR_YELLOW "================================================================" S_COLOR_WHITE "\n\n");
    ...
    if (extendedInfo & EXTINFO_SABER) {
        ...
        if (japp_saberTweaks.integer) {
            ...
            Q_PrintBuffer(&pb, va("    [%s" S_COLOR_WHITE "] Nerf damage\n", d_saberSPStyleDamage.integer    ? S_COLOR_YELLOW "-"
                                                                             : (tweaks & SABERTWEAK_NERFDMG) ? S_COLOR_GREEN "+"
                                                                                                             : S_COLOR_RED "x"));
            Q_PrintBuffer(
                &pb, va("    [%s" S_COLOR_WHITE "] Never location based damage\n",
                        g_locationBasedDamage.integer ? ((tweaks & SABERTWEAK_NOTLOCATIONBASED) ? S_COLOR_GREEN "+" : S_COLOR_RED "x") : S_COLOR_YELLOW "-"));
        ...
        Q_PrintBuffer(&pb, va("  " S_COLOR_WHITE "Idle damage %s %s" S_COLOR_WHITE "\n",
                              (japp_saberIdleDamage.integer != atoff(G_Cvar_DefaultString(&japp_saberIdleDamage))) ? S_COLOR_RED : S_COLOR_GREEN,
...
static void Cmd_Origin_f(gentity_t *ent) {
    ...
    trap->SendServerCommand(ent - g_entities, va("print \"Origin: %s\nAngles: %s\n\"", vtos(&targ->client->ps.origin), vtos(&targ->client->ps.viewangles)));
}
...
static void Cmd_Saber_f(gentity_t *ent) {
    ...
    if (argc == 1) {
        ...
        // busy
        if (ent->client->ps.weaponTime > 0 || ent->client->ps.saberMove > LS_READY ||
            ent->client->ps.fd.forcePowersActive
            //	|| ent->client->ps.groundEntityNum == ENTITYNUM_NONE
            || ent->client->ps.duelInProgress || BG_InKnockDown(ent->client->ps.legsAnim) || BG_InRoll(&ent->client->ps, ent->client->ps.legsAnim)) {
        ...
        if (!G_SetSaber(ent, 0, saber1, qfalse) || !G_SetSaber(ent, 1, saber2, qfalse) || !ClientUserinfoChanged(ent - g_entities)) {
...
qboolean SetEmote(gentity_t *ent, const emote_t *emote) {
    ...
    if (ent->client->ps.weaponTime > 0 || ent->client->ps.saberMove > LS_READY || ent->client->ps.fd.forcePowersActive ||
        ent->client->ps.groundEntityNum == ENTITYNUM_NONE || ent->client->ps.duelInProgress || BG_InKnockDown(ent->client->ps.legsAnim) ||
        BG_InRoll(&ent->client->ps, ent->client->ps.legsAnim) || ent->client->ps.forceHandExtend != HANDEXTEND_NONE || ent->client->emote.freeze) {
        return qfalse;
    }
...
#define EMOTE(x)                                                                                                                                               \
    static void Cmd_Emote_##x(gentity_t *ent) { RegularEmote(ent, XSTRING(x)); }
EMOTE(aimgun)
EMOTE(amhiltthrow1)
EMOTE(amhiltthrow2)
EMOTE(atease)
...
static int cmdcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((command_t *)b)->name); }
...
uint32_t G_CmdValid(const gentity_t *ent, const command_t *cmd) {
    ...
    if ((cmd->flags & CMDFLAG_ALIVE) && (ent->health <= 0 || ent->client->tempSpectate >= level.time || ent->client->sess.sessionTeam == TEAM_SPECTATOR)) {
