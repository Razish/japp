    ...
    */
    {
        ...
        self->client->ps.velocity.x = Q_irand(-20, -10);
        self->client->ps.velocity.y = Q_irand(-20, -10);
        self->client->ps.velocity.z = -100;
    }
...
void Interrogator_Melee(qboolean visible, qboolean advance) {
    if (TIMER_Done(NPC, "attackDelay")) // Attack?
    {
        ...
        if (NPC->r.currentOrigin.z >= NPC->enemy->r.currentOrigin.z + NPC->enemy->r.mins.z &&
            NPC->r.currentOrigin.z + NPC->r.mins.z + 8 < NPC->enemy->r.currentOrigin.z + NPC->enemy->r.maxs.z) {
            // gentity_t *tent;
