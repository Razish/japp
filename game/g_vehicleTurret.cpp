...
void VEH_TurretCheckFire(Vehicle_t *pVeh, gentity_t *parent,
    ...
    // if ( turretEnemy )
    {
        ...
        nextMuzzle = ((curMuzzle + 1) == pVeh->m_pVehicleInfo->turret[turretNum].iMuzzle[0]) ? pVeh->m_pVehicleInfo->turret[turretNum].iMuzzle[1]
...
qboolean VEH_TurretAim(Vehicle_t *pVeh, gentity_t *parent, gentity_t *turretEnemy, turretStats_t *turretStats, vehWeaponInfo_t *vehWeapon, int turretNum,
                       int curMuzzle, vector3 *desiredAngles) {
    ...
    if (pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft && desiredAngles->yaw > pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft) {
        aimCorrect = qfalse;
        desiredAngles->yaw = pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft;
    }
    if (pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight && desiredAngles->yaw < pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight) {
        aimCorrect = qfalse;
    ...
    if (pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown && desiredAngles->pitch > pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown) {
        aimCorrect = qfalse;
        desiredAngles->pitch = pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown;
    }
    if (pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp && desiredAngles->pitch < pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp) {
        aimCorrect = qfalse;
...
static qboolean VEH_TurretFindEnemies(Vehicle_t *pVeh, gentity_t *parent, turretStats_t *turretStats, int turretNum, int curMuzzle) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (target == parent || !target->takedamage || target->health <= 0 || (target->flags & FL_NOTARGET)) {
            continue;
        }
        if (!target->client) {               // only attack clients
            if (!(target->flags & FL_BBRUSH) // not a breakable brush
                ...
                || (target->NPC_targetname && parent->targetname &&
                    Q_stricmp(target->NPC_targetname, parent->targetname) != 0)) // not in invicible bbrush, but can only be broken by an NPC that is not me
            {
                if (target->s.weapon == WP_TURRET && target->classname &&
        ...
        if (target == ((gentity_t *)pVeh->m_pPilot) || target->r.ownerNum == parent->s.number) { // don't get angry at my pilot or passengers?
            continue;
        }
...
void VEH_TurretThink(Vehicle_t *pVeh, gentity_t *parent, int turretNum) {
    ...
    if (turretStats->passengerNum && pVeh->m_iNumPassengers >= turretStats->passengerNum) { // the passenger that has control of this turret is on the ship
    ...
    if (pVeh->turretStatus[turretNum].enemyEntNum < ENTITYNUM_WORLD) {
        ...
        if (turretEnemy->health < 0 || !turretEnemy->inuse || turretEnemy == ((gentity_t *)pVeh->m_pPilot) // enemy became my pilot///?
            || turretEnemy == parent || turretEnemy->r.ownerNum == parent->s.number                        // a passenger?
        ...
        if (turretEnemy->health > 0) {
            ...
            if (enemyDist < rangeSq) {
                ...
                if (trap->InPVS(&pVeh->m_vMuzzlePos[curMuzzle], &turretEnemy->r.currentOrigin)) {
                    ...
                    if (tr.entityNum == turretEnemy->s.number || (!tr.allsolid && !tr.startsolid)) {
