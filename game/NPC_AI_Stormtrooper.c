...
static qboolean ST_Move( void ) {
	...
	if ( moved == qfalse ) {//FIXME: if we're going to a combat point, need to pick a different one
		if ( !trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPC, TID_MOVE_NAV ) ) {//can't transfer movegoal or stop when a script we're running is waiting to complete
			...
			{//dammit, something is in our way
				...
				for ( j = 0; j < NPCInfo->group->numGroup; j++ ) {
					if ( NPCInfo->group->member[j].number == NPCInfo->blockingEntNum ) {//we're being blocked by one of our own, pass our goal onto them and I'll stand still
...
void NPC_BSST_Sleep( void ) {
	...
	if ( alertEvent >= 0 ) {
		...
		if ( level.alertEvents[alertEvent].level == AEL_DISCOVERED && (NPCInfo->scriptFlags&SCF_LOOK_FOR_ENEMIES) ) { //rwwFIXMEFIXME: Care about all clients not just 0
#if 0
...
qboolean NPC_CheckEnemyStealth( gentity_t *target ) {
	...
	if ( target->client->ps.weapon == WP_SABER && !target->client->ps.saberHolstered && !target->client->ps.saberInFlight ) {//if target has saber in hand and activated, we wake up even sooner even if not facing him
		minDist = 100;
	}
...
static qboolean NPC_ST_InvestigateEvent( int eventID, qboolean extraSuspicious ) {
	...
	*/
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCInfo->investigateCount > 1 && (NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) ) {
		...
		if ( G_ExpandPointToBBox( &NPCInfo->investigateGoal, &NPC->r.mins, &NPC->r.maxs, NPC->s.number, ((NPC->clipmask&~CONTENTS_BODY) | CONTENTS_BOTCLIP) ) ) {//we were able to move the investigateGoal to a point in which our bbox would fit
			...
			trap->Trace( &trace, &NPCInfo->investigateGoal, &NPC->r.mins, &NPC->r.maxs, &end, ENTITYNUM_NONE, ((NPC->clipmask&~CONTENTS_BODY) | CONTENTS_BOTCLIP), qfalse, 0, 0 );
...
static void ST_CheckMoveState( void ) {
	...
	if ( (NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL) ) {
		...
		if ( NAV_HitNavGoal( &NPC->r.currentOrigin, &NPC->r.mins, &NPC->r.maxs, &NPCInfo->goalEntity->r.currentOrigin, 16, FlyingCreature( NPC ) ) ||
			(!trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPC, TID_MOVE_NAV ) && NPCInfo->squadState == SQUAD_SCOUT && enemyLOS && enemyDist <= 10000) ) {//either hit our navgoal or our navgoal was not a crucial (scripted) one (maybe a combat point) and we're scouting and found our enemy
...
static void ST_CheckFireState( void ) {
	...
	if ( NPCInfo->squadState == SQUAD_RETREAT || NPCInfo->squadState == SQUAD_TRANSITION || NPCInfo->squadState == SQUAD_SCOUT ) {//runners never try to fire at the last pos
		return;
	}
...
void ST_Commander( void ) {
	...
	*/
	...
	if ( group->numState[SQUAD_SCOUT] > 0 ||
		...
		group->numState[SQUAD_RETREAT] > 0 ) {//someone is running
		runner = qtrue;
	}

	if ( /*!runner &&*/ group->lastSeenEnemyTime > level.time - 32000 && group->lastSeenEnemyTime < level.time - 30000 ) {//no-one has seen the enemy for 30 seconds// and no-one is running after him
	...
	for ( i = curMemberNum; i < lastMemberNum; i++ ) {
		...
		if ( NPCInfo->squadState != SQUAD_RETREAT ) {//not already retreating
			if ( NPC->client->ps.weapon == WP_NONE ) {//weaponless, should be hiding
				if ( NPCInfo->goalEntity == NULL || NPCInfo->goalEntity->enemy == NULL || NPCInfo->goalEntity->enemy->s.eType != ET_ITEM ) {//not running after a pickup
					if ( TIMER_Done( NPC, "hideTime" ) || (DistanceSquared( &group->enemy->r.currentOrigin, &NPC->r.currentOrigin ) < 65536 && NPC_ClearLOS4( NPC->enemy )) ) {//done hiding or enemy near and can see us
			...
			if ( TIMER_Done( NPC, "roamTime" ) && TIMER_Done( NPC, "hideTime" ) && NPC->health > 10 && !trap->InPVS( &group->enemy->r.currentOrigin, &NPC->r.currentOrigin ) ) {//cant even see enemy
				//better go after him
							...
							if ( !group->enemy->client->ps.saberHolstered ) {
								if ( DistanceSquared( &group->enemy->r.currentOrigin, &NPC->r.currentOrigin ) < 65536 ) {
									if ( TIMER_Done( NPC, "hideTime" ) ) {
										if ( NPCInfo->squadState != SQUAD_TRANSITION ) {//not already moving: FIXME: we need to see if where we're going is good now?
											cpFlags |= (CP_AVOID_ENEMY | CP_CLEAR | CP_AVOID);
											avoidDist = 256;
										}
									}
		...
		if ( !cpFlags ) {//okay, we have no new enemy-driven reason to run... let's use tactics now
			if ( runner && NPCInfo->combatPoint != -1 ) {//someone is running and we have a combat point already
				if ( NPCInfo->squadState != SQUAD_SCOUT &&
					...
					NPCInfo->squadState != SQUAD_RETREAT ) {//it's not us
					if ( TIMER_Done( NPC, "verifyCP" ) && DistanceSquared( &NPC->r.currentOrigin, &level.combatPoints[NPCInfo->combatPoint].origin ) > 64 * 64 ) {//1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason, you've stopped running...
					...
					if ( NPCInfo->aiFlags & NPCAI_BLOCKED ) {//dammit, something is in our way
						...
						for ( j = 0; j < group->numGroup; j++ ) {
							if ( group->member[j].number == NPCInfo->blockingEntNum ) {//we're being blocked by one of our own, pass our goal onto them and I'll stand still
				...
				if ( NPCInfo->combatPoint != -1 ) {//we have a combat point we're supposed to be running to
					if ( NPCInfo->squadState != SQUAD_SCOUT &&
						...
						if ( TIMER_Done( NPC, "verifyCP" ) ) {//1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason, you've stopped running...
							if ( DistanceSquared( &NPC->r.currentOrigin, &level.combatPoints[NPCInfo->combatPoint].origin ) > 64 * 64 ) {//uh, WTF, we're not on our combat point?
								//er, try again, I guess?
								cp = NPCInfo->combatPoint;
								cpFlags |= ST_GetCPFlags();
							}
		...
		if ( cpFlags ) {//we want to run to a combat point
			...
			*/
			...
			if ( group->enemy->client->ps.weapon == WP_SABER && /*group->enemy->client->ps.SaberLength() > 0*/!group->enemy->client->ps.saberHolstered ) {//we obviously want to avoid the enemy if he has a saber
...
void NPC_BSST_Attack( void ) {
	...
	else if ( TIMER_Done( NPC, "flee" ) && NPC_CheckForDanger( NPC_CheckAlertEvents( qtrue, qtrue, -1, qfalse, AEL_DANGER ) ) ) {//not already fleeing, and going to run
		ST_Speech( NPC, SPEECH_COVER, 0 );
		NPC_UpdateAngles( qtrue, qtrue );
		return;
	}
