...
static bool WriteToTemporaryFile(const void *data, size_t dataLength, char **tempFilePath, const char *extension) {
    ...
    if (tempPathResult) {
        ...
        if (tempFileNameResult) {
            HANDLE file = CreateFile(tempFileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
            if (file != INVALID_HANDLE_VALUE) {
                ...
                if (WriteFile(file, data, dataLength, &bytesWritten, NULL)) {
                    ...
                    while (deletesRemaining > 0 && fs_temporaryFileNames[fs_temporaryFileWriteIdx][0] != '\0') {
                        ...
                        fs_temporaryFileWriteIdx = (fs_temporaryFileWriteIdx + 1) % ARRAY_LEN(fs_temporaryFileNames);
                        deletesRemaining--;
                    }
                    ...
                    Q_strncpyz(fs_temporaryFileNames[fs_temporaryFileWriteIdx], tempFileName, sizeof(fs_temporaryFileNames[0]));
                    fs_temporaryFileWriteIdx = (fs_temporaryFileWriteIdx + 1) % ARRAY_LEN(fs_temporaryFileNames);
