...
void BG_ForcePowerDrain( playerState_t *ps, forcePowers_t forcePower, int overrideAmt ) {
	...
	*/
	...
	if ( forcePower == FP_LEVITATION ) { //special case
		...
		if ( ps->velocity.z > 250 )		jumpDrain = 20;
		else if ( ps->velocity.z > 200 )		jumpDrain = 16;
		else if ( ps->velocity.z > 150 )		jumpDrain = 12;
		else if ( ps->velocity.z > 100 )		jumpDrain = 8;
		else if ( ps->velocity.z > 50 )		jumpDrain = 6;
		else if ( ps->velocity.z > 0 )		jumpDrain = 4;
...
saberMoveName_t PM_CheckStabDown( void ) {
	...
	if ( ent &&
		...
		BG_InKnockDown( ent->s.legsAnim ) ) {//guy is on the ground below me, do a top-down attack
...
int PM_SaberLockResultAnim( playerState_t *duelist, qboolean superBreak, qboolean won ) {
	...
	if ( superBreak
		&& !won ) {//if you lose a superbreak, you're defenseless
		/*
...
static qboolean PM_CheckEnemyPresence( int dir, float radius ) { //anyone in this dir?
	...
	if ( tr.fraction != 1.0f && tr.entityNum < ENTITYNUM_WORLD ) { //let's see who we hit
		...
		if ( bgEnt &&
			(bgEnt->s.eType == ET_PLAYER || bgEnt->s.eType == ET_NPC) ) { //this guy can be considered an "enemy"... if he is on the same team, oh well. can't bg-check that (without a whole lot of hassle).
			return qtrue;
		}
	}
...
static saberMoveName_t PM_SaberAttackForMovement( saberMoveName_t curmove ) {
	...
	if ( pm->cmd.rightmove > 0 ) {//moving right
		if ( !noSpecials
			&& overrideJumpRightAttackMove != LS_NONE
			&& pm->ps->velocity.z > 20.0f //pm->ps->groundEntityNum != ENTITYNUM_NONE//on ground
		...
		if ( pm->cmd.forwardmove > 0 ) {//forward= T2B slash
			...
			if ( !PM_ButterflyDisabled() &&
				...
				BG_EnoughForcePowerForMove( SABER_ALT_ATTACK_POWER_FB ) ) { //DUAL/STAFF JUMP ATTACK
			...
			// blue lunge
				...
				pm->ps->weaponTime <= 0 &&
				!BG_SaberInSpecialAttack( pm->ps->torsoAnim ) &&
				BG_EnoughForcePowerForMove( SABER_ALT_ATTACK_POWER_FB ) ) { //LUNGE (weak)
	...
	if ( pm->ps->fd.saberAnimLevel == SS_DUAL ) {
		if ( (newmove == LS_A_R2L || newmove == LS_S_R2L
			...
			&& PM_CheckEnemyPresence( DIR_LEFT, 100.0f ) ) {//enemy both on left and right
			newmove = LS_DUAL_LR;
			...
			&& PM_CheckEnemyPresence( DIR_BACK, 100.0f ) ) {//enemy both in front and back
			newmove = LS_DUAL_FB;
			//probably already moved, but...
...
qboolean PM_CanDoKata( void ) {
	...
	{//FIXME: check rage, etc...
		saberInfo_t *saber = BG_MySaber( pm->ps->clientNum, 0 );
		if ( saber
			&& saber->kataMove == LS_NONE ) {//kata move has been overridden in a way that should stop you from doing it at all
			return qfalse;
		}
		saber = BG_MySaber( pm->ps->clientNum, 1 );
		if ( saber
	...
	*/
	...
	if ( pm->ps->saberBlocked ) {
		if ( pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT
			&& pm->ps->saberBlocked < BLOCKED_UPPER_RIGHT_PROJ ) {//hold the parry for a bit
			...
			if ( pm->ps->saberMove >= LS_T1_BR__R ) {//an actual bounce?  Other bounces before this are actually transitions?
				...
				if ( PM_SaberInBounce( pm->ps->saberMove ) || !BG_SaberInAttack( pm->ps->saberMove ) ) {
					if ( pm->cmd.buttons & BUTTON_ATTACK ) {//transition to a new attack
						...
						while ( newQuad == saberMoveData[pm->ps->saberMove].startQuad ) {//player is still in same attack quad, don't repeat that attack because it looks bad,
...
weapChecks:
	...
	if ( pm->ps->weaponstate == WEAPON_RAISING ) {//Just selected the weapon
		...
		if ( pm->ps->legsAnim == BOTH_WALK1 )		PM_SetAnim( SETANIM_TORSO, BOTH_WALK1, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_RUN1 )		PM_SetAnim( SETANIM_TORSO, BOTH_RUN1, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_RUN2 )		PM_SetAnim( SETANIM_TORSO, BOTH_RUN2, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_RUN_STAFF )	PM_SetAnim( SETANIM_TORSO, BOTH_RUN_STAFF, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_RUN_DUAL )	PM_SetAnim( SETANIM_TORSO, BOTH_RUN_DUAL, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_WALK1 )		PM_SetAnim( SETANIM_TORSO, BOTH_WALK1, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_WALK2 )		PM_SetAnim( SETANIM_TORSO, BOTH_WALK2, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_WALK_STAFF )	PM_SetAnim( SETANIM_TORSO, BOTH_WALK_STAFF, SETANIM_FLAG_NORMAL, 100 );
		else if ( pm->ps->legsAnim == BOTH_WALK_DUAL )	PM_SetAnim( SETANIM_TORSO, BOTH_WALK_DUAL, SETANIM_FLAG_NORMAL, 100 );
		else											PM_SetAnim( SETANIM_TORSO, PM_GetSaberStance(), SETANIM_FLAG_NORMAL, 100 );
	...
	// *********************************************************
	if ( pm->ps->fd.saberAnimLevel == SS_STAFF &&
		(pm->cmd.buttons & BUTTON_ALT_ATTACK) ) { //ok, try a kick I guess.
		int kickMove = -1;
	...
	if ( !delayed_fire ) {
		...
		if ( curmove == LS_A_JUMP_T__B_
			|| curmove == LS_JAPLUS_NEWANIM_FLIPSABERSTAB
		...
		if ( pm->ps->weaponTime > 0 ) {	// Last attack is not yet complete.
			...
			if ( pm->ps->torsoAnim == BOTH_FORCELONGLEAP_ATTACK
				|| pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND ) {//can't attack in these anims
				return;
			}
			...
			if ( anim == -1 ) {//not side-stepping, pick neutral anim
				...
				if ( !pm->cmd.forwardmove && !pm->cmd.rightmove && pm->cmd.upmove >= 0 && pm->ps->groundEntityNum != ENTITYNUM_NONE ) {//not moving at all, so set the anim on entire body
					both = qtrue;
				}
...
void PM_SetSaberMove( short newMove ) {
	...
	else if ( pm->ps->fd.saberAnimLevel == SS_STAFF && newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL ) {//staff has an entirely new set of anims, besides special attacks
	...
	else if ( pm->ps->fd.saberAnimLevel == SS_DUAL && newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL ) { //akimbo has an entirely new set of anims, besides special attacks
		//FIXME: include ready and draw/putaway?
		//FIXME: get hand-made bounces and deflections?
		if ( newMove >= LS_V1_BR && newMove <= LS_REFLECT_LL ) {//there aren't 1-7, just 1, 6 and 7, so just set it
	...
	*/
		...
		!BG_SaberInIdle( newMove ) && !PM_SaberInParry( newMove ) && !PM_SaberInKnockaway( newMove ) && !PM_SaberInBrokenParry( newMove ) && !PM_SaberInReflect( newMove ) && !BG_SaberInSpecial( newMove ) ) {//readies, parries and reflections have only 1 level
		anim += (pm->ps->fd.saberAnimLevel - FORCE_LEVEL_1) * SABER_ANIM_GROUP_SIZE;
	}
	...
	if ( BG_InSaberStandAnim( anim ) || anim == BOTH_STAND1 ) {
		...
		if ( anim == BOTH_WALKBACK1 || anim == BOTH_WALKBACK2 || anim == BOTH_WALK1 ) { //normal stance when walking backward so saber doesn't look like it's cutting through leg
			anim = PM_GetSaberStance();
		}
	...
	if ( !pm->ps->m_iVehicleNum ) { //if not riding a vehicle
		if ( newMove == LS_JUMPATTACK_ARIAL_RIGHT ||
			newMove == LS_JUMPATTACK_ARIAL_LEFT ) { //force only on legs
			parts = SETANIM_LEGS;
		}
			...
			|| BG_KickMove( newMove )
			|| newMove == LS_JAPLUS_NEWANIM_FLIPSABERSTAB )
		{
			parts = SETANIM_BOTH;
		}
	...
	if ( (pm->ps->torsoAnim) == anim ) {//successfully changed anims
		...
		if ( BG_SaberInSpecial( newMove ) &&
			pm->ps->weaponTime < pm->ps->torsoTimer ) { //rww 01-02-03 - I think this will solve the issue of special attacks being interruptable, hopefully without side effects
			pm->ps->weaponTime = pm->ps->torsoTimer;
		}
