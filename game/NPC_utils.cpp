...
void CalcEntitySpot(const gentity_t *ent, const spot_e spot, vector3 *point) {
    ...
    switch (spot) {
    case SPOT_ORIGIN:
        if (VectorCompare(&ent->r.currentOrigin, &vec3_origin)) {  // brush
        ...
        if (ent->client &&
            VectorLengthSquared(
                &ent->client->renderInfo
                     .eyePoint) /*&& (ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >= ENTITYNUM_WORLD)*/) { // Actual tag_head eyespot!
            // FIXME: Stasis aliens may have a problem here...
            VectorCopy(&ent->client->renderInfo.eyePoint, point);
            if (ent->client->NPC_class == CLASS_ATST) { // adjust up some
                point->z += 28;                         // magic number :)
            }
            if (ent->NPC) { // always aim from the center of my bbox, so we don't wiggle when we lean forward or backwards
...
qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw) {
    ...
    // aimTime isn't even set anymore... so this code was never reached, but I need a way to lock NPC's yaw, so instead of making a new SCF_ flag, just use the
    // existing render flag... - dmv
    ...
    if (exact && trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_ANGLE_FACE))
        trap->ICARUS_TaskIDComplete((sharedEntity_t *)NPC, TID_ANGLE_FACE);
    return exact;
}
...
void NPC_AimWiggle(vector3 *enemy_org) {
    ...
    if (NPCInfo->aimErrorDebounceTime < level.time) {
        NPCInfo->aimOfs.x = 0.3f * flrand(NPC->enemy->r.mins.x, NPC->enemy->r.maxs.x);
        NPCInfo->aimOfs.y = 0.3f * flrand(NPC->enemy->r.mins.y, NPC->enemy->r.maxs.y);
...
void NPC_SetSurfaceOnOff(gentity_t *ent, const char *surfaceName, uint32_t surfaceFlags) {
    ...
    while (i < BG_NUM_TOGGLEABLE_SURFACES && bgToggleableSurfaces[i]) {
        if (!Q_stricmp(surfaceName, bgToggleableSurfaces[i])) { // got it
            foundIt = qtrue;
            break;
        }
        i++;
...
qboolean NPC_SomeoneLookingAtMe(gentity_t *ent) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (pEnt && pEnt->inuse && pEnt->client && pEnt->client->sess.sessionTeam != TEAM_SPECTATOR && pEnt->client->tempSpectate < level.time &&
            !(pEnt->client->ps.pm_flags & PMF_FOLLOW) && pEnt->s.weapon != WP_NONE) {
            if (trap->InPVS(&ent->r.currentOrigin, &pEnt->r.currentOrigin)) {
                if (InFOV(ent, pEnt, 30, 30)) { // I'm in a 30 fov or so cone from this player.. that's enough I guess.
                    return qtrue;
                }
            }
        }
...
qboolean NPC_ClearLOS(const vector3 *start, const vector3 *end) { return G_ClearLOS(NPC, start, end); }
qboolean NPC_ClearLOS5(const vector3 *end) { return G_ClearLOS5(NPC, end); }
qboolean NPC_ClearLOS4(gentity_t *ent) { return G_ClearLOS4(NPC, ent); }
qboolean NPC_ClearLOS3(const vector3 *start, gentity_t *ent) { return G_ClearLOS3(NPC, start, ent); }
qboolean NPC_ClearLOS2(gentity_t *ent, const vector3 *end) { return G_ClearLOS2(NPC, ent, end); }
...
qboolean NPC_ValidEnemy(gentity_t *ent) {
    ...
    if (ent->client == NULL) {
        ...
        if (ent->s.eType != ET_NPC) { // still potentially valid
    ...
    } else if (ent->client && ent->client->sess.sessionTeam == TEAM_SPECTATOR) { // don't go after spectators
        return qfalse;
    } else if (ent->client && ent->client->tempSpectate >= level.time) { // don't go after spectators
        return qfalse;
    }
    ...
    if (entTeam == NPC->client->enemyTeam // simplest case: they're on my enemy team
        || (NPC->client->enemyTeam == NPCTEAM_FREE &&
            ent->client->NPC_class != NPC->client->NPC_class)     // I get mad at anyone and this guy isn't the same class as me
        ...
        || (entTeam == NPCTEAM_FREE && ent->client->enemyTeam == NPCTEAM_FREE && ent->enemy && ent->enemy->client &&
            (ent->enemy->client->playerTeam == NPC->client->playerTeam ||
             (ent->enemy->client->playerTeam != NPCTEAM_ENEMY &&
              NPC->client->playerTeam == NPCTEAM_PLAYER))) // enemy is a rampaging non-aligned creature who is attacking someone on our team or a non-enemy
                                                           // (this last condition is used only if we're a good guy - in effect, we protect the innocent)
    ) {
        return qtrue;
    }
...
qboolean NPC_FindEnemy(qboolean checkAlerts) {
    ...
    if (NPC->client->NPC_class != CLASS_RANCOR &&
        ...
        && NPC_CheckPlayerDistance()) { // rancors, wampas & sand creatures don't care if player is closer, they always go with closest
        return qtrue;
    }
    ...
    if (self->client) {
        if (self->client->renderInfo.lookTarget >= 0 && self->client->renderInfo.lookTarget < ENTITYNUM_WORLD) { // within valid range
            if (!g_entities[self->client->renderInfo.lookTarget].inuse) { // lookTarget not inuse or not valid anymore
                ...
                NPC_ClearLookTarget(self);
            } else if (g_entities[self->client->renderInfo.lookTarget].client && self->enemy &&
                       (&g_entities[self->client->renderInfo.lookTarget] != self->enemy)) { // should always look at current enemy if engaged in battle...
                                                                                            // FIXME: this could override certain scripted lookTargets...???
                NPC_ClearLookTarget(self);
            } else {
                return qtrue;
            }
