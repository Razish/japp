...
static void ST_Speech(gentity_t *self, speechType_e speechType, float failChance) {
    ...
    if (failChance >= 0) {      // a negative failChance makes it always talk
        if (self->NPC->group) { // group AI speech debounce timer
            ...
            */
        } else if (!TIMER_Done(self, "chatter")) { // personal timer
            return;
        } else if (groupSpeechDebounceTime[self->client->playerTeam] > level.time) { // for those not in group AI
            // FIXME: let certain speech types interrupt others?  Let closer NPCs interrupt farther away ones?
            return;
        }
    }

    if (self->NPC->group) { // So they don't all speak at once...
...
static void ST_HoldPosition(void) {
    ...
    if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC,
                                    TID_MOVE_NAV)) { // don't have a script waiting for me to get to my point, okay to stop trying and stand
...
void NPC_ST_SayMovementSpeech(void) {
    ...
    if (NPCInfo->group && NPCInfo->group->commander && NPCInfo->group->commander->client && NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL &&
        !Q_irand(0, 3)) { // imperial (commander) gives the order
        ST_Speech(NPCInfo->group->commander, NPCInfo->movementSpeech, NPCInfo->movementSpeechChance);
    } else { // really don't want to say this unless we can actually get there...
...
static qboolean ST_Move(void) {
    ...
    if (moved == qfalse) { // FIXME: if we're going to a combat point, need to pick a different one
        if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC,
                                        TID_MOVE_NAV)) { // can't transfer movegoal or stop when a script we're running is waiting to complete
            if (info.blocker && info.blocker->NPC && NPCInfo->group != NULL &&
                info.blocker->NPC->group == NPCInfo->group) //(NPCInfo->aiFlags&NPCAI_BLOCKED) && NPCInfo->group != NULL )
            {                                               // dammit, something is in our way
                // see if it's one of ours
                int j;

                for (j = 0; j < NPCInfo->group->numGroup; j++) {
                    if (NPCInfo->group->member[j].number ==
                        NPCInfo->blockingEntNum) { // we're being blocked by one of our own, pass our goal onto them and I'll stand still
...
void NPC_BSST_Sleep(void) {
    ...
    if (alertEvent >= 0) {
        ...
        if (level.alertEvents[alertEvent].level == AEL_DISCOVERED && (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
            ...
            for (i = 0; i < MAX_CLIENTS; i++) { // Raz: Now we care about all clients >_>
                ent = &g_entities[i];
                if (ent->inuse && ent->health > 0 && !(ent->client->ps.eFlags & EF_DEAD) && G_ClearLOS(NPC, &NPC->s.origin, &ent->s.origin)) {
...
qboolean NPC_CheckEnemyStealth(gentity_t *target) {
    ...
    if (target->client->ps.weapon == WP_SABER && !target->client->ps.saberHolstered &&
        !target->client->ps.saberInFlight) { // if target has saber in hand and activated, we wake up even sooner even if not facing him
        minDist = 100;
    }
    ...
    if (!(target->client->ps.pm_flags & PMF_DUCKED) && (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) && (target_dist) < (minDist * minDist)) {
    ...
    if (NPCInfo->stats.visrange > maxViewDist) { // FIXME: should we always just set maxViewDist to this?
    ...
    if (clearLOS) {
        ...
        float dist_rating, speed_rating, turning_rating, vis_rating, target_rating;
        ...
        VectorSet(&targ_org, target->r.currentOrigin.x, target->r.currentOrigin.y, target->r.currentOrigin.z + target->r.maxs.z - 4);
        ...
        if (contents & CONTENTS_WATER) {
            int myContents = trap->PointContents(&NPC->client->renderInfo.eyePoint, NPC->s.number);
            if (!(myContents & CONTENTS_WATER)) {                   // I'm not in water
        ...
        if (target_rating > cautious && !(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) { // FIXME: ambushing guys should never talk
            if (TIMER_Done(NPC, "enemyLastVisible")) {                                 // If we haven't already, start the counter
...
static qboolean NPC_ST_InvestigateEvent(int eventID, qboolean extraSuspicious) {
    ...
    if (NPCInfo->confusionTime < level.time) {
        if (level.alertEvents[eventID].level == AEL_DISCOVERED && (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
            ...
            if (!level.alertEvents[eventID].owner || !level.alertEvents[eventID].owner->client || level.alertEvents[eventID].owner->health <= 0 ||
                level.alertEvents[eventID].owner->client->playerTeam != NPC->client->enemyTeam) { // not an enemy
                return qfalse;
            }
            // FIXME: what if can't actually see enemy, don't know where he is... should we make them just become very alert and start looking for him?  Or just
            // let combat AI handle this... (act as if you lost him) ST_Speech( NPC, SPEECH_CHARGE, 0 );
            G_SetEnemy(NPC, level.alertEvents[eventID].owner);
            NPCInfo->enemyLastSeenTime = level.time;
            TIMER_Set(NPC, "attackDelay", Q_irand(500, 2500));
            if (level.alertEvents[eventID].type == AET_SOUND) { // heard him, didn't see him, stick for a bit
    ...
    */
    ...
    if (level.alertEvents[eventID].type == AET_SIGHT) {                                                   // sight alert, check the light level
        if (level.alertEvents[eventID].light < Q_irand(ST_MIN_LIGHT_THRESHOLD, ST_MAX_LIGHT_THRESHOLD)) { // below my threshhold of potentially seeing
            return qfalse;
        }
    }
    ...
    if (level.alertEvents[eventID].level > AEL_MINOR && NPCInfo->investigateCount > 1 && (NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) {
        ...
        if (G_ExpandPointToBBox(
                &NPCInfo->investigateGoal, &NPC->r.mins, &NPC->r.maxs, NPC->s.number,
                ((NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP))) { // we were able to move the investigateGoal to a point in which our bbox would fit
            // drop the goal to the ground so we can get at it
            vector3 end;
            trace_t trace;
            VectorCopy(&NPCInfo->investigateGoal, &end);
            end.z -= 512; // FIXME: not always right?  What if it's even higher, somehow?
            trap->Trace(&trace, &NPCInfo->investigateGoal, &NPC->r.mins, &NPC->r.maxs, &end, ENTITYNUM_NONE,
                        ((NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP), qfalse, 0, 0);
            if (trace.fraction >= 1.0f) { // too high to even bother
                // FIXME: look at them???
            } else {
        ...
        if (NPCInfo->investigateDebounceTime + NPCInfo->pauseTime > level.time) { // was already investigating
            if (NPCInfo->group && NPCInfo->group->commander && NPCInfo->group->commander->client &&
                NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL && !Q_irand(0, 3)) {
...
void NPC_BSST_Investigate(void) {
    ...
    if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
        ...
        if (alertEvent >= 0) {
            if (NPCInfo->confusionTime < level.time) {
                if (NPC_CheckForDanger(alertEvent)) { // running like hell
...
void NPC_BSST_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
    ...
    if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
        ...
        // There is an event to look at
        if (alertEvent >= 0) {
            if (NPC_ST_InvestigateEvent(alertEvent, qfalse)) { // actually going to investigate it
                NPC_UpdateAngles(qtrue, qtrue);
                return;
            }
        }
    ...
    {
        if (NPC->client->NPC_class != CLASS_IMPERIAL && NPC->client->NPC_class != CLASS_IMPWORKER) { // imperials do not look around
    ...
    if (NPC->client->NPC_class == CLASS_IMPERIAL || NPC->client->NPC_class == CLASS_IMPWORKER) { // hack
        if (ucmd.forwardmove || ucmd.rightmove || ucmd.upmove) {                                 // moving

            if ((NPC->client->ps.torsoTimer <= 0) || (NPC->client->ps.torsoAnim == BOTH_STAND4)) {
                if ((ucmd.buttons & BUTTON_WALKING) && !(NPCInfo->scriptFlags & SCF_RUNNING)) { // not running, only set upper anim
        ...
        } else { // standing still, set both torso and legs anim
            //  No longer overrides scripted anims
            if ((NPC->client->ps.torsoTimer <= 0 || (NPC->client->ps.torsoAnim == BOTH_STAND4)) &&
...
static void ST_CheckMoveState(void) {
    ...
    // See if we're running away
        ...
        if (NPCInfo->goalEntity) {
            faceEnemy = qfalse;
        } else { // um, lost our goal?  Just stand and shoot, then
    ...
    // see if we're heading to some other combatPoint
        ...
        // ucmd.buttons |= BUTTON_CAREFUL;
        if (!NPCInfo->goalEntity) { // um, lost our goal?  Just stand and shoot, then
            NPCInfo->squadState = SQUAD_STAND_AND_SHOOT;
        }
    }
    ...
    else if (NPCInfo->squadState == SQUAD_STAND_AND_SHOOT) { // from this squadState we can transition to others?
        move = qfalse;
        return;
    }
    // see if we're hiding
    ...
    if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
        ...
        if (NAV_HitNavGoal(&NPC->r.currentOrigin, &NPC->r.mins, &NPC->r.maxs, &NPCInfo->goalEntity->r.currentOrigin, 16, FlyingCreature(NPC)) ||
            (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV) && NPCInfo->squadState == SQUAD_SCOUT && enemyLOS &&
             enemyDist <= 10000)) { // either hit our navgoal or our navgoal was not a crucial (scripted) one (maybe a combat point) and we're scouting and
                                    // found our enemy
...
void ST_ResolveBlockedShot(int hit) {
    ...
    if (TIMER_Done(NPC, "duck")) { // we're not ducking
        if (AI_GroupContainsEntNum(NPCInfo->group, hit)) {
            gentity_t *member = &g_entities[hit];
            if (TIMER_Done(member, "duck")) {      // they aren't ducking
                if (TIMER_Done(member, "stand")) { // they're not being forced to stand
    ...
    } else {                            // maybe we should stand
        if (TIMER_Done(NPC, "stand")) { // stand for as long as we'll be here
...
static void ST_CheckFireState(void) {
    ...
    if (NPCInfo->squadState == SQUAD_RETREAT || NPCInfo->squadState == SQUAD_TRANSITION ||
        NPCInfo->squadState == SQUAD_SCOUT) { // runners never try to fire at the last pos
        return;
    }

    if (!VectorCompare(&NPC->client->ps.velocity, &vec3_origin)) { // if moving at all, don't do this
        return;
    }
    ...
    if (!hitAlly                          // we're not going to hit an ally
        ...
        && (NPCInfo->group->numState[SQUAD_RETREAT] > 0 || NPCInfo->group->numState[SQUAD_TRANSITION] > 0 ||
            NPCInfo->group->numState[SQUAD_SCOUT] > 0)) // laying down covering fire
    {
        if (level.time - NPCInfo->enemyLastSeenTime < 10000 && // we have seem the enemy in the last 10 seconds
            (!NPCInfo->group ||
             level.time - NPCInfo->group->lastSeenEnemyTime < 10000)) // we are not in a group or the group has seen the enemy in the last 10 seconds
        {
            if (!Q_irand(0, 10)) {
...
void ST_TrackEnemy(gentity_t *self, vector3 *enemyPos) {
    // clear timers
...
void ST_TransferMoveGoal(gentity_t *self, gentity_t *other) {
    if (trap->ICARUS_TaskIDPending((sharedEntity_t *)self, TID_MOVE_NAV)) { // can't transfer movegoal when a script we're running is waiting to complete
        return;
    }
    if (self->NPC->combatPoint != -1) { // I've got a combatPoint I'm going to, give it to him
        self->NPC->lastFailedCombatPoint = other->NPC->combatPoint = self->NPC->combatPoint;
        self->NPC->combatPoint = -1;
    } else { // I must be going for a goal, give that to him instead
        if (self->NPC->goalEntity == self->NPC->tempGoal) {
            NPC_SetMoveGoal(other, &self->NPC->tempGoal->r.currentOrigin, self->NPC->goalRadius, ((self->NPC->tempGoal->flags & FL_NAVGOAL) ? qtrue : qfalse),
                            -1, NULL);
        } else {
...
uint32_t ST_GetCPFlags(void) {
    ...
    if (NPC && NPCInfo->group) {
        if (NPC == NPCInfo->group->commander && NPC->client->NPC_class == CLASS_IMPERIAL) {  // imperials hang back and give orders
            if (NPCInfo->group->numGroup > 1 && Q_irand(-3, NPCInfo->group->numGroup) > 1) { // FIXME: make sure he;s giving orders with these lines
                if (Q_irand(0, 1)) {
        ...
        } else if (NPCInfo->group->morale < 0) { // hide
            cpFlags = (CP_COVER | CP_AVOID | CP_SAFE | CP_RETREAT);
        } else if (NPCInfo->group->morale < NPCInfo->group->numGroup) { // morale is low for our size
            int moraleDrop = NPCInfo->group->numGroup - NPCInfo->group->morale;
            if (moraleDrop < -6) { // flee (no clear shot needed)
                cpFlags = (CP_FLEE | CP_RETREAT | CP_COVER | CP_AVOID | CP_SAFE);
            } else if (moraleDrop < -3) { // retreat (no clear shot needed)
                cpFlags = (CP_RETREAT | CP_COVER | CP_AVOID | CP_SAFE);
            } else if (moraleDrop < 0) { // cover (no clear shot needed)
                cpFlags = (CP_COVER | CP_AVOID | CP_SAFE);
            }
        } else {
            int moraleBoost = NPCInfo->group->morale - NPCInfo->group->numGroup;
            if (moraleBoost > 20) { // charge to any one and outflank (no cover needed)
                cpFlags = (CP_CLEAR | CP_FLANK | CP_APPROACH_ENEMY);
            } else if (moraleBoost > 15) { // charge to closest one (no cover needed)
                cpFlags = (CP_CLEAR | CP_CLOSEST | CP_APPROACH_ENEMY);
            } else if (moraleBoost > 10) { // charge closer (no cover needed)
...
void ST_Commander(void) {
    ...
    if (group->enemy == NULL || group->enemy->client == NULL) { // hmm, no enemy...?!
        return;
    }
    ...
    if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
        ...
        for (i = 0; i < group->numGroup; i++) {
            ...
            if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go, don't break from that
                continue;
            }
            if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not allowed to move on my own
                continue;
            }
            ...
            } else if (group->enemy->waypoint == WAYPOINT_NONE || (trap->Nav_GetPathCost(NPC->waypoint, group->enemy->waypoint) >= Q3_INFINITE)) {
    ...
    */
    ...
    if (group->numState[SQUAD_SCOUT] > 0 || group->numState[SQUAD_TRANSITION] > 0 || group->numState[SQUAD_RETREAT] > 0) { // someone is running
        runner = qtrue;
    }

    if (/*!runner &&*/ group->lastSeenEnemyTime > level.time - 32000 &&
        group->lastSeenEnemyTime < level.time - 30000) { // no-one has seen the enemy for 30 seconds// and no-one is running after him
    ...
    if (group->lastSeenEnemyTime < level.time - 10000) { // no-one has seen the enemy for at least 10 seconds!  Should send a scout
        enemyLost = qtrue;
    }

    if (group->lastClearShotTime < level.time - 5000) { // no-one has had a clear shot for 5 seconds!
    ...
    for (i = curMemberNum; i < lastMemberNum; i++) {
        ...
        // get the next guy
        member = &g_entities[group->member[i].number];
        if (!member->enemy) { // don't include guys that aren't angry
            continue;
        }
        SetNPCGlobals(member);

        if (!TIMER_Done(NPC, "flee")) { // running away
            continue;
        }

        if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go
            continue;
        }

        if (NPC->s.weapon == WP_NONE && NPCInfo->goalEntity && NPCInfo->goalEntity == NPCInfo->tempGoal && NPCInfo->goalEntity->enemy &&
            NPCInfo->goalEntity->enemy->s.eType == ET_ITEM) { // running to pick up a gun, don't do other logic
            continue;
        }
        ...
        if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not allowed to do combat-movement
            continue;
        }

        // check the local state
        if (NPCInfo->squadState != SQUAD_RETREAT) {  // not already retreating
            if (NPC->client->ps.weapon == WP_NONE) { // weaponless, should be hiding
                if (NPCInfo->goalEntity == NULL || NPCInfo->goalEntity->enemy == NULL ||
                    NPCInfo->goalEntity->enemy->s.eType != ET_ITEM) { // not running after a pickup
                    if (TIMER_Done(NPC, "hideTime") || (DistanceSquared(&group->enemy->r.currentOrigin, &NPC->r.currentOrigin) < 65536 &&
                                                        NPC_ClearLOS4(NPC->enemy))) { // done hiding or enemy near and can see us
            ...
            if (TIMER_Done(NPC, "roamTime") && TIMER_Done(NPC, "hideTime") && NPC->health > 10 &&
                !trap->InPVS(&group->enemy->r.currentOrigin, &NPC->r.currentOrigin)) { // cant even see enemy
                // better go after him
                cpFlags |= (CP_CLEAR | CP_COVER);
            } else if (NPCInfo->localState == LSTATE_UNDERFIRE) { // we've been shot
            ...
            } else {                                                                      // not hit, see if there are other reasons we should run
                if (trap->InPVS(&NPC->r.currentOrigin, &group->enemy->r.currentOrigin)) { // in the same room as enemy
                    if (NPC->client->ps.weapon == WP_ROCKET_LAUNCHER &&
                        ...
                        NPCInfo->squadState != SQUAD_TRANSITION) { // too close for me to fire my weapon and I'm not already on the move
                                ...
                                if (DistanceSquared(&group->enemy->r.currentOrigin, &NPC->r.currentOrigin) < 65536) {
                                    if (TIMER_Done(NPC, "hideTime")) {
                                        if (NPCInfo->squadState !=
                                            SQUAD_TRANSITION) { // not already moving: FIXME: we need to see if where we're going is good now?
                                            cpFlags |= (CP_AVOID_ENEMY | CP_CLEAR | CP_AVOID);
                                            avoidDist = 256;
                                        }
                                    }
        ...
        if (!cpFlags) {                                 // okay, we have no new enemy-driven reason to run... let's use tactics now
            if (runner && NPCInfo->combatPoint != -1) { // someone is running and we have a combat point already
                if (NPCInfo->squadState != SQUAD_SCOUT && NPCInfo->squadState != SQUAD_TRANSITION && NPCInfo->squadState != SQUAD_RETREAT) { // it's not us
                    if (TIMER_Done(NPC, "verifyCP") &&
                        DistanceSquared(&NPC->r.currentOrigin, &level.combatPoints[NPCInfo->combatPoint].origin) >
                            64 * 64) { // 1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason, you've stopped running...
                    ...
                    if (NPCInfo->aiFlags & NPCAI_BLOCKED) { // dammit, something is in our way
                        // see if it's one of ours
                        for (j = 0; j < group->numGroup; j++) {
                            if (group->member[j].number ==
                                NPCInfo->blockingEntNum) { // we're being blocked by one of our own, pass our goal onto them and I'll stand still
            ...
            } else {                              // okay no-one is running, use some tactics
                if (NPCInfo->combatPoint != -1) { // we have a combat point we're supposed to be running to
                    if (NPCInfo->squadState != SQUAD_SCOUT && NPCInfo->squadState != SQUAD_TRANSITION &&
                        NPCInfo->squadState != SQUAD_RETREAT) { // but we're not running
                        if (TIMER_Done(NPC, "verifyCP")) { // 1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason, you've
                                                           // stopped running...
                            if (DistanceSquared(&NPC->r.currentOrigin, &level.combatPoints[NPCInfo->combatPoint].origin) >
                                64 * 64) { // uh, WTF, we're not on our combat point?
                                // er, try again, I guess?
                                cp = NPCInfo->combatPoint;
                                cpFlags |= ST_GetCPFlags();
                            }
                ...
                if (enemyLost) { // if no-one has seen the enemy for a while, send a scout
                    // ask where he went
                    ...
                    runner = qtrue;
                } else if (enemyProtected) { // if no-one has a clear shot at the enemy, someone should go after him
                    // FIXME: if I'm in an area where no safe combat points have a clear shot at me, they don't come after me... they should anyway, though
                    // after some extra hesitation. ALSO: seem to give up when behind an area portal? since no-one else here has done this, I should be the
                    // closest one
                    if (TIMER_Done(NPC, "roamTime") && !Q_irand(0, group->numGroup)) { // only do this if we're ready to move again and we feel like it
                        cpFlags |= ST_ApproachEnemy(NPC);
                        // set me into scout mode
                        AI_GroupUpdateSquadstates(group, NPC, SQUAD_SCOUT);
                    }
                } else { // group can see and has been shooting at the enemy
                    // see if we should do something fancy?

                    {                                     // we're ready to move
                        if (NPCInfo->combatPoint == -1) { // we're not on a combat point
                            if (!Q_irand(0, 2)) {         // we should go for a combat point
                                cpFlags |= ST_GetCPFlags();
                            } else {
                                TIMER_Set(NPC, "stick", Q_irand(2000, 4000));
                                TIMER_Set(NPC, "roamTime", Q_irand(1000, 3000));
                            }
                        } else if (TIMER_Done(NPC, "roamTime")) {                              // we are already on a combat point
                            if (i == 0) {                                                      // we're the closest
                                if ((group->morale - group->numGroup > 0) && !Q_irand(0, 4)) { // try to outflank him
                                    cpFlags |= (CP_CLEAR | CP_COVER | CP_FLANK | CP_APPROACH_ENEMY);
                                } else if ((group->morale - group->numGroup < 0)) { // better move!
                                    cpFlags |= ST_GetCPFlags();
                                } else { // If we're point, then get down
                            ...
                            } else if (i == group->numGroup - 1) {           // farthest from the enemy
                                if ((group->morale - group->numGroup < 0)) { // low morale, just hang here
                                    TIMER_Set(NPC, "roamTime", Q_irand(2000, 5000));
                                    TIMER_Set(NPC, "stick", Q_irand(2000, 5000));
                                } else if ((group->morale - group->numGroup > 0)) { // try to move in on the enemy
                                    cpFlags |= ST_ApproachEnemy(NPC);
                                    // set me into scout mode
                                    AI_GroupUpdateSquadstates(group, NPC, SQUAD_SCOUT);
                                } else { // use normal decision making process
                                    cpFlags |= ST_GetCPFlags();
                                }
                            } else {                                                           // someone in-between
                                if ((group->morale - group->numGroup < 0) || !Q_irand(0, 4)) { // do something
                                    cpFlags |= ST_GetCPFlags();
                                } else {
                                    TIMER_Set(NPC, "stick", Q_irand(2000, 4000));
                    ...
                    if (!cpFlags) { // still not moving
                        ...
                        */
                        ...
                        // toy with ducking
                        if (TIMER_Done(NPC, "duck")) {                                                                           // not ducking
                            if (TIMER_Done(NPC, "stand")) {                                                                      // don't have to keep standing
                                if (NPCInfo->combatPoint == -1 || (level.combatPoints[NPCInfo->combatPoint].flags & CPF_DUCK)) { // okay to duck here
                                    if (!Q_irand(0, 3)) {
        ...
        if (cpFlags) { // we want to run to a combat point
            /*
            ...
            */
            ...
            if (group->enemy->client->ps.weapon == WP_SABER && /*group->enemy->client->ps.SaberLength() > 0*/ !group->enemy->client->ps
                                                                   .saberHolstered) { // we obviously want to avoid the enemy if he has a saber
            ...
            if (cp == -1) { // may have had sone set above
                cp = NPC_FindCombatPoint(&NPC->r.currentOrigin, &NPC->r.currentOrigin, &group->enemy->r.currentOrigin, cpFlags | CP_HAS_ROUTE, avoidDist,
                                         NPCInfo->lastFailedCombatPoint);
            }
            while (cp == -1 && cpFlags != CP_ANY) { // start "OR"ing out certain flags to see if we can find *any* point
                if (cpFlags & CP_INVESTIGATE) {     // don't need to investigate
                    cpFlags &= ~CP_INVESTIGATE;
                } else if (cpFlags & CP_SQUAD) { // don't need to stick to squads
                    cpFlags &= ~CP_SQUAD;
                } else if (cpFlags & CP_DUCK) { // don't need to duck
                    cpFlags &= ~CP_DUCK;
                } else if (cpFlags & CP_NEAREST) { // don't need closest one to me
                    cpFlags &= ~CP_NEAREST;
                } else if (cpFlags & CP_FLANK) { // don't need to flank enemy
                    cpFlags &= ~CP_FLANK;
                } else if (cpFlags & CP_SAFE) { // don't need one that hasn't been shot at recently
                    cpFlags &= ~CP_SAFE;
                } else if (cpFlags & CP_CLOSEST) { // don't need to get closest to enemy
                    cpFlags &= ~CP_CLOSEST;
                    // but let's try to approach at least
                    cpFlags |= CP_APPROACH_ENEMY;
                } else if (cpFlags & CP_APPROACH_ENEMY) { // don't need to approach enemy
                    cpFlags &= ~CP_APPROACH_ENEMY;
                } else if (cpFlags & CP_COVER) { // don't need cover
                    cpFlags &= ~CP_COVER;
                    // but let's pick one that makes us duck
                    cpFlags |= CP_DUCK;
                } else if (cpFlags & CP_CLEAR) { // don't need a clear shot to enemy
                    cpFlags &= ~CP_CLEAR;
                } else if (cpFlags & CP_AVOID_ENEMY) { // don't need to avoid enemy
                    cpFlags &= ~CP_AVOID_ENEMY;
                } else if (cpFlags & CP_RETREAT) { // don't need to retreat
                    cpFlags &= ~CP_RETREAT;
                } else if (cpFlags & CP_FLEE) { // don't need to flee
                    cpFlags &= ~CP_FLEE;
                    // but at least avoid enemy and pick one that gives cover
                    cpFlags |= (CP_COVER | CP_AVOID_ENEMY);
                } else if (cpFlags & CP_AVOID) { // okay, even pick one right by me
                    cpFlags &= ~CP_AVOID;
                } else {
                    cpFlags = CP_ANY;
                }
            ...
            if (cp != -1) { // found a combat point
                ...
                { // we actually can get to it, so okay to say you're going there.
                    ...
                    } else if (cpFlags & CP_FLEE) { // outright running for your life
                        AI_GroupUpdateSquadstates(group, NPC, SQUAD_RETREAT);
                    } else { // any other kind of transition between combat points
                    ...
                    } else { // okay, let's cheat
                        if (group->numGroup > 1) {
                            float dot = 1.0f;
                            if (!Q_irand(0, 3)) { // 25% of the time, see if we're flanking the enemy
                                vector3 eDir2Me, eDir2CP;
                            ...
                            if (dot < 0.4f) { // flanking!
                                NPC_ST_StoreMovementSpeech(SPEECH_OUTFLANK, -1);
                            } else if (!Q_irand(0, 10)) {                      // regular movement
            ...
            } else if (NPCInfo->squadState == SQUAD_SCOUT) { // we couldn't find a combatPoint by the player, so just go after him directly
                ST_HuntEnemy(NPC);
                // set me into scout mode
                AI_GroupUpdateSquadstates(group, NPC, SQUAD_SCOUT);
...
void NPC_BSST_Attack(void) {
    ...
    if (NPCInfo->group) {                 // I belong to a squad of guys - we should *always* have a group
        if (!NPCInfo->group->processed) { // I'm the first ent in my group, I'll make the command decisions
#if AI_TIMERS
    ...
    } else if (TIMER_Done(NPC, "flee") &&
               NPC_CheckForDanger(NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse, AEL_DANGER))) { // not already fleeing, and going to run
        ST_Speech(NPC, SPEECH_COVER, 0);
        NPC_UpdateAngles(qtrue, qtrue);
        return;
    }
                ...
                if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) ||
                    (hitEnt && hitEnt->takedamage &&
                     ((hitEnt->r.svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 ||
                      NPC->s.weapon ==
                          WP_EMPLACED_GUN))) { // can hit enemy or enemy ally or will hit glass or other minor breakable (or in emplaced gun), so shoot anyway
    ...
    if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not supposed to chase my enemies
        if (NPCInfo->goalEntity == NPC->enemy) {       // goal is my entity, so don't move
            move = qfalse;
        }
    }
    ...
    if (!faceEnemy) { // we want to face in the dir we're running
        if (!move) {  // if we haven't moved, we should look in the direction we last looked?
            VectorCopy(&NPC->client->ps.viewangles, &NPCInfo->lastPathAngles);
        }
        NPCInfo->desiredYaw = NPCInfo->lastPathAngles.yaw;
        NPCInfo->desiredPitch = 0;
        NPC_UpdateAngles(qtrue, qtrue);
        if (move) { // don't run away and shoot
            shoot = qfalse;
        }
    }
    ...
    if (NPC->enemy && NPC->enemy->enemy) {
        if (NPC->enemy->s.weapon == WP_SABER &&
            NPC->enemy->enemy->s.weapon == WP_SABER) { // don't shoot at an enemy jedi who is fighting another jedi, for fear of injuring one or causing rogue
                                                       // blaster deflections (a la Obi Wan/Vader duel at end of ANH)
            shoot = qfalse;
        }
    }
    ...
    if (NPC->client->ps.weaponTime > 0) {
        ...
        if (TIMER_Done(NPC, "attackDelay")) {
            ...
            // NASTY
            if (NPC->s.weapon == WP_ROCKET_LAUNCHER && (ucmd.buttons & BUTTON_ATTACK) && !move && g_spSkill.integer > 1 &&
