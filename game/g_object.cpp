...
void G_BounceObject(gentity_t *ent, trace_t *trace) {
    ...
    if (ent->flags & FL_BOUNCE_HALF) {
        ...
        // check for stop
        if (((trace->plane.normal.z > 0.7f && g_gravity.value > 0) || (trace->plane.normal.z < -0.7f && g_gravity.value < 0)) &&
            ((ent->s.pos.trDelta.z < 40 && g_gravity.value > 0) ||
             (ent->s.pos.trDelta.z > -40 && g_gravity.value < 0))) // this can happen even on very slightly sloped walls, so changed it from > 0 to > 0.7f
        {
...
void G_RunObject(gentity_t *ent) {
    ...
    trap->Trace(&tr, &ent->r.currentOrigin, &ent->r.mins, &ent->r.maxs, &origin, ent->parent ? ent->parent->s.number : ent->s.number, ent->clipmask, qfalse, 0,
                0);
