...
static void ProcessMoveCommands( Vehicle_t *pVeh ) {
	...
	if ( parentPS->stats[STAT_HEALTH] <= 0 ) { //don't keep moving while you're dying!
		parentPS->speed = 0;
	}
...
static void ProcessOrientCommands( Vehicle_t *pVeh ) {
	...
	if ( rider->s.number < MAX_CLIENTS ) {//FIXME: use the vehicle's turning stat in this calc
		...
		if ( rider->s.eType == ET_NPC ) {//help NPCs out some
			turnSpeed *= 2.0f;
		...
		if ( pVeh->m_pVehicleInfo->malfunctionArmorLevel && pVeh->m_iArmor <= pVeh->m_pVehicleInfo->malfunctionArmorLevel ) {//damaged badly
		}
	}
...
static void AnimateVehicle( Vehicle_t *pVeh ) {
	...
	if ( fSpeedPercToMax > 0.0f ) //fSpeedPercToMax >= 0.85f )
	{
		...
		if ( fSpeedPercToMax < -0.018f ) {
			...
			iBlend = 600;
			if ( parent->client->ps.m_iVehicleNum ) {//occupado
				Anim = BOTH_STAND1;
			}
			else {//wide open for you, baby
				Anim = BOTH_STAND2;
			}
		}
	}
