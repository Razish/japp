...
static float PM_DuckScale(void) { return GetCInfo(CINFO_VQ3PHYS) ? 0.25f : 0.50f; }
...
float forceJumpStrength[NUM_FORCE_POWER_LEVELS] = {JUMP_VELOCITY, // normal jump
                                                   420, 590, 840};
...
int PM_GetSaberStance(void) {
    ...
    if (saber1 && saber2 && !pm->ps->saberHolstered) { // dual sabers, both on
...
static void PM_SetVehicleAngles(vector3 *normal) {
    ...
    if (pVeh->m_pVehicleInfo->type == VH_FIGHTER) {
        ...
        pitchBias =
            90.0f * pVeh->m_pVehicleInfo->centerOfGravity.raw[0]; // if centerOfGravity is all the way back (-1.0f), vehicle pitches up 90 degrees when in air
    }
...
void BG_VehicleTurnRateForSpeed(Vehicle_t *pVeh, float speed, float *mPitchOverride, float *mYawOverride) {
    if (pVeh && pVeh->m_pVehicleInfo) {
        ...
        if (pVeh->m_pVehicleInfo->speedDependantTurning) {
            if (pVeh->m_LandTrace.fraction >= 1.0f || pVeh->m_LandTrace.plane.normal.z < MIN_LANDING_SLOPE) {
...
void PM_HoverTrace(void) {
    ...
    if (pm->waterlevel && relativeWaterLevel >= 0) {  // in water
        if (pVeh->m_pVehicleInfo->bouyancy <= 0.0f) { // sink like a rock
            ...
            float floatHeight = (pVeh->m_pVehicleInfo->bouyancy * ((pm->maxs.z - pm->mins.z) * 0.5f)) -
        ...
        if (trace->plane.normal.x > 0.5f || trace->plane.normal.x < -0.5f || trace->plane.normal.y > 0.5f ||
            trace->plane.normal.y < -0.5f) { // steep slanted hill, don't go up it.
...
void PM_AddEventWithParm(int newEvent, int parm) { BG_AddPredictableEventToPlayerstate(newEvent, parm, pm->ps); }
...
static void PM_Friction(void) {
    ...
    if (pm_flying != FLY_VEHICLE && pEnt && pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle && pEnt->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL &&
        pEnt->m_pVehicle->m_pVehicleInfo->type != VH_WALKER && pEnt->m_pVehicle->m_pVehicleInfo->friction) {
...
static void PM_Accelerate(vector3 *wishdir, float wishspeed, float accel) {
    if ((pm->gametype != GT_SIEGE && !GetCInfo(CINFO_NOSTRAFEJUMP)) || pm->ps->m_iVehicleNum || pm->ps->clientNum >= MAX_CLIENTS ||
...
qboolean PM_AdjustAngleForWallRun(playerState_t *ps, usercmd_t *ucmd, qboolean doMove) {
    if (((ps->legsAnim) == BOTH_WALL_RUN_RIGHT || (ps->legsAnim) == BOTH_WALL_RUN_LEFT) && ps->legsTimer > 500) { // wall-running and not at end of anim
        ...
        if (trace.fraction < 1.0f && (trace.plane.normal.z >= 0.0f && trace.plane.normal.z <= 0.4f)) //&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
        {
            trace_t trace2;
            vector3 traceTo2;
        ...
        if (trace.fraction < 1.0f && (trace.plane.normal.z >= 0.0f && trace.plane.normal.z <= 0.4f /*MAX_WALL_RUN_Z_NORMAL*/)) { // still a wall there
            if ((ps->legsAnim) == BOTH_WALL_RUN_RIGHT) {
                ucmd->rightmove = 127;
            } else {
                ucmd->rightmove = -127;
...
qboolean PM_AdjustAngleForWallRunUp(playerState_t *ps, usercmd_t *ucmd, qboolean doMove) {
    if (ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START) { // wall-running up
        ...
        if (trace.fraction > 0.5f) { // hmm, some room, see if there's a floor right here
            ...
            if (!trace2.allsolid && !trace2.startsolid && trace2.fraction < 1.0f && trace2.plane.normal.z > 0.7f) // slope we can stand on
        ...
        if ( // ucmd->upmove <= 0 &&
            ps->legsTimer > 0 && ucmd->forwardmove > 0 && trace.fraction < 1.0f &&
...
qboolean PM_AdjustAngleForWallJump(playerState_t *ps, usercmd_t *ucmd, qboolean doMove) {
    ...
    if (((BG_InReboundJump(ps->legsAnim) || BG_InReboundHold(ps->legsAnim)) && (BG_InReboundJump(ps->torsoAnim) || BG_InReboundHold(ps->torsoAnim))) ||
        ...
        pm->trace(&trace, &ps->origin, &mins, &maxs, &traceTo, ps->clientNum, MASK_PLAYERSOLID);
        if (                                                                                                               // ucmd->upmove <= 0 &&
            ps->legsTimer > 100 && trace.fraction < 1.0f && fabsf(trace.plane.normal.z) <= 0.2f /*MAX_WALL_GRAB_SLOPE*/) { // still a vertical wall there
            // FIXME: don't pull around 90 turns
            /*
            if ( ent->s.number || !player_locked )
            {
            ...
            */
            ...
            return qtrue;
        } else if (doMove && (ps->pm_flags & PMF_STUCK_TO_WALL)) { // jump off
            // push off of it!
            ps->pm_flags &= ~PMF_STUCK_TO_WALL;
...
static qboolean PM_CheckJump(void) {
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS) {
        ...
        if (pEnt->s.eType == ET_NPC && pEnt->s.NPC_class == CLASS_VEHICLE) { // no!
            return qfalse;
        }
    }

    if (pm->ps->forceHandExtend == HANDEXTEND_KNOCKDOWN || pm->ps->forceHandExtend == HANDEXTEND_PRETHROWN ||
    ...
    if (pm->ps->fd.forcePowersActive & (1 << FP_LEVITATION)) { // Force jump is already active.. continue draining power appropriately until we land.
        if (pm->ps->fd.forcePowerDebounce[FP_LEVITATION] < pm->cmd.serverTime) {
            if (pm->gametype == GT_DUEL || pm->gametype == GT_POWERDUEL) { // jump takes less power
        ...
        if (pm->ps->gravity > 0) {     // can't do this in zero-G
            if (PM_ForceJumpingUp()) { // holding jump in air
                ...
                {                                                                   // can still go up
                    if (curHeight > forceJumpHeight[0]) {                           // passed normal jump height  *2?
                        if (!(pm->ps->fd.forcePowersActive & (1 << FP_LEVITATION))) // haven't started forcejump yet
                        {
                            ...
                            if ((pm->cmd.forwardmove || pm->cmd.rightmove) && // pushing in a dir
                                ...
                                (pm->ps->legsAnim) != BOTH_FLIP_B && (pm->ps->legsAnim) != BOTH_FLIP_R && (pm->ps->legsAnim) != BOTH_FLIP_L && allowFlips) {
                                int anim = BOTH_FORCEINAIR1;
                                int parts = SETANIM_BOTH;
                    ...
                    pm->ps->velocity.z = (forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]] - curHeight) /
                                         forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]] *
                                         forceJumpStrength[pm->ps->fd.forcePowerLevel[FP_LEVITATION]]; // JUMP_VELOCITY;
                    pm->ps->velocity.z /= 10;
                    pm->ps->velocity.z += JUMP_VELOCITY;
                    pm->ps->pm_flags |= PMF_JUMP_HELD;
                } else if (curHeight > forceJumpHeight[0] && curHeight < forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]] -
                                                                             forceJumpHeight[0]) { // still have some headroom, don't totally stop it
    ...
    if (pm->ps->gravity <= 0) { // in low grav, you push in the dir you're facing as long as there is something behind you to shove off of
        ...
        pm->cmd.upmove = 0;
    } else if (pm->cmd.upmove > 0 && pm->waterlevel < 2 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 && !(pm->ps->pm_flags & PMF_JUMP_HELD)
...
#endif
               && !PM_IsRocketTrooper() && !BG_HasYsalamiri(pm->gametype, pm->ps) && BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION)) {
        ...
        if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
                ...
                if (!doTrace || (trace.fraction < 1.0f && (trace.entityNum < MAX_CLIENTS || DotProduct(&wallNormal, &idealNormal) > 0.7f))) {
                    ...
                    if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START) ||
                        ...
                        if (anim == BOTH_WALL_FLIP_LEFT) {
                            ...
                            pm->ps->velocity.x = pm->ps->velocity.y = 0;
                            VectorMA(&pm->ps->velocity, -150, &right, &pm->ps->velocity);
                        } else if (anim == BOTH_FLIP_BACK1 || anim == BOTH_FLIP_BACK2 || anim == BOTH_FLIP_BACK3 || anim == BOTH_WALL_FLIP_BACK1) {
                            pm->ps->velocity.x = pm->ps->velocity.y = 0;
            ...
            } else if (GetCInfo(CINFO_FLIPKICK) && pm->cmd.forwardmove > 0 // pushing forward
                       && pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 && pm->ps->velocity.z > 200 &&
                       PM_GroundDistance() <= 80 // unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we wanted
                                                 // prediction workign right), so we'll just use the actual ground distance.
            ...
            // NEW JKA
                ...
                if (pm->cmd.upmove != 0) { // jump failed, so don't try to do normal jump code, just return
                    return qfalse;
                       ...
                       PM_WalkableGroundDistance() <= 80 // unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we
                                                         // wanted prediction workign right), so we'll just use the actual ground distance.
            ...
            {                                                                                  // run up wall, flip backwards
                if (allowWallRuns) {
                    ...
                    if (1) // sure, we have it! Because I SAID SO.
                    {
                        ...
                        if (trace.fraction < 1.0f && ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) ||
                                                      DotProduct(&trace.plane.normal, &idealNormal) > 0.7f)) { // there is a wall there
                            pm->ps->velocity.x = pm->ps->velocity.y = 0;
                            if (wallWalkAnim == BOTH_FORCEWALLRUNFLIP_START) {
                                pm->ps->velocity.z = forceJumpStrength[FORCE_LEVEL_3] / 2.0f;
                            } else {
                       ...
                       (pm->ps->origin.z - pm->ps->fd.forceJumpZStart) <
                           (forceJumpHeightMax[FORCE_LEVEL_3] -
                            (BG_ForceWallJumpStrength() / 2.0f)) // can fit at least one more wall jump in (yes, using "magic numbers"... for now)
                       //&& (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_INAIR1 ) )//not in a flip or spin or anything
                ...
                if (allowWallGrabs) {
                    ...
                    if (anim != -1) { // trace in the dir we're pushing in and see if there's a vertical wall there
                        ...
                        VectorNormalize(&idealNormal);
                        traceEnt = PM_BGEntForNum(trace.entityNum);
                        if (trace.fraction < 1.0f && fabsf(trace.plane.normal.z) <= 0.2f /*MAX_WALL_GRAB_SLOPE*/
...
static void PM_WalkMove(void) {
    ...
    } else if ((pm->ps->pm_flags & PMF_ROLLING) && !BG_InRoll(pm->ps, pm->ps->legsAnim) && !PM_InRollComplete(pm->ps, pm->ps->legsAnim)) {
        if (wishspeed > pm->ps->speed * PM_DuckScale()) {
            wishspeed = pm->ps->speed * PM_DuckScale();
        }
    }
...
static int PM_TryRoll(void) {
    ...
    if (BG_SaberInAttack(pm->ps->saberMove) || BG_SaberInSpecialAttack(pm->ps->torsoAnim) || BG_SpinningSaberAnim(pm->ps->legsAnim) ||
    ...
    if (!jpRollAllowed || PM_IsRocketTrooper() || BG_HasYsalamiri(pm->gametype, pm->ps) ||
    ...
    if (pm->ps->weapon == WP_SABER) {
        ...
        if (saber && (saber->saberFlags & SFL_NO_ROLLS)) {
            return 0;
        }
...
static void PM_CrashLand(void) {
    ...
    if (pm->ps->legsAnim == BOTH_A7_KICK_F_AIR || pm->ps->legsAnim == BOTH_A7_KICK_B_AIR || pm->ps->legsAnim == BOTH_A7_KICK_R_AIR ||
    ...
    } else if (pm->ps->legsAnim == BOTH_FORCEJUMPLEFT1 || pm->ps->legsAnim == BOTH_FORCEJUMPRIGHT1 || pm->ps->legsAnim == BOTH_FORCEJUMPBACK1 ||
    ...
    // decide which landing animation to use
             ...
             !pm->ps->m_iVehicleNum) { // only play a land animation if we transitioned into an in-air animation while off the ground
        if (!BG_SaberInSpecial(pm->ps->saberMove)) {
    ...
    if (!BG_InSpecialJump(pm->ps->legsAnim) || pm->ps->legsTimer < 1 || (pm->ps->legsAnim) == BOTH_WALL_RUN_LEFT ||
        ...
        if (!BG_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->inAirAnim) {
            if (!BG_SaberInSpecial(pm->ps->saberMove) || pm->ps->weapon != WP_SABER) {
                if (pm->ps->legsAnim != BOTH_FORCELAND1 && pm->ps->legsAnim != BOTH_FORCELANDBACK1 && pm->ps->legsAnim != BOTH_FORCELANDRIGHT1 &&
                    pm->ps->legsAnim != BOTH_FORCELANDLEFT1) { // don't override if we have started a force land
    ...
    if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
        if (delta > 7) {
            ...
            if (pm->ps->fd.forceJumpZStart) {
                if ((int)pm->ps->origin.z >=
                    (int)pm->ps->fd.forceJumpZStart) { // was force jumping, landed on higher or same level as when force jump was started
                    if (delta_send > 8) {
                        delta_send = 8;
                    }
                } else {
...
static void PM_GroundTraceMissed(void) {
    ...
    if (PM_InRollComplete(pm->ps, pm->ps->legsAnim)) { // Client won't catch an animation restart because it only checks frame against incoming frame, so if you
                                                       // roll when you land after rolling
...
#ifdef PROJECT_GAME
        if (pm->ps->clientNum < MAX_CLIENTS && !pm->ps->m_iVehicleNum && trace.entityNum < ENTITYNUM_WORLD && trace.entityNum >= MAX_CLIENTS &&
            !pm->ps->zoomMode && pm_entSelf) { // check if we landed on a vehicle
            gentity_t *trEnt = &g_entities[trace.entityNum];
            if (trEnt->inuse && trEnt->client && trEnt->s.eType == ET_NPC && trEnt->s.NPC_class == CLASS_VEHICLE && !trEnt->client->ps.m_iVehicleNum &&
                trEnt->m_pVehicle && trEnt->m_pVehicle->m_pVehicleInfo->type != VH_WALKER &&
                trEnt->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER) { // it's a vehicle alright, let's board it.. if it's not an atst or ship
                if (!BG_SaberInSpecial(pm->ps->saberMove) && pm->ps->forceHandExtend == HANDEXTEND_NONE && pm->ps->weaponTime <= 0) {
                    gentity_t *servEnt = (gentity_t *)pm_entSelf;
                    if (level.gametype < GT_TEAM || !trEnt->alliedTeam ||
...
qboolean PM_CheckDualForwardJumpDuck(void) {
    ...
    if (pm->ps->legsAnim == BOTH_JUMPATTACK6) {
        ...
        if ((pm->ps->legsTimer >= 1450 && PM_AnimLength(0, BOTH_JUMPATTACK6) - pm->ps->legsTimer >= 400) ||
            (pm->ps->legsTimer >= 400 &&
            ...
            if (trace.fraction >= 1.0f) {           // all clear
                ...
                if (!trace.allsolid && !trace.startsolid) { // should never start in solid
                    if (trace.fraction >= 1.0f) {           // all clear
                        ...
                        if (pm->ps->legsAnim != BOTH_JUMPATTACK6 || pm->ps->legsTimer <= 200) { // at the end of the anim, and we can't leave ourselves like
                                                                                                // this
...
static void PM_CheckDuck(void) {
    if (pm->ps->m_iVehicleNum > 0 && pm->ps->m_iVehicleNum < ENTITYNUM_NONE) { // riding a vehicle or are a vehicle
        ...
        if (pm_entVeh && pm_entVeh->m_pVehicle &&
            (pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER || pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL)) {
            trace_t solidTr;
        ...
        } else if (pm->cmd.upmove < 0 || pm->ps->forceHandExtend == HANDEXTEND_KNOCKDOWN || pm->ps->forceHandExtend == HANDEXTEND_PRETHROWN ||
...
#define SLOPERECALCVAR pm->ps->slopeRecalcTime // this is purely convenience
    ...
    if ((legsAnim >= LEGS_LEFTUP1 && legsAnim <= LEGS_LEFTUP5) || (legsAnim >= LEGS_S1_LUP1 && legsAnim <= LEGS_S1_LUP5) ||
        (legsAnim >= LEGS_S3_LUP1 && legsAnim <= LEGS_S3_LUP5) || (legsAnim >= LEGS_S4_LUP1 && legsAnim <= LEGS_S4_LUP5) ||
        ...
        destAnim = legsAnim;
    } else if ((legsAnim >= LEGS_RIGHTUP1 && legsAnim <= LEGS_RIGHTUP5) || (legsAnim >= LEGS_S1_RUP1 && legsAnim <= LEGS_S1_RUP5) ||
               (legsAnim >= LEGS_S3_RUP1 && legsAnim <= LEGS_S3_RUP5) || (legsAnim >= LEGS_S4_RUP1 && legsAnim <= LEGS_S4_RUP5) ||
...
static void PM_Footsteps(void) {
    ...
    if ((PM_InSaberAnim((pm->ps->legsAnim)) && !BG_SpinningSaberAnim((pm->ps->legsAnim))) || (pm->ps->legsAnim) == BOTH_STAND1 ||
        (pm->ps->legsAnim) == BOTH_STAND1TO2 || (pm->ps->legsAnim) == BOTH_STAND2TO1 || (pm->ps->legsAnim) == BOTH_STAND2 ||
        (pm->ps->legsAnim) == BOTH_SABERFAST_STANCE || (pm->ps->legsAnim) == BOTH_SABERSLOW_STANCE || (pm->ps->legsAnim) == BOTH_BUTTON_HOLD ||
        (pm->ps->legsAnim) == BOTH_BUTTON_RELEASE || PM_LandingAnim((pm->ps->legsAnim)) ||
    ...
    // if not trying to move
        ...
        if (pm->xyspeed < 5) {
            ...
            } else if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_WAMPA) {
    ...
    if (pm->ps->saberMove == LS_SPINATTACK) {
        ...
        //		if ( ( ((PM_RunningAnim( pm->ps->legsAnim ) && ((JP_GetJPFixRoll() & 2) || (JP_GetJPFixRoll() & 1 && !(pm->cmd.buttons &
        // BUTTON_FORCEGRIP)) )) && VectorLengthSquared( pm->ps->velocity ) >= 30000/*200*200*/)
        //				|| PM_CanRollFromSoulCal( pm->ps ) )
        //			 && !BG_InRoll(pm->ps, pm->ps->legsAnim) )
        if (canRoll)
#endif
        ...
        if ((pm->ps->legsAnim == BOTH_FORCELAND1 || pm->ps->legsAnim == BOTH_FORCELANDBACK1 || pm->ps->legsAnim == BOTH_FORCELANDRIGHT1 ||
            ...
            } else if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_RANCOR) { // no run anims
                if ((pm->ps->pm_flags & PMF_BACKWARDS_RUN)) {
...
int BG_VehTraceFromCamPos(trace_t *camTrace, bgEntity_t *bgEnt, const vector3 *entOrg, const vector3 *shotStart, const vector3 *end, vector3 *newEnd,
                          vector3 *shotDir, float bestDist) {
    ...
    if (!camTrace->allsolid && !camTrace->startsolid && camTrace->fraction < 1.0f && (camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) > minAutoAimDist &&
        ((camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) - Distance(entOrg, &camPos)) <
            bestDist) { // this trace hit *something* that's closer than the thing the main trace hit, so use this result instead
...
void PM_RocketLock(float lockDist, qboolean vehicleLock) {
    ...
    if (vehicleLock) { // vehicles also do a trace from the camera point if the main one misses
        if (tr.fraction >= 1.0f) {
            ...
            if (BG_VehTraceFromCamPos(&camTrace, PM_BGEntForNum(pm->ps->clientNum), &pm->ps->origin, &muzzlePoint, &tr.endpos, &newEnd, &shotDir,
...
static qboolean PM_DoChargedWeapons(qboolean vehicleRocketLock, bgEntity_t *veh) {
    ...
    if (vehicleRocketLock) {
        if ((pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // actually charging
            if (veh && veh->m_pVehicle) {                              // just make sure we have this veh info
                if (((pm->cmd.buttons & BUTTON_ATTACK) && g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].fHoming &&
                     pm->ps->ammo[0] >= g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].iAmmoPerShot) ||
                    ((pm->cmd.buttons & BUTTON_ALT_ATTACK) && g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID].fHoming &&
                     pm->ps->ammo[1] >= g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID]
                                            .iAmmoPerShot)) { // pressing the appropriate fire button for the lock-on/charging weapon
        ...
        case WP_ROCKET_LAUNCHER:
            if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) && pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] >= weaponData[pm->ps->weapon].alt.shotCost) {
        ...
        if (altFire) {
            ...
            if (vehicleRocketLock) { // check vehicle ammo
                ...
                if (pm->ps->weaponChargeSubtractTime < pm->cmd.serverTime) {
                    ...
                    if ((self->client->pers.adminData.merc && !japp_mercInfiniteAmmo.integer) || !self->client->ps.duelInProgress)
#endif
                    {
...
void PM_VehicleWeaponAnimate(void) {
    ...
    if (!veh || !veh->m_pVehicle || !veh->m_pVehicle->m_pPilot || !veh->m_pVehicle->m_pPilot->playerState ||
    ...
    if (pVeh->m_pVehicleInfo->type == VH_WALKER || pVeh->m_pVehicleInfo->type == VH_FIGHTER) { // slightly hacky I guess, but whatever.
        return;
    }
backAgain:
    ...
    } else if (veh->playerState && veh->playerState->speed < 0 && pVeh->m_pVehicleInfo->type == VH_ANIMAL) { // tauntaun is going backwards
        Anim = BOTH_VT_WALK_REV;
        iBlend = 600;
    } else if (veh->playerState && veh->playerState->speed < 0 && pVeh->m_pVehicleInfo->type == VH_SPEEDER) { // speeder is going backwards
        Anim = BOTH_VS_REV;
        iBlend = 600;
    }
...
static qboolean BG_SPCartwheel(void) {
    ...
    // TOOD: PM_InCartwheel
    if (allowedSP && (anim == BOTH_ARIAL_LEFT || anim == BOTH_ARIAL_RIGHT || anim == BOTH_CARTWHEEL_LEFT || anim == BOTH_CARTWHEEL_RIGHT)) {
        return qtrue;
    }
    return qfalse;
}
...
static void PM_Weapon(void) {
...
#ifdef PROJECT_GAME
    if (pm->ps->clientNum >= MAX_CLIENTS && pm->ps->weapon == WP_NONE && pm->cmd.weapon == WP_NONE && pm_entSelf) { // npc with no weapon
        gentity_t *gent = (gentity_t *)pm_entSelf;
        if (gent->inuse && gent->client && !gent->localAnimIndex) { // humanoid
    ...
    if (!pm->ps->emplacedIndex && pm->ps->weapon == WP_EMPLACED_GUN) { // oh no!
        int i = 0;
        int weap = -1;
    ...
    if (pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm->ps->m_iVehicleNum) { // riding a vehicle
        veh = pm_entVeh;
        if (veh && veh->m_pVehicle &&
            (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER || veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER)) { // riding a walker/fighter
    ...
    if (pm->ps->forceHandExtend == HANDEXTEND_WEAPONREADY && PM_CanSetWeaponAnims()) {           // reset into weapon stance
        if (pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE && !PM_IsRocketTrooper()) { // saber handles its own anims
            if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1) {
                // PM_StartTorsoAnim( TORSO_WEAPONREADY4 );
                PM_StartTorsoAnim(TORSO_RAISEWEAP1);
        ...
        case HANDEXTEND_FORCE_HOLD:
            if ((pm->ps->fd.forcePowersActive & (1 << FP_GRIP))) { // gripping
                ...
                if (pm->ps->weapon == WP_MELEE && pm->ps->activeForcePass > FORCE_LEVEL_2) { // 2-handed lightning
        ...
        if (playFullBody) { // sorry if all these exceptions are getting confusing. This one just means play on both legs and torso.
            ...
            pm->ps->legsTimer = pm->ps->torsoTimer = 1;
                   ...
                   (pm->ps->forceHandExtend == HANDEXTEND_CHOKE &&
                    pm->ps->groundEntityNum == ENTITYNUM_NONE)) { // special case, play dodge anim on whole body, choke anim too if off ground
            if (seperateOnTorso) {
    ...
    if (PM_CanSetWeaponAnims()) {
        if (pm->ps->weapon == WP_THERMAL || pm->ps->weapon == WP_TRIP_MINE || pm->ps->weapon == WP_DET_PACK) {
        ...
        if (!(pm->ps->pm_flags & PMF_USE_ITEM_HELD)) {
            ...
            if (!PM_ItemUsable(pm->ps, 0)) {
                ...
                if (pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag)) {
                    if (bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_BINOCULARS &&
                        ...
                        bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag !=
                            HI_EWEB) { // never use up the binoculars or jetpack or dispensers or cloak or ...
                ...
                if (bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_BINOCULARS &&
                    bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_JETPACK &&
                    bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_HEALTHDISP &&
                    bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_AMMODISP && bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_CLOAK &&
    ...
    */
    ...
    if (pm->ps->isJediMaster || (pm->ps->duelInProgress && !GetCInfo(CINFO_PRIVDUELWEAP)) || pm->ps->trueJedi) {
        pm->cmd.weapon = WP_SABER;
        pm->ps->weapon = WP_SABER;
    ...
    if (pm->ps->weapon != WP_NONE && pm->ps->weapon == pm->cmd.weapon && (pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING)) {
        if (pm->ps->clientNum < MAX_CLIENTS && pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] != -1) {
            ...
            if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] < weaponData[pm->ps->weapon].shotCost &&
                pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <
                    weaponData[pm->ps->weapon]
                        .alt.shotCost) { // the weapon is out of ammo essentially because it cannot fire primary or secondary, so do the switch
    ...
    if (PM_CanSetWeaponAnims() && !PM_IsRocketTrooper() && pm->ps->weaponstate == WEAPON_READY && pm->ps->weaponTime <= 0 &&
        (pm->ps->weapon >= WP_BRYAR_PISTOL || pm->ps->weapon == WP_STUN_BATON) && pm->ps->torsoTimer <= 0 &&
        (pm->ps->torsoAnim) != WeaponReadyAnim[pm->ps->weapon] && pm->ps->torsoAnim != TORSO_WEAPONIDLE3 && pm->ps->weapon != WP_EMPLACED_GUN) {
        PM_StartTorsoAnim(WeaponReadyAnim[pm->ps->weapon]);
        ...
        if (pm->ps->weaponTime <= 0 && pm->ps->forceHandExtend == HANDEXTEND_NONE) {
            ...
            if (desTAnim == BOTH_STAND1 || desTAnim == BOTH_STAND2) { // remap the standard standing anims for melee stance
    ...
    if (((pm->ps->torsoAnim) == TORSO_WEAPONREADY4 || (pm->ps->torsoAnim) == BOTH_ATTACK4) && (pm->ps->weapon != WP_DISRUPTOR || pm->ps->zoomMode != 1)) {
        if (pm->ps->weapon == WP_EMPLACED_GUN) {
            PM_StartTorsoAnim(BOTH_GUNSIT1);
        } else if (PM_CanSetWeaponAnims()) {
            PM_StartTorsoAnim(WeaponReadyAnim[pm->ps->weapon]);
        }
    } else if (((pm->ps->torsoAnim) != TORSO_WEAPONREADY4 && (pm->ps->torsoAnim) != BOTH_ATTACK4) && PM_CanSetWeaponAnims() &&
               (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)) {
        PM_StartTorsoAnim(TORSO_WEAPONREADY4);
    }

    if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_VEHICLE) { // we are a vehicle
        veh = pm_entSelf;
    }
    if (veh && veh->m_pVehicle) {
        if (g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].fHoming ||
    ...
    if (!vehicleRocketLock) {
        if (pm->ps->weapon != WP_ROCKET_LAUNCHER) {
            if (pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm->ps->m_iVehicleNum) { // riding a vehicle, the vehicle will tell me my rocketlock stuff...
            } else {
                pm->ps->rocketLockIndex = ENTITYNUM_NONE;
                pm->ps->rocketLockTime = 0;
    ...
    if (pm->ps->weapon == WP_EMPLACED_GUN) {
        ...
        return;
    } else if (pm->ps->m_iVehicleNum && pm_entSelf->s.NPC_class == CLASS_VEHICLE) { // a vehicle NPC that has a pilot
    ...
    if (pm->ps->weapon == WP_DISRUPTOR && (pm->cmd.buttons & BUTTON_ALT_ATTACK) && !pm->ps->zoomLocked) {
        return;
    }
    ...
    if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1) {
        ...
        if (!pm->ps->m_iVehicleNum) { // if riding a vehicle don't do this stuff at all
            if (pm->debugMelee && (pm->cmd.buttons & BUTTON_ATTACK) && (pm->cmd.buttons & BUTTON_ALT_ATTACK)) { // ok, grapple time
...
#endif
#endif
            } else if (pm->debugMelee && (pm->cmd.buttons & BUTTON_ALT_ATTACK)) { // kicks
                if (!BG_KickingAnim(pm->ps->torsoAnim) && !BG_KickingAnim(pm->ps->legsAnim)) {
                    ...
                    if (kickMove != -1) {
                        if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // if in air, convert kick to an in-air kick
                            ...
                            if ((!BG_FlippingAnim(pm->ps->legsAnim) || pm->ps->legsTimer <= 0) && gDist > 64.0f && // strict minimum
...
static void PM_Animate(void) {
    if (pm->cmd.buttons & BUTTON_GESTURE) {
        ...
        if (pm->ps->torsoTimer < 1 && pm->ps->forceHandExtend == HANDEXTEND_NONE && pm->ps->legsTimer < 1 && pm->ps->weaponTime < 1 &&
...
void PM_UpdateViewAngles(playerState_t *ps, const usercmd_t *cmd) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (pm_entVeh && pm_entVeh->m_pVehicle && pm_entVeh->m_pVehicle->m_pVehicleInfo && pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
            (cmd->serverTime - pm_entVeh->playerState->hyperSpaceTime) >= HYPERSPACE_TIME) { // in a vehicle and not hyperspacing
            if (i == 0 /*PITCH*/) {
...
void PM_AdjustAttackStates(pmove_t *pmove) {
    ...
    if (pm_entSelf->s.NPC_class != CLASS_VEHICLE && pmove->ps->m_iVehicleNum) { // riding a vehicle
        bgEntity_t *veh = pm_entVeh;
        if (veh && veh->m_pVehicle &&
            (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER || veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER)) { // riding a walker/fighter
            // not firing, ever
    ...
    if (pmove->ps->weapon == WP_DISRUPTOR && pmove->ps->weaponstate == WEAPON_READY) {
        ...
        if ( !(pmove->ps->eFlags & EF_ALT_FIRING) && (pmove->cmd.buttons & BUTTON_ALT_ATTACK) /*&&
            ...
            if (!pmove->ps->zoomMode && pmove->ps->pm_type != PM_DEAD) {
                ...
                PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
            } else if (pmove->ps->zoomMode == 1 &&
                       pmove->ps->zoomLockTime < pmove->cmd.serverTime) { // check for == 1 so we can't turn binoculars off with disruptor alt fire
            ...
            if (pmove->ps->zoomMode) {
                if (pmove->ps->zoomMode == 1 &&
                    !pmove->ps->zoomLocked) { // approximate what level the client should be zoomed at based on how long zoom was held
    ...
    */
    ...
    if (!(pmove->ps->pm_flags & PMF_RESPAWNED) && pmove->ps->pm_type != PM_INTERMISSION && pmove->ps->pm_type != PM_NOCLIP &&
        (pmove->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) && (amount >= 0 || pmove->ps->weapon == WP_SABER)) {
        if (pmove->cmd.buttons & BUTTON_ALT_ATTACK) {
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime) {
    ...
    if (ps->forceHandExtend == HANDEXTEND_KNOCKDOWN || ps->forceHandExtend == HANDEXTEND_PRETHROWN || ps->forceHandExtend == HANDEXTEND_POSTTHROWN) {
        ps->speed = 0;
    }

    if (cmd->forwardmove < 0 && !(cmd->buttons & BUTTON_WALKING) &&
        pm->ps->groundEntityNum != ENTITYNUM_NONE) { // running backwards is slower than running forwards (like SP)
        ps->speed *= 0.75f;
    }
    ...
    if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1 && pm->ps->zoomLockTime < pm->cmd.serverTime) {
        ps->speed *= 0.5f;
    }
...
void BG_UpdateLookAngles(int lookingDebounceTime, vector3 *lastHeadAngles, int time, vector3 *lookAngles, float lookSpeed, float minPitch, float maxPitch,
...
static void BG_G2ClientNeckAngles(void *ghoul2, int time, const vector3 *lookAngles, vector3 *headAngles, vector3 *neckAngles, vector3 *thoracicAngles,
                                  vector3 *headClampMinAngles, vector3 *headClampMaxAngles, entityState_t *cent) {
    vector3 lA;
...
static void BG_G2ClientSpineAngles(void *ghoul2, int motionBolt, vector3 *cent_lerpOrigin, vector3 *cent_lerpAngles, entityState_t *cent, int time,
                                   vector3 *viewAngles, int ciLegs, int ciTorso, const vector3 *angles, vector3 *thoracicAngles, vector3 *ulAngles,
                                   vector3 *llAngles, vector3 *modelScale, float *tPitchAngle, float *tYawAngle, int *corrTime) {
...
#if 1
    if (!BG_FlippingAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->torsoAnim) &&
        !BG_InSpecialJump(cent->legsAnim) && !BG_InSpecialJump(cent->torsoAnim) && !BG_InDeathAnim(cent->legsAnim) && !BG_InDeathAnim(cent->torsoAnim) &&
        !BG_InRollES(cent, cent->legsAnim) && !BG_InRollAnim(cent) && !BG_SaberInSpecial(cent->saberMove) && !BG_SaberInSpecialAttack(cent->torsoAnim) &&
        !BG_SaberInSpecialAttack(cent->legsAnim) &&

        !BG_InKnockDown(cent->torsoAnim) && !BG_InKnockDown(cent->legsAnim) && !BG_InKnockDown(ciTorso) && !BG_InKnockDown(ciLegs) &&

        !BG_FlippingAnim(ciLegs) && !BG_SpinningSaberAnim(ciLegs) && !BG_SpinningSaberAnim(ciTorso) && !BG_InSpecialJump(ciLegs) &&
        !BG_InSpecialJump(ciTorso) && !BG_InDeathAnim(ciLegs) && !BG_InDeathAnim(ciTorso) && !BG_SaberInSpecialAttack(ciTorso) &&
        !BG_SaberInSpecialAttack(ciLegs) &&

        !(cent->eFlags & EF_DEAD) && (cent->legsAnim) != (cent->torsoAnim) && (ciLegs) != (ciTorso) && !cent->m_iVehicleNum) {
        doCorr = qtrue;
    }
#else
    if (((!BG_FlippingAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->torsoAnim) &&
          (cent->legsAnim) != (cent->torsoAnim)) // NOTE: presumes your legs & torso are on the same frame, though they *should* be because PM_SetAnimFinal
                                                 // tries to keep them in synch
         || (!BG_FlippingAnim(ciLegs) && !BG_SpinningSaberAnim(ciLegs) && !BG_SpinningSaberAnim(ciTorso) && (ciLegs) != (ciTorso))) ||
        ciLegs != cent->legsAnim || ciTorso != cent->torsoAnim) {
        doCorr = qtrue;
        *corrTime =
            time +
            1000; // continue correcting for a second after to smooth things out. SP doesn't need this for whatever reason but I can't find a way around it.
    } else if (*corrTime >= time) {
        if (!BG_FlippingAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->torsoAnim) && !BG_FlippingAnim(ciLegs) &&
            !BG_SpinningSaberAnim(ciLegs) && !BG_SpinningSaberAnim(ciTorso)) {
            doCorr = qtrue;
        }
    }
#endif
...
void BG_G2PlayerAngles(void *ghoul2, int motionBolt, entityState_t *cent, int time, vector3 *cent_lerpOrigin, vector3 *cent_lerpAngles, vector3 legs[3],
...
#endif
    static vector3 eyeAngles, neckAngles, velocity, torsoAngles, headAngles, velPos, velAng, ulAngles, llAngles, viewAngles, angles,
        thoracicAngles = {0, 0, 0}, headClampMinAngles = {-25, -55, -10}, headClampMaxAngles = {50, 50, 10};

    if (cent->m_iVehicleNum || cent->forceFrame || BG_SaberLockBreakAnim(cent->legsAnim) ||
        BG_SaberLockBreakAnim(cent->torsoAnim)) { // a vehicle or riding a vehicle - in either case we don't need to be in here
    ...
    if (((cent->legsAnim) != BOTH_STAND1) || (cent->torsoAnim) != WeaponReadyAnim[cent->weapon]) {
    ...
    *tYawAngle = torsoAngles.yaw;
    ...
    } else if (cent->weapon == WP_SABER && BG_SaberInSpecial(cent->saberMove)) {
        VectorClear(&velocity);
    }
    ...
    if (cent->groundEntityNum == ENTITYNUM_NONE || cent->forceFrame ||
    ...
    */
    ...
    if (cent->weapon == WP_EMPLACED_GUN && emplaced) { // if using an emplaced gun, then we want to make sure we're angled to "hold" it right
        ...
        if (emplaced->weapon == WP_NONE) { // e-web
            ...
            */
            ...
            if (cent->legsAnim == BOTH_STRAFE_LEFT1 || cent->legsAnim == BOTH_STRAFE_RIGHT1) { // try to adjust so it doesn't look wrong
                ...
                BG_G2ClientSpineAngles(ghoul2, motionBolt, cent_lerpOrigin, cent_lerpAngles, cent, time, &viewAngles, ciLegs, ciTorso, &angles, &thoracicAngles,
                                       &ulAngles, &llAngles, modelScale, tPitchAngle, tYawAngle, corrTime);
    ...
    BG_G2ClientSpineAngles(ghoul2, motionBolt, cent_lerpOrigin, cent_lerpAngles, cent, time, &viewAngles, ciLegs, ciTorso, &angles, &thoracicAngles, &ulAngles,
                           &llAngles, modelScale, tPitchAngle, tYawAngle, corrTime);
...
static void PM_CmdForSaberMoves(usercmd_t *ucmd) {
    ...
    if ((pm->ps->legsAnim == BOTH_JUMPATTACK6 && pm->ps->saberMove == LS_JUMPATTACK_DUAL) ||
        ...
        if (pm->ps->legsAnim == BOTH_JUMPATTACK6) {   // dual stance attack
            ...
            if ((pm->ps->legsTimer >= 900                                          // not at end
                ...
                || (pm->ps->legsTimer >= 1600 && aLen - pm->ps->legsTimer >= 400)) // not in beginning
            ...
            if (pm->ps->legsAnim == BOTH_BUTTERFLY_RIGHT || pm->ps->legsAnim == BOTH_BUTTERFLY_LEFT) {
    ...
    else if (pm->ps->saberMove == LS_A_BACKFLIP_ATK && pm->ps->legsAnim == BOTH_JUMPATTACK7) {
    ...
    else if (pm->ps->saberMove == LS_SPINATTACK || pm->ps->saberMove == LS_SPINATTACK_DUAL) {
        ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd) {
    ...
    if (veh->m_pVehicle->m_pPilot && veh->m_pVehicle->m_pPilot->s.number == ps->clientNum) { // set the pilot's viewangles to the vehicle's viewangles
#ifdef VEH_CONTROL_SCHEME_4
        if (1)
#else     // VEH_CONTROL_SCHEME_4
...
int PM_GetOkWeaponForVehicle(void) {
    ...
    while (i < WP_NUM_WEAPONS) {
        if ((pm->ps->stats[STAT_WEAPONS] & (1 << i)) && PM_WeaponOkOnVehicle(i)) { // this one's good
            return i;
        }
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (fabsf(aDelta) < turnRate) { // all is good
                ...
                if (fabsf(aDelta) < turnRate) {
                    ...
                    if (i == 1 /*YAW*/)
                        pm->ps->viewangles.raw[i] = AngleNormalize360(pm->ps->viewangles.raw[i] + turnRate);
                    else
                        pm->ps->viewangles.raw[i] = AngleNormalize180(pm->ps->viewangles.raw[i] + turnRate);
                } else {
                    if (i == 1 /*YAW*/)
                        pm->ps->viewangles.raw[i] = AngleNormalize360(pm->ps->viewangles.raw[i] - turnRate);
                    else
...
void BG_VehicleAdjustBBoxForOrientation(Vehicle_t *veh, vector3 *origin, vector3 *mins, vector3 *maxs, int clientNum, int tracemask,
                                        void (*localTrace)(trace_t *results, const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end,
                                                           int passEntityNum, int contentMask)) {
    if (!veh || !veh->m_pVehicleInfo->length || !veh->m_pVehicleInfo->width || !veh->m_pVehicleInfo->height)
...
qboolean BG_FighterUpdate(Vehicle_t *pVeh, const usercmd_t *pUcmd, vector3 *trMins, vector3 *trMaxs, float gravity,
                          void (*traceFunc)(trace_t *results, const vector3 *start, const vector3 *lmins, const vector3 *lmaxs, const vector3 *end,
                                            int passEntityNum, int contentMask)); // FighterNPC.c
...
void PM_MoveForKata(usercmd_t *ucmd) {
    if (pm->ps->legsAnim == BOTH_A7_SOULCAL && pm->ps->saberMove == LS_STAFF_SOULCAL) { // forward spinning staff attack
        ucmd->upmove = 0;
        ...
        if (pm->ps->legsTimer >= 2650 && pm->ps->legsTimer < 2850) { // the jump
            if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {         // still on ground?
                // jump!
                pm->ps->velocity.z = 250;
                pm->ps->fd.forceJumpZStart = pm->ps->origin.z; // so we don't take damage if we land at same height
...
void PmoveSingle(pmove_t *pmove) {
    ...
    if (pm->ps->pm_type == PM_FLOAT) { // You get no control over where you go in grip movement
        ...
        stiffenedUp = qtrue;
    } else if (BG_SaberLockBreakAnim(pm->ps->legsAnim) || BG_SaberLockBreakAnim(pm->ps->torsoAnim) ||
    ...
    // DFA
    else if (pm->ps->saberMove == LS_A_BACK || pm->ps->saberMove == LS_A_BACK_CR || pm->ps->saberMove == LS_A_BACKSTAB || pm->ps->saberMove == LS_A_FLIP_STAB ||
             pm->ps->saberMove == LS_A_FLIP_SLASH || pm->ps->saberMove == LS_A_JUMP_T__B_ || pm->ps->saberMove == LS_DUAL_LR ||
             pm->ps->saberMove == LS_DUAL_FB || pm->ps->saberMove == LS_JAPLUS_NEWANIM_FLIPSABERSTAB) {
        if (!GetCInfo(CINFO_YELLOWDFA) && (pm->ps->legsAnim == BOTH_JUMPFLIPSTABDOWN || pm->ps->legsAnim == BOTH_JUMPFLIPSLASHDOWN1)) {
        ...
        stiffenedUp = qtrue;
    } else if (pm->ps->legsAnim == BOTH_A2_STABBACK1 || pm->ps->legsAnim == BOTH_ATTACK_BACK || pm->ps->legsAnim == BOTH_CROUCHATTACKBACK1 ||
               pm->ps->legsAnim == BOTH_FORCELEAP2_T__B_ || pm->ps->legsAnim == BOTH_JUMPFLIPSTABDOWN || pm->ps->legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
               pm->ps->legsAnim == BOTH_NEW_STABEE || pm->ps->legsAnim == BOTH_NEW_STABER || pm->ps->legsAnim == BOTH_JUMP_BACKFLIP_ATCK ||
               pm->ps->legsAnim == BOTH_JUMP_BACKFLIP_ATCKEE || pm->ps->legsAnim == BOTH_JUMP_BACKFLIP_ATCK_MISSED ||
               pm->ps->legsAnim == BOTH_ALORA_SPIN_SLASH || pm->ps->legsAnim == BOTH_FJSS_TR_BL) {
        stiffenedUp = qtrue;
    ...
    // Raz: Bug fix http://forum.cosmosgaming.net/threads/1510/
        ...
        stiffenedUp = qtrue;
        PM_SetPMViewAngle(pm->ps, &pm->ps->viewangles, &pm->cmd);
    } else if (pm->ps->saberMove == LS_STABDOWN_DUAL || pm->ps->saberMove == LS_STABDOWN_STAFF ||
        ...
        pm->ps->legsAnim == BOTH_FORCE_GETUP_F2)       // ampower
    {
    ...
    else if (BG_SaberInKata(pm->ps->saberMove) || BG_InKataAnim(pm->ps->torsoAnim) || BG_InKataAnim(pm->ps->legsAnim)) {
        PM_MoveForKata(&pm->cmd);
    } else if (BG_FullBodyTauntAnim(pm->ps->legsAnim) && BG_FullBodyTauntAnim(pm->ps->torsoAnim)) {
        if ((pm->cmd.buttons & BUTTON_ATTACK) || (pm->cmd.buttons & BUTTON_ALT_ATTACK) || (pm->cmd.buttons & BUTTON_FORCEPOWER) ||
            (pm->cmd.buttons & BUTTON_FORCEGRIP) || (pm->cmd.buttons & BUTTON_FORCE_LIGHTNING) || (pm->cmd.buttons & BUTTON_FORCE_DRAIN) ||
            pm->cmd.upmove) { // stop the anim
            if (pm->ps->legsAnim == BOTH_MEDITATE && pm->ps->torsoAnim == BOTH_MEDITATE) {
        ...
        //	else
            ...
            if (pm->ps->legsAnim == BOTH_MEDITATE &&
                (pm->ps->forceHandExtend != HANDEXTEND_KNOCKDOWN && pm->ps->forceHandExtendTime != INT32_MAX && pm->ps->forceDodgeAnim != 0)) {
    ...
    else if (pm->ps->legsAnim == BOTH_FORCELAND1 || pm->ps->legsAnim == BOTH_FORCELANDBACK1 || pm->ps->legsAnim == BOTH_FORCELANDRIGHT1 ||
    ...
    */
    ...
    if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->weaponstate == WEAPON_CHARGING_ALT) { // not allowed to move while charging the disruptor
        if (pm->cmd.forwardmove || pm->cmd.rightmove || pm->cmd.upmove > 0) {           // get out
            pm->ps->weaponstate = WEAPON_READY;
    ...
    */
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_VEHICLE) { // we are a vehicle
    ...
    } else if (pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm->ps->m_iVehicleNum) {
        bgEntity_t *veh = pm_entVeh;
        ...
        } else if (veh && veh->playerState && veh->playerState->vehTurnaroundIndex &&
                   veh->playerState->vehTurnaroundTime > pm->cmd.serverTime) { // riding this vehicle, turn my view too
            PM_VehForcedTurning(veh);
        }
    }

    if (pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_ALT && pm->ps->legsTimer > 0) {
        vector3 vFwd, fwdAng;
    ...
    if (pm->ps->saberMove == LS_A_JUMP_T__B_ || pm->ps->saberMove == LS_A_LUNGE || pm->ps->saberMove == LS_A_BACK_CR || pm->ps->saberMove == LS_A_BACK ||
    ...
    */
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
        pm_entSelf->m_pVehicle) { // Now update our mins/maxs to match our m_vOrientation based on our length, width & height
    ...
    if (pm->ps->pm_type == PM_DEAD) {
        if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_VEHICLE &&
    ...
    if (pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm->ps->m_iVehicleNum) { // a player riding a vehicle
        bgEntity_t *veh = pm_entVeh;

        if (veh && veh->m_pVehicle &&
            (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER ||
             veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER)) { //*sigh*, until we get forced weapon-switching working?
            pm->cmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
            pm->ps->eFlags &= ~(EF_FIRING | EF_ALT_FIRING);
            // pm->cmd.weapon = pm->ps->weapon;
        }
    ...
    if (!pm->ps->m_iVehicleNum && pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm_entSelf->s.NPC_class != CLASS_RANCOR &&
        ...
        if (pEnt && pEnt->s.eType == ET_NPC && pEnt->s.NPC_class != CLASS_VEHICLE) // don't bounce on vehicles
...
#ifdef PROJECT_GAME
        else if (!pm->ps->zoomMode && pm_entSelf // I exist
                 && pEnt->m_pVehicle)            // ent has a vehicle
        {
            gentity_t *gEnt = (gentity_t *)pEnt;
            if (gEnt->client && !gEnt->client->ps.m_iVehicleNum // vehicle is empty
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_VEHICLE) { // we are a vehicle
        ...
        if (!pm->ps->m_iVehicleNum) { // no one is driving, just update and get out
            ...
            if (pm->ps->pm_type == PM_DEAD && (veh->m_pVehicle->m_ulFlags & VEH_CRASHING)) {
    ...
    if (pm->cmd.forcesel != (byte)-1 && (pm->ps->fd.forcePowersKnown & (1 << pm->cmd.forcesel)) && pm->cmd.forcesel != FP_LEVITATION &&
    ...
    if (pm->ps->m_iVehicleNum
        ...
        } else if (!PM_WeaponOkOnVehicle(pm->cmd.weapon) ||
                   !PM_WeaponOkOnVehicle(pm->ps->weapon)) { // this weapon is not legal for the vehicle, force to our current one
    ...
    if (!pm->ps->m_iVehicleNum                      // not a vehicle and not riding one
        ...
        || (!(pm->ps->eFlags & EF_NODRAW) &&
            PM_WeaponOkOnVehicle(pm->cmd.weapon))) // you're not inside the vehicle and the weapon you're holding can be used when riding this vehicle
    ...
    if (!pm->ps->m_iVehicleNum &&
        (pm->ps->clientNum < MAX_CLIENTS || !pm_entSelf || pm_entSelf->s.NPC_class != CLASS_VEHICLE)) { // don't do this if we're on a vehicle, or we are one
    ...
    if (                                                                                              // pm->ps->m_iVehicleNum &&
        pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_VEHICLE) { // a vehicle with passengers
        bgEntity_t *veh;
