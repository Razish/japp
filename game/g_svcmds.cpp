...
int G_SortPlayersByScoreRate(const void *a, const void *b) {
    gclient_t *cla = &level.clients[*((int *)a)], *clb = &level.clients[*((int *)b)];
    float arate, brate;
...
static void SV_AdminAdd_f(void) {
    char argUser[MAX_TOKEN_CHARS] = {0}, argPass[MAX_TOKEN_CHARS] = {0}, argPrivs[MAX_TOKEN_CHARS] = {0}, argRank[MAX_TOKEN_CHARS] = {0},
         argEffect[MAX_TOKEN_CHARS] = {0}, *argMsg = NULL;
...
static void SV_AdminList_f(void) { AM_ListAdmins(); }
...
static void SV_EntityList_f(void) {
    ...
    for (e = 0, check = g_entities; e < level.num_entities; e++, check++) {
        char buf[256] = {0};
...
static void SV_GameMemory_f(void) { G_ShowGameMem(); }

static void SV_Gametype_f(void) { trap->Print("Gametype is currently: %s\n", BG_GetGametypeString(level.gametype)); }
...
static int cmdcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((svCommand_t *)b)->cmd); }
