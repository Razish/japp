        ...
        if (attack_ok) {
            ...
            if (enemyVisibility == VIS_FOV) { // He's in our FOV
                ...
                if (attack_ok) {
                    ...
                    if (traceEnt != NPC->enemy && (!traceEnt || !traceEnt->client || !NPC->client->enemyTeam ||
                                                   NPC->client->enemyTeam != traceEnt->client->playerTeam)) { // no, so shoot for the head
                        attack_scale *= 0.75f;
...
void NPC_BSWait(void) { NPC_UpdateAngles(qtrue, qtrue); }
...
void NPC_BSFollowLeader(void) {
    ...
    if (!NPC->enemy) {                                                      // no enemy, find one
            ...
            if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
                ...
                if (level.alertEvents[eventID].level >= AEL_SUSPICIOUS && (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
                    ...
                    if (!level.alertEvents[eventID].owner || !level.alertEvents[eventID].owner->client || level.alertEvents[eventID].owner->health <= 0 ||
                        ...
                        // FIXME: what if can't actually see enemy, don't know where he is... should we make them just become very alert and start looking for
                        // him?  Or just let combat AI handle this... (act as if you lost him)
        ...
        if (!NPC->enemy) {
            if (NPC->client->leader && NPC->client->leader->enemy && NPC->client->leader->enemy != NPC &&
        ...
        if (NPC->enemy->health <= 0 || (NPC->enemy->flags & FL_NOTARGET)) {
            ...
            NPC_CheckEnemy((NPCInfo->confusionTime < level.time || NPCInfo->tempBehavior != BS_FOLLOW_LEADER), qfalse,
                           qtrue); // don't find new enemy if this is tempbehav
        }
    }
        ...
        if (enemyVisibility > VIS_PVS) {                                            // face
            ...
            if (enemyVisibility >= VIS_SHOOT) { // shoot
                ...
                if (NPC_GetHFOVPercentage(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, NPCInfo->stats.hfov) > 0.6f &&
                    NPC_GetHFOVPercentage(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, NPCInfo->stats.vfov) >
                        0.5f) { // actually withing our front cone
                    WeaponThink(qtrue);
                }
            } else
                NPC_AimAdjust(1);
    ...
    if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 && curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 &&
        curAnim != BOTH_MELEE2) { // Don't move toward leader if we're in a full-body attack anim
...
qboolean NPC_CheckSurrender(void) {
    if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV) && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE && !NPC->client->ps.weaponTime &&
        !PM_InKnockDown(&NPC->client->ps) && NPC->enemy && NPC->enemy->client && NPC->enemy->enemy == NPC && NPC->enemy->s.weapon != WP_NONE &&
        NPC->enemy->s.weapon != WP_STUN_BATON && NPC->enemy->health > 20 && NPC->enemy->painDebounceTime < level.time - 3000 &&
        NPC->enemy->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] <
            level.time - 1000) { // don't surrender if scripted to run somewhere or if we're in the air or if we're busy or if we don't have an enemy or if the
                                 // enemy is not mad at me or is hurt or not a threat or busy being attacked
        // FIXME: even if not in a group, don't surrender if there are other enemies in the PVS and within a certain range?
        if (NPC->s.weapon != WP_ROCKET_LAUNCHER && NPC->s.weapon != WP_REPEATER && NPC->s.weapon != WP_FLECHETTE &&
...
void NPC_StartFlee(gentity_t *enemy, vector3 *dangerPoint, int dangerLevel, int fleeTimeMin, int fleeTimeMax) {
    ...
    if (dangerLevel > AEL_DANGER || NPC->s.weapon == WP_NONE ||
        ((!NPCInfo->group || NPCInfo->group->numGroup <= 1) &&
         NPC->health <= 10)) { // IF either great danger OR I have no weapon OR I'm alone and low on health, THEN try to find a combat point out of PVS
...
void NPC_BSEmplaced(void) {
    ...
    if (NPC_ClearLOS4(NPC->enemy)) {
        ...
        if (hit == NPC->enemy->s.number ||
            (hitEnt && hitEnt->takedamage)) { // can hit enemy or will hit glass or other minor breakable (or in emplaced gun), so shoot anyway
            enemyCS = qtrue;
