...
static const stringID_table_t eventNames[JPLUA_EVENT_MAX] = {
    ENUM2STRING(JPLUA_EVENT_UNLOAD),        ENUM2STRING(JPLUA_EVENT_RUNFRAME),      ENUM2STRING(JPLUA_EVENT_CHATMSGRECV),
    ENUM2STRING(JPLUA_EVENT_CHATMSGSEND),   ENUM2STRING(JPLUA_EVENT_CHATMSGPLUGIN), ENUM2STRING(JPLUA_EVENT_CLIENTBEGIN),
    ENUM2STRING(JPLUA_EVENT_CLIENTCOMMAND), ENUM2STRING(JPLUA_EVENT_CLIENTCONNECT), ENUM2STRING(JPLUA_EVENT_CLIENTDISCONNECT),
    ENUM2STRING(JPLUA_EVENT_CLIENTINFO),    ENUM2STRING(JPLUA_EVENT_CLIENTSPAWN),   ENUM2STRING(JPLUA_EVENT_CLIENTUSERINFOCHANGED),
    ENUM2STRING(JPLUA_EVENT_HUD),           ENUM2STRING(JPLUA_EVENT_VEHICLEHUD),    ENUM2STRING(JPLUA_EVENT_CONNECTSCREEN),
    ENUM2STRING(JPLUA_EVENT_PAIN),          ENUM2STRING(JPLUA_EVENT_PLAYERDEATH),   ENUM2STRING(JPLUA_EVENT_PRIVATEDUEL),
    ENUM2STRING(JPLUA_EVENT_SABERTOUCH),    ENUM2STRING(JPLUA_EVENT_KEYDOWN),
};
...
int Event_AddListener(lua_State *L) {
    ...
    if (lua_type(L, 1) != LUA_TSTRING || lua_type(L, 2) != LUA_TFUNCTION) {
        ...
        G_LogPrintf(level.log.console,
                    "JPLua AddListener failed, function signature invalid registering %s "
...
#if defined(PROJECT_GAME)
    G_LogPrintf(level.log.console,
                "JPLua AddListener failed, could not find event %s (plugin: %s) - Is it up to "
                "date?\n",
                listenerArg, ls.currentPlugin->name);
#elif defined(PROJECT_CGAME)
...
#if defined(PROJECT_GAME)
        G_LogPrintf(level.log.console,
                    "JPLua RemoveListener failed, function signature invalid registering %s "
                    "(plugin: %s) - Is it up to date?\n",
                    listenerArg, ls.currentPlugin->name);
#elif defined(PROJECT_CGAME)
...
#if defined(PROJECT_GAME)
    G_LogPrintf(level.log.console,
                "JPLua RemoveListener failed, could not find event %s (plugin: %s) - Is it up "
                "to date?\n",
                listenerArg, ls.currentPlugin->name);
#elif defined(PROJECT_CGAME)
    ...
    while (IteratePlugins(&plugin)) {
        if (plugin->eventListeners[JPLUA_EVENT_CHATMSGRECV]) {
            ...
            if (retType == LUA_TNIL) {
                ...
                trap->Print("Invalid return value in %s (JPLUA_EVENT_CHATMSGRECV), expected string or nil but got %s", plugin->name, lua_typename(ls.L, -1));
            }
            lua_pop(ls.L, 1);
        }
    }
    ...
    while (IteratePlugins(&plugin)) {
        if (plugin->eventListeners[JPLUA_EVENT_CHATMSGSEND]) {
            ...
            if (lua_type(ls.L, -1) == LUA_TNIL) {
                ...
                trap->Print("Invalid return value in %s (JPLUA_EVENT_CHATMSGSEND), expected string or nil but got %s\n", plugin->name, lua_typename(ls.L, -1));
            }
            lua_pop(ls.L, 1);
        }
    }
    ...
    while (IteratePlugins(&plugin)) {
        if (plugin->eventListeners[JPLUA_EVENT_CHATMSGPLUGIN]) {
            ...
            if (retType == LUA_TNIL) {
                ...
                trap->Print("Invalid return value in %s (JPLUA_EVENT_CHATMSGPLUGIN), expected string or nil but got %s", plugin->name, lua_typename(ls.L, -1));
            }
            lua_pop(ls.L, 1);
        }
    }
