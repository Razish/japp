...
const vehField_t vehWeaponFields[NUM_VWEAP_PARMS] = {
    ...
    {"homingFOV", VWFOFS(fHomingFOV),
     VF_FLOAT}, // missile will lose lock on if DotProduct of missile direction and direction to target ever drops below this (-1 to 1, -1 = never lose target,
                // 0 = lose if ship gets behind missile, 1 = pretty much will lose it's target right away)
    ...
    for (i = 0; i < NUM_VWEAP_PARMS; i++) {
        if (vehWeaponFields[i].name && !Q_stricmp(vehWeaponFields[i].name, parmName)) {
            ...
            case VF_LSTRING:                                                         // string on disk, pointer in memory, TAG_LEVEL
                if (!*(char **)(b + vehWeaponFields[i].ofs)) {                       // just use 1024 bytes in case we want to write over the string
                ...
                break;
            case VF_VECTOR:
                _iFieldsRead = sscanf(value, "%f %f %f", &vec.x, &vec.y, &vec.z);
                assert(_iFieldsRead == 3);
                if (_iFieldsRead != 3) {
                    Com_Printf(S_COLOR_YELLOW "BG_ParseVehWeaponParm: VEC3 sscanf() failed to read 3 floats ('angle' key bug?)\n");
                }
                ((float *)(b + vehWeaponFields[i].ofs))[0] = vec.x;
                ((float *)(b + vehWeaponFields[i].ofs))[1] = vec.y;
                ((float *)(b + vehWeaponFields[i].ofs))[2] = vec.z;
                break;
            case VF_BOOL:
...
int VEH_VehWeaponIndexForName(const char *vehWeaponName) {
    ...
    for (vw = VEH_WEAPON_BASE; vw < numVehicleWeapons; vw++) {
        if (g_vehWeaponInfo[vw].name && Q_stricmp(g_vehWeaponInfo[vw].name, vehWeaponName) == 0) { // already loaded this one
            return vw;
        }
    }
    // haven't loaded it yet
    if (vw >= MAX_VEH_WEAPONS) { // no more room!
...
const vehField_t vehicleFields[] = {
    ...
    {"centerOfGravity", VFOFS(centerOfGravity),
     VF_VECTOR}, // offset from origin: {forward, right, up} as a modifier on that dimension (-1.0f is all the way back, 1.0f is all the way forward)
    ...
    {"strafePerc", VFOFS(strafePerc),
     VF_FLOAT}, // multiplier on current speed for strafing.  If 1.0f, you can strafe at the same speed as you're going forward, 0.5f is half, 0 is no strafing
    ...
    {"toughness", VFOFS(toughness),
     VF_FLOAT}, // modifies incoming damage, 1.0f is normal, 0.5f is half, etc.  Simulates being made of tougher materials/construction
    ...
    {"cameraHorzOffset", VFOFS(cameraHorzOffset),
     VF_FLOAT}, // how far to left/right (negative/positive) of of the vehicle origin the camera should be - normal is 0
...
stringID_table_t VehicleTable[VH_NUM_VEHICLES + 1] = {ENUM2STRING(VH_NONE),
...
void BG_VehicleSetDefaults(vehicleInfo_t *vehicle) {
    ...
    /*
       ...
       //How far you can look left and right off the forward of the vehicle
            ...
            VectorClear( vehicle->centerOfGravity );//offset from origin: {forward, right, up} as a modifier on that dimension (-1.0f is all the way back, 1.0f
       is all the way forward)
            ...
            vehicle->strafePerc = VEH_DEFAULT_STRAFE_PERC;//multiplier on current speed for strafing.  If 1.0f, you can strafe at the same speed as you're going
       forward, 0.5f is half, 0 is no strafing
            ...
            vehicle->toughness = VEH_DEFAULT_TOUGHNESS;	//modifies incoming damage, 1.0f is normal, 0.5f is half, etc.  Simulates being made of tougher
       materials/construction vehicle->malfunctionArmorLevel = 0;			//when armor drops to or below this point, start malfunctioning
...
void BG_VehicleClampData(vehicleInfo_t *vehicle) { // sanity check and clamp the vehicle's data
    int i;
        ...
        if (!Q_stricmp(vehicleFields[i].name, parmName)) {
            ...
            case VF_FLOAT:
                *(float *)(b + vehicleFields[i].ofs) = atof(value);
                break;
            case VF_LSTRING:                                                      // string on disk, pointer in memory, TAG_LEVEL
                if (!*(char **)(b + vehicleFields[i].ofs)) {                      // just use 128 bytes in case we want to write over the string
...
int VEH_LoadVehicle(const char *vehicleName) { // load up specified vehicle and save in array: g_vehicleInfo
    const char *token;
    ...
    while (1) {
        ...
        } else if (Q_stricmp("weap1", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weap1, value, sizeof(weap1));
        } else if (Q_stricmp("weap2", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weap2, value, sizeof(weap2));
        } else if (Q_stricmp("weapMuzzle1", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle1, value, sizeof(weapMuzzle1));
        } else if (Q_stricmp("weapMuzzle2", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle2, value, sizeof(weapMuzzle2));
        } else if (Q_stricmp("weapMuzzle3", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle3, value, sizeof(weapMuzzle3));
        } else if (Q_stricmp("weapMuzzle4", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle4, value, sizeof(weapMuzzle4));
        } else if (Q_stricmp("weapMuzzle5", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle5, value, sizeof(weapMuzzle5));
        } else if (Q_stricmp("weapMuzzle6", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle6, value, sizeof(weapMuzzle6));
        } else if (Q_stricmp("weapMuzzle7", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle7, value, sizeof(weapMuzzle7));
        } else if (Q_stricmp("weapMuzzle8", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle8, value, sizeof(weapMuzzle8));
        } else if (Q_stricmp("weapMuzzle9", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
            Q_strncpyz(weapMuzzle9, value, sizeof(weapMuzzle9));
        } else if (Q_stricmp("weapMuzzle10", parmName) ==
                   0) { // hmm, store this off because we don't want to call another one of these text parsing routines while we're in the middle of one...
...
int VEH_VehicleIndexForName(const char *vehicleName) {
    ...
    for (v = VEHICLE_BASE; v < numVehicles; v++) {
        if (g_vehicleInfo[v].name && Q_stricmp(g_vehicleInfo[v].name, vehicleName) == 0) { // already loaded this one
            return v;
        }
    }
    // haven't loaded it yet
    if (v >= MAX_VEHICLES) { // no more room!
...
void BG_VehicleLoadParms(void) { // HMM... only do this if there's a vehicle on the level?
    int len, totallen, vehExtFNLen, fileCnt, i;
...
int BG_VehicleGetIndex(const char *vehicleName) { return (VEH_VehicleIndexForName(vehicleName)); }
...
void AttachRidersGeneric(Vehicle_t *pVeh) {
    ...
    if (pVeh->m_pPilot) {
        ...
        // Get the driver tag.
        trap->G2API_GetBoltMatrix(parent->ghoul2, 0, crotchBolt, &boltMatrix, &yawOnlyAngles, &parent->playerState->origin, BG_GetTime(), NULL,
                                  &parent->modelScale);
