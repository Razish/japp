...
qboolean VEH_TryEject( Vehicle_t *pVeh,
	...
	if ( ent->s.number < MAX_CLIENTS ) {//for some reason, in MP, player client mins and maxs are never stored permanently, just set to these hardcoded numbers in PMove
		vEntMaxs.x = 15;
		vEntMaxs.y = 15;
	}
...
static qboolean Update( Vehicle_t *pVeh, const usercmd_t *pUmcd ) {
	...
	// Vehicle dead!
		...
		// Instant kill.
		if ( pVeh->m_pVehicleInfo->type == VH_FIGHTER &&
			pVeh->m_iLastImpactDmg > 500 ) { //explode instantly in inferno-y death
	...
	if ( parent->spawnflags & 1 ) {
		if ( pVeh->m_pPilot || !pVeh->m_bHasHadPilot ) {
			...
			if ( !oldPilot->inuse || !oldPilot->client ||
				oldPilot->client->pers.connected != CON_CONNECTED ) { //no longer in the game?
...
static qboolean UpdateRider( Vehicle_t *pVeh, bgEntity_t *pRider, usercmd_t *pUmcd ) {
	...
	if ( rider && rider->client
		&& parent && parent->client ) {//so they know who we're locking onto with our rockets, if anyone
...
int G_FlyVehicleImpactDir( gentity_t *veh, trace_t *trace ) {
	...
	if ( relativeAngle > 130 ||
		relativeAngle < -130 ) { //consider this front
...
void G_SetVehDamageFlags( gentity_t *veh, int shipSurf, int damageLevel ) {
	...
	case 3://destroyed
		...
		if ( shipSurf == SHIPSURF_BACK ) {//destroy the droid if we have one
			if ( veh->m_pVehicle
				&& veh->m_pVehicle->m_pDroidUnit ) {//we have one
				gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
				if ( droidEnt
					&& ((droidEnt->flags&FL_UNDYING) || droidEnt->health > 0) ) {//boom
					//make it vulnerable
					droidEnt->flags &= ~FL_UNDYING;
					//blow it up
	...
	case 2://heavy only
		...
		if ( shipSurf == SHIPSURF_BACK ) {//make the droid vulnerable if we have one
			if ( veh->m_pVehicle
				...
				gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
				if ( droidEnt
					&& (droidEnt->flags&FL_UNDYING) ) {//make it vulnerab;e
					droidEnt->flags &= ~FL_UNDYING;
				}
			}
		}
