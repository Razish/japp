...
void G_VehicleTrace(trace_t *results, const vector3 *start, const vector3 *tMins, const vector3 *tMaxs, const vector3 *end, int passEntityNum,
                    int contentmask) {
...
Vehicle_t *G_IsRidingVehicle(gentity_t *pEnt) {
    ...
    if (ent && ent->client && ent->client->NPC_class != CLASS_VEHICLE &&
        ent->s.m_iVehicleNum != 0) // ent->client && ( ent->client->ps.eFlags & EF_IN_VEHICLE ) && ent->owner )
    {
...
float G_CanJumpToEnemyVeh(Vehicle_t *pVeh, const usercmd_t *pUcmd) { return 0.0f; }
...
void G_VehicleSpawn(gentity_t *self) {
    ...
    if (vehEnt->spawnflags & 1) { // die without pilot
        if (!vehEnt->damage) {    // default 10 sec
            vehEnt->damage = 10000;
        }
        if (!vehEnt->speed) { // default 512 units
            vehEnt->speed = 512.0f;
        }
...
void G_AttachToVehicle(gentity_t *pEnt, usercmd_t **ucmd) {
    ...
    // Get the driver tag.
    trap->G2API_GetBoltMatrix(vehEnt->ghoul2, 0, crotchBolt, &boltMatrix, vehEnt->m_pVehicle->m_vOrientation, &vehEnt->r.currentOrigin, level.time, NULL,
                              &vehEnt->modelScale);
...
qboolean ValidateBoard(Vehicle_t *pVeh, bgEntity_t *pEnt) {
    ...
    if (pVeh->m_pPilot != NULL) {                       // already have a driver!
        ...
        } else if (pVeh->m_pVehicleInfo->type == VH_WALKER) {                          // I know, I know, this should by in the walker's validateboard()
            if (!ent->client || ent->client->ps.groundEntityNum != parent->s.number) { // can only steal an occupied AT-ST if you're on top (by the hatch)
                return qfalse;
            }
        } else if (pVeh->m_pVehicleInfo->type == VH_SPEEDER) { // you can only steal the bike from the driver if you landed on the driver or bike
...
qboolean Board(Vehicle_t *pVeh, bgEntity_t *pEnt) {
    ...
    if (!ent || parent->health <= 0 /*|| !( parent->client->ps.eFlags & EF_EMPTY_VEHICLE )*/ || (pVeh->m_iBoarding > 0) || (ent->client->ps.m_iVehicleNum))
        return qfalse;
    ...
    if (ent->s.number < MAX_CLIENTS) {
        ...
        if (!pVeh->m_pPilot) { // become the pilot, if there isn't one now
...
#ifdef PROJECT_GAME
        {
            ...
            if ((gParent->spawnflags & 2)) { // was being suspended
                ...
                if (gParent->fly_sound_debounce_time) { // we should drop like a rock for a few seconds
    ...
    if (pVeh->m_pVehicleInfo->numHands == 2) { // switch to vehicle weapon
    }

    if (pVeh->m_pVehicleInfo->hideRider) { // hide the rider
...
#ifdef VEH_CONTROL_SCHEME_4
    if (pVeh->m_pVehicleInfo->type == VH_FIGHTER) { // clear their angles
...
qboolean VEH_TryEject(Vehicle_t *pVeh, gentity_t *parent, gentity_t *ent, int ejectDir, vector3 *vExitPos) {
    float fBias;
    float fVehDiag;
    float fEntDiag;
    int oldOwner;
    vector3 vEntMins, vEntMaxs, vVehLeaveDir, vVehAngles;
    trace_t m_ExitTrace;
    ...
    case VEH_EJECT_LEFT:
        ...
        vVehLeaveDir.x = -vVehLeaveDir.x;
        vVehLeaveDir.y = -vVehLeaveDir.y;
        vVehLeaveDir.z = -vVehLeaveDir.z;
        break;
        // Right.
    ...
    fBias = 1.0f;
    if (pVeh->m_pVehicleInfo->type == VH_WALKER) { // hacktastic!
        fBias += 0.2f;
    }
    VectorCopy(&ent->r.currentOrigin, vExitPos);
    fVehDiag = sqrtf((parent->r.maxs.x * parent->r.maxs.x) + (parent->r.maxs.y * parent->r.maxs.y));
    VectorCopy(&ent->r.maxs, &vEntMaxs);
    if (ent->s.number <
        MAX_CLIENTS) { // for some reason, in MP, player client mins and maxs are never stored permanently, just set to these hardcoded numbers in PMove
        vEntMaxs.x = 15;
        vEntMaxs.y = 15;
    }
    fEntDiag = sqrtf((vEntMaxs.x * vEntMaxs.x) + (vEntMaxs.y * vEntMaxs.y));
    vVehLeaveDir.x *= (fVehDiag + fEntDiag) * fBias; // x
    vVehLeaveDir.y *= (fVehDiag + fEntDiag) * fBias; // y
    vVehLeaveDir.z *= (fVehDiag + fEntDiag) * fBias;
    ...
    if (m_ExitTrace.fraction < 1.0f) {            // not totally clear
...
qboolean Eject(Vehicle_t *pVeh, bgEntity_t *pEnt, qboolean forceEject) {
    ...
    while (!VEH_TryEject(pVeh, parent, ent, pVeh->m_EjectDir, &vExitPos)) {
        ...
        if (pVeh->m_EjectDir == firstEjectDir) { // they all failed
            if (!deadRider) {                    // if he's dead.. just shove him in solid, who cares.
                return qfalse;
            }
            if (forceEject) { // we want to always get out, just eject him here
                VectorCopy(&ent->r.currentOrigin, &vExitPos);
                break;
            } else { // can't eject
                return qfalse;
...
getItOutOfMe:
    ...
    if (taintedRider) { // you can go now
    ...
    if (ent->client->ps.weapon == WP_NONE) { // FIXME: check against this vehicle's gun from the g_vehicleInfo table
    ...
    } else { // FIXME: if they have their saber out:
...
static void DeathUpdate(Vehicle_t *pVeh) {
    ...
    if (level.time >= pVeh->m_iDieTime) {
        ...
        if (pVeh->m_pVehicleInfo->Inhabited(pVeh)) {
            ...
            if (pVeh->m_pVehicleInfo->Inhabited(pVeh)) { // if we've still got people in us, just kill the bastards
                if (pVeh->m_pPilot) {
                    // FIXME: does this give proper credit to the enemy who shot you down?
                    G_Damage((gentity_t *)pVeh->m_pPilot, (gentity_t *)pVeh->m_pParentEntity, (gentity_t *)pVeh->m_pParentEntity, NULL,
                             &pVeh->m_pParentEntity->playerState->origin, 999, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
                }
                if (pVeh->m_iNumPassengers) {
                    int i;
                    ...
                    for (i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++) {
                        if (pVeh->m_ppPassengers[i]) {
                            // FIXME: does this give proper credit to the enemy who shot you down?
                            G_Damage((gentity_t *)pVeh->m_ppPassengers[i], (gentity_t *)pVeh->m_pParentEntity, (gentity_t *)pVeh->m_pParentEntity, NULL,
                                     &pVeh->m_pParentEntity->playerState->origin, 999, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
                        }
                    }
                }
        ...
        if (!pVeh->m_pVehicleInfo->Inhabited(pVeh)) { // explode now as long as we managed to kick everyone out
            vector3 lMins, lMaxs, bottom;
            trace_t trace;
            ...
            if (pVeh->m_pVehicleInfo->explosionRadius > 0 && pVeh->m_pVehicleInfo->explosionDamage > 0) {
                ...
                lMins.z = -4; // to keep it off the ground a *little*
                VectorCopy(&parent->r.maxs, &lMaxs);
                VectorCopy(&parent->r.currentOrigin, &bottom);
                bottom.z += parent->r.mins.z - 32;
                G_VehicleTrace(&trace, &parent->r.currentOrigin, &lMins, &lMaxs, &bottom, parent->s.number, CONTENTS_SOLID);
                G_RadiusDamage(&trace.endpos, NULL, pVeh->m_pVehicleInfo->explosionDamage, pVeh->m_pVehicleInfo->explosionRadius, NULL, NULL,
                               MOD_SUICIDE); // FIXME: extern damage and radius or base on fuel
            }
...
qboolean Initialize(Vehicle_t *pVeh) {
    ...
    parent->s.m_iVehicleNum = 0;
    {
        ...
        parent->client->pers.maxHealth = parent->client->ps.stats[STAT_MAX_HEALTH] = parent->NPC->stats.health = parent->health =
    ...
    for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
        ...
        if (pVeh->m_pVehicleInfo->turret[i].bAI) { // they're going to be finding enemies, init this to NONE
    ...
    if (pVeh->m_pVehicleInfo->gravity && pVeh->m_pVehicleInfo->gravity != g_gravity.value) { // not normal gravity
        if (parent->NPC) {
    ...
    */
    ...
    // Initialize to landed (wings closed, gears down) animation
    {
        ...
        pVeh->m_ulFlags |= VEH_GEARSOPEN;
        BG_SetAnim(pVeh->m_pParentEntity->playerState, bgAllAnims[pVeh->m_pParentEntity->localAnimIndex].anims, SETANIM_BOTH, BOTH_VS_IDLE, iFlags, iBlend);
    }
...
static qboolean Update(Vehicle_t *pVeh, const usercmd_t *pUmcd) {
    ...
    for (i = 0; i < MAX_VEHICLE_WEAPONS; i++) {
        ...
        {                                                                                                               // add 1 to the ammo
            ...
            // NOTE: in order to send the vehicle's ammo info to the client, we copy the ammo into the first 2 ammo slots on the vehicle NPC's client->ps.ammo
            // array
            if (parent && parent->client) {
    ...
    if (pVeh->m_iDieTime != 0) { // NOTE!!!: This HAS to be consistent with cgame!!!
    ...
    // Vehicle dead!
        ...
        // Instant kill.
        if (pVeh->m_pVehicleInfo->type == VH_FIGHTER && pVeh->m_iLastImpactDmg > 500) { // explode instantly in inferno-y death
    ...
    if (parent->spawnflags & 1) {
        ...
        } else if (pVeh->m_iPilotTime) { // die
            gentity_t *oldPilot = &g_entities[pVeh->m_iPilotLastIndex];

            if (!oldPilot->inuse || !oldPilot->client || oldPilot->client->pers.connected != CON_CONNECTED) { // no longer in the game?
                ...
                if (VectorLength(&v) < parent->speed) { // they are still within the minimum distance to their vehicle
                    pVeh->m_iPilotTime = level.time + parent->damage;
                } else if (pVeh->m_iPilotTime < level.time) { // dying time
                    G_Damage(parent, parent, parent, NULL, &parent->client->ps.origin, 99999, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
                }
            }
        }
    ...
    if (pVeh->m_iBoarding != 0) {
        ...
        if (pilotEnt) {
            if (!pilotEnt->inuse || !pilotEnt->client || pilotEnt->health <= 0 || pilotEnt->client->pers.connected != CON_CONNECTED) {
                pVeh->m_pVehicleInfo->Eject(pVeh, pVeh->m_pPilot, qtrue);
                return qfalse;
            }
        }
    ...
    if (pVeh->m_iNumPassengers > 0) {
        ...
        for (i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++) {
            ...
            if (psngr && (!psngr->inuse || !psngr->client || psngr->health <= 0 || psngr->client->pers.connected != CON_CONNECTED)) {
                pVeh->m_pVehicleInfo->Eject(pVeh, pVeh->m_ppPassengers[i], qtrue);
                pVeh->m_iNumPassengers--;
            }
        }
    ...
    */
    ...
    for (i = 0; i < MAX_VEHICLE_WEAPONS; i++) {              // HMM... can't get a seperate command for each weapon, so do them all...?
        if (pVeh->m_pVehicleInfo->weapon[i].linkable == 2) { // always linked
            // FIXME: just set this once, on Initialize...?
            if (!pVeh->weaponStatus[i].linked) {
                pVeh->weaponStatus[i].linked = qtrue;
            }
        } else if ((pVeh->m_ucmd.buttons & BUTTON_USE_HOLDABLE)) {   // pilot pressed the "weapon link" toggle button
            if (!pVeh->linkWeaponToggleHeld)                         // so we don't hold it down and toggle it back and forth
            {                                                        // okay to toggle
                if (pVeh->m_pVehicleInfo->weapon[i].linkable == 1) { // link-toggleable
...
maintainSelfDuringBoarding:
    ...
    */
    ...
    // Shifting Sounds
    if (pVeh->m_iTurboTime < curTime && pVeh->m_iSoundDebounceTimer < curTime &&
    ...
    if (pVeh->m_pVehicleInfo->surfDestruction) {
        if (pVeh->m_iRemovedSurfaces) {
            ...
            G_Damage(parent, killer, killer, NULL, &parent->client->ps.origin, dmg,
                     DAMAGE_NO_SELF_PROTECTION | DAMAGE_NO_HIT_LOC | DAMAGE_NO_PROTECTION | DAMAGE_NO_ARMOR, MOD_SUICIDE);
        }
...
static qboolean UpdateRider(Vehicle_t *pVeh, bgEntity_t *pRider, usercmd_t *pUmcd) {
    ...
    if (rider && rider->client && parent && parent->client) { // so they know who we're locking onto with our rockets, if anyone
    ...
    if (pUmcd->buttons & BUTTON_USE && pVeh->m_pVehicleInfo->type != VH_SPEEDER) {
        if (pVeh->m_pVehicleInfo->type == VH_WALKER) { // just get the fuck out
    ...
    if (pVeh->m_pVehicleInfo->type != VH_FIGHTER && pVeh->m_pVehicleInfo->type != VH_WALKER) {
        ...
        if (pUmcd->upmove > 0) {
            ...
            if (pVeh->m_pVehicleInfo->Eject(pVeh, pRider, qfalse)) {
                ...
                rider->client->ps.velocity.z += JUMP_VELOCITY;
                rider->client->ps.fd.forceJumpZStart = rider->client->ps.origin.z;
...
static void AttachRiders(Vehicle_t *pVeh) {
    ...
    while (i < pVeh->m_iNumPassengers) {
        if (pVeh->m_ppPassengers[i]) {
            ...
            // Get the driver tag.
            trap->G2API_GetBoltMatrix(parent->ghoul2, 0, crotchBolt, &boltMatrix, &yawOnlyAngles, &parent->client->ps.origin, level.time, NULL,
                                      &parent->modelScale);
    ...
    if (pVeh->m_pDroidUnit && pVeh->m_iDroidUnitTag != -1) {
        ...
        if (droid->client) {
            ...
            // Get the droid tag.
            trap->G2API_GetBoltMatrix(parent->ghoul2, 0, pVeh->m_iDroidUnitTag, &boltMatrix, &yawOnlyAngles, &parent->r.currentOrigin, level.time, NULL,
                                      &parent->modelScale);
...
void G_VehicleDamageBoxSizing(Vehicle_t *pVeh) {
    ...
    if (!parent->ghoul2 || !parent->m_pVehicle || !parent->client) { // shouldn't have gotten in here then
        return;
    }
    ...
    if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) || !(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_D) || !(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) ||
    ...
    if (!trace.allsolid && !trace.startsolid && trace.fraction == 1.0f) { // all clear!
...
int G_FlyVehicleImpactDir(gentity_t *veh, trace_t *trace) {
    ...
    if (!localTrace.startsolid && !localTrace.allsolid && localTrace.fraction == 1.0f) { // otherwise I guess it's not clear..
        noseClear = qtrue;
    }

    if (noseClear) { // if nose is clear check for tearing the wings off
        ...
        if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) || !(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_F)) {
            ...
            if (localTrace.startsolid || localTrace.allsolid || localTrace.fraction != 1.0f) { // impact
    ...
    if (relativeAngle > 130 || relativeAngle < -130) { // consider this front
...
void G_SetVehDamageFlags(gentity_t *veh, int shipSurf, int damageLevel) {
    ...
    case 3: // destroyed
        ...
        // check droid
        if (shipSurf == SHIPSURF_BACK) {                            // destroy the droid if we have one
            if (veh->m_pVehicle && veh->m_pVehicle->m_pDroidUnit) { // we have one
                gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
                if (droidEnt && ((droidEnt->flags & FL_UNDYING) || droidEnt->health > 0)) { // boom
                    // make it vulnerable
                    droidEnt->flags &= ~FL_UNDYING;
                    // blow it up
    ...
    case 2: // heavy only
        ...
        veh->s.brokenLimbs = veh->client->ps.brokenLimbs;
        // check droid
        if (shipSurf == SHIPSURF_BACK) {                            // make the droid vulnerable if we have one
            if (veh->m_pVehicle && veh->m_pVehicle->m_pDroidUnit) { // we have one
                gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
                if (droidEnt && (droidEnt->flags & FL_UNDYING)) { // make it vulnerab;e
                    droidEnt->flags &= ~FL_UNDYING;
                }
            }
        }
...
void G_VehicleSetDamageLocFlags(gentity_t *veh, int impactDir, int deathPoint) {
    if (!veh->client) {
        ...
        if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo && veh->m_pVehicle->m_pVehicleInfo->malfunctionArmorLevel &&
        ...
        if (veh->locationDamage[impactDir] >= deathPoint) { // destroyed
            G_SetVehDamageFlags(veh, impactDir, 3);
        } else if (veh->locationDamage[impactDir] <= lightDamagePoint) { // light only
            G_SetVehDamageFlags(veh, impactDir, 1);
        } else if (veh->locationDamage[impactDir] <= heavyDamagePoint) { // heavy only
...
qboolean G_FlyVehicleDestroySurface(gentity_t *veh, int surface) {
    ...
    if (numSurfs < 1) { // didn't get any valid surfs..
        return qfalse;
    }

    while (numSurfs > 0) { // use my silly system of automatically managing surf status on both client and server
        numSurfs--;
        NPC_SetSurfaceOnOff(veh, surfName[numSurfs], TURN_OFF);
    }

    if (!veh->m_pVehicle->m_iRemovedSurfaces) { // first time something got blown off
        if (veh->m_pVehicle->m_pPilot) {        // make the pilot scream to his death
...
void G_FlyVehicleSurfaceDestruction(gentity_t *veh, trace_t *trace, int magnitude, qboolean force) {
    ...
    if (!veh->ghoul2 || !veh->m_pVehicle) { // no g2 instance.. or no vehicle instance
        return;
    }
...
anotherImpact:
    ...
    if (deathPoint != -1) {                                         // got a valid health value
        if (force && veh->locationDamage[impactDir] < deathPoint) { // force that surf to be destroyed
            veh->locationDamage[impactDir] = deathPoint;
        }
        if (veh->locationDamage[impactDir] >= deathPoint) {   // do it
            if (G_FlyVehicleDestroySurface(veh, impactDir)) { // actually took off a surface
    ...
    if (!alreadyRebroken) {
        ...
        if (impactDir != secondImpact) { // can break off another piece in this same impact.. but only break off up to 2 at once
...
void G_VehUpdateShields(gentity_t *targ) {
    ...
    if (targ->m_pVehicle->m_pVehicleInfo->shields <= 0) { // doesn't have shields, so don't have to send it
        return;
    }
