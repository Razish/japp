...
static void GM_CheckMoveState( void ) {
	...
	if ( (NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL) ) {
		...
		if ( NAV_HitNavGoal( &NPC->r.currentOrigin, &NPC->r.mins, &NPC->r.maxs, &NPCInfo->goalEntity->r.currentOrigin, 16, qfalse ) ||
			(!trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPC, TID_MOVE_NAV ) && enemyLOS4 && enemyDist4 <= 10000) ) {//either hit our navgoal or our navgoal was not a crucial (scripted) one (maybe a combat point) and we're scouting and found our enemy
			NPC_ReachedGoal();
...
void NPC_BSGM_Attack( void ) {
	...
	{//sometimes use the laser beam attack, but only after he's taken down our generator
		...
		{//enemy within 256
			if ( (NPC->client->ps.weapon == WP_REPEATER) && (NPCInfo->scriptFlags & SCF_ALT_FIRE) ) {//shooting an explosive, but enemy too close, switch to primary fire
				NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
				NPC->alt_fire = qfalse;
	...
	if ( NPC->client->ps.weapon == WP_REPEATER && (NPCInfo->scriptFlags&SCF_ALT_FIRE) && shoot4 && TIMER_Done( NPC, "attackDelay" ) ) {
		...
		clearshot = WP_LobFire( NPC, &muzzle, &target, &mins, &maxs, MASK_SHOT | CONTENTS_LIGHTSABER,
			...
			300, 1100, 1500, qtrue );
		if ( VectorCompare( &vec3_origin, &velocity ) || (!clearshot&&enemyLOS4&&enemyCS4) ) {//no clear lob shot and no lob shot that will hit something breakable
