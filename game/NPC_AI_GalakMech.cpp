...
qboolean WP_LobFire(gentity_t *self, vector3 *start, vector3 *target, vector3 *mins, vector3 *maxs, int clipmask, vector3 *velocity, qboolean tracePath,
                    int ignoreEntNum, int enemyNum, float minSpeed, float maxSpeed, float idealSpeed, qboolean mustHit);
...
static void GM_CreateExplosion(gentity_t *self, const int boltID, qboolean doSmall) {
    if (boltID >= 0) {
        ...
        trap->G2API_GetBoltMatrix(self->ghoul2, 0, boltID, &boltMatrix, &self->r.currentAngles, &self->r.currentOrigin, level.time, NULL, &self->modelScale);
...
void GM_Dying(gentity_t *self) {
    if (level.time - self->s.time < 4000) { // FIXME: need a real effect
        ...
        if (TIMER_Done(self, "dyingExplosion")) {
            ...
            case 1:
                if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "r_hand")) { // r_hand still there
                    GM_CreateExplosion(self, trap->G2API_AddBolt(self->ghoul2, 0, "*flasha"), qtrue);
                    NPC_SetSurfaceOnOff(self, "r_hand", TURN_OFF);
                } else if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "r_arm_middle")) { // r_arm_middle still there
            ...
            case 2:
                // FIXME: do only once?
                if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "l_hand")) { // l_hand still there
                    GM_CreateExplosion(self, trap->G2API_AddBolt(self->ghoul2, 0, "*flashc"), qfalse);
                    NPC_SetSurfaceOnOff(self, "l_hand", TURN_OFF);
                } else if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "l_arm_wrist")) { // l_arm_wrist still there
                    trap->G2API_AddBolt(self->ghoul2, 0, "*l_arm_cap_l_hand");
                    NPC_SetSurfaceOnOff(self, "l_arm_wrist", TURN_OFF);
                } else if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "l_arm_middle")) { // l_arm_middle still there
                    trap->G2API_AddBolt(self->ghoul2, 0, "*l_arm_cap_l_hand");
                    NPC_SetSurfaceOnOff(self, "l_arm_middle", TURN_OFF);
                } else if (!trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "l_arm_augment")) { // l_arm_augment still there
...
void NPC_GM_Pain(gentity_t *self, gentity_t *attacker, int damage) {
    ...
    if (!self->lockCount && self->client->ps.torsoTimer <= 0) { // don't interrupt laser sweep attack or other special attacks/moves
    ...
    if (inflictor && inflictor->lastEnemy == self) { // He force-pushed my own lobfires back at me
        if (mod == MOD_REPEATER_ALT && !Q_irand(0, 2)) {
            ...
            } else { // hopefully this will make us fire the laser
...
static void GM_CheckMoveState(void) {
    ...
    if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
        ...
        if (NAV_HitNavGoal(&NPC->r.currentOrigin, &NPC->r.mins, &NPC->r.maxs, &NPCInfo->goalEntity->r.currentOrigin, 16, qfalse) ||
            (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV) && enemyLOS4 &&
             enemyDist4 <= 10000)) { // either hit our navgoal or our navgoal was not a crucial (scripted) one (maybe a combat point) and we're scouting and
                                     // found our enemy
            NPC_ReachedGoal();
    ...
    if (!hitAlly4 && NPCInfo->enemyLastSeenTime > 0) {
        if (level.time - NPCInfo->enemyLastSeenTime < 10000) {
            if (!Q_irand(0, 10)) {
                ...
                if (VectorCompare(&impactPos4, &vec3_origin)) { // never checked ShotEntity this frame, so must do a trace...
                    trace_t tr;
                ...
                if (dist < distThreshold) { // impact would be too close to me
                    tooClose = qtrue;
                } else if (level.time - NPCInfo->enemyLastSeenTime > 5000) { // we've haven't seen them in the last 5 seconds
...
void NPC_GM_StartLaser(void) {
    if (!NPC->lockCount) { // haven't already started a laser attack
...
void NPC_BSGM_Attack(void) {
	...
	if ( NPC->client->ps.torsoAnim == BOTH_ATTACK4 || NPC->client->ps.torsoAnim == BOTH_ATTACK5 ) {
		...
		if ( TIMER_Done( NPC, "smackTime" ) && !NPCInfo->blockedDebounceTime ) {//time to smack
			...
			if ( enemyDist4 < MELEE_DIST_SQUARED && InFront( &NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, 0.3f ) ) {
				...
				if ( NPC->client->ps.torsoAnim == BOTH_ATTACK4 ) {//smackdown
					int knockAnim = BOTH_KNOCKDOWN1;
					if ( BG_CrouchAnim( NPC->enemy->client->ps.legsAnim ) ) {//knockdown from crouch
    ...
    { // sometimes use the laser beam attack, but only after he's taken down our generator
        ...
        if (NPC->lockCount == 1) {              // charging up
            if (TIMER_Done(NPC, "beamDelay")) { // time to start the beam
                int laserAnim;
                // if ( Q_irand( 0, 1 ) )
                if (1) {
                ...
                */
                ...
                if (!NPCInfo->coverTarg) { // for moving looping sound at end of trace
        ...
        } else {                                   // in the actual attack now
            if (NPC->client->ps.torsoTimer <= 0) { // attack done!
                NPC->lockCount = 0;
            ...
            } else { // attack still going
                // do the trace and damage
                trace_t trace;
                vector3 end, mins = {-3, -3, -3}, maxs = {3, 3, 3};
                VectorMA(&NPC->client->renderInfo.muzzlePoint, 1024, &NPC->client->renderInfo.muzzleDir, &end);
                trap->Trace(&trace, &NPC->client->renderInfo.muzzlePoint, &mins, &maxs, &end, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
                if (trace.allsolid || trace.startsolid) { // oops, in a wall
                    if (NPCInfo->coverTarg) {
                        G_SetOrigin(NPCInfo->coverTarg, &NPC->client->renderInfo.muzzlePoint);
                    }
                } else {                         // clear
                    if (trace.fraction < 1.0f) { // hit something
                        ...
                        if (traceEnt && traceEnt->takedamage) { // damage it
    ...
    else { // Okay, we're not in a special attack, see if we should switch weapons or start a special attack
        /*
        if ( NPC->s.weapon == WP_REPEATER
        ...
        if (  // !NPC->client->ps.powerups[PW_GALAK_SHIELD]
            ...
            && enemyDist4 < MELEE_DIST_SQUARED && InFront(&NPC->enemy->r.currentOrigin, &NPC->r.currentOrigin, &NPC->client->ps.viewangles, 0.3f) &&
        ...
        {                                    // our shield is down, and enemy within 80, if very close, use melee attack to slap away
        ...
        } else if (!NPC->lockCount && NPC->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH && TIMER_Done(NPC, "attackDelay") &&
            ...
            shoot4 = qfalse;
            NPC_GM_StartLaser();
        } else if (enemyDist4 < MIN_LOB_DIST_SQUARED && (NPC->enemy->s.weapon != WP_TURRET || Q_stricmp("PAS", NPC->enemy->classname)) &&
        ...
        {                                    // enemy more than 448 away and we are ready to try lob fire again
            if ((NPC->client->ps.weapon == WP_REPEATER) && !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // enemy far enough away to use lobby explosives
                NPCInfo->scriptFlags |= SCF_ALT_FIRE;
                NPC->alt_fire = qtrue;
                // FIXME: use weap raise & lower anims
                NPC_ChangeWeapon(WP_REPEATER);
        ...
        if (NPC->client->ps.weapon == WP_NONE) {
            ...
            if (((NPC->client->ps.weapon == WP_REPEATER && (NPCInfo->scriptFlags & SCF_ALT_FIRE))) && enemyDist4 < MIN_LOB_DIST_SQUARED) // 256
            {
                ...
                if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) ||
                    (hitEnt && hitEnt->takedamage)) { // can hit enemy or will hit glass or other breakable, so shoot anyway
                    enemyCS4 = qtrue;
    ...
    if (enemyLOS4) {
        ...
        if (NPCInfo->goalEntity == NPC->enemy) { // for now, always chase the enemy
            move4 = qtrue;
        }
    }
    if (enemyCS4) {
    ...
    if (NPC->client->ps.weapon == WP_REPEATER && (NPCInfo->scriptFlags & SCF_ALT_FIRE) && shoot4 && TIMER_Done(NPC, "attackDelay")) {
        ...
        vector3 mins = {-REPEATER_ALT_SIZE, -REPEATER_ALT_SIZE, -REPEATER_ALT_SIZE}, maxs = {REPEATER_ALT_SIZE, REPEATER_ALT_SIZE, REPEATER_ALT_SIZE};
        qboolean clearshot;
        ...
        target.x += flrand(-5, 5) + (crandom() * (6 - NPCInfo->currentAim) * 2);
        target.y += flrand(-5, 5) + (crandom() * (6 - NPCInfo->currentAim) * 2);
        target.z += flrand(-5, 5) + (crandom() * (6 - NPCInfo->currentAim) * 2);

        // Find the desired angles
        clearshot = WP_LobFire(NPC, &muzzle, &target, &mins, &maxs, MASK_SHOT | CONTENTS_LIGHTSABER, &velocity, qtrue, NPC->s.number, NPC->enemy->s.number, 300,
                               1100, 1500, qtrue);
        if (VectorCompare(&vec3_origin, &velocity) ||
            (!clearshot && enemyLOS4 && enemyCS4)) {                   // no clear lob shot and no lob shot that will hit something breakable
            if (enemyLOS4 && enemyCS4 && TIMER_Done(NPC, "noRapid")) { // have a clear straight shot, so switch to primary
                NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
                NPC->alt_fire = qfalse;
                NPC_ChangeWeapon(WP_REPEATER);
    ...
    } else if (faceEnemy4) { // face the enemy
        NPC_FaceEnemy(qtrue);
    }
    ...
    if (move4 && !NPC->lockCount) { // move toward goal
        if ( NPCInfo->goalEntity
    ...
    if (!faceEnemy4) { // we want to face in the dir we're running
        if (!move4) {  // if we haven't moved, we should look in the direction we last looked?
            VectorCopy(&NPC->client->ps.viewangles, &NPCInfo->lastPathAngles);
        }
        if (move4) { // don't run away and shoot
    ...
    if (shoot4) { // try to shoot if it's time
        if (TIMER_Done(NPC, "attackDelay")) {
    ...
    if (NPC->enemy && NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp("PAS", NPC->enemy->classname)) { // crush turrets
        if (G_BoundsOverlap(&NPC->r.absmin, &NPC->r.absmax, &NPC->enemy->r.absmin,
                            &NPC->enemy->r.absmax)) { // have to do this test because placed turrets are not solid to NPCs (so they don't obstruct navigation)
#if 0
...
void NPC_BSGM_Default(void) {
    ...
    if (NPC->client->ps.stats[STAT_ARMOR] <= 0) { // armor gone
        //	if ( !NPCInfo->investigateDebounceTime )
        if (0) { // start regenerating the armor
            ...
            if (NPC->locationDamage[HL_GENERIC1] < GENERATOR_HEALTH) { // still have the generator bolt-on
        ...
        } else if (NPCInfo->investigateDebounceTime < level.time) { // armor regenerated, turn shield back on
            // do a trace and make sure we can turn this back on?
            trace_t tr;
            trap->Trace(&tr, &NPC->r.currentOrigin, &shieldMins, &shieldMaxs, &NPC->r.currentOrigin, NPC->s.number, NPC->clipmask, qfalse, 0, 0);
            if (!tr.startsolid) {
                VectorCopy(&shieldMins, &NPC->r.mins);
                VectorCopy(&shieldMaxs, &NPC->r.maxs);
                NPC->client->ps.crouchheight = NPC->client->ps.standheight = shieldMaxs.z;
