...
const char *weaponNameFromIndex[WP_NUM_WEAPONS] = {"No Weapon", "Stun Baton", "Saber",        "Bryar Pistol", "Blaster",         "Disruptor",
                                                   "Bowcaster", "Repeater",   "Demp2",        "Flechette",    "Rocket Launcher", "Thermal",
                                                   "Tripmine",  "Detpack",    "Emplaced gun", "Turret"};
...
void G_LogWeaponOutput(void) {
    ...
    G_LogPrintf(level.log.console, "*****************************Weapon Log:\n");
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        if (G_WeaponLogClientTouch[i]) { // Ignore any entity/clients we don't care about!
    ...
    for (j = 0; j < WP_NUM_WEAPONS; j++) {
        G_LogPrintf(level.log.console, "%15s:  Pickups: %4d,  Time:  %5d,  Deaths: %5d\n", weaponNameFromIndex[j], totalpickups[j], (int)(totaltime[j] / 1000),
                    totaldeaths[j]);
    }
    ...
    for (j = 0; j < WP_NUM_WEAPONS; j++) {
        ...
        G_LogPrintf(level.log.console, "%15s:  Damage: %6d,  Kills: %5d,  Dmg per Shot: %f\n", weaponNameFromIndex[j], totaldamage[j], totalkills[j], (double)pershot);
    }

    G_LogPrintf(level.log.console, "\n****Combat Data By Damage Type:\n");
    for (j = 0; j < MOD_MAX; j++) {
        G_LogPrintf(level.log.console, "%25s:  Damage: %6d,  Kills: %5d\n", modNames[j], totaldamageMOD[j], totalkillsMOD[j]);
    }
...
#ifdef LOGGING_WEAPONS
    int i = 0, nBestPlayer = -1, nKills = 0, nMostKills = 0, playTime = (level.time - ent->client->pers.enterTime) / 60000;
    gentity_t *player = NULL;
...
#ifdef LOGGING_WEAPONS
    int i = 0, j = 0, nBestPlayer = -1, nStuffUsed = 0, nMostStuffUsed = 0, nDifferent = 0, nMostDifferent = 0;
    gentity_t *player = NULL;
...
qboolean CalculateTeamMVP(gentity_t *ent) {
    int i = 0, nBestPlayer = -1, nScore = 0, nHighestScore = 0, team = ent->client->ps.persistant[PERS_TEAM];
    gentity_t *player = NULL;
...
qboolean CalculateTeamMVPByRank(gentity_t *ent) {
    int i = 0, nBestPlayer = -1, nScore = 0, nHighestScore = 0, team = ent->client->ps.persistant[PERS_RANK] + 1;
...
int CalculateTeamAward(gentity_t *ent) {
    ...
    if (GT_CTF == level.gametype || GT_CTY == level.gametype) {
    ...
    if (!teamAwards && CalculateTeamRedShirt(ent)) { // if you got nothing else and died a lot, at least get bravery
...
qboolean CalculateSection31Award(gentity_t *ent) {
    ...
    for (i = 0; i < sv_maxclients.integer; i++) {
        ...
        if (!CalculateSharpshooter(ent, &frags) || !CalculateUntouchable(ent) ||
...
int GetWorstEnemyForClient(int nClient) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (G_WeaponLogFrags[i][nClient] > nMostDeaths || (G_WeaponLogFrags[i][nClient] == nMostDeaths && i != nClient && nMostDeaths != 0)) {
            nMostDeaths = G_WeaponLogFrags[i][nClient];
            nWorstEnemy = i;
        }
    }
