    ...
    if (dying) {
        ...
        */
        ...
        if (!(self->spawnflags & SPF_TURRETG2_TURBO)) {
            ...
            // set the new onw
            trap->G2API_InitGhoul2Model(&self->ghoul2, name,
                                        0, // base->s.modelindex,
                                        // note, this is not the same kind of index - this one's referring to the actual
                                        // index of the model in the g2 instance, whereas modelindex is the index of a
                                        // configstring -rww
            ...
            // set the new onw
            trap->G2API_InitGhoul2Model(&self->ghoul2, name3,
                                        0, // base->s.modelindex,
                                        // note, this is not the same kind of index - this one's referring to the actual
                                        // index of the model in the g2 instance, whereas modelindex is the index of a
                                        // configstring -rww
        ...
        if ((self->spawnflags & SPF_TURRETG2_TURBO)) { // larger
            self->s.g2radius = 128;
        } else {
            self->s.g2radius = 80;
        }

        if ((self->spawnflags & SPF_TURRETG2_TURBO)) { // different pitch bone and muzzle flash points
...
void turretG2_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
    ...
    if (self->s.modelindex2) {
        ...
        if (self->spawnflags & SPF_TURRETG2_CANRESPAWN) {   // respawn
...
void turretG2_head_think(gentity_t *self) {
    ...
    if (self->enemy && self->setTime < level.time && self->attackDebounceTime < level.time) {
        ...
        trap->G2API_GetBoltMatrix(self->ghoul2, 0, (self->alt_fire ? self->genericValue12 : self->genericValue11), &boltMatrix, &self->r.currentAngles,
...
static void turretG2_aim(gentity_t *self) {
    ...
    if (self->enemy) {
        ...
        if ((self->spawnflags & SPF_TURRETG2_LEAD_ENEMY)) { // we want to lead them a bit
        ...
        trap->G2API_GetBoltMatrix(self->ghoul2, 0, (self->alt_fire ? self->genericValue12 : self->genericValue11), &boltMatrix, &self->r.currentAngles,
                                  &self->s.origin, level.time, NULL, &self->modelScale);
    ...
    if (diffYaw || diffPitch) { // FIXME: turbolaser sounds
        if ((self->spawnflags & SPF_TURRETG2_TURBO))
...
static qboolean turretG2_find_enemies(gentity_t *self) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (!target->client) {
            ...
            if (!(target->flags & FL_BBRUSH) // not a breakable brush
                ...
                || (target->NPC_targetname && self->targetname &&
                    Q_stricmp(target->NPC_targetname, self->targetname) != 0)) // not in invicible bbrush, but can only be broken by an NPC that is not me
            {
                continue;
            }
        ...
        if (target->client && (target->client->sess.sessionTeam == TEAM_SPECTATOR || target->client->tempSpectate >= level.time)) {
            continue;
        }
        if (self->alliedTeam) {
            if (target->client) {
...
void turretG2_base_think(gentity_t *self) {
    ...
    if (self->health <= 0) {                                               // dead
        if (self->spawnflags & SPF_TURRETG2_CANRESPAWN) {                  // can respawn
            if (self->genericValue5 && self->genericValue5 < level.time) { // we are dead, see if it's time to respawn
                turretG2_respawn(self);
            }
        }
        return;
    } else if (self->spawnflags & 1) { // not turned on
    ...
    if (self->enemy) {
        if (self->enemy->health < 0 || !self->enemy->inuse) {
            self->enemy = NULL;
        }
    }

    if (self->last_move_time < level.time) { // MISNOMER: used a enemy recalcing debouncer
        if (turretG2_find_enemies(self)) {   // found one
            turnOff = qfalse;
            if (self->enemy->client) { // hold on to clients for a min of 3 seconds
    ...
    if (self->enemy != NULL) {
        if (self->enemy->client &&
            (self->enemy->client->sess.sessionTeam == TEAM_SPECTATOR || self->enemy->client->tempSpectate >= level.time)) { // don't keep going after spectators
            self->enemy = NULL;
        } else { // FIXME: remain single-minded or look for a new enemy every now and then?
            // enemy is alive
...
void SP_misc_turretG2(gentity_t *base) {
    ...
    if (base->s.iModelScale) {
        ...
        base->modelScale.x = base->modelScale.y = base->modelScale.z = base->s.iModelScale / 100.0f;
    }
...
void finish_spawning_turretG2(gentity_t *base) {
    ...
    if ((base->spawnflags & SPF_TURRETG2_TURBO)) {
        if (!base->random) { // error worked into projectile direction
            base->random = 2.0f;
        }

        if (!base->mass) { // misnomer: speed of projectile
            base->mass = 20000;
        }
        ...
        if (level.gametype == GT_SIEGE) { // FIXME: designer-specified?
        ...
        if (base->spawnflags & 2) { // upside-down, invert r.mins and maxe
    ...
    if (base->s.iModelScale) { // let's scale the bbox too...
