...
const stringID_table_t ClassTable[] = {ENUM2STRING(CLASS_NONE), // hopefully this will never be used by an npc), just covering all bases
...
stringID_table_t BSETTable[] = {ENUM2STRING(BSET_SPAWN),     //# script to use when first spawned
...
const char *TeamNames[TEAM_NUM_TEAMS] = {"", "player", "enemy", "neutral"};

// this list was made using the model directories, this MUST be in the same order as the CLASS_ enum in teams.h
const char *ClassNames[CLASS_NUM_CLASSES] = {
    "", // class none
    "atst",  "bartender",    "bespin_cop", "claw",        "commando",     "desann",       "fish",     "flier2",  "galak",    "glider",        "gonk",
    "gran",  "howler",       "imperial",   "impworker",   "interrogator", "jan",          "jedi",     "kyle",    "lando",    "lizard",        "luke",
    "mark1", "mark2",        "galak_mech", "minemonster", "monmotha",     "morgankatarn", "mouse",    "murjj",   "prisoner", "probe",         "protocol",
    "r2d2",  "r5d2",         "rebel",      "reborn",      "reelo",        "remote",       "rodian",   "seeker",  "sentry",   "shadowtrooper", "stormtrooper",
    "swamp", "swamptrooper", "tavion",     "trandoshan",  "ugnaught",     "weequay",      "bobafett", "vehicle", "rancor",   "wampa",
};
...
int NPC_ReactionTime(void) { return 200 * (6 - NPCInfo->stats.reactions); }
...
void NPC_Precache(gentity_t *spawner) {
    ...
    if (!Q_stricmp("random", spawner->NPC_type)) { // sorry, can't precache a random just yet
        return;
    }
    ...
    if (!spawner->client || spawner->client->NPC_class != CLASS_VEHICLE) {
        if (md3Model) {
        ...
        } else { // if we have a model/skin then index them so they'll be registered immediately
            ...
            if (customSkin[0]) { // append it after a *
...
qboolean NPC_ParseParms(const char *NPCName, gentity_t *NPC) {
    ...
    if (!NPC->s.number && NPC->client != NULL) { // player, only want certain data
        parsingPlayer = qtrue;
    }
    ...
    */
    ...
    if (!Q_stricmp("random", NPCName)) { // Randomly assemble a starfleet guy
        // NPC_BuildRandom( NPC );
        ...
        while (1) {
            ...
            if (!Q_stricmp(token, "scale")) {
                ...
                if (n != 100) {
                    ...
                    NPC->modelScale.x = NPC->modelScale.y = NPC->modelScale.z = n / 100.0f;
                }
                continue;
            }
            ...
            if (!Q_stricmp(token, "class")) {
                ...
                if (NPC->client->NPC_class == CLASS_VEHICLE) {
                    if (!NPC->m_pVehicle) { // you didn't spawn this guy right!
                        Com_Printf(S_COLOR_RED "ERROR: Tried to spawn a vehicle NPC (%s) without using NPC_Vehicle or 'NPC spawn vehicle <vehiclename>'!!!  "
            ...
            if (!Q_stricmp(token, "width")) {
                ...
                NPC->r.mins.x = NPC->r.mins.y = -n;
                NPC->r.maxs.x = NPC->r.maxs.y = n;
                continue;
            }
            ...
            if (!Q_stricmp(token, "height")) {
                ...
                if (NPC->client->NPC_class == CLASS_VEHICLE && NPC->m_pVehicle && NPC->m_pVehicle->m_pVehicleInfo &&
                    NPC->m_pVehicle->m_pVehicleInfo->type ==
                        VH_FIGHTER) { // a flying vehicle's origin must be centered in bbox and it should spawn on the ground
                    // trace_t		tr;
                    // vector3		bottom;
                    // float		adjust = 32.0f;
                    NPC->r.maxs.z = NPC->client->ps.standheight = (n / 2.0f);
                    NPC->r.mins.z = -NPC->r.maxs.z;
                    NPC->s.origin.z += (DEFAULT_MINS_2 - NPC->r.mins.z) + 0.125f;
                    ...
                    bottom[2] -= adjust;
                    trap->Trace( &tr, NPC->s.origin, NPC->r.mins, NPC->r.maxs, bottom, NPC->s.number, MASK_NPCSOLID, qfalse, 0, 0 );
                    ...
                    */
                    ...
                    NPC->r.mins.z = DEFAULT_MINS_2; // Cannot change
                    NPC->r.maxs.z = NPC->client->ps.standheight = n + DEFAULT_MINS_2;
                }
                NPC->radius = n;
                continue;
            }
            ...
            if (!Q_stricmp(token, "saber2")) {
                ...
                if (!(NPC->client->saber[0].saberFlags & SFL_TWO_HANDED)) { // can't use a second saber if first one is a two-handed saber...?
                    char *saberName = (char *)BG_TempAlloc(4096);           // G_NewString( value );
                    strcpy(saberName, value);

                    WP_SaberParseParms(saberName, &NPC->client->saber[1]);
                    if ((NPC->client->saber[1].saberFlags & SFL_TWO_HANDED)) { // tsk tsk, can't use a twoHanded saber as second saber
    ...
    */
    if (!md3Model) {
        ...
        if (npcSaber1 == 0) { // use "kyle" for a default then
        ...
        if (NPC->client && NPC->client->NPC_class == CLASS_VEHICLE) { // vehicles want their names fed in as models
        ...
        if (!NPC->NPC_type) { // just do this for now so NPC_Precache can see the name.
        ...
        if (setTypeBack) { // don't want this being set if we aren't ready yet.
            NPC->NPC_type = NULL;
        }
    } else {
...
int classcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((const precache_t *)b)->classname); }
