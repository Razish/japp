...
void MineMonster_TryDamage(gentity_t *enemy, int damage) {
    ...
    trap->Trace(&tr, &NPC->r.currentOrigin, &vec3_origin, &vec3_origin, &end, NPC->s.number, MASK_SHOT, qfalse, 0, 0);
...
void MineMonster_Attack(void) {
    if (!TIMER_Exists(NPC, "attacking")) {
        ...
        if (NPC->enemy && ((NPC->enemy->r.currentOrigin.z - NPC->r.currentOrigin.z > 10 && random() > 0.1f) || random() > 0.8f)) {
            // Going to do ATTACK4
