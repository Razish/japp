...
typedef struct gameImport_s {
    ...
    int (*EntitiesInBox)(const vector3 *mins, const vector3 *maxs, int *list, int maxcount);
    qboolean (*EntityContact)(const vector3 *mins, const vector3 *maxs, const sharedEntity_t *ent, int capsule);
    ...
    int (*ICARUS_GetVectorVariable)(const char *name, const vector3 *value);
    ...
    int (*Nav_GetProjectedNode)(vector3 *origin, int nodeID);
    ...
    void (*BotInitialChat)(int chatstate, char *type, int mcontext, char *var0, char *var1, char *var2, char *var3, char *var4, char *var5, char *var6,
                           char *var7);
    int (*BotReplyChat)(int chatstate, char *message, int mcontext, int vcontext, char *var0, char *var1, char *var2, char *var3, char *var4, char *var5,
                        char *var6, char *var7);
    ...
    int (*AAS_PredictClientMovement)(void *move, int entnum, vector3 *origin, int presencetype, int onground, vector3 *velocity, vector3 *cmdmove,
                                     int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize);
    int (*AAS_AlternativeRouteGoals)(vector3 *start, int startareanum, vector3 *goal, int goalareanum, uint32_t travelflags, void *altroutegoals,
                                     int maxaltroutegoals, int type);
    int (*AAS_PredictRoute)(void *route, int areanum, vector3 *origin, int goalareanum, uint32_t travelflags, int maxareas, int maxtime, int stopevent,
...
typedef struct gameExport_s {
    ...
    qboolean (*NAV_ClearPathToPoint)(int entID, vector3 *pmins, vector3 *pmaxs, vector3 *point, int clipmask, int okToHitEnt);
    qboolean (*NPC_ClearLOS2)(int entID, const vector3 *end);
