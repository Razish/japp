...
static bool AM_CanInflict(const gentity_t *entInflicter, const gentity_t *entVictim) {
    ...
    if (inflicter && inflicter->rank == victim->rank) {
        if (japp_passRankConflicts.integer) {
            G_LogPrintf(level.log.console, "%s (User: %s) (Rank: %d) inflicting command on lower ranked player %s (User: %s) (Rank: %d)",
...
static void AM_SaveTelemarksCmd(gentity_t *ent) { AM_SaveTelemarks(); }
...
static void AM_Protect(gentity_t *ent) {
    ...
    G_LogPrintf(level.log.admin, "\t%s %sprotected %s\n", G_PrintClient(ent), !!(targ->client->ps.eFlags & EF_INVULNERABLE) ? "" : "un",
...
static void AM_GunProtect(gentity_t *ent) {
    ...
    if (tr->entityNum >= 0 && tr->entityNum < MAX_CLIENTS) {
        ...
        G_LogPrintf(level.log.admin, "\t%s %sprotected %s\n", G_PrintClient(ent), !!(e->client->ps.eFlags & EF_INVULNERABLE) ? "" : "un",
...
static void AM_Silence(gentity_t *ent) {
    ...
    if (atoi(arg1) == -1) {
        ...
        gentity_t *e = NULL;
        gclient_t *cl = NULL;
        for (i = 0, e = g_entities, cl = level.clients; i < level.maxclients; i++, e++, cl++) {
...
static qboolean Can_Slay(gentity_t *ent) {
    if (ent->client->pers.connected == CON_DISCONNECTED || ent->client->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR || ent->client->tempSpectate >= level.time ||
        ent->client->pers.adminData.isSlept || ent->health <= 0) {
        return qfalse;
    }
    return qtrue;
}
...
static const char *weatherEffects[] = {
    "acidrain",  "clear",       "constantwind", "die",  "fog",  "freeze", "gustingwind", "heavyrain", "heavyrainfog", "light_fog",
    "lightrain", "outsidepain", "outsideshake", "rain", "sand", "snow",   "spacedust",   "wind",      "zone",
};
static const size_t numWeatherEffects = ARRAY_LEN(weatherEffects);

static int weathercmp(const void *a, const void *b) { return Q_stricmp((const char *)a, *(const char **)b); }
...
static void AM_Give(gentity_t *ent) {
    ...
    trap->Argv(1, arg1, sizeof(arg1));
    if ((!Q_stricmp(arg1, "weapon") || !Q_stricmp(arg1, "force") || !Q_stricmp(arg1, "ammo")) && ent)
        shift = qtrue;
...
static int cmdcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((adminCommand_t *)b)->cmd); }
...
qboolean AM_HasPrivilege(const gentity_t *ent, uint64_t privilege) {
    ...
    if (user && (user->privileges & privilege) == privilege) {
        return qtrue;
