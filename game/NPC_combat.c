...
qboolean G_TeamEnemy( gentity_t *self ) {//FIXME: Probably a better way to do this, is a linked list of your teammates already available?
	...
	for ( i = 1; i < level.num_entities; i++ ) {
		...
		if ( ent->enemy ) {//they have an enemy
			if ( !ent->enemy->client || ent->enemy->client->playerTeam != self->client->playerTeam ) {//the ent's enemy is either a normal ent or is a player/NPC that is not on my team
				return qtrue;
			}
		}
	}
...
gentity_t *NPC_PickEnemy( gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst, qboolean findClosest ) {
	...
	if ( NPCInfo->behaviorState == BS_STAND_AND_SHOOT ||
		NPCInfo->behaviorState == BS_HUNT_AND_KILL ) {//Formations guys don't require inFov to pick up a target
...
gentity_t *NPC_CheckEnemy( qboolean findNew, qboolean tooFarOk, qboolean setEnemy ) {
	...
	*/
	...
	if ( !NPC->enemy || (NPC->enemy && NPC->enemy->health <= 0) || forcefindNew ) {//FIXME: NPCs that are moving after an enemy should ignore the can't hit enemy counter- that should only be for NPCs that are standing still
...
qboolean NPC_EvaluateShot( int hit, qboolean glassOK ) {
	...
	if ( hit == NPC->enemy->s.number || (&g_entities[hit] != NULL && (g_entities[hit].r.svFlags&SVF_GLASS_BRUSH)) ) {//can hit enemy or will hit glass, so shoot anyway
		return qtrue;
	}
	return qfalse;
}
	...
	for ( j = 0; j < numPoints; j++ ) {
		...
		if ( flags & CP_HAS_ROUTE ) {
			...
			*/
			...
			if ( waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE || trap->Nav_GetBestNodeAltRoute2( waypoint, level.combatPoints[i].waypoint, NODE_NONE ) == WAYPOINT_NONE ) {//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
				if ( !NAV_ClearPathToPoint( NPC, &NPC->r.mins, &NPC->r.maxs, &level.combatPoints[i].origin, NPC->clipmask, ENTITYNUM_NONE ) ) {//don't even have a clear straight path to this one
					continue;
				}
			}
		}
			...
			if ( trap->InPVS( &found->r.currentOrigin, &NPC->r.currentOrigin ) ) {
				...
				if ( dist < bestDist ) {
					if ( !trap->Nav_GetBestPathBetweenEnts( (sharedEntity_t *)NPC, (sharedEntity_t *)found, NF_CLEAR_PATH )
						|| trap->Nav_GetBestNodeAltRoute2( NPC->waypoint, found->waypoint, NODE_NONE ) == WAYPOINT_NONE ) {//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
						if ( NAV_ClearPathToPoint( NPC, &NPC->r.mins, &NPC->r.maxs, &found->r.currentOrigin, NPC->clipmask, ENTITYNUM_NONE ) ) {//have a clear straight path to this one
							bestDist = dist;
							bestFound = found;
						}
					}
