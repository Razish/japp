...
const char *bgToggleableSurfaces[BG_NUM_TOGGLEABLE_SURFACES] = {
    "l_arm_key", // 0
    "torso_canister1", "torso_canister2", "torso_canister3", "torso_tube1",
    "torso_tube2", // 5
    "torso_tube3", "torso_tube4", "torso_tube5", "torso_tube6",
    "r_arm", // 10
    "l_arm", "torso_shield", "torso_galaktorso", "torso_collar",
    //	"torso_eyes_mouth",				//15
    //	"torso_galakhead",
    //	"torso_galakface",
    ...
    "r_wing1", // 15
    "r_wing2", "l_wing1", "l_wing2", "r_gear",
    "l_gear", // 20
    "nose", "blah4", "blah5", "l_hand",
    "r_hand", // 25
    "helmet", "head", "head_concussion_charger",
    "head_light_blaster_cann", // 29
    NULL};

const int bgToggleableSurfaceDebris[BG_NUM_TOGGLEABLE_SURFACES] = {0, // 0
                                                                   0, 0, 0, 0,
                                                                   0, // 5
                                                                   0, 0, 0, 0,
                                                                   0, // 10
...
const char *bg_customSiegeSoundNames[MAX_CUSTOM_SIEGE_SOUNDS] = {"*att_attack",    "*att_primary", "*att_second",   "*def_guns",     "*def_position",
                                                                 "*def_primary",   "*def_second",  "*reply_coming", "*reply_go",     "*reply_no",
                                                                 "*reply_stay",    "*reply_yes",   "*req_assist",   "*req_demo",     "*req_hvy",
                                                                 "*req_medic",     "*req_sup",     "*req_tech",     "*spot_air",     "*spot_defenses",
                                                                 "*spot_emplaced", "*spot_sniper", "*spot_troops",  "*tac_cover",    "*tac_fallback",
                                                                 "*tac_follow",    "*tac_hold",    "*tac_split",    "*tac_together", NULL};
...
const int forcePowerSorted[NUM_FORCE_POWERS] = { // rww - always use this order when drawing force powers for any reason
    FP_TELEPATHY, FP_HEAL,      FP_ABSORB, FP_PROTECT, FP_TEAM_HEAL, FP_LEVITATION, FP_SPEED,         FP_PUSH,          FP_PULL,
    FP_SEE,       FP_LIGHTNING, FP_DRAIN,  FP_RAGE,    FP_GRIP,      FP_TEAM_FORCE, FP_SABER_OFFENSE, FP_SABER_DEFENSE, FP_SABERTHROW};
...
const stringID_table_t eTypes[ET_MAX] = {
    ...
    ENUM2STRING(ET_PORTAL),    ENUM2STRING(ET_SPEAKER),  ENUM2STRING(ET_PUSH_TRIGGER), ENUM2STRING(ET_TELEPORT_TRIGGER),
    ENUM2STRING(ET_INVISIBLE), ENUM2STRING(ET_NPC),      ENUM2STRING(ET_TEAM),         ENUM2STRING(ET_BODY),
    ENUM2STRING(ET_TERRAIN),   ENUM2STRING(ET_FX),       ENUM2STRING(ET_EVENTS),
};
...
qboolean BG_LegalizedForcePowers(char *powerOut, size_t powerOutSize, int maxRank, qboolean freeSaber, int teamForce, int gametype, int fpDisabled) {
    ...
    while (i < NUM_FORCE_POWERS) { // if this power doesn't match the side we're on, then 0 it now.
        ...
        if (final_Powers[i] && (fpDisabled & (1 << i))) { // if this power is disabled on the server via said server option, then we don't get it.
            final_Powers[i] = 0;
        }
    ...
    while (i < NUM_FORCE_POWERS) {
        ...
        while (countDown > 0) {
            ...
            if (countDown == 1 && ((i == FP_LEVITATION) || (i == FP_SABER_OFFENSE && freeSaber) || (i == FP_SABER_DEFENSE && freeSaber))) {
    ...
    if (usedPoints >
        allowedPoints) { // Time to do the fancy stuff. (meaning, slowly cut parts off while taking a guess at what is most or least important in the config)
                ...
                if (final_Powers[c] && final_Powers[c] < powerCycle) { // kill in order of lowest powers, because the higher powers are probably more important
                    if (c == FP_SABER_OFFENSE &&
                        (final_Powers[FP_SABER_DEFENSE] > minPow ||
                         final_Powers[FP_SABERTHROW] > 0)) { // if we're on saber attack, only suck it down if we have no def or throw either
                        ...
                        while (final_Powers[c] > 0 && usedPoints > allowedPoints) {
                            if (final_Powers[c] > 1 ||
                                ((c != FP_LEVITATION) && (c != FP_SABER_OFFENSE || !freeSaber) && (c != FP_SABER_DEFENSE || !freeSaber))) {
        ...
        if (usedPoints > allowedPoints) { // Still? Fine then.. we will kill all of your powers, except the freebies.
            ...
            while (i < NUM_FORCE_POWERS) {
                final_Powers[i] = 0;
                if (i == FP_LEVITATION || (i == FP_SABER_OFFENSE && freeSaber) || (i == FP_SABER_DEFENSE && freeSaber)) {
                    final_Powers[i] = 1;
                }
                i++;
            }
...
const gitem_t bg_itemlist[] = {
    // classname	pickup_sound	icon	quantity	type	tag		view_model	world_model		precache	sounds
    // description
    ...
    {"item_shield_sm_instant",
     ...
     1,
     NULL,
     {"models/map_objects/mp/psd_sm.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_shield_lrg_instant",
     ...
     2,
     NULL,
     {"models/map_objects/mp/psd.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_medpak_instant",
     ...
     0,
     NULL,
     {"models/map_objects/mp/medpac.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_seeker",
     ...
     HI_SEEKER,
     NULL,
     {"models/items/remote.md3", NULL, NULL},
     "",
     "",
     "@MENUS_AN_ATTACK_DRONE_SIMILAR"},
    {"item_shield",
     "sound/weapons/w_pkup.wav",
     ...
     HI_SHIELD,
     NULL,
     {"models/map_objects/mp/shield.md3", NULL, NULL},
     "",
     "sound/weapons/detpack/stick.wav sound/movers/doors/forcefield_on.wav sound/movers/doors/forcefield_off.wav sound/movers/doors/forcefield_lp.wav "
     "sound/effects/bumpfield.wav",
     "@MENUS_THIS_STATIONARY_ENERGY"},
    {"item_medpac",
     "sound/weapons/w_pkup.wav",
     ...
     HI_MEDPAC,
     NULL,
     {"models/map_objects/mp/bacta.md3", NULL, NULL},
     "",
     "",
     "@SP_INGAME_BACTA_DESC"},
    {"item_medpac_big",
     "sound/weapons/w_pkup.wav",
     ...
     HI_MEDPAC_BIG,
     NULL,
     {"models/items/big_bacta.md3", NULL, NULL},
     "",
     "",
     "@SP_INGAME_BACTA_DESC"},
    {"item_binoculars",
     ...
     HI_BINOCULARS,
     NULL,
     {"models/items/binoculars.md3", NULL, NULL},
     "",
     "",
     "@SP_INGAME_LA_GOGGLES_DESC"},
    {"item_sentry_gun",
     "sound/weapons/w_pkup.wav",
     ...
     HI_SENTRY_GUN,
     NULL,
     {"models/items/psgun.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THIS_DEADLY_WEAPON_IS"},
    {"item_jetpack",
     "sound/weapons/w_pkup.wav",
     ...
     NULL,
     {"models/items/psgun.glm", NULL, NULL},
     "effects/boba/jet.efx",
     "sound/chars/boba/JETON.wav sound/chars/boba/JETHOVER.wav sound/effects/fire_lp.wav",
     "@MENUS_JETPACK_DESC"},
    {"item_healthdisp",
     "sound/weapons/w_pkup.wav",
    ...
    {"item_eweb_holdable",
     ...
     HI_EWEB,
     NULL,
     {"models/map_objects/hoth/eweb_model.glm", NULL, NULL},
     "",
     "",
     "@MENUS_EWEB_DESC"},
    {"item_cloak",
    ...
    {"item_force_enlighten_light",
     ...
     NULL,
     {"models/map_objects/mp/jedi_enlightenment.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_force_enlighten_dark",
     ...
     NULL,
     {"models/map_objects/mp/dk_enlightenment.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_force_boon",
     ...
     PW_FORCE_BOON,
     NULL,
     {"models/map_objects/mp/force_boon.md3", NULL, NULL},
     "",
     "",
     ""},
    {"item_ysalimari",
     ...
     PW_YSALAMIRI,
     NULL,
     {"models/map_objects/mp/ysalimari.md3", NULL, NULL},
     "",
     "",
     ""},
    {"weapon_stun_baton",
     ...
     {"models/weapons2/stun_baton/baton_w.glm", NULL, NULL},
     "",
     "",
     ""},
    {"weapon_melee",
    ...
    {"weapon_saber",
     ...
     {"models/weapons2/saber/saber_w.glm", NULL, NULL},
     "",
     "",
    ...
    {"weapon_blaster_pistol",
     ...
     {"models/weapons2/blaster_pistol/blaster_pistol_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_BLASTER_PISTOL_DESC"},
    {"weapon_concussion_rifle",
     ...
     {"models/weapons2/concussion/c_rifle_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_CONC_RIFLE_DESC"},
    {"weapon_bryar_pistol",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/briar_pistol/briar_pistol_w.glm", NULL, NULL},
     "",
     "",
     "@SP_INGAME_BLASTER_PISTOL"},
    {"weapon_blaster",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/blaster_r/blaster_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THE_PRIMARY_WEAPON_OF"},
    {"weapon_disruptor",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/disruptor/disruptor_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THIS_NEFARIOUS_WEAPON"},
    {"weapon_bowcaster",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/bowcaster/bowcaster_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THIS_ARCHAIC_LOOKING"},
    {"weapon_repeater",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/heavy_repeater/heavy_repeater_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THIS_DESTRUCTIVE_PROJECTILE"},
    {"weapon_demp2",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/demp2/demp2_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_COMMONLY_REFERRED_TO"},
    {"weapon_flechette",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/golan_arms/golan_arms_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_WIDELY_USED_BY_THE_CORPORATE"},
    {"weapon_rocket_launcher",
     ...
     {"models/weapons2/merr_sonn/merr_sonn_w.glm", NULL, NULL},
     "",
     "",
     "@MENUS_THE_PLX_2M_IS_AN_EXTREMELY"},
    {"ammo_thermal",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/thermal/thermal_pu.md3", "models/weapons2/thermal/thermal_w.glm", NULL},
     "",
     "",
     "@MENUS_THE_THERMAL_DETONATOR"},
    {"ammo_tripmine",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/laser_trap/laser_trap_pu.md3", "models/weapons2/laser_trap/laser_trap_w.glm", NULL},
     "",
     "",
     "@MENUS_TRIP_MINES_CONSIST_OF"},
    {"ammo_detpack",
     "sound/weapons/w_pkup.wav",
     ...
     {"models/weapons2/detpack/det_pack_pu.md3", "models/weapons2/detpack/det_pack_proj.glm", "models/weapons2/detpack/det_pack_w.glm"},
     "",
     "",
     "@MENUS_A_DETONATION_PACK_IS"},
    {"weapon_thermal",
     "sound/weapons/w_pkup.wav",
     "gfx/hud/w_icon_thermal",
     4,
     ...
     WP_THERMAL,
     "models/weapons2/thermal/thermal.md3",
     {"models/weapons2/thermal/thermal_w.glm", "models/weapons2/thermal/thermal_pu.md3", NULL},
     "",
     "",
     "@MENUS_THE_THERMAL_DETONATOR"},
    {"weapon_trip_mine",
     ...
     WP_TRIP_MINE,
     "models/weapons2/laser_trap/laser_trap.md3",
     {"models/weapons2/laser_trap/laser_trap_w.glm", "models/weapons2/laser_trap/laser_trap_pu.md3", NULL},
     "",
     "",
     "@MENUS_TRIP_MINES_CONSIST_OF"},
    {"weapon_det_pack",
     ...
     WP_DET_PACK,
     "models/weapons2/detpack/det_pack.md3",
     {"models/weapons2/detpack/det_pack_proj.glm", "models/weapons2/detpack/det_pack_pu.md3", "models/weapons2/detpack/det_pack_w.glm"},
     "",
     "",
     "@MENUS_A_DETONATION_PACK_IS"},
    {"weapon_emplaced",
    ...
    {"ammo_force",
     ...
     AMMO_FORCE,
     NULL,
     {"models/items/energy_cell.md3", NULL, NULL},
     "",
     "",
     ""},
    {"ammo_blaster",
    ...
    {"ammo_powercell",
     ...
     AMMO_POWERCELL,
     NULL,
     {"models/items/power_cell.md3", NULL, NULL},
     "",
     "",
     ""},
    {"ammo_metallic_bolts",
     ...
     AMMO_METAL_BOLTS,
     NULL,
     {"models/items/metallic_bolts.md3", NULL, NULL},
     "",
     "",
     ""},
    {"ammo_rockets",
     ...
     AMMO_ROCKETS,
     NULL,
     {"models/items/rockets.md3", NULL, NULL},
     "",
     "",
     ""},
    ...
    {"team_CTF_redflag",
     ...
     PW_REDFLAG,
     NULL,
     {"models/flags/r_flag.md3", "models/flags/r_flag_ysal.md3", NULL},
     "",
     "",
     ""},
    {"team_CTF_blueflag",
     ...
     PW_BLUEFLAG,
     NULL,
     {"models/flags/b_flag.md3", "models/flags/b_flag_ysal.md3", NULL},
     "",
     "",
     ""},
...
qboolean BG_HasYsalamiri(int gametype, playerState_t *ps) {
    if (gametype == GT_CTY && (ps->powerups[PW_REDFLAG] || ps->powerups[PW_BLUEFLAG])) {
        return qtrue;
    }
...
qboolean BG_CanUseFPNow(int gametype, playerState_t *ps, int time, forcePowers_t power) {
    ...
    if ((ps->brokenLimbs & (1 << BROKENLIMB_RARM)) || (ps->brokenLimbs & (1 << BROKENLIMB_LARM))) { // powers we can't use with a broken arm
        switch (power) {
        case FP_PUSH:
        case FP_PULL:
        case FP_GRIP:
...
qboolean BG_PlayerTouchesItem(playerState_t *ps, entityState_t *item, int atTime) {
    ...
    if (ps->origin.x - origin.x > 44 || ps->origin.x - origin.x < -50 || ps->origin.y - origin.y > 36 || ps->origin.y - origin.y < -36 ||
        ps->origin.z - origin.z > 36 || ps->origin.z - origin.z < -36) {
        return qfalse;
    }
...
qboolean BG_CanItemBeGrabbed(int gametype, const entityState_t *ent, const playerState_t *ps) {
    ...
    case IT_WEAPON:
        ...
        if (!BG_AlwaysPickupWeapons()) {
            if (!(ent->eFlags & EF_DROPPEDWEAPON) && (ps->stats[STAT_WEAPONS] & (1 << item->giTag)) && item->giTag != WP_THERMAL &&
                item->giTag != WP_TRIP_MINE &&
                item->giTag != WP_DET_PACK) { // weaponstay stuff.. if this isn't dropped, and you already have it, you don't get it.
                return qfalse;
            }
        } else if (BG_AlwaysPickupWeapons() || (item->giTag == WP_THERMAL || item->giTag == WP_TRIP_MINE ||
                                                item->giTag == WP_DET_PACK)) { // check to see if full on ammo for this, if so, then..
...
const char *eventnames[EV_NUM_ENTITY_EVENTS] = {
    ...
    "EV_GLOBAL_DUEL", "EV_PRIVATE_DUEL",
    ...
    "EV_NOAMMO", "EV_CHANGE_WEAPON", "EV_FIRE_WEAPON", "EV_ALT_FIRE", "EV_SABER_ATTACK", "EV_SABER_HIT", "EV_SABER_BLOCK", "EV_SABER_CLASHFLARE",
    "EV_SABER_UNHOLSTER", "EV_BECOME_JEDIMASTER", "EV_DISRUPTOR_MAIN_SHOT", "EV_DISRUPTOR_SNIPER_SHOT", "EV_DISRUPTOR_SNIPER_MISS", "EV_DISRUPTOR_HIT",
    ...
    "EV_USE_ITEM0", "EV_USE_ITEM1", "EV_USE_ITEM2", "EV_USE_ITEM3", "EV_USE_ITEM4", "EV_USE_ITEM5", "EV_USE_ITEM6", "EV_USE_ITEM7", "EV_USE_ITEM8",
    "EV_USE_ITEM9", "EV_USE_ITEM10", "EV_USE_ITEM11", "EV_USE_ITEM12", "EV_USE_ITEM13", "EV_USE_ITEM14", "EV_USE_ITEM15",
    ...
    "EV_PLAYDOORSOUND", "EV_PLAYDOORLOOPSOUND", "EV_BMODEL_SOUND",

    "EV_MUTE_SOUND", "EV_VOICECMD_SOUND", "EV_GENERAL_SOUND",
    "EV_GLOBAL_SOUND", // no attenuation
    "EV_GLOBAL_TEAM_SOUND", "EV_ENTITY_SOUND",

    "EV_PLAY_ROFF",

    "EV_GLASS_SHATTER", "EV_DEBRIS", "EV_MISC_MODEL_EXP",
    ...
    "EV_POWERUP_QUAD", "EV_POWERUP_BATTLESUIT",
    //"EV_POWERUP_REGEN",
    ...
    "EV_SIEGE_ROUNDOVER", "EV_SIEGE_OBJECTIVECOMPLETE",

    "EV_DESTROY_GHOUL2_INSTANCE",

    "EV_DESTROY_WEAPON_MODEL",

    "EV_GIVE_NEW_RANK", "EV_SET_FREE_SABER", "EV_SET_FORCE_DISABLE",

    "EV_WEAPON_CHARGE", "EV_WEAPON_CHARGE_ALT",

    "EV_SHIELD_HIT",

    "EV_DEBUG_LINE", "EV_TESTLINE", "EV_STOPLOOPINGSOUND", "EV_STARTLOOPINGSOUND", "EV_TAUNT",
...
qboolean BG_ValidateSkinForTeam(const char *modelName, char *skinName, int team, vector3 *colors) {
    ...
    if (team == TEAM_RED) {
        if (Q_stricmp("red", skinName) != 0) {                                                                   // not "red"
            if (Q_stricmp("blue", skinName) == 0 || Q_stricmp("default", skinName) == 0 || strchr(skinName, '|') // a multi-skin playerModel
        ...
        if (Q_stricmp("blue", skinName) != 0) {
            if (Q_stricmp("red", skinName) == 0 || Q_stricmp("default", skinName) == 0 || strchr(skinName, '|') // a multi-skin playerModel
                || !BG_IsValidCharacterModel(modelName, skinName)) {
...
qboolean BG_OutOfMemory(void) { return (bg_poolSize >= MAX_POOL_SIZE) ? qtrue : qfalse; }

const char *gametypeStringShort[GT_MAX_GAME_TYPE] = {"FFA", "HOLO", "JM", "1v1", "2v1", "SP", "TDM", "SAGA", "CTF", "CTY"};
...
int BG_GetGametypeForString(const char *gametype) {
    if (!Q_stricmp(gametype, "ffa") || !Q_stricmp(gametype, "dm"))
        return GT_FFA;
    else if (!Q_stricmp(gametype, "holocron"))
        return GT_HOLOCRON;
    else if (!Q_stricmp(gametype, "jm"))
        return GT_JEDIMASTER;
    else if (!Q_stricmp(gametype, "duel"))
        return GT_DUEL;
    else if (!Q_stricmp(gametype, "powerduel"))
        return GT_POWERDUEL;
    else if (!Q_stricmp(gametype, "sp") || !Q_stricmp(gametype, "coop"))
        return GT_SINGLE_PLAYER;
    else if (!Q_stricmp(gametype, "tdm") || !Q_stricmp(gametype, "tffa") || !Q_stricmp(gametype, "team"))
        return GT_TEAM;
    else if (!Q_stricmp(gametype, "siege"))
        return GT_SIEGE;
    else if (!Q_stricmp(gametype, "ctf"))
        return GT_CTF;
    else if (!Q_stricmp(gametype, "cty"))
        return GT_CTY;
    else
        return -1;
}
...
bool BG_HasSetSaberOnly(
    ...
    for (int wp = WP_NONE; wp < WP_NUM_WEAPONS; wp++) {
        if (!(weaponDisable & (1 << wp)) && wp != WP_NONE && wp != WP_STUN_BATON && wp != WP_MELEE && wp != WP_SABER && wp != WP_EMPLACED_GUN &&
            wp != WP_TURRET) {
            return false;
        }
    }
