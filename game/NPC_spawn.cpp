...
void NPC_SetMiscDefaultData(gentity_t *ent) {
    ...
    case NPCTEAM_ENEMY: {
        ...
        if (ent->client->NPC_class == CLASS_TAVION || ent->client->NPC_class == CLASS_REBORN || ent->client->NPC_class == CLASS_DESANN ||
        ...
        } else if (ent->client->NPC_class == CLASS_PROBE || ent->client->NPC_class == CLASS_REMOTE || ent->client->NPC_class == CLASS_INTERROGATOR ||
    ...
    if (ent->client->NPC_class == CLASS_SEEKER && ent->activator) { // assume my teams are already set correctly
    } else {
...
int NPC_WeaponsForTeam(team_e team, int spawnflags, const char *NPC_type) {
    ...
    switch (team) {
        ...
        //	case TEAM_IMPERIAL:
    case NPCTEAM_ENEMY:
        if (Q_stricmp("tavion", NPC_type) == 0 || Q_strncmp("reborn", NPC_type, 6) == 0 || Q_stricmp("desann", NPC_type) == 0 ||
...
qboolean NPC_SpotWouldTelefrag(gentity_t *npc) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (hit->inuse && hit->client && hit->s.number != npc->s.number && (hit->r.contents & MASK_NPCSOLID) && hit->s.number != npc->r.ownerNum &&
...
void NPC_Begin(gentity_t *ent) {
    ...
    } else if (ent->client->NPC_class == CLASS_STORMTROOPER || ent->client->NPC_class == CLASS_SWAMPTROOPER || ent->client->NPC_class == CLASS_IMPWORKER ||
    ...
    */
    ...
    if (ent->client->NPC_class != CLASS_R2D2 && ent->client->NPC_class != CLASS_R5D2 && ent->client->NPC_class != CLASS_MOUSE &&
        ent->client->NPC_class != CLASS_GONK && ent->client->NPC_class != CLASS_PROTOCOL) {
    ...
    if (ent->m_pVehicle) { // a vehicle
        ...
        if (ent->m_pVehicle->m_iDroidUnitTag != -1) {
            ...
            if (ent->model2 && ent->model2[0]) { // specified on the NPC_Vehicle spawner ent
                droidNPCType = ent->model2;
            } else if (ent->m_pVehicle->m_pVehicleInfo->droidNPC && ent->m_pVehicle->m_pVehicleInfo->droidNPC[0]) { // specified in the vehicle's .veh file
                droidNPCType = ent->m_pVehicle->m_pVehicleInfo->droidNPC;
            }

            if (droidNPCType != NULL) {
                if (Q_stricmp("random", droidNPCType) == 0 || Q_stricmp("default", droidNPCType) == 0) { // use default - R2D2 or R5D2
                    if (Q_irand(0, 1)) {
                        droidNPCType = "r2d2";
                    } else {
                        droidNPCType = "r5d2";
                ...
                if (droidEnt != NULL) {
                    if (droidEnt->client) {
                        droidEnt->client->ps.m_iVehicleNum = droidEnt->s.m_iVehicleNum =
                            // droidEnt->s.otherEntityNum2 =
                            droidEnt->s.owner = droidEnt->r.ownerNum = ent->s.number;
...
gentity_t *NPC_Spawn_Do(gentity_t *ent) {
    ...
    if (Q_stricmp(ent->classname, "NPC_Vehicle") == 0) {
        ...
        assert(newent->m_pVehicle && newent->m_pVehicle->m_pVehicleInfo && newent->m_pVehicle->m_pVehicleInfo->Initialize);
        ...
        if (g_vehicleInfo[iVehIndex].type == VH_FIGHTER) { // FIXME: EXTERN!!!
            newent->flags |=
                (FL_NO_KNOCKBACK | FL_SHIELDED | FL_DMG_BY_HEAVY_WEAP_ONLY); // don't get pushed around, blasters bounce off, only damage from heavy weaps
        }
...
void NPC_Spawn_Go(gentity_t *ent) { NPC_Spawn_Do(ent); }
...
void SP_NPC_spawner(gentity_t *self) {
    ...
    */
    ...
    if (self->targetname) { // Wait for triggering
        ...
        if (level.spawning) { // in entity spawn stage - map starting up
...
SUSPENDED - Fighters: Don't drop until someone gets in it (this only works as long as no-nw has *ever* ridden the vehicle, to simulate ships that are
suspended-docked) - note: ships inside trigger_spaces do not drop when unoccupied CINEMATIC - Will spawn with no default AI (BS_CINEMATIC) NOTSOLID - Starts not
solid STARTINSOLID - Don't try to fix if spawn in solid SHY - Spawner is shy
...
"model2" - if the vehicle can have a droid (has "*droidunit" tag), this NPC will be spawned and placed there - note: game will automatically use the one
specified in the .veh file (if any) or, absent that, it will use an R2D2 or R5D2 NPC)
...
qboolean NPC_VehiclePrecache(gentity_t *spawner) {
    ...
    if (droidNPCType) {
        if (Q_stricmp("random", droidNPCType) == 0 || Q_stricmp("default", droidNPCType) == 0) { // precache both r2 and r5, as defaults
...
THROW - can throw their saber (level 2) - reduces their defense some (can use this spawnflag alone or in combination with any *one* of the previous 3
spawnflags)
...
"message" - turns on his key surface.  This is the name of the key you get when you walk over his body.  This must match the "message" field of the
func_security_panel you want this key to open.  Set to "goodie" to have him carrying a goodie key that player can use to operate doors with "GOODIE" spawnflag.
...
"message" - if a COMMANDER, turns on his key surface.  This is the name of the key you get when you walk over his body.  This must match the "message" field of
the func_security_panel you want this key to open.  Set to "goodie" to have him carrying a goodie key that player can use to operate doors with "GOODIE"
spawnflag.
    ...
    for (n = 1; n < ENTITYNUM_MAX_NORMAL; n++) {
        ...
        if (killNonSF) {
            if (player) {
                ...
                } else if (player->NPC_type && player->classname && player->classname[0] &&
                           Q_stricmp("NPC_starfleet", player->classname) != 0) { // A spawner, remove it
            ...
            if (killTeam != NPCTEAM_FREE) {
                if (player->client->playerTeam == killTeam) {
                    ...
                    JPLua::Entity_CallFunction(player, JPLua::JPLUA_ENTITY_DIE, (intptr_t)player, (intptr_t)player, (intptr_t)player->client->pers.maxHealth,
...
void NPC_PrintScore(gentity_t *ent, gentity_t *npc) {
    trap->SendServerCommand(ent - g_entities, va("print \"%s: %d\n\"", npc->targetname, npc->client->ps.persistant[PERS_SCORE]));
}
