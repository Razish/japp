		...
		for ( i = 0; i < group->numGroup; i++ ) {
			for ( j = 0; j < group->numGroup; j++ ) {
				if ( bestMembers[j].number != ENTITYNUM_NONE ) {//slot occupied
					if ( group->member[i].pathCostToEnemy < bestMembers[j].pathCostToEnemy ) {//this guy has a shorter path than the one currenly in this spot, bump him and put myself in here
...
void AI_GetGroup( gentity_t *self ) {
	...
	for ( i = 0; i < level.num_entities; i++ ) {
		...
		if ( !AI_ValidateGroupMember( self->NPC->group, member ) ) {//FIXME: keep track of those who aren't angry yet and see if we should wake them after we assemble the core group
			continue;
		}
...
void AI_SetNewGroupCommander( AIGroupInfo_t *group ) {
	...
	for ( i = 0; i < group->numGroup; i++ ) {
		...
		if ( !group->commander || (member && member->NPC && group->commander->NPC && member->NPC->rank > group->commander->NPC->rank) ) {//keep track of highest rank
			group->commander = member;
		}
	}
}
	...
	if ( !noflee ) {
		...
		for ( i = 0; i < group->numGroup; i++ ) {
			...
			if ( member->NPC->rank < RANK_ENSIGN ) {//grunt
				if ( group->enemy && DistanceSquared( &member->r.currentOrigin, &group->enemy->r.currentOrigin ) < 65536/*256*256*/ ) {//those close to enemy run away!
