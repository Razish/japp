...
int G_ModelIndex(const char *name) { return G_FindConfigstringIndex(name, CS_MODELS, MAX_MODELS, qtrue); }
...
void TryUse(gentity_t *ent) {
    ...
    if ((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & ((1 << HI_HEALTHDISP) | (1 << HI_AMMODISP))) && target && target->inuse && target->client &&
        target->health > 0 && OnSameTeam(ent, target) && (G_CanUseDispOn(target, HI_HEALTHDISP) || G_CanUseDispOn(target, HI_AMMODISP))) {
...
qboolean G_PointInBounds(vector3 *point, vector3 *mins, vector3 *maxs) {
    ...
    for (i = 0; i < 3; i++) {
        if (point->raw[i] < mins->raw[i])
            return qfalse;
        if (point->raw[i] > maxs->raw[i])
            return qfalse;
    }
...
qboolean G_BoxInBounds(vector3 *point, vector3 *mins, vector3 *maxs, vector3 *boundsMins, vector3 *boundsMaxs) {
    ...
    if (boxMaxs.x > boundsMaxs->x || boxMaxs.y > boundsMaxs->y || boxMaxs.z > boundsMaxs->z || boxMins.x < boundsMins->x || boxMins.y < boundsMins->y ||
...
static qboolean cmpSubCase(const char *s1, const char *s2) { return (strstr(s1, s2) != NULL) ? qtrue : qfalse; }
static qboolean cmpSub(const char *s1, const char *s2) { return (Q_stristr(s1, s2) != NULL) ? qtrue : qfalse; }
static qboolean cmpWholeCase(const char *s1, const char *s2) { return (!strcmp(s1, s2)) ? qtrue : qfalse; }
static qboolean cmpWhole(const char *s1, const char *s2) { return (!Q_stricmp(s1, s2)) ? qtrue : qfalse; }
...
int G_ClientFromString(const gentity_t *ent, const char *match, uint32_t flags) {
    ...
    if (firstMatch) {
        for (i = 0, e = g_entities; i < level.maxclients; i++, e++) {
            if (compareFunc(e->client->pers.netnameClean, cleanedMatch) && e->inuse && e->client->pers.connected != CON_DISCONNECTED) {
                return i;
            }
        }
    } else {
...
void G_Announce(const char *msg) { trap->SendServerCommand(-1, va("cp \"%s\"", msg)); }
...
bool G_IsValidEntity(const gentity_t *ent, bool doAssert) {
    ...
    const bool result = ent && ent->inuse && entityNum >= 0 && entityNum < MAX_GENTITIES;
...
bool G_PlayerIsDueling(const gentity_t *ent) { return G_IsValidEntity(ent) && ent->client && ent->s.eType == ET_PLAYER && ent->client->ps.duelInProgress; }
