...
void G_TimeShiftClient(gentity_t *ent, int time) {
    ...
    if (j != k) {
        ...
        if (j != ent->client->trailHead) {
            ...
            // FOR TESTING ONLY
            //			Com_Printf( "level time: %d, fire time: %d, j time: %d, k time: %d\n", level.time, time, ent->client->trail[j].time,
            //ent->client->trail[k].time );
...
void G_TimeShiftAllClients(int time, gentity_t *skip) {
    ...
    for (i = 0, ent = g_entities; i < MAX_CLIENTS; i++, ent++) {
        if (ent->client && ent->inuse && ent->client->sess.sessionTeam != TEAM_SPECTATOR && ent->client->tempSpectate < level.time && ent != skip) {
