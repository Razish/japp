...
void PM_VehicleImpact(bgEntity_t *pEnt, trace_t *trace) {
    ...
    if (!hitEnt ||
        (pSelfVeh && pSelfVeh->m_pPilot && hitEnt && hitEnt->s.eType == ET_MISSILE && hitEnt->inuse && hitEnt->r.ownerNum == pSelfVeh->m_pPilot->s.number)) {
        return;
    }
    ...
    if (trace->entityNum < ENTITYNUM_WORLD && hitEnt->s.eType == ET_MOVER && hitEnt->s.apos.trType != TR_STATIONARY && (hitEnt->spawnflags & 16) /*IMPACT*/ &&
        !Q_stricmp("func_rotating", hitEnt->classname)) {
        ...
        if (pEnt->m_pVehicle->m_iHitDebounce < pm->cmd.serverTime || forceSurfDestruction) {
            ...
            if (trace && !pSelfVeh->m_iRemovedSurfaces && !forceSurfDestruction) {
                ...
                if ((trace->entityNum == ENTITYNUM_WORLD || hitEnt->s.solid == SOLID_BMODEL) &&
                    ...
                    } else if (trace->plane.normal.z >= MIN_LANDING_SLOPE && pSelfVeh->m_LandTrace.fraction < 1.0f && pm->ps->speed <= MIN_LANDING_SPEED) {
                ...
                if (turnFromImpact) {
                    ...
                    if (!bounceDir.z) {
                        ...
                        if (pitchTurnStrength > MAX_IMPACT_TURN_ANGLE)
                            ...
                            pitchTurnStrength = -MAX_IMPACT_TURN_ANGLE;
                        //	pSelfVeh->m_vOrientation->pitch = AngleNormalize180(
                        //pSelfVeh->m_vOrientation->pitch+pitchTurnStrength/turnDivider*pSelfVeh->m_fTimeModifier );
                        pSelfVeh->m_vFullAngleVelocity.pitch =
                            AngleNormalize180(pSelfVeh->m_vOrientation->pitch + pitchTurnStrength / turnDivider * pSelfVeh->m_fTimeModifier);
                    }
                    // now do yaw
                    if (!bounceDir.x && !bounceDir.y) {
                        ...
                        if (yawTurnStrength > MAX_IMPACT_TURN_ANGLE)
                            ...
                            yawTurnStrength = -MAX_IMPACT_TURN_ANGLE;
                        //	pSelfVeh->m_vOrientation->roll = AngleNormalize180(
                        //pSelfVeh->m_vOrientation->roll-yawTurnStrength/turnDivider*pSelfVeh->m_fTimeModifier );
                        pSelfVeh->m_vFullAngleVelocity.roll =
                            AngleNormalize180(pSelfVeh->m_vOrientation->roll - yawTurnStrength / turnDivider * pSelfVeh->m_fTimeModifier);
                    }
                    /*
                    ...
                    */
                    ...
                    if (turnHitEnt && hitEnt->client && !FighterIsLanded(hitEnt->m_pVehicle, &hitEnt->client->ps) && !(hitEnt->spawnflags & 2) /*SUSPENDED*/) {
                        ...
                        if (!bounceDir.z) {
                            ...
                            if (pitchTurnStrength > MAX_IMPACT_TURN_ANGLE)
                                ...
                                pitchTurnStrength = -MAX_IMPACT_TURN_ANGLE;
                            //	hitEnt->m_pVehicle->m_vOrientation->pitch = AngleNormalize180(
                            //hitEnt->m_pVehicle->m_vOrientation->pitch+pitchTurnStrength/turnDivider*pSelfVeh->m_fTimeModifier );
                            hitEnt->m_pVehicle->m_vFullAngleVelocity.pitch =
                                AngleNormalize180(hitEnt->m_pVehicle->m_vOrientation->pitch + pitchTurnStrength / turnDivider * pSelfVeh->m_fTimeModifier);
                        }
                        // now do yaw
                        if (!bounceDir.x && !bounceDir.y) {
                            // shouldn't be any yaw
                            ...
                            if (yawTurnStrength > MAX_IMPACT_TURN_ANGLE)
                                ...
                                yawTurnStrength = -MAX_IMPACT_TURN_ANGLE;
                            //	hitEnt->m_pVehicle->m_vOrientation->roll = AngleNormalize180(
                            //hitEnt->m_pVehicle->m_vOrientation->roll-yawTurnStrength/turnDivider*pSelfVeh->m_fTimeModifier );
                            hitEnt->m_pVehicle->m_vFullAngleVelocity.roll =
                                AngleNormalize180(hitEnt->m_pVehicle->m_vOrientation->roll - yawTurnStrength / turnDivider * pSelfVeh->m_fTimeModifier);
                        }
...
qboolean PM_GroundSlideOkay(float zNormal) {
    if (zNormal > 0 && pm->ps->velocity.z > 0) {
        if (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
            pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
            pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND || BG_InReboundJump(pm->ps->legsAnim)) {
            return qfalse;
        }
    }
...
qboolean PM_ClientImpact(trace_t *trace) {
    ...
    if (VectorLength(&pm->ps->velocity) >= 100 && pm_entSelf->s.NPC_class != CLASS_VEHICLE && pm->ps->lastOnGround + 100 < level.time
...
void PM_StepSlideMove(qboolean gravity) {
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS) {
        ...
        if (pEnt && (pEnt->s.NPC_class == CLASS_ATST ||
                     (pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle && pEnt->m_pVehicle->m_pVehicleInfo->type == VH_WALKER))) { // AT-STs can step high
            up.z += 66.0f;
            isGiant = qtrue;
        }
    ...
    if (!trace.allsolid && !skipStep) {
        ...
        if (pm->ps->clientNum >= MAX_CLIENTS && isGiant && trace.entityNum < MAX_CLIENTS && pEnt && pEnt->s.NPC_class == CLASS_RANCOR) {
            if (pm->stepSlideFix) {
