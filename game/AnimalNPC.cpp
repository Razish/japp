...
static void DeathUpdate(Vehicle_t *pVeh) {
    if (level.time >= pVeh->m_iDieTime) {
        ...
        /*		else
                        ...
                        G_RadiusDamage( trace.endpos, NULL, pVeh->m_pVehicleInfo->explosionDamage, pVeh->m_pVehicleInfo->explosionRadius, NULL, MOD_EXPLOSIVE
...
static qboolean Update(Vehicle_t *pVeh, const usercmd_t *pUcmd) { return g_vehicleInfo[VEHICLE_BASE].Update(pVeh, pUcmd); }
#endif // PROJECT_GAME
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
    ...
    if (pVeh->m_pPilot /*&& (pilotPS->weapon == WP_NONE || pilotPS->weapon == WP_MELEE )*/ && (pVeh->m_ucmd.buttons & BUTTON_ALT_ATTACK) &&
        (int)pVeh->m_pVehicleInfo->turboSpeed) {
    ...
    if (!parentPS->m_iVehicleNum) { // drifts to a stop
    ...
    if ((int)parentPS->speed || parentPS->groundEntityNum == ENTITYNUM_NONE || pVeh->m_ucmd.forwardmove || pVeh->m_ucmd.upmove > 0) {
...
void AnimalProcessOri(Vehicle_t *pVeh) { ProcessOrientCommands(pVeh); }
