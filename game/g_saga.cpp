            ...
            if (scl) {
                ...
                while (j < WP_NUM_WEAPONS) {
                    if (scl->weapons & (1 << j)) { // we use this weapon so register it.
                        RegisterItem(BG_FindItemForWeapon((weapon_e)j));
                    }
                    j++;
                }
                j = 0;
                while (j < HI_NUM_HOLDABLE) {
                    if (scl->invenItems & (1 << j)) { // we use this item so register it.
                        RegisterItem(BG_FindItemForHoldable((holdable_e)j));
                    }
                    j++;
                }
            }
...
// or whatever.
void SiegeSetCompleteData(int team) { trap->SetConfigstring(CS_SIEGE_WINTEAM, va("%i", team)); }
...
void UseSiegeTarget(gentity_t *other, gentity_t *en,
                    char *target) { // actually use the player which triggered the object which triggered the siege objective to trigger the target
    gentity_t *t;
    gentity_t *ent;

    if (!en || !en->client) { // looks like we don't have access to a player, so just use the activating entity
        ent = other;
    } else {
        ent = en;
    }
...
void BroadcastObjectiveCompletion(int team, int objective, int final, int client) {
    if (client != ENTITYNUM_NONE && g_entities[client].client &&
        g_entities[client].client->sess.sessionTeam == team) { // guy who completed this objective gets points, providing he's on the opposing team
...
void SiegeDoTeamAssign(void) {
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent->inuse && ent->client && ent->client->pers.connected == CON_CONNECTED) { // a connected client, switch his frickin teams around
...
void SiegeTeamSwitch(int winTeam, int winTime) {
    trap->SiegePersGet(&g_siegePersistant);
    if (g_siegePersistant.beatingTime) { // was already in "switched" mode, change back
    ...
    } else { // go into "beat their time" mode
...
void SiegeRoundComplete(int winningteam, int winningclient) {
    ...
    if (winningclient != ENTITYNUM_NONE && g_entities[winningclient].client &&
        g_entities[winningclient].client->sess.sessionTeam != winningteam) { // this person just won the round for the other team..
    ...
    if (BG_SiegeGetValueGroup(siege_info, teamstr, gParseObjectives)) {
        if (!BG_SiegeGetPairedValue(gParseObjectives, "roundover_target",
                                    teamstr)) { // didn't find the name of the thing to target upon win, just logexit now then.
            LogExit("Objectives completed");
            return;
        }

        if (originalWinningClient == ENTITYNUM_NONE) { // oh well, just find something active and use it then.
            int i = 0;
            gentity_t *ent;
    ...
    if (g_siegeTeamSwitch.integer && (imperial_time_limit || rebel_time_limit)) { // handle stupid team switching crap
        int time = 0;
    ...
    } else { // assure it's clear for next round
        SiegeClearSwitchData();
    }
}
...
void G_ValidateSiegeClassForTeam(gentity_t *ent, int team) {
    ...
    siegeTeam_t *stm;
    int newClassIndex = -1;
    if (ent->client->siegeClass == -1) { // uh.. sure.
        return;
    }
    ...
    if (stm) {
        ...
        while (i < stm->numClasses) { // go through the team and see its valid classes, can we find one that matches our current player class?
            if (stm->classes[i]) {
                if (!Q_stricmp(scl->name, stm->classes[i]->name)) { // the class we're using is already ok for this team.
                    return;
                }
                if (stm->classes[i]->playerClass == scl->playerClass || newClassIndex == -1) {
                    newClassIndex = i;
                }
            }
            i++;
        ...
        if (newClassIndex != -1) { // ok, let's find it in the global class array
...
void SetTeamQuick(gentity_t *ent, int team, qboolean doBegin) {
    ...
    ent->client->sess.sessionTeam = (team_e)team;
...
void SiegeBeginRound(int entNum) { // entNum is just used as something to fire targets from.
    char targname[1024];

    if (!g_preroundState) { // if players are not ingame on round start then respawn them now
        int i = 0;
        gentity_t *ent;
        ...
        while (i < MAX_CLIENTS) {
            ...
            if (ent->inuse && ent->client) {
                if (ent->client->sess.sessionTeam != TEAM_SPECTATOR && !(ent->client->ps.pm_flags & PMF_FOLLOW)) { // not a spec, just respawn them
                    spawnEnt = qtrue;
                } else if (ent->client->sess.sessionTeam == TEAM_SPECTATOR &&
                           (ent->client->sess.siegeDesiredTeam == TEAM_RED ||
                            ent->client->sess.siegeDesiredTeam == TEAM_BLUE)) { // spectator but has a desired team
                    spawnEnt = qtrue;
                }
            }
...
void SiegeCheckTimers(void) {
    ...
    if (!gSiegeRoundBegun) { // check if anyone is active on this team - if not, keep the timer set up.
        i = 0;

        while (i < MAX_CLIENTS) {
            ent = &g_entities[i];

            if (ent && ent->inuse && ent->client && ent->client->pers.connected == CON_CONNECTED && ent->client->sess.siegeDesiredTeam == SIEGETEAM_TEAM1) {
                numTeam1++;
            }
            i++;
        }
        ...
        while (i < MAX_CLIENTS) {
            ...
            if (ent && ent->inuse && ent->client && ent->client->pers.connected == CON_CONNECTED && ent->client->sess.siegeDesiredTeam == SIEGETEAM_TEAM2) {
                numTeam2++;
            }
            i++;
        }
    ...
    if (!gSiegeRoundBegun) {
        if (!numTeam1 || !numTeam2) { // don't have people on both teams yet.
            gSiegeBeginTime = level.time + SIEGE_ROUND_BEGIN_TIME;
            trap->SetConfigstring(CS_SIEGE_STATE, "1"); //"waiting for players on both teams"
        } else if (gSiegeBeginTime < level.time) {      // mark the round as having begun
...
void siegeTriggerUse(gentity_t *ent, gentity_t *other, gentity_t *activator) {
    ...
    if (!(ent->s.eFlags & EF_RADAROBJECT)) { // toggle radar on and exit if it is not showing up already
        ent->s.eFlags |= EF_RADAROBJECT;
        return;
    }

    if (activator && activator->client) { // activator will hopefully be the person who triggered this event
    ...
    if (BG_SiegeGetValueGroup(siege_info, teamstr, gParseObjectives)) {
        ...
        if (BG_SiegeGetValueGroup(gParseObjectives, objectivestr, desiredobjective)) {
            ...
            if (ent->target && ent->target[0]) { // use this too
...
void SP_info_siege_objective(gentity_t *ent) {
    ...
    if (!ent->objective || !ent->side) { // j00 fux0red something up
        G_FreeEntity(ent);
...
void SiegeIconUse(gentity_t *ent, gentity_t *other, gentity_t *activator) { // toggle it on and off
...
void SP_info_siege_radaricon(gentity_t *ent) {
    ...
    G_SpawnString("icon", "", &s);
    if (!s || !s[0]) { // that's the whole point of the entity
...
void decompTriggerUse(gentity_t *ent, gentity_t *other, gentity_t *activator) {
    ...
    if (!G_SiegeGetCompletionStatus(ent->side, ent->objective)) { // if it's not complete then there's nothing to do here
        return;
    }
...
void siegeEndUse(gentity_t *ent, gentity_t *other, gentity_t *activator) { LogExit("Round ended"); }
...
static void SiegeItemRespawnEffect(gentity_t *ent, vector3 *newOrg) {
    vector3 upAng;
    ...
    if (!ent->genericValue10) { // no respawn effect
        return;
    }
...
void SiegeItemThink(gentity_t *ent) {
    ...
    if (ent->genericValue12) { // recharge health
    ...
    if (ent->genericValue8 !=
        ENTITYNUM_NONE) { // Just keep sticking it on top of the owner. We need it in the same PVS as him so it will render bolted onto him properly.
    ...
    } else if (ent->genericValue1) { // this means we want to run physics on the object
    ...
    if (carrier) {
        ...
        if (!carrier->inuse || !carrier->client ||
            ...
            (carrier->client->ps.pm_flags & PMF_FOLLOW)) { // respawn on the original spot
            SiegeItemRespawnOnOriginalSpot(ent, NULL);
        } else if (carrier->health < 1) { // The carrier died so pop out where he is (unless in nodrop).
            if (ent->target6 && ent->target6[0]) {
                G_UseTargets2(ent, ent, ent->target6);
            }

            if (trap->PointContents(&carrier->client->ps.origin, carrier->s.number) & CONTENTS_NODROP) { // In nodrop land, go back to the original spot.
                ...
                trace_t tr;
                trap->Trace(&tr, &carrier->client->ps.origin, &ent->r.mins, &ent->r.maxs, &carrier->client->ps.origin, ent->s.number, ent->clipmask, qfalse, 0,
                            0);

                if (tr.startsolid) { // bad spawning area, try again with the trace up a bit.
                    vector3 TracePoint;
                    ...
                    if (tr.startsolid) { // hmm, well that didn't work. try one last time with the item back
    ...
    if (ent->genericValue9 && ent->genericValue9 < level.time) { // time to respawn on the original spot then
        SiegeItemRespawnEffect(ent, &ent->pos1);
        G_SetOrigin(ent, &ent->pos1);
        ent->genericValue9 = 0;
...
void SiegeItemTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
    if (!other || !other->inuse || !other->client || other->s.eType == ET_NPC) {
        if (trace && trace->startsolid) { // let me out! (ideally this should not happen, but such is life)
            vector3 escapePos;
    ...
    if (other->client->holdingObjectiveItem) { // this guy's already carrying a siege item
        return;
    }
    ...
    if (self->genericValue2) { // Am I already picked up?
        return;
    }

    if (self->genericValue6 == other->client->sess.sessionTeam) { // Set to not be touchable by players on this team.
        return;
    }

    if (!gSiegeRoundBegun) { // can't pick it up if round hasn't started yet
        return;
    }

    if (self->noise_index) { // play the pickup noise.
    ...
    if (self->target2 && self->target2[0] &&
        (!self->genericValue4 || !self->genericValue5)) { // fire the target for pickup, if it's set to fire every time, or set to only fire the first time and
                                                          // the first time has not yet occured.
...
void SiegeItemDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
    ...
    if (self->genericValue3) { // An indexed effect to play on death
        vector3 upAng;
...
void SiegeItemUse(gentity_t *ent, gentity_t *other, gentity_t *activator) { // once used, become active
    if (ent->spawnflags & SIEGEITEM_STARTOFFRADAR) {                        // start showing on radar
        ent->s.eFlags |= EF_RADAROBJECT;

        if (!(ent->s.eFlags & EF_NODRAW)) { // we've nothing else to do here
            return;
        }
    } else { // make sure it's showing up
        ent->s.eFlags |= EF_RADAROBJECT;
    }

    if (ent->genericValue11 || !ent->takedamage) { // We want to be able to walk into it to pick it up then.
    ...
    if (ent->paintarget && ent->paintarget[0]) { // want to be on this guy's origin now then
        ...
        if (targ && targ->inuse) {
            ...
            if (tr.startsolid) { // bad spawning area, try again with the trace up a bit.
                ...
                if (tr.startsolid) { // hmm, well that didn't work. try one last time with the item back
                    ...
                    if (tr.startsolid) { // crap, that's all we got.  just spawn at the defualt location.
                        return;
                    }
                }
            }
...
void SP_misc_siege_item(gentity_t *ent) {
    ...
    if (ent->genericValue1) { // if we're using physics we want lerporigin smoothing
    ...
    if (s && s[0]) { // We have a pickup sound, so index it now.
        ent->noise_index = G_SoundIndex(s);
    }

    G_SpawnString("deathfx", "", &s);

    if (s && s[0]) { // We have a death effect, so index it now.
        ent->genericValue3 = G_EffectIndex(s);
    }

    G_SpawnString("respawnfx", "", &s);

    if (s && s[0]) { // We have a respawn effect, so index it now.
    ...
    if (!Q_stricmp(&ent->model[strlen(ent->model) - 4], ".glm")) { // apparently so.
        ent->s.modelGhoul2 = 1;
    }
    ...
    if (ent->health > 0) { // If it has health, it can be killed.
        int t;
        ...
        if (t) { // a non-0 maxhealth value will mean we want to show the health on the hud
    ...
    if ((!ent->targetname || !ent->targetname[0]) || (ent->spawnflags & SIEGEITEM_STARTOFFRADAR)) {
        if (canpickup || !ent->takedamage) { // We want to be able to walk into it to pick it up then.
            ent->r.contents = CONTENTS_TRIGGER;
            ent->clipmask = CONTENTS_SOLID | CONTENTS_TERRAIN;
            if (canpickup) {
                ent->touch = SiegeItemTouch;
...
void G_SiegeClientExData(gentity_t *msgTarg) {
    ...
    while (i < level.num_entities && count < MAX_EXDATA_ENTS_TO_SEND) {
        ...
        if (ent->inuse && ent->client && msgTarg->s.number != ent->s.number && ent->s.eType == ET_PLAYER &&
            msgTarg->client->sess.sessionTeam == ent->client->sess.sessionTeam &&
            trap->InPVS(&msgTarg->client->ps.origin, &ent->client->ps.origin)) { // another client in the same pvs, send his jive
            if (count) {                                                         // append a seperating space if we are not the first in the list
                Q_strcat(str, sizeof(str), " ");
            } else { // otherwise create the prepended chunk
                strcpy(str, "sxd ");
            }

            // append the stats
            Com_sprintf(scratch, sizeof(scratch), "%i|%i|%i|%i", ent->s.number, ent->client->ps.stats[STAT_HEALTH], ent->client->ps.stats[STAT_MAX_HEALTH],
