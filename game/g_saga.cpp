...
void InitSiegeMode( void ) {
	...
	if ( BG_SiegeGetValueGroup( siege_info, "Teams", teams ) ) {
		if ( g_siegeTeam1.stringlen() && Q_stricmp( g_siegeTeam1.getStr(), "none" ) ) { //check for override
			Q_strncpyz( team1, g_siegeTeam1.getStr(), sizeof(team1) );
		}
		else { //otherwise use level default
			BG_SiegeGetPairedValue( teams, "team1", team1 );
		}

		if ( g_siegeTeam2.stringlen() && Q_stricmp( g_siegeTeam2.getStr(), "none" ) ) { //check for override
			Q_strncpyz( team2, g_siegeTeam2.getStr(), sizeof(team2) );
		}
		else { //otherwise use level default
...
void UseSiegeTarget( gentity_t *other, gentity_t *en, char *target ) { //actually use the player which triggered the object which triggered the siege objective to trigger the target
	gentity_t		*t;
	gentity_t		*ent;
...
void BroadcastObjectiveCompletion( int team, int objective, int final, int client ) {
	if ( client != ENTITYNUM_NONE && g_entities[client].client && g_entities[client].client->sess.sessionTeam == team ) { //guy who completed this objective gets points, providing he's on the opposing team
...
void SiegeDoTeamAssign( void ) {
	...
	while ( i < MAX_CLIENTS ) {
		...
		if ( ent->inuse && ent->client &&
			ent->client->pers.connected == CON_CONNECTED ) { //a connected client, switch his frickin teams around
...
void SiegeRoundComplete( int winningteam, int winningclient ) {
	...
	if ( BG_SiegeGetValueGroup( siege_info, teamstr, gParseObjectives ) ) {
		if ( !BG_SiegeGetPairedValue( gParseObjectives, "roundover_target", teamstr ) ) { //didn't find the name of the thing to target upon win, just logexit now then.
	...
	if ( g_siegeTeamSwitch.getInt() &&
		(imperial_time_limit || rebel_time_limit) ) { //handle stupid team switching crap
		int time = 0;
		...
		while ( i < MAX_CLIENTS ) {
			...
			if ( ent->inuse && ent->client ) {
				if ( ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
					!(ent->client->ps.pm_flags & PMF_FOLLOW) ) { //not a spec, just respawn them
					spawnEnt = qtrue;
				}
...
void SiegeItemThink( gentity_t *ent ) {
	...
	if ( ent->genericValue8 != ENTITYNUM_NONE ) { //Just keep sticking it on top of the owner. We need it in the same PVS as him so it will render bolted onto him properly.
...
void SiegeItemTouch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( self->target2 && self->target2[0] && (!self->genericValue4 || !self->genericValue5) ) { //fire the target for pickup, if it's set to fire every time, or set to only fire the first time and the first time has not yet occured.
