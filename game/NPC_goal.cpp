...
qboolean G_BoundsOverlap(const vector3 *mins1, const vector3 *maxs1, const vector3 *mins2,
                         const vector3 *maxs2) { // NOTE: flush up against counts as overlapping
    if (mins1->x > maxs2->x || mins1->y > maxs2->y || mins1->z > maxs2->z || maxs1->x < mins2->x || maxs1->y < mins2->y || maxs1->z < mins2->z)
        return qfalse;
...
gentity_t *UpdateGoal(void) {
    ...
    if (!NPCInfo->goalEntity->inuse) { // Somehow freed it, but didn't clear it
        NPC_ClearGoal();
        return NULL;
    }
