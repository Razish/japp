...
const int mindTrickTime[NUM_FORCE_POWER_LEVELS] = {0, // none
                                                   5000, 10000, 15000};
...
void WP_InitForcePowers(gentity_t *ent) {
    ...
    if (fpStringError) {
        ...
        trap->SendServerCommand(ent - g_entities, va("print \"" S_COLOR_RED "Invalid forcepowers string (%s), setting default\n\"", fpStringError));
    }

    if (g_forceBasedTeams.integer) {
        if (ent->client->sess.sessionTeam == TEAM_RED) {
            warnClient = !BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, BG_HasSetSaberOnly(), FORCESIDE_DARK,
                                                  level.gametype, g_forcePowerDisable.integer);
        } else if (ent->client->sess.sessionTeam == TEAM_BLUE) {
            warnClient = !BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, BG_HasSetSaberOnly(), FORCESIDE_LIGHT,
                                                  level.gametype, g_forcePowerDisable.integer);
        } else {
            warnClient = !BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, BG_HasSetSaberOnly(), FORCESIDE_NEUTRAL,
                                                  level.gametype, g_forcePowerDisable.integer);
        }
    } else {
        warnClient = !BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, BG_HasSetSaberOnly(), FORCESIDE_NEUTRAL, level.gametype,
...
int ForcePowerUsableOn(gentity_t *attacker, gentity_t *other, forcePowers_t forcePower) {
    ...
    if (other && other->client && other->s.eType == ET_NPC && other->s.NPC_class == CLASS_VEHICLE) { // can't use the force on vehicles.. except lightning
    ...
    if (other && other->client && other->s.eType == ET_NPC && level.gametype == GT_SIEGE) { // can't use powers at all on npc's normally in siege...
        return 0;
    }
...
qboolean WP_ForcePowerAvailable(gentity_t *self, forcePowers_t forcePower, int overrideAmt) {
    int drain = overrideAmt ? overrideAmt : forcePowerNeeded[self->client->ps.fd.forcePowerLevel[forcePower]][forcePower];
    ...
    if ((forcePower == FP_DRAIN || forcePower == FP_LIGHTNING) && self->client->ps.fd.forcePower >= 25) { // it's ok then, drain/lightning are actually duration
        return qtrue;
    }
...
qboolean WP_ForcePowerUsable(gentity_t *self, forcePowers_t forcePower) {
    ...
    if (self->health <= 0 || self->client->ps.stats[STAT_HEALTH] <= 0 || (self->client->ps.eFlags & EF_DEAD)) {
        return qfalse;
    }
...
int WP_AbsorbConversion(gentity_t *attacked, int atdAbsLevel, gentity_t *attacker, int atPower, int atPowerLevel, int atForceSpent) {
    ...
    if (atPower != FP_LIGHTNING && atPower != FP_DRAIN && atPower != FP_GRIP && atPower != FP_PUSH && atPower != FP_PULL) { // Only these powers can be absorbed
        return -1;
    }
...
void ForceTeamHeal(gentity_t *self) {
    ...
    for (i = 0, ent = g_entities; i < level.maxclients; i++, ent++) {
        if (ent && ent->client && self != ent && OnSameTeam(self, ent) && ent->client->ps.stats[STAT_HEALTH] < ent->client->ps.stats[STAT_MAX_HEALTH] &&
...
void ForceGrip(gentity_t *self) {
    ...
    if (self->client->ps.weaponTime > 0) {
        if (japp_fixGetups.integer && (self->client->ps.legsAnim == BOTH_GETUP_BROLL_R || self->client->ps.legsAnim == BOTH_GETUP_BROLL_L ||
                                       self->client->ps.legsAnim == BOTH_GETUP_BROLL_F || self->client->ps.legsAnim == BOTH_GETUP_BROLL_B)) {
        } else if (japp_fixRoll.integer && ((self->client->ps.legsAnim == BOTH_ROLL_F || self->client->ps.legsAnim == BOTH_ROLL_B ||
                                             self->client->ps.legsAnim == BOTH_ROLL_R || self->client->ps.legsAnim == BOTH_ROLL_L))) {
        } else {
            return;
        }
    }
    ...
    if (tr.fraction != 1.0f && tr.entityNum != ENTITYNUM_NONE && g_entities[tr.entityNum].client && !g_entities[tr.entityNum].client->ps.fd.forceGripCripple &&
        g_entities[tr.entityNum].client->ps.fd.forceGripBeingGripped < level.time && ForcePowerUsableOn(self, &g_entities[tr.entityNum], FP_GRIP) &&
        (g_friendlyFire.integer || !OnSameTeam(self, &g_entities[tr.entityNum]))) // don't grip someone who's still crippled
    {
        if (g_entities[tr.entityNum].s.number < MAX_CLIENTS && g_entities[tr.entityNum].client->ps.m_iVehicleNum) { // a player on a vehicle
            ...
            if (vehEnt->inuse && vehEnt->client && vehEnt->m_pVehicle) {
                if (vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER || vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL) { // push the guy off
...
void ForceSpeed(gentity_t *self, int forceDuration) {
    ...
    if (self->client->holdingObjectiveItem >= MAX_CLIENTS && self->client->holdingObjectiveItem < ENTITYNUM_WORLD) { // holding Siege item
...
void ForceSeeing(gentity_t *self) {
    ...
    if (self->client->ps.forceAllowDeactivateTime < level.time && (self->client->ps.fd.forcePowersActive & (1 << FP_SEE))) {
...
void ForceProtect(gentity_t *self) {
    ...
    if (self->client->ps.forceAllowDeactivateTime < level.time && (self->client->ps.fd.forcePowersActive & (1 << FP_PROTECT))) {
...
void ForceAbsorb(gentity_t *self) {
    ...
    if (self->client->ps.forceAllowDeactivateTime < level.time && (self->client->ps.fd.forcePowersActive & (1 << FP_ABSORB))) {
        WP_ForcePowerStop(self, FP_ABSORB);
        return;
    }
...
void ForceRage(gentity_t *self) {
    ...
    if (self->client->ps.forceAllowDeactivateTime < level.time && (self->client->ps.fd.forcePowersActive & (1 << FP_RAGE))) {
        WP_ForcePowerStop(self, FP_RAGE);
        return;
    }
    ...
    if (traceEnt && traceEnt->takedamage) {
        ...
        if (traceEnt->client) {                                    // an enemy or object
            ...
            if (ForcePowerUsableOn(self, traceEnt, FP_LIGHTNING)) {
                ...
                if (traceEnt->client) {
                    modPowerLevel = WP_AbsorbConversion(traceEnt, traceEnt->client->ps.fd.forcePowerLevel[FP_ABSORB], self, FP_LIGHTNING,
                                                        self->client->ps.fd.forcePowerLevel[FP_LIGHTNING], 1);
                }
                ...
                if (self->client->ps.weapon == WP_MELEE && self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2) { // 2-handed lightning
                ...
                if (traceEnt->client) {
                    ...
                    if (traceEnt->client->ps.electrifyTime <
                        (level.time + 400)) { // only update every 400ms to reduce bandwidth usage (as it is passing a 32-bit time value)
...
void ForceShootLightning(gentity_t *self) {
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2) { // arc
        ...
        for (e = 0; e < numListedEntities; e++) {
            ...
            for (i = 0; i < 3; i++) {
                if (center.raw[i] < traceEnt->r.absmin.raw[i])
                    v.raw[i] = traceEnt->r.absmin.raw[i] - center.raw[i];
                else if (center.raw[i] > traceEnt->r.absmax.raw[i])
                    v.raw[i] = center.raw[i] - traceEnt->r.absmax.raw[i];
                else
                    v.raw[i] = 0;
            }
...
void ForceDrainDamage(gentity_t *self, gentity_t *traceEnt, vector3 *dir, vector3 *impactPoint) {
    ...
    if (traceEnt && traceEnt->takedamage) {
        if (traceEnt->client && (!OnSameTeam(self, traceEnt) || g_friendlyFire.integer) && self->client->ps.fd.forceDrainTime < level.time &&
            traceEnt->client->ps.fd.forcePower) {                   // an enemy or object
            if (!traceEnt->client && traceEnt->s.eType == ET_NPC) { // g2animent
                if (traceEnt->s.genericenemyindex < level.time) {
                    traceEnt->s.genericenemyindex = level.time + 2000;
                }
            ...
            if (ForcePowerUsableOn(self, traceEnt, FP_DRAIN)) {
                ...
                if (traceEnt->client) {
                    modPowerLevel = WP_AbsorbConversion(traceEnt, traceEnt->client->ps.fd.forcePowerLevel[FP_ABSORB], self, FP_DRAIN,
                                                        self->client->ps.fd.forcePowerLevel[FP_DRAIN], 1);
                }
                ...
                if (self->client->ps.stats[STAT_HEALTH] < self->client->ps.stats[STAT_MAX_HEALTH] && self->health > 0 &&
...
int ForceShootDrain(gentity_t *self) {
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2) { // arc
        ...
        trap->Trace(&tr, &self->client->ps.origin, &vec3_origin, &vec3_origin, &end, self->s.number, MASK_SHOT, qfalse, 0, 0);
        if (tr.entityNum == ENTITYNUM_NONE || tr.fraction == 1.0f || tr.allsolid || tr.startsolid || !g_entities[tr.entityNum].client ||
...
void ForceJumpCharge(gentity_t *self, usercmd_t *ucmd) { // I guess this is unused now. Was used for the "charge" jump type.
    ...
    if (self->client->ps.fd.forceJumpCharge / forceJumpChargeInterval / (FORCE_JUMP_CHARGE_TIME / FRAMETIME) *
            forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]][FP_LEVITATION] >
        self->client->ps.fd.forcePower) { // can't use more than you have
        G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50], CHAN_VOICE);
...
void ForceJump(gentity_t *self, usercmd_t *ucmd) {
    ...
    WP_ForcePowerStart(self, FP_LEVITATION,
                       self->client->ps.fd.forceJumpCharge / forceJumpChargeInterval / (FORCE_JUMP_CHARGE_TIME / FRAMETIME) *
                           forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]][FP_LEVITATION]);
...
qboolean ForceTelepathyCheckDirectNPCTarget(gentity_t *self, trace_t *tr, qboolean *tookPower) {
    ...
    if (targetLive && traceEnt->NPC) { // hit an organic non-player
        ...
        if (G_ActivateBehavior(traceEnt, BSET_MINDTRICK)) { // activated a script on him
            ...
            if ((traceEnt->NPC->scriptFlags & SCF_NO_MIND_TRICK)) {
            } else if (traceEnt->s.weapon != WP_SABER && traceEnt->client->NPC_class != CLASS_REBORN) { // haha!  Jedi aren't easily confused!
                if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_2) {                // turn them to our side
                    ...
                    // somehow confuse them?  Set don't fire to true for a while?  Drop their aggression?  Maybe just take their enemy away and don't let them
                    // pick one up for a while unless shot?
                    traceEnt->NPC->confusionTime =
                        level.time + mindTrickTime[self->client->ps.fd.forcePowerLevel[FP_TELEPATHY]]; // confused for about 10 seconds
        ...
        if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_1 &&
            tr->fraction * 2048 > 64) { // don't create a diversion less than 64 from you of if at power level 1
...
void ForceTelepathy(gentity_t *self) {
    ...
    if (self->client->ps.powerups[PW_REDFLAG] || self->client->ps.powerups[PW_BLUEFLAG]) { // can't mindtrick while carrying the flag
        return;
    }

    if (self->client->ps.forceAllowDeactivateTime < level.time && (self->client->ps.fd.forcePowersActive & (1 << FP_TELEPATHY))) {
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] == FORCE_LEVEL_1) {
        if (tr.fraction != 1.0f && tr.entityNum != ENTITYNUM_NONE && g_entities[tr.entityNum].inuse && g_entities[tr.entityNum].client &&
            g_entities[tr.entityNum].client->pers.connected && g_entities[tr.entityNum].client->sess.sessionTeam != TEAM_SPECTATOR) {
...
void GEntity_UseFunc(gentity_t *self, gentity_t *other, gentity_t *activator) { GlobalUse(self, other, activator); }
...
void G_LetGoOfWall(gentity_t *ent) {
    ...
    if (BG_InReboundJump(ent->client->ps.legsAnim) || BG_InReboundHold(ent->client->ps.legsAnim)) {
        ent->client->ps.legsTimer = 0;
    }
    if (BG_InReboundJump(ent->client->ps.torsoAnim) || BG_InReboundHold(ent->client->ps.torsoAnim)) {
...
void ForceThrow(gentity_t *self, qboolean pull) {
    ...
    float dist, dot1;
    trace_t tr;
    vector3 mins, maxs, v, center, ent_org, size, forward, right, end, dir, fwdangles, pushDir, thispush_org, tfrom, tto, fwd, a;
    ...
    if (powerLevel == FORCE_LEVEL_1) { // can only push/pull targeted things at level 1
        ...
        for (e = 0; e < numListedEntities; e++) {
            ...
            if (ent) { // not in the arc, don't consider it
                ...
                if (ent->client && !InFieldOfVision(&self->client->ps.viewangles, visionArc, &a) && ForcePowerUsableOn(self, ent, (forcePowers_t)powerUse)) {
    ...
    for (e = 0; e < numListedEntities; e++) {
        if (entityList[e] != ENTITYNUM_NONE && entityList[e] >= 0 && entityList[e] < MAX_GENTITIES) {
            ent = &g_entities[entityList[e]];
        } else {
            ent = NULL;
        }
        ...
        if (ent->s.eType != ET_MISSILE) {
            if (japp_itemPush.integer || ent->s.eType != ET_ITEM) {
                ...
                if (Q_stricmp("func_button", ent->classname) == 0) {         // we might push it
                            ...
                            if (Q_stricmp("lightsaber", ent->classname) != 0) {                                                       // not a lightsaber
                                if (Q_stricmp("func_door", ent->classname) != 0 || !(ent->spawnflags & 2 /*MOVER_FORCE_ACTIVATE*/)) { // not a force-usable door
                                    if (Q_stricmp("func_static", ent->classname) != 0 ||
                                        (!(ent->spawnflags & 1 /*F_PUSH*/) && !(ent->spawnflags & 2 /*F_PULL*/))) { // not a force-usable func_static
                    ...
                    } else if (ent->client->NPC_class == CLASS_GALAKMECH || ent->client->NPC_class == CLASS_ATST || ent->client->NPC_class == CLASS_RANCOR) {
    ...
    if (ent_count) {
        ...
        for (x = 0; x < ent_count; x++) {
            ...
            if (push_list[x]->client) {
                modPowerLevel = WP_AbsorbConversion(push_list[x], push_list[x]->client->ps.fd.forcePowerLevel[FP_ABSORB], self, powerUse, powerLevel,
            ...
            if (push_list[x]->client) { // FIXME: make enemy jedi able to hunker down and resist this?
                ...
                if ((modPowerLevel > otherPushPower || push_list[x]->client->ps.m_iVehicleNum) && push_list[x]->client) {
                    if (modPowerLevel == FORCE_LEVEL_3 && push_list[x]->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN) {
                        ...
                        } else if (push_list[x]->s.number < MAX_CLIENTS && push_list[x]->client->ps.m_iVehicleNum && dirLen <= 128.0f) { // a player on a
                                                                                                                                         // vehicle
                ...
                if (push_list[x]->client) {
                    ...
                    if (self->client->ps.fd.forceGripBeingGripped > level.time) { // force the enemy to stop gripping me if I managed to push him
                        if (push_list[x]->client->ps.fd.forceGripEntityNum == self->s.number) {
                            if (modPowerLevel >=
                                push_list[x]->client->ps.fd.forcePowerLevel[FP_GRIP]) { // only break the grip if our push/pull level is >= their grip level
            ...
            } else if (push_list[x]->s.eType == ET_MISSILE && push_list[x]->s.pos.trType != TR_STATIONARY &&
                       (push_list[x]->s.pos.trType != TR_INTERPOLATE ||
                        push_list[x]->s.weapon != WP_THERMAL)) // rolling and stationary thermal detonators are dealt with below
            {
                ...
                if (VectorCompare(&vec3_origin,
                    ...
                    if ((push_list[x]->spawnflags & 1) &&
                        push_list[x]->moverState == MOVER_POS1) { // if at pos1 and started open, make sure we get the center where it *started* because we're
                                                                  // going to add back in the relative values pos1 and pos2
                        VectorSubtract(&center, &push_list[x]->pos1, &center);
                    } else if (!(push_list[x]->spawnflags & 1) &&
                               push_list[x]->moverState == MOVER_POS2) { // if at pos2, make sure we get the center where it *started* because we're going to
                                                                         // add back in the relative values pos1 and pos2
                ...
                if (japp_allowFlagPull.integer && push_list[x]->s.eType == ET_ITEM &&
                    (!strcmp(push_list[x]->classname, "team_CTF_redflag") || !strcmp(push_list[x]->classname, "team_CTF_blueflag"))) { // Flags
                    vector3 end = {0, 0, 0};
...
void WP_ForcePowerStop(gentity_t *self, forcePowers_t forcePower) {
    ...
    case FP_GRIP:
        ...
        if (self->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 && g_entities[self->client->ps.fd.forceGripEntityNum].client &&
            g_entities[self->client->ps.fd.forceGripEntityNum].health > 0 && g_entities[self->client->ps.fd.forceGripEntityNum].inuse &&
            (level.time - g_entities[self->client->ps.fd.forceGripEntityNum].client->ps.fd.forceGripStarted) >
                500) { // if we had our throat crushed in for more than half a second, gasp for air when we're let go
            if (wasActive & (1 << FP_GRIP)) {
                G_EntitySound(&g_entities[self->client->ps.fd.forceGripEntityNum], CHAN_VOICE, G_SoundIndex("*gasp.wav"));
            }
        }

        if (g_entities[self->client->ps.fd.forceGripEntityNum].client && g_entities[self->client->ps.fd.forceGripEntityNum].inuse) {
        ...
        break;
    case FP_LIGHTNING:
        if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] <
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower) {
    ...
    gripLevel = WP_AbsorbConversion(gripEnt, gripEnt->client->ps.fd.forcePowerLevel[FP_ABSORB], self, FP_GRIP, self->client->ps.fd.forcePowerLevel[FP_GRIP],
    ...
    if (!InFront(&gripEnt->client->ps.origin, &self->client->ps.origin, &self->client->ps.viewangles, 0.9f) && gripLevel < FORCE_LEVEL_3) {
        WP_ForcePowerStop(self, forcePower);
        return;
    }
    ...
    if (self->client->ps.fd.forcePowerDebounce[FP_GRIP] <
        level.time) { // 2 damage per second while choking, resulting in 10 damage total (not including The Squeeze<tm>)
    ...
    if (gripLevel == FORCE_LEVEL_2) {
        ...
        if ((level.time - gripEnt->client->ps.fd.forceGripStarted) > 3000 &&
            !self->client->ps.fd.forceGripDamageDebounceTime) { // if we managed to lift him into the air for 2 seconds, give him a crack
...
static void WP_UpdateMindtrickEnts(gentity_t *self) {
    ...
    for (i = 0, ent = g_entities; i < level.maxclients; i++, ent++) {
        if (Q_InBitflags(self->client->ps.fd.forceMindtrickTargetIndex, i, 16) && !Q_InBitflags(level.admin.mindtricked, i, 16)) {
            if (!ent || !ent->client || !ent->inuse || ent->health < 1 || (ent->client->ps.fd.forcePowersActive & (1 << FP_SEE))) {
    ...
    uint32_t tmpTricked[4] = {self->client->ps.fd.forceMindtrickTargetIndex[0] & ~level.admin.mindtricked[0],
                              self->client->ps.fd.forceMindtrickTargetIndex[1] & ~level.admin.mindtricked[1],
                              self->client->ps.fd.forceMindtrickTargetIndex[2] & ~level.admin.mindtricked[2],
                              self->client->ps.fd.forceMindtrickTargetIndex[3] & ~level.admin.mindtricked[3]};
...
static void WP_ForcePowerRun(gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd) {
    ...
    case FP_HEAL:
        ...
        if (self->health > self->client->ps.stats[STAT_MAX_HEALTH]) { // rww - we might start out over max_health and we don't want force heal taking us down to
                                                                      // 100 or whatever max_health is
            WP_ForcePowerStop(self, forcePower);
            break;
        }
    ...
    case FP_SPEED:
        ...
        if (self->client->holdingObjectiveItem >= MAX_CLIENTS && self->client->holdingObjectiveItem < ENTITYNUM_WORLD) {
            if (g_entities[self->client->holdingObjectiveItem].genericValue15) { // disables force powers
                WP_ForcePowerStop(self, forcePower);
            }
        }
    ...
    case FP_GRIP:
        ...
        if (self->client->ps.fd.forcePowerDebounce[FP_PULL] <
            level.time) { // This is sort of not ideal. Using the debounce value reserved for pull for this because pull doesn't need it.
    ...
    case FP_DRAIN:
        ...
        if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1) { // higher than level 1
            if ((cmd->buttons & BUTTON_FORCE_DRAIN) ||
                ((cmd->buttons & BUTTON_FORCEPOWER) && self->client->ps.fd.forcePowerSelected == FP_DRAIN)) { // holding it keeps it going
        ...
        break;
    case FP_LIGHTNING:
        if (self->client->ps.forceHandExtend !=
            HANDEXTEND_FORCE_HOLD) { // Animation for hand extend doesn't end with hand out, so we have to limit lightning intervals by animation intervals
        ...
        if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1) { // higher than level 1
            if ((cmd->buttons & BUTTON_FORCE_LIGHTNING) ||
                ((cmd->buttons & BUTTON_FORCEPOWER) && self->client->ps.fd.forcePowerSelected == FP_LIGHTNING)) { // holding it keeps it going
...
void FindGenericEnemyIndex(gentity_t *self) { // Find another client that would be considered a threat.
    ...
    while (i < MAX_CLIENTS) {
        ...
        if (ent && ent->client && ent->s.number != self->s.number && ent->health > 0 && !OnSameTeam(self, ent) && ent->client->ps.pm_type != PM_INTERMISSION &&
            ent->client->ps.pm_type != PM_SPECTATOR) {
            VectorSubtract(&ent->client->ps.origin, &self->client->ps.origin, &a);
            tlen = VectorLength(&a);

            if (tlen < blen && InFront(&ent->client->ps.origin, &self->client->ps.origin, &self->client->ps.viewangles, 0.8f) &&
...
void SeekerDroneUpdate(gentity_t *self) {
    ...
    if (self->client->ps.droneExistTime >= level.time && self->client->ps.droneExistTime < (level.time + 5000)) {
...
void JediMasterUpdate(gentity_t *self) { // keep jedi master status updated for JM gametype
    ...
    while (i < NUM_FORCE_POWERS) {
        if (self->client->ps.isJediMaster) {
            ...
            if (i == FP_TEAM_HEAL || i == FP_TEAM_FORCE || i == FP_DRAIN ||
                i == FP_ABSORB) { // team powers are useless in JM, absorb is too because no one else has powers to absorb. Drain is just
...
qboolean
G_SpecialRollGetup(gentity_t *self) { // fixme: currently no knockdown will actually land you on your front... so froll's are pretty useless at the moment.
    ...
    */
    ...
    if (/*!self->client->pers.cmd.upmove &&*/
        self->client->pers.cmd.rightmove > 0 && !self->client->pers.cmd.forwardmove) {
        ...
        rolled = qtrue;
               ...
               !self->client->pers.cmd.rightmove && self->client->pers.cmd.forwardmove > 0) {
...
void WP_ForcePowersUpdate(gentity_t *self, usercmd_t *ucmd) {
    ...
    */
    ...
    if (((self->client->sess.selectedFP != self->client->ps.fd.forcePowerSelected) || (self->client->sess.saberLevel != self->client->ps.fd.saberAnimLevel)) &&
    ...
    if (self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN && self->client->ps.forceHandExtendTime >= level.time) {
    ...
    } else if (self->client->ps.forceHandExtend != HANDEXTEND_NONE && self->client->ps.forceHandExtendTime < level.time) {
        if (self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN && !self->client->ps.forceDodgeAnim) {
            if (self->health < 1 || (self->client->ps.eFlags & EF_DEAD))
                ...
                if ((level.time - self->client->ps.forceHandExtendTime) > 4000) { // 4 seconds elapsed, I guess they're too dumb to push something to get up!
                    if (self->client->pers.cmd.upmove && self->client->ps.fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1) { // force getup
                        G_PreDefSound(&self->client->ps.origin, PDSOUND_FORCEJUMP);
                        self->client->ps.forceDodgeAnim = 2;
                        self->client->ps.forceHandExtendTime = level.time + 500;
    ...
    if (self && self->client &&
        (BG_HasYsalamiri(level.gametype, &self->client->ps) || self->client->ps.fd.forceDeactivateAll ||
         self->client->tempSpectate >= level.time)) { // has ysalamiri.. or we want to forcefully stop all his active powers
    ...
    if (self->client->ps.powerups[PW_FORCE_ENLIGHTENED_LIGHT] || self->client->ps.powerups[PW_FORCE_ENLIGHTENED_DARK]) { // enlightenment
        if (!self->client->ps.fd.forceUsingAdded) {
            ...
            while (i < NUM_FORCE_POWERS) {
                ...
                if (!forcePowerDarkLight[i] || self->client->ps.fd.forceSide == forcePowerDarkLight[i]) {
                    self->client->ps.fd.forcePowerLevel[i] = FORCE_LEVEL_3;
                    self->client->ps.fd.forcePowersKnown |= (1 << i);
                }
    ...
    } else if (self->client->ps.fd
                   .forceUsingAdded) { // we don't have enlightenment but we're still using enlightened powers, so clear them back to how they should be.
        for (i = 0; i < NUM_FORCE_POWERS; i++) {
    ...
    if (self->client->ps.fd.forceJumpCharge && self->client->ps.groundEntityNum == ENTITYNUM_NONE &&
        self->client->fjDidJump) { // this was for the "charge" jump method... I guess
...
#ifndef METROID_JUMP
    else if ((ucmd->upmove > 10) && (self->client->ps.pm_flags & PMF_JUMP_HELD) && self->client->ps.groundTime &&
             (level.time - self->client->ps.groundTime) > 150 && !BG_HasYsalamiri(level.gametype, &self->client->ps) &&
             BG_CanUseFPNow(level.gametype, &self->client->ps, level.time, FP_LEVITATION)) { // just charging up
    ...
    if ((!self->client->ps.fd.forcePowersActive || self->client->ps.fd.forcePowersActive == (1 << FP_DRAIN)) && !self->client->ps.saberInFlight &&
                ...
                if (level.gametype == GT_POWERDUEL && self->client->sess.duelTeam == DUELTEAM_LONE) {
                    if (duel_fraglimit.integer) {
                        self->client->ps.fd.forcePowerRegenDebounceTime +=
                            std::max(g_forceRegenTime.integer * (0.6f + (0.3f * (float)self->client->sess.wins / (float)duel_fraglimit.integer)), 1.0f);
                    } else {
