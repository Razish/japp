...
int NAV_TestBestNode( gentity_t *self, int startID, int endID, qboolean failEdge ) {//check only against architectrure
	...
	if ( trace.entityNum < ENTITYNUM_WORLD ) {
		...
		if VALIDSTRING( blocker->classname ) {//special case: doors are architecture, but are dynamic, like entitites
			...
			else if ( blocker->targetname && blocker->s.solid == SOLID_BMODEL && ((blocker->r.contents&CONTENTS_MONSTERCLIP) || (blocker->r.contents&CONTENTS_BOTCLIP)) ) {//some other kind of do not enter entity brush that will probably be removed
...
void NAV_StoreWaypoint( gentity_t *ent ) {
	...
	*/
	...
	if ( ent->targetname )	Q_strncpyz( tempWaypointList[numStoredWaypoints].targetname, ent->targetname, MAX_QPATH );
	if ( ent->target )		Q_strncpyz( tempWaypointList[numStoredWaypoints].target, ent->target, MAX_QPATH );
	if ( ent->target2 )		Q_strncpyz( tempWaypointList[numStoredWaypoints].target2, ent->target2, MAX_QPATH );
	if ( ent->target3 )		Q_strncpyz( tempWaypointList[numStoredWaypoints].target3, ent->target3, MAX_QPATH );
	if ( ent->target4 )		Q_strncpyz( tempWaypointList[numStoredWaypoints].target4, ent->target4, MAX_QPATH );
