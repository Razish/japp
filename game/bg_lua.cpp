    ...
    while (IteratePluginsTemp(&plugin, false)) {
        if (plugin->enabled) {
            trap->Print(S_COLOR_WHITE "  %s " S_COLOR_GREEN "v%i.%i.%i " S_COLOR_WHITE " (%s)\n", plugin->longname, plugin->version.major,
                        plugin->version.minor, plugin->version.patch, plugin->name);
        } else {
...
void PushInfostring(lua_State *L, const char *info) {
    ...
    while (s) {
        ...
        lua_pushstring(L, ip.key);
        lua_pushstring(L, ip.value);
        lua_settable(L, top);
    }
}
...
static int RegisterPlugin(lua_State *L) {
    ...
    if (semver_gt(ls.currentPlugin->requiredJPLuaVersion, jpluaVersion)) {
        luaO_pushfstring(L, S_COLOR_RED " %s requires JPLua v%d.%d.%d, you have JPLua v%d.%d.%d\n", ls.currentPlugin->name,
                         ...
                         ls.currentPlugin->requiredJPLuaVersion.patch, jpluaVersion.major, jpluaVersion.minor, jpluaVersion.patch);
...
bool EnablePlugin(plugin_t *plugin) {
    ...
    ls.currentPlugin = current;
    trap->Print(S_COLOR_CYAN "  loaded %s " S_COLOR_GREEN "v%i.%i.%i " S_COLOR_WHITE " (%s)\n", plugin->longname, plugin->version.major, plugin->version.minor,
                plugin->version.patch, plugin->name);
    plugin->enabled = true;
...
static int System_Index(lua_State *L) {
    ...
    if (!strcmp(key, "isClient")) {
        ...
        while (IteratePluginsTemp(&plugin, false)) {
            ...
            lua_pushboolean(L, plugin->enabled);
            lua_settable(L, top2);
            lua_pushstring(L, "name");
            lua_pushstring(L, plugin->name);
            lua_settable(L, top2);
            lua_pushstring(L, "longname");
            lua_pushstring(L, plugin->longname);
            lua_settable(L, top2);
            semver_render(&plugin->requiredJPLuaVersion, buf);
            lua_pushstring(L, "requiredJPLuaversion");
...
static int System_NewIndex(lua_State *L) { return luaL_error(L, "Attempt to modify read-only data (JPLua)"); }
static const struct luaL_Reg systemMeta[] = {{"__index", System_Index}, {"__newindex", System_NewIndex}, {NULL, NULL}};
...
#ifdef PROJECT_CGAME
static int Export_GetItemName(lua_State *L) {
    int itemNum = luaL_checkinteger(L, 1);
    if (itemNum < 1 || itemNum >= bg_numItems) {
        lua_pushnil(L);
        return 1;
    }
    lua_pushstring(L, CG_GetStringEdString("SP_INGAME", bg_itemlist[itemNum].classname));
    return 1;
}
#endif
...
static int Export_GetMousePos(lua_State *L) {
    if (trap->Key_GetCatcher() & KEYCATCH_CGAME) {
        ...
        top = lua_gettop(L);
        lua_pushstring(L, "x");
        lua_pushinteger(L, cgs.cursorX);
        lua_settop(L, top);
        lua_pushstring(L, "y");
        lua_pushinteger(L, cgs.cursorY);
        lua_settop(L, top);
    } else
        lua_pushnil(L);
...
int Export_TestLine(lua_State *L) {
    ...
    lua_getfield(L, 1, "x");
    start.x = lua_tonumber(L, -1);
    lua_getfield(L, 1, "y");
    start.y = lua_tonumber(L, -1);
    lua_getfield(L, 1, "z");
    start.z = lua_tonumber(L, -1);
...
static const importTable_t imports[] = {
...
#ifdef PROJECT_GAME
    {"ConnectToDB", ConnectToDB}, // ConnectToDB ( int type(1 - MySQL , 2 - SQLite), ...) // SQLite (type, string path) || MySQL ( type, string host, string
                                  // user, string db, string password, int port )
#endif
    ...
    {"DrawRotatedPic", Export_DrawRotatedPic}, // DrawPic( float x, float y, float width, float height, float angle, table { float r, float g, float b, float a
                                               // }, integer shaderHandle )
    {"DrawText", Export_DrawText}, // DrawText( float x, float y, string text, table { float r, float g, float b, float a }, float scale, integer fontStyle,
                                   // integer fontIndex )
    ...
    {"GetItemName", Export_GetItemName},     // string GetItemName( integer itemNum )
...
bool IsInitialised(void) { return ls.L != nullptr; }
