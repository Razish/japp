...
static void NPC_CheckAttacker(gentity_t *other, int mod) {
    ...
    if (NPC->client->ps.weapon == WP_SABER) { // I'm a jedi
        if (mod == MOD_SABER) {               // I was hit by a saber  FIXME: what if this was a thrown saber?
...
float NPC_GetPainChance(gentity_t *self, int damage) {
    ...
    pain_chance = (float)(self->client->ps.stats[STAT_MAX_HEALTH] - self->health) / (self->client->ps.stats[STAT_MAX_HEALTH] * 2.0f) +
                  (float)damage / (self->client->ps.stats[STAT_MAX_HEALTH] / 2.0f);
    switch (g_spSkill.integer) {
    case 0: // easy
        // return 0.75f;
        break;
...
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, vector3 *point, int damage, int mod, int hitLoc, int voiceEvent) {
    ...
    if (self->s.weapon == WP_THERMAL && self->client->ps.weaponTime > 0) { // don't interrupt thermal throwing anim
        return;
    } else if (self->client->NPC_class == CLASS_GALAKMECH) {
        if (hitLoc == HL_GENERIC1) { // hit the antenna!
            pain_chance = 1.0f;
        ...
        else if (self->health > 200 && damage < 100) { // have a *lot* of health
            pain_chance = 0.05f;
        } else { // the lower my health and greater the damage, the more likely I am to play a pain anim
            pain_chance = (200.0f - self->health) / 100.0f + damage / 50.0f;
        }
    } else if (self->client && self->client->playerTeam == NPCTEAM_PLAYER && other && !other->s.number) { // ally shot by player always complains
        pain_chance = 1.1f;
    } else {
        if (other && (other->s.weapon == WP_SABER || mod == MOD_CRUSH)) {
            pain_chance = 1.0f;        // always take pain from saber
        } else if (mod == MOD_MELEE) { // higher in rank (skill) we are, less likely we are to be fazed by a punch
    ...
    if (random() < pain_chance) {
        ...
        if (self->client->ps.fd.forceGripBeingGripped < level.time) { // not being force-gripped or force-drained
            if (                                                      /*G_CheckForStrongAttackMomentum( self ) //rwwFIXMEFIXME: Is this needed?
                                                                         ||*/
                PM_SpinningAnim(self->client->ps.legsAnim) || BG_SaberInSpecialAttack(self->client->ps.torsoAnim) || PM_InKnockDown(&self->client->ps) ||
                PM_RollingAnim(self->client->ps.legsAnim) ||
                (BG_FlippingAnim(self->client->ps.legsAnim) &&
                 !PM_InCartwheel(self->client->ps.legsAnim))) { // strong attacks, rolls, knockdowns, flips and spins cannot be interrupted by pain
                return;
            } else { // play an anim
                int parts;

                if (self->client->NPC_class == CLASS_GALAKMECH) { // only has 1 for now
                ...
                } else if (self->s.weapon == WP_SABER) { // temp HACK: these are the only 2 pain anims that look good when holding a saber
                    pain_anim = BG_PickAnim(self->localAnimIndex, BOTH_PAIN2, BOTH_PAIN3);
                }
                /*
...
void NPC_Pain(gentity_t *self, gentity_t *attacker, int damage) {
    ...
    if (self->client->playerTeam && other->client && otherTeam == self->client->playerTeam
    ...
    {                                                       // hit by a teammate
        if (other != self->enemy && self != other->enemy) { // we weren't already enemies
            if (self->enemy || other->enemy

                //|| (other->s.number&&other->s.number!=player->client->ps.viewEntity)
                // rwwFIXMEFIXME: same

                /*|| (!other->s.number&&Q_irand( 0, 3 ))*/) { // if one of us actually has an enemy already, it's okay, just an accident OR wasn't hit by player
                                                              // or someone controlled by player OR player hit ally and didn't get 25% chance of getting mad
                                                              // (FIXME:accumulate anger+base on diff?)
                ...
                if (self->client && self->NPC) { // Run any pain instructions
            ...
            {                                         // dammit, stop that!
                if (self->NPC->charmedTime) {         // mindtricked
                    ...
                    return;
                } else if (G_ActivateBehavior(self, BSET_FFIRE)) { // we have a specific script to run, so do that instead
                    return;
                } else { // okay, we're going to turn on our ally, we need to set and lock our enemy and put ourselves in a bstate that lets us attack him (and
                         // clear any flags that would stop us)
    ...
    if (NPCInfo->ignorePain == qfalse) {
        ...
        if (NPC->enemy != other && NPC != other) { // not already mad at them
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
    ...
    if (self->message && self->health <= 0) { // I am dead and carrying a key
        // if ( other && player && player->health > 0 && other == player )
        if (other && other->client && other->s.number < MAX_CLIENTS) { // player touched me
            /*
            char *text;
            qboolean	keyTaken;
            //give him my key
    ...
    if (other->client) { // FIXME:  if pushing against another bot, both ucmd.rightmove = 127???
        ...
        if (/*!(self->svFlags&SVF_LOCKEDENEMY) && !(self->svFlags&SVF_IGNORE_ENEMIES) &&*/ !(other->flags & FL_NOTARGET)) {
            if (self->client->enemyTeam) {                                  // See if we bumped into an enemy
                if (other->client->playerTeam == self->client->enemyTeam) { // bumped into an enemy
                    if (NPCInfo->behaviorState != BS_HUNT_AND_KILL &&
                        !NPCInfo->tempBehavior) { // MCG - Begin: checking specific BS mode here, this is bad, a HACK
                        // FIXME: not medics?
                        if (NPC->enemy != other) { // not already mad at them
                            G_SetEnemy(NPC, other);
                        }
...
void NPC_TempLookTarget(gentity_t *self, int lookEntNum, int minLookTime, int maxLookTime) {
    ...
    if (!NPC_CheckLookTarget(self)) { // Not already looking at something else
...
void NPC_Respond(gentity_t *self, int userNum) {
    ...
    */
    ...
    if (!Q_irand(0, 1)) { // set looktarget to them for a second or two
    ...
    case CLASS_BESPIN_COP:
        if (!Q_stricmp("bespincop", self->NPC_type)) { // variant 1
            if (self->enemy) {
                if (Q_irand(0, 9) > 6) {
                    event = Q_irand(EV_CHASE1, EV_CHASE3);
...
void NPC_UseResponse(gentity_t *self, gentity_t *user, qboolean useWhenDone) {
    ...
    if (user->s.number >= MAX_CLIENTS) { // not used by the player
        if (useWhenDone) {
    ...
    if (user->client && self->client->playerTeam != user->client->playerTeam &&
        self->client->playerTeam != NPCTEAM_NEUTRAL) { // only those on the same team react
        if (useWhenDone) {
            G_ActivateBehavior(self, BSET_USE);
        }
        return;
    ...
    if (self->NPC->blockedSpeechDebounceTime > level.time) { // I'm not responding right now
        return;
    }
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
    ...
    if (self->client && self->NPC) {
        ...
        //			NPC_TakePatient( activator );
        //		}
        else if (activator && !self->enemy && activator->s.number >= 0 && activator->s.number < MAX_CLIENTS &&
