...
qboolean G_HeavyMelee(gentity_t *attacker) {
    if (level.gametype == GT_SIEGE && attacker && attacker->client && attacker->client->siegeClass != -1 &&
...
void TossClientItems(gentity_t *self) {
    ...
    if (weapon > WP_BRYAR_PISTOL && weapon != WP_EMPLACED_GUN && weapon != WP_TURRET && self->client->ps.ammo[weaponData[weapon].ammoIndex]) {
        gentity_t *te;
    ...
    if (level.gametype != GT_TEAM && level.gametype != GT_SIEGE) {
        ...
        for (i = 1; i < PW_NUM_POWERUPS; i++) {
            if (self->client->ps.powerups[i] > level.time) {
                ...
                if ((self->client->pers.adminData.logineffect == PW_FORCE_ENLIGHTENED_DARK ||
                     self->client->pers.adminData.logineffect == PW_FORCE_ENLIGHTENED_LIGHT || self->client->pers.adminData.logineffect == PW_FORCE_BOON) &&
                    self->client->pers.adminData.logineffect == i) {
                    continue;
                }
...
void body_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
    ...
    if (self->s.eType == ET_NPC) { // well, just rem it then, so long as it's done with its death anim and it's not a standard weapon.
        if (self->client && self->client->ps.torsoTimer <= 0 &&
            (meansOfDeath == MOD_UNKNOWN || meansOfDeath == MOD_WATER || meansOfDeath == MOD_SLIME || meansOfDeath == MOD_LAVA || meansOfDeath == MOD_CRUSH ||
             meansOfDeath == MOD_TELEFRAG || meansOfDeath == MOD_FALLING || meansOfDeath == MOD_SUICIDE || meansOfDeath == MOD_TARGET_LASER ||
...
const char *modNames[MOD_MAX] = {"MOD_UNKNOWN",
                                 "MOD_STUN_BATON",
                                 "MOD_MELEE",
                                 "MOD_SABER",
                                 "MOD_BRYAR_PISTOL",
...
static int G_CheckSpecialDeathAnim(gentity_t *self, vector3 *point, int damage, int mod, int hitLoc) {
    ...
    if (BG_InRoll(&self->client->ps, self->client->ps.legsAnim)) {
        ...
        int animLength = bgAllAnims[self->localAnimIndex].anims[self->client->ps.legsAnim].numFrames *
                         fabsf((float)(bgHumanoidAnimations[self->client->ps.legsAnim].frameLerp));
...
int G_PickDeathAnim(gentity_t *self, vector3 *point, int damage, int mod,
                    int hitLoc) { // FIXME: play dead flop anims on body if in an appropriate _DEAD anim when this func is called
    int deathAnim;
    int max_health;
    int legAnim = 0;
    vector3 objVelocity;
...
void G_CheckVictoryScript(gentity_t *self) {
    if (!G_ActivateBehavior(self, BSET_VICTORY)) {
        ...
        if (self->NPC && self->NPC->group && self->NPC->group->commander && self->NPC->group->commander->NPC &&
            self->NPC->group->commander->NPC->rank > self->NPC->rank && !Q_irand(0, 2)) { // sometimes have the group commander speak instead
...
void G_AddPowerDuelScore(int team, int score) {
    ...
    while (i < MAX_CLIENTS) {
        check = &g_entities[i];
        if (check->inuse && check->client && check->client->pers.connected == CON_CONNECTED && !check->client->iAmALoser &&
            check->client->ps.stats[STAT_HEALTH] > 0 && check->client->sess.sessionTeam != TEAM_SPECTATOR &&
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
    ...
    if (self->s.eType == ET_NPC && self->s.NPC_class == CLASS_VEHICLE && self->m_pVehicle && !self->m_pVehicle->m_pVehicleInfo->explosionDelay &&
        (self->m_pVehicle->m_pPilot || self->m_pVehicle->m_iNumPassengers > 0 ||
         self->m_pVehicle->m_pDroidUnit)) { // kill everyone on board in the name of the attacker... if the vehicle has no death delay
        ...
        if (self->client->ps.otherKillerTime >= level.time) { // use the last attacker
            ...
            if (!murderer->inuse || !murderer->client) {
                ...
                if (murderer->s.number >= MAX_CLIENTS && murderer->s.eType == ET_NPC && murderer->s.NPC_class == CLASS_VEHICLE && murderer->m_pVehicle &&
            ...
            if (attacker->s.number >= MAX_CLIENTS && attacker->s.eType == ET_NPC && attacker->s.NPC_class == CLASS_VEHICLE && attacker->m_pVehicle &&
        ...
        if (self->m_pVehicle->m_pVehicleInfo->hideRider) { // pilot is *inside* me, so kill him, too
            ...
            if (self->m_pVehicle->m_pVehicleInfo) { // FIXME: this wile got stuck in an endless loop, that's BAD!!  This method SUCKS (not initting "i", not
                                                    // incrementing it or using it directly, all sorts of badness), so I'm rewriting it
    ...
    if (self->client->holdingObjectiveItem >
        0) { // carrying a siege objective item - make sure it updates and removes itself from us now in case this is an instant death-respawn situation
    ...
    if ((self->client->inSpaceIndex && self->client->inSpaceIndex != ENTITYNUM_NONE) || (self->client->ps.eFlags2 & EF2_SHIP_DEATH)) {
        self->client->noCorpse = qtrue;
    }

    if (self->client->NPC_class != CLASS_VEHICLE && self->client->ps.m_iVehicleNum) { // I'm riding a vehicle
    ...
    if (self->client->ps.weapon == WP_SABER && !self->client->ps.saberHolstered && self->client->ps.saberEntityNum) {
        if (!self->client->ps.saberInFlight && self->client->saber[0].soundOff) {
    ...
    */
    ...
    if (inflictor && inflictor->activator && !inflictor->client && !attacker->client && inflictor->activator->client && inflictor->activator->inuse &&
    ...
    if (g_austrian.integer && (level.gametype == GT_DUEL) && level.numPlayingClients >= 2) {
        int spawnTime = (level.clients[level.sortedClients[0]].respawnTime > level.clients[level.sortedClients[1]].respawnTime)
    ...
    if (attacker && attacker->client) {
        ...
        if (attacker == self || OnSameTeam(self, attacker)) {
            if (level.gametype == GT_DUEL) { // in duel, if you kill yourself, the person you are dueling against gets a kill for it
                ...
                if (otherClNum >= 0 && otherClNum < MAX_CLIENTS && g_entities[otherClNum].inuse && g_entities[otherClNum].client &&
            ...
            if (level.gametype == GT_JEDIMASTER) {
                if ((attacker->client && attacker->client->ps.isJediMaster) || (self->client && self->client->ps.isJediMaster)) {
                    AddScore(attacker, &self->r.currentOrigin, 1);
        ...
        if (level.gametype == GT_DUEL) { // in duel, if you kill yourself, the person you are dueling against gets a kill for it
            ...
            if (otherClNum >= 0 && otherClNum < MAX_CLIENTS && g_entities[otherClNum].inuse && g_entities[otherClNum].client && otherClNum != self->s.number) {
                AddScore(&g_entities[otherClNum], &self->r.currentOrigin, 1);
            } else {
...

    {
        ...
        if (anim >= 1) {
            ...
            self->client->respawnTime =
                level.time + 1000; //((self->client->animations[anim].numFrames*40)/(50.0f / self->client->animations[anim].frameLerp))+300;
        ...
        } else if (self->NPC && self->client && self->client->NPC_class != CLASS_MARK1 && self->client->NPC_class != CLASS_VEHICLE) {
    ...
    */
    ...
    if (level.gametype == GT_POWERDUEL && !g_noPDuelCheck) { // powerduel checks
        if (self->client->sess.duelTeam == DUELTEAM_LONE) {  // automatically means a win as there is only one
            ...
            while (i < MAX_CLIENTS) {
                check = &g_entities[i];
                if (check->inuse && check->client && check->s.number != self->s.number && check->client->pers.connected == CON_CONNECTED &&
                    !check->client->iAmALoser && check->client->ps.stats[STAT_HEALTH] > 0 && check->client->sess.sessionTeam != TEAM_SPECTATOR &&
        ...
        if (japp_duelRacemode.integer) {
            ...
            for (uint32_t i = 0u; i < level.maxclients; i++, ent++) {
                if (ent->s.number != self->s.number && ent->client->pers.connected == CON_CONNECTED && ent->client->sess.sessionTeam == TEAM_FREE) {
...
int CheckArmor(gentity_t *ent, int damage, uint32_t dflags) {
    ...
    if (client->NPC_class == CLASS_VEHICLE && ent->m_pVehicle &&
...
int RaySphereIntersections(vector3 *origin, float radius, vector3 *point, vector3 *dir, vector3 intersections[2]) {
    ...
    VectorNormalize(dir);
    b = 2 * (dir->x * (point->y - origin->x) + dir->y * (point->y - origin->y) + dir->z * (point->z - origin->z));

    c = (point->x - origin->x) * (point->x - origin->x) + (point->y - origin->y) * (point->y - origin->y) + (point->z - origin->z) * (point->z - origin->z) -
        radius * radius;
...
void G_Dismember(gentity_t *ent, gentity_t *enemy, vector3 *point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath) {
    ...
    if (ent->ghoul2 && limbName[0] &&
        trap->G2API_GetSurfaceRenderStatus(ent->ghoul2, 0, limbName)) { // is it already off? If so there's no reason to be doing it again, so get out of here.
        return;
    }
    ...
    */
    ...
    if (limbType == G2_MODELPART_HEAD || limbType == G2_MODELPART_WAIST) {
        limb->epVelocity.z += 10;
    }

    if (enemy && enemy->client && ent && ent != enemy && ent->s.number != enemy->s.number && enemy->client->ps.weapon == WP_SABER &&
        enemy->client->olderIsValid &&
        (level.time - enemy->client->lastSaberStorageTime) <
            200) { // The enemy has valid saber positions between this and last frame. Use them to factor in direction of the limb.
        vector3 dif;
        float totalDistance;
        ...
        if (ent->client && (ent->client->ps.torsoTimer > 0 ||
                            !BG_InDeathAnim(ent->client->ps.torsoAnim))) { // if he's done with his death anim we don't actually want the limbs going far
            vector3 preVel;
    ...
    if (ent->s.eType == ET_NPC && ent->ghoul2 && limbName[0] && stubCapName[0]) { // if it's an npc remove these surfs on the server too. For players we don't
                                                                                  // even care cause there's no further dismemberment after death.
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName, int *hitLoc, vector3 *point, vector3 *dir, vector3 *bladeDir, int mod) {
    ...
    *hitLoc = HL_NONE;
    ...
    if (ent->client && (ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE ||
                        ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_INTERROGATOR ||
    ...
    if (!Q_strncmp("hips", surfName, 4)) { // FIXME: test properly for legs
        *hitLoc = HL_WAIST;
        if (ent->client != NULL && ent->ghoul2) {
            ...
            if (kneeLBolt >= 0) {
                trap->G2API_GetBoltMatrix(ent->ghoul2, 0, kneeLBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
            ...
            if (*hitLoc == HL_WAIST) {
                if (kneeRBolt >= 0) {
                    trap->G2API_GetBoltMatrix(ent->ghoul2, 0, kneeRBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                    BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
                    if (DistanceSquared(point, &tagOrg) < 100) { // actually hit the knee
                        *hitLoc = HL_LEG_RT;
                    }
        ...
        *hitLoc = HL_ARM_RT;
        if (ent->client != NULL && ent->ghoul2) {
            ...
            VectorSet(&angles, 0, ent->r.currentAngles.yaw, 0);
            if (handRBolt >= 0) {
                trap->G2API_GetBoltMatrix(ent->ghoul2, 0, handRBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
        ...
        *hitLoc = HL_ARM_LT;
        if (ent->client != NULL && ent->ghoul2) {
            ...
            VectorSet(&angles, 0, ent->r.currentAngles.yaw, 0);
            if (handLBolt >= 0) {
                trap->G2API_GetBoltMatrix(ent->ghoul2, 0, handLBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
                if (DistanceSquared(point, &tagOrg) < 256) { // actually hit the hand
                    *hitLoc = HL_HAND_LT;
                }
        ...
        *hitLoc = HL_LEG_RT;
        if (ent->client != NULL && ent->ghoul2) {
            ...
            VectorSet(&angles, 0, ent->r.currentAngles.yaw, 0);
            if (footRBolt >= 0) {
                trap->G2API_GetBoltMatrix(ent->ghoul2, 0, footRBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
        ...
        *hitLoc = HL_LEG_LT;
        if (ent->client != NULL && ent->ghoul2) {
            ...
            VectorSet(&angles, 0, ent->r.currentAngles.yaw, 0);
            if (footLBolt >= 0) {
                trap->G2API_GetBoltMatrix(ent->ghoul2, 0, footLBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
                if (DistanceSquared(point, &tagOrg) < 100) { // actually hit the foot
                    *hitLoc = HL_FOOT_LT;
                }
    ...
    */
    ...
    if (g_dismember.integer == 100) { // full probability...
        if (ent->client && ent->client->NPC_class == CLASS_PROTOCOL) {
            dismember = qtrue;
        } else if (dir && (dir->x || dir->y || dir->z) && bladeDir &&
                   (bladeDir->x || bladeDir->y || bladeDir->z)) { // we care about direction (presumably for dismemberment)
            ...
            {      // either we don't care about probabilties or the probability let us continue
                ...
                if (tagName) {
                    ...
                    if (tagBolt != -1) {
                        ...
                        VectorSet(&angles, 0, ent->r.currentAngles.yaw, 0);
                        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, tagBolt, &boltMatrix, &angles, &ent->r.currentOrigin, actualTime, NULL, &ent->modelScale);
                        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &tagOrg);
...
*/
    ...
    if (ent->client) {
        ...
        if (g_austrian.integer && (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL)) {
...
void G_LocationBasedDamageModifier(gentity_t *ent, vector3 *point, int mod, uint32_t dflags, int *damage) {
    ...
    if ((d_saberGhoul2Collision.integer && ent->client && ent->client->g2LastSurfaceTime == level.time &&
        ...
        (d_projectileGhoul2Collision.integer && ent->client &&
         ent->client->g2LastSurfaceTime ==
             level.time)) // It's safe to assume we died from the projectile that just set our surface index. So, go ahead and use that as the surf I guess.
    {
        char hitSurface[MAX_QPATH];
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vector3 *dir, vector3 *point, int damage, uint32_t dflags, int mod) {
    ...
    const qboolean nonClientDamage = mod == MOD_UNKNOWN || mod == MOD_TURBLAST || mod == MOD_SENTRY || mod == MOD_WATER || mod == MOD_SLIME ||
                                     mod == MOD_LAVA || mod == MOD_CRUSH || mod == MOD_FALLING || mod == MOD_SUICIDE || mod == MOD_TARGET_LASER ||
    ...
    if (mod == MOD_DEMP2 && targ && targ->inuse && targ->client) {
        if (targ->client->ps.electrifyTime < level.time) { // electrocution effect
            if (targ->s.eType == ET_NPC && targ->s.NPC_class == CLASS_VEHICLE && targ->m_pVehicle &&
                (targ->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
                 targ->m_pVehicle->m_pVehicleInfo->type == VH_WALKER)) { // do some extra stuff to speeders/walkers
    ...
    if (level.gametype == GT_SIEGE && !gSiegeRoundBegun) { // nothing can be damaged til the round starts.
        return;
    }
    ...
    if (targ->client) { // don't take damage when in a walker, or fighter
        ...
        if (targ->client->ps.clientNum < MAX_CLIENTS && targ->client->ps.m_iVehicleNum) {
            ...
            if (veh->m_pVehicle && veh->health > 0) {
                if (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER || veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) {
    ...
    if ((targ->flags & FL_DMG_BY_HEAVY_WEAP_ONLY)) { // only take damage from explosives and such
        if (mod != MOD_REPEATER_ALT && mod != MOD_ROCKET && mod != MOD_FLECHETTE_ALT_SPLASH && mod != MOD_ROCKET_HOMING && mod != MOD_THERMAL &&
            mod != MOD_THERMAL_SPLASH && mod != MOD_TRIP_MINE_SPLASH && mod != MOD_TIMED_MINE_SPLASH && mod != MOD_DET_PACK_SPLASH && mod != MOD_VEHICLE &&
            mod != MOD_CONC && mod != MOD_CONC_ALT && mod != MOD_SABER && mod != MOD_TURBLAST && mod != MOD_SUICIDE && mod != MOD_FALLING && mod != MOD_CRUSH &&
            mod != MOD_TELEFRAG && mod != MOD_TRIGGER_HURT) {
    ...
    if (targ->flags & FL_BBRUSH) {
        if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT || mod == MOD_BRYAR_PISTOL || mod == MOD_BRYAR_PISTOL_ALT ||
    ...
    if (attacker->client && attacker != targ && attacker->s.eType == ET_PLAYER && level.gametype != GT_SIEGE) {
        max = attacker->client->ps.stats[STAT_MAX_HEALTH];
        damage = damage * max / 100;
    }

    if (!(dflags & DAMAGE_NO_HIT_LOC)) { // see if we should modify it by damage location
        if (targ->inuse && (targ->client || targ->s.eType == ET_NPC) && attacker->inuse &&
            (attacker->client || attacker->s.eType == ET_NPC)) { // check for location based damage stuff.
    ...
    if (knockback && targ->client) {
        ...
        if (mod == MOD_SABER) {
            ...
            if ((dflags & DAMAGE_SABER_KNOCKBACK1) || (dflags & DAMAGE_SABER_KNOCKBACK2)) { // saber does knockback, scale it by the right number
        ...
        if (!targ->client->ps.pm_time &&
            (g_saberDmgVelocityScale.integer || mod != MOD_SABER || (dflags & DAMAGE_SABER_KNOCKBACK1) || (dflags & DAMAGE_SABER_KNOCKBACK2) ||
             (dflags & DAMAGE_SABER_KNOCKBACK1_B2) || (dflags & DAMAGE_SABER_KNOCKBACK2_B2))) {
            int t;
    ...
    if ((g_jediVmerc.integer || level.gametype == GT_SIEGE) && client) { // less explosive damage for jedi, more saber damage for non-jedi
        if (client->ps.trueJedi ||
            ...
            default:
                break;
                   ...
                   mod == MOD_SABER) { // if the target is a trueNonJedi, take more saber damage... combined with the 1.5f in the w_saber stuff, this is 6 times
                                       // damage!
            if (damage < 100) {
                damage *= 4;
                if (damage > 100) {
    ...
    if (attacker->client && targ->client && level.gametype == GT_SIEGE && targ->client->siegeClass != -1 &&
        (bgSiegeClasses[targ->client->siegeClass].classflags &
         (1 << CFL_STRONGAGAINSTPHYSICAL))) { // this class is flagged to take less damage from physical attacks.
    ...
    if (!(dflags & DAMAGE_NO_PROTECTION)) {
        ...
        if (targ != attacker) {
            ...
            } else if (attacker && attacker->inuse && !attacker->client && attacker->activator && targ != attacker->activator && attacker->activator->inuse &&
                       attacker->activator->client) { // emplaced guns don't hurt teammates of user
                if (OnSameTeam(targ, attacker->activator)) {
                    if (!g_friendlyFire.integer) {
                        return;
                    }
                }
            } else if (targ->inuse && targ->client && level.gametype >= GT_TEAM && attacker->s.number >= MAX_CLIENTS && attacker->alliedTeam &&
        ...
        if (level.gametype == GT_JEDIMASTER && !g_friendlyFire.integer && targ && targ->client && attacker && attacker->client && targ != attacker &&
            !targ->client->ps.isJediMaster && !attacker->client->ps.isJediMaster && G_ThereIsAMaster()) {
            return;
        }

        if (targ->s.number >= MAX_CLIENTS && targ->client && targ->s.shouldtarget && targ->s.teamowner && attacker && attacker->inuse && attacker->client &&
            targ->s.owner >= 0 && targ->s.owner < MAX_CLIENTS) {
        ...
        if (targ && targ->client && (targ->client->ps.eFlags & EF_INVULNERABLE) && attacker && attacker->client && targ != attacker) {
    ...
    if (attacker && !targ->client) {                                  // attacker hit a non-client
        if (level.gametype == GT_SIEGE && !g_ff_objectives.integer) { // in siege mode (and...?)
            if (targ->teamnodmg) {                                    // targ shouldn't take damage from a certain team
                if (attacker->client) {                               // a client hit a non-client object
                    ...
                    if (targ->teamnodmg == attacker->teamnodmg) {
                        if (attacker->activator && attacker->activator->inuse && attacker->activator->s.number < MAX_CLIENTS && attacker->activator->client &&
    ...
    if (attacker->client && targ != attacker && targ->health > 0 && targ->s.eType != ET_MISSILE && targ->s.eType != ET_GENERAL && client) {
        if (OnSameTeam(targ, attacker)) {
    ...
    if (japp_instagib.integer && mod != MOD_UNKNOWN && mod != MOD_FORCE_DARK && mod != MOD_WATER && mod != MOD_SLIME && mod != MOD_LAVA && mod != MOD_CRUSH &&
        mod != MOD_TELEFRAG && mod != MOD_FALLING && mod != MOD_SUICIDE && mod != MOD_TRIGGER_HURT && mod != MOD_TEAM_CHANGE) {
        // instagib
        shieldAbsorbed = 0;
        take = targ->health;
    }
    ...
    if (japp_damageNotifications.integer) {
        trap->SendServerCommand(attacker - g_entities,
                                va("chat \"" S_COLOR_WHITE "* Damage given: " S_COLOR_RED "%i" S_COLOR_WHITE "/" S_COLOR_GREEN "%i " S_COLOR_WHITE " (%i)\"",
    ...
    if (targ->client) {                                                     // update vehicle shields and armor, check for explode
        if (targ->client->NPC_class == CLASS_VEHICLE && targ->m_pVehicle) { // FIXME: should be in its own function in g_vehicles.c now, too big to be here
            int surface = -1;
            ...
            if (targ->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL) {
                ...
                if (attacker
                    //&& attacker->client
                    && targ != attacker && point && !VectorCompare(&targ->client->ps.origin, point) &&
    ...
    if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) { // FIXME: screw with non-animal vehicles, too?
        if (client) {
            if (client->NPC_class == CLASS_VEHICLE && targ->m_pVehicle && targ->m_pVehicle->m_pVehicleInfo &&
                ...
                if (client->NPC_class == CLASS_PROTOCOL || client->NPC_class == CLASS_SEEKER || client->NPC_class == CLASS_R2D2 ||
                    client->NPC_class == CLASS_R5D2 || client->NPC_class == CLASS_MOUSE || client->NPC_class == CLASS_GONK) {
                    // DEMP2 does more damage to these guys.
                    take *= 2;
                } else if (client->NPC_class == CLASS_PROBE || client->NPC_class == CLASS_INTERROGATOR || client->NPC_class == CLASS_MARK1 ||
                           client->NPC_class == CLASS_MARK2 || client->NPC_class == CLASS_SENTRY || client->NPC_class == CLASS_ATST) {
    ...
    if (take) {
        ...
        if (targ->client && targ->ghoul2 && targ->client->g2LastSurfaceTime == level.time) {
            ...
            if (gPainHitLoc < HL_MAX && gPainHitLoc >= 0 && targ->locationDamage[gPainHitLoc] < Q3_INFINITE &&
                ...
                if (g_armBreakage.integer && !targ->client->ps.brokenLimbs && targ->client->ps.stats[STAT_HEALTH] > 0 && targ->health > 0 &&
                    !(targ->s.eFlags & EF_DEAD)) { // check for breakage
        ...
        if (targ->health <= 0) {
            ...
            JPLua::Entity_CallFunction(targ, JPLua::JPLUA_ENTITY_DIE, (intptr_t)inflictor, (intptr_t)attacker, (intptr_t)take, (intptr_t)mod);
...
qboolean G_RadiusDamage(vector3 *origin, gentity_t *attacker, float damage, float radius, gentity_t *ignore, gentity_t *missile, int mod) {
    ...
    */
    ...
    for (e = 0; e < numListedEntities; e++) {
        ...
        for (i = 0; i < 3; i++) {
            if (origin->raw[i] < ent->r.absmin.raw[i])
                v.raw[i] = ent->r.absmin.raw[i] - origin->raw[i];
            else if (origin->raw[i] > ent->r.absmax.raw[i])
                v.raw[i] = origin->raw[i] - ent->r.absmax.raw[i];
            else
                v.raw[i] = 0;
        }
        ...
        if (CanDamage(ent, attacker, origin)) {
            ...
            dir.z += 24;
            if (attacker && attacker->inuse && attacker->client && attacker->s.eType == ET_NPC && attacker->s.NPC_class == CLASS_VEHICLE &&
            ...
            if (ent && ent->client && roastPeople && missile &&
                !VectorCompare(&ent->r.currentOrigin,
                               &missile->r.currentOrigin)) { // the thing calling this function can create burn marks on people, so create an event to do so
