...
void JP_Bans_Init(void) { JP_Bans_LoadBans(); }
...
const char *JP_Bans_IsBanned(byte *ip) {
    ...
    for (entry = banlist; entry; prev = entry, entry = entry ? entry->next : NULL) { // Find the ban entry
        ...
        if (!(entry->mask & 1) && entry->ip.b[0] != ip[0])
            continue; //	*.0.0.0
        if (!(entry->mask & 2) && entry->ip.b[1] != ip[1])
            continue; //	0.*.0.0
        if (!(entry->mask & 4) && entry->ip.b[2] != ip[2])
            continue; //	0.0.*.0
        if (!(entry->mask & 8) && entry->ip.b[3] != ip[3])
            continue; //	0.0.0.*
        ...
        if (entry->expireTime) {                   // Temporary ban
            if (time(NULL) >= entry->expireTime) { // The ban expired, unlink this ban and then continue the search
                ...
                if (!prev)
                    banlist = entry->next; //	root item
                else
                    prev->next = entry->next; //	other-wise, fix the link
...
void JP_Bans_List(void) {
    ...
    for (entry = banlist; entry; entry = entry->next) {
        ...
        if (entry->mask & 1)
            ...
            Q_strcat(tmp, sizeof(tmp), va("%i", entry->ip.b[0]));
        for (i = 1; i < 4; i++) {
            if (entry->mask & (1 << i))
                Q_strcat(tmp, sizeof(tmp), ".*");
            else
                Q_strcat(tmp, sizeof(tmp), va(".%i", entry->ip.b[i]));
        }
