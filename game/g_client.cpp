...
"nohumans" will prevent non-bots from using this spot.
*/
void SP_info_player_start_red(gentity_t *ent) { SP_info_player_deathmatch(ent); }
...
"nohumans" will prevent non-bots from using this spot.
*/
void SP_info_player_start_blue(gentity_t *ent) { SP_info_player_deathmatch(ent); }
...
void ThrowSaberToAttacker(gentity_t *self, gentity_t *attacker) {
    ...
    if (attacker && attacker->client &&
        self->client->ps.saberInFlight) { // someone killed us and we had the saber thrown, so actually move this saber to the saber location
...
gentity_t *SelectRandomFurthestSpawnPoint(vector3 *avoidPoint, vector3 *origin, vector3 *angles, team_e team) {
    gentity_t *spot = NULL;
    vector3 delta;
    float dist;
    float list_dist[64];
...
gentity_t *SelectInitialSpawnPoint(vector3 *origin, vector3 *angles, team_e team) {
    gentity_t *spot;
...
void MaintainBodyQueue(gentity_t *ent) { // do whatever should be done taking ragdoll and dismemberment states into account.
    ...
    if (ent->client->tempSpectate >= level.time || (ent->client->ps.eFlags2 & EF2_SHIP_DEATH)) {
...
int TeamCount(int ignoreClientNum, team_e team) {
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        if (level.clients[i].sess.sessionTeam == team) {
            count++;
        } else if (level.gametype == GT_SIEGE && level.clients[i].sess.siegeDesiredTeam == team) {
            count++;
        }
    }
...
static qboolean IsSuffixed(const char *cleanName, int32_t *suffixNum) {
    ...
    if (cleanName - bracketL < (ptrdiff_t)(len - 3)) {
        ...
        trap->Print("DuplicateNames: bracket came too early, not removing suffix. [cleanName - bracketL](%d) < "
                    "[len - 3](%d) for '%s'\n",
                    (int)(cleanName - bracketL), (int)len - 3, cleanName);
#endif
        return qfalse;
    }
...
qboolean CheckDuplicateName(int clientNum) {
    ...
    for (i = 0, ent = g_entities; i < level.maxclients; i++, ent++) {
        ...
        if (!Q_stricmp(ent->client->pers.netnameClean, cleanName)) {
            ...
            trap->Print("Adding suffix on name for \"%s\" because \"%s\" == \"%s\"\n", G_PrintClient(clientNum), ent->client->pers.netnameClean, cleanName);
...
void ClientCleanName(const char *in, char *out, int outSize) {
    ...
    for (; *in && outpos < outSize - 1; in++) {
        ...
        } else if ((byte)*in < 0x20 || (byte)*in == 0x81 || (byte)*in == 0x8D || (byte)*in == 0x8F || (byte)*in == 0x90 || (byte)*in == 0x9D ||
...
void SetupGameGhoul2Model(gentity_t *ent, char *modelname, char *skinName) {
    ...
    if (precachedKyle && trap->G2API_HaveWeGhoul2Models(precachedKyle)) {
        ...
        if (d_perPlayerGhoul2.integer || ent->s.number >= MAX_CLIENTS) {
            ...
            if (ent->client->NPC_class == CLASS_VEHICLE) {
                ...
                skin[0] = '\0';
                if (ent->m_pVehicle && ent->m_pVehicle->m_pVehicleInfo && ent->m_pVehicle->m_pVehicleInfo->skin && ent->m_pVehicle->m_pVehicleInfo->skin[0]) {
                    skinHandle = trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", vehModel, ent->m_pVehicle->m_pVehicleInfo->skin));
                } else
            ...
            if (handle < 0) { // Huh. Guess we don't have this model. Use the default.
                ...
                if (ent->s.number >= MAX_CLIENTS) {
                    ...
                    if (ent->client->NPC_class == CLASS_VEHICLE) { // vehicles are tricky and send over their vehicle names as the model (the model is then
                                                                   // retrieved based on the vehicle name)
    ...
    if (ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle) { // do special vehicle stuff
        char strTemp[128];
        int i;
...
#define UIF(x, _min, _max)                                                                                                                                     \
    { STRING(\\) #x STRING(\\), STRING(x), _min, _max }
...
void SV_ToggleUserinfoValidation_f(void) {
    if (trap->Argc() == 1) {
        ...
        if (index < numUserinfoFields) {
            trap->Print("%s %s\n", userinfoFields[index].fieldClean, ((japp_userinfoValidate.bits & (1 << index)) ? "Validated" : "Ignored"));
        } else {
            trap->Print("%s %s\n", userinfoValidateExtra[index - numUserinfoFields], ((japp_userinfoValidate.bits & (1 << index)) ? "Validated" : "Ignored"));
        }
    }
}
...
qboolean ClientUserinfoChanged(int clientNum) {
    ...
    if (s[0] && sscanf(s, "%u", &client->pers.CPD) != 1) {
        G_LogPrintf(level.log.security,
                    "ClientUserinfoChanged(): Client %i (%s) userinfo 'cp_pluginDisable' was found"
                    ", but invalid. [IP: %s]\n",
    ...
    if (level.gametype == GT_SIEGE) {
        ...
        client->siegeClass = BG_SiegeFindClassIndexByName(siegeClass);
        if (client->siegeClass == -1) {
            // ok, get the first valid class for the team you're on then, I guess.
            BG_SiegeCheckClassLegality(team, siegeClass);
            Q_strncpyz(client->sess.siegeClass, siegeClass, sizeof(client->sess.siegeClass));
            client->siegeClass = BG_SiegeFindClassIndexByName(siegeClass);
        } else {
    ...
    char cp_sbRGB1[MAX_CVAR_VALUE_STRING];
    char cp_sbRGB2[MAX_CVAR_VALUE_STRING];
    ...
    if (level.gametype == GT_SIEGE) {
        Q_strcat(buf, sizeof(buf), va("siegeclass\\%s\\", siegeClass));
    ...
    if (japp_detectCountry.integer && !isBot && firstTime && Q_stricmp(value, "localhost")) {
        ...
        if (!gip) {
            ...
            if (gip->getStatus()) {
                ...
                trap->Print("Got GeoIP data. IP: %s Data: %s", gip->getIp().c_str(), sData);
                Q_strncpyz(ent->client->sess.geoipData, sData, sizeof(ent->client->sess.geoipData));
            } else {
                trap->Print("Failed to Get GeoIP data. IP: %s Error: %s", gip->getIp().c_str(), gip->getData()->c_str());
...
void ClientBegin(int clientNum, qboolean allowTeamReset) {
    ...
    if ((ent->r.svFlags & SVF_BOT) && level.gametype >= GT_TEAM) {
        if (allowTeamReset) {
            ...
            ent->client->sess.sessionTeam = (team_e)preSess;
...
void G_BreakArm(gentity_t *ent, int arm) {
    ...
    if (arm == BROKENLIMB_LARM) {
        if (ent->client->saber[1].model[0] && ent->client->ps.weapon == WP_SABER && !ent->client->ps.saberHolstered &&
...
void G_UpdateClientAnims(gentity_t *self, float animSpeedScale) {
    ...
    animNumber_e torsoAnim = (animNumber_e)self->client->ps.torsoAnim;
    animNumber_e legsAnim = (animNumber_e)self->client->ps.legsAnim;

    if (self->client->ps.saberLockFrame) {
        trap->G2API_SetBoneAnim(self->ghoul2, 0, "model_root", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame + 1,
                                BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
        trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame + 1,
                                BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
        trap->G2API_SetBoneAnim(self->ghoul2, 0, "Motion", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame + 1,
                                BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
        return;
    }

    if (self->localAnimIndex > 1 && bgAllAnims[self->localAnimIndex].anims[legsAnim].firstFrame == 0 &&
    ...
    if (self->client->legsAnimExecute != legsAnim || self->client->legsLastFlip != self->client->ps.legsFlip) {
        ...
        aFlags |= BONE_ANIM_BLEND; // since client defaults to blend. Not sure if this will make much difference if any on server position, but it's here just
                                   // for the sake of matching them.
...
tryTorso:
    if (self->localAnimIndex > 1 && bgAllAnims[self->localAnimIndex].anims[torsoAnim].firstFrame == 0 &&
        bgAllAnims[self->localAnimIndex].anims[torsoAnim].numFrames == 0)

    { // If this fails as well just return.
        return;
    } else if (self->s.number >= MAX_CLIENTS && self->s.NPC_class == CLASS_VEHICLE) { // we only want to set the root bone for vehicles
        return;
    }

    if ((self->client->torsoAnimExecute != torsoAnim || self->client->torsoLastFlip != self->client->ps.torsoFlip) && !self->noLumbar) {
        aFlags = 0;
        animSpeed = 0;
        ...
        aFlags |= BONE_ANIM_BLEND; // since client defaults to blend. Not sure if this will make much difference if any on client position, but it's here just
                                   // for the sake of matching them.
        ...
        trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time,
                                /*firstFrame why was it this before?*/ -1, 150);
    ...
    if (setTorso && self->localAnimIndex <= 1) { // only set the motion bone for humanoids.
...
void ClientSpawn(gentity_t *ent) {
    int i, index = 0, saveSaberNum = ENTITYNUM_NONE, savedSiegeIndex = 0, maxHealth = 100, gameFlags, savedPing, accuracy_hits, accuracy_shots, eventSequence,
    ...
    if (changedSaber) { // make sure our new info is sent out to all the other clients, and give us a valid stance
        ...
        if (ent->client->saber[0].model[0] && ent->client->saber[1].model[0]) { // dual
            ...
            if (level.gametype != GT_SIEGE && ent->client->ps.fd.saberAnimLevel > ent->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE])
                ent->client->ps.fd.saberAnimLevelBase = ent->client->ps.fd.saberAnimLevel = ent->client->ps.fd.saberDrawAnimLevel =
                    ent->client->sess.saberLevel = ent->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE];
        }
    ...
    if (ent->client->ps.fd.saberAnimLevel != SS_STAFF && ent->client->ps.fd.saberAnimLevel != SS_DUAL &&
        ent->client->ps.fd.saberAnimLevel == ent->client->ps.fd.saberDrawAnimLevel && ent->client->ps.fd.saberAnimLevel == ent->client->sess.saberLevel) {
        ent->client->sess.saberLevel = Q_clampi(SS_FAST, ent->client->sess.saberLevel, SS_STRONG);
    ...
    if (client->sess.sessionTeam == TEAM_SPECTATOR) {
        ...
        spawnPoint = SelectSiegeSpawnPoint(client->siegeClass, client->sess.sessionTeam, client->pers.teamState.state, &spawn_origin, &spawn_angles);
    } else {
        do {
            if (level.gametype == GT_POWERDUEL) {
    ...
    if (level.gametype != GT_HOLOCRON && level.gametype != GT_JEDIMASTER && !BG_HasSetSaberOnly() && !AllForceDisabled(g_forcePowerDisable.integer) &&
        g_jediVmerc.integer) {
        if (level.gametype >= GT_TEAM &&
            (client->sess.sessionTeam == TEAM_BLUE || client->sess.sessionTeam == TEAM_RED)) { // In Team games, force one side to be merc and other to be jedi
    ...
    if (level.gametype != GT_SIEGE) {
        if (ent->client->pers.adminData.merc) {
            ...
            for (i = WP_BRYAR_PISTOL; i <= LAST_USEABLE_WEAPON; i++) {
                if (spawnWeaps & (1 << i)) {
                    ammo_e ammo = weaponData[i].ammoIndex;
    ...
    if (level.gametype == GT_SIEGE && client->siegeClass != -1 && bgSiegeClasses[client->siegeClass].powerups &&
    ...
    if (level.gametype == GT_SIEGE && client->siegeClass != -1 && bgSiegeClasses[client->siegeClass].starthealth) { // class specifies a start health, so use it
        ...
        if (level.gametype == GT_POWERDUEL && client->sess.duelTeam == DUELTEAM_LONE) {
            if (duel_fraglimit.integer) {
                ...
                ent->health = client->ps.stats[STAT_HEALTH] = client->ps.stats[STAT_MAX_HEALTH] =
                    g_powerDuelStartHealth.integer -
                    ((g_powerDuelStartHealth.integer - g_powerDuelEndHealth.integer) * (float)client->sess.wins / (float)duel_fraglimit.integer);
            } else {
    ...
    if (level.gametype == GT_SIEGE) { // Imperial (team1) team is allied with "enemy" NPCs in this mode
        if (client->sess.sessionTeam == SIEGETEAM_TEAM1) {
            ...
            client->playerTeam = (npcteam_e)ent->s.teamowner;
