...
static qboolean Grenadier_Move(void) {
    ...
    if (moved == qfalse) { // couldn't get to enemy
        if ((NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) && NPC->client->ps.weapon == WP_THERMAL && NPCInfo->goalEntity &&
            NPCInfo->goalEntity == NPC->enemy) { // we were running after enemy
...
void NPC_BSGrenadier_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
    if (NPCInfo->confusionTime < level.time) {
        ...
        if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
            ...
            if (NPCInfo->investigateDebounceTime > level.time) { // FIXME: walk over to it, maybe?  Not if not chase enemies
...
void NPC_BSGrenadier_Attack(void) {
    ...
    if (enemyDist3 < 16384                                                                                                // 128
        && (!NPC->enemy->client || NPC->enemy->client->ps.weapon != WP_SABER || BG_SabersOff(&NPC->enemy->client->ps))) { // enemy is close and not using saber
        if (NPC->client->ps.weapon == WP_THERMAL) {                                                                       // grenadier
            trace_t trace;
            trap->Trace(&trace, &NPC->r.currentOrigin, &NPC->enemy->r.mins, &NPC->enemy->r.maxs, &NPC->enemy->r.currentOrigin, NPC->s.number,
                        NPC->enemy->clipmask, qfalse, 0, 0);
            if (!trace.allsolid && !trace.startsolid &&
                (flcmp(trace.fraction, 1.0f, 0.001f) || trace.entityNum == NPC->enemy->s.number)) { // I can get right to him
    ...
    { // enemy is far or using saber
        if (NPC->client->ps.weapon == WP_STUN_BATON &&
            (NPC->client->ps.stats[STAT_WEAPONS] & (1 << WP_THERMAL))) { // fisticuffs, make switch to thermal if have it
            // reset fire-timing variables
    ...
    if (enemyCS3) {
        ...
        if (NPC->client->ps.weapon == WP_THERMAL) { // don't chase and throw
            move3 = qfalse;
                   ...
                   enemyDist3 < (NPC->r.maxs.x + NPC->enemy->r.maxs.y + 16) * (NPC->r.maxs.x + NPC->enemy->r.maxs.x + 16)) { // close enough
            move3 = qfalse;
        }
