...
saber_colors_t TranslateSaberColor( const char *name ) 
{
	...
	else if ( !Q_stricmp( name, "orange" ) ) 	return SABER_ORANGE;
	else if ( !Q_stricmp( name, "yellow" ) ) 	return SABER_YELLOW;
	else if ( !Q_stricmp( name, "green" ) ) 	return SABER_GREEN;
	else if ( !Q_stricmp( name, "blue" ) ) 		return SABER_BLUE;
	else if ( !Q_stricmp( name, "purple" ) ) 	return SABER_PURPLE;
	else if ( !Q_stricmp( name, "random" ) ) 	return ((saber_colors_t)(Q_irand( SABER_ORANGE, SABER_PURPLE )));
	else										return SABER_BLUE;
}
