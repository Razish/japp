...
stringID_table_t saberTable[] = {ENUM2STRING(SABER_NONE),
...
stringID_table_t saberMoveTable[] = {ENUM2STRING(LS_NONE),
                                     // Attacks
...
const char *SaberColorToString(saber_colors_t color) {
    ...
    if (color == SABER_RGB)
        return "rgb";
    if (color == SABER_FLAME1)
        return "rgb2";
    if (color == SABER_ELEC1)
        return "rgb3";
    if (color == SABER_FLAME2)
        return "rgb4";
    if (color == SABER_ELEC2)
        return "rgb5";
...
saber_styles_e TranslateSaberStyle(const char *name) {
    if (!Q_stricmp(name, "fast"))
        ...
        return SS_MEDIUM;
    else if (!Q_stricmp(name, "strong"))
        return SS_STRONG;
    else if (!Q_stricmp(name, "desann"))
        return SS_DESANN;
    else if (!Q_stricmp(name, "tavion"))
        return SS_TAVION;
    else if (!Q_stricmp(name, "dual"))
        return SS_DUAL;
    else if (!Q_stricmp(name, "staff"))
        return SS_STAFF;
    else
        return SS_NONE;
}
...
qboolean WP_SaberBladeUseSecondBladeStyle(saberInfo_t *saber, int bladeNum) {
    if (saber && saber->bladeStyle2Start > 0 && bladeNum >= saber->bladeStyle2Start)
        return qtrue;
...
void WP_SaberSetDefaults(saberInfo_t *saber) {
    ...
    saber->bounceSound[0] = 0;     // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                   // these sounds) - NOTE: must provide all 3!!!
    saber->bounceSound[1] = 0;     // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                   // these sounds) - NOTE: must provide all 3!!!
    saber->bounceSound[2] = 0;     // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                   // these sounds) - NOTE: must provide all 3!!!
    ...
    saber->bounce2Sound[0] = 0;  // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                 // these sounds) - NOTE: must provide all 3!!!
    saber->bounce2Sound[1] = 0;  // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                 // these sounds) - NOTE: must provide all 3!!!
    saber->bounce2Sound[2] = 0;  // none - if set, plays one of these 3 sounds when saber/sword hits a wall and bounces off (must set bounceOnWall to 1 to use
                                 // these sounds) - NOTE: must provide all 3!!!
...
static void Saber_ParseOnInWater(saberInfo_t *saber, const char **p) { SkipRestOfLine(p); }
static void Saber_ParseNotInMP(saberInfo_t *saber, const char **p) { SkipRestOfLine(p); }
...
static keywordHash_t saberParseKeywords[] = {{"name", Saber_ParseName, NULL},
...
void WP_SetSaber(int entNum, saberInfo_t *sabers, int saberNum, const char *saberName) {
    ...
    if ((sabers[1].saberFlags & SFL_TWO_HANDED)) { // not allowed to use a 2-handed saber as second saber
        ...
        return;
    } else if ((sabers[0].saberFlags & SFL_TWO_HANDED) && sabers[1].model[0]) { // you can't use a two-handed saber with a second saber, so remove saber 2
        WP_RemoveSaber(sabers, 1);
        return;
    }
}
...
void BG_SI_SetLengthGradual(saberInfo_t *saber, int time) {
    ...
    for (i = 0; i < saber->numBlades; i++) {
        ...
        if (saber->blade[i].length == saber->blade[i].lengthMax || saber->blade[i].length == 0) {
