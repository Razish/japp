...
static qboolean Jedi_ClearPathToSpot( vector3 *dest, int impactEntNum )
{
	...
	if ( dest->z > NPC->r.currentOrigin.z )
	...
	for ( i = NPC->r.maxs.x*2; i < dist; i += NPC->r.maxs.x*2 )
...
qboolean NPC_MoveDirClear( int forwardmove, int rightmove, qboolean reset )
{
	...
	*/
	...
	{
		if ( NPCInfo->goalEntity->r.currentOrigin.z < NPC->r.currentOrigin.z )
...
static qboolean Jedi_Jump( vector3 *dest, int goalEntNum )
