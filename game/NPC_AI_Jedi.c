			...
			if ( NPC->client->ps.weapon == WP_NONE ) {
				...
				if ( (NPC->client->ps.weapon == WP_ROCKET_LAUNCHER || (NPC->client->ps.weapon == WP_FLECHETTE && (NPCInfo->scriptFlags&SCF_ALT_FIRE))) && enemyDist < MIN_ROCKET_DIST_SQUARED )//128*128
				{
					...
					if ( hit == NPC->enemy->s.number
						...
						|| (hitEnt && hitEnt->takedamage && ((hitEnt->r.svFlags&SVF_GLASS_BRUSH) || hitEnt->health < 40 || NPC->s.weapon == WP_EMPLACED_GUN)) ) {//can hit enemy or enemy ally or will hit glass or other minor breakable (or in emplaced gun), so shoot anyway
						enemyCS = qtrue;
...
static qboolean Jedi_BattleTaunt( void ) {
	if ( TIMER_Done( NPC, "chatter" )
		...
		if ( NPC->client->playerTeam == NPCTEAM_PLAYER
			&& NPC->enemy && NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_JEDI ) {//a jedi fighting a jedi - training
		...
		if ( event != -1 ) {
			...
			if ( NPC->enemy && NPC->enemy->NPC && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_JEDI ) {//Have the enemy jedi say something in response when I'm done?
			}
			return qtrue;
		}
	}
...
static qboolean Jedi_ClearPathToSpot( vector3 *dest, int impactEntNum ) {
	...
	for ( i = NPC->r.maxs.x * 2; i < dist; i += NPC->r.maxs.x * 2 ) {//FIXME: does this check the last spot, too?  We're assuming that should be okay since the enemy is there?
...
qboolean NPC_MoveDirClear( int forwardmove, int rightmove, qboolean reset ) {
	...
	*/
	...
	if ( trace.fraction < 0.6f ) {//Going to bump into something very close, don't move, just turn
		if ( (NPC->enemy && trace.entityNum == NPC->enemy->s.number) || (NPCInfo->goalEntity && trace.entityNum == NPCInfo->goalEntity->s.number) ) {//okay to bump into enemy or goal
...
static void Jedi_AdjustSaberAnimLevel( gentity_t *self, int newLevel ) {
	...
	if ( self->client->playerTeam == NPCTEAM_ENEMY ) {
		...
		if ( self->NPC->rank == RANK_CREWMAN
			|| self->NPC->rank == RANK_ENSIGN ) {//acrobat & force-users always use medium attacks
...
static void Jedi_CombatDistance( int enemy_dist ) {//FIXME: for many of these checks, what we really want is horizontal distance to enemy
	if ( NPC->client->ps.fd.forcePowersActive&(1 << FP_GRIP) &&
		NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 ) {//when gripping, don't move
		return;
	}
	...
	if ( NPC->client->ps.fd.forcePowersActive&(1 << FP_DRAIN) &&
		NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1 ) {//when draining, don't move
		return;
	}
	...
	if ( NPC->client->NPC_class == CLASS_BOBAFETT ) {
		...
		else if ( NPC->client->ps.forceHandExtend == HANDEXTEND_JEDITAUNT && (NPC->client->ps.forceHandExtendTime - level.time) < 200 ) {//we're almost done with our special taunt
	...
	*/
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're throwing their saber, grip them!
			...
			if ( NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.fd.forcePowersActive&(1 << FP_GRIP)) ) {//They're choking someone, probably an ally, run at them and do some sort of attack
				if ( NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE ) {//they're on the ground, so advance
					if ( TIMER_Done( NPC, "parryTime" ) || NPCInfo->rank > RANK_LT ) {//not parrying
						if ( enemy_dist > 200 || !(NPCInfo->scriptFlags&SCF_DONT_FIRE) ) {//far away or allowed to use saber
							Jedi_Advance();
			...
			if ( chanceScale
				...
				&& enemy_dist < 500
				&& (Q_irand( 0, chanceScale * 10 )<5 || (NPC->enemy->client && NPC->enemy->client->ps.weapon != WP_SABER && !Q_irand( 0, chanceScale ))) ) {//else, randomly try some kind of attack every now and then
...
static qboolean Jedi_Strafe( int strafeTimeMin, int strafeTimeMax, int nextStrafeTimeMin, int nextStrafeTimeMax, qboolean walking ) {
	if ( NPC->client->ps.saberEventFlags&SEF_LOCK_WON && NPC->enemy && NPC->enemy->painDebounceTime > level.time ) {//don't strafe if pressing the advantage of winning a saberLock
		return qfalse;
	}
	...
	if ( self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT || self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT ) {//already running on a wall
		...
		if ( self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT && rightdot < 0 ) {//I'm running on a wall to my left and the attack is on the left
			if ( animLength - self->client->ps.legsTimer > 400
				&& self->client->ps.legsTimer > 400 ) {//not at the beginning or end of the anim
		...
		if ( trace.fraction >= 1.0f && allowCartWheels ) {//it's clear, let's do it
			...
			if ( (trace.entityNum<ENTITYNUM_WORLD&&traceEnt&&traceEnt->s.solid != SOLID_BMODEL) || DotProduct( &trace.plane.normal, &idealNormal ) > 0.7f ) {//it's a ent of some sort or it's a wall roughly facing us
			...
			if ( evasionType == EVASION_DODGE ) {
				...
				if ( self->client->NPC_class == CLASS_TAVION ) {//Tavion is faster
					...
					if ( evasionType == EVASION_PARRY
						...
						|| evasionType == EVASION_JUMP_PARRY ) {//slower with parries
...
qboolean Jedi_SaberBusy( gentity_t *self ) {
	if ( self->client->ps.torsoTimer > 300
		...
		|| PM_RollingAnim( self->client->ps.torsoAnim )) ) {//my saber is not in a parrying position
		return qtrue;
	}
	return qfalse;
}
...
evasionType_t Jedi_SaberBlockGo( gentity_t *self, usercmd_t *cmd, vector3 *pHitloc, vector3 *phitDir, gentity_t *incoming, float dist ) //dist = 0.0f
{
	...
	if ( zdiff >= -5 )//was 0
	{
		if ( incoming || !saberBusy ) {
			if ( rightdot > 12
				|| (rightdot > 3 && zdiff < 5)
				|| (!incoming&&fabsf( hitdir.z ) < 0.25f) )//was normalized, 0.3f
			{//coming from right
				if ( doDodge ) {
...
static void Jedi_EvasionSaber( vector3 *enemy_movedir, float enemy_dist, vector3 *enemy_dir ) {
	...
	if ( TIMER_Done( NPC, "parryTime" ) ) {
		if ( NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE &&
			NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN ) {//wasn't blocked myself
	...
	if ( NPC->enemy->client->ps.saberInFlight && NPC->enemy->client->ps.saberEntityNum != ENTITYNUM_NONE && NPC->enemy->client->ps.saberEntityState != SES_RETURNING ) {//enemy is shooting lightning
		enemy_attacking = qtrue;
	...
	if ( Q_irand( 0, 100 ) < evasionChance ) {//check to see if he's coming at me
		float facingAmt;
		if ( VectorCompare( enemy_movedir, &vec3_origin ) || shooting_lightning || throwing_saber ) {//he's not moving (or he's using a ranged attack), see if he's facing me
			vector3	enemy_fwd;
		...
		if ( flrand( 0.25f, 1 ) < facingAmt ) {//coming at/facing me!
			int whichDefense = 0;
			if ( NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight || NPC->client->NPC_class == CLASS_BOBAFETT ) {//I'm attacking or recovering from a parry, can only try to strafe/jump right now
			...
			case 3:
				...
				if ( (NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && TIMER_Done( NPC, "parryTime" ) ) {//FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
					ForceThrow( NPC, qfalse );
				}
				break;
			case 4:
				...
				if ( !Q_irand( 0, 5 ) || !Jedi_Strafe( 300, 1000, 0, 1000, qfalse ) ) {//certain chance they will pick an alternative evasion
					...
					if ( shooting_lightning || throwing_saber || enemy_dist < 80 ) {
						...
						if ( shooting_lightning || (!Q_irand( 0, 2 ) && NPCInfo->stats.aggression < 4 && TIMER_Done( NPC, "parryTime" )) ) {
							if ( (NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !shooting_lightning && Q_irand( 0, 2 ) ) {//FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
								ForceThrow( NPC, qfalse );
							}
					...
					if ( !(NPCInfo->scriptFlags&SCF_NO_ACROBATICS)
						...
						&& !Q_irand( 0, 5 ) ) {//FIXME: make this a function call?
						//FIXME: check for clearance, safety of landing spot?
						if ( NPC->client->NPC_class == CLASS_BOBAFETT ) {
...
static void Jedi_FaceEnemy( qboolean doPitch ) {
	...
	if ( NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) &&
		NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 ) {//don't update?
...
static qboolean Jedi_AttackDecide( int enemy_dist ) {
	...
	if ( NPC->client->NPC_class == CLASS_TAVION ||
		...
		(NPC->client->NPC_class == CLASS_JEDI && NPCInfo->rank == RANK_COMMANDER) ) {//tavion, fencers, jedi trainer are all good at following up a parry with an attack
...
static qboolean Jedi_Jump( vector3 *dest, int goalEntNum ) {//FIXME: if land on enemy, knock him down & jump off again
	/*
	if ( dest[2] - NPC->r.currentorigin.z < 64 && DistanceHorizontal( NPC->r.currentOrigin, dest ) > 256 )
	...
	VectorScale( enemy_diff, enemy_xy_diff*0.8f, NPC->client->ps.velocity );
	if ( enemy_z_diff < 64 )
	{
	...
	*/
	if ( 1 ) {
		...
		while ( hitCount < maxHits ) {
			...
			else {//no need to check the path, go with first calc
				break;
			}
		}
	...
	else {//a more complicated jump
		vector3		dir, p1, p2, apex;
		float		time, height, forward, z, xy, dist, apexHeight;

		if ( NPC->r.currentOrigin.z > dest->z )//NPCInfo->goalEntity->r.currentOrigin
		{
			VectorCopy( &NPC->r.currentOrigin, &p1 );
			VectorCopy( dest, &p2 );//NPCInfo->goalEntity->r.currentOrigin
		}
		else if ( NPC->r.currentOrigin.z < dest->z )//NPCInfo->goalEntity->r.currentOrigin
		{
		...
		*/
		...
		time = sqrtf( height / (.5f * NPC->client->ps.gravity) );//was 0.5f, but didn't work well for very long jumps
		if ( !(int)time ) {
		...
		//Com_Printf("Jump Velocity: %4.2f, %4.2f, %4.2f\n", NPC->client->ps.velocity.x, NPC->client->ps.velocity.y, NPC->client->ps.velocity[2] );
	}
	return qtrue;
}
		...
		if ( (!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE) ) {
			if ( !PM_InKnockDown( &NPC->client->ps ) && !BG_InRoll( &NPC->client->ps, NPC->client->ps.legsAnim ) ) {//enemy is on terra firma
				...
				if ( goal_xy_dist < 550 && goal_z_diff > -400/*was -256*/ )//for now, jedi don't take falling damage && (NPC->health > 20 || goal_z_diff > 0 ) && (NPC->health >= 100 || goal_z_diff > -128 ))//closer than @512
				{
					...
					if ( NPC->health < 150 && ((NPC->health < 30 && goal_z_diff < 0) || goal_z_diff < -128) ) {//don't jump, just walk off... doesn't help with ledges, though
						debounce = qtrue;
					}
						...
						if ( goal_z_diff > 0 || goal_xy_dist > 128 ) {//Fake a force-jump
							...
							if ( goal == NPC->enemy ) {
								...
								while ( sideTry < 10 ) {//FIXME: make it so it doesn't try the same spot again?
									...
									if ( Q_irand( 0, 1 ) )	dest.x += NPC->enemy->r.maxs.x*1.25f;
									else					dest.x += NPC->enemy->r.mins.x*1.25f;
									if ( Q_irand( 0, 1 ) )	dest.y += NPC->enemy->r.maxs.y*1.25f;
									else					dest.y += NPC->enemy->r.mins.y*1.25f;
...
static void Jedi_Combat( void ) {
	...
	if ( !NPC->client->ps.saberInFlight && (!(NPC->client->ps.fd.forcePowersActive&(1 << FP_GRIP)) || NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2) ) {//not throwing saber or using force grip
			...
			if ( enemy && enemy->client && NPC_ValidEnemy( enemy ) && enemy->client->playerTeam == NPC->client->enemyTeam ) {
				if ( trap->InPVS( &NPC->r.currentOrigin, &enemy->r.currentOrigin ) ) {//we could potentially see him
					...
					if ( enemy->s.eType == ET_PLAYER || enemy_dist < best_enemy_dist ) {
						...
						else if ( enemy->client->ps.saberInFlight && !enemy->client->ps.saberHolstered ) {//threw his saber, see if it's heading toward me and close enough to consider a threat
							float	saberDist;
							vector3	saberDir2Me;
							vector3	saberMoveDir;
...
void NPC_BSJedi_FollowLeader( void ) {
	...
	if ( NPCInfo->goalEntity ) {
		...
		if ( !NAV_CheckAhead( NPC, &NPCInfo->goalEntity->r.currentOrigin, &trace, (NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP ) ) {//can't get straight to him
...
static void Jedi_Attack( void ) {
	...
	if ( NPC->enemy ) {
		...
		{//my enemy is dead and I killed him
			...
			if ( NPC->client->NPC_class == CLASS_BOBAFETT ) {
				...
				NPC->client->ps.saberBlocked = BLOCKED_NONE;
				if ( !NPC->client->ps.saberHolstered && NPC->client->ps.saberInFlight ) {//saber is still on (or we're trying to pull it back), count down erosion and keep facing the enemy
	...
	if ( !(NPCInfo->scriptFlags&SCF_CHASE_ENEMIES)
		|| ((NPC->client->ps.fd.forcePowersActive&(1 << FP_HEAL)) && NPC->client->ps.fd.forcePowerLevel[FP_HEAL]<FORCE_LEVEL_2) ) {//this is really stupid, but okay...
		ucmd.forwardmove = 0;
		ucmd.rightmove = 0;
		if ( ucmd.upmove > 0 ) {
			ucmd.upmove = 0;
	...
	if ( NPC->client->NPC_class != CLASS_BOBAFETT ) {
		if ( PM_SaberInBrokenParry( NPC->client->ps.saberMove ) || NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN ) {//just make sure they don't pull their saber to them if they're being blocked
			ucmd.buttons &= ~BUTTON_ATTACK;
		}
	}
...
void NPC_BSJedi_Default( void ) {
	...
	{//have an enemy
		...
		if ( ((!ucmd.buttons&&!NPC->client->ps.fd.forcePowersActive) || (NPC->enemy&&NPC->enemy->health <= 0)) && NPCInfo->enemyCheckDebounceTime < level.time ) {//not doing anything (or walking toward a vanquished enemy - fixme: always taunt the player?), not using force powers and it's time to look again
