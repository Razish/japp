...
void turret_head_think(gentity_t *self) {
    ...
    if (self->painDebounceTime > level.time) {
        ...
        if (Q_irand(0, 3)) { // 25% chance of still firing
            return;
        }
    }
...
static void turret_aim(gentity_t *self) {
    ...
    if (self->painDebounceTime > level.time) {
        ...
        org.z += self->enemy->r.maxs.z * 0.5f;
        if (self->enemy->s.eType == ET_NPC && self->enemy->s.NPC_class == CLASS_VEHICLE && self->enemy->m_pVehicle &&
            self->enemy->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) { // hack!
            org.z += 32.0f;
        }
        ...
        diffYaw = AngleSubtract(desiredAngles.yaw, top->r.currentAngles.yaw);
        diffPitch = AngleSubtract(desiredAngles.pitch, top->r.currentAngles.pitch);
    } else { // FIXME: Pan back and forth in original facing
        ...
        if (!tr.allsolid && !tr.startsolid && (tr.fraction == 1.0f || tr.entityNum == target->s.number)) {
            ...
            if (enemyDist < bestDist // all things equal, keep current
                || (!Q_stricmp("atst_vehicle", target->NPC_type) && bestTarget &&
                    Q_stricmp("atst_vehicle", bestTarget->NPC_type))) // target AT-STs over non-AT-STs... FIXME: must be a better, easier way to tell this, no?
            {
    ...
    if (!self->enemy) {
        if (turret_find_enemies(self)) {
            turnOff = qfalse;
                                       ...
                                       self->enemy->client->tempSpectate >= level.time)) { // don't keep going after spectators
        self->enemy = NULL;
