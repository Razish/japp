...
typedef struct botlib_import_s {
    ...
    void (*Print)(int type, char *fmt, ...) Q_PRINT_FORMAT(2, 3);
...
typedef struct aas_export_s {
    ...
    int (*AAS_PredictRoute)(struct aas_predictroute_s *route, int areanum, vector3 *origin, int goalareanum, uint32_t travelflags, int maxareas, int maxtime,
    ...
    int (*AAS_PredictClientMovement)(struct aas_clientmove_s *move, int entnum, vector3 *origin, int presencetype, int onground, vector3 *velocity,
                                     vector3 *cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize);
} aas_export_t;
...
typedef struct ai_export_s {
    ...
    int (*BotChooseNBGItem)(int goalstate, vector3 *origin, int *inventory, uint32_t travelflags, struct bot_goal_s *ltg, float maxtime);
