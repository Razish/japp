...
void NPC_Mark1_Part_Explode(gentity_t *self, int bolt) {
    if (bolt >= 0) {
        ...
        trap->G2API_GetBoltMatrix(self->ghoul2, 0, bolt, &boltMatrix, &self->r.currentAngles, &self->r.currentOrigin, level.time, NULL, &self->modelScale);
...
void Mark1Dead_FireRocket(void) {
    ...
    trap->G2API_GetBoltMatrix(NPC->ghoul2, 0, bolt, &boltMatrix, &NPC->r.currentAngles, &NPC->r.currentOrigin, level.time, NULL, &NPC->modelScale);
...
void NPC_Mark1_Pain(gentity_t *self, gentity_t *attacker, int damage) {
    ...
    if ((trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "l_arm") > 0) && (trap->G2API_GetSurfaceRenderStatus(self->ghoul2, 0, "r_arm") > 0)) {
...
void Mark1_AttackDecision(void) {
    ...
    blasterTest = trap->G2API_GetSurfaceRenderStatus(NPC->ghoul2, 0, "l_arm");
    rocketTest = trap->G2API_GetSurfaceRenderStatus(NPC->ghoul2, 0, "r_arm");

    // It has both side weapons
    if (!blasterTest && !rocketTest) {
        ; // So do nothing.
    } else if (blasterTest != -1 && blasterTest) {
        distRate = DIST_LONG;
    } else if (rocketTest != -1 && rocketTest) {
        distRate = DIST_MELEE;
    } else // It should never get here, but just in case
    {
        ...
        JPLua::Entity_CallFunction(NPC, JPLua::JPLUA_ENTITY_DIE, (intptr_t)NPC, (intptr_t)NPC, (intptr_t)100, (intptr_t)MOD_UNKNOWN);
    }
