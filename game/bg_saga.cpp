...
// class flags
stringID_table_t bgSiegeClassFlagNames[] = {ENUM2STRING(CFL_MORESABERDMG),
...
// saber stances
stringID_table_t StanceTable[] = {ENUM2STRING(SS_NONE),   ENUM2STRING(SS_FAST),   ENUM2STRING(SS_MEDIUM),
                                  ENUM2STRING(SS_STRONG), ENUM2STRING(SS_DESANN), ENUM2STRING(SS_TAVION),
                                  ENUM2STRING(SS_DUAL),   ENUM2STRING(SS_STAFF),  {"", 0}};

// Weapon and force power tables are also used in NPC parsing code and some other places.
stringID_table_t WPTable[] = {{"NULL", WP_NONE},
                              ENUM2STRING(WP_NONE),
                              // Player weapons
...
stringID_table_t FPTable[] = {ENUM2STRING(FP_HEAL),
...
stringID_table_t HoldableTable[] = {ENUM2STRING(HI_NONE),
...
stringID_table_t PowerupTable[] = {ENUM2STRING(PW_NONE),
                                   ENUM2STRING(PW_QUAD),
...
void BG_SiegeStripTabs(char *buf) {
    ...
    while (buf[i]) {
        if (buf[i] != '\t') { // not a tab, just stick it in
            buf[i_r] = buf[i];
        } else { // If it's a tab, convert it to a space.
            buf[i_r] = ' ';
        }
...
int BG_SiegeGetValueGroup(const char *buf, const char *group, char *outbuf) {
    ...
    while (buf[i]) {
        if (buf[i] != ' ' && buf[i] != '{' && buf[i] != '}' && buf[i] != '\n' && buf[i] != '\r' && buf[i] != '\t') { // we're on a valid character
            if (buf[i] == '/' && buf[i + 1] == '/') {                                                                // this is a comment, so skip over it
                while (buf[i] && buf[i] != '\n' && buf[i] != '\r' && buf[i] != '\t') {
                    i++;
                }
            } else { // parse to the next space/endline/eos and check this value against our group value.
                j = 0;

                while (buf[i] != ' ' && buf[i] != '\n' && buf[i] != '\r' && buf[i] != '\t' && buf[i] != '{' && buf[i]) {
                    if (buf[i] == '/' && buf[i + 1] == '/') { // hit a comment, break out.
                        break;
                    }
                ...
                if (buf[i] == '/' && buf[i + 1] == '/') { // stopped on a comment, so first parse to the end of it.
                    while (buf[i] && buf[i] != '\n' && buf[i] != '\r') {
                        i++;
                    }
                    while (buf[i] == '\n' || buf[i] == '\r') {
                ...
                while (buf[i] && (buf[i] == ' ' || buf[i] == '\t' || buf[i] == '\n' || buf[i] == '\r')) { // parse to the next valid character
                    i++;
                }

                if (buf[i] == '{') { // if the next valid character is an opening bracket, then this is indeed a group
                    isGroup = qtrue;
                }

                // Is this the one we want?
                if (isGroup && !Q_stricmp(checkGroup, group)) { // guess so. Parse until we hit the { indicating the beginning of the group.
                    while (buf[i] != '{' && buf[i]) {
                        i++;
                    }

                    if (buf[i]) { // We're at the start of the group now, so parse to the closing bracket.
                        j = 0;

                        parseGroups = 0;

                        while ((buf[i] != '}' || parseGroups) && buf[i]) {
                            if (buf[i] == '{') { // increment for the opening bracket.
                                parseGroups++;
                            } else if (buf[i] == '}') { // decrement for the closing bracket
                                parseGroups--;
                            }

                            if (parseGroups < 0) { // Syntax error, I guess.
                            ...
                            if ((buf[i] != '{' || parseGroups > 1) &&
                                (buf[i] != '}' || parseGroups > 0)) { // don't put the start and end brackets for this group into the output buffer
                                outbuf[j] = buf[i];
                                j++;
                            }

                            if (buf[i] == '}' && !parseGroups) { // Alright, we can break out now.
                                break;
                            }
                ...
                } else if (!isGroup) { // if it wasn't a group, parse to the end of the line
                    while (buf[i] && buf[i] != '\n' && buf[i] != '\r') {
                        i++;
                    }
                } else { // this was a group but we not the one we wanted to find, so parse by it.
                    parseGroups = 0;
        ...
        } else if (buf[i] == '{') { // we're in a group that isn't the one we want, so parse to the end.
            parseGroups = 0;
...
int BG_SiegeGetPairedValue(const char *buf, const char *key, char *outbuf) {
    ...
    while (buf[i]) {
        if (buf[i] != ' ' && buf[i] != '{' && buf[i] != '}' && buf[i] != '\n' && buf[i] != '\r') { // we're on a valid character
            if (buf[i] == '/' && buf[i + 1] == '/') {                                              // this is a comment, so skip over it
                while (buf[i] && buf[i] != '\n' && buf[i] != '\r') {
                    i++;
                }
            } else { // parse to the next space/endline/eos and check this value against our key value.
                j = 0;
                ...
                if (buf[k] == '{') { // this is not the start of a value but rather of a group. We don't want to look in subgroups so skip over the whole thing.
                    int openB = 0;
                    ...
                    while (buf[i] && (buf[i] != '}' || openB)) {
                        ...
                        if (buf[i] == '}' && !openB) { // this is the end of the group
                            break;
                        }
                        i++;
                    }
                    ...
                    // Is this the one we want?
                    if (buf[i] != '/' || buf[i + 1] != '/') { // make sure we didn't stop on a comment, if we did then this is considered an error in the file.
                        if (!Q_stricmp(checkKey, key)) {      // guess so. Parse along to the next valid character, then put that into the output buffer and
                                                         // return 1.
                            while ((buf[i] == ' ' || buf[i] == '\n' || buf[i] == '\r' || buf[i] == '\t') && buf[i]) {
                                i++;
                            }

                            if (buf[i]) { // We're at the start of the value now.
                                qboolean parseToQuote = qfalse;

                                if (buf[i] == '\"') { // if the value is in quotes, then stop at the next quote instead of ' '
                                    i++;
                                    parseToQuote = qtrue;
                                }
                        ...
                        } else { // if that wasn't the desired key, then make sure we parse to the end of the line, so we don't mistake a value for a key
                            while (buf[i] && buf[i] != '\n') {
                                i++;
                            }
                        }
...
void BG_SiegeTranslateForcePowers(char *buf, siegeClass_t *siegeClass) {
    ...
    if (!Q_stricmp(buf, "FP_ALL")) { // this is a special case, just give us all the powers on level 3
        allPowers = qtrue;
    }

    if (buf[0] == '0' && !buf[1]) { // no powers then
        noPowers = qtrue;
    }
    ...
    if (allPowers || noPowers) { // we're done now then.
        return;
    }

    i = 0;
    while (buf[i]) { // parse through the list which is seperated by |, and add all the weapons into a bitflag
        if (buf[i] != ' ' && buf[i] != '|') {
            j = 0;
            ...
            if (buf[i] == ',') { // parse the power level
                i++;
                l = 0;
            ...
            } else { // if it's not there, assume level 3 I guess.
                parsedLevel = 3;
            }

            if (checkPower[0]) { // Got the name, compare it against the weapon table strings.
                k = 0;

                if (!Q_stricmp(checkPower, "FP_JUMP")) { // haqery
                    strcpy(checkPower, "FP_LEVITATION");
                }

                while (FPTable[k].id != -1 && FPTable[k].name[0]) {
                    if (!Q_stricmp(checkPower, FPTable[k].name)) { // found it, add the weapon into the weapons value
...
int BG_SiegeTranslateGenericTable(char *buf, stringID_table_t *table, qboolean bitflag) {
    ...
    if (buf[0] == '0' && !buf[1]) { // special case, no items.
        return 0;
    }

    while (buf[i]) { // Using basically the same parsing method as we do for weapons and forcepowers.
        if (buf[i] != ' ' && buf[i] != '|') {
            j = 0;
            ...
            if (checkItem[0]) {
                ...
                while (table[k].name && table[k].name[0]) {     // go through the list and check the parsed flag name against the hardcoded names
                    if (!Q_stricmp(checkItem, table[k].name)) { // Got it, so add the value into our items value.
                        if (bitflag) {
                            items |= (1 << table[k].id);
...
void BG_SiegeParseClassFile(const char *filename, siegeClassDesc_t *descBuffer) {
    ...
    } else { // It's ok if there isn't one, it's optional.
    ...
    } else { // fine, clear out the powers.
        i = 0;
        while (i < NUM_FORCE_POWERS) {
    ...
    } else { // It's alright, just default to 100 then.
    ...
    } else { // It's alright, just default to 0 then.
    ...
    if (BG_SiegeGetPairedValue(classInfo, "startarmor", parseBuf)) {
        ...
        if (!bgSiegeClasses[bgNumSiegeClasses].maxarmor) { // if they didn't specify a damn max armor then use this.
            bgSiegeClasses[bgNumSiegeClasses].maxarmor = bgSiegeClasses[bgNumSiegeClasses].startarmor;
        }
    } else { // default to maxarmor.
    ...
    } else { // It's alright, just default to 1 then.
...
#endif
    } else { // I guess this is an essential.. we don't want to render bad shaders or anything.
    ...
    } else { // Just don't start out with any then.
...
siegeClass_t *BG_SiegeFindClassByName(const char *classname) {
    ...
    while (i < bgNumSiegeClasses) {
        if (!Q_stricmp(bgSiegeClasses[i].name, classname)) { // found it
...
void BG_SiegeParseTeamFile(const char *filename) {
    ...
    if (BG_SiegeGetValueGroup(teamInfo, "Classes", teamInfo)) {
        while (success && i < MAX_SIEGE_CLASSES) { // keep checking for group values named class# up to MAX_SIEGE_CLASSES until we can't find one.
        ...
        while (i < t->numClasses) {
            ...
            while (sNum < MAX_SABERS) {
                ...
                if (saberName && saberName[0]) {
                    ...
                    if (!Q_stricmp(saberName, saber.name)) { // found the matching saber
                        if (saber.model[0]) {
...
qboolean BG_SiegeCheckClassLegality(int team, char *classname) {
    ...
    if (team == SIEGETEAM_TEAM1) {
        ...
        teamPtr = &team2Theme;
    } else { // spectator? Whatever, you're legal then.
        return qtrue;
    }

    if (!teamPtr || !(*teamPtr)) { // Well, guess the class is ok, seeing as there is no team theme to begin with.
        return qtrue;
    }

    // See if the class is listed on the team
    while (i < (*teamPtr)->numClasses) {
        if (!Q_stricmp(classname, (*teamPtr)->classes[i]->name)) { // found it, so it's alright
            return qtrue;
        }
        i++;
    }
...
siegeTeam_t *BG_SiegeFindTeamForTheme(char *themeName) {
    for (int i = 0; i < bgNumSiegeTeams; i++) {
        if (bgSiegeTeams[i].name[0] && !Q_stricmp(bgSiegeTeams[i].name, themeName)) { // this is what we're looking for
