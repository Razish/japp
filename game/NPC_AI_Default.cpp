    ...
    if (NPC->enemy == NULL) { // Possible to pick one up by being shot
        if (random() < 0.5f) {
            if (NPC->client->enemyTeam) {
                gentity_t *newenemy =
                    NPC_PickEnemy(NPC, NPC->client->enemyTeam, (NPC->cantHitEnemyCounter < 10), (NPC->client->enemyTeam == NPCTEAM_PLAYER), qtrue);
...
void NPC_BSHuntAndKill(void) {
    ...
    if (NPC->enemy) {
        ...
        if (enemyVisibility > VIS_PVS) {
            if (!NPC_EnemyTooFar(NPC->enemy, 0, qtrue)) { // Enemy is close enough to shoot - FIXME: this next func does this also, but need to know here for
                                                          // info on whether ot not to turn later
        ...
        if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 && curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 &&
            curAnim != BOTH_MELEE2) { // Don't move toward enemy if we're in a full-body attack anim
            ...
            if (enemyDist > 48 && ((enemyDist * 1.5f) * (enemyDist * 1.5f) >= NPC_MaxDistSquaredForWeapon() || oEVis != VIS_SHOOT ||
...
void NPC_BSPointShoot(qboolean shoot) { // FIXME: doesn't check for clear shot...
    ...
    if (!NPC->enemy || !NPC->enemy->inuse ||
        (NPC->enemy->NPC && NPC->enemy->health <= 0)) { // FIXME: should still keep shooting for a second or two after they actually die...
        trap->ICARUS_TaskIDComplete((sharedEntity_t *)NPC, TID_BSTATE);
        goto finished;
    }
...
void NPC_BSDefault(void) {
    ...
    if (NPC->enemy && !(NPCInfo->scriptFlags & SCF_FORCED_MARCH)) {
        ...
        if (NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader && !trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV)) {
            NPC_ClearGoal();
        }
        NPC_BSST_Attack();
        return;
    ...
    if (UpdateGoal()) { // have a goal
        if (!NPC->enemy && NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader &&
            !trap->ICARUS_TaskIDPending((sharedEntity_t *)NPC, TID_MOVE_NAV)) {
            NPC_BSFollowLeader();
        } else {
            // set angles
            if ((NPCInfo->scriptFlags & SCF_FACE_MOVE_DIR) ||
                NPCInfo->goalEntity != NPC->enemy) { // face direction of movement, NOTE: default behavior when not chasing enemy
