...
void NPC_BSGrenadier_Attack( void )
{
	...
	{//enemy is close and not using saber
		...
		{//grenadier
			...
			if ( !trace.allsolid && !trace.startsolid && (FloatCompare( trace.fraction, 1.0f, 0.001f ) || trace.entityNum == NPC->enemy->s.number ) )
	...
	*/
	...
	if ( enemyCS3 )
	{
		...
		else if ( NPC->client->ps.weapon == WP_STUN_BATON && enemyDist3 < (NPC->r.maxs.x+NPC->enemy->r.maxs.y+16)*(NPC->r.maxs.x+NPC->enemy->r.maxs.x+16) )
		{//close enough
			move3 = qfalse;
		}
