...
static void JP_Crash_AddOSData(fileHandle_t f) { Q_FSWriteString(f, va("Operating system: %s\n", JP_GetOSDisplayString())); }
...
static void JP_Crash_ListModules(fileHandle_t f) { SymEnumerateModules(GetCurrentProcess(), (PSYM_ENUMMODULES_CALLBACK)JP_Crash_EnumModules, (PVOID)f); }
...
static void JP_Crash_DisAsm(struct _EXCEPTION_POINTERS *EI, fileHandle_t f) {
    ...
    if (SymGetSymFromAddr(GetCurrentProcess(), (DWORD)EI->ExceptionRecord->ExceptionAddress, (PDWORD)&disp, sym)) {
        ...
        Q_FSWriteString(f, va("Crash location located at 0x%08X: %s::%s(+0x%X) [Func at 0x%08X]\n", EI->ExceptionRecord->ExceptionAddress, modname, sym->Name,
                              disp, sym->Address));
    ...
    addr = DisasmBacktrace((unsigned char *)mem.BaseAddress, (unsigned long)mem.BaseAddress, mem.RegionSize,
                           (unsigned long)EI->ExceptionRecord->ExceptionAddress, 10);
...
void ActivateCrashHandler(void) { oldHandler = SetUnhandledExceptionFilter(UnhandledExceptionHandler_Failsafe); }
...
static void JP_Crash_AddRegisterDump(ucontext_t *ctx, fileHandle_t f) {
    Q_FSWriteString(f, "General Purpose & Control Registers:\n");
    Q_FSWriteString(f, va("EAX: 0x%08X, EBX: 0x%08X, ECX: 0x%08X, EDX: 0x%08X\n", ctx->uc_mcontext.gregs[REG_EAX], ctx->uc_mcontext.gregs[REG_EBX],
                          ctx->uc_mcontext.gregs[REG_ECX], ctx->uc_mcontext.gregs[REG_EDX]));
    Q_FSWriteString(f, va("EDI: 0x%08X, ESI: 0x%08X, ESP: 0x%08X, EBP: 0x%08X\n", ctx->uc_mcontext.gregs[REG_EDI], ctx->uc_mcontext.gregs[REG_ESI],
                          ctx->uc_mcontext.gregs[REG_ESP], ctx->uc_mcontext.gregs[REG_EBP]));
    Q_FSWriteString(f, va("EIP: 0x%08X\n\n", ctx->uc_mcontext.gregs[REG_EIP]));
    Q_FSWriteString(f, "Segment Registers:\n");
    Q_FSWriteString(f,
                    va("CS: 0x%08X, DS: 0x%08X, ES: 0x%08X\n", ctx->uc_mcontext.gregs[REG_CS], ctx->uc_mcontext.gregs[REG_DS], ctx->uc_mcontext.gregs[REG_ES]));
    Q_FSWriteString(
        f, va("FS: 0x%08X, GS: 0x%08X, SS: 0x%08X\n\n", ctx->uc_mcontext.gregs[REG_FS], ctx->uc_mcontext.gregs[REG_GS], ctx->uc_mcontext.gregs[REG_SS]));
}
...
static void JP_Crash_ListModules(fileHandle_t f) {
    ...
    for (phdr = (ElfW(Phdr) *)((char *)ehdr + ehdr->e_phoff); phdr < (ElfW(Phdr) *)((char *)phdr + (ehdr->e_phnum * sizeof(ElfW(Phdr)))); phdr++) {
