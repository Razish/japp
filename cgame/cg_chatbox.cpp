...
static int CG_GetChatboxFont(void) { return Q_clampi(FONT_SMALL, cg_chatboxFont.integer, FONT_NUM_FONTS); }
...
bool CG_ContainsChatTabEscapeChar(const char *text) { return strchr(text, *TAB_EC) != nullptr; }
...
void CG_ChatboxAddMessage(const char *message, qboolean multiLine, const char *cbName) {
        ...
        if (accumLength > std::max(cg.chatbox.size.width, 128) && (i > 0 && !Q_IsColorString(p - 1))) {
            ...
            if (!multiLine) {
                ...
                if (cg_chatboxTimeShow.integer == 1) {
                    ...
                    Com_sprintf(chat->timeStamp, sizeof(chat->timeStamp), "[^%c%02i:%02i:%02i" S_COLOR_WHITE "] ", *(char *)cg_chatboxTimeColour.string,
                ...
                // server time
                    ...
                    Com_sprintf(chat->timeStamp, sizeof(chat->timeStamp), "[^%c%02i:%02i:%02i" S_COLOR_WHITE "] ", *(char *)cg_chatboxTimeColour.string, hours,
                                mins, seconds);
                }
            }
    ...
    if (!multiLine) {
        ...
        // server time
            ...
            Com_sprintf(chat->timeStamp, sizeof(chat->timeStamp), "[^%c%02i:%02i:%02i" S_COLOR_WHITE "] ", *(char *)cg_chatboxTimeColour.string, hours, mins,
                        seconds);
        }
    }
...
static void CG_ChatboxDrawTabs(void) {
    ...
    while (cb) {
        ...
        CG_FillRect(cg.chatbox.pos.x + cg.chatbox.size.width - textWidth - 16.0f - xOffset, cg.chatbox.pos.y + chatboxHeight + textHeight, textWidth + 16.0f,
                    textHeight, (cb == currentChatbox) ? &colorTable[CT_DKGREY] : &colorTable[CT_BLACK]);
        ...
        font.Paint(cg.chatbox.pos.x + cg.chatbox.size.width - textWidth - 8.0f - xOffset, cg.chatbox.pos.y + chatboxHeight + textHeight,
                   va("^%c%s", nameColour, cb->shortname), &colorWhite, uiTextStyle_e::Outlined);
...
void CG_ChatboxDraw(void) {
    ...
    if (!skipDraw && currentChatbox->scrollAmount < 0 && CG_ChatboxActive()) {
        font.Paint(cg.chatbox.pos.x, cg.chatbox.pos.y + yAccum - (height / 2.0f), scrollMsg, &colorWhite, uiTextStyle_e::Outlined);
    }
    yAccum += height;
    ...
    if (!skipDraw) {
        if (cg.chatbox.background.a > 0.0f) {
            for (done = 0; done < cg_chatboxLineCount.integer && i < MAX_CHATBOX_ENTRIES; i++, done++) {
                chatEntry_t *chat = &currentChatbox->chatBuffer[i];
                if (chat->isUsed && (chat->time >= cg.time - cg_chatbox.integer || currentChatbox->scrollAmount || CG_ChatboxActive())) {
                    CG_FillRect(cg.chatbox.pos.x, cg.chatbox.pos.y + yAccum, cg.chatbox.size.width, chatboxHeight, &cg.chatbox.background);
                    break;
                }
            }
        }
    ...
    if (!skipDraw) {
        for (done = 0; done < cg_chatboxLineCount.integer && i < MAX_CHATBOX_ENTRIES; i++, done++) {
            ...
            if (chat->isUsed) {
                //	last = chat;
                if (chat->time >= cg.time - cg_chatbox.integer || (currentChatbox->scrollAmount && CG_ChatboxActive()) || CG_ChatboxActive()) {
                    ...
                    font.Paint(cg.chatbox.pos.x, cg.chatbox.pos.y + yAccum - (height / 2.0f), tmp, &colorWhite, uiTextStyle_e::Outlined);
                    ...
                    for (chatEntry_t::urlLocation *url = chat->URLs; url; url = url->next) {
                        ...
                        font.Paint(url->pos.x, url->pos.y, scratch, &colorTable[CT_LTBLUE2], uiTextStyle_e::Outlined);
    ...
    if (CG_ChatboxActive()) {
        ...
        font.Paint(cg.chatbox.pos.x, cg.chatbox.pos.y + yAccum - (height / 2.0f), msg, &g_color_table[ColorIndex(COLOR_WHITE)], uiTextStyle_e::Outlined);
        if ((trap->Milliseconds() >> 8) & 1) {
            ...
            if (overstrike) {
                CG_FillRect(cg.chatbox.pos.x + cursorPre + cursorOffset, cg.chatbox.pos.y + yAccum, cursorWidth, cursorHeight / 1.5f,
                            &g_color_table[ColorIndex(COLOR_WHITE)]);
            } else {
                font.Paint(cg.chatbox.pos.x + cursorPre + cursorOffset, cg.chatbox.pos.y + yAccum - (cursorHeight / 2.0f), cursorChar,
...
void CG_ChatboxScroll(int direction) {
    ...
    // up
        ...
        currentChatbox->scrollAmount =
            std::max(scrollAmount - 1,
                     numActiveLines >= cg_chatboxLineCount.integer ? ((std::min(numActiveLines, MAX_CHATBOX_ENTRIES) - cg_chatboxLineCount.integer) * -1) : 0);
    }
}
...
void CG_ChatboxSelectTabNext(void) { currentChatbox = currentChatbox->next ? currentChatbox->next : chatboxList; }
...
void CG_ChatboxSelect(const char *cbName) { currentChatbox = CG_GetChatboxByName(cbName); }
...
static void ChatField_CharEvent(field_t *edit, int key) {
    ...
    if (!(key & K_CHAR_FLAG)) {
        ...
        case A_MOUSE1: {
            if (Q_PointInBounds(cgs.cursorX, cgs.cursorY, cg.chatbox.pos.x, inputLinePosY, SCREEN_WIDTH, inputLineHeight)) {
            ...
            } else if (Q_PointInBounds(cgs.cursorX, cgs.cursorY, cg.chatbox.pos.x, cg.chatbox.pos.y, chatboxWidth, chatboxHeight)) {
                // check for URLs
                int i = MAX_CHATBOX_ENTRIES - std::min(cg_chatboxLineCount.integer, currentChatbox->numActiveLines);
                int done = 0;
                i += currentChatbox->scrollAmount;
                for (done = 0; done < cg_chatboxLineCount.integer && i < MAX_CHATBOX_ENTRIES; i++, done++) {
                    ...
                    if (chat->isUsed) {
                        for (const chatEntry_t::urlLocation *url = chat->URLs; url; url = url->next) {
                            if (Q_PointInBounds(cgs.cursorX, cgs.cursorY, url->pos.x, url->pos.y, url->size.x, url->size.y)) {
                                Q_OpenURL(url->text);
                            }
                        }
                    }
...
void CG_ChatboxChar(int key) { ChatField_CharEvent(&chatField, key); }

qboolean CG_ChatboxActive(void) { return chatActive && (trap->Key_GetCatcher() & KEYCATCH_CGAME); }
