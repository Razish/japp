...
void CG_RegisterItemVisuals(int itemNum) {
    ...
    if (item->giType == IT_TEAM && cgs.gametype == GT_CTY && (item->giTag == PW_REDFLAG || item->giTag == PW_BLUEFLAG)) {
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team, vector3 *newAngles, qboolean thirdPerson) {
    ...
    if (ps || cg.renderingThirdPerson || cg_trueGuns.integer || cent->currentState.number != cg.predictedPlayerState.clientNum) {
...
void CG_AddViewWeapon(playerState_t *ps) {
    ...
    if (!cg.renderingThirdPerson && (cg_trueGuns.integer || weap == WP_SABER || weap == WP_MELEE) && cg_trueFOV.value &&
    ...
    if (!cg_fakeGun.integer && (!cg_drawGun.integer || cg.predictedPlayerState.zoomMode || cg_trueGuns.integer || weap == WP_SABER || weap == WP_MELEE)) {
        return;
    }
...
qboolean CG_WeaponCheck(int weap) {
    if (cg.snap->ps.ammo[weaponData[weap].ammoIndex] < weaponData[weap].shotCost &&
        cg.snap->ps.ammo[weaponData[weap].ammoIndex] < weaponData[weap].alt.shotCost) {
        return qfalse;
    }
...
static qboolean CG_WeaponSelectable(int i) {
    ...
    if (cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] < weaponData[i].shotCost &&
        cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] < weaponData[i].alt.shotCost) {
        return qfalse;
    }
...
void CG_DrawWeaponSelect(void) {
    int i, bits, count, smallIconSize, bigIconSize, holdX, x, y, pad, sideLeftIconCnt, sideRightIconCnt, sideMax, holdCount, iconCnt, yOffset = 0;
    ...
    if (media.gfx.interface.weaponIcons[cg.weaponSelect]) {
        ...
        if (!CG_WeaponCheck(cg.weaponSelect))
            CG_DrawPic(x - (bigIconSize / 2 * cgs.widthRatioCoef), (y - ((bigIconSize - smallIconSize) / 2)) + 10 + yOffset, bigIconSize * cgs.widthRatioCoef,
                       bigIconSize, media.gfx.interface.weaponIconsInactive[cg.weaponSelect]);
        else
            CG_DrawPic(x - (bigIconSize / 2 * cgs.widthRatioCoef), (y - ((bigIconSize - smallIconSize) / 2)) + 10 + yOffset, bigIconSize * cgs.widthRatioCoef,
                       bigIconSize, media.gfx.interface.weaponIcons[cg.weaponSelect]);
    }
...
void CG_GetWeaponMuzzleBolt(int clIndex, vector3 *to) {
    ...
    if (!cent || !cent->ghoul2 || !trap->G2_HaveWeGhoul2Models(cent->ghoul2) || !trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1)) {
        return;
    }
...
void CG_FireWeapon(centity_t *cent, qboolean altFire) {
    ...
    if (cg.predictedPlayerState.clientNum == cent->currentState.number) {
        if ((altFire && (ent->weapon == WP_BRYAR_PISTOL || ent->weapon == WP_BRYAR_OLD || ent->weapon == WP_DEMP2)) ||
            ...
            CGCam_Shake(val, 250);
        } else if (ent->weapon == WP_ROCKET_LAUNCHER || (ent->weapon == WP_REPEATER && altFire) || ent->weapon == WP_FLECHETTE ||
...
qboolean CG_VehicleWeaponImpact(centity_t *cent) {
    ...
    if ((cent->currentState.eFlags & EF_JETPACK_ACTIVE) // hack so we know we're a vehicle Weapon shot
        && cent->currentState.otherEntityNum2 && g_vehWeaponInfo[cent->currentState.otherEntityNum2].iImpactFX) {
...
void CG_CheckPlayerG2Weapons(playerState_t *ps, centity_t *cent) {
        ...
        ps->clientNum == cent->currentState.number) // don't want spectator mode forcing one client's weapon instance over another's
    {
        ...
        if (cent->weapon == WP_SABER && cent->weapon != ps->weapon && !ps->saberHolstered) {
            ...
            if (cgs.clientinfo[ps->clientNum].saber[1].soundOff && cgs.clientinfo[ps->clientNum].saber[1].model[0] && !ps->saberHolstered) {
