...
typedef struct TCGPointContents_s {
    vector3 mPoint;     // input
    int mPassEntityNum; // input
} TCGPointContents;

// CG_GET_BOLT_POS
typedef struct TCGGetBoltData_s {
    vector3 mOrigin; // output
    vector3 mAngles; // output
    vector3 mScale;  // output
...
typedef struct TCGVectorData_s {
    int mEntityNum; // input
    vector3 mPoint; // output
} TCGVectorData;

// CG_TRACE/CG_G2TRACE
typedef struct TCGTrace_s {
    trace_t mResult;                    // output
    vector3 mStart, mMins, mMaxs, mEnd; // input
...
typedef struct TCGMiscEnt_s {
    ...
    vector3 mOrigin, mAngles, mScale; // input
} TCGMiscEnt;
...
typedef struct TCGPositionOnBolt_s {
    ...
    vector3 angles;     // input
    vector3 modelScale; // input
} TCGPositionOnBolt;
...
typedef struct ragCallbackBoneInSolid_s {
    vector3 bonePos; // world coordinate position of the bone
    int entNum;      // index of entity who owns the bone in question
...
typedef struct cgameImport_s {
    ...
    uint32_t (*CM_PointContents)(const vector3 *p, clipHandle_t model);
    int (*CM_RegisterTerrain)(const char *config);
    clipHandle_t (*CM_TempModel)(const vector3 *mins, const vector3 *maxs, int capsule);
    void (*CM_Trace)(trace_t *results, const vector3 *start, const vector3 *end, const vector3 *mins, const vector3 *maxs, clipHandle_t model, int brushmask,
                     int capsule);
    uint32_t (*CM_TransformedPointContents)(const vector3 *p, clipHandle_t model, const vector3 *origin, const vector3 *angles);
    void (*CM_TransformedTrace)(trace_t *results, const vector3 *start, const vector3 *end, const vector3 *mins, const vector3 *maxs, clipHandle_t model,
                                int brushmask, const vector3 *origin, const vector3 *angles, int capsule);
    void (*RMG_Init)(int terrainID, const char *terrainInfo);

    // sound
    int (*S_AddLocalSet)(const char *name, vector3 *listener_origin, vector3 *origin, int entID, int time);
    ...
    void (*S_StartSound)(const vector3 *origin, int entnum, int entchannel, sfxHandle_t sfx);
    void (*S_StopBackgroundTrack)(void);
    void (*S_StopLoopingSound)(int entityNum);
    void (*S_UpdateEntityPosition)(int entityNum, const vector3 *origin);
    void (*S_UpdateAmbientSet)(const char *name, vector3 *origin);
    ...
    void (*R_AddDecalToScene)(qhandle_t shader, const vector3 *origin, const vector3 *dir, float orientation, float r, float g, float b, float a,
    ...
    qboolean (*R_InPVS)(const vector3 *p1, const vector3 *p2, byte *mask);
    ...
    int (*R_MarkFragments)(int numPoints, const vector3 *points, const vector3 *projection, int maxPoints, vector3 *pointBuffer, int maxFragments,
    ...
    void (*R_SetColor)(const vector4 *rgba); // NULL = 1,1,1,1
    ...
    void (*WE_AddWeatherZone)(vector3 *mins, vector3 *maxs);
    ...
    void (*SetClientForceAngle)(int time, vector3 *angle);
    void (*SetUserCmdValue)(int stateValue, float sensitivityScale, float mPitchOverride, float mYawOverride, float mSensitivityOverride, int fpSel,
    ...
    // FX
    void (*FX_AddLine)(vector3 *start, vector3 *end, float size1, float size2, float sizeParm, float alpha1, float alpha2, float alphaParm, const vector3 *sRGB,
    ...
    qboolean (*FX_PlayBoltedEffectID)(int id, vector3 *org, void *pGhoul2, const int boltNum, const int entNum, const int modelNum, int iLooptime,
                                      qboolean isRelative);
    ...
    qboolean (*G2API_RagPCJConstraint)(void *ghoul2, const char *boneName, vector3 *min, vector3 *max);
    qboolean (*G2API_RagPCJGradientSpeed)(void *ghoul2, const char *boneName, const float speed);
    qboolean (*G2API_RagEffectorGoal)(void *ghoul2, const char *boneName, vector3 *pos);
    qboolean (*G2API_GetRagBonePos)(void *ghoul2, const char *boneName, vector3 *pos, vector3 *entAngles, vector3 *entPos, vector3 *entScale);
    qboolean (*G2API_RagEffectorKick)(void *ghoul2, const char *boneName, vector3 *velocity);
...
typedef struct cgameExport_s {
    ...
    void (*GetOrigin)(int entID, vector3 *out);
    void (*GetAngles)(int entID, vector3 *out);
