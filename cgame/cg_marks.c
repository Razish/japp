...
vector3			pvforward, pvright, pvup;
...
void CG_Particle_OilSlick (qhandle_t pshader, centity_t *cent)
{
	...
	if (cent->currentState.angles2.z)
		p->endtime = cg.time + cent->currentState.angles2.z;
	else
...
void CG_ParticleDust (centity_t *cent, vector3 *origin, vector3 *dir)
{
	...
	for (i=0; i<dist; i++)
	{
		...
		p->accel.x = crandom()*3;
		p->accel.y = crandom()*3;
		p->accel.z = -PARTICLE_GRAVITY*0.4;
