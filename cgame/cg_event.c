...
void CG_EntityEvent( centity_t *cent, vector3 *position ) {
	...
	case EV_JUMP:
		DEBUGNAME("EV_JUMP");
		if ( cg_jumpSounds.integer && (!cg.predictedPlayerState.duelInProgress || (cg.predictedPlayerState.duelIndex == es->clientNum || cg.predictedPlayerState.clientNum == es->clientNum)) )
		{//Raz: duel isolation
...
//		trap->S_StartSound (NULL, es->number, CHAN_AUTO, media.sounds.weapons.noAmmo );
	...
	case EV_DISRUPTOR_MAIN_SHOT:
		...
		{
			if (cg.lastFPFlashPoint.x ||cg.lastFPFlashPoint.y || cg.lastFPFlashPoint.z)
