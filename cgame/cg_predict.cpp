...
void CG_BuildSolidList(void) {
    ...
    for (i = 0; i < snap->numEntities; i++) {
        ...
        if (ent->eType == ET_ITEM || ent->eType == ET_PUSH_TRIGGER || ent->eType == ET_TELEPORT_TRIGGER || (ent->eType == ET_SPECIAL && ent->userInt1)) {
...
static qboolean CG_VehicleClipCheck(centity_t *ignored, trace_t *trace) {
    ...
    if (ignored->currentState.m_iVehicleNum) {
        ...
        if (otherguy->currentState.m_iVehicleNum) {
            ...
            if (ignored->currentState.eType == ET_PLAYER || (ignored->currentState.eType == ET_NPC && ignored->currentState.NPC_class != CLASS_VEHICLE)) {
...
static void CG_ClipMoveToEntities(const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end, int skipNumber, int mask, trace_t *tr,
                                  qboolean g2Check) {
    int i, x, zd, zu;
    ...
    for (i = 0; i < cg_numSolidEntities; i++) {
        ...
        if (ent->solid == SOLID_BMODEL) {
            ...
            if (ent->eType == ET_NPC && ent->NPC_class == CLASS_VEHICLE && cent->m_pVehicle) {
                ...
                BG_VehicleAdjustBBoxForOrientation(cent->m_pVehicle, &cent->lerpOrigin, &bmins, &bmaxs, cent->currentState.number, MASK_PLAYERSOLID, NULL);
...
void CG_G2Trace(trace_t *result, const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end, int skipNumber, int mask) {
    trace_t t;
...
static void CG_TouchTriggerPrediction(void) {
    ...
    if (cg.predictedPlayerState.pm_type != PM_NORMAL && cg.predictedPlayerState.pm_type != PM_JETPACK && cg.predictedPlayerState.pm_type != PM_FLOAT &&
        !spectator) {
        return;
    }
    ...
    for (i = 0; i < cg_numTriggerEntities; i++) {
        ...
        if (ent->eType == ET_SPECIAL && ent->userInt1) {
            if (!(cg.predictedPlayerState.origin.x - ent->origin.x > 44 || cg.predictedPlayerState.origin.x - ent->origin.x < -50 ||
                  cg.predictedPlayerState.origin.y - ent->origin.y > 36 || cg.predictedPlayerState.origin.y - ent->origin.y < -36 ||
...
static int FindGrappleHook(int clientNum) {
    for (centity_t &cent : cg_entities) {
        if (cent.currentValid && cent.currentState.eType == ET_MISSILE && cent.currentState.weapon == WP_STUN_BATON && !cent.currentState.generic1) {
            // this is a hook
...
void CG_PredictPlayerState(void) {
    ...
    for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
        ...
        if (CG_Piloting(oldPlayerState.m_iVehicleNum) && cg.predictedVehicleState.commandTime == oldVehicleState.commandTime) {
            vector3 delta;
            float len;
            ...
            if (cg.thisFrameTeleport) {
                ...
                vector3 adjusted;
                CG_AdjustPositionForMover(&cg.predictedVehicleState.origin, cg.predictedVehicleState.groundEntityNum, cg.physicsTime, cg.oldTime, &adjusted);
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (((ps->duelInProgress && ps->duelIndex != es->number) ||
                 (!ps->duelInProgress && es->bolt1)) ||                     // They're dueling, or we're dueling but not with them
    ...
    if (CG_Piloting(cg.predictedPlayerState.m_iVehicleNum)) {
        CG_AdjustPositionForMover(&cg.predictedVehicleState.origin, cg.predictedVehicleState.groundEntityNum, cg.physicsTime, cg.time,
                                  &cg.predictedVehicleState.origin);
    } else {
        // adjust for the movement of the groundentity
        CG_AdjustPositionForMover(&cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.time,
