...
void CG_NewClientInfo(int clientNum, qboolean entitiesInitialized) {
    ...
    if (cgs.gametype >= GT_TEAM && !cgs.jediVmerc && cgs.gametype != GT_SIEGE && (!cg_forceModel.integer || clientNum == cg.clientNum)) {
        BG_ValidateSkinForTeam(newInfo.modelName, newInfo.skinName, newInfo.team, reinterpret_cast<vector3 *>(&newInfo.colorOverride));
    } else
    ...
    *ci = newInfo;
    ...
    if (entitiesInitialized && ci->ghoul2Model && oldGhoul2 != ci->ghoul2Model) {
        ...
        if (anim) {
            ...
            // rww - Set the animation again because it just got reset due to the model change
            trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "lower_lumbar", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame,
                                    150);
...
static void CG_PlayerFootsteps(centity_t *cent, footstepType_e footStepType) {
    ...
    if (cent->currentState.NPC_class != CLASS_ATST && cent->currentState.NPC_class != CLASS_CLAW && cent->currentState.NPC_class != CLASS_FISH &&
        cent->currentState.NPC_class != CLASS_FLIER2 && cent->currentState.NPC_class != CLASS_GLIDER && cent->currentState.NPC_class != CLASS_INTERROGATOR &&
        cent->currentState.NPC_class != CLASS_MURJJ && cent->currentState.NPC_class != CLASS_PROBE && cent->currentState.NPC_class != CLASS_R2D2 &&
        cent->currentState.NPC_class != CLASS_R5D2 && cent->currentState.NPC_class != CLASS_REMOTE && cent->currentState.NPC_class != CLASS_SEEKER &&
        cent->currentState.NPC_class != CLASS_SENTRY && cent->currentState.NPC_class != CLASS_SWAMP) {
        ...
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, footBolt, &boltMatrix, &tempAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
...
static void CG_SetLerpFrameAnimation(centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float animSpeedMult, qboolean torsoOnly,
    ...
    if (cent->ghoul2) {
        ...
        if (torsoOnly && !cent->noLumbar) {
            ...
            if (cent->currentState.torsoAnim == cent->currentState.legsAnim && (ci->legsAnim != newAnimation || oldSpeed != animSpeed)) {
                float GBAcFrame = 0;
...
static void CG_RunLerpFrame(centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, qboolean flipState, int newAnimation, float speedScale, qboolean torsoOnly) {
    ...
    if (cent->currentState.forceFrame) {
        if (lf->lastForcedFrame != cent->currentState.forceFrame) {
            ...
            trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed,
                                    cg.time, -1, 150);
            trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed, cg.time,
                                    -1, 150);
            trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed, cg.time, -1,
                                    150);
        }
        ...
        if ((newAnimation != lf->animationNumber || cent->currentState.brokenLimbs != ci->brokenLimbs || lf->lastFlip != flipState || !lf->animation) ||
...
void CG_G2SetBoneAngles(void *ghoul2, int modelIndex, const char *boneName, const vector3 *angles, const int flags, const int up, const int right,
                        const int forward, qhandle_t *modelList, int blendTime, int currentTime) {
    ...
    trap->G2API_SetBoneAngles(ghoul2, modelIndex, boneName, angles, flags, up, right, forward, modelList, blendTime, currentTime);
}
...
void CG_Rag_Trace(trace_t *result, const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end, int skipNumber, int mask) {
...
void CG_TempTestFunction(centity_t *cent, vector3 *forcedAngles) {
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, tAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
...
static const char *cg_effectorStringTable[] = {
    //	"thoracic",
    //	"rhand",
    "lhand", "rtibia", "ltibia", "rtalus", "ltalus",
    //	"rradiusX",
...
static int CG_RagAnimForPositioning(centity_t *cent) {
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &matrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
...
qboolean CG_RagDoll(centity_t *cent, vector3 *forcedAngles) {
        ...
        if (!inSomething) {
            ...
            while (i < 5) {
                if (i < 2) {
                    ...
                    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, boltChecks[i], &boltMatrix, &tAng, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                              &cent->modelScale);
    ...
    if (cent->isRagging) {
...
#else // with my new method of doing things I want it to continue the anim
        {
            ...
            if (trap->G2API_GetBoneAnim(cent->ghoul2, "model_root", cg.time, &currentFrame, &startFrame, &endFrame, &flags, &animSpeed, cgs.gameModels,
                                        0)) { // lock the anim on the current frame.
                int blendTime = 500;
        ...
        if (cent->currentState.ragAttach && (cent->currentState.eType != ET_NPC || cent->currentState.NPC_class != CLASS_VEHICLE)) {
            centity_t *grabEnt;
            ...
            if (grabEnt->ghoul2) {
                ...
                trap->G2API_GetBoltMatrix(grabEnt->ghoul2, 0, 0, &matrix, &grabEnt->turAngles, &grabEnt->lerpOrigin, cg.time, cgs.gameModels,
                                          &grabEnt->modelScale);
                BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, &bOrg);

                // Get our hand's location
                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, 0, &matrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
                ...
                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, thorBolt, &matrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
...
void CG_G2ServerBoneAngles(centity_t *cent) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i == 0)
            bone = &cent->currentState.boneAngles2;
        else if (i == 1)
            bone = &cent->currentState.boneAngles3;
        else if (i == 2)
            bone = &cent->currentState.boneAngles4;
...
static void CG_G2SetHeadBlink(centity_t *cent, qboolean bStart) {
    ...
    trap->G2API_SetBoneAngles(cent->ghoul2, 0, "leye", &desiredAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, blendTime, cg.time);

    if (hReye == -1)
        return;

    if (!bWink) {
        trap->G2API_SetBoneAngles(cent->ghoul2, 0, "reye", &desiredAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, blendTime, cg.time);
    }
}
...
static void CG_G2PlayerAngles(centity_t *cent, vector3 legs[3], vector3 *legsAngles) {
    ...
    if (cent->localAnimIndex <= 1) {
        ...
        BG_G2PlayerAngles(cent->ghoul2, ci->bolt_motion, &cent->currentState, cg.time, &cent->lerpOrigin, &cent->lerpAngles, legs, legsAngles,
                          &cent->pe.torso.yawing, &cent->pe.torso.pitching, &cent->pe.legs.yawing, &cent->pe.torso.yawAngle, &cent->pe.torso.pitchAngle,
                          &cent->pe.legs.yawAngle, cg.frametime, &cent->turAngles, &cent->modelScale, ci->legsAnim, ci->torsoAnim, &ci->corrTime, &lookAngles,
                          &ci->lastHeadAngles, ci->lookTime, emplaced, &ci->superSmoothTime);
        ...
        if (cent->currentState.heldByClient && cent->currentState.heldByClient <= MAX_CLIENTS) {
            ...
            if (other && other->ghoul2 && ci->bolt_lhand) {
                ...
                trap->G2API_GetBoltMatrix(other->ghoul2, 0, ci->bolt_lhand, &boltMatrix, &other->turAngles, &other->lerpOrigin, cg.time, cgs.gameModels,
                                          &other->modelScale);
                BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, &boltOrg);

                BG_IK_MoveArm(cent->ghoul2, ci->bolt_lhand, cg.time, &cent->currentState, cent->currentState.torsoAnim, &boltOrg, &cent->ikStatus,
                              &cent->lerpOrigin, &cent->lerpAngles, &cent->modelScale, 500, qfalse);
            }
        } else if (cent->ikStatus) {
            // make sure we aren't IKing if we don't have anyone to hold onto us.
            BG_IK_MoveArm(cent->ghoul2, ci->bolt_lhand, cg.time, &cent->currentState, cent->currentState.torsoAnim, &vec3_origin, &cent->ikStatus,
                          &cent->lerpOrigin, &cent->lerpAngles, &cent->modelScale, 500, qtrue);
        }
...
static void CG_PlayerFlag(centity_t *cent, qhandle_t hModel) {
    ...
    if (cent->currentState.number == cg.snap->ps.clientNum && !cg.renderingThirdPerson && !cg_trueGuns.integer && cg.snap->ps.weapon != WP_SABER) {
        return;
    }
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane) {
    ...
    *shadowPlane = 0.0f;
    ...
    if (cent->currentState.NPC_class == CLASS_REMOTE || cent->currentState.NPC_class == CLASS_SEEKER)
        radius = 8.0f;
    CG_ImpactMark(media.gfx.world.shadowMark, &trace.endpos, &trace.plane.normal, cent->pe.legs.yawAngle, alpha, alpha, alpha, 1, qfalse, radius, qtrue);
...
static const char *cg_pushBoneNames[] = {"cranium", "lower_lumbar", "rhand", "lhand", "ltibia", "rtibia", "lradius", "rradius", NULL};
...
void CG_DrawPlayerShield(centity_t *cent, vector3 *origin) {
    ...
    if (cent->currentState.eFlags & EF_DEAD || cg_shieldStyle.integer != -1 || (!cg.renderingThirdPerson && cent->currentState.number == cg.clientNum)) {
...
int getint(const char **buf) { return (int)strtod(*buf, (char **)buf); }
...
void CG_DoSaber(vector3 *origin, vector3 *dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, int cnum,
                int bnum) {
    vector3 mid, rgb;
...
void CG_DoSFXSaber(vector3 *blade_muz, vector3 *blade_tip, vector3 *trail_tip, vector3 *trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx,
                   qboolean doLight, qboolean doTrail, int cnum, int bnum) {
    vector3 dif, mid, blade_dir, end_dir, trail_dir, base_dir, rgb;
    float radiusmult, effectradius, coreradius, effectalpha, AngleScale, blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif, glowscale = 0.5f, v1,
                                                                                                                                          v2, len;
    int i;
    refdef_t *refdef = CG_GetRefdef();
    qhandle_t glow = 0;
...
qboolean CG_G2TraceCollide(trace_t *tr, const vector3 *mins, const vector3 *maxs, vector3 *lastValidStart, vector3 *lastValidEnd) {
    ...
    if (g2Hit && g2Hit->ghoul2) {
        ...
        if (com_optvehtrace.integer && g2Hit->currentState.eType == ET_NPC && g2Hit->currentState.NPC_class == CLASS_VEHICLE && g2Hit->m_pVehicle) {
            trap->G2API_CollisionDetectCache(G2Trace, g2Hit->ghoul2, &angles, &g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, lastValidStart,
                                             lastValidEnd, &g2Hit->modelScale, 0, cg_g2TraceLod.integer, fRadius);
        } else {
            trap->G2API_CollisionDetect(G2Trace, g2Hit->ghoul2, &angles, &g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, lastValidStart, lastValidEnd,
...
void CG_AddGhoul2Mark(int shader, float size, vector3 *start, vector3 *end, int entnum, vector3 *entposition, float entangle, void *ghoul2, vector3 *scale,
                      int lifeTime) {
            ...
            if (cg_entities[trace.entityNum].ghoul2) {
                ...
                if (trace.entityNum != ENTITYNUM_NONE) {
                    ...
                    if (cg_ghoul2Marks.integer) {
                        ...
                        if (trEnt->ghoul2) {
                            if (trEnt->currentState.eType != ET_NPC || trEnt->currentState.NPC_class != CLASS_VEHICLE || !trEnt->m_pVehicle ||
                                trEnt->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER) { // don't do on fighters cause they have crazy full axial angles
                                ...
                                CG_AddGhoul2Mark(markShader, flrand(3.0f, 4.0f), &trace.endpos, &ePos, trace.entityNum, &trEnt->lerpOrigin,
                                                 trEnt->lerpAngles.yaw, trEnt->ghoul2, &trEnt->modelScale, Q_irand(5000, 10000));
                                if (weaponMarkShader) {
                                    vector3 splashBackDir;
                                    VectorScale(&ePos, -1, &splashBackDir);
                                    CG_AddGhoul2Mark(weaponMarkShader, flrand(0.5f, 2.0f), &trace.endpos, &splashBackDir, owner->currentState.clientNum,
                                                     &owner->lerpOrigin, owner->lerpAngles.yaw, owner->ghoul2, &owner->modelScale, Q_irand(5000, 10000));
                                }
                            }
                        }
                    }
...
void CG_AddSaberBlade(centity_t *cent, centity_t *scent, refEntity_t *saber, int renderfx, int modelIndex, int saberNum, int bladeNum, vector3 *origin,
                      vector3 *angles, qboolean fromSaber, qboolean dontDraw) {
    ...
    if (!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) && client->saber[saberNum].bladeEffect) {
        trap->FX_PlayBoltedEffectID(client->saber[saberNum].bladeEffect, &scent->lerpOrigin, scent->ghoul2, bladeNum, scent->currentState.number, useModelIndex,
                                    -1, qfalse);
    } else if (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) && client->saber[saberNum].bladeEffect2) {
        trap->FX_PlayBoltedEffectID(client->saber[saberNum].bladeEffect2, &scent->lerpOrigin, scent->ghoul2, bladeNum, scent->currentState.number,
                                    useModelIndex, -1, qfalse);
    }
    ...
    if ((cg_teamSabersRGB.integer < 1 || (cg_teamSabersRGB.integer == 1 && cg.snap && cg.snap->ps.clientNum != cent->currentState.number)) &&
        cgs.gametype >= GT_TEAM && cgs.gametype != GT_SIEGE && !cgs.jediVmerc && cent->currentState.eType != ET_NPC) {
...
CheckTrail:
    ...
    if ((!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) && client->saber[saberNum].trailStyle > 1) ||
        (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) &&
         client->saber[saberNum].trailStyle2 > 1)) { // don't actually draw the trail at all
        goto JustDoIt;
    }
        ...
        if (cg.time > saberTrail->lastTime + 2 || cg_saberTrail.integer == 2) {
            if (!dontDraw) {
                if ((cg_saberTrail.integer == 3 && cg.time < saberTrail->lastTime + 2000) ||
                    ((BG_SuperBreakWinAnim(cent->currentState.torsoAnim) || saberMoveData[cent->currentState.saberMove].trailLength > 0 ||
                      ((cent->currentState.powerups & (1 << PW_SPEED) && cg_speedTrail.integer)) || (cent->currentState.saberInFlight && saberNum == 0)) &&
                    ...
                    if (diff <= 10000) {
                        ...
                        if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4) {
                            ...
                            if (BG_SaberInAttack(cent->currentState.saberMove) || BG_SuperBreakWinAnim(cent->currentState.torsoAnim)) {
                                fx.mKillTime = 300;
                            } else
                                fx.mKillTime = 40;
...
JustDoIt:
    ...
    if (sfxSabers) {
        CG_DoSFXSaber(&fx.mVerts[0].origin, &fx.mVerts[1].origin, &fx.mVerts[2].origin, &fx.mVerts[3].origin,
                      (client->saber[saberNum].blade[bladeNum].lengthMax), (client->saber[saberNum].blade[bladeNum].radius), (saber_colors_t)scolor, renderfx,
                      (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)),
                      (qboolean)(cg_saberTrail.integer > 0), cent->currentState.clientNum, saberNum);
    } else {
        CG_DoSaber(&org, &axis[0], saberLen, client->saber[saberNum].blade[bladeNum].lengthMax, client->saber[saberNum].blade[bladeNum].radius,
                   (saber_colors_t)scolor, renderfx, (client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)),
...
static void CG_CreateSurfaceDebris(centity_t *cent, int surfNum, int fxID, qboolean throwPart) {
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, b, &boltMatrix, &cent->lerpAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
...
qboolean CG_VehicleShouldDrawShields(centity_t *vehCent) {
    if (vehCent->damageTime > cg.time && vehCent->currentState.NPC_class == CLASS_VEHICLE && vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo) {
        return qtrue;
    }
...
qboolean CG_VehicleAttachDroidUnit(centity_t *droidCent, refEntity_t *legs) {
    if (droidCent && droidCent->currentState.owner && droidCent->currentState.clientNum >= MAX_CLIENTS) {
        ...
        if (vehCent && vehCent->m_pVehicle && vehCent->ghoul2 && vehCent->m_pVehicle->m_iDroidUnitTag != -1) {
            ...
            trap->G2API_GetBoltMatrix(vehCent->ghoul2, 0, vehCent->m_pVehicle->m_iDroidUnitTag, &boltMatrix, &vehCent->lerpAngles, &vehCent->lerpOrigin,
                                      cg.time, cgs.gameModels, &vehCent->modelScale);
...
void CG_CreateNPCClient(clientInfo_t **ci) { *ci = (clientInfo_t *)BG_Alloc(sizeof(clientInfo_t)); }

void CG_DestroyNPCClient(clientInfo_t **ci) { memset(*ci, 0, sizeof(clientInfo_t)); }
...
static void CG_ForceElectrocution(centity_t *cent, const vector3 *origin, vector3 *tempAngles, qhandle_t shader, qboolean alwaysDo) {
    ...
    while (bolt < 0) {
        ...
        if (test == 0)
            bolt = elbowRBolt; // right elbow
        else if (test == 1)
            bolt = handLBolt; // left hand
        else if (test == 2)
            bolt = handRBolt; // right hand
        else if (test == 3)
            bolt = footLBolt; // left foot
        else if (test == 4)
            bolt = footRBolt; // right foot
        else if (test == 5)
            bolt = torsoBolt; // torso
        else
            bolt = elbowLBolt; // left elbow
    ...
    if (bolt >= 0) {
        found = trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &boltMatrix, tempAngles, origin, cg.time, cgs.gameModels, &cent->modelScale);
    }
...
static void CG_VehicleEffects(centity_t *cent) {
    ...
    if (cent->currentState.clientNum == cg.predictedPlayerState.m_iVehicleNum && (cent->currentState.eFlags2 & EF2_HYPERSPACE)) { // in hyperspace!
        if (cg.predictedVehicleState.hyperSpaceTime && (cg.time - cg.predictedVehicleState.hyperSpaceTime) < HYPERSPACE_TIME) {
            if (!cg_lastHyperSpaceEffectTime || (cg.time - cg_lastHyperSpaceEffectTime) > HYPERSPACE_TIME + 500) {
                ...
                trap->FX_PlayBoltedEffectID(media.efx.hyperspaceStars, &cent->lerpOrigin, cent->ghoul2, 0, cent->currentState.number, 0, 0, qtrue);
            ...
            if (cent->currentState.speed > 0) {
                ...
                if (doExhaust && cent->ghoul2) {
                    ...
                    for (i = 0; i < MAX_VEHICLE_EXHAUSTS; i++) {
                        ...
                        if (pVehNPC->m_pVehicleInfo->type == VH_FIGHTER) {
                            trap->FX_PlayBoltedEffectID(fx, &cent->lerpOrigin, cent->ghoul2, pVehNPC->m_iExhaustTag[i], cent->currentState.number, 0, 0, qtrue);
                        } else {
                            // fixme: bolt these too
                            mdxaBone_t boltMatrix;
                            vector3 boltOrg, boltDir;

                            trap->G2API_GetBoltMatrix(cent->ghoul2, 0, pVehNPC->m_iExhaustTag[i], &boltMatrix, &flat, &cent->lerpOrigin, cg.time,
                ...
                if (pVehNPC->m_pVehicleInfo->iTrailFX != 0 && cent->ghoul2) {
                    ...
                    for (i = 1; i < 5; i++) {
                        ...
                        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, trailBolt, &boltMatrix, &getBoltAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                                  &cent->modelScale);
...
void CG_CheckThirdPersonAlpha(centity_t *cent, refEntity_t *legs) {
    ...
    if (cent->m_pVehicle) {
        if (cg.predictedPlayerState.m_iVehicleNum != cent->currentState.clientNum && cent->m_pVehicle->m_pVehicleInfo &&
            cent->m_pVehicle->m_pVehicleInfo->cameraOverride && cent->m_pVehicle->m_pVehicleInfo->cameraAlpha) {
...
void CG_Player(centity_t *cent) {
    qboolean shadow = qfalse, iwantout = qfalse, successchange = qfalse, doAlpha = qfalse, gotLHandMatrix = qfalse, g2HasWeapon = qfalse,
             drawPlayerSaber = qfalse, checkDroidShields = qfalse;
    ...
    if (cent->currentState.weapon == WP_EMPLACED_GUN && cent->currentState.otherEntityNum2) {
        ...
        if (BG_EmplacedView(&cent->lerpAngles, &cg_entities[cent->currentState.otherEntityNum2].currentState.angles, &empYaw,
                            cg_entities[cent->currentState.otherEntityNum2].currentState.origin2.pitch)) {
    ...
    if ((cent->doLerp || cent->currentState.heldByClient) &&
        (cent->currentState.groundEntityNum >= ENTITYNUM_WORLD || cent->currentState.eType == ET_TERRAIN) && !(cent->currentState.eFlags2 & EF2_HYPERSPACE) &&
        cg.predictedPlayerState.m_iVehicleNum != cent->currentState.number) { // always smooth when being thrown
        vector3 posDif;
    ...
    if (cgs.gametype >= GT_TEAM) {
        if (cent->currentState.eType != ET_NPC && cg.snap->ps.clientNum != cent->currentState.number && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
            CG_AddRadarEnt(cent);
        }
    }
        ...
        if (cent->currentState.eFlags & EF_JETPACK_ACTIVE) {
            ...
            for (n = 0; n < 2; n++) {
                ...
                if (cent->currentState.eFlags & EF_JETPACK_FLAMING) {
                    ...
                    trap->S_AddLoopingSound(cent->currentState.number, &cent->lerpOrigin, &vec3_origin, media.sounds.items.jetpackLoop);
                } else // FIXME: Different smaller effect for idle
                    trap->FX_PlayEffectID(media.efx.bobaJet, &flamePos, &flameDir, -1, -1, qfalse);
            }

            trap->S_AddLoopingSound(cent->currentState.number, &cent->lerpOrigin, &vec3_origin, media.sounds.items.jetpackHover);
        }
    }
    ...
    if (cent->currentState.number == cg.snap->ps.clientNum) {
        if (!cg.renderingThirdPerson) {
            if ((!cg_trueGuns.integer && cg.predictedPlayerState.weapon != WP_SABER && cg.predictedPlayerState.weapon != WP_MELEE) ||
                (cg.predictedPlayerState.weapon == WP_SABER && cg_trueSaberOnly.integer) || cg.predictedPlayerState.zoomMode || cg_fakeGun.integer) {
    ...
    if (cent->ghoul2 &&
        (cent->currentState.eType != ET_NPC ||
         (cent->currentState.NPC_class != CLASS_VEHICLE && cent->currentState.NPC_class != CLASS_REMOTE && cent->currentState.NPC_class != CLASS_SEEKER)) &&
        cent->ghoul2weapon != CG_G2WeaponInstance(cent, cent->currentState.weapon) && !(cent->currentState.eFlags & EF_DEAD) && !cent->torsoBolt && cg.snap &&
        ...
        if (ci->team == TEAM_SPECTATOR) {
            ...
            if (cent->currentState.eType != ET_NPC) {
                if (cent->weapon == WP_SABER && cent->weapon != cent->currentState.weapon && !cent->currentState.saberHolstered) {
                ...
                } else if (cent->currentState.weapon == WP_SABER && cent->weapon != cent->currentState.weapon && !cent->saberWasInFlight) {
    ...
    if (cgs.gametype >= GT_TEAM && cg_drawFriend.integer && cent->currentState.number != cg.snap->ps.clientNum && cent->currentState.eType != ET_NPC) {
        // If the view is either a spectator or on the same team as this character, show a symbol above their head.
        if ((cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR || cg.snap->ps.persistant[PERS_TEAM] == team) && !(cent->currentState.eFlags & EF_DEAD)) {
            if (cgs.gametype == GT_SIEGE) {
    ...
    if ((cent->currentState.eFlags2 & EF2_HELD_BY_MONSTER) && cent->currentState.hasLookTarget) {
        ...
        if (rancor) {
            BG_AttachToRancor(rancor->ghoul2, rancor->lerpAngles.yaw, &rancor->lerpOrigin, cg.time, cgs.gameModels, &rancor->modelScale,
                              (rancor->currentState.eFlags2 & EF2_GENERIC_NPC_FLAG), &legs.origin, &legs.angles, NULL);
...
skipTrail:
    ...
    if (cent->currentState.activeForcePass > FORCE_LEVEL_3 && cent->currentState.NPC_class != CLASS_VEHICLE) {
        ...
        if (cent->currentState.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_HOLD && Q_irand(0, 1)) {
            ...
            trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &rHandMatrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                      &cent->modelScale);
            ...
            //	cgs.gameModels, cent->modelScale );
            if (!gotLHandMatrix) {
                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &lHandMatrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                          &cent->modelScale);
                gotLHandMatrix = qtrue;
            }
    ...
    if ((cent->currentState.eFlags & EF_BODYPUSH) && (cent->currentState.number != cg.clientNum || cg.renderingThirdPerson)) {
    ...
    if (cent->currentState.powerups & (1 << PW_DISINT_4)) {
        ...
        if ((cent->currentState.forcePowersActive & (1 << FP_GRIP)) &&
            (cg.renderingThirdPerson || cent->currentState.number != cg.snap->ps.clientNum || cg_trueGuns.integer ||
    ...
    if (cent->currentState.weapon == WP_STUN_BATON) {
        trap->S_AddLoopingSound(cent->currentState.number, &cent->lerpOrigin, &vec3_origin, media.sounds.weapons.stunBatonIdle);
    }
    ...
    if ((cent->currentState.powerups & (1 << PW_YSALAMIRI)) ||
        (cgs.gametype == GT_CTY && (cent->currentState.powerups & ((1 << PW_REDFLAG) | (1 << PW_BLUEFLAG))))) {
...
stillDoSaber:
            ...
            if (ci->saber[0].model[0] && ci->saber[0].soundLoop && !cent->currentState.saberInFlight) {
                ...
                for (i = 0; i < ci->saber[0].numBlades; i++) {
                    if (ci->saber[0].blade[i].length) {
                        trap->S_AddLoopingSound(cent->currentState.number, &soundSpot, &vec3_origin, ci->saber[0].soundLoop);
                        didFirstSound = qtrue;
                        break;
                    }
                }
            }
            if (ci->saber[1].model[0] && ci->saber[1].soundLoop && (!didFirstSound || ci->saber[0].soundLoop != ci->saber[1].soundLoop)) {
                int i;

                for (i = 0; i < ci->saber[1].numBlades; i++) {
                    if (ci->saber[1].blade[i].length) {
                        trap->S_AddLoopingSound(cent->currentState.number, &soundSpot, &vec3_origin, ci->saber[1].soundLoop);
                        break;
                    }
                }
            }
        ...
        if (cent->currentState.saberInFlight && cent->currentState.saberEntityNum) {
            ...
            if (g2HasWeapon || !cent->bolt3 || saberEnt->serverSaberHitIndex != saberEnt->currentState.modelindex) {
                ...
                if (g2HasWeapon) {
                    ...
                    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &boltMat, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                              &cent->modelScale);
    ...
    if (drawPlayerSaber) {
        ...
        for (; l < MAX_SABERS; l++) {
            ...
            for (k = 0; k < ci->saber[l].numBlades; k++) {
                ...
                else if (ci->saber[1].model[0] && cent->currentState.saberHolstered == 1 && l > 0 && ci->saber[l].blade[k].length <= 0) {
    ...
    if (((cent->shieldHit || (cg_shieldTest.integer && cent->currentState.number == cg.clientNum)) && cg_shieldStyle.integer != -1 &&
         !(cent->playerState->eFlags & EF_DEAD) &&
         (cent->currentState.number != cg.clientNum || (cg.predictedPlayerState.zoomMode == 0 && cg.renderingThirdPerson)))) {
        uint32_t rfx = legs.renderfx;
        int i, customShader = legs.customShader;
        byte savedRGBA[4] = {0xff};

        savedRGBA[0] = legs.shaderRGBA[0];
        legs.shaderRGBA[0] = cg.shieldColour.rgba.r;
        savedRGBA[1] = legs.shaderRGBA[1];
        legs.shaderRGBA[1] = cg.shieldColour.rgba.g;
        savedRGBA[2] = legs.shaderRGBA[2];
        legs.shaderRGBA[2] = cg.shieldColour.rgba.b;
        savedRGBA[3] = legs.shaderRGBA[3];
        legs.shaderRGBA[3] = cg.shieldColour.rgba.a;
        legs.renderfx = RF_RGB_TINT;

        if (cg.shieldColour.forceAlpha == 1)
            legs.renderfx |= RF_FORCE_ENT_ALPHA;

        if (cg_shieldStyle.integer == 1)
            legs.customShader = media.gfx.world.protect;
        else if (cg_shieldStyle.integer == 2)
            legs.customShader = media.gfx.world.forceSightBubble;
        else if (cg_shieldStyle.integer == 3)
            legs.customShader = media.gfx.world.forceShell;
        else if (cg_shieldStyle.integer == 4)
            legs.customShader = media.gfx.world.sightShell;
        else if (cg_shieldStyle.integer == 5)
            legs.customShader = media.gfx.world.halfShield;
        else if (cg_shieldStyle.integer == 6)
    ...
    if (cg.snap->ps.duelInProgress && cent->currentState.number != cg.snap->ps.clientNum) {
    ...
    } else if ((cent->currentState.bolt1 && !(cent->currentState.eFlags & EF_DEAD) && cent->currentState.number != cg.snap->ps.clientNum &&
                (!cg.snap->ps.duelInProgress || cg.snap->ps.duelIndex != cent->currentState.number)) ||
               cg_duelShaderTest.integer) {
        ...
        if (!Q_stricmp(cg_duelShaderStyle.string, "none"))
            legs.customShader = 0;
        else if (cg_duelShaderStyle.integer == 1)
            legs.customShader = media.gfx.world.protect;
        else if (cg_duelShaderStyle.integer == 2)
            legs.customShader = media.gfx.world.playerShieldDamage;
        else if (cg_duelShaderStyle.integer == 3)
            legs.customShader = media.gfx.world.forceShell;
        else if (cg_duelShaderStyle.integer == 4)
            legs.customShader = media.gfx.world.sightShell;
        else if (cg_duelShaderStyle.integer == 5)
            legs.customShader = media.gfx.world.halfShield;
        else if (cg_duelShaderStyle.integer == 6)
            legs.customShader = media.gfx.world.cloaked;
        else
            legs.customShader = media.gfx.world.forceSightBubble;
    ...
    if (cent->currentState.eFlags & EF_DISINTEGRATION) {
        ...
        if (!cent->noLumbar) {
            trap->G2API_SetBoneAnim(legs.ghoul2, 0, "lower_lumbar", cent->miscTime, cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->miscTime,
                                    -1);
    ...
    if (!(cent->currentState.powerups & (1 << PW_CLOAKED))) {
        ...
        if (cg_forceModel.integer && cent->currentState.number != cg.clientNum && cent->currentState.eType == ET_PLAYER) {
    ...
    if ((cent->currentState.forcePowersActive & (1 << FP_RAGE)) &&
        (cg.renderingThirdPerson || cent->currentState.number != cg.snap->ps.clientNum || cg_trueGuns.integer || cg.predictedPlayerState.weapon == WP_SABER ||
    ...
    if (CG_VehicleShouldDrawShields(cent) || (checkDroidShields && CG_VehicleShouldDrawShields(&cg_entities[cent->currentState.m_iVehicleNum]))) {
...
void CG_ResetPlayerEntity(centity_t *cent) {
    ...
    if (cent->currentState.eType == ET_NPC) {
        if (cent->currentState.NPC_class == CLASS_VEHICLE && cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
            cg.predictedPlayerState.m_iVehicleNum && cent->currentState.number == cg.predictedPlayerState.m_iVehicleNum) { // holy hackery, batman!
            return;
        }
    ...
    if (cent->currentState.number != cg.predictedPlayerState.clientNum && cent->currentState.weapon == WP_SABER && cent->weapon != cent->currentState.weapon) {
        cent->weapon = cent->currentState.weapon;
