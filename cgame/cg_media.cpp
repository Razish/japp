...
static const size_t numSounds = ARRAY_LEN(sounds), numGraphics = ARRAY_LEN(gfx), numEffects = ARRAY_LEN(efx), numModels = ARRAY_LEN(models);
...
static void CG_RegisterGraphics(void) {
    ...
    if (cg_drawRewards.integer) {
        if (!(media.gfx.interface.medals.assist && media.gfx.interface.medals.capture && media.gfx.interface.medals.defend &&
              media.gfx.interface.medals.excellent && media.gfx.interface.medals.gauntlet && media.gfx.interface.medals.impressive)) {
...
static void CG_RegisterModels(void) {
    ...
    for (i = 1; i < numBSPInstances + 1; i++) {
        ...
        Com_sprintf(loadingStr, sizeof(loadingStr), "Loading BSP instance %i/%i: %s", (int)i, (int)numBSPInstances, bspName + 1);
