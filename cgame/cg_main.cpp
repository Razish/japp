...
qboolean CG_NoUseableForce(void) {
    ...
    while (i < NUM_FORCE_POWERS) {
        if (i != FP_SABERTHROW && i != FP_SABER_OFFENSE && i != FP_SABER_DEFENSE && i != FP_LEVITATION) { // valid selectable power
...
static void C_G2Mark(void) {
    ...
    if (trace.entityNum < ENTITYNUM_WORLD && cg_entities[trace.entityNum].ghoul2) { // hit someone with a ghoul2 instance, let's project the decal on them then.
        centity_t *cent = &cg_entities[trace.entityNum];

        // CG_TestLine(tr.endpos, end, 2000, 0xFF0000u, 1);

        CG_AddGhoul2Mark(td->shader, td->size, &trace.endpos, &end, trace.entityNum, &cent->lerpOrigin, cent->lerpAngles.yaw, cent->ghoul2, &cent->modelScale,
                         Q_irand(2000, 4000));
...
static void C_ImpactMark(void) {
    ...
    */
    CG_ImpactMark(data->mHandle, &data->mPoint, &data->mAngle, data->mRotation, data->mRed, data->mGreen, data->mBlue, data->mAlphaStart, qtrue,
                  data->mSizeStart, qfalse);
}
...
static void CVU_SVRunning(void) { cgs.localServer = sv_running.integer; }
...
static void CVU_ChatboxTabs(void) { CG_ChatboxSelect("normal"); }
...
int BG_GetTime(void) { return cg.time; }

// a global weather effect (rain, snow, etc)
void CG_ParseWeatherEffect(const char *str) { trap->R_WorldEffectCommand(str + 1); }
...
static qboolean CG_OwnerDrawHandleKey(int ownerDraw, uint32_t flags, float *special, int key) { return qfalse; }
...
static const char *CG_FeederItemText(int feederID, int index, int column, qhandle_t *handle1, qhandle_t *handle2, qhandle_t *handle3) {
    const gitem_t *item;
...
static qhandle_t CG_FeederItemImage(int feederID, int index) { return NULL_HANDLE; }
...
void CG_Text_PaintWithCursor(float x, float y, float scale, const vector4 *color, const char *text, int cursorPos, char cursor, int limit, uiTextStyle_e style,
...
static int CG_PlayCinematic(const char *name, float x, float y, float w, float h) { return trap->CIN_PlayCinematic(name, x, y, w, h, CIN_loop); }

static void CG_StopCinematic(int handle) { trap->CIN_StopCinematic(handle); }
...
static void CG_RunCinematicFrame(int handle) { trap->CIN_RunCinematic(handle); }
...
void CG_StrPool_Reset(void) { cg_strPoolSize = 0; }
...
static const BG_field_t cg_spawnFields[] = {
    {"angle", CGFOFS(angle), F_FLOAT},           {"angles", CGFOFS(angles), F_VECTOR},    {"classname", CGFOFS(classname), F_LSTRING},
    {"fogstart", CGFOFS(fogstart), F_FLOAT},     {"maxs", CGFOFS(maxs), F_VECTOR},        {"mins", CGFOFS(mins), F_VECTOR},
    {"model", CGFOFS(model), F_LSTRING},         {"modelscale", CGFOFS(fScale), F_FLOAT}, {"modelscale_vec", CGFOFS(scale), F_VECTOR},
    {"onlyfoghere", CGFOFS(onlyFogHere), F_INT}, {"origin", CGFOFS(origin), F_VECTOR},    {"radarrange", CGFOFS(radarrange), F_FLOAT},
...
void CG_CreateBrushEntData(cgSpawnEnt_t *ent) { trap->R_ModelBounds(trap->R_RegisterModel(ent->model), &ent->mins, &ent->maxs); }
...
void CG_CreateModelFromSpawnEnt(cgSpawnEnt_t *ent) {
    ...
    if (NumMiscEnts >= MAX_MISC_ENTS) {
        Com_Error(ERR_DROP,
                  "Too many misc_model_static's on level, ask a programmer to raise the limit (currently " XSTRING(MAX_MISC_ENTS) "), or take some out.");
        return;
    }
...
void CG_SpawnCGameEntFromVars(void) {
    ...
    if (ent.classname &&
        ent.classname[0]) { // we'll just stricmp this bastard, since there aren't all that many cgame-only things, and they all have special handling
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum, qboolean demoPlayback) {
    ...
    // logging
    if (cg_logConsole.integer)
        CG_OpenLog(LOG_DIRECTORY "console.log", &cg.log.console, cg_logConsole.integer == 2);
    else
        trap->Print("Not logging console to disk.\n");
    if (cg_logChat.integer)
        CG_OpenLog(LOG_DIRECTORY "chat.log", &cg.log.chat, cg_logChat.integer == 2);
    else
        trap->Print("Not logging chat to disk.\n");
    if (cg_logDebug.integer)
        CG_OpenLog(LOG_DIRECTORY "debug.log", &cg.log.debug, cg_logDebug.integer == 2);
    else
        trap->Print("Not logging debug messages to disk.\n");
    if (cg_logSecurity.integer)
        CG_OpenLog(LOG_DIRECTORY "security.log", &cg.log.security, cg_logSecurity.integer == 2);
    else
...
#define JP_TIMESTAMP_REGISTRY_NAME "sTimeFormat"
        ...
        if ((error = RegOpenKeyExA((HKEY)HKEY_CURRENT_USER, (LPCSTR)JP_TIMESTAMP_REGISTRY_KEY, (DWORD)0, (REGSAM)KEY_QUERY_VALUE, &hkey)) == ERROR_SUCCESS) {
            if ((error = RegQueryValueExA((HKEY)hkey, (LPCSTR)JP_TIMESTAMP_REGISTRY_NAME, (LPDWORD)NULL, (LPDWORD)&datatype, (LPBYTE)registryValue,
                                          (LPDWORD)&datalen)) == ERROR_SUCCESS) {
...
qboolean CG_IsSpectating(void) {
    ...
    if (CG_IsFollowing() || cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR || cg.predictedPlayerState.pm_type == PM_SPECTATOR) {
        return qtrue;
    }
...
static void CG_GetOrigin(int entID, vector3 *out) { VectorCopy(&cg_entities[entID].currentState.pos.trBase, out); }

static void CG_GetAngles(int entID, vector3 *out) { VectorCopy(&cg_entities[entID].currentState.apos.trBase, out); }

static trajectory_t *CG_GetOriginTrajectory(int entID) { return &cg_entities[entID].nextState.pos; }

static trajectory_t *CG_GetAngleTrajectory(int entID) { return &cg_entities[entID].nextState.apos; }

static void _CG_ROFF_NotetrackCallback(int entID, const char *notetrack) { CG_ROFF_NotetrackCallback(&cg_entities[entID], notetrack); }
...
static void CG_AutomapInput(void) {
...
#endif
    {
        if (cg_autoMapInput.yaw)
            cg.automapAngle.yaw += cg_autoMapInput.yaw;
        if (cg_autoMapInput.pitch)
            cg.automapAngle.pitch += cg_autoMapInput.pitch;
