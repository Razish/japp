...
static void DrawServerInfo(float fade) {
    ...
    case GT_FFA:
        ...
        trap->R_Font_DrawString(SCREEN_WIDTH / 2.0f - fontLarge.Width(tmp) / 2.0f, y, tmp, &colour, fontLarge.handle | STYLE_DROPSHADOW, -1, fontLarge.scale);
        break;
    ...
    case GT_TEAM:
        if (cgs.scores1 == cgs.scores2) {
            ...
            trap->R_Font_DrawString(SCREEN_WIDTH / 2.0f - fontLarge.Width(tmp) / 2.0f, y, tmp, &colour, fontLarge.handle | STYLE_DROPSHADOW, -1,
                                    fontLarge.scale);
            y += lineHeightBig;
...
static void DrawPlayerCount_Team(float fade) {
    ...
    tmp = va("%2i spectators", specCount);
    trap->R_Font_DrawString(width - trap->R_Font_StrLenPixels(tmp, font.handle, font.scale) / 2.0f, y + lineHeight * 20, tmp, &colour, font.handle, -1,
                            font.scale);
}
...
static int ListPlayers_FFA(float fade, float x, float y, float fontScale, int fontHandle, int startIndex, int playerCount) {
    const char *tmp = NULL;
    vector4 white = {1.0f, 1.0f, 1.0f, 1.0f}, background = {0.75f, 0.75f, 0.75f, 1.0f}, blue = {0.6f, 0.6f, 1.0f, 1.0f};
    ...
    tmp = "Name";
    font.Paint(x - (font.Width(tmp) / 2.0f) + columnOffset[column++], y, tmp, &white, uiTextStyle_e::ShadowedMore);
            ...
            */
            ...
            if (score->ping != -1) {
                if (ci->botSkill != -1) {
                    ...
                    tmp = va("%i", score->ping);
                    font.Paint(x - (font.Width(tmp) / 2.0f) + columnOffset[column++], y, tmp, &pingColour, uiTextStyle_e::ShadowedMore);
                }
        ...
        if (ci->team == team) {
            ...
            if (score->ping != -1) {
                if (ci->botSkill != -1) {
                    ...
                    // Ping
                    tmp = va("%i", score->ping);
                    font.Paint(x + columnOffset[column++] - font.Width(tmp) / 2.0f, y, tmp, &pingColour, uiTextStyle_e::Shadowed);
                }
...
static int ListPlayers_Free(float fade, float x, float y, float fontScale, int fontHandle, float lineHeight, int startIndex, int playerCount) {
    switch (cgs.gametype) {
    case GT_FFA:
...
static void DrawSpectators(float fade) {
    ...
    if (cg.scoreboard.spectatorLen) {
        ...
        font.Paint(cg.scoreboard.spectatorX, (y + lineHeight * 20) - 3, cg.scoreboard.spectatorList, &white, uiTextStyle_e::Shadowed);
    }
}
