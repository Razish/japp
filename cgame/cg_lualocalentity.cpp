...
extern void CG_FreeLocalEntity(localEntity_t *le);
extern std::deque<localEntity_t*> cg_localEntities;
namespace JPLua{
	static const char LOCALENTITY_META[] = "LocalEntity.meta";

	void LocalEntity_CreateRef(lua_State *L, localEntity_t *ent) {
		jplua_localentity_t *data = NULL;
		if (ent){ /// : dddd
			data = (jplua_localentity_t *)lua_newuserdata(L, sizeof(jplua_localentity_t));
			data->id = ent->id;
		}
	...
	static int LocalEntity_Equals(lua_State *L) {
		const localEntity_t*e1 = CheckLocalEntity(L, 1), *e2 = CheckLocalEntity(L, 2);
	...
	static void LocalEntity_SetType(lua_State *L, localEntity_t *ent){
		ent->leType = (leType_t)luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetFlags(lua_State *L, localEntity_t *ent){
		ent->leFlags = luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetLifeTime(lua_State *L, localEntity_t *ent){
		ent->lifeRate = luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetFadeInTime(lua_State *L, localEntity_t *ent){
		ent->fadeInTime = luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetBFactor(lua_State *L, localEntity_t *ent){
		ent->bounceFactor = luaL_checknumber(L, 3);
	}
	...
	static void LocalEntity_SetBSound(lua_State *L, localEntity_t *ent){
		ent->bounceSound = luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetAlpha(lua_State *L, localEntity_t *ent){
		ent->alpha = luaL_checknumber(L, 3);
	}
	...
	static void LocalEntity_SetDAlpha(lua_State *L, localEntity_t *ent){
		ent->dalpha = luaL_checknumber(L, 3);
	}
	...
	static void LocalEntity_SetForceAlpha(lua_State *L, localEntity_t *ent){
		ent->forceAlpha = luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetColor(lua_State *L, localEntity_t *ent){
		ReadFloats(ent->color, 4, L, 3);
	}

	static int LocalEntity_GetColor(lua_State *L, localEntity_t *ent){
		lua_newtable(L);
		int top = lua_gettop(L);
		lua_pushnumber(L, ent->color[0]); lua_settable(L, top);
		lua_pushnumber(L, ent->color[1]); lua_settable(L, top);
		lua_pushnumber(L, ent->color[2]); lua_settable(L, top);
		lua_pushnumber(L, ent->color[3]); lua_settable(L, top);
		return 1;
	}

	static void LocalEntity_SetRadius(lua_State *L, localEntity_t *ent){
		ent->radius = luaL_checknumber(L, 3);
	}
	...
	static void LocalEntity_SetLight(lua_State *L, localEntity_t *ent){
		ent->light = luaL_checknumber(L, 3);
	}
	...
	static void LocalEntity_SetMarkType(lua_State *L, localEntity_t *ent){
		ent->leMarkType = (leMarkType_t)luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_SetBSndType(lua_State *L, localEntity_t *ent){
		ent->leBounceSoundType = (leBounceSoundType_t)luaL_checkinteger(L, 3);
	}
	...
	static void LocalEntity_Line_SetWidth(lua_State *L, localEntity_t *ent){
		ent->data.line.width = luaL_checknumber(L, 3);
	}

	static void LocalEntity_Line_SetDWidth(lua_State *L, localEntity_t *ent){
		ent->data.line.dwidth = luaL_checknumber(L, 3);
	}
	...
	static const le_prop_t localEntityProperties[] = {
		{
			"alpha",
			LocalEntity_GetAlpha,
			LocalEntity_SetAlpha
		},
		{
			"bounceFactor",
			LocalEntity_GetBFactor,
			LocalEntity_SetBFactor
		},
		{
			"bounceSound",
			LocalEntity_GetBSound,
			LocalEntity_SetBSound
		},
		{
			"bounceSoundType",
			LocalEntity_GetBSndType,
			LocalEntity_SetBSndType
		},
		{
			"color",
			LocalEntity_GetColor,
			LocalEntity_SetColor
		},
		{
			"dalpha",
			LocalEntity_GetDAlpha,
			LocalEntity_SetDAlpha
		},
		{
			"fadeintime",
			LocalEntity_GetFadeInTime,
			LocalEntity_SetFadeInTime
		},
		{
			"flags",
			LocalEntity_GetFlags,
			LocalEntity_SetFlags
		},
		{
			"forcealpha",
			LocalEntity_GetForceAlpha,
			LocalEntity_SetForceAlpha
		},
		{
			"lifetime",
	...
	static int LocalEntity_Index(lua_State *L) {
		...
		// assume it's a field
		const le_prop_t *property = (le_prop_t *)bsearch(
			key,
	...
	static const struct luaL_Reg localentity_meta[] = {
		{ "__index", LocalEntity_Index },
		{ "__newindex", LocalEntity_NewIndex },
		{ "__eq", LocalEntity_Equals },
		{ "__tostring", LocalEntity_ToString },
		{ "Free", LocalEntity_Free },
		{ NULL, NULL }
	};
	...
	void Register_LocalEntity(lua_State *L) {
		...
		for (r = localentity_meta; r->name; r++) {
