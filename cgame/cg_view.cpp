...
refdef_t *CG_GetRefdef(void) { return &cg.refdef[cg.currentRefdef]; }
...
//	These are bound to F5, F6, F7, and F8 in q3default.cfg.
// If a gun is being tested, the "gun_x", "gun_y", and "gun_z" variables will let you adjust the positioning.
// Note that none of the model testing features update while the game is paused, so it may be convenient to test with
//	deathmatch set to 1 so that bringing down the console doesn't pause the game.
...
static void CG_CalcIdealThirdPersonViewTarget(int clientNum) {
    ...
    cameraFocusLoc[clientNum].z += cg.predictedPlayerState.viewheight;
    ...
    if (cg.snap && cg.snap->ps.m_iVehicleNum) {
        ...
        } else if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo && veh->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL)
            vertOffset = 0;
    }
...
static void CG_CalcIdealThirdPersonViewLocation(int clientNum) {
    ...
    if (cg.snap && (cg.snap->ps.eFlags2 & EF2_HELD_BY_MONSTER) && cg.snap->ps.hasLookTarget &&
        cg_entities[cg.snap->ps.lookTarget].currentState.NPC_class == CLASS_RANCOR) {
        // stay back
        thirdPersonRange = 120.0f; // 180.0f
    }
...
static void CG_UpdateThirdPersonTargetDamp(int clientNum) {
    ...
    // Note that previously there was an upper limit to the number of physics traces that are done through the world for
    //	the sake of camera collision, since it wasn't calced per frame.  Now it is calculated every frame.
    // This has the benefit that the camera is a lot smoother now (before it lerped between tested points), however two
...
static void CG_UpdateThirdPersonCameraDamp(int clientNum) {
    ...
    if (cg.predictedVehicleState.hyperSpaceTime && (cg.time - cg.predictedVehicleState.hyperSpaceTime) < HYPERSPACE_TIME)
        ...
        if (cameraStiffFactor[clientNum] > 0.0f)
            dampfactor += (1.0f - dampfactor) * cameraStiffFactor[clientNum];
    }
    ...
    if (dampfactor >= 1.0f || cg.thisFrameTeleport)
        ...
        dtime = (float)(cg.time - cameraLastFrame[clientNum]) *
                (1.0f / (float)CAMERA_DAMP_INTERVAL); // Our dampfactor is geared towards a time interval equal to "1".
    ...
    if (trace.fraction < 1.0f) {
        if (trace.entityNum < ENTITYNUM_WORLD && cg_entities[trace.entityNum].currentState.solid == SOLID_BMODEL &&
            cg_entities[trace.entityNum].currentState.eType == ET_MOVER) { // get a different position for movers -rww
            centity_t *mover = &cg_entities[trace.entityNum];

            // this is absolutely hackiful, since we calc view values before we add packet ents and lerp, if we hit a
            //	mover we want to update its lerp pos and force it when we do the trace against it.
            if (mover->currentState.pos.trType != TR_STATIONARY && mover->currentState.pos.trType != TR_LINEAR) {
                ...
                // force the calc'd lerp to be the base and say we are stationary so we don't try to extrapolate out further.
...
static void CG_OffsetThirdPersonView(int clientNum) {
    ...
    if (cg.snap && (cg.snap->ps.eFlags2 & EF2_HELD_BY_MONSTER) && cg.snap->ps.hasLookTarget &&
        cg_entities[cg.snap->ps.lookTarget].currentState.NPC_class == CLASS_RANCOR) { // being held
        ...
        if (cg.predictedPlayerState.m_iVehicleNum // in a vehicle
            && BG_UnrestrainedPitchRoll(&cg.predictedPlayerState,
                                        cg_entities[cg.predictedPlayerState.m_iVehicleNum].m_pVehicle)) // can roll/pitch without restriction
        {
            float pitchPerc = ((90.0f - fabsf(cameraFocusAngles[clientNum].roll)) / 90.0f);
    ...
    if (cameraLastFrame[clientNum] == 0 || cameraLastFrame[clientNum] > cg.time)
        ...
        deltayaw = fabsf(cameraFocusAngles[clientNum].yaw - cameraLastYaw[clientNum]);
        ...
        if (cameraStiffFactor[clientNum] < 1.0f)
            cameraStiffFactor[clientNum] = 0.0f;
...
static void CG_OffsetFirstPersonView(void) {
    vector3 *origin, *angles, predictedVelocity;
...
#if 0
	{
		const float neckLength = 8.0f;
		vector3			forward, up;

		refdef->vieworg[2] -= neckLength;
		AngleVectors( refdef->viewangles, forward, NULL, up );
		VectorMA( refdef->vieworg, 3, forward, refdef->vieworg );
		VectorMA( refdef->vieworg, neckLength, up, refdef->vieworg );
	}
#endif
}
...
qboolean CG_CalcFOVFromX(float fov_x) {
    ...
    //	slime/water etc properties to a func_door area brush in order to move the whole water level up/down this doesn't
    //	take into account the door position, so warps the view the whole time whether the water is up or not.
    // Fortunately there's only one slime area in Trek that you can be under, so lose it...
#if 0
	// warp if underwater
...
static int CG_CalcFov(void) {
    float x, phase, v, fov_x, fov_y, f, cgFov;
    qboolean inwater;
    refdef_t *refdef = CG_GetRefdef();
    ...
    if (cg.predictedPlayerState.zoomMode)
        ...
        cg.zoomSensitivity = refdef->fov_y / 75.0f;
...
static int cg_actionCamLastTime = 0;
static vector3 cg_actionCamLastPos[MAX_CLIENTS];
...
static vector3 cg_lastTurretViewAngles = {0};
qboolean CG_CheckPassengerTurretView(void) {
    ...
    if (cg.predictedPlayerState.m_iVehicleNum && cg.predictedPlayerState.generic1) {
        ...
        if (vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo && vehCent->m_pVehicle->m_pVehicleInfo->maxPassengers) {
                ...
                if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum].iAmmoMax) {
                    ...
                    if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum].passengerNum == cg.predictedPlayerState.generic1) {
                        ...
                        if (boltIndex != -1) {
                            ...
                            vector3 fwd, up, newAngles, deltaAngles;
                            trap->G2API_GetBoltMatrix_NoRecNoRot(vehCent->ghoul2, 0, boltIndex, &boltMatrix, &vehCent->lerpAngles, &vehCent->lerpOrigin,
                                                                 cg.time, NULL, &vehCent->modelScale);
...
static int CG_CalcViewValues(int clientNum) {
    ...
    if (!manningTurret) {
        ...
        if (cg.predictedPlayerState.m_iVehicleNum) {
            ...
            else if (pVeh && pVeh->m_pVehicleInfo && pVeh->m_pVehicleInfo->type == VH_FIGHTER) {
                // fighter
                VectorCopy(cg.predictedVehicleState.viewangles, refdef->viewangles);
    ...
    for (num = 0; num < ENTITYNUM_NONE; num++) {
        ...
        if (cent && (cent->currentState.eFlags & EF_SOUNDTRACKER) && cent->currentState.number == num) {
            ...
            if (cg.snap && (signed)cent->currentState.trickedEntIndex[0] == cg.snap->ps.clientNum) {
                ...
                trap->S_UpdateEntityPosition(cent->currentState.number, &cg_entities[cent->currentState.trickedEntIndex[0]].lerpOrigin);
            }
        }
...
void CG_DoCameraShake(vector3 *origin, float intensity, int radius, int time) {
    ...
    // FIXME: When exactly is the vieworg calculated in relation to the rest of the frames?
...
static void CG_AddRefentForAutoMap(centity_t *cent) {
    ...
    if (cent->ghoul2 && (cent->currentState.eType == ET_PLAYER || cent->currentState.eType == ET_NPC || cent->currentState.modelGhoul2)) {
        ...
        if (!ent.radius)
            ent.radius = 64.0f;
    } else // then assume a standard indexed model
...
void CG_DrawAutoMap(void) {
    ...
    refdef_t refdef;
    trace_t tr;
    vector3 fwd, playerMins, playerMaxs;
    int vWidth, vHeight;
    ...
    if ((cg.predictedPlayerState.m_iVehicleNum && cg_entities[cg.predictedPlayerState.m_iVehicleNum].currentState.eType == ET_NPC &&
...
void CG_DrawAltView(int clientNum) {
    ...
    altViewX += 160;
    if (altViewX > SCREEN_WIDTH) {
        altViewX = 0;
        altViewY += 160;
        if (altViewY > SCREEN_HEIGHT)
            altViewY = 0;
    }
}
#endif
...
static qboolean addIdealVectors(void) {
    ...
    cosLeftAngle = cosf(idealAngleToLeft);
    sinLeftAngle = sinf(idealAngleToLeft);

    cosRightAngle = cosf(idealAngleToRight);
    sinRightAngle = sinf(idealAngleToRight);
...
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView, qboolean demoPlayback) {
    ...
    qboolean bUseFighterPitch = qfalse, isFighter = qfalse;
#endif
    //	int i;
...
#ifdef VEH_CONTROL_SCHEME_4
    else if (cg.predictedPlayerState.m_iVehicleNum && !cg.predictedPlayerState.generic1) {
        // not a passenger
        centity_t *cent = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
        if (cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo && cent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) {
            // fighter
            ...
            trap->SetUserCmdValue(cg.weaponSelect, mSensitivity, mPitchOverride, mYawOverride, mSensitivityOverride, cg.forceSelect, cg.itemSelect,
                                  bUseFighterPitch);
            isFighter = qtrue;
        }
    }
    ...
    if (cg.snap->ps.stats[STAT_HEALTH] > 0) {
        ...
        if (cg_trueInvertSaber.integer == 2 && (cg.predictedPlayerState.weapon == WP_SABER || cg.predictedPlayerState.weapon == WP_MELEE))
            ...
            cg.renderingThirdPerson = qtrue;
                 ...
                 (cg_trueInvertSaber.integer == 1 && !cg_thirdPerson.integer &&
