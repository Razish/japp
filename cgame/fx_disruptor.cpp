...
void FX_DisruptorMainShot(vector3 *start, vector3 *end) {
    trap->FX_AddLine(start, end, 0.1f, 6.0f, 0.0f, 1.0f, 0.0f, 0.0f, &WHITE, &WHITE, 0.0f, 150, trap->R_RegisterShaderNoMip("gfx/effects/redLine"),
...
void FX_DisruptorAltShot(vector3 *start, vector3 *end, qboolean fullCharge) {
    trap->FX_AddLine(start, end, 0.1f, 10.0f, 0.0f, 1.0f, 0.0f, 0.0f, &WHITE, &WHITE, 0.0f, 175, trap->R_RegisterShaderNoMip("gfx/effects/redLine"),
                     FX_SIZE_LINEAR | FX_ALPHA_LINEAR);
    ...
    if (fullCharge) {
        ...
        // add some beef
        trap->FX_AddLine(start, end, 0.1f, 7.0f, 0.0f, 1.0f, 0.0f, 0.0f, &YELLER, &YELLER, 0.0f, 150, trap->R_RegisterShaderNoMip("gfx/misc/whiteline2"),
                         FX_SIZE_LINEAR | FX_ALPHA_LINEAR);
    }
}
...
void FX_DisruptorAltHit(vector3 *origin, vector3 *normal) { trap->FX_PlayEffectID(media.efx.disruptor.altHit, origin, normal, -1, -1, qfalse); }

void FX_DisruptorHitWall(vector3 *origin, vector3 *normal) { trap->FX_PlayEffectID(media.efx.disruptor.wallImpact, origin, normal, -1, -1, qfalse); }
