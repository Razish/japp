...
saberStyleItem_t saberStyleItems[] = {{"None", &colorTable[CT_WHITE]},  {"Fast", &colorTable[CT_LTBLUE2]},      {"Medium", &colorTable[CT_YELLOW]},
                                      {"Strong", &colorTable[CT_RED]},  {"Desann", &colorTable[CT_HUD_ORANGE]}, {"Tavion", &colorTable[CT_LTPURPLE1]},
                                      {"Dual", &colorTable[CT_YELLOW]}, {"Staff", &colorTable[CT_YELLOW]}};

const char *showPowersName[] = {"HEAL2",           // FP_HEAL
                                "JUMP2",           // FP_LEVITATION
                                "SPEED2",          // FP_SPEED
                                "PUSH2",           // FP_PUSH
                                "PULL2",           // FP_PULL
...
int UI_ParseAnimationFile(const char *filename, animation_t *animset, qboolean isHumanoid) { return BG_ParseAnimationFile(filename, animset, isHumanoid); }
...
void CG_DrawHealth(menuDef_t *menuHUD) {
    ...
    for (i = (MAX_HUD_TICS - 1); i >= 0; i--) {
        ...
        CG_DrawPic(focusItem->window.rect.x * cgs.widthRatioCoef, focusItem->window.rect.y, focusItem->window.rect.w * cgs.widthRatioCoef,
                   focusItem->window.rect.h, focusItem->window.background);
    ...
    if (focusItem) {
        ...
        CG_DrawNumField(focusItem->window.rect.x * cgs.widthRatioCoef, focusItem->window.rect.y, 3, ps->stats[STAT_HEALTH],
                        focusItem->window.rect.w * cgs.widthRatioCoef, focusItem->window.rect.h, NUM_FONT_SMALL, qfalse);
    }
}
...
void CG_DrawArmor(menuDef_t *menuHUD) {
    ...
    if (focusItem) {
        ...
        CG_DrawNumField(focusItem->window.rect.x * cgs.widthRatioCoef, focusItem->window.rect.y, 3, ps->stats[STAT_ARMOR],
                        focusItem->window.rect.w * cgs.widthRatioCoef, focusItem->window.rect.h, NUM_FONT_SMALL, qfalse);
    }
...
static void CG_DrawSaberStyle(centity_t *cent, menuDef_t *menuHUD) {
    ...
    case 5: // FORCE_LEVEL_5://Tavion
        ...
        if (focusItem) {
            ...
            CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - focusItem->window.rect.x) * cgs.widthRatioCoef, focusItem->window.rect.y,
                       focusItem->window.rect.w * cgs.widthRatioCoef, focusItem->window.rect.h, focusItem->window.background);
        }
...
static void CG_DrawAmmo(centity_t *cent, menuDef_t *menuHUD) {
    ...
    if (weaponData[cent->currentState.weapon].shotCost == 0 && weaponData[cent->currentState.weapon].alt.shotCost == 0) { // just draw "infinite"
        inc = 8 / MAX_HUD_TICS;
        value = 8;
        ...
        if (focusItem) {
            ...
            font.Paint(SCREEN_WIDTH - (SCREEN_WIDTH - focusItem->window.rect.x) * cgs.widthRatioCoef, focusItem->window.rect.y, "--",
                       &focusItem->window.foreColor, uiTextStyle_e::Normal);
        }
    } else {
        ...
        if (focusItem) {
            ...
            CG_DrawNumField(SCREEN_WIDTH - (SCREEN_WIDTH - focusItem->window.rect.x) * cgs.widthRatioCoef, focusItem->window.rect.y, 3, value,
                            focusItem->window.rect.w, focusItem->window.rect.h, NUM_FONT_SMALL, false);
        }
    }
...
void CG_DrawForcePower(menuDef_t *menuHUD) {
    ...
    if (focusItem) {
        ...
        CG_DrawNumField(SCREEN_WIDTH - (SCREEN_WIDTH - focusItem->window.rect.x) * cgs.widthRatioCoef, focusItem->window.rect.y, 3, cg.snap->ps.fd.forcePower,
                        focusItem->window.rect.w * cgs.widthRatioCoef, focusItem->window.rect.h, NUM_FONT_SMALL, qfalse);
    }
}
...
void JP_DrawMovementKeys(void) {
    ...
    Com_sprintf(str1, sizeof(str1),
                va("^%cv ^%cW ^%c^", (cmd.upmove < 0) ? COLOR_RED : COLOR_WHITE, (cmd.forwardmove > 0) ? COLOR_RED : COLOR_WHITE,
                   (cmd.upmove > 0) ? COLOR_RED : COLOR_WHITE));
    Com_sprintf(str2, sizeof(str2),
                va("^%cA ^%cS ^%cD", (cmd.rightmove < 0) ? COLOR_RED : COLOR_WHITE, (cmd.forwardmove < 0) ? COLOR_RED : COLOR_WHITE,
                   (cmd.rightmove > 0) ? COLOR_RED : COLOR_WHITE));

    font.Paint(cg.moveKeysPos.x - std::max(w1, w2) / 2.0f, cg.moveKeysPos.y, str1, &colorWhite, uiTextStyle_e::Outlined);
    font.Paint(cg.moveKeysPos.x - std::max(w1, w2) / 2.0f, cg.moveKeysPos.y + height, str2, &colorWhite, uiTextStyle_e::Outlined);
}
    ...
    if (!cg_accelerometer.integer)
        ...
        */
        if (percent && percent < 100)
            CG_FillRect(cg.accelerometer.position.x, cg.accelerometer.position.y, cg.accelerometer.size.w * percent, cg.accelerometer.size.h,
                        &colorTable[CT_RED]);
        /*
...
static void CG_DrawFlagStatusQ3P(void) {
    ...
    if (CG_OtherTeamHasFlag()) {
        ...
        CG_DrawPic(leftX, y, iconSize * cgs.widthRatioCoef, iconSize, media.gfx.interface.defer);

        const char *s = "Flag stolen!";
        const float width = font.Width(s);
        const float height = font.Height(s);
        font.Paint(leftX + ((iconSize * cgs.widthRatioCoef) / 2.0f) - (width / 2.0f), y - height * 2.0f, s, &colorYellow, uiTextStyle_e::ShadowedMore);
    ...
    font.Paint(leftX + ((iconSize * cgs.widthRatioCoef) / 2.0f) - (scoreWidth / 2.0f), y - scoreHeight, scoreStr, scoreColour, uiTextStyle_e::Shadowed);
    trap->R_SetColor(NULL);
    ...
    font.Paint(rightX + ((iconSize * cgs.widthRatioCoef) / 2.0f) - (scoreWidth / 2.0f), y - scoreHeight, scoreStr, scoreColour, uiTextStyle_e::Shadowed);
    trap->R_SetColor(NULL);
}
...
static void CG_DrawFlagStatus(void) {
    ...
    if (CG_YourTeamHasFlag()) {
        CG_DrawPic(2 * cgs.widthRatioCoef, 330 - startDrawPos, flagSize * cgs.widthRatioCoef, flagSize, theirFlag);
        startDrawPos += flagSize + 2;
    }

    if (CG_OtherTeamHasFlag()) {
        CG_DrawPic(2 * cgs.widthRatioCoef, 330 - startDrawPos, flagSize * cgs.widthRatioCoef, flagSize, myFlag);
    }
}
...
const char *CG_GetScoreString(void) {
    ...
    if (cgs.gametype == GT_DUEL) {
        ...
        Com_sprintf(scoreStr, sizeof(scoreStr), "%s: %i/%i", CG_GetStringEdString("MP_INGAME", "SCORE"), cg.snap->ps.persistant[PERS_SCORE], cgs.fraglimit);
        ...
        Com_sprintf(scoreStr, sizeof(scoreStr), "%s: %i%s", CG_GetStringEdString("MP_INGAME", "SCORE"), cg.snap->ps.persistant[PERS_SCORE], scoreBiasStr);
    } else if (cgs.gametype == GT_CTY || cgs.gametype == GT_CTF) {
        Com_sprintf(scoreStr, sizeof(scoreStr), "%s: %i", CG_GetStringEdString("MP_INGAME", "SCORE"), cg.snap->ps.persistant[PERS_SCORE]);
    } else {
        const int net = cg.snap->ps.persistant[PERS_SCORE] - cg.snap->ps.persistant[PERS_KILLED];
        Com_sprintf(scoreStr, sizeof(scoreStr), "Net: %c%i", (net >= 0) ? '+' : '-', abs(net));
    }
...
void CG_DrawHUD(centity_t *cent) {
    ...
    if (cg_debugInfo.integer) {
        ...
        str = va(S_COLOR_CYAN "saberMove: %i\n" S_COLOR_CYAN "legsAnim: %i\n" S_COLOR_CYAN "torsoAnim: %i\n" S_COLOR_CYAN "legsTimer: %i\n" S_COLOR_CYAN
                 ...
                 cg.forceSelect, cg.time, cg.haveDeferredPlayers,

                 !!(cg.snap->ps.eFlags & EF_ALT_DIM), cgs.japp.jp_cinfo, !!cg_smartEntities.integer);
    ...
    if (hudEvents & HUDEVENT_STATS) {
        if (cg_hudFiles.integer == 1) {
            ...
            str = va("%s", ammoString);
            font.Paint(SCREEN_WIDTH - (weapX + 16 + 32), y + 40, str, &colorTable[CT_HUD_ORANGE], uiTextStyle_e::Shadowed);
            str = va("%i", cg.snap->ps.fd.forcePower);
            font.Paint(SCREEN_WIDTH - (x + 18 + 14 + 32), y + 40 + 14, str, &colorTable[CT_ICON_BLUE], uiTextStyle_e::Shadowed);
        } else {
            // draw the left HUD
                ...
                if (cgs.gametype != GT_POWERDUEL) {
                    if (hudEvents & HUDEVENT_SCORES) {
                        ...
                        if (focusItem) {
                            ...
                            // FIXME: right-aligned
                            font.Paint(SCREEN_WIDTH - (SCREEN_WIDTH - focusItem->window.rect.x) - textWidth, focusItem->window.rect.y, s,
                                       &focusItem->window.foreColor, uiTextStyle_e::Shadowed);
                        }
                    }
                }
...
void CG_DrawForceSelect(void) {
    ...
    for (iconCnt = 1; iconCnt < (sideLeftIconCnt + 1); i--) {
        ...
        if (media.gfx.interface.forcePowerIcons[forcePowerSorted[i]]) {
            CG_DrawPic(holdX, y + yOffset, smallIconSize * cgs.widthRatioCoef, smallIconSize, media.gfx.interface.forcePowerIcons[forcePowerSorted[i]]);
    ...
    if (ForcePower_Valid(cg.forceSelect)) {
        ...
        if (media.gfx.interface.forcePowerIcons[cg.forceSelect]) {
            ...
            CG_DrawPic(x - (bigIconSize / 2) * cgs.widthRatioCoef, (y - ((bigIconSize - smallIconSize) / 2)) + yOffset, bigIconSize * cgs.widthRatioCoef,
                       bigIconSize, media.gfx.interface.forcePowerIcons[cg.forceSelect]);
        }
    }
    ...
    if (showPowersName[cg.forceSelect]) {
        ...
        font.Paint((SCREEN_WIDTH / 2.0f) - (width / 2.0f), y + 30 + yOffset, str, &colorTable[CT_ICON_BLUE], uiTextStyle_e::Normal);
    }
}
...
void CG_DrawInvenSelect(void) {
    ...
    if (!count) {
        ...
        const float width = font.Width(str);
        font.Paint((SCREEN_WIDTH / 2.0f) - (width / 2.0f), y2 + 22, str, &colorTable[CT_ICON_BLUE], uiTextStyle_e::Normal);
        return;
    }
    ...
    for (iconCnt = 0; iconCnt < sideLeftIconCnt; i--) {
        ...
        if (media.gfx.interface.invenIcons[i]) {
            trap->R_SetColor(NULL);
            CG_DrawPic(holdX, y + 10, smallIconSize * cgs.widthRatioCoef, smallIconSize, media.gfx.interface.invenIcons[i]);
    ...
    if (media.gfx.interface.invenIcons[cg.itemSelect] && BG_IsItemSelectable(&cg.predictedPlayerState, cg.itemSelect)) {
        int itemNdex;
        trap->R_SetColor(NULL);
        CG_DrawPic(x - (bigIconSize / 2) * cgs.widthRatioCoef, (y - ((bigIconSize - smallIconSize) / 2)) + 10, bigIconSize * cgs.widthRatioCoef, bigIconSize,
                   media.gfx.interface.invenIcons[cg.itemSelect]);
        trap->R_SetColor(&colorTable[CT_ICON_BLUE]);
        /*
...
qboolean CG_CheckTargetVehicle(centity_t **pTargetVeh, float *alpha) {
    ...
    *alpha = 1.0f;
    ...
    } else if (cg.crosshairVehNum < ENTITYNUM_WORLD && cg.time - cg.crosshairVehTime < 3000) { // crosshair was on a vehicle in the last 3 seconds
    ...
    if (targetNum < ENTITYNUM_WORLD && targetNum >= MAX_CLIENTS) {
        //	centity_t *targetVeh = &cg_entities[targetNum];
        targetVeh = &cg_entities[targetNum];
        if (targetVeh->currentState.NPC_class == CLASS_VEHICLE && targetVeh->m_pVehicle && targetVeh->m_pVehicle->m_pVehicleInfo &&
...
float CG_DrawVehicleShields(const menuDef_t *menuHUD, const centity_t *veh) {
    ...
    if (item) {
        ...
        CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w, item->window.rect.h, item->window.background);
    }
...
void CG_DrawVehicleAmmo(const menuDef_t *menuHUD, const centity_t *veh) {
    ...
    for (int i = 1; i <= MAX_VHUD_AMMO_TICS; i++) {
        ...
        if (cg_vehicleAmmoWarningTime > cg.time && cg_vehicleAmmoWarning == 0) {
...
void CG_DrawVehicleAmmoLower(const menuDef_t *menuHUD, const centity_t *veh) {
    ...
    for (i = 1; i < MAX_VHUD_AMMO_TICS; i++) {
        ...
        if (cg_vehicleAmmoWarningTime > cg.time && cg_vehicleAmmoWarning == 1) {
            memcpy(&calcColor, &g_color_table[ColorIndex(COLOR_RED)], sizeof(vector4));
            calcColor.a = sinf(cg.time * 0.005f) * 0.5f + 0.5f;
        } else {
            memcpy(&calcColor, &item->window.foreColor, sizeof(vector4));
...
void CG_DrawVehicleTurboRecharge(const menuDef_t *menuHUD, const centity_t *veh) {
    ...
    if (item) {
        ...
        CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w, height, media.gfx.world.whiteShader);
    }
}
...
void CG_DrawVehicleWeaponsLinked(const menuDef_t *menuHUD, const centity_t *veh) {
    ...
    if (drawLink) {
        ...
        if (item) {
            ...
            CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w, item->window.rect.h, media.gfx.world.whiteShader);
        }
    }
}
    ...
    if (item) {
        ...
        if (graphicHandle) {
            if (item->window.rect.x <= SCREEN_WIDTH / 2) {
                CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h, graphicHandle);
            } else {
                CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y,
                           item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h, graphicHandle);
            }
        }
    }
}
...
void CG_DrawVehicleDamageHUD(const centity_t *veh, int brokenLimbs, float percShields, const char *menuName, float alpha) {
    ...
    if (item) {
        if (veh->m_pVehicle->m_pVehicleInfo->dmgIndicBackgroundHandle) {
            ...
            if (item->window.rect.x <= SCREEN_WIDTH / 2) {
                CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h,
                           veh->m_pVehicle->m_pVehicleInfo->dmgIndicBackgroundHandle);
            } else {
                CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y,
                           item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h, veh->m_pVehicle->m_pVehicleInfo->dmgIndicBackgroundHandle);
            }
        }
    }
    ...
    if (item) {
        if (veh->m_pVehicle->m_pVehicleInfo->dmgIndicFrameHandle) {
            ...
            if (item->window.rect.x <= SCREEN_WIDTH / 2) {
                ...
                CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y,
                           item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h, veh->m_pVehicle->m_pVehicleInfo->dmgIndicFrameHandle);
            }
        }
    }
    ...
    if (item) {
        if (veh->m_pVehicle->m_pVehicleInfo->dmgIndicShieldHandle) {
            ...
            if (item->window.rect.x <= SCREEN_WIDTH / 2) {
                ...
                CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y,
                           item->window.rect.w * cgs.widthRatioCoef, item->window.rect.h, veh->m_pVehicle->m_pVehicleInfo->dmgIndicShieldHandle);
            }
        }
...
static void CG_DrawPickupItem(void) {
    ...
    if (value && cg_items[value].icon != -1) {
        ...
        if (fadeColor) {
            ...
            CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - 585) * cgs.widthRatioCoef, 320, ICON_SIZE * cgs.widthRatioCoef, ICON_SIZE, cg_items[value].icon);
            trap->R_SetColor(NULL);
        }
    }
}
        ...
        if (cg_drawScores.integer == 2 && !cg.scoreBoardShowing) {
            ...
            for (int i = 0; i < numScores; i++) {
                ...
                if (cg.numScores && ourScore) {
                    if (score->client == cg.snap->ps.clientNum) {
                        Com_sprintf(buf[numWritten], sizeof(buf[0]), "%s " S_COLOR_WHITE "%s", ci->name, CG_PlaceString(numWritten + 1));
                    } else {
                        const int net = score->score - ourScore->score;
                        const char sign = (net >= 0) ? '-' : '+';

                        Com_sprintf(buf[numWritten], sizeof(buf[0]), "%s " S_COLOR_WHITE "(%s%c%i" S_COLOR_WHITE ") %s", ci->name,
                                    (net < 0) ? (S_COLOR_GREEN) : ((net > 0) ? S_COLOR_RED : S_COLOR_WHITE), sign, std::abs(net),
            ...
            for (int i = 0; i < numWritten; i++) {
                ...
                font.Paint(SCREEN_WIDTH - textWidth, y + (i * textHeight), buf[i], &colorWhite, uiTextStyle_e::Shadowed);
                y += textHeight;
            }
        }
    }
...
static float CG_DrawEnemyInfo(float y) {
    ...
    if (!cg.snap || cg_drawEnemyInfo.integer != 1 || cg.predictedPlayerState.stats[STAT_HEALTH] <= 0 || cgs.gametype == GT_POWERDUEL) {
        return y;
    }
    ...
    if (cgs.gametype == GT_JEDIMASTER) {
        ...
        if (clientNum < 0 || clientNum >= MAX_CLIENTS) {
            ...
            CG_DrawPic(SCREEN_WIDTH - (ICON_SIZE * cgs.widthRatioCoef), y, ICON_SIZE * cgs.widthRatioCoef, ICON_SIZE,
    ...
    if (ci->modelIcon) {
        CG_DrawPic(SCREEN_WIDTH - (ICON_SIZE * cgs.widthRatioCoef), y, ICON_SIZE * cgs.widthRatioCoef, ICON_SIZE, ci->modelIcon);
    }
    ...
    if (cgs.showDuelHealths >= 2) {
        ...
        if (cgs.duelist1 == clientNum) {
            CG_DrawDuelistHealth(SCREEN_WIDTH - (ICON_SIZE - 5) * cgs.widthRatioCoef, y, 64 * cgs.widthRatioCoef, 8, 1);
        } else if (cgs.duelist2 == clientNum) {
            CG_DrawDuelistHealth(SCREEN_WIDTH - (ICON_SIZE - 5) * cgs.widthRatioCoef, y, 64 * cgs.widthRatioCoef, 8, 2);
        }
    }
...
static float CG_DrawFPS(float y) {
    ...
    if (cg_drawFPS.integer) {
        ...
        Q_LerpColour(&fpsBad, &fpsGood, &fpsColour, std::min(std::max(0.0f, static_cast<float>(fps)) / std::max(IDEAL_FPS, static_cast<float>(maxFPS)), 1.0f));
...
float CG_DrawRadar(float y) {
    ...
    for (i = cg.radarEntityCount - 1; i >= 0; i--) {
        ...
        default: {
            ...
            if (shader) {
                ...
                if ((cent->currentState.time2 && cg.time - cent->currentState.time2 < 5000) || (cent->currentState.time2 == 0xFFFFFFFF)) {
                    if ((cg.time >> 8) & 1)
        ...
        case ET_NPC: // FIXME: draw last, with players...
            if (cent->currentState.NPC_class == CLASS_VEHICLE && cent->currentState.speed > 0) {
                if (cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo->radarIconHandle) {
                    ...
                    CG_DrawPic(x - 4 * cgs.widthRatioCoef + xOffset, ly - 4, arrowBaseScale * cgs.widthRatioCoef, arrowBaseScale,
        ...
        case ET_MISSILE:
            if (                                       // cent->currentState.weapon == WP_ROCKET_LAUNCHER &&//a rocket
                ...
                if (cg.predictedPlayerState.m_iVehicleNum) { // I'm in a vehicle
                    ...
                    if (cent->currentState.otherEntityNum == cg.predictedPlayerState.clientNum ||
                        cent->currentState.otherEntityNum == cg.predictedPlayerState.m_iVehicleNum) {
                ...
                CG_DrawPic(x - 4 * cgs.widthRatioCoef + xOffset, ly - 4, arrowBaseScale * cgs.widthRatioCoef, arrowBaseScale,
                           media.gfx.interface.automap.rocketIcon);
            }
            break;
        ...
        case ET_PLAYER: {
            ...
            CG_DrawRotatePic2(RADAR_X + (RADAR_RADIUS + sinf(angle) * distance + xOffset) * cgs.widthRatioCoef, y + RADAR_RADIUS + cosf(angle) * distance,
                              arrowW, arrowH, (360 - cent->lerpAngles.yaw) + cg.predictedPlayerState.viewangles.yaw, media.gfx.interface.automap.playerIcon);
            break;
        }
        }
    }
    ...
    CG_DrawRotatePic2(RADAR_X + (RADAR_RADIUS + xOffset) * cgs.widthRatioCoef, y + RADAR_RADIUS, arrowW, arrowH, 0, media.gfx.interface.automap.playerIcon);
...
static float CG_DrawTeamOverlay(float y, qboolean right, qboolean upper) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
            ...
            CG_DrawStringExt(xx + xOffset, y, ci->name, &hcolor, qfalse, qfalse, TINYCHAR_WIDTH * cgs.widthRatioCoef, TINYCHAR_HEIGHT,
            ...
            if (lwidth) {
                ...
                CG_DrawStringExt(xx + xOffset, y, p, &hcolor, qfalse, qfalse, TINYCHAR_WIDTH * cgs.widthRatioCoef, TINYCHAR_HEIGHT,
            ...
            xx = x + TINYCHAR_WIDTH * 3 * cgs.widthRatioCoef + TINYCHAR_WIDTH * pwidth * cgs.widthRatioCoef + TINYCHAR_WIDTH * lwidth * cgs.widthRatioCoef;

            CG_DrawStringExt(xx + xOffset, y, st, &hcolor, qfalse, qfalse, TINYCHAR_WIDTH * cgs.widthRatioCoef, TINYCHAR_HEIGHT, 0);

            // draw weapon icon
            xx += TINYCHAR_WIDTH * 3 * cgs.widthRatioCoef;

            if (cg_weapons[ci->curWeapon].weaponIcon) {
                CG_DrawPic(xx + xOffset, y, TINYCHAR_WIDTH * cgs.widthRatioCoef, TINYCHAR_HEIGHT, cg_weapons[ci->curWeapon].weaponIcon);
            } else {
                CG_DrawPic(xx + xOffset, y, TINYCHAR_WIDTH * cgs.widthRatioCoef, TINYCHAR_HEIGHT, media.gfx.interface.defer);
            }
        ...
        if (cg.snap->ps.powerups[j] > cg.time) {
            ...
            if (item) {
                ...
                if (cgs.gametype == GT_CTY && (j == PW_REDFLAG || j == PW_BLUEFLAG)) {
                    icon = trap->R_RegisterShaderNoMip((j == PW_REDFLAG) ? "gfx/hud/mpi_rflag_ys" : "gfx/hud/mpi_bflag_ys");
                } else {
                ...
                if (j != PW_REDFLAG && j != PW_BLUEFLAG && secondsLeft < 999) {
                    ...
                    const Font font(fontHandle, fontScale, false);
                    font.Paint((SCREEN_WIDTH - iconWidth - (iconWidth * cgs.widthRatioCoef)), y - 8, s, &colorWhite, uiTextStyle_e::Shadowed);
                }
...
static void CG_DrawLagometer(void) {
    ...
    for (int a = 0; a < w; a++) {
        ...
        if (v > 0) {
            ...
            trap->R_DrawStretchPic(x + (w - a) * cgs.widthRatioCoef, mid - v, 1.0f * cgs.widthRatioCoef, v, 0, 0, 0, 0, media.gfx.world.whiteShader);
        } else if (v < 0) {
            if (color != COLOR_BLUE) {
                color = COLOR_BLUE;
            ...
            trap->R_DrawStretchPic(x + (w - a) * cgs.widthRatioCoef, mid, 1.0f * cgs.widthRatioCoef, v, 0, 0, 0, 0, media.gfx.world.whiteShader);
        }
    }
    ...
    for (int a = 0; a < w; a++) {
        ...
        if (v > 0) {
            ...
            trap->R_DrawStretchPic(x + (w - a) * cgs.widthRatioCoef, y + h - v, 1.0f * cgs.widthRatioCoef, v, 0, 0, 0, 0, media.gfx.world.whiteShader);
        } else if (v < 0) {
            // dropped snapshots
            if (color != COLOR_RED) {
                color = COLOR_RED;
            ...
            trap->R_DrawStretchPic(x + (w - a) * cgs.widthRatioCoef, y + h - range, 1.0f * cgs.widthRatioCoef, range, 0, 0, 0, 0, media.gfx.world.whiteShader);
        }
    }
...
qboolean BG_IsWhiteSpace(char c) { return c == ' ' || c == '\n' || c == '\0'; }
...
void CG_DrawSiegeInfo(centity_t *cent, float chX, float chY, float chW, float chH) {
    ...
    if (!weaponData[cent->currentState.weapon].shotCost && !weaponData[cent->currentState.weapon].alt.shotCost) { // a weapon that takes no ammo, so show full
...
void CG_DrawHaqrBar(float chX, float chY, float chW, float chH) {
    ...
    float y = (chY + chH) + 8.0f;
    float percent =
        (((float)cg.predictedPlayerState.hackingTime - (float)cg.time) / (float)cg.predictedPlayerState.hackingBaseTime) * HEALTH_WIDTH * cgs.widthRatioCoef;

    if (percent > HEALTH_WIDTH || percent < 1.0f) {
        return;
    }
...
void CG_DrawGenericTimerBar(void) {
    ...
    // now draw the part to show how much health there is in the color specified
    CG_FillRect(x + 1.0f * cgs.widthRatioCoef, y + 1.0f + (CGTIMERBAR_H - percent), (CGTIMERBAR_W - 2.0f) * cgs.widthRatioCoef,
...
static void CG_DrawCrosshair(vector3 *worldPoint, qboolean chEntValid) {
    ...
    if (cg_crosshairHealth.integer)
        ...
        if (cg.crosshairClientNum >= ENTITYNUM_WORLD || !chEntValid || cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_CLOAKED)) {
            VectorSet4(&colour, 1.0f, 1.0f, 1.0f, 1.0f);
        } else {
            entityState_t *es = &cg_entities[cg.crosshairClientNum].currentState;
            if (chEntValid &&
                (es->number < MAX_CLIENTS || es->eType == ET_NPC || es->shouldtarget || es->health ||
                 (es->eType == ET_MOVER && es->bolt1 && cg.predictedPlayerState.weapon == WP_SABER) || (es->eType == ET_MOVER && es->teamowner))) {
                crossEnt = &cg_entities[cg.crosshairClientNum];

                if (crossEnt->currentState.number < MAX_CLIENTS) {
                    if (cgs.gametype >= GT_TEAM && cgs.clientinfo[crossEnt->currentState.number].team == cgs.clientinfo[cg.snap->ps.clientNum].team) {
                        // ally
                    ...
                    if (crossEnt->currentState.eType == ET_NPC) {
                        ...
                        // ally
                            ...
                            VectorSet4(&colour, 0.0f, 1.0f, 0.0f, 1.0f);
                    } else if (crossEnt->currentState.teamowner == TEAM_RED || crossEnt->currentState.teamowner == TEAM_BLUE) {
                        // neutral
                        if (cgs.gametype < GT_TEAM)
                            VectorSet4(&colour, 1.0f, 1.0f, 0.0f, 1.0f);
                        // enemy
                        ...
                        // ally
                        VectorSet4(&colour, 0.0f, 1.0f, 0.0f, 1.0f);
                    } else if (crossEnt->currentState.teamowner == 16 || (cgs.gametype >= GT_TEAM && crossEnt->currentState.teamowner &&
    ...
    if (cg.predictedPlayerState.m_iVehicleNum) {
        ...
        if (vehCent && vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo && vehCent->m_pVehicle->m_pVehicleInfo->crosshairShaderHandle) {
    ...
    if (crossEnt && crossEnt->currentState.maxhealth) {
        ...
        if (cg.crosshairVehNum && cg.time == cg.crosshairVehTime) {
            ...
            if (hisVeh->currentState.eType == ET_NPC && hisVeh->currentState.NPC_class == CLASS_VEHICLE && hisVeh->currentState.maxhealth &&
                hisVeh->m_pVehicle) {
...
void CG_SaberClashFlare(void) {
    ...
    */
    ...
    CG_DrawPic(x - (v * 300) * cgs.widthRatioCoef, y - (v * 300), v * 600 * cgs.widthRatioCoef, v * 600, trap->R_RegisterShader("gfx/effects/saberFlare"));
}
...
void CG_BracketEntity(centity_t *cent, float radius) {
    ...
    if (cg.crosshairClientNum != cent->currentState.clientNum &&
        (!cg.snap || cg.snap->ps.rocketLockIndex !=
                         cent->currentState.clientNum)) { // if they're the entity you're locking onto or under your crosshair, always draw bracket
                ...
                if (!VectorCompare(&cent->currentState.pos.trDelta, &vec3_origin)) { // enemy vehicle is moving
                    if (cg.predictedPlayerState.m_iVehicleNum) {                     // I'm in a vehicle
                            ...
                            && veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID > VEH_WEAPON_BASE) // valid vehicle weapon
                        {
                            ...
                            if (vehWeapon && vehWeapon->bIsProjectile // primary weapon's shot is a projectile
                            ...
                            {                                         // our primary weapon's projectile has a speed
                                ...
                                eta = (vehDist /
                                       vehWeapon->fSpeed); // how many seconds it would take for my primary weapon's projectile to get from my ship to theirs
...
qboolean CG_InATST(void) {
    if (cg.predictedPlayerState.m_iVehicleNum) { // I'm in a vehicle
        centity_t *vehCent = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
        if (vehCent && vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo && vehCent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) { // I'm in an atst
            return qtrue;
        }
    }
    return qfalse;
...
static qboolean CG_IsDurationPower(int power) {
    if (power == FP_HEAL || power == FP_SPEED || power == FP_TELEPATHY || power == FP_RAGE || power == FP_PROTECT || power == FP_ABSORB || power == FP_SEE) {
        return qtrue;
    }
...
//--------------------------------------------------------------
{
    ...
    while (i < NUM_FORCE_POWERS) {
        if ((cg.snap->ps.fd.forcePowersActive & (1 << forcePowerSorted[i])) && CG_IsDurationPower(forcePowerSorted[i])) {
...
//--------------------------------------------------------------
{
    ...
    if (cg.snap->ps.m_iVehicleNum) { // driving a vehicle
        ...
        if (veh->m_pVehicle) {
            ...
            if (cg.predictedVehicleState.weaponstate == WEAPON_CHARGING_ALT) {
                ...
                if (veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID > VEH_WEAPON_BASE && veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID < MAX_VEH_WEAPONS) {
    ...
    if (cg.snap->ps.rocketLockIndex >= 0 && cg.snap->ps.rocketLockIndex < ENTITYNUM_NONE) {
        ...
        if (ci) {
            if (ci->team == cgs.clientinfo[cg.snap->ps.clientNum].team) {
                ...
                if (hitEnt->currentState.eType == ET_NPC && hitEnt->currentState.NPC_class == CLASS_VEHICLE &&
...
static void CG_ScanForCrosshairEntity(void) {
    ...
    if (cg_dynamicCrosshair.integer) {
        ...
        if (cg.predictedPlayerState.m_iVehicleNum && (cg.predictedPlayerState.eFlags & EF_NODRAW)) {
            ...
            if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo && veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
        ...
        } else if (cg.snap && cg.snap->ps.weapon == WP_EMPLACED_GUN && cg.snap->ps.emplacedIndex && cg_entities[cg.snap->ps.emplacedIndex].ghoul2 &&
...
traceAgain:
    if (cg_dynamicCrosshair.integer && cg_dynamicCrosshairPrecision.integer) {
        ...
        if (bVehCheckTraceFromCamPos) {
            ...
            CG_G2Trace(&extraTrace, &refdef->vieworg, &vec3_origin, &vec3_origin, &extraEnd, ignore, CONTENTS_SOLID | CONTENTS_BODY);
            if (!extraTrace.allsolid && !extraTrace.startsolid) {
                if (extraTrace.fraction < 1.0f) {
                    if ((extraTrace.fraction * MAX_XHAIR_DIST_ACCURACY) > minAutoAimDist) {
                        if (((extraTrace.fraction * MAX_XHAIR_DIST_ACCURACY) - Distance(&veh->lerpOrigin, &refdef->vieworg)) <
    ...
    if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
        if (trace.entityNum < /*MAX_CLIENTS*/ ENTITYNUM_WORLD) {
            ...
            if (veh->currentState.eType == ET_NPC && veh->currentState.NPC_class == CLASS_VEHICLE && veh->currentState.owner < MAX_CLIENTS) {
...
static void CG_DrawCrosshairNames(void) {
    ...
    if (!cg_drawCrosshairNames.integer || (cg_drawSpectatorNames.integer && CG_IsSpectating())) {
        return;
    }
    ...
    if (isVeh) {
        ...
        const float width = font.Width(name);
        font.Paint((SCREEN_WIDTH / 2) - (width / 2.0f), (SCREEN_HEIGHT / 2) - 70.0f, name, &tcolor, uiTextStyle_e::ShadowedMore);
    }
...
static void CG_DrawSpectator(void) {
    ...
    if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) && cgs.duelist1 != -1 && cgs.duelist2 != -1) {
        char text[1024];
        int size = 64;

        if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
            Com_sprintf(text, sizeof(text), "%s" S_COLOR_WHITE " %s %s" S_COLOR_WHITE " %s %s", cgs.clientinfo[cgs.duelist1].name,
                        CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"), cgs.clientinfo[cgs.duelist2].name, CG_GetStringEdString("MP_INGAME", "AND"),
                        cgs.clientinfo[cgs.duelist3].name);
        } else {
            Com_sprintf(text, sizeof(text), "%s" S_COLOR_WHITE " %s %s", cgs.clientinfo[cgs.duelist1].name, CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
        ...
        if (cgs.gametype != GT_POWERDUEL) {
            ...
            mediumFont.Paint(42 - mediumFont.Width(text) / 2, SCREEN_HEIGHT - (size * 1.5f) + 64, text, &colorWhite);

            Com_sprintf(text, sizeof(text), "%i/%i", cgs.clientinfo[cgs.duelist2].score, cgs.fraglimit);
            mediumFont.Paint(SCREEN_WIDTH - size + 22 - mediumFont.Width(text) / 2, SCREEN_HEIGHT - (size * 1.5f) + 64, text, &colorWhite);
        }
...
static void CG_DrawVote(void) {
    ...
    if (!strncmp(cgs.voteString, "map_restart", 11))
        ...
        if (!Q_stricmp(cgs.voteString + 11, "Free For All"))
            sParm = CG_GetStringEdString("MENUS", "FREE_FOR_ALL");
        else if (!stricmp(cgs.voteString + 11, "Duel"))
            sParm = CG_GetStringEdString("MENUS", "DUEL");
        else if (!stricmp(cgs.voteString + 11, "Holocron FFA"))
            sParm = CG_GetStringEdString("MENUS", "HOLOCRON_FFA");
        else if (!stricmp(cgs.voteString + 11, "Power Duel"))
            sParm = CG_GetStringEdString("MENUS", "POWERDUEL");
        else if (!stricmp(cgs.voteString + 11, "Team FFA"))
            sParm = CG_GetStringEdString("MENUS", "TEAM_FFA");
        else if (!stricmp(cgs.voteString + 11, "Siege"))
            sParm = CG_GetStringEdString("MENUS", "SIEGE");
        else if (!stricmp(cgs.voteString + 11, "Capture the Flag"))
            sParm = CG_GetStringEdString("MENUS", "CAPTURE_THE_FLAG");
        else if (!stricmp(cgs.voteString + 11, "Capture the Ysalamiri"))
...
static void CG_DrawTeamVote(void) {
    ...
    if (strstr(cgs.teamVoteString[cs_offset], "leader")) {
        ...
        if (cgs.teamVoteString[cs_offset][i] == ' ') {
            ...
            s = va("TEAMVOTE(%i):(Make %s the new team leader) yes:%i no:%i", sec, cgs.clientinfo[voteIndex].name, cgs.teamVoteYes[cs_offset],
...
void CG_DrawJetpackFuel(void) {
    ...
    // now draw the part to show how much health there is in the color specified
    CG_FillRect(x + 1.0f * cgs.widthRatioCoef, y + 1.0f + (JPFUELBAR_H - percent), (JPFUELBAR_W - 1.0f) * cgs.widthRatioCoef,
                JPFUELBAR_H - 1.0f - (JPFUELBAR_H - percent), &aColor);
...
void CG_DrawEWebHealth(void) {
    ...
    // now draw the part to show how much health there is in the color specified
    CG_FillRect(x + 1.0f * cgs.widthRatioCoef, y + 1.0f + (EWEBHEALTH_H - percent), (EWEBHEALTH_W - 1.0f) * cgs.widthRatioCoef,
                EWEBHEALTH_H - 1.0f - (EWEBHEALTH_H - percent), &aColor);
...
void CG_DrawCloakFuel(void) {
    ...
    CG_FillRect(x + 1.0f * cgs.widthRatioCoef, y + 1.0f + (CLFUELBAR_H - percent), (CLFUELBAR_W - 1.0f) * cgs.widthRatioCoef,
                CLFUELBAR_H - 1.0f - (CLFUELBAR_H - percent), &aColor);
...
static void CG_DrawSiegeTimer(int timeRemaining, qboolean isMyTeam) {
    ...
    if (item) {
        trap->R_SetColor(&item->window.foreColor);
        CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y, item->window.rect.w * cgs.widthRatioCoef,
                   item->window.rect.h, item->window.background);
    }
    ...
    if (item) {
        ...
        font.Paint(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y, timeStr, &colorTable[fColor],
...
static void CG_DrawSiegeDeathTimer(int timeRemaining) {
    ...
    if (item) {
        trap->R_SetColor(&item->window.foreColor);
        CG_DrawPic(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y, item->window.rect.w, item->window.rect.h,
    ...
    if (item) {
        const Font font(FONT_SMALL, 1.0f, false);
        font.Paint(SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef, item->window.rect.y, timeStr, &item->window.foreColor,
                   uiTextStyle_e::Shadowed);
    }
}
...
static void CG_Draw2DScreenTints(void) {
    ...
    if ((refdef->viewContents & CONTENTS_LAVA)) { // tint screen red
        ...
        CG_FillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, &hcolor);
    } else if ((refdef->viewContents & CONTENTS_WATER)) { // tint screen light blue -- FIXME: don't do this if CONTENTS_FOG? (in case someone *does* make a
                                                          // water shader with fog in it?)
        float phase = cg.time / 1000.0f * WAVE_FREQUENCY * M_PI * 2;
