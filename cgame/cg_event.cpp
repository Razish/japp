...
clientkilled:
    ...
    if (attacker == cg.snap->ps.clientNum) {
        ...
        if (cgs.gametype < GT_TEAM && cgs.gametype != GT_DUEL && cgs.gametype != GT_POWERDUEL) {
            if (cgs.gametype == GT_JEDIMASTER && attacker < MAX_CLIENTS && !ent->isJediMaster && !cg.snap->ps.isJediMaster && CG_ThereIsAMaster()) {
                char part1[512];
                char part2[512];
            ...
            } else if (cgs.gametype == GT_JEDIMASTER && attacker < MAX_CLIENTS && !ent->isJediMaster && !cg.snap->ps.isJediMaster) { // no JM, saber must be out
                char part1[512];
                trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", part1, sizeof(part1));
                /*
                kmsg1 = "for 0 points.\nGo for the saber!";
                ...
                */
                ...
                if (cg_scorePlums.integer == 2)
                    CG_ScorePlum(cg.predictedPlayerState.clientNum, &cg_entities[target].lerpOrigin, 1);
            }
        } else {
            char sKilledStr[256];
            trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", sKilledStr, sizeof(sKilledStr));
...
void CG_ToggleBinoculars(centity_t *cent, int forceZoom) {
    ...
    */
    ...
    if (cg.snap->ps.zoomMode == 0) {
        trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, media.sounds.weapons.zoomStart);
    } else if (cg.snap->ps.zoomMode == 2) {
        trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, media.sounds.weapons.zoomEnd);
    }
}
...
extern vector4 cg_genericTimerColor;
void CG_LocalTimingBar(int startTime, int duration) {
    cg_genericTimerBar = startTime + duration;
    cg_genericTimerDur = duration;

    cg_genericTimerColor.r = 1.0f;
    cg_genericTimerColor.g = 1.0f;
    cg_genericTimerColor.b = 0.0f;
...
static void CG_UseItem(centity_t *cent) {
    ...
    case HI_NONE:
        ...
        //	trap->S_StartSound( NULL, es->number, CHAN_BODY, media.sounds.useNothing );
        break;
    ...
    case HI_SEEKER:
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.interface.deploySeeker);
        break;
    ...
    case HI_MEDPAC_BIG:
        ...
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.interface.medkit);
        break;
    case HI_JETPACK:
        break; // Do something?
    case HI_HEALTHDISP:
    ...
    if (cg.snap && cg.snap->ps.clientNum == cent->currentState.number && itemNum != HI_BINOCULARS && itemNum != HI_JETPACK && itemNum != HI_HEALTHDISP &&
        itemNum != HI_AMMODISP && itemNum != HI_CLOAK &&
        itemNum != HI_EWEB) { // if not using binoculars/jetpack/dispensers/cloak, we just used that item up, so switch
...
static void CG_ItemPickup(int itemNum) {
    ...
    if (cg.snap && bg_itemlist[itemNum].giType == IT_WEAPON) {
        ...
        if (0 == cg_autoSwitch.integer) {
            // don't switch
        } else if (cg_autoSwitch.integer == 1) { // only autoselect if not explosive ("safe")
            if (bg_itemlist[itemNum].giTag != WP_TRIP_MINE && bg_itemlist[itemNum].giTag != WP_DET_PACK && bg_itemlist[itemNum].giTag != WP_THERMAL &&
                bg_itemlist[itemNum].giTag != WP_ROCKET_LAUNCHER && bg_itemlist[itemNum].giTag > cg.snap->ps.weapon && cg.snap->ps.weapon != WP_SABER) {
                if (!cg.snap->ps.emplacedIndex) {
                    cg.weaponSelectTime = cg.time;
                }
                cg.weaponSelect = bg_itemlist[itemNum].giTag;
            }
        } else if (cg_autoSwitch.integer == 2) { // autoselect if better
    ...
    if (bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0] &&
        (bg_itemlist[itemNum].giType != IT_TEAM ||
         (bg_itemlist[itemNum].giTag != PW_REDFLAG &&
          bg_itemlist[itemNum].giTag != PW_BLUEFLAG))) { // don't print messages for flags, they have their own pickup event broadcasts
        char text[1024];
        char upperKey[1024];
...
void DoFall(centity_t *cent, entityState_t *es, int clientNum) {
    ...
    if (cent->currentState.eFlags & EF_DEAD) { // corpses crack into the ground ^_^
        if (delta > 25) {
            trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.fall);
        } else {
        ...
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.fall);
        trap->S_StartSound(NULL, cent->currentState.number, CHAN_VOICE, CG_CustomSound(cent->currentState.number, "*land1.wav"));
        ...
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.land);
    }
...
void CG_TryPlayCustomSound(vector3 *origin, int entityNum, int channel, const char *soundName) {
...
void CG_G2MarkEvent(entityState_t *es) {
    ...
    if (!pOwner->ghoul2) { // can't do anything then...
        return;
    }
    ...
    if (es->eventParm) {
        ...
        if (tr.entityNum != es->otherEntityNum) { // try again if we hit an ent but not the one we wanted.
            ...
            if (tr.entityNum < ENTITYNUM_WORLD) {
                ignore = tr.entityNum;
                CG_G2Trace(&tr, &es->origin, NULL, NULL, &es->origin2, ignore, MASK_PLAYERSOLID);
                if (tr.entityNum != es->otherEntityNum) { // try extending the trace a bit.. or not
                    /*
                    vector3 v;
    ...
    if ((es->eFlags & EF_JETPACK_ACTIVE)) { // a vehicle weapon, make it a larger size mark
        ...
        if (g_vehWeaponInfo[es->otherEntityNum2].iG2MarkShaderHandle) { // have one we want to use instead of defaults
    ...
    case WP_TURRET:
        ...
        if (!shader) {
            shader = media.gfx.world.bdecal_bodyburn1;
        }
        CG_AddGhoul2Mark(shader, size, &startPoint, &es->origin2, es->owner, &pOwner->lerpOrigin, pOwner->lerpAngles.yaw, pOwner->ghoul2, &pOwner->modelScale,
                         Q_irand(10000, 20000));
        break;
    ...
    case WP_THERMAL:
        ...
        if (!shader) {
            shader = media.gfx.world.bdecal_burn1;
        }
        CG_AddGhoul2Mark(shader, size, &startPoint, &es->origin2, es->owner, &pOwner->lerpOrigin, pOwner->lerpAngles.yaw, pOwner->ghoul2, &pOwner->modelScale,
                         Q_irand(10000, 20000));
        break;
        /*
case WP_FLECHETTE:
CG_AddGhoul2Mark(media.gfx.world.bdecal_bodyburn1, flrand(0.5f, 1.0f),
...
void CG_CalcVehMuzzle(Vehicle_t *pVeh, centity_t *ent, int muzzleNum) {
    ...
    if (pVeh->m_iMuzzleTime[muzzleNum] == cg.time) { // already done for this frame, don't need to do it again
        return;
    }
    ...
    if (pVeh->m_pVehicleInfo) {
        if (pVeh->m_pVehicleInfo->type == VH_ANIMAL || pVeh->m_pVehicleInfo->type == VH_WALKER) {
            vehAngles.pitch = vehAngles.roll = 0.0f;
        } else if (pVeh->m_pVehicleInfo->type == VH_SPEEDER) {
            vehAngles.pitch = 0.0f;
        }
    }
    trap->G2API_GetBoltMatrix_NoRecNoRot(ent->ghoul2, 0, pVeh->m_iMuzzleTag[muzzleNum], &boltMatrix, &vehAngles, &ent->lerpOrigin, cg.time, NULL,
                                         &ent->modelScale);
            ...
            if (pVeh->m_pVehicleInfo->weapMuzzle[curMuz] == 0) { // no weaopon for this muzzle?  check turrets
                ...
                for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
                    for (j = 0; j < MAX_VEHICLE_TURRETS; j++) {
                        if (pVeh->m_pVehicleInfo->turret[i].iMuzzle[j] - 1 == curMuz) { // this muzzle belongs to this turret
...
const char *cg_stringEdVoiceChatTable[MAX_CUSTOM_SIEGE_SOUNDS] = {"VC_ATT",           //"*att_attack",
...
void CG_EntityEvent(centity_t *cent, vector3 *position) {
    ...
    case EV_FOOTSTEP:
        ...
        if (cg_footsteps.integer) {
            ...
            trap->S_StartSound(NULL, es->number, CHAN_BODY, media.sounds.environment.footsteps[soundType][rand() & 3]);
        }
        break;
    case EV_FOOTSTEP_METAL:
        DEBUGNAME("EV_FOOTSTEP_METAL");
        if (cg_footsteps.integer) {
            trap->S_StartSound(NULL, es->number, CHAN_BODY, media.sounds.environment.footsteps[FOOTSTEP_METALWALK][rand() & 3]);
        }
        break;
    case EV_FOOTSPLASH:
        DEBUGNAME("EV_FOOTSPLASH");
        if (cg_footsteps.integer) {
            trap->S_StartSound(NULL, es->number, CHAN_BODY, media.sounds.environment.footsteps[FOOTSTEP_SPLASH][rand() & 3]);
        }
        break;
    case EV_FOOTWADE:
    ...
    case EV_GHOUL2_MARK:
        ...
        if (cg_ghoul2Marks.integer) { // Can we put a burn mark on him?
            CG_G2MarkEvent(es);
        }
        break;
    ...
    case EV_GLOBAL_DUEL:
        ...
        if (es->otherEntityNum == cg.predictedPlayerState.clientNum || es->otherEntityNum2 == cg.predictedPlayerState.clientNum ||
    ...
    case EV_JUMP:
        DEBUGNAME("EV_JUMP");
        if (cg_jumpSounds.integer && (!cg.predictedPlayerState.duelInProgress || (cg.predictedPlayerState.duelIndex == es->clientNum ||
                                                                                  cg.predictedPlayerState.clientNum == es->clientNum))) { // Raz: duel isolation
    ...
    case EV_ROLL:
        ...
        if (cg.predictedPlayerState.duelInProgress &&
            (cg.predictedPlayerState.clientNum != es->clientNum && cg.predictedPlayerState.duelIndex != es->clientNum))
            break;
        ...
        trap->S_StartSound(NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, "*jump1.wav"));
        trap->S_StartSound(NULL, es->number, CHAN_BODY, media.sounds.environment.roll);
    ...
    case EV_WATER_TOUCH:
        ...
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.waterIn);
        break;
    case EV_WATER_LEAVE:
        DEBUGNAME("EV_WATER_LEAVE");
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.waterOut);
        break;
    case EV_WATER_UNDER:
        DEBUGNAME("EV_WATER_UNDER");
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.waterUnder);
        break;
    case EV_WATER_CLEAR:
    ...
    case EV_ITEM_PICKUP:
        DEBUGNAME("EV_ITEM_PICKUP");
        {
            ...
            if (index < 1 && cg_entities[es->eventParm].currentState.isJediMaster) { // a holocron most likely
                ...
                if (fpIndex != FP_SABER_OFFENSE && fpIndex != FP_SABER_DEFENSE && fpIndex != FP_SABERTHROW && fpIndex != FP_LEVITATION &&
            ...
            if (cg_entities[es->eventParm].weapon >= cg.time) { // rww - an unfortunately necessary hack to prevent double item pickups
                break;
            }
    ...
    case EV_NOAMMO:
        DEBUGNAME("EV_NOAMMO");
        //		trap->S_StartSound (NULL, es->number, CHAN_AUTO, media.sounds.weapons.noAmmo );
        if (es->number == cg.snap->ps.clientNum) {
            if (CG_InFighter() || CG_InATST() || cg.snap->ps.weapon == WP_NONE) { // just letting us know our vehicle is out of ammo
                ...
                if (localCent->m_pVehicle && localCent->m_pVehicle->m_pVehicleInfo &&
                    localCent->m_pVehicle->m_pVehicleInfo->weapon[es->eventParm].soundNoAmmo) { // play the "no Ammo" sound for this weapon
                ...
                } else { // play the default "no ammo" sound
                    trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, media.sounds.weapons.noAmmo);
                }
                ...
                if (cg_vehicleAmmoWarningTime < cg.time ||
                    cg_vehicleAmmoWarning !=
                        es->eventParm) { // if there's already one going, don't interrupt it (unless they tried to fire another weapon that's out of ammo)
    ...
    case EV_CHANGE_WEAPON:
        DEBUGNAME("EV_CHANGE_WEAPON");
        {
            ...
            } else if (weapon != WP_SABER) { // not sure what SP is doing for this but I don't want a select sound for saber (it has the saber-turn-on)
                trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.interface.select);
            }
        }
        break;
    case EV_FIRE_WEAPON:
        DEBUGNAME("EV_FIRE_WEAPON");
        if (cent->currentState.number >= MAX_CLIENTS && cent->currentState.eType != ET_NPC) { // special case for turret firing
            vector3 gunpoint, gunangle;
            mdxaBone_t matrix;
            ...
            if (cent->ghoul2) {
                ...
                trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Bone02", 1, 4, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 300);
            } else {
                break;
            }

            if (cent->currentState.eventParm)
                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt2, &matrix, &cent->currentState.angles, &cent->currentState.origin, cg.time,
                                          cgs.gameModels, &cent->modelScale);
            else
                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt1, &matrix, &cent->currentState.angles, &cent->currentState.origin, cg.time,
                                          cgs.gameModels, &cent->modelScale);

            gunpoint.x = matrix.matrix[0][3];
            gunpoint.y = matrix.matrix[1][3];
            gunpoint.z = matrix.matrix[2][3];

            gunangle.x = -matrix.matrix[0][0];
            gunangle.y = -matrix.matrix[1][0];
            gunangle.z = -matrix.matrix[2][0];

            trap->FX_PlayEffectID(media.efx.emplacedMuzzleFlash, &gunpoint, &gunangle, -1, -1, qfalse);
            ...
            if (cent->currentState.eType == ET_NPC && cent->currentState.NPC_class == CLASS_VEHICLE &&
                cent->m_pVehicle) { // vehicles do nothing for clientside weapon fire events.. at least for now.
                break;
            }
    ...
    case EV_ALT_FIRE:
        ...
        if (cent->currentState.weapon == WP_EMPLACED_GUN) { // don't do anything for emplaced stuff
            break;
        }
        ...
        if (cg.snap->ps.clientNum == cent->currentState.number && cg.snap->ps.weapon == WP_DET_PACK) {
    ...
    case EV_SABER_ATTACK:
        DEBUGNAME("EV_SABER_ATTACK");
        {
            ...
            if (client && client->infoValid && client->saber[0].swingSound[0]) { // custom swing sound
    ...
    case EV_SABER_HIT:
        DEBUGNAME("EV_SABER_HIT");
        {
            ...
            int hitPersonSmallFxID = media.efx.saberBloodSparksSmall;
            int hitPersonMidFxID = media.efx.saberBloodSparksMid;
            ...
            if (es->otherEntityNum2 >= 0 &&
                es->otherEntityNum2 <
                    ENTITYNUM_NONE) { // we have a specific person who is causing this effect, see if we should override it with any custom saber effects/sounds
                clientInfo_t *client = NULL;
                ...
                if (client && client->infoValid) {
                    ...
                    if (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum)) { // use second blade style values
                        if (client->saber[saberNum].hitPersonEffect2) {                         // custom hit person effect
                            hitPersonFxID = hitPersonSmallFxID = hitPersonMidFxID = client->saber[saberNum].hitPersonEffect2;
                        }
                        if (client->saber[saberNum].hitOtherEffect2) { // custom hit other effect
                            hitOtherFxID = client->saber[saberNum].hitOtherEffect2;
                        }
                        if (client->saber[saberNum].hit2Sound[0]) { // custom hit sound
                            hitSound = client->saber[saberNum].hit2Sound[Q_irand(0, 2)];
                        }
                    } else {                                           // use first blade style values
                        if (client->saber[saberNum].hitPersonEffect) { // custom hit person effect
                            hitPersonFxID = hitPersonSmallFxID = hitPersonMidFxID = client->saber[saberNum].hitPersonEffect;
                        }
                        if (client->saber[saberNum].hitOtherEffect) { // custom hit other effect
                            hitOtherFxID = client->saber[0].hitOtherEffect;
                        }
                        if (client->saber[saberNum].hitSound[0]) { // custom hit sound
            ...
            if (es->eventParm == 16) { // Make lots of sparks, something special happened
                vector3 fxDir;
                ...
                trap->FX_PlayEffectID(hitPersonFxID, &es->origin, &fxDir, -1, -1, qfalse);
                trap->FX_PlayEffectID(hitPersonFxID, &es->origin, &fxDir, -1, -1, qfalse);
            } else if (es->eventParm) { // hit a person
                vector3 fxDir;
                VectorCopy(&es->angles, &fxDir);
                if (!fxDir.x && !fxDir.y && !fxDir.z) {
                    fxDir.y = 1;
                ...
                trap->S_StartSound(&es->origin, es->number, CHAN_AUTO, hitSound);
                if (es->eventParm == 3) { // moderate or big hits.
                    trap->FX_PlayEffectID(hitPersonSmallFxID, &es->origin, &fxDir, -1, -1, qfalse);
                } else if (es->eventParm == 2) { // this is for really big hits.
                    trap->FX_PlayEffectID(hitPersonMidFxID, &es->origin, &fxDir, -1, -1, qfalse);
                } else { // this should really just be done in the effect itself, no?
                    trap->FX_PlayEffectID(hitPersonFxID, &es->origin, &fxDir, -1, -1, qfalse);
                    trap->FX_PlayEffectID(hitPersonFxID, &es->origin, &fxDir, -1, -1, qfalse);
                    trap->FX_PlayEffectID(hitPersonFxID, &es->origin, &fxDir, -1, -1, qfalse);
                }
    ...
    case EV_SABER_BLOCK:
        ...
        {
            // Raz: Ignore blocks if we're in a duel
            if (cg.snap->ps.duelInProgress && es->otherEntityNum2 != cg.snap->ps.clientNum && es->otherEntityNum2 != cg.snap->ps.duelIndex)
                return;

            if (es->eventParm) { // saber block
                qboolean cullPass = qfalse;
                int blockFXID = media.efx.saberBlock;
                qhandle_t blockSound = trap->S_RegisterSound(va("sound/weapons/saber/saberblock%d.wav", Q_irand(1, 9)));
                qboolean noFlare = qfalse;

                if (es->otherEntityNum2 >= 0 && es->otherEntityNum2 < ENTITYNUM_NONE) { // we have a specific person who is causing this effect, see if we
                                                                                        // should override it with any custom saber effects/sounds
                    clientInfo_t *client = NULL;
                    if (cg_entities[es->otherEntityNum2].currentState.eType == ET_NPC) {
                        client = cg_entities[es->otherEntityNum2].npcClient;
                    } else if (es->otherEntityNum2 < MAX_CLIENTS) {
                    ...
                    if (client && client->infoValid) {
                        ...
                        int bladeNum = es->legsAnim;
                        if (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum)) { // use second blade style values
                            if (client->saber[saberNum].blockEffect2) {                             // custom saber block effect
                                blockFXID = client->saber[saberNum].blockEffect2;
                            }
                            if (client->saber[saberNum].block2Sound[0]) { // custom hit sound
                                blockSound = client->saber[saberNum].block2Sound[Q_irand(0, 2)];
                            }
                        } else {
                            if (client->saber[saberNum].blockEffect) { // custom saber block effect
                                blockFXID = client->saber[saberNum].blockEffect;
                            }
                            if (client->saber[saberNum].blockSound[0]) { // custom hit sound
                ...
                if (cg.mInRMG) {
                    ...
                    VectorSubtract(&refdef->vieworg, &es->origin, &vecSub);
            ...
            } else { // projectile block
                vector3 fxDir;
                VectorCopy(&es->angles, &fxDir);
                if (!fxDir.x && !fxDir.y && !fxDir.z) {
                    fxDir.y = 1;
                }
                trap->FX_PlayEffectID(media.efx.blasterDeflect, &es->origin, &fxDir, -1, -1, qfalse);
            }
        }
        break;
    ...
    case EV_BECOME_JEDIMASTER:
        DEBUGNAME("EV_BECOME_JEDIMASTER");
        {
            ...
            trap->FX_PlayEffectID(media.efx.jediSpawn, &pos, &ang, -1, -1, qfalse);

            trap->S_StartSound(NULL, es->number, CHAN_AUTO, trap->S_RegisterSound("sound/weapons/saber/saberon.wav"));

            if (cg.snap->ps.clientNum == es->number) {
                trap->S_StartLocalSound(media.sounds.happyMusic, CHAN_LOCAL);
    ...
    case EV_DISRUPTOR_MAIN_SHOT:
        ...
        if (cent->currentState.eventParm != cg.snap->ps.clientNum || cg.renderingThirdPerson || cg_trueGuns.integer ||
            cg.predictedPlayerState.weapon == WP_SABER || cg.predictedPlayerState.weapon == WP_MELEE) { // h4q3ry
            CG_GetWeaponMuzzleBolt(cent->currentState.eventParm, &cent->currentState.origin2);
        } else {
            if (cg.lastFPFlashPoint.x || cg.lastFPFlashPoint.y ||
                cg.lastFPFlashPoint.z) { // get the position of the muzzle flash for the first person weapon model from the last frame
    ...
    case EV_DISRUPTOR_SNIPER_SHOT:
        ...
        if (cent->currentState.eventParm != cg.snap->ps.clientNum || cg.renderingThirdPerson) { // h4q3ry
            CG_GetWeaponMuzzleBolt(cent->currentState.eventParm, &cent->currentState.origin2);
        } else {
            if (cg.lastFPFlashPoint.x || cg.lastFPFlashPoint.y ||
                cg.lastFPFlashPoint.z) { // get the position of the muzzle flash for the first person weapon model from the last frame
    ...
    case EV_SCREENSHAKE:
        ...
        if (!es->modelindex || cg.predictedPlayerState.clientNum == es->modelindex - 1) {
            CGCam_Shake(es->angles.x, es->time);
        }
        break;
    case EV_LOCALTIMER:
    ...
    case EV_PLAYER_TELEPORT_IN:
        DEBUGNAME("EV_PLAYER_TELEPORT_IN");
        {
            ...
            trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.teleIn);

            if (tr.fraction == 1) {
                break;
            }
            trap->FX_PlayEffectID(media.efx.spawn, &pos, &ang, -1, -1, qfalse);
        }
        break;
    ...
    case EV_PLAYER_TELEPORT_OUT:
        DEBUGNAME("EV_PLAYER_TELEPORT_OUT");
        {
            ...
            trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.teleOut);
    ...
    case EV_ITEM_POP:
        ...
        trap->S_StartSound(NULL, es->number, CHAN_AUTO, media.sounds.environment.respawn);
        break;
    ...
    case EV_SIEGE_OBJECTIVECOMPLETE:
        ...
        CG_SiegeObjectiveCompleted(&cg_entities[cent->currentState.weapon], cent->currentState.eventParm, cent->currentState.trickedEntIndex[0]);
        break;
    ...
    case EV_DESTROY_GHOUL2_INSTANCE:
        ...
        if (cg_entities[es->eventParm].ghoul2 && trap->G2_HaveWeGhoul2Models(cg_entities[es->eventParm].ghoul2)) {
            if (es->eventParm < MAX_CLIENTS) { // You try to do very bad thing!
#ifdef _DEBUG
    ...
    case EV_GIVE_NEW_RANK:
        ...
        if ((signed)es->trickedEntIndex[0] == cg.snap->ps.clientNum) {
            ...
            if (!(trap->Key_GetCatcher() & KEYCATCH_UI) && !es->bolt1 && !cg.demoPlayback) {
    ...
    case EV_CONC_ALT_IMPACT:
        DEBUGNAME("EV_CONC_ALT_IMPACT");
        {
            ...
            for (dist = 0.0f; dist < shotDist; dist += 64.0f) { // one effect would be.. a whole lot better
                VectorMA(&es->origin2, dist, &es->angles, &spot);
                trap->FX_PlayEffectID(media.efx.concussion.altRing, &spot, &es->angles2, -1, -1, qfalse);
            }
    ...
    case EV_MISSILE_STICK:
        ...
        //		trap->S_StartSound (NULL, es->number, CHAN_AUTO, media.sounds.weapons.missileStick );
        break;

    case EV_MISSILE_HIT:
        DEBUGNAME("EV_MISSILE_HIT");
        ByteToDir(es->eventParm, &dir);
        if (es->emplacedOwner) { // hack: this is an index to a custom effect to use
            trap->FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, &dir, -1, -1, qfalse);
        } else if (CG_VehicleWeaponImpact(cent)) { // a vehicle missile that uses an overridden impact effect...
    ...
    case EV_MISSILE_MISS:
        ...
        if (es->emplacedOwner) { // hack: this is an index to a custom effect to use
            trap->FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, &dir, -1, -1, qfalse);
        } else if (CG_VehicleWeaponImpact(cent)) { // a vehicle missile that used an overridden impact effect...
        } else if (cent->currentState.eFlags & EF_ALT_FIRING) {
    ...
    case EV_PLAY_EFFECT:
        ...
        switch (es->eventParm) { // it isn't a hack, it's ingenuity!
        case EFFECT_SMOKE:
            eID = media.efx.emplacedDeadSmoke;
            break;
        case EFFECT_EXPLOSION:
            eID = media.efx.emplacedExplode;
            break;
        case EFFECT_EXPLOSION_PAS:
            eID = media.efx.turretExplode;
            break;
        case EFFECT_SPARK_EXPLOSION:
            eID = media.efx.sparkExplosion;
            break;
        case EFFECT_EXPLOSION_TRIPMINE:
            eID = media.efx.tripmine.explosion;
            break;
        case EFFECT_EXPLOSION_DETPACK:
            eID = media.efx.detpack.explosion;
            break;
        case EFFECT_EXPLOSION_FLECHETTE:
            eID = media.efx.flechette.altBlow;
            break;
        case EFFECT_STUNHIT:
            eID = media.efx.stunbaton.fleshImpact;
            break;
        case EFFECT_EXPLOSION_DEMP2ALT:
            FX_DEMP2_AltDetonate(&cent->lerpOrigin, es->weapon);
            eID = media.efx.demp2.altDetonate;
            break;
        case EFFECT_EXPLOSION_TURRET:
            eID = media.efx.turretExplode;
            break;
        case EFFECT_SPARKS:
            eID = media.efx.sparksExplodeNoSound;
            break;
        case EFFECT_WATER_SPLASH:
            eID = media.efx.splash.water;
            break;
        case EFFECT_ACID_SPLASH:
            eID = media.efx.splash.acid;
            break;
        case EFFECT_LAVA_SPLASH:
            eID = media.efx.splash.lava;
            break;
        case EFFECT_LANDING_MUD:
            eID = media.efx.landing.mud;
            break;
        case EFFECT_LANDING_SAND:
            eID = media.efx.landing.sand;
            break;
        case EFFECT_LANDING_DIRT:
            eID = media.efx.landing.dirt;
            break;
        case EFFECT_LANDING_SNOW:
            eID = media.efx.landing.snow;
            break;
        case EFFECT_LANDING_GRAVEL:
            eID = media.efx.landing.gravel;
            break;
        default:
            eID = -1;
            break;
    ...
    case EV_PLAY_PORTAL_EFFECT_ID:
        DEBUGNAME("EV_PLAY_EFFECT_ID");
        {
            ...
            if (event == EV_PLAY_PORTAL_EFFECT_ID) { // This effect should only be played inside sky portals.
                portalEffect = qtrue;
            }
    ...
    case EV_GLOBAL_TEAM_SOUND: // play from the player's head so it never diminishes
    {
        ...
        case GTS_RED_CAPTURE: // CTF: red team captured the blue flag, 1FCTF: red team captured the neutral flag
            // CG_AddBufferedSound( media.sounds.team.redScored );
            break;
        case GTS_BLUE_CAPTURE: // CTF: blue team captured the red flag, 1FCTF: blue team captured the neutral flag
            // CG_AddBufferedSound( media.sounds.team.blueScored );
            break;
        case GTS_RED_RETURN: // CTF: blue flag returned, 1FCTF: never used
            if (cgs.gametype == GT_CTY) {
                CG_AddBufferedSound(media.sounds.team.blueYsalReturned);
            } else {
                CG_AddBufferedSound(media.sounds.team.blueFlagReturned);
            }
            break;
        case GTS_BLUE_RETURN: // CTF red flag returned, 1FCTF: neutral flag returned
            if (cgs.gametype == GT_CTY) {
                CG_AddBufferedSound(media.sounds.team.redYsalReturned);
            } else {
                CG_AddBufferedSound(media.sounds.team.redFlagReturned);
            }
            break;
        ...
        case GTS_RED_TAKEN: // CTF: red team took blue flag, 1FCTF: blue team took the neutral flag
            ...
            if (cgs.gametype == GT_CTY) {
                CG_AddBufferedSound(media.sounds.team.redTookYsal);
            } else {
                CG_AddBufferedSound(media.sounds.team.redTookFlag);
            }
            break;
        case GTS_BLUE_TAKEN: // CTF: blue team took the red flag, 1FCTF red team took the neutral flag
            // if this player picked up the flag then a sound is played in CG_CheckLocalSounds
            if (cgs.gametype == GT_CTY) {
                CG_AddBufferedSound(media.sounds.team.blueTookYsal);
            } else {
                CG_AddBufferedSound(media.sounds.team.blueTookFlag);
            }
            break;
        ...
        case GTS_REDTEAM_TOOK_LEAD:
            CG_AddBufferedSound(media.sounds.team.redLeads);
            break;
        case GTS_BLUETEAM_TOOK_LEAD:
            CG_AddBufferedSound(media.sounds.team.blueLeads);
            break;
        case GTS_TEAMS_ARE_TIED:
            CG_AddBufferedSound(media.sounds.team.teamsTied);
            break;
        default:
            break;
        }
    ...
    case EV_DEBRIS:
        DEBUGNAME("EV_DEBRIS");
        CG_Chunks(es->owner, &es->origin, &es->angles, &es->origin2, &es->angles2, es->speed, es->eventParm, (material_t)es->trickedEntIndex[0], es->modelindex,
                  es->apos.trBase.x);
        break;
    ...
    case EV_DEATH3:
        ...
        if (es->eventParm && es->number == cg.snap->ps.clientNum) {
            trap->S_StartLocalSound(media.sounds.dramaticFailure, CHAN_LOCAL);
            CGCam_SetMusicMult(0.3f, 5000);
        }
        break;
    ...
    case EV_POWERUP_QUAD:
        ...
        // trap->S_StartSound (NULL, es->number, CHAN_ITEM, media.sounds.quad );
        break;
    case EV_POWERUP_BATTLESUIT:
        ...
        // trap->S_StartSound (NULL, es->number, CHAN_ITEM, media.sounds.force.protect );
        break;
    ...
    case EV_FORCE_DRAINED:
        ...
        trap->S_StartSound(NULL, es->owner, CHAN_AUTO, media.sounds.force.drain);
    ...
    case EV_GIB_PLAYER:
        ...
        // trap->S_StartSound( NULL, es->number, CHAN_BODY, media.sounds.gib );
    ...
    case EV_WEAPON_CHARGE:
        ...
        if (cg_weapons[es->eventParm].chargeSound) {
            ...
            trap->S_StartSound(NULL, es->number, CHAN_WEAPON, media.sounds.weapons.zoomLoop);
        }
        break;
