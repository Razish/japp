...
static void CG_UseItem( centity_t *cent ) {
	...
	if ( cg.snap && cg.snap->ps.clientNum == cent->currentState.number && itemNum != HI_BINOCULARS &&
		itemNum != HI_JETPACK && itemNum != HI_HEALTHDISP && itemNum != HI_AMMODISP && itemNum != HI_CLOAK && itemNum != HI_EWEB ) { //if not using binoculars/jetpack/dispensers/cloak, we just used that item up, so switch
...
static void CG_ItemPickup( int itemNum ) {
	...
	if ( cg.snap && bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		if ( 0 == cg_autoSwitch.getInt() ) {
			// don't switch
		}
		else if ( cg_autoSwitch.getInt() == 1 ) { //only autoselect if not explosive ("safe")
		...
		else if ( cg_autoSwitch.getInt() == 2 ) { //autoselect if better
	...
	if ( bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0] &&
		(bg_itemlist[itemNum].giType != IT_TEAM || (bg_itemlist[itemNum].giTag != PW_REDFLAG && bg_itemlist[itemNum].giTag != PW_BLUEFLAG)) ) { //don't print messages for flags, they have their own pickup event broadcasts
		char	text[1024];
		char	upperKey[1024];
...
#define	DEBUGNAME(x) if(cg_debugEvents.getInt()){trap->Print(x"\n");}
...
void CG_EntityEvent( centity_t *cent, vector3 *position ) {
	...
	case EV_GHOUL2_MARK:
		...
		if ( cg_ghoul2Marks.getInt() ) { //Can we put a burn mark on him?
			CG_G2MarkEvent( es );
		}
		break;
	...
	case EV_JUMP:
		DEBUGNAME( "EV_JUMP" );
		if ( cg_jumpSounds.getInt() && (!cg.predictedPlayerState.duelInProgress || (cg.predictedPlayerState.duelIndex == es->clientNum || cg.predictedPlayerState.clientNum == es->clientNum)) ) {//Raz: duel isolation
		...
		if ( es->number == cg.snap->ps.clientNum ) {
			if ( CG_InFighter() || CG_InATST() || cg.snap->ps.weapon == WP_NONE ) {//just letting us know our vehicle is out of ammo
				...
				if ( cg_vehicleAmmoWarningTime < cg.time
					|| cg_vehicleAmmoWarning != es->eventParm ) {//if there's already one going, don't interrupt it (unless they tried to fire another weapon that's out of ammo)
	...
	case EV_SABER_HIT:
		DEBUGNAME( "EV_SABER_HIT" );
		{
			...
			if ( es->otherEntityNum2 >= 0
				&& es->otherEntityNum2 < ENTITYNUM_NONE ) {//we have a specific person who is causing this effect, see if we should override it with any custom saber effects/sounds
				clientInfo_t *client = NULL;
	...
	case EV_SABER_UNHOLSTER:
		DEBUGNAME( "EV_SABER_UNHOLSTER" );
		{
			...
			if ( ci ) {
				if ( ci->saber[0].soundOn )		trap->S_StartSound( NULL, es->number, CHAN_AUTO, ci->saber[0].soundOn );
				if ( ci->saber[1].soundOn )		trap->S_StartSound( NULL, es->number, CHAN_AUTO, ci->saber[1].soundOn );
			}
		}
		break;
	...
	case EV_DISRUPTOR_MAIN_SHOT:
		...
		if ( cent->currentState.eventParm != cg.snap->ps.clientNum ||
			cg.renderingThirdPerson || cg_trueGuns.getInt()
			...
			if ( cg.lastFPFlashPoint.x || cg.lastFPFlashPoint.y || cg.lastFPFlashPoint.z ) { //get the position of the muzzle flash for the first person weapon model from the last frame
				VectorCopy( &cg.lastFPFlashPoint, &cent->currentState.origin2 );
			}
		}
		if ( (cg_newFX.getBits() & NEWFX_RUPTOR) )
	...
	case EV_MISSILE_HIT:
		...
		if ( cg_ghoul2Marks.getInt() && es->trickedEntIndex[0] ) {
	...
	case EV_PAIN:
		...
		if ( !cg_oldPainSounds.getInt() || (cent->currentState.number != cg.snap->ps.clientNum) ) {
