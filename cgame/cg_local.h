...
void CG_DrawStringExt(int x, int y, const char *string, const vector4 *setColor, qboolean forceColor, qboolean shadow, int charWidth, int charHeight,
                      int maxChars);
...
void CG_ImpactMark(qhandle_t markShader, const vector3 *origin, const vector3 *dir, float orientation, float r, float g, float b, float a, qboolean alphaFade,
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, uint32_t ownerDrawFlags, int align, float special, float scale,
                  const vector4 *color, qhandle_t shader, uiTextStyle_e textStyle, int font, bool customFont);
