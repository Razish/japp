...
void			CG_AddGhoul2Mark( int shader, float size, vector3 *start, vector3 *end, int entnum, vector3 *entposition, float entangle, void *ghoul2,
					vector3 *scale, int lifeTime );
...
void			CG_Chunks( int owner, vector3 *origin, const vector3 *normal, const vector3 *mins, const vector3 *maxs, float speed, int numChunks,
					material_t chunkType, int customChunk, float baseScale );
...
void			CG_Draw3DModel( float x, float y, float w, float h, qhandle_t model, void *ghoul2, int g2radius, qhandle_t skin, vector3 *origin,
					vector3 *angles );
...
void			CG_DrawStringExt( int x, int y, const char *string, const vector4 *setColor, qboolean forceColor, qboolean shadow, int charWidth,
...
void			CG_ImpactMark( qhandle_t markShader, const vector3 *origin, const vector3 *dir, float orientation, float r, float g, float b, float a,
