...
void CG_DrawPic(float x, float y, float width, float height, qhandle_t hShader) { trap->R_DrawStretchPic(x, y, width, height, 0, 0, 1, 1, hShader); }
...
void CG_TileClear(void) {
    ...
    if (refdef->x == 0 && refdef->y == 0 && refdef->width == w && refdef->height == h) {
