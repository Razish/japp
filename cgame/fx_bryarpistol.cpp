...
void FX_BryarHitWall(vector3 *origin, vector3 *normal) { trap->FX_PlayEffectID(media.efx.pistol.wallImpact, origin, normal, -1, -1, qfalse); }
...
void FX_TurretHitWall(vector3 *origin, vector3 *normal) { trap->FX_PlayEffectID(media.efx.pistol.wallImpact, origin, normal, -1, -1, qfalse); }

void FX_TurretHitPlayer(vector3 *origin, vector3 *normal, qboolean humanoid) {
    trap->FX_PlayEffectID(humanoid ? media.efx.pistol.fleshImpact : media.efx.pistol.droidImpact, origin, normal, -1, -1, qfalse);
}

void FX_ConcussionHitWall(vector3 *origin, vector3 *normal) { trap->FX_PlayEffectID(media.efx.concussion.impact, origin, normal, -1, -1, qfalse); }
...
void FX_ConcAltShot(vector3 *start, vector3 *end) {
    trap->FX_AddLine(start, end, 0.1f, 10.0f, 0.0f, 1.0f, 0.0f, 0.0f, &WHITE, &WHITE, 0.0f, 175, trap->R_RegisterShader("gfx/effects/blueLine"),
                     FX_SIZE_LINEAR | FX_ALPHA_LINEAR);

    // add some beef
    trap->FX_AddLine(start, end, 0.1f, 7.0f, 0.0f, 1.0f, 0.0f, 0.0f, &BRIGHT, &BRIGHT, 0.0f, 150, trap->R_RegisterShader("gfx/misc/whiteline2"),
                     FX_SIZE_LINEAR | FX_ALPHA_LINEAR);
}
