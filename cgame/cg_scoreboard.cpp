...
static void CG_DrawClientScore(int y, score_t *score, const vector4 *color, float fade, qboolean largeFormat) {
    ...
    float scale = largeFormat ? 1.0f : 0.75f, iconSize = largeFormat ? SB_NORMAL_HEIGHT : SB_INTER_HEIGHT;
    ...
    if (ci->powerups & (1 << PW_NEUTRALFLAG)) {
        ...
        CG_DrawPic(iconx, y, iconSize, iconSize,
                   trap->R_RegisterShaderNoMip((ci->duelTeam == DUELTEAM_LONE) ? "gfx/mp/pduel_icon_lone" : "gfx/mp/pduel_icon_double"));
    }
    ...
    Font font(FONT_MEDIUM, 0.9f * scale, false);
    const char *tmp = ci->name;
    font.Paint(SB_NAME_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
    font.scale = scale;

    if (score->ping != -1) {
        if (ci->team != TEAM_SPECTATOR || cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
            if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
                tmp = va("%i/%i", ci->wins, ci->losses);
                font.Paint(SB_SCORE_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
            } else {
                if (Server_Supports(SSF_SCOREBOARD_KD)) {
                    tmp = va("%i/%i", score->score, score->deaths);
                    font.Paint(SB_SCORE_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
                } else {
        ...
        if (cgs.clientinfo[score->client].botSkill != -1 && cg_oldScoreboardShowBots.integer == 2) {
            tmp = "-";
            font.Paint(SB_PING_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
        } else {
            tmp = va("%i", score->ping);
            font.Paint(SB_PING_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
        }
        ...
        tmp = va("%i", score->time);
        font.Paint(SB_TIME_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
    } else {
        tmp = "-";
        font.Paint(SB_SCORE_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
        font.Paint(SB_PING_X, y, tmp, &colorWhite, uiTextStyle_e::Outlined);
    ...
    font.handle = FONT_MEDIUM;
    font.scale = 0.7f * scale;
    // add the "ready" marker for intermission exiting
    if (cg.snap->ps.stats[STAT_CLIENTS_READY] & (1 << score->client)) {
        tmp = CG_GetStringEdString("MP_INGAME", "READY");
        font.Paint(cg_oldScoreboardSkinIcons.integer ? 4 : SB_NAME_X - 48, y + 2, tmp, &colorWhite, uiTextStyle_e::Outlined);
...
qboolean CG_DrawOldScoreboard(void) {
    ...
    Font font(FONT_MEDIUM, 1.0f, false);

    // fragged by ... line
    // or if in intermission and duel, prints the winner of the duel round
    if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) && cgs.duelWinner != -1 && cg.predictedPlayerState.pm_type == PM_INTERMISSION) {
        s = va("%s" S_COLOR_WHITE " %s", cgs.clientinfo[cgs.duelWinner].name, CG_GetStringEdString("MP_INGAME", "DUEL_WINS"));
        x = (SCREEN_WIDTH) / 2;
        y = 40;
        font.Paint(x - font.Width(s) / 2, y, s, &colorWhite, uiTextStyle_e::Outlined);
        ...
        if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
            s = va("%s" S_COLOR_WHITE " %s %s" S_COLOR_WHITE " %s %s", cgs.clientinfo[cgs.duelist1].name, CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
                   cgs.clientinfo[cgs.duelist2].name, CG_GetStringEdString("MP_INGAME", "AND"), cgs.clientinfo[cgs.duelist3].name);
        } else {
            s = va("%s" S_COLOR_WHITE " %s %s", cgs.clientinfo[cgs.duelist1].name, CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
    ...
    font.scale = 0.75f;
    font.Paint(x - (font.Width(s) / 2), y, s, &colorTable[CT_WHITE], uiTextStyle_e::Outlined);
    ...
    if (cgs.gametype >= GT_TEAM) {
        ...
        s = va("Players: " S_COLOR_GREEN "%i" S_COLOR_WHITE "/" S_COLOR_GREEN "%i " S_COLOR_WHITE "(" S_COLOR_RED "%i" S_COLOR_WHITE "/" S_COLOR_CYAN
               "%i" S_COLOR_WHITE ") - %i spectators",
               cg.numScores, cgs.maxclients, redCount, blueCount, specCount);
    } else {
        int specCount = 0;
        for (i = 0; i < cg.numScores; i++) {
    ...
    font.Paint(x - (font.Width(s) / 2), y + 15, s, &colorTable[CT_WHITE], uiTextStyle_e::Outlined);

    s = va("%s (%s)", (char *)CG_ConfigString(CS_MESSAGE), cgs.mapname);
    font.Paint(x - (font.Width(s) / 2), y + 30, s, &colorTable[CT_WHITE], uiTextStyle_e::Outlined);
    ...
    s = CG_GetStringEdString("MP_INGAME", "NAME");
    font.Paint(SB_NAME_X, y, s, &colorWhite, uiTextStyle_e::Outlined);
    if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
        char sWL[100];
        trap->SE_GetStringTextString("MP_INGAME_W_L", sWL, sizeof(sWL));

        font.Paint(SB_SCORE_X, y, sWL, &colorWhite, uiTextStyle_e::Outlined);
    } else {
        s = CG_GetStringEdString("MP_INGAME", "SCORE");
        font.Paint(SB_SCORE_X, y, s, &colorWhite, uiTextStyle_e::Outlined);
    }
    s = CG_GetStringEdString("MP_INGAME", "PING");
    font.Paint(SB_PING_X, y, s, &colorWhite, uiTextStyle_e::Outlined);
    s = CG_GetStringEdString("MP_INGAME", "TIME");
    font.Paint(SB_TIME_X, y, s, &colorWhite, uiTextStyle_e::Outlined);
    ...
    if (cgs.gametype >= GT_TEAM) {
        ...
        if (cg.teamScores[0] >= cg.teamScores[1]) {
            ...
            CG_DrawTeamBackground(SB_SCORELINE_X - 5, y - topBorderSize, SCREEN_WIDTH - SB_SCORELINE_X * 2 + 10, n1 * lineHeight + bottomBorderSize, 0.33f,
                                  TEAM_RED);
            ...
            CG_DrawTeamBackground(SB_SCORELINE_X - 5, y - topBorderSize, SCREEN_WIDTH - SB_SCORELINE_X * 2 + 10, n2 * lineHeight + bottomBorderSize, 0.33f,
                                  TEAM_BLUE);
