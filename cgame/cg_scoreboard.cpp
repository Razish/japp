...
static void CG_DrawClientScore( int y, score_t *score, const vector4 *color, float fade, qboolean largeFormat ) {
	...
	if ( cg.snap->ps.stats[STAT_CLIENTS_READY] & (1 << score->client) ) {
		...
		font.Paint(
			cg_oldScoreboardSkinIcons.integer ? 4 : SB_NAME_X - 48, y + 2, tmp, &colorWhite, uiTextStyle_e::Outlined
		);
	}
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		s = va(
			"Players: " S_COLOR_GREEN "%i" S_COLOR_WHITE "/" S_COLOR_GREEN "%i " S_COLOR_WHITE "(" S_COLOR_RED "%i"
				S_COLOR_WHITE "/" S_COLOR_CYAN "%i" S_COLOR_WHITE ") - %i spectators",
