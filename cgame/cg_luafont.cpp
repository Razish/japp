...
int FontPropertyCompare(const void *a, const void *b) { return strcmp((const char *)a, ((fontProperty_t *)b)->name); }
...
static void Font_SetIndex(lua_State *L, luaFont_t *font) { font->index = luaL_checkinteger(L, 3); }

static const fontProperty_t fontProperties[] = {{"index", Font_GetIndex, Font_SetIndex}};
...
static int Font_Index(lua_State *L) {
    ...
    // assume it's a field
    const fontProperty_t *property = (fontProperty_t *)bsearch(key, fontProperties, numFontProperties, sizeof(fontProperty_t), FontPropertyCompare);
    if (property) {
        if (property->Get) {
...
static const struct luaL_Reg fontMeta[] = {
    {"__index", Font_Index}, {"__newindex", Font_NewIndex}, {"__eq", Font_Equals}, {"__tostring", Font_ToString}, {NULL, NULL}};
