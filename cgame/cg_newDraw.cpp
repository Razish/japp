...
void CG_Text_Paint_Limit(float *maxX, float x, float y, float scale, const vector4 *color, const char *text, float adjust, int limit, int iMenuFont,
    ...
    if (x + iPixelLen > *maxX) {
        ...
        while (*psText && (x + font.Width(sTemp) <= *maxX) && psOut < &sTemp[sizeof(sTemp) - 1]) {
            int iAdvanceCount;
            psOutLastGood = psOut;
...
void CG_DrawTeamSpectators(rectDef_t *rect, float scale, const vector4 *color, qhandle_t shader) {
    if (cg.spectatorLen) {
        ...
        CG_Text_Paint_Limit(&maxX, cg.spectatorPaintX, rect->y + rect->h - 3, scale, color, &cg.spectatorList[cg.spectatorOffset], 0, 0, FONT_MEDIUM, false);
        if (cg.spectatorPaintX2 >= 0) {
            float maxX2 = rect->x + rect->w - 2;
            CG_Text_Paint_Limit(&maxX2, cg.spectatorPaintX2, rect->y + rect->h - 3, scale, color, cg.spectatorList, 0, cg.spectatorOffset, FONT_MEDIUM, false);
        }
...
void CG_KeyEvent(int key, qboolean down) {
    ...
    if (cg.predictedPlayerState.pm_type == PM_NORMAL || cg.predictedPlayerState.pm_type == PM_JETPACK || cg.predictedPlayerState.pm_type == PM_NORMAL ||
...
qboolean CG_DeferMenuScript(char **args) { return qfalse; }
