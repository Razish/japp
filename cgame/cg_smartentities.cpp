...
void SE_PerformTrace(trace_t *results, vector3 *start, vector3 *end, int mask) { trap->CM_Trace(results, start, end, NULL, NULL, 0, mask, qfalse); }
...
qboolean SE_RenderIsVisible(vector3 *startPos, vector3 *testOrigin, qboolean reversedCheck) {
    ...
    if (results.fraction < 1.0f) {
        if ((results.surfaceFlags & SURF_FORCEFIELD)
            ...
            || (results.surfaceFlags & MATERIAL_MASK) == MATERIAL_GLASS || (results.surfaceFlags & MATERIAL_MASK) == MATERIAL_SHATTERGLASS) {
