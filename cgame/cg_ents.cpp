...
void CG_S_UpdateLoopingSounds(int entityNum) {
    ...
    /*	Sil: well with looping sounds, you would notice it playing with force powers
            ...
            */
    if ((cent->currentState.eFlags & EF_SOUNDTRACKER) && (!cg.snap || (signed)cent->currentState.trickedEntIndex[0] != cg.snap->ps.clientNum)) {
...
static void CG_EntityEffects(centity_t *cent) {
    ...
    if (cent->currentState.loopSound || (cent->currentState.loopIsSoundset && cent->currentState.number >= MAX_CLIENTS)) {
        ...
        if (cent->currentState.loopIsSoundset &&
            cent->currentState.number >=
                MAX_CLIENTS) { // If this is so, then first get our soundset from the index, and loopSound actually contains which part of the set to
    ...
    if (cent->currentState.constantLight && cent->currentState.eType != ET_PLAYER && cent->currentState.eType != ET_BODY &&
        cent->currentState.eType != ET_NPC && cent->currentState.eType != ET_INVISIBLE) {
        //	if ( cent->currentState.constantLight && cent->currentState.eType != ET_PLAYER && cent->currentState.eType != ET_BODY ) {
        int cl;
        float i, r, g, b;
...
localEntity_t *FX_AddOrientedLine(vector3 *start, vector3 *end, vector3 *normal, float stScale, float scale, float dscale, float startalpha, float endalpha,
...
void G2_BoltToGhoul2Model(centity_t *cent, refEntity_t *ent) {
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, modelNum, boltNum, &boltMatrix, &cg_entities[entNum].currentState.angles, &cg_entities[entNum].currentState.origin,
...
void CG_Disintegration(centity_t *cent, refEntity_t *ent) {
    ...
    if (cg.time - ent->endTime < 1000 && (timescale.value * timescale.value * random()) > 0.05f) {
        ...
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, torsoBolt, &boltMatrix, &cent->lerpAngles, &cent->lerpOrigin, cg.time, cgs.gameModels, &cent->modelScale);
...
static void CG_General(centity_t *cent) {
    ...
    if (cent->ghoul2 && (cent->currentState.eType == ET_BODY || (cent->currentState.eFlags & EF_RAG))) {
        if (!(cent->currentState.eFlags & EF_NODRAW) && !(cent->currentState.eFlags & EF_DISINTEGRATION) && cent->bodyFadeTime <= cg.time) {
        ...
        if (cent->ghoul2 &&
            trap->G2_HaveWeGhoul2Models(cent->ghoul2)) { // May not be valid, in the case of a ragged entity being removed and a non-g2 ent filling its slot.
    ...
    if ((cent->currentState.eFlags & EF_G2ANIMATING) && cent->ghoul2) { // mini-animation routine for general objects that want to play quick ghoul2 anims
        ...
        if (cent->currentState.torsoAnim != cent->pe.torso.animationNumber || cent->currentState.legsAnim != cent->pe.legs.animationNumber ||
            cent->currentState.torsoFlip != cent->pe.torso.lastFlip) {
            trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", cent->currentState.torsoAnim, cent->currentState.legsAnim,
                                    (BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND), 1.0f, cg.time, -1, 100);
    ...
    if (cent->currentState.modelGhoul2 >= G2_MODELPART_HEAD && cent->currentState.modelGhoul2 <= G2_MODELPART_RLEG &&
        ...
        if (cg_dismember.integer < 2 &&
            (cent->currentState.modelGhoul2 == G2_MODELPART_HEAD ||
             cent->currentState.modelGhoul2 == G2_MODELPART_WAIST)) { // dismember settings are not high enough to display decaps and torso slashes
            return;
        }
        ...
        if (!cent->ghoul2) {
            ...
            if (clEnt && !(clEnt->currentState.eFlags &
                           EF_DEAD)) { // death flag hasn't made it through yet for the limb owner, we cannot create the limb until he's flagged as dead
                return;
            }

            if (clEnt && (!BG_InDeathAnim(clEnt->currentState.torsoAnim) ||
                          !BG_InDeathAnim(clEnt->pe.torso.animationNumber))) {   // don't make it unless we're in an actual death anim already
            ...
            if (clEnt && clEnt->ghoul2) {
                if (trap->G2API_HasGhoul2ModelOnIndex(&(clEnt->ghoul2),
                                                      2)) { // don't want to bother dealing with a second saber on limbs and stuff, just remove the thing
                ...
                if (clEnt->localAnimIndex <= 0) { // humanoid
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "model_root", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 100, cg.time);
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "pelvis", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 0, cg.time);
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "thoracic", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 0, cg.time);
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "upper_lumbar", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 100, cg.time);
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "lower_lumbar", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 100, cg.time);
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "cranium", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X,
                                              cgs.gameModels, 100, cg.time);
                } else {
                    trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "model_root", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                              cgs.gameModels, 100, cg.time);
            ...
            if (newBolt != -1) {
                ...
                trap->G2API_GetBoltMatrix(clEnt->ghoul2, 0, newBolt, &matrix, &clEnt->lerpAngles, &clEnt->lerpOrigin, cg.time, cgs.gameModels,
                                          &clEnt->modelScale);
            ...
            if (cent->currentState.modelGhoul2 == G2_MODELPART_RARM || cent->currentState.modelGhoul2 == G2_MODELPART_RHAND ||
                cent->currentState.modelGhoul2 == G2_MODELPART_WAIST) { // Cut his weapon holding arm off, so remove the weapon
        ...
        if (cent->ghoul2 && cent->bolt4 != -1 && cent->trailTime < cg.time) {
            if (cent->bolt4 != -1 && (cent->currentState.pos.trDelta.x || cent->currentState.pos.trDelta.y || cent->currentState.pos.trDelta.z)) {
                vector3 boltOrg, boltAng;

                trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt4, &matrix, &cent->lerpAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                          &cent->modelScale);
    ...
    if (cent->currentState.number >= MAX_CLIENTS && cent->currentState.activeForcePass == NUM_FORCE_POWERS + 1 &&
    ...
    */
    ...
    if (s1->eType == ET_BODY) { // bodies should have a radius as well
        ...
        if (cent->ghoul2) { // all bodies should already have a ghoul2 instance. Use it to set the torso/head angles to 0.
            ...
            trap->G2API_SetBoneAngles(cent->ghoul2, 0, "pelvis", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0,
                                      cg.time);
            trap->G2API_SetBoneAngles(cent->ghoul2, 0, "thoracic", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0,
                                      cg.time);
            trap->G2API_SetBoneAngles(cent->ghoul2, 0, "upper_lumbar", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels,
                                      100, cg.time);
            trap->G2API_SetBoneAngles(cent->ghoul2, 0, "lower_lumbar", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels,
                                      100, cg.time);
            trap->G2API_SetBoneAngles(cent->ghoul2, 0, "cranium", &vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100,
                                      cg.time);
        }
    }
    ...
    if ((cent->currentState.eFlags & EF_DISINTEGRATION) && cent->currentState.eType == ET_BODY) {
        ...
        if (cent->bodyFadeTime > cg.time) {
            ...
            */
            ...
            if (curTimeDif < 3400) {
                if (lightSide) {
                    if (curTimeDif < 2200 && cg.frametime > 0 &&
                        ((cg.frametime < 50 && cg.time % 50 <= cg.frametime) || cg.frametime >= 50) // probably temporary
                    ...
                    if (random() > 0.9f && cg.frametime > 0 && ((cg.frametime < 50 && cg.time % 50 <= cg.frametime) || cg.frametime >= 50) &&
                        cg_noBodies.integer != 2) {
    ...
    if (cent->currentState.modelGhoul2 && !ent.ghoul2 && !ent.hModel) {
        return;
    }
    ...
    if (cent->bolt3 == 999) { // this is an in-flight saber being rendered manually
        ...
        if (cent->currentState.trickedEntIndex[2] == 1) { // dark
            ...
            if ((s1->modelindex + 128) == FP_SABER_OFFENSE || (s1->modelindex + 128) == FP_SABER_DEFENSE ||
    ...
    if (cent->currentState.time == -1 && cent->currentState.weapon == WP_TRIP_MINE &&
        (cent->currentState.eFlags &
         EF_FIRING)) { // if force sight is active, render the laser multiple times up to the force sight level to increase visibility
...
qboolean CG_GreyItem(int type, int tag, int plSide) {
    if (type == IT_POWERUP && (tag == PW_FORCE_ENLIGHTENED_LIGHT || tag == PW_FORCE_ENLIGHTENED_DARK)) {
...
static void CG_Item(centity_t *cent) {
    ...
    */
    ...
    if ((item->giType == IT_WEAPON || item->giType == IT_POWERUP) && !(cent->currentState.eFlags & EF_DROPPEDWEAPON) && !(cg_simpleItems.value > 0.0f)) {
        vector3 uNorm;
        qboolean doGrey;
    ...
    */
    ...
    if (cg_simpleItems.value > 0.0f && item->giType != IT_TEAM) {
        ...
        if (CG_GreyItem(item->giType, item->giTag, cg.snap->ps.fd.forceSide)) {
            ...
            ent.customShader =
                trap->R_RegisterShader((item->giTag == PW_FORCE_ENLIGHTENED_LIGHT) ? "gfx/misc/mp_light_enlight_disable" : "gfx/misc/mp_dark_enlight_disable");
        }
    ...
    if ((item->giType == IT_WEAPON || item->giType == IT_POWERUP) && !(cent->currentState.eFlags & EF_DROPPEDWEAPON)) {
        cent->lerpOrigin.z += 16;
    }

    if ((!(cent->currentState.eFlags & EF_DROPPEDWEAPON) || item->giType == IT_POWERUP) && (item->giType == IT_WEAPON || item->giType == IT_POWERUP)) {
    ...
    if (!(cent->currentState.eFlags & EF_DROPPEDWEAPON)) {
        if (item->giType == IT_WEAPON) {
            ...
            cent->lerpOrigin.x -= wi->weaponMidpoint.x * ent.axis[0].x + wi->weaponMidpoint.y * ent.axis[1].x + wi->weaponMidpoint.z * ent.axis[2].x;
            cent->lerpOrigin.y -= wi->weaponMidpoint.x * ent.axis[0].y + wi->weaponMidpoint.y * ent.axis[1].y + wi->weaponMidpoint.z * ent.axis[2].y;
            cent->lerpOrigin.z -= wi->weaponMidpoint.x * ent.axis[0].z + wi->weaponMidpoint.y * ent.axis[1].z + wi->weaponMidpoint.z * ent.axis[2].z;

            cent->lerpOrigin.z += 8; // an extra height boost
        }
    } else {
        if (item->giTag == WP_BLASTER)
            cent->lerpOrigin.z -= 12;
        else if (item->giTag == WP_DISRUPTOR)
            cent->lerpOrigin.z -= 13;
        else if (item->giTag == WP_BOWCASTER)
            cent->lerpOrigin.z -= 16;
        else if (item->giTag == WP_REPEATER)
            cent->lerpOrigin.z -= 12;
        else if (item->giTag == WP_DEMP2)
            cent->lerpOrigin.z -= 10;
        else if (item->giTag == WP_FLECHETTE)
            cent->lerpOrigin.z -= 6;
        else if (item->giTag == WP_ROCKET_LAUNCHER)
            cent->lerpOrigin.z -= 11;
        else if (item->giTag == WP_THERMAL)
            cent->lerpOrigin.z -= 12;
        else if (item->giTag == WP_TRIP_MINE)
            cent->lerpOrigin.z -= 16;
        else if (item->giTag == WP_DET_PACK)
            cent->lerpOrigin.z -= 16;
        else
            cent->lerpOrigin.z -= 8;
    }
    ...
    */
    ...
    if (!(cent->currentState.eFlags & EF_DROPPEDWEAPON) && (item->giType == IT_WEAPON || item->giType == IT_POWERUP)) {
        ent.renderfx |= RF_MINLIGHT;
    }

    if (item->giType != IT_TEAM && msec >= 0 && msec < ITEM_SCALEUP_TIME && !(es->eFlags & EF_ITEMPLACEHOLDER) &&
        !(es->eFlags & EF_DROPPEDWEAPON)) { // if just respawned, fade in, but don't do this for flags.
        float alpha;
        int a;
        ...
        if (item->giType != IT_POWERUP ||
            item->giTag != PW_FORCE_BOON) { // boon model uses a different blending mode for the sprite inside and doesn't look proper with this method
            ent.renderfx |= RF_FORCE_ENT_ALPHA;
        }
        SE_R_AddRefEntityToScene(&ent, cent->currentState.number);
...
static void CG_Missile(centity_t *cent) {
    ...
    if (s1->weapon == WP_STUN_BATON) {
        if (s1->generic1) { // Raz: Portals
            ...
            trap->G2API_GetBoltMatrix(cg_entities[clientNum].ghoul2, 0, cgs.clientinfo[clientNum].bolt_rhand, &boltMatrix, &cg_entities[clientNum].turAngles,
    ...
    if (s1->weapon == WP_SABER) {
        if ((cent->currentState.modelindex != cent->serverSaberHitIndex || !cent->ghoul2) &&
            !(s1->eFlags & EF_NODRAW)) { // no g2, or server changed the model we are using
    ...
    if (s1->otherEntityNum2 && s1->weapon != WP_SABER) { // using an over-ridden trail effect!
        ...
        if (weapon->altMissileDlight) {
            trap->R_AddLightToScene(&cent->lerpOrigin, weapon->altMissileDlight, weapon->altMissileDlightColor.r, weapon->altMissileDlightColor.g,
                                    weapon->altMissileDlightColor.b);
        }
        ...
        if (weapon->missileDlight) {
            trap->R_AddLightToScene(&cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor.r, weapon->missileDlightColor.g,
                                    weapon->missileDlightColor.b);
        }
...
static void CG_Mover(centity_t *cent) {
    ...
    if ((cent->currentState.eFlags2 & EF2_HYPERSPACE)) { // I'm the hyperspace brush
        ...
        if (cg.predictedPlayerState.m_iVehicleNum && cg.predictedVehicleState.hyperSpaceTime &&
            (cg.time - cg.predictedVehicleState.hyperSpaceTime) < HYPERSPACE_TIME && (cg.time - cg.predictedVehicleState.hyperSpaceTime) > 1000) {
            if (cg.snap && cg.snap->ps.pm_type == PM_INTERMISSION) {          // in the intermission, stop drawing hyperspace ent
...
void CG_CalcEntityLerpPositions(centity_t *cent) {
    ...
    if (cg.predictedPlayerState.m_iVehicleNum && cg.predictedPlayerState.m_iVehicleNum == cent->currentState.number && cent->currentState.eType == ET_NPC &&
        cent->currentState.NPC_class == CLASS_VEHICLE) {
...
static void CG_AddCEntity(centity_t *cent) {
    ...
    if (cg.predictedPlayerState.pm_type == PM_INTERMISSION) { // don't render anything then
        if (cent->currentState.eType == ET_GENERAL || cent->currentState.eType == ET_PLAYER || cent->currentState.eType == ET_INVISIBLE) {
            return;
        }
...
void CG_ManualEntityRender(centity_t *cent) { CG_AddCEntity(cent); }
    ...
    */
    ...
    for (num = 0; num < cg.snap->numEntities; num++) {
        ...
        if (cg.snap->entities[num].number != cg.snap->ps.clientNum) {
            cent = &cg_entities[cg.snap->entities[num].number];
            if (cent->currentState.eType == ET_PLAYER && cent->currentState.m_iVehicleNum) { // add his veh first
                int j = 0;
