...
static float GetSelfLegAnimPoint(void) { return BG_GetLegsAnimPoint(&cg.predictedPlayerState, cg_entities[cg.predictedPlayerState.clientNum].localAnimIndex); }
...
static void CG_SmoothTrueView(vector3 *eyeAngles) {
    ...
    if (cg_trueRoll.integer) {
        if (cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT ||
            cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT_STOP || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
            cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT_FLIP || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT_FLIP ||
            cg.predictedPlayerState.legsAnim == BOTH_WALL_FLIP_LEFT || cg.predictedPlayerState.legsAnim == BOTH_WALL_FLIP_RIGHT) {
    ...
    } else if (cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT ||
               cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT_STOP || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
               cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_LEFT_FLIP || cg.predictedPlayerState.legsAnim == BOTH_WALL_RUN_RIGHT_FLIP ||
               cg.predictedPlayerState.legsAnim == BOTH_WALL_FLIP_LEFT || cg.predictedPlayerState.legsAnim == BOTH_WALL_FLIP_RIGHT ||
    ...
    if (cg_trueSpin.integer) {
        if (cg_trueSpin.integer == 1) {
            ...
            if (cg.predictedPlayerState.torsoAnim == BOTH_T1_TL_BR || cg.predictedPlayerState.torsoAnim == BOTH_T1__L_BR ||
                ...
                cg.predictedPlayerState.torsoAnim == BOTH_CROUCHATTACKBACK1 || cg.predictedPlayerState.torsoAnim == BOTH_BUTTERFLY_LEFT ||
            ...
            if (BG_SpinningSaberAnim(cg.predictedPlayerState.torsoAnim) && cg.predictedPlayerState.torsoAnim != BOTH_JUMPFLIPSLASHDOWN1 &&
    ...
    } else if (BG_SpinningSaberAnim(cg.predictedPlayerState.torsoAnim) && cg.predictedPlayerState.torsoAnim != BOTH_JUMPFLIPSLASHDOWN1 &&
               cg.predictedPlayerState.torsoAnim != BOTH_JUMPFLIPSTABDOWN) {
    ...
    if (cg.predictedPlayerState.torsoAnim == BOTH_JUMPFLIPSLASHDOWN1 || cg.predictedPlayerState.torsoAnim == BOTH_JUMPFLIPSTABDOWN) {
        eyeRange = qfalse;
        didSpecial = qtrue;
    }
...
void CG_TrueView(centity_t *cent) {
    ...
    if (cg.snap && cent->currentState.number == cg.snap->ps.clientNum) {
        if (!cg.renderingThirdPerson && (cg_trueGuns.integer || cent->currentState.weapon == WP_SABER || cent->currentState.weapon == WP_MELEE) &&
            !cg.predictedPlayerState.zoomMode && !cg_fakeGun.integer) {
            ...
            if (!trap->G2API_GetBoltMatrix(cent->ghoul2, 0, eyesBolt, &eyeMatrix, &cent->turAngles, &cent->lerpOrigin, cg.time, cgs.gameModels,
                                           &cent->modelScale)) {
