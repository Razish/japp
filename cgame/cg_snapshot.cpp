...
static void CG_ResetEntity(centity_t *cent) {
    // if the previous snapshot this entity was updated in is at least an event window back in time then we can reset
    //	the previous event
...
static void CG_TransitionSnapshot(void) {
    ...
    if (oldFrame) {
        ...
        if (cg.demoPlayback || CG_IsSpectating() || cg_noPredict.integer || g_synchronousClients.integer || CG_UsingEWeb()) {
...
static snapshot_t *CG_ReadNextSnapshot(void) {
    ...
    while (cgs.processedSnapshotNum < cg.latestSnapshotNum) {
        ...
        if (cg.snap && r && dest->serverTime == cg.snap->serverTime) {
            // According to dumbledore, this situation occurs when you're playing back a demo that was record when the
            //	game was running in local mode.  As such, we need to skip those snaps or the demo looks laggy.
            if (cg.demoPlayback)
                continue;
        }
        ...
        // a GetSnapshot will return failure if the snapshot never arrived, or  is so old that its entities have been
        //	shoved off the end of the circular buffer in the client system.
...
// Even if cg.snap is valid, cg.nextSnap may not be, if the snapshot hasn't arrived yet (it becomes an extrapolating
//	situation instead of an interpolating one)
