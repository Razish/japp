...
static int TextBoxPropertyCompare(const void *a, const void *b) { return strcmp((const char *)a, ((textBoxProperty_t *)b)->name); }
...
static void TextBox_SetColour(lua_State *L, luaTextBox_t *box) { ReadFloats(box->colour.raw, 4, L, 3); }
...
static const textBoxProperty_t textBoxProperties[] = {{"centered", TextBox_GetCentered, TextBox_SetCentered},
                                                      {"colour", TextBox_GetColour, TextBox_SetColour},
                                                      {"font", TextBox_GetFont, TextBox_SetFont},
                                                      {"height", TextBox_GetHeight, nullptr},
                                                      {"scale", TextBox_GetScale, TextBox_SetScale},
                                                      {"style", TextBox_GetStyle, TextBox_SetStyle},
                                                      {"text", TextBox_GetText, TextBox_SetText},
                                                      {"width", TextBox_GetWidth, nullptr}};
...
static int TextBox_Index(lua_State *L) {
    ...
    // assume it's a field
    const textBoxProperty_t *property =
        (textBoxProperty_t *)bsearch(key, textBoxProperties, numTextBoxProperties, sizeof(textBoxProperty_t), TextBoxPropertyCompare);
    if (property) {
        if (property->Get) {
...
static int TextBox_Equals(lua_State *L) {
    ...
    bool eq = (box1->text.compare(box2->text) == 0) && (box1->font == box2->font) && flcmp(box1->scale, box2->scale);
...
static const struct luaL_Reg textBoxMeta[] = {{"__index", TextBox_Index},
