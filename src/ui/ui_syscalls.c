...
{
	float width = (float)syscall( UI_R_FONT_STRLENPIXELS, text, iFontIndex, PASSFLOAT(2.0f));
	return (width/2.0f) * scale;
}
...
{
	float height = (float)syscall( UI_R_FONT_STRHEIGHTPIXELS, iFontIndex, PASSFLOAT(2.0f));
	return (height/2.0f) * scale;
}
...
void trap_R_AddRefEntityToScene( const refEntity_t *re, int gameEntity ) {
	gameEntity++; //Smart entities requires this arg, pretend we use it 8D
