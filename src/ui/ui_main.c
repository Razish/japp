...
*/
//Raz: modified
static const int numNetSources = AS_GLOBAL5+1; //3;	// now hard-entered in StringEd file
...
void AssetCache() {
	...
	uiInfo.uiDC.Assets.sliderThumbDefault = trap_R_RegisterShaderNoMip( ASSET_SLIDER_THUMB_DEFAULT );
...
void Text_Paint(float x, float y, float scale, vector4 *color, const char *text, float adjust, int limit, int style, int iMenuFont)
{
	int iStyleOR = 0;
...
void Text_PaintWithCursor(float x, float y, float scale, vector4 *color, const char *text, int cursorPos, char cursor, int limit, int style, int iMenuFont) 
{
	...
	{
		char sTemp[1024];
		int iCopyCount = limit ? min( strlen( text ), (size_t)limit) : strlen( text );
		...
		{
			int iFontIndex = MenuFontToHandle( iMenuFont );	
			float iNextXpos  = trap_R_Font_StrLenPixels(sTemp, iFontIndex, scale );
...
//
static void Text_Paint_Limit(float *maxX, float x, float y, float scale, vector4 *color, const char* text, float adjust, int limit, int iMenuFont) 
{
	...
	//float fMax = *maxX;
	float iPixelLen = trap_R_Font_StrLenPixels(text, iFontIndex, scale);
	if (x + iPixelLen > *maxX)
	{
...
void Text_PaintCenter(float x, float y, float scale, vector4 *color, const char *text, float adjust, int iMenuFont);
...
void _UI_Refresh( int realtime )
{
	...
	*/
	{//don't draw cursor on loading screen
...
qboolean Asset_Parse(int handle) {
	...
	while ( 1 ) {
		...
		if (Q_stricmp(token.string, "monoFont") == 0) {
			...
			//trap_R_RegisterFont(token, pointSize, &uiInfo.uiDC.Assets.bigFont);
			uiInfo.uiDC.Assets.japp.fontMono = trap_R_RegisterFont(token.string);
			continue;
		}
#endif
...
static void UI_DrawClanName(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
...
{
	Text_Paint(rect->x, rect->y, scale, color, BG_GetGametypeString(uiInfo.gameTypes[ui_gameType.integer].gtEnum), 0, 0, textStyle, iMenuFont);
}

static void UI_DrawNetGameType(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) 
{
	...
	//	Text_Paint(rect->x, rect->y, scale, color, BG_GetGametypeString(uiInfo.gameTypes[ui_netGameType.integer].gtEnum) , 0, 0, textStyle, iMenuFont);
	Text_Paint(rect->x, rect->y, scale, color, uiInfo.gameTypes[ui_netGameType.integer].gameType , 0, 0, textStyle, iMenuFont);
}

static void UI_DrawAutoSwitch(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
...
static void UI_DrawJoinGameType(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) 
{
	...
	//	Text_Paint(rect->x, rect->y, scale, color, BG_GetGametypeString(uiInfo.joinGameTypes[ui_joinGameType.integer].gtEnum) , 0, 0, textStyle, iMenuFont);
...
static void UI_DrawGenericNum(rectDef_t *rect, float scale, vector4 *color, int textStyle, int val, int min, int max, int type,int iMenuFont) 
{
	int i;
	char s[256];
...
static void UI_DrawForceMastery(rectDef_t *rect, float scale, vector4 *color, int textStyle, int val, int min, int max, int iMenuFont)
{
	int i;
	char *s;
...
static void UI_DrawSkinColor(rectDef_t *rect, float scale, vector4 *color, int textStyle, int val, int min, int max, int iMenuFont)
{
	char s[256];
...
static void UI_DrawForceSide(rectDef_t *rect, float scale, vector4 *color, int textStyle, int val, int min, int max, int iMenuFont)
{
	char s[256];
	menuDef_t *menu;
...
static void UI_DrawJediNonJedi(rectDef_t *rect, float scale, vector4 *color, int textStyle, int val, int min, int max, int iMenuFont)
{
	int i;
	char s[256];
	//menuDef_t *menu;
...
static void UI_DrawTeamName(rectDef_t *rect, float scale, vector4 *color, qboolean blue, int textStyle, int iMenuFont) {
  int i;
...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vector4 *color, qboolean blue, int num, int textStyle, int iMenuFont) 
{
	// 0 - None
	// 1 - Human
	// 2..NumCharacters - Bot
	...
	VectorCopy4( color, &finalColor );

	if (numval > maxcl)
	{
		VectorScale( (vector3*)&finalColor, 0.5, (vector3*)&finalColor );
...
static void UI_DrawMapTimeToBeat(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
...
static void UI_DrawNetSource(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) 
{
...
static void UI_DrawNetFilter(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) 
{
	...
	Text_Paint(rect->x, rect->y, scale, color, va("%s %s",holdSPString, holdSPString2), 0, 0, textStyle, iMenuFont);
#else
	Text_Paint( rect->x, rect->y, scale, color, va( "%s %s",holdSPString, serverFilters[ui_serverFilterType.integer].description ), 0, 0, textStyle, iMenuFont );
#endif
}


static void UI_DrawTier(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
	int i;
...
static void UI_DrawTierMapName(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
	int i, j;
	i = trap_Cvar_VariableValue( "ui_currentTier" );
	if (i < 0 || i >= uiInfo.tierCount) {
		i = 0;
...
static void UI_DrawTierGameType(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
	int i, j;
	i = trap_Cvar_VariableValue( "ui_currentTier" );
	if (i < 0 || i >= uiInfo.tierCount) {
		i = 0;
...
static void UI_DrawAllMapsSelection(rectDef_t *rect, float scale, vector4 *color, int textStyle, qboolean net, int iMenuFont) {
	int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
	if (map >= 0 && map < uiInfo.mapCount) {
...
static void UI_DrawOpponentName(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
...
static void UI_DrawBotName(rectDef_t *rect, float scale, vector4 *color, int textStyle,int iMenuFont) 
{
...
static void UI_DrawRedBlue(rectDef_t *rect, float scale, vector4 *color, int textStyle,int iMenuFont) 
{
...
static void UI_DrawSelectedPlayer(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) {
...
static void UI_DrawServerRefreshDate(rectDef_t *rect, float scale, vector4 *color, int textStyle, int iMenuFont) 
{
...
static void UI_DrawServerMOTD(rectDef_t *rect, float scale, vector4 *color, int iMenuFont) {
...
static void UI_DrawKeyBindStatus(rectDef_t *rect, float scale, vector4 *color, int textStyle,int iMenuFont) {
...
static void UI_DrawGLInfo(rectDef_t *rect, float scale, vector4 *color, int textStyle,int iMenuFont) 
{
	char * eptr;
	char buff[4096];
...
=================
*/
static void UI_Version(rectDef_t *rect, float scale, vector4 *color, int iMenuFont) 
{
	int width;
...
// FIXME: table drive
//
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vector4 *color, qhandle_t shader, int textStyle,int iMenuFont) 
{
	rectDef_t rect;
	int findex;
...
static qboolean UI_NetSource_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		UI_BuildServerDisplayList(qtrue);
		if (ui_netSource.integer >= AS_LOCAL/*!= AS_GLOBAL*/) { //Raz: Modified
...
static void UI_LoadDemos( void )
{
	...
	uiInfo.demoCount = Com_Clampi( 0, MAX_DEMOS, trap_FS_GetFileList( DEMOS_DIRECTORY, demoExt, demolist, sizeof( demolist ) ) );
	Com_sprintf( demoExt, sizeof( demoExt ), ".dm_%d", (int)trap_Cvar_VariableValue( "protocol" ) );
...
//	trap_Cvar_Set( "r_lodbias",					UI_Cvar_VariableString( "ui_r_lodbias" ) );
	...
	trap_Cvar_Set( "r_intensity",				UI_Cvar_VariableString( "ui_r_intensity" ) );
...
//	trap_Cvar_Set( "r_texturemode",				UI_Cvar_VariableString( "ui_r_texturemode" ) );
	...
	trap_Cvar_Set( "r_stencilbits",				UI_Cvar_VariableString( "ui_r_stencilbits" ) );
...
//	trap_Cvar_Register( NULL, "ui_r_lodbias",				"0", CVAR_ROM|CVAR_INTERNAL );
	...
	trap_Cvar_Register( NULL, "ui_r_intensity",				"0", CVAR_ROM|CVAR_INTERNAL );
...
//	trap_Cvar_Register( NULL, "ui_r_texturemode",			"0", CVAR_ROM|CVAR_INTERNAL );
	...
	trap_Cvar_Register( NULL, "ui_r_stencilbits",			"0", CVAR_ROM|CVAR_INTERNAL );
...
//	trap_Cvar_Set( "ui_r_lodbias",					UI_Cvar_VariableString( "r_lodbias" ) );
	...
	trap_Cvar_Set( "ui_r_intensity",				UI_Cvar_VariableString( "r_intensity" ) );
...
//	trap_Cvar_Set( "ui_r_texturemode",				UI_Cvar_VariableString( "r_texturemode" ) );
	...
	trap_Cvar_Set( "ui_r_stencilbits",				UI_Cvar_VariableString( "r_stencilbits" ) );
...
static void UI_UpdateSaberCvars ( void )
{
	...
	if ( !Q_stricmpn( UI_Cvar_VariableString( "ui_saber_color" ), "rgb", 3 ) )
		trap_Cvar_Set( "cp_sbRGB1", va( "%i", ui_sab1_r.integer | ((ui_sab1_g.integer | (ui_sab1_b.integer << 8)) << 8)) );
	else
		trap_Cvar_Set( "cp_sbRGB1", "0" );
	if ( !Q_stricmpn( UI_Cvar_VariableString( "ui_saber2_color" ), "rgb", 3 ) )
		trap_Cvar_Set( "cp_sbRGB2", va( "%i", ui_sab2_r.integer | ((ui_sab2_g.integer | (ui_sab2_b.integer << 8)) << 8)) );
	else
		trap_Cvar_Set( "cp_sbRGB2", "0" );
...
//	trap_Cvar_Set ( "ui_saber2", UI_Cvar_VariableString ( "saber2" ));
	...
	//[RGBSabers]
	/*
	trap_Cvar_Set("g_saber_color", SaberColorToString((saber_colors_t)trap_Cvar_VariableValue("color1")));
	trap_Cvar_Set("g_saber2_color", SaberColorToString((saber_colors_t)trap_Cvar_VariableValue("color2")));
...
//	qboolean startRefresh = qtrue; TTimo: unused
	...
	//RAZTODO: Build fs_game list to replace serverFilters
		...
		{ //we want it to load them as it draws them, like the TA feeder
			...
			{ //this isn't the best way of doing this I guess, but I didn't want a whole seperate string array
			  ...
			  //print what's in q3HeadNames and the icon name would look funny.
				...
				if ( !uiInfo.q3HeadIcons[index] )
					uiInfo.q3HeadIcons[index] = trap_R_RegisterShaderNoMip( "gfx/2d/defer.tga" );
#endif
			}
			return uiInfo.q3HeadIcons[index];
		}
...
int UI_UpdateNormalMenuCharacter( void )
{
	...
	if ( !item )
	{
		if ( (int)trap_Cvar_VariableValue( "g_gametype" ) != GT_SIEGE )
		{//Let's just ignore siege..?!
...
static void UI_BuildQ3Model_List( void )
{
	...
	for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		numfiles = trap_FS_GetFileList( va("models/players/%s",dirptr), "skin", filelist, sizeof(filelist) );
		fileptr  = filelist;
		for ( j=0;
		...
		//	if ( *dirptr )
			{ //if it exists
				...
				uiInfo.q3HeadIcons[uiInfo.q3HeadCount] = 0;//trap_R_RegisterShaderNoMip(fpath);
				{
					...
					//and now we are ready to register (thankfully this will only happen once)
					uiInfo.q3HeadIcons[uiInfo.q3HeadCount] = trap_R_RegisterShaderNoMip(iconNameFromSkinName);
					if ( !uiInfo.q3HeadIcons[uiInfo.q3HeadCount] )
						uiInfo.q3HeadIcons[uiInfo.q3HeadCount] = trap_R_RegisterShaderNoMip( "gfx/2d/defer.tga" );
				}
				uiInfo.q3HeadCount++;
...
//	trap_Cvar_Register( &siegeTeamSwitch, "g_siegeTeamSwitch", "1", CVAR_SERVERINFO|CVAR_ARCHIVE );
...
#ifdef OPENJK
	trap_Cvar_Register( &cjp_client, "cjp_client", JAPLUS_CLIENT_VERSION, CVAR_USERINFO|CVAR_ROM );
	trap_Cvar_Register( &japp_version, "japp_version", JAPP_CLIENT_VERSION, CVAR_ROM );
	trap_Cvar_Reset( "cjp_client" ); trap_Cvar_Reset( "japp_version" );	
#else
	...
	trap_Cvar_Register( &csf, "csf", va( "%X", JAPP_CLIENT_FLAGS ), CVAR_USERINFO|CVAR_ROM );
	trap_Cvar_Reset( "csf" );

	//Raz: Register userinfo cvars
	trap_Cvar_Register( NULL,	"cp_clanpwd",		"", CVAR_USERINFO | CVAR_ARCHIVE );
	trap_Cvar_Register( NULL,	"cp_login",			"", CVAR_USERINFO );
	trap_Cvar_Register( NULL,	"cp_pluginDisable",	"", CVAR_USERINFO | CVAR_ARCHIVE );
  ...
  //UI_Load();
	...
	value = trap_R_RegisterFont( "ocr_a" );
	trap_Cvar_Register( NULL, "font_small", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.qhSmallFont = value;

	value = trap_R_RegisterFont( "arialnb" );
	trap_Cvar_Register( NULL, "font_small2", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.qhSmall2Font = value;

	value = trap_R_RegisterFont( "ergoec" );
	trap_Cvar_Register( NULL, "font_medium", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.qhMediumFont = value;

	//Raz: fonts
	value = trap_R_RegisterFont( "japplarge" );
	trap_Cvar_Register( NULL, "font_japplarge", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.japp.fontLarge = value;

	value = trap_R_RegisterFont( "jappsmall" );
	trap_Cvar_Register( NULL, "font_jappsmall", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.japp.fontSmall = value;

	value = trap_R_RegisterFont( "jappmono" );
	trap_Cvar_Register( NULL, "font_jappmono", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
	uiInfo.uiDC.Assets.japp.fontMono = value;

	value = uiInfo.uiDC.Assets.qhMediumFont;
	trap_Cvar_Register( NULL, "font_big", va("%i", value), CVAR_ROM|CVAR_INTERNAL );
...
//	{ &japp_version, "japp_version", "", CVAR_ROM },
	...
	{ &ui_sab2_r,			"ui_sab2_r", "255", CVAR_INTERNAL|CVAR_ARCHIVE},
	{ &ui_sab2_g,			"ui_sab2_g", "255", CVAR_INTERNAL|CVAR_ARCHIVE},
...
#ifdef _WIN32
	#define CL_SERVERSTATUS_ADDY 0x4212C0
	#define CL_SERVERSTATUS_BUFF 0xB3980C
#elif defined(MACOS_X)
#endif

EFUNC( void, CL_ServerStatus_f, ( void ), CL_SERVERSTATUS_ADDY );

void __declspec(naked) CL_ServerStatus_f( void )
{
	__asm
	{
	//	pushad
		mov eax, CL_SERVERSTATUS_ADDY
		call eax
		//call CL_SERVERSTATUS_ADDY
	//	popad
	}
}

//static void (*CL_ServerStatus_f)( void ) = (void (*)(void)) CL_SERVERSTATUS_ADDY;
void JP_QueryServer( void )
{
	//0xB3980C command buffer
	//0x436ED0 WHAT THE FU-B000000000000000000M
	unsigned int i;
	for ( i=0; i<uiLocal.serversCount; i++ )
	{
		if ( !Q_stricmp( uiLocal.servers[i].name, UI_Argv( 1 ) ) )
		{//Found it
			*(char **)CL_SERVERSTATUS_BUFF = &uiLocal.servers[i].ip[0];
			break;
		}
	}
	ENG_CL_ServerStatus_f();
//	trap_Cmd_ExecuteText( EXEC_APPEND, va( "ping %s", *(char **)CL_SERVERSTATUS_BUFF ) );
