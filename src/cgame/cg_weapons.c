...
extern vector4	bluehudtint;
extern vector4	redhudtint;
extern vector4	*hudTintColor;
...
static void CG_CalculateWeaponPosition( vector3 *origin, vector3 *angles ) {
	...
	{// idle drift
		scale = cg.xyspeed + 40;
		fracsin = sin( cg.time * cg.gunIdleDrift.speed );
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups ) {
	// add powerup effects
	trap_R_AddRefEntityToScene( gun, MAX_CLIENTS );
		...
		if (weaponNum == WP_STUN_BATON)
		{
			...
			while (i < 3)
			{
				...
				barrel.renderfx = parent->renderfx;
					 ...
					 if ( i == 0 )	barrel.hModel = trap_R_RegisterModel( "models/weapons2/stun_baton/baton_barrel.md3" );
				else if ( i == 1 )	barrel.hModel = trap_R_RegisterModel( "models/weapons2/stun_baton/baton_barrel2.md3" );
				else				barrel.hModel = trap_R_RegisterModel( "models/weapons2/stun_baton/baton_barrel3.md3" );
				angles.yaw = 0;
				angles.pitch = 0;
				angles.roll = 0;
				...
				AnglesToAxis( &angles, barrel.axis );
					 ...
					 if ( i == 0 )	CG_PositionRotatedEntityOnTag( &barrel, parent/*&gun*/, /*weapon->weaponModel*/weapon->handsModel, "tag_barrel" );
				else if ( i == 1 )	CG_PositionRotatedEntityOnTag( &barrel, parent/*&gun*/, /*weapon->weaponModel*/weapon->handsModel, "tag_barrel2" );
				else				CG_PositionRotatedEntityOnTag( &barrel, parent/*&gun*/, /*weapon->weaponModel*/weapon->handsModel, "tag_barrel3" );
	...
	{	// Make sure we don't do the thirdperson model effects for the local player if we're in first person
		...
		{	// Handle muzzle flashes
			...
			{	// Check the alt firing first.
				...
				{
					if (!thirdPerson)
						trap_FX_PlayEntityEffectID(weapon->altMuzzleEffect, &flashorigin, tpflash.axis, -1, -1, -1, -1  );
					else
			...
			{	// Regular firing
				...
				{
					if (!thirdPerson)
						trap_FX_PlayEntityEffectID(weapon->muzzleEffect, &flashorigin, tpflash.axis, -1, -1, -1, -1  );
					else
		...
		if ( weapon->flashDlightColor.r || weapon->flashDlightColor.g || weapon->flashDlightColor.b )
			trap_R_AddLightToScene( &flashorigin, 300 + (rand()&31), weapon->flashDlightColor.r, weapon->flashDlightColor.g, weapon->flashDlightColor.b );
	}
}
