...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->pos.trType == TR_STATIONARY ) {
		...
		if ( t < (SINK_TIME*2) ) {
			...
			trap_R_AddRefEntityToScene( &le->refEntity, MAX_CLIENTS );
		} else {
			trap_R_AddRefEntityToScene( &le->refEntity, MAX_CLIENTS );
		}
...
void CG_AddFadeRGB( localEntity_t *le ) {
	...
	trap_R_AddRefEntityToScene( re, MAX_CLIENTS );
}
...
static void CG_AddExplosion( localEntity_t *ex ) {
	...
	if ( ex->light ) {
		...
		trap_R_AddLightToScene(&ent->origin, light, ex->lightColor.r, ex->lightColor.g, ex->lightColor.b );
	}
}
...
static void CG_AddSpriteExplosion( localEntity_t *le ) {
	...
	if ( le->light ) {
		...
		trap_R_AddLightToScene(&re.origin, light, le->lightColor.r, le->lightColor.g, le->lightColor.b );
	}
}
