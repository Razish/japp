...
//		if (ent->number > MAX_CLIENTS && cg.snap && ent->genericenemyindex && (ent->genericenemyindex-MAX_GENTITIES) == cg.snap->ps.clientNum)
		...
		if (g2Check)
		{
			...
			{
				CG_G2TraceCollide(&trace, (vector3 *)mins, (vector3 *)maxs, start, end);
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; i++ ) {
		...
		if ( ent->eType == ET_SPECIAL && ent->userInt1 )
		{
			if ( !(cg.predictedPlayerState.origin.x - ent->origin.x > 44 ||
				   ...
				   cg.predictedPlayerState.origin.z - ent->origin.z < -36) )
			{//This check is NOT correct, only a placeholder
			}
		}
...
void CG_PredictPlayerState( void ) {
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		for ( i=0; i<MAX_CLIENTS; i++ )
		{
			...
			if ( CG_IsMindTricked( ps->fd.forceMindtrickTargetIndex, ps->fd.forceMindtrickTargetIndex2, ps->fd.forceMindtrickTargetIndex3, ps->fd.forceMindtrickTargetIndex4, es->number ) && !(ps->fd.forcePowersKnown&(1<<FP_TELEPATHY)) && cg_attemptGhostPredict.integer )
		...
		for ( i=0; i<MAX_CLIENTS; i++ )
		{//Revert the solid and eType of each entity.
