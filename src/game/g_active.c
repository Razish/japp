	...
	*/
		...
		if ( damageSelf && self->takedamage )
		{
			...
			{//we were force-jumping
				if ( self->r.currentOrigin.z >= self->client->ps.fd.forceJumpZStart )
		...
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		{
			...
			if ( g_privateDuel.integer & PRIVDUEL_RESPAWN )
			{
				...
				if ( SpotWouldTelefrag3( &duelAgainst->client->pers.duel.startPos ) ) {
					...
					trap_LinkEntity( duelAgainst );
				}
			}
			else
			{// only respawn the winner
				if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0)
				{
...
//	WP_SaberPositionUpdate(ent, ucmd); //check the server-side saber point, do apprioriate server-side actions (effects are cs-only)
	...
	*/
	...
	else
	{
		...
		//check for taunt
		if ( (pm.cmd.generic_cmd == GENCMD_ENGAGE_DUEL) && (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) )
...
void ClientEndFrame( gentity_t *ent ) {
	...
	if ( japp_showLaggingClients.integer )
