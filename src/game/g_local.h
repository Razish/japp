...
typedef struct clientSession_s {
	...
	//Raz: validation
	char				IP[NET_ADDRSTRMAXLEN]; //DEPRECATED: use svs->clients[num].netchan.remoteAddress
	int					validated;
} clientSession_t;
...
typedef struct clientPersistant_s {
	...
	struct japp
	{
		...
		channel_t *activeChannel; //only used for legacy clients that don't support inline chat filters/tabs
	} japp;

	int				connectTime;
	unsigned int	CSF;			//	CSF_XXX bit-flags
	unsigned int	CPD;			//	CPD_XXX bit-flags
	...
	struct {
		int weapon; // for weapon-specific duels - pistols etc
		vector3 startPos;
	} duel;
...
typedef struct renderInfo_s
{
	...
	vector3		muzzlePoint;
	vector3		muzzleDir;
	vector3		muzzlePointOld;
	vector3		muzzleDirOld;
...
qboolean	trap_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
								const vector3 *angles, const vector3 *position, const int frameNum, qhandle_t *modelList, vector3 *scale);
...
void		trap_G2API_CollisionDetect ( CollisionRecord_t *collRecMap, void* ghoul2, const vector3 *angles, const vector3 *position,
								int frameNumber, int entNum, vector3 *rayStart, vector3 *rayEnd, vector3 *scale, int traceFlags, int useLod, float fRadius );
void		trap_G2API_CollisionDetectCache ( CollisionRecord_t *collRecMap, void* ghoul2, const vector3 *angles, const vector3 *position,
								int frameNumber, int entNum, vector3 *rayStart, vector3 *rayEnd, vector3 *scale, int traceFlags, int useLod, float fRadius );

qboolean	trap_G2API_SetBoneAngles(void *ghoul2, int modelIndex, const char *boneName, const vector3 *angles, const int flags,
...
qboolean	trap_G2API_RagPCJConstraint(void *ghoul2, const char *boneName, vector3 *min, vector3 *max); //override default pcj bonee constraints
...
qboolean	trap_G2API_RagEffectorGoal(void *ghoul2, const char *boneName, vector3 *pos); //override an effector bone's goal position (world coordinates)
qboolean	trap_G2API_GetRagBonePos(void *ghoul2, const char *boneName, vector3 *pos, vector3 *entAngles, vector3 *entPos, vector3 *entScale); //current position of said bone is put into pos (world coordinates)
qboolean	trap_G2API_RagEffectorKick(void *ghoul2, const char *boneName, vector3 *velocity); //add velocity to a rag bone
...
// NPC_senses.cpp
//
extern void AddSightEvent( gentity_t *owner, vector3 *position, float radius, alertEventLevel_e alertLevel, float addLight ); //addLight = 0.0f
extern void AddSoundEvent( gentity_t *owner, vector3 *position, float radius, alertEventLevel_e alertLevel, qboolean needLOS ); //needLOS = qfalse
...
qboolean trap_InPVSIgnorePortals( const vector3 *p1, const vector3 *p2 );
void	trap_AdjustAreaPortalState( gentity_t *ent, qboolean open );
qboolean trap_AreasConnected( int area1, int area2 );
void	trap_LinkEntity( gentity_t *ent );
void	trap_UnlinkEntity( gentity_t *ent );
int		trap_EntitiesInBox( const vector3 *mins, const vector3 *maxs, int *entityList, int maxcount );
qboolean trap_EntityContact( const vector3 *mins, const vector3 *maxs, const gentity_t *ent );
...
int		trap_DebugPolygonCreate(int color, int numPoints, vector3 *points);
...
int		trap_BotLibTest(int parm0, char *parm1, vector3 *parm2, vector3 *parm3);
...
int		trap_AAS_PointReachabilityAreaIndex(vector3 *point);
...
int		trap_AAS_VectorForBSPEpairKey(int ent, char *key, vector3 *v);
...
int		trap_AAS_PredictRoute(void /*struct aas_predictroute_s*/ *route, int areanum, vector3 *origin,
							int goalareanum, int travelflags, int maxareas, int maxtime,
							int stopevent, int stopcontents, int stoptfl, int stopareanum);

int		trap_AAS_AlternativeRouteGoals(vector3 *start, int startareanum, vector3 *goal, int goalareanum, int travelflags,
										void /*struct aas_altroutegoal_s*/ *altroutegoals, int maxaltroutegoals,
										int type);
int		trap_AAS_Swimming(vector3 *origin);
int		trap_AAS_PredictClientMovement(void /* aas_clientmove_s */ *move, int entnum, vector3 *origin, int presencetype, int onground, vector3 *velocity, vector3 *cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize);
...
void	trap_EA_Move(int client, vector3 *dir, float speed);
void	trap_EA_View(int client, vector3 *viewangles);
...
int		trap_BotChooseNBGItem(int goalstate, vector3 *origin, int *inventory, int travelflags, void /* struct bot_goal_s */ *ltg, float maxtime);
int		trap_BotTouchingGoal(vector3 *origin, void /* struct bot_goal_s */ *goal);
int		trap_BotItemGoalInVisButNotVisible(int viewer, vector3 *eye, vector3 *viewangles, void /* struct bot_goal_s */ *goal);
...
int		trap_BotMovementViewTarget(int movestate, void /* struct bot_goal_s */ *goal, int travelflags, float lookahead, vector3 *target);
int		trap_BotPredictVisiblePosition(vector3 *origin, int areanum, void /* struct bot_goal_s */ *goal, int travelflags, vector3 *target);
...
int			trap_ICARUS_GetVectorVariable( const char *name, const vector3 *value );
...
int			trap_Nav_GetNodePosition( int nodeID, vector3 *out );
...
int			trap_Nav_GetProjectedNode( vector3 *origin, int nodeID );
