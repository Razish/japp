...
extern void trap_FX_PlayEffect( const char *file, vector3 *org, vector3 *fwd, int vol, int rad );
#endif
...
void PM_HoverTrace( void )
{
	...
	else
	{
		...
		else if ( trace->plane.normal.z >= minNormal ) 
...
qboolean PM_AdjustAngleForWallRun( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//wall-running and not at end of anim
		...
		if ( trace.fraction < 1.0f 
			&& (trace.plane.normal.z >= 0.0f&&trace.plane.normal.z <= 0.4f/*MAX_WALL_RUN_Z_NORMAL*/) )
		{//still a wall there
			if ( (ps->legsAnim) == BOTH_WALL_RUN_RIGHT )
			{
				ucmd->rightmove = 127;
...
//[LedgeGrab]
qboolean LedgeTrace( trace_t *trace, vector3 *dir, float *lerpup, float *lerpfwd, float *lerpyaw)
			...
			if ( anim != -1 )
			{
				...
				{//there is a wall there.. or hit a client
					if ( (anim != BOTH_WALL_RUN_LEFT 
						...
						|| (wallNormal.z >= 0.0f&&wallNormal.z<=0.4f/*MAX_WALL_RUN_Z_NORMAL*/) )
...
static void PM_FlyVehicleMove( void ) 
{
	...
	// normal slowdown
	if ( pm->ps->gravity && pm->ps->velocity.z < 0 && pm->ps->groundEntityNum == ENTITYNUM_NONE )
	{//falling
...
static void PM_CrashLand( void ) {
	...
	if ( !(pml.groundTrace.surfaceFlags & SURF_NODAMAGE) )  {
		if (delta > 7)
		{
			...
			if (pm->ps->fd.forceJumpZStart)
			{
				if ((int)pm->ps->origin.z >= (int)pm->ps->fd.forceJumpZStart)
...
//static void BG_G2ClientNeckAngles( void *ghoul2, int time, const vector3 *lookAngles, vector3 *headAngles, vector3 *neckAngles, vector3 *thoracicAngles, vector3 *headClampMinAngles, vector3 *headClampMaxAngles )
//[/LedgeGrab]
{
	vector3	lA;
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd)
{
	...
	if ( setAngles )
	{
		for ( i = 0; i < 3; i++ )
		{//clamp viewangles
			if ( clampMin.data[i] == -1 || clampMax.data[i] == -1 )
			{//no clamp
			}
			else if ( !clampMin.data[i] && !clampMax.data[i] )
			{//no allowance
...
//	PM_AdjustAnglesForStabDown( pm->ps, &pm->cmd );
	...
	*/
	...
	if (pm->ps->pm_type == PM_JETPACK)
	{
		...
		if (pm->cmd.upmove > 0 && pm->ps->velocity.z < 256)
