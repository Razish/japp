...
extern void trap_FX_PlayEffectID( int id, vector3 *org, vector3 *fwd, int vol, int rad );
#endif
...
void PM_VehicleImpact(bgEntity_t *pEnt, trace_t *trace)
{
	...
	else if ( (fabs(pm->ps->velocity.x)+fabs(pm->ps->velocity.y)) < 100.0f
		&& pm->ps->velocity.z > -100.0f )
#else
	if ( (fabs(pm->ps->velocity.x)+fabs(pm->ps->velocity.y)) < 100.0f
		&& pm->ps->velocity.z > -100.0f )
#endif
		...
		{//a bit of a hack, may conflict with getting shot, but...
			...
			if ( trace && !pSelfVeh->m_iRemovedSurfaces && !forceSurfDestruction )
			{
				...
				{ //bounce off in the opposite direction of the impact
					...
					else if ( trace->plane.normal.z >= MIN_LANDING_SLOPE//flat enough to land on
				...
				{//bounce off impact surf and turn away
					vector3	pushDir={0}, turnAwayAngles, turnDelta;
...
//	vector3		delta, delta2;
	...
	if ( pm->ps->velocity.z > 0 && (trace.fraction == 1.0 ||
	...
	{
		if ( pm->ps->clientNum < MAX_CLIENTS
			&& trace.plane.normal.z < MIN_WALK_NORMAL )
