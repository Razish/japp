...
extern qboolean G_ClearLineOfSight(const vector3 *point1, const vector3 *point2, int ignore, int clipmask);
...
extern void SetClientViewAngle( gentity_t *ent, vector3 *angle );
...
float	G_CanJumpToEnemyVeh(Vehicle_t *pVeh, const usercmd_t *pUcmd )
{
	...
	if (rider && 
		rider->enemy && 
		pUcmd->rightmove && 
		fabsf(rider->enemy->currentorigin.z - rider->currentorigin.z)<50.0f)
	{
...
{
	Vehicle_t *pVeh = NULL;
	vector3 riderAngles, fDir, rDir, dir2Me;
	float	fDot, rDot;
				  ...
				  vector3 *vExitPos )
{
	...
	fVehDiag = sqrtf( ( parent->maxs.x*parent->maxs.x ) + ( parent->maxs.y*parent->maxs.y ) );
	...
	if ( (gentity_t *)pVeh->m_pPilot == ent )
	{
		...
		while (j < pVeh->m_iNumPassengers)
		{
			if (pVeh->m_ppPassengers[j])
			{
				...
				//Raz: fixed rare vehicle crash
				if ( pVeh->m_ppPassengers[k-1] && ((gentity_t *)pVeh->m_ppPassengers[j])->client )
				{
...
//===================================================================
			...
			if ( pVeh->m_pVehicleInfo->Eject( pVeh, pRider, qfalse ) )
			{
				...
				rider->client->ps.forceJumpZStart = rider->client->ps.origin.z;
