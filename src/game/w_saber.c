...
static GAME_INLINE qboolean G_CheckLookTarget( gentity_t *ent, vector3 *lookAngles, float *lookingSpeed )
{
...
static GAME_INLINE void G_G2NPCAngles(gentity_t *ent, vector3 legs[3], vector3 *angles)
{
...
static GAME_INLINE void G_G2PlayerAngles( gentity_t *ent, vector3 legs[3], vector3 *legsAngles)
{
...
qboolean WP_SabersCheckLock( gentity_t *ent1, gentity_t *ent2 )
{
	...
	if ( fabs( ent1->r.currentOrigin.z-ent2->r.currentOrigin.z ) > 16 )
	{
		return qfalse;
	}
...
static GAME_INLINE qboolean G_G2TraceCollide(trace_t *tr, vector3 *lastValidStart, vector3 *lastValidEnd, vector3 *traceMins, vector3 *traceMaxs)
...
static GAME_INLINE void G_BuildSaberFaces(vector3 *base, vector3 *tip, float radius, vector3 *fwd,
...
static GAME_INLINE void G_SabCol_CalcPlaneEq(vector3 *x, vector3 *y, vector3 *z, float *planeEq)
{
...
//collision utility function -rww
static GAME_INLINE int G_SabCol_PointRelativeToPlane(vector3 *pos, float *side, float *planeEq)
{
...
static GAME_INLINE qboolean G_SaberFaceCollisionCheck(int fNum, saberFace_t *fList, vector3 *atkStart,
...
static GAME_INLINE qboolean G_SaberCollide(gentity_t *atk, gentity_t *def, vector3 *atkStart,
						vector3 *atkEnd, vector3 *atkMins, vector3 *atkMaxs, vector3 *impactPoint)
{
	static int i, j;
...
// JA version
static GAME_INLINE qboolean CheckSaberDamageOriginal(gentity_t *self, int rSaberNum, int rBladeNum, vector3 *saberStart, vector3 *saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	static trace_t tr;
	static vector3 dir;
	...
	{
		if ( doInterpolate && !d_saberSPStyleDamage.integer && japp_saberSystem.integer != SABERSYSTEM_JAPP )
	...
	{ //this animation is that of the last attack movement, and so it should do full damage
		...
		{//SP
			...
			{
				if ( !g_entities[self->client->ps.saberEntityNum].s.saberInFlight )
...
// JK2 version
static GAME_INLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vector3 *saberStart, vector3 *saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	trace_t tr;
	vector3 dir;
		...
		g_entities[tr.entityNum].inuse)
	{
		...
		else
		{
			...
			*/
			...
			if (g_entities[tr.entityNum].client && !g_entities[tr.entityNum].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE])
			{ //not a "jedi", so make them suffer more
				if (dmg > SABER_NONATTACK_DAMAGE)
				{ //don't bother increasing just for idle touch damage
					dmg *= 1.5;
...
blockStuff:
		...
		{//knockaways can make fast-attacker go into a broken parry anim if the ent is using fast or med. In MP, we also randomly decide this for level 3 attacks.
			...
			didDefense = qtrue;
				 ...
				 ( otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] < self->client->ps.fd.saberAnimLevel || (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == self->client->ps.fd.saberAnimLevel && (Q_irand(1, 10) >= otherOwner->client->ps.fd.saberAnimLevel*1.5 || unblockable)) ) &&
				 PM_SaberInParry(otherOwner->client->ps.saberMove) &&
				 !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
				 !PM_SaberInParry(self->client->ps.saberMove) &&
				 !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
...
GAME_INLINE int VectorCompare2( const vector3 *v1, const vector3 *v2 ) {
							...
							if (!CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qfalse, trMask, qfalse))
							{
								if (!CheckSaberDamage(self, rSaberNum, rBladeNum, &boltOrigin, &end, qtrue, trMask, qfalse))
								{
									...
									//and don't bother if the angle is the same
										...
										saberMidPoint.x = boltOrigin.x + (saberSubBase.x*0.5);
										saberMidPoint.y = boltOrigin.y + (saberSubBase.y*0.5);
										saberMidPoint.z = boltOrigin.z + (saberSubBase.z*0.5);
