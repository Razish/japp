...
static int (*Q_syscall)( int arg, ... ) = (int (*)( int, ...))-1;

#if defined(__linux__) && !defined(__GCC__)
extern "C" {
#endif
void Q_EXPORT dllEntry( int (QDECL *Q_syscallptr)( int arg,... ) ) {
	Q_syscall = Q_syscallptr;
}
#if defined(__linux__) && !defined(__GCC__)
}
#endif
...
//Returns -1 for errors such as FNF (Not 0! - which means empty file)
...
int trap_EntitiesInBox( const vector3 *mins, const vector3 *maxs, int *list, int maxcount ) {
...
void trap_TraceCapsule( trace_t *results, const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end, int passEntityNum, int contentmask ) {
	Q_syscall( G_TRACECAPSULE, results, start, mins, maxs, end, passEntityNum, contentmask, 0, 10 );
}

qboolean trap_EntityContactCapsule( const vector3 *mins, const vector3 *maxs, const gentity_t *ent ) {
...
int trap_AAS_PredictClientMovement(void /* struct aas_clientmove_s */ *move, int entnum, vector3 *origin, int presencetype, int onground, vector3 *velocity, vector3 *cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize) {
