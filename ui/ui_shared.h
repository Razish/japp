...
enum ratioFix_e : uint32_t { RATIOFIX_HUD, RATIOFIX_MOUSEINPUT };
...
typedef struct itemDef_s {
    ...
    int iMenuFont;        // FONT_SMALL,FONT_MEDIUM,FONT_LARGE	// changed from 'font' so I could see what didn't compile, and differentiate between font handles
                          // returned from RegisterFont -ste
...
struct displayContextDef_t {
    ...
    void (*ownerDrawItem)(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, uint32_t ownerDrawFlags, int align, float special,
    ...
    void (*drawTextWithCursor)(float x, float y, float scale, const vector4 *color, const char *text, int cursorPos, char cursor, int limit,
