...
void UI_ReadLegalForce( void ) {
	...
	for ( c = 0; fcfString[i] && c < NUM_FORCE_POWERS; c++, i++ ) {
		...
		if ( uiForcePowerDarkLight[c] && uiForcePowerDarkLight[c] != uiForceSide ) { //Apparently the user has crafted a force config that has powers that don't fit with the config's side.
			continue;  // skip this power
		}
