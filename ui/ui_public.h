...
typedef struct uiImport_s {
	...
	void( *Key_SetOverstrikeMode )(
		qboolean state
	);

	int( *PC_AddGlobalDefine )(
		char *define
	);
	...
	int( *LAN_GetServerCount )(
		int source
	);

	void( *LAN_GetServerInfo )(
		int source,
		int n,
		char *buf,
		int buflen
	...
	int( *LAN_ServerIsVisible )(
		int source,
		int n
	);

	int( *LAN_ServerStatus )(
		const char *serverAddress,
		char *serverStatus,
		int maxLen
	);

	qboolean( *LAN_UpdateVisiblePings )(
		int source
	);

	void( *S_StartBackgroundTrack )(
		const char *intro,
		const char *loop,
	...
	sfxHandle_t( *S_RegisterSound )(
		const char *sample
	);

	void( *SE_GetLanguageName )(
	...
	qboolean( *SE_GetStringTextString )(
		const char *text,
		char *buffer,
		int bufferLength
	);

	qboolean( *R_Language_IsAsian )(
		void
	);

	qboolean( *R_Language_UsesSpaces )(
		void
	);

	unsigned int( *R_AnyLanguage_ReadCharFromString )(
		const char *psText,
		int *piAdvanceCount,
		qboolean *pbIsTrailingPunctuation
	);

	void( *R_AddLightToScene )(
		const vector3 *org,
		float intensity,
		float r,
		float g,
	...
	qhandle_t( *R_RegisterModel )(
		const char *name
	);

	qhandle_t( *R_RegisterSkin )(
		const char *name
	);

	qhandle_t( *R_RegisterShaderNoMip )(
		const char *name
	);

	qhandle_t( *R_RegisterFont )(
		const char *fontName
	);
	...
	void( *R_ShaderNameFromIndex )(
		char *name,
		int index
	);

	void( *G2_ListModelSurfaces )(
		void *ghlInfo
	);

	void( *G2_ListModelBones )(
		void *ghlInfo,
		int frame
	);

	void( *G2_SetGhoul2ModelIndexes )(
		void *ghoul2,
		qhandle_t *modelList,
		qhandle_t *skinList
	);

	qboolean( *G2_HaveWeGhoul2Models )(
		void *ghoul2
	);
	...
	qboolean( *G2API_GetBoltMatrix_NoReconstruct )(
		void *ghoul2,
		const int modelIndex,
		const int boltIndex,
		mdxaBone_t *matrix,
	...
	qboolean( *G2API_GetBoltMatrix_NoRecNoRot )(
		void *ghoul2,
		const int modelIndex,
		const int boltIndex,
		mdxaBone_t *matrix,
	...
	void( *G2API_GetGLAName )(
		void *ghoul2,
		int modelIndex,
		char *fillBuf
	);

	int( *G2API_CopyGhoul2Instance )(
		void *g2From,
		void *g2To,
		int modelIndex
	);

	void( *G2API_CopySpecificGhoul2Model )(
		void *g2From,
		int modelFrom,
		void *g2To,
		int modelTo
	...
	void( *G2API_DuplicateGhoul2Instance )(
		void *g2From,
		void **g2To
	);

	qboolean( *G2API_HasGhoul2ModelOnIndex )(
		void *ghlInfo,
		int modelIndex
	);

	qboolean( *G2API_RemoveGhoul2Model )(
		void *ghlInfo,
		int modelIndex
	);
	...
	void( *G2API_SetBoltInfo )(
		void *ghoul2,
		int modelIndex,
		int boltInfo
	);

	qboolean( *G2API_SetRootSurface )(
		void *ghoul2,
		const int modelIndex,
		const char *surfaceName
	);

	qboolean( *G2API_SetSurfaceOnOff )(
		void *ghoul2,
		const char *surfaceName,
		const uint32_t flags
	);

	qboolean( *G2API_SetNewOrigin )(
		void *ghoul2,
		const int boltIndex
	);
...
typedef uiExport_t* (Q_DECL *GetUIAPI_t)(int apiVersion, uiImport_t *import);
