...
const char *forcepowerDesc[NUM_FORCE_POWERS] = {
    "@MENUS_OF_EFFECT_JEDI_ONLY_NEFFECT",  "@MENUS_DURATION_IMMEDIATE_NAREA",  "@MENUS_DURATION_5_SECONDS_NAREA",     "@MENUS_DURATION_INSTANTANEOUS",
    "@MENUS_INSTANTANEOUS_EFFECT_NAREA",   "@MENUS_DURATION_VARIABLE_20",      "@MENUS_DURATION_INSTANTANEOUS_NAREA", "@MENUS_OF_EFFECT_LIVING_PERSONS",
    "@MENUS_DURATION_VARIABLE_10",         "@MENUS_DURATION_VARIABLE_NAREA",   "@MENUS_DURATION_CONTINUOUS_NAREA",    "@MENUS_OF_EFFECT_JEDI_ALLIES_NEFFECT",
    "@MENUS_EFFECT_JEDI_ALLIES_NEFFECT",   "@MENUS_VARIABLE_NAREA_OF_EFFECT",  "@MENUS_EFFECT_NAREA_OF_EFFECT",       "@SP_INGAME_FORCE_SABER_OFFENSE_DESC",
...
const char *datapadMoveTitleData[MD_MOVE_TITLE_MAX] = {
    "@MENUS_ACROBATICS", "@MENUS_SINGLE_FAST", "@MENUS_SINGLE_MEDIUM", "@MENUS_SINGLE_STRONG", "@MENUS_DUAL_SABERS", "@MENUS_SABER_STAFF",
};

const char *datapadMoveTitleBaseAnims[MD_MOVE_TITLE_MAX] = {
    "BOTH_RUN1", "BOTH_SABERFAST_STANCE", "BOTH_STAND2", "BOTH_SABERSLOW_STANCE", "BOTH_SABERDUAL_STANCE", "BOTH_SABERSTAFF_STANCE",
};
...
int uiHoldSkinColor = TEAM_FREE; // Stores the skin color so that in non-team games, the player screen remembers the team you chose, in case you're coming back
                                 // from the force powers screen.
...
static const char *netNames[] = {"???", "UDP", NULL};
...
void Text_PaintWithCursor(float x, float y, float scale, const vector4 *color, const char *text, int cursorPos, char cursor, int limit, uiTextStyle_e style,
...
static void Text_Paint_Limit(float *maxX, float x, float y, float scale, const vector4 *color, const char *text, float adjust, int limit, int iMenuFont,
    ...
    if (x + iPixelLen > *maxX) {
        ...
        while (*psText && (x + font.Width(sTemp) <= *maxX) && psOut < &sTemp[sizeof(sTemp) - 1]) // sanity
        {
            int iAdvanceCount;
            psOutLastGood = psOut;
...
void Text_PaintCenter(float x, float y, float scale, const vector4 *color, const char *text, float adjust, int iMenuFont, bool customFont);
...
void UI_SetColor(const vector4 *rgba) { trap->R_SetColor(rgba); }
...
void UI_ParseMenu(const char *menuFile) {
    ...
    while (1) {
        ...
        if ( Q_stricmp( token.string, "{" ) ) {
                ...
                Com_Printf( "Missing { in menu file %s on line %i, got '%s' instead\n", filename, line, token.string );
                break;
        }
        */
        ...
        if (Q_stricmp(token.string, "menudef") == 0) {
            // start a new menu
            /*
            trap->Print( "UI_ParseMenu: %s (%i/%i)\n", menuFile, menuCount, MAX_MENUS );
            trap->Cmd_ExecuteText( EXEC_NOW, va( "which %s\n", menuFile ) );
            */
            Menu_New(handle);
        }
    }
...
static const char *handicapValues[] = {"None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50",
                                       "45",   "40", "35", "30", "25", "20", "15", "10", "5",  NULL};

static void UI_DrawHandicap(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    int h = Q_clampi(5, trap->Cvar_VariableValue("handicap"), 100);
    int i = 20 - h / 5;

    const Font font(iMenuFont, scale);
    font.Paint(rect->x, rect->y, handicapValues[i], color);
}
...
static void UI_DrawGameType(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    const Font font(iMenuFont, scale, customFont);
    font.Paint(rect->x, rect->y, BG_GetGametypeString(ui_gameType.integer), color);
}

static void UI_DrawNetGameType(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
...
static void UI_DrawAutoSwitch(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    ...
    if (stripString) {
        Font font(iMenuFont, scale, customFont);
        font.Paint(rect->x, rect->y, stripString, color, textStyle);
    }
}

static void UI_DrawJoinGameType(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    ...
    const Font font(iMenuFont, scale, customFont);
    const char *str = ui_joinGameType.integer == -1 ? "Any" : BG_GetGametypeString(ui_joinGameType.integer);
...
static void UI_DrawSkill(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    int i;
...
static void UI_DrawGenericNum(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int val, int min, int max, int type, int iMenuFont,
                              bool customFont) {
    char s[32];
...
static void UI_DrawForceMastery(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int val, int min, int max, int iMenuFont,
                                bool customFont) {
    int i;
    ...
    const Font font(iMenuFont, scale, customFont);
    const char *s = UI_GetStringEdString("MP_INGAME", forceMasteryLevels[i]);
    font.Paint(rect->x, rect->y, s, color, textStyle);
}

static void UI_DrawSkinColor(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int val, int min, int max, int iMenuFont,
                             bool customFont) {
    char s[256];
...
static void UI_DrawForceSide(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int val, int min, int max, int iMenuFont,
                             bool customFont) {
    char s[256];
    menuDef_t *menu;
...
static void UI_DrawJediNonJedi(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int val, int min, int max, int iMenuFont,
                               bool customFont) {
    int i;
    char s[256];
    // menuDef_t *menu;
...
static void UI_DrawTeamName(rectDef_t *rect, float scale, const vector4 *color, qboolean blue, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    int i;
    i = UI_TeamIndexFromName(UI_Cvar_VariableString((blue) ? "ui_blueTeam" : "ui_redTeam"));
    if (i >= 0 && i < uiInfo.teamCount) {
        const Font font(iMenuFont, scale, customFont);
        const char *str = va("%s: %s", blue ? "Blue" : "Red", uiInfo.teamList[i].teamName);
        font.Paint(rect->x, rect->y, str, color, textStyle);
    }
}

static void UI_DrawTeamMember(rectDef_t *rect, float scale, const vector4 *color, qboolean blue, int num, uiTextStyle_e textStyle, int iMenuFont,
                              bool customFont) {
    // 0 - None
    // 1 - Human
    // 2..NumCharacters - Bot
    ...
    const Font font(iMenuFont, scale, customFont);
    font.Paint(rect->x, rect->y, text, &finalColor, textStyle);
}
...
static void UI_DrawNetSource(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    ...
    const Font font(iMenuFont, scale, customFont);
    const char *str = va("%s %s", holdSPString, GetNetSourceString(ui_netSource.integer));
    font.Paint(rect->x, rect->y, str, color, textStyle);
}
...
static void UI_DrawNetFilter(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    trap->SE_GetStringTextString("MENUS_GAME", holdSPString, sizeof(holdSPString));

    const Font font(iMenuFont, scale, customFont);
    const char *str = va("%s %s", holdSPString, UI_FilterDescription(ui_serverFilterType.integer));
    font.Paint(rect->x, rect->y, str, color, textStyle);
}
...
static void UI_DrawAllMapsSelection(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, qboolean net, int iMenuFont, bool customFont) {
    int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
    if (map >= 0 && map < uiInfo.mapCount) {
        const Font font(iMenuFont, scale, customFont);
        font.Paint(rect->x, rect->y, uiInfo.mapList[map].mapName, color, textStyle);
    }
}

static void UI_DrawOpponentName(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    const Font font(iMenuFont, scale, customFont);
    font.Paint(rect->x, rect->y, UI_Cvar_VariableString("ui_opponentName"), color, textStyle);
}
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale) {
    ...
    if (s) {
        const Font font(FONT_NONE, scale, false);
        return font.Width(s);
    }
    return 0;
}

static void UI_DrawBotName(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
...
static void UI_DrawBotSkill(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    if (uiInfo.skillIndex < numSkillLevels) {
        const Font font(iMenuFont, scale, customFont);
        const char *str = UI_GetStringEdString("MP_INGAME", skillLevels[uiInfo.skillIndex]);
        font.Paint(rect->x, rect->y, str, color, textStyle);
    }
}

static void UI_DrawRedBlue(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    const Font font(iMenuFont, scale, customFont);
    const char *str = UI_GetStringEdString("MP_INGAME", uiInfo.redBlue == 0 ? "RED" : "BLUE");
    font.Paint(rect->x, rect->y, str, color, textStyle);
}
...
static void UI_DrawSelectedPlayer(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    if (uiInfo.uiDC.realTime > uiInfo.playerRefresh) {
        uiInfo.playerRefresh = uiInfo.uiDC.realTime + 3000;
        UI_BuildPlayerList();
    }
    const Font font(iMenuFont, scale, customFont);
    const char *str = UI_Cvar_VariableString("cg_selectedPlayerName");
    font.Paint(rect->x, rect->y, str, color, textStyle);
}

static void UI_DrawServerRefreshDate(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    if (uiInfo.serverStatus.refreshActive) {
        ...
        const Font font(iMenuFont, scale, customFont);
        const char *str = va(holdSPString, trap->LAN_GetServerCount(UI_SourceForLAN()));
        font.Paint(rect->x, rect->y, str, &newColor, textStyle);
    } else {
        char buff[64];
        ...
        const Font font(iMenuFont, scale, customFont);
        font.Paint(rect->x, rect->y, va("%s: %s", holdSPString, buff), color, textStyle);
    }
}
    ...
    if (uiInfo.serverStatus.motdLen) {
        ...
        if (uiInfo.uiDC.realTime > uiInfo.serverStatus.motdTime) {
            ...
            if (uiInfo.serverStatus.motdPaintX <= rect->x + 2) {
                if (uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen) {
                    uiInfo.serverStatus.motdPaintX += font.Width(&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset]) - 1;
        ...
        Text_Paint_Limit(&maxX, uiInfo.serverStatus.motdPaintX, rect->y + rect->h - 3, scale, color, &uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset],
                         0, 0, iMenuFont, customFont);
        if (uiInfo.serverStatus.motdPaintX2 >= 0) {
            float maxX2 = rect->x + rect->w - 2;
            Text_Paint_Limit(&maxX2, uiInfo.serverStatus.motdPaintX2, rect->y + rect->h - 3, scale, color, uiInfo.serverStatus.motd, 0,
                             uiInfo.serverStatus.motdOffset, iMenuFont, customFont);
        }
...
static void UI_DrawKeyBindStatus(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    if (Display_KeyBindPending()) {
        const Font font(iMenuFont, scale, customFont);
        font.Paint(rect->x, rect->y, UI_GetStringEdString("MP_INGAME", "WAITING_FOR_NEW_KEY"), color, textStyle);
    }
}

static void UI_DrawGLInfo(rectDef_t *rect, float scale, const vector4 *color, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    char *eptr;
    char buff[4096];
    ...
    const Font font(iMenuFont, scale, customFont);
    const char *str = va("GL_VENDOR: %s", uiInfo.uiDC.glconfig.vendor_string);
    font.Paint(rect->x + 2, rect->y + y, str, color, textStyle, rect->w);
    y += lineHeight;
    str = va("GL_VERSION: %s: %s", uiInfo.uiDC.glconfig.version_string, uiInfo.uiDC.glconfig.renderer_string);
    font.Paint(rect->x + 2, rect->y + y, str, color, textStyle, rect->w);
    y += lineHeight;
    str = va("GL_PIXELFORMAT: color(%d-bits) Z(%d-bits) stencil(%d-bits)", uiInfo.uiDC.glconfig.colorBits, uiInfo.uiDC.glconfig.depthBits,
    ...
    for (int i = 0; i < numLines;) {
        font.Paint(rect->x + 2, y, lines[i++], color, textStyle, (rect->w / 2));
        if (i < numLines) {
            font.Paint(rect->x + rect->w / 2, y, lines[i++], color, textStyle, (rect->w / 2));
        }
        y += 10.0f;
...
static void UI_Version(rectDef_t *rect, float scale, const vector4 *color, int iMenuFont, bool customFont) {
    const Font font(iMenuFont, scale, customFont);
    const float width = font.Width(JAPP_VERSION);
    font.Paint(rect->x - width, rect->y, JAPP_VERSION, color);
}

// FIXME: table drive
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, uint32_t ownerDrawFlags, int align, float special,
                         float scale, const vector4 *color, qhandle_t shader, uiTextStyle_e textStyle, int iMenuFont, bool customFont) {
    rectDef_t rect;
    int findex;
    ...
    case UI_FORCE_POINTS:
        UI_DrawGenericNum(&rect, scale, color, textStyle, uiForceAvailable, 1, forceMasteryPoints[MAX_FORCE_RANK], ownerDraw, iMenuFont, customFont);
        break;
...
static qboolean UI_OwnerDrawVisible(uint32_t flags) {
    ...
    while (flags) {
        if (flags & UI_SHOW_FFA) {
            if (trap->Cvar_VariableValue("g_gametype") != GT_FFA && trap->Cvar_VariableValue("g_gametype") != GT_HOLOCRON &&
    ...
    if (String_Parse(args, &name)) {
        if (Q_stricmp(name, "StartServer") == 0) {
            ...
            for (i = 0; i < PLAYERS_PER_TEAM; i++) {
                ...
                if (added >= maxcl) { // this means the client filled up all their slots in the UI with bots. So stretch out an extra slot for them, and then
                                      // stop adding bots.
            ...
            trap->LAN_GetServerAddressString(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer],
        ...
        // On the solo game creation screen, we can't see siege maps
            ...
            */
            ...
            if (trap->Cvar_VariableValue("g_gametype") >= GT_TEAM) {
                trap->Cmd_ExecuteText(EXEC_APPEND, va("addbot \"%s\" %i %s\n", UI_GetBotNameByNumber(uiInfo.botIndex), (int)(uiInfo.skillIndex + 1),
            ...
            if (ui_netSource.integer != UIAS_FAVORITES) {
                ...
                if (strlen(name) > 0 && strlen(addr) > 0) {
                    ...
                    if (res == 0) {
                        ...
                        font.Paint( rect->x, rect->y, scale, newColor,
        ...
        // Save current team and class
            ...
            case 0: {
                // clansay
                trap->Cmd_ExecuteText(EXEC_APPEND, va("clansay %s\n", ui_modSayText.string));
            } break;
            case 1: {
                // tell
                trap->Cmd_ExecuteText(EXEC_APPEND, va("tell %i %s\n", uiInfo.playerIndex, ui_modSayText.string));
            } break;
            case 2: {
                // amsay
                trap->Cmd_ExecuteText(EXEC_APPEND, va("amsay %s\n", ui_modSayText.string));
            } break;
            case 3: {
                // ampsay
                trap->Cmd_ExecuteText(EXEC_APPEND, va("ampsay %i %s\n", ui_modSayAllPlayer.integer ? -1 : uiInfo.playerIndex, ui_modSayText.string));
            } break;
            }
...
static void UI_BinaryServerInsertion(int num) {
    ...
    while (mid > 0) {
        mid = len >> 1;
        //
        res = trap->LAN_CompareServers(UI_SourceForLAN(), uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir, num,
...
} serverStatusCvars[] = {{"sv_hostname", "Name"}, {"Address", ""},  {"gamename", "Game name"}, {"g_gametype", "Game type"}, {"mapname", "Map"},
                         {"version", ""},         {"protocol", ""}, {"timelimit", ""},         {"fraglimit", ""},           {NULL, NULL}};
...
static void UI_BuildFindPlayerList(qboolean force) {
                ...
                for (j = 0; j < info.numLines; j++) {
                    ...
                    if (Q_stristr(name, uiInfo.findPlayerName)) {
                        ...
                        if (uiInfo.numFoundPlayerServers < MAX_FOUNDPLAYER_SERVERS - 1) {
                            //
                            Q_strncpyz(uiInfo.foundPlayerServerAddresses[uiInfo.numFoundPlayerServers - 1], uiInfo.pendingServerStatus.server[i].adrstr,
                                       sizeof(uiInfo.foundPlayerServerAddresses[0]));
                            Q_strncpyz(uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers - 1], uiInfo.pendingServerStatus.server[i].name,
    ...
    if (i < MAX_SERVERSTATUSREQUESTS) {
        ...
        if (!uiInfo.numFoundPlayerServers) {
            Com_sprintf(uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers], sizeof(uiInfo.foundPlayerServerAddresses[0]), "no servers found");
        } else {
            trap->SE_GetStringTextString("MENUS_SERVERS_FOUNDWITH", holdSPString, sizeof(holdSPString));
            trap->Cvar_Set("ui_playerServersFound",
                           va(holdSPString, uiInfo.numFoundPlayerServers - 1, uiInfo.numFoundPlayerServers == 2 ? "" : "s", uiInfo.findPlayerName));
        }
        uiInfo.nextFindPlayerRefresh = 0;
...
static int UI_FeederCount(int feederID) {
    ...
    case FEEDER_SIEGE_CLASS_INVENTORY:
        ...
        for (i = 0; i < HI_NUM_HOLDABLE; i++) {
            ...
            if (Q_stricmp(info, "gfx/2d/select") && Q_stricmp(info, "gfx/hud/i_icon_healthdisp") && Q_stricmp(info, "gfx/hud/i_icon_ammodisp")) {
                count++;
            }
        }
        return count;
...
static const char *UI_FeederItemText(int feederID, int index, int column, qhandle_t *handle1, qhandle_t *handle2, qhandle_t *handle3) {
    ...
    *handle1 = *handle2 = *handle3 = -1;
    ...
    case FEEDER_PLAYER_SKIN_HEAD:
        if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount) {
            *handle1 = trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
...
static qhandle_t UI_FeederItemImage(int feederID, int index) {
    ...
    case FEEDER_PLAYER_SKIN_HEAD:
        if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount) {
            return trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
                                                  uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name));
        }
        break;
    ...
    if (feederID == FEEDER_Q3HEADS) {
        ...
        if (feederID == FEEDER_MAPS) {
            ...
            uiInfo.mapList[ui_currentMap.integer].cinematic =
                trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0, 0, 0, (CIN_loop | CIN_silent));
            ...
            uiInfo.mapList[ui_currentNetMap.integer].cinematic =
                trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName), 0, 0, 0, 0, (CIN_loop | CIN_silent));
        }
...
static int UI_PlayCinematic(const char *name, float x, float y, float w, float h) { return trap->CIN_PlayCinematic(name, x, y, w, h, (CIN_loop | CIN_silent)); }
...
static void UI_RunCinematicFrame(int handle) { trap->CIN_RunCinematic(handle); }
...
static void UI_BuildQ3Model_List(void) {
    ...
    for (i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS; i++, dirptr += dirlen + 1) {
        ...
        for (j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS; j++, fileptr += filelen + 1) {
            ...
            */
            check = &skinname[1];
            if ((ui_showMissingSkins.integer && bIsSkinFile(dirptr, check) && !bIsAnimFile(dirptr) && Q_stricmp(check, "menu")) || bIsImageFile(dirptr, check))
            //	if ( *dirptr )
            { // if it exists
...
void UI_Init(qboolean inGameLoad) {
...

    {
        ...
        //	trap->Cvar_Register( NULL, "ui_name", UI_Cvar_VariableString( "name" ), CVAR_INTERNAL );	//get this now, jic the menus change again
        // trying to setName before getName
...
static void UI_DisplayDownloadInfo(const char *downloadName, float centerPoint, float yStart, float scale, int iMenuFont, bool customFont) {
...
static void UI_StartServerRefresh(qboolean full) {
    ...
    trap->Cvar_Set(va("ui_lastServerRefresh_%i", ui_netSource.integer),
                   va("%s-%i, %i @ %i:%02i", GetMonthAbbrevString(q.tm_mon), q.tm_mday, 1900 + q.tm_year, q.tm_hour, q.tm_min));
