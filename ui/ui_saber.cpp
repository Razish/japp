...
static qhandle_t redSaberGlowShader, redSaberCoreShader, orangeSaberGlowShader, orangeSaberCoreShader, yellowSaberGlowShader, yellowSaberCoreShader,
    greenSaberGlowShader, greenSaberCoreShader, blueSaberGlowShader, blueSaberCoreShader, purpleSaberGlowShader, purpleSaberCoreShader, rgbSaberGlowShader,
    rgbSaberCoreShader, rgbSaberGlow2Shader, rgbSaberCore2Shader, rgbSaberTrail2Shader, rgbSaberGlow3Shader, rgbSaberCore3Shader, rgbSaberTrail3Shader,
    rgbSaberGlow4Shader, rgbSaberCore4Shader, rgbSaberTrail4Shader, rgbSaberGlow5Shader, rgbSaberCore5Shader, rgbSaberTrail5Shader, blackSaberGlowShader,
    blackSaberCoreShader, blackBlurShader, sfxSaberTrailShader, sfxSaberBladeShader, sfxSaberBlade2Shader, sfxSaberEndShader, sfxSaberEnd2Shader;
...
qboolean UI_SaberShouldDrawBlade(const char *saberName, int bladeNum) {
    ...
    if (bladeStyle2Start && bladeNum >= bladeStyle2Start) { // use second blade style
...
void UI_DoSFXSaber(vector3 *blade_muz, vector3 *blade_tip, vector3 *trail_tip, vector3 *trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx,
...
void UI_SaberDrawBlade(itemDef_t *item, char *saberName, int saberModel, saberType_e saberType, vector3 *origin, vector3 *angles, int bladeNum) {
    ...
    if (bladeColor == SABER_FLAME1 || bladeColor == SABER_ELEC1 || bladeColor == SABER_FLAME2 || bladeColor == SABER_ELEC2 || bladeColor == SABER_BLACK)
        styleToUse = 0;
    ...
    case 0:
        ...
        /*
                case 1:
                UI_DoEp1Saber( fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin, bladeLength, bladeRadius, bladeColor, 0,
           false, false, 0, snum ); break; case 2: UI_DoEp2Saber( fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin,
           bladeLength, bladeRadius, bladeColor, 0, false, false, 0, snum ); break; case 3: UI_DoEp3Saber( fx.mVerts[0].origin, fx.mVerts[1].origin,
           fx.mVerts[2].origin, fx.mVerts[3].origin, bladeLength, bladeRadius, bladeColor, 0, false, false, 0, snum ); break;
                */
    default:
    case 1:
        UI_DoSFXSaber(&fx.mVerts[0].origin, &fx.mVerts[1].origin, &fx.mVerts[2].origin, &fx.mVerts[3].origin, bladeLength, bladeRadius, bladeColor, 0, qfalse,
                      qfalse, 0, snum);
        break;
        /*
                case 5:
                UI_DoOTSaber( fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin, bladeLength, bladeRadius, bladeColor, 0,
           false, false, 0, snum ); break;
                */
    }
}
