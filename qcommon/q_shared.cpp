...
// min
float Q_cap(float value, float max) { return (value > max) ? max : value; }

int Q_capi(int value, int max) { return (value > max) ? max : value; }

// max
float Q_bump(float min, float value) { return (value < min) ? min : value; }

int Q_bumpi(int min, int value) { return (value < min) ? min : value; }
...
uint32_t Q_RoundToNextPoT(uint32_t value, uint32_t roundTo) { return (value + (roundTo - 1)) & ~(roundTo - 1); }

uint32_t Q_RoundToNearMultipleDown(uint32_t value, uint32_t roundTo) { return std::floor(value / roundTo) * roundTo; }
...
uint32_t COM_GetCurrentParseLine(void) { return com_lines; }
...
qboolean Q_isprint(int c) { return (c >= 0x20 && c <= 0x7E) ? qtrue : qfalse; }

qboolean Q_islower(int c) { return (c >= 'a' && c <= 'z') ? qtrue : qfalse; }

qboolean Q_isupper(int c) { return (c >= 'A' && c <= 'Z') ? qtrue : qfalse; }

qboolean Q_isalpha(int c) { return (Q_islower(c) || Q_isupper(c)) ? qtrue : qfalse; }
...
qboolean Q_isintegral(float f) { return ((int)f == f) ? qtrue : qfalse; }
...
bool Q_InBitflags(const uint32_t *bits, int index, uint32_t bitsPerChunk) { return !!(bits[index / bitsPerChunk] & (1 << (index % bitsPerChunk))); }
void Q_AddToBitflags(uint32_t *bits, int index, uint32_t bitsPerChunk) { bits[index / bitsPerChunk] |= (1 << (index % bitsPerChunk)); }
void Q_ToggleBitflags(uint32_t *bits, int index, uint32_t bitsPerChunk) { bits[index / bitsPerChunk] ^= (1 << (index % bitsPerChunk)); }
void Q_RemoveFromBitflags(uint32_t *bits, int index, uint32_t bitsPerChunk) { bits[index / bitsPerChunk] &= ~(1 << (index % bitsPerChunk)); }
