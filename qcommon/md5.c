...
static const void *MD5_Body( md5Context_t *ctx, const void *data, unsigned long size ) {
	const byte *ptr;
	uint32_t a, b, c, d;
...
static void MD5_Update( md5Context_t *ctx, const void *data, unsigned long size ) {
	uint32_t saved_lo;
	...
	if ( used ) {
		...
		size -= available;
		MD5_Body( ctx, ctx->buffer, CHUNK_SIZE );
	}
...
static void MD5_Final( md5Context_t *ctx, byte *result ) {
	unsigned long used, available;
...
void Q_ChecksumMD5( const char *in, size_t inLen, char out[16] ) {
	byte digest[16] = { '\0' };
	md5Context_t md5;
	int i;
