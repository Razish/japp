...
typedef enum warningLevel_e { WL_ERROR = 1, WL_WARNING, WL_VERBOSE, WL_DEBUG } warningLevel_t;
...
typedef enum ha_pref_e { h_high, h_low, h_dontcare } ha_pref;
...
typedef enum forcePowerLevels_e { FORCE_LEVEL_0, FORCE_LEVEL_1, FORCE_LEVEL_2, FORCE_LEVEL_3, NUM_FORCE_POWER_LEVELS } forcePowerLevels_t;
...
typedef struct sharedSetBoneIKStateParams_s {
    ...
    qboolean forceAnimOnBone; // normally if the bone has specified start/end frames already it will leave it alone.. if this is true, then the animation will
                              // be restarted on the bone with the specified frames anyway.
} sharedSetBoneIKStateParams_t;

enum sharedEIKMoveState_t { IKS_NONE = 0, IKS_DYNAMIC };
...
typedef enum tokenType_e { TT_NONE = 0, TT_STRING, TT_LITERAL, TT_NUMBER, TT_NAME, TT_PUNCTUATION } tokenType_t;
...
typedef enum fsMode_e { FS_READ = 0, FS_WRITE, FS_APPEND, FS_APPEND_SYNC, FS_WRITE_SYNC } fsMode_t;

typedef enum fsOrigin_e { FS_SEEK_CUR = 0, FS_SEEK_END, FS_SEEK_SET } fsOrigin_t;
...
typedef enum itemUseFail_e { SENTRY_NOROOM = 1, SENTRY_ALREADYPLACED, SHIELD_NOROOM, SEEKER_ALREADYDEPLOYED } itemUseFail_t;
...
typedef enum forceSide_e { FORCESIDE_NEUTRAL, FORCESIDE_LIGHT, FORCESIDE_DARK } forceSide_t;
...
typedef struct playerState_s {
    ...
    qboolean legsFlip; // set to opposite when the same anim needs restarting, sent over in only 1 bit. Cleaner and makes porting easier than having that god
                       // forsaken ANIM_TOGGLEBIT.
    qboolean torsoFlip;
...
typedef enum Eorientations { ORIGIN = 0, POSITIVE_X, POSITIVE_Z, POSITIVE_Y, NEGATIVE_X, NEGATIVE_Z, NEGATIVE_Y } orientations_t;
/*
Ghoul2 Insert End
*/
...
typedef struct SSkinGoreData_s {
    ...
    int fadeOutTime;      // specify the duration of fading, from the lifeTime (e.g. 3000 will start fading 3 seconds before removal and be faded entirely by
                          // removal)
...
typedef enum netadrtype_s { NA_BAD = 0, NA_BOT, NA_LOOPBACK, NA_BROADCAST, NA_IP } netadrtype_t;
